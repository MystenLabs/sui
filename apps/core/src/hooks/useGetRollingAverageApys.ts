...
// Calculate the APY and return the data for a given validator
...
//Since parsedJson is a type, we need to define the type of the object
type ParsedJson = {
...
export function useGetRollingAverageApys(numberOfValidators: number | null) {
    // Fetch the limit to the number of validators  * the rolling average
    // Order the validators in descending order so that the most recent validators are at the top
    ...
    const apyCal = useMemo<ApyByValidator | null>(() => {
        ...
        validatorEpochEvents.data.data.forEach(({ parsedJson }) => {
            // parsedJson is not a type, so we need to define the type of the object
            ...
            // If the APY is greater than 10000%, we set it to 0
