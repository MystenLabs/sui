...
import { SuiObjectResponse, getObjectType, getObjectOwner, Coin } from '@mysten/sui.js';
...
const OwnerCoins = ({ id }: { id: string }) => {
    ...
    useEffect(() => {
        rpc.getOwnedObjects({ owner: id }).then((objects) => {
            const ids: string[] = objects.data.map(obj => obj.data?.objectId ?? '')
            rpc.multiGetObjects({
                ids,
                options: {
                    showType: true,
        ...
        paginatedContent={
            <>
                <div className="flex flex-wrap">
                    {results.slice((currentPage - 1) * OBJECTS_PER_PAGE,
                        currentPage * OBJECTS_PER_PAGE).map((obj) => {
