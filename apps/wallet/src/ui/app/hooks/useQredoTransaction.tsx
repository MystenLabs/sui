...
export function useQredoTransaction() {
    const [clientIdentifier, setClientIdentifier] = useState(() => uuidV4());
    const [qredoTransactionID, setQredoTransactionID] = useState<string | null>(
        null
    );
            ...
            body={
                <div className="flex flex-col gap-2.5 text-center items-center relative">
                    ...
                    <Text variant="pBodySmall" color="steel-dark">
                        Check your Qredo app for status. Once all required
    ...
    useEffect(() => {
        function qredoTransactionCreated(
            event: QredoEventsType['qredoTransactionCreated']
        ) {
            if (event.clientIdentifier === clientIdentifier) {
                setQredoTransactionID(event.qredoTransaction.txID);
            }
        }
        function qredoTransactionComplete(
            event: QredoEventsType['qredoActionDone']
        ) {
            if (event.clientIdentifier === clientIdentifier) {
                setQredoTransactionID(null);
