...
pub struct CommitConsumerArgs {
    ...
    /// Discover the replay target from storage and pace its finalized prefix
    /// with application acknowledgements. The unfinalized tail and live
    /// consensus retain their existing flow control.
...
impl CommitConsumerArgs {
    ...
    ///
    /// This is an integration API for external consensus consumers, including
    /// Ika, which rebuilds its in-memory consensus handler state on restart.
    /// Sui uses [`Self::new`]; DO NOT REMOVE this entry point solely because it
...
///
/// External consumers, including Ika, use [`Self::progress`] and
/// [`Self::subscribe_progress`] to monitor catch-up without reading consensus
/// storage. DO NOT REMOVE these accessors solely because Sui has no callers.
/// They support the external integration described by
