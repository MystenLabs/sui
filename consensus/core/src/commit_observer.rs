...
///
/// Live output has no flow control; consensus backpressure is applied earlier at consensus
/// input level and on commit sync. Full-epoch replay can optionally be paced by consumer
/// acknowledgements during recovery.
///
...
impl CommitObserver {
    ...
    async fn recover_and_send_commits(&mut self, commit_consumer: &CommitConsumerArgs) {
        ...
        let replay_after_commit_index = match commit_consumer.replay_mode {
            ReplayMode::FromWatermark {
                replay_after_commit_index,
                ..
            } => replay_after_commit_index,
            ReplayMode::FullEpochPaced => 0,
        };
        ...
        let replay_target = if matches!(commit_consumer.replay_mode, ReplayMode::FullEpochPaced) {
            ...
            .step_by(COMMIT_RECOVERY_BATCH_SIZE as usize)
        {
            ...
            for commit in unsent_commits.into_iter() {
                ...
                if matches!(commit_consumer.replay_mode, ReplayMode::FullEpochPaced)
                    && self.context.protocol_config.transaction_voting_enabled()
                    && committed_sub_dag.commit_ref.index <= replay_target
                {
                    // Finalized commits must have rejection metadata, even if empty.
                    // Without it, finalization may need later commits while replay
                    // waits for an acknowledgement, blocking startup.
                    assert!(
...
mod tests {
    ...
    fn full_replay_fixture(
        ...
        config.set_transaction_voting_enabled_for_testing(voting);
    ...
    async fn supplied_watermark_replay_keeps_its_target_without_waiting_for_acknowledgements() {
        ...
        assert_eq!(monitor.replay_target(), Some(4));
    ...
    async fn full_replay_is_paced_by_applied_commits_even_without_finalized_rows() {
        ...
        let stored_head_round = dag_state.read().last_commit_round();
        ...
        for index in 1..=3 {
            ...
            assert_eq!(monitor.progress().highest_committed_index, 7);
            assert_eq!(
                monitor.progress().highest_committed_round,
                stored_head_round
            );
            if index < 3 {
    ...
    #[case(3)]
    #[tokio::test]
    async fn full_replay_does_not_wait_for_an_unfinalized_tail(#[case] finalized: u32) {
        let (context, dag_state, tracker) = full_replay_fixture(7, finalized, true);
        let (args, mut receiver) = CommitConsumerArgs::new_with_full_replay();
        let monitor = args.monitor();
        let recovery = tokio::spawn(CommitObserver::new(context, args, dag_state, tracker));
        for index in 1..=finalized {
            let commit = timeout(Duration::from_secs(5), receiver.recv())
                .await
                .unwrap()
                .unwrap();
        ...
        let mut observer = timeout(Duration::from_secs(5), recovery)
            ...
            .unwrap();
        assert_eq!(monitor.replay_target(), Some(finalized));
        assert_eq!(monitor.highest_handled_commit(), finalized);
        monitor
            .replay_to_consumer_last_processed_commit_complete()
            .await;
        observer.stop().await;
    ...
    async fn full_replay_of_an_empty_store_publishes_an_empty_target() {
        ...
        assert_eq!(monitor.replay_target(), Some(0));
        timeout(
            Duration::from_secs(1),
            monitor.replay_to_consumer_last_processed_commit_complete(),
        )
