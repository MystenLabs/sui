...
impl CommitObserver {
    ...
    async fn recover_and_send_commits(&mut self, commit_consumer: &CommitConsumerArgs) {
        ...
        let mut replay_after_commit_index = commit_consumer.replay_after_commit_index;
        ...
        // If last_commit_index is 0, the logic should have returned earlier.
        replay_after_commit_index = replay_after_commit_index.min(last_commit_index.saturating_sub(1));
    ...
    fn truncate_bad_finalized_commits(&mut self) {
        ...
        if interested_commits.len() == 1 {
            ...
            {
                info!(
                    "Skipping truncation. Rejected transactions: {:?}",
                    rejected
                );
                return;
            }
