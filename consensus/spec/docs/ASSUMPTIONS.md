...
- **Status:** Partially verified.
- **Effect if false:** Safety.
- **Lean use:** Current safety lemmas use this condition. `V3AdaptiveScheduleRule.consistent_runs_are_equal` derives the common chain for the adaptive v3 schedule. `V3FlexScheduleHost.consistentRun` now derives consistency for one modeled host. It uses separate local facts for schedule initialization, the no-commit step, the `add_commit` step, the selected order, and each post-scan slot. It does not assume `ConsistentRun`. The remaining refinement must instantiate these local fields from Rust. It must map the full pending-prefix scan to final per-round verdicts. It must then show that local execution, certified commit installation, and restart give every correct host the same `commitMaterial` and `postScanSlots` functions. The gate is no longer such a function: `V3ScheduleGateSequence.derivedGoverningIndex` derives the governing commit index from the schedule states. `committed_candidates_agree_across_hosts` states the cross-host step at the level the protocol supports. Two hosts agree on the installed candidates when their selected slots agree and neither host is ahead of the other in deciding. Full verdict equality is not the target, because the round scan stops at the first undecided slot and a lagging host returns no candidate. The open condition is `CrossViewExactSlotAgreement` for each selected slot, which is the named cross-view safety goal.
