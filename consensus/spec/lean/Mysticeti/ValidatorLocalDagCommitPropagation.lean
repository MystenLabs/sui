...
/-- One-host source rules for a local Flex scan at a GC cutoff.

The exact snapshot prior supplies the opaque committed prefix. The causal walk
reads only accepted blocks above the local cutoff. Pending decision evidence and
new commit material are also strictly above that cutoff. Therefore a parent at
or below the cutoff supplies no new vote, anchor, or material item. -/
structure ValidatorLocalGcCutoffCausalSourceMap
    {BlockId CommitId History Encoding PacketId LocalView : Type}
    [DecidableEq BlockId]
    {config : ValidatorEpochConfig CommitId}
    {faults : FixedFaultInterval config}
    {protocolPacket :
    ...
    (causal : ExactAnchorCausalDataMap LocalView BlockId) where
  viewAt : Time → Nat → LocalView
  /-- Accepted above-GC parent closure is complete for the exact local anchor
  traversal. The traversal does not request a body for a stopped root. -/
  acceptedAboveGcCompletesAnchor : ∀ time observer anchor,
    let state := (timed.execution.trace time).validatorState observer
    state.gcRound < anchor.round →
    ValidatorAcceptedCausalClosureAboveRound (timed.execution.trace time)
      observer state.gcRound anchor →
    causal.complete (viewAt time observer) anchor.toLeaderBlockRef
  /-- Each pending round that can supply a vote or anchor is above the local GC
  cutoff. -/
  pendingRoundsAboveGc : ∀ time observer index,
    index < (source.snapshot observer
      ((timed.execution.trace time).validatorState observer)).pending.roundCount →
    ((timed.execution.trace time).validatorState observer).gcRound <
      ((source.snapshot observer
        ((timed.execution.trace time).validatorState observer)).pending.rounds
          index).round
  /-- A returned candidate adds only material above the local GC cutoff. The
  exact prior digest supplies the older committed prefix. -/
  returnedCandidateMaterialAboveGc : ∀ time observer candidate,
    ReferenceFlexCandidateReturned
        (context observer
          ((timed.execution.trace time).validatorState observer))
        (source.snapshot observer
          ((timed.execution.trace time).validatorState observer)) candidate →
    ∀ block,
      block ∈ ((source.snapshot observer
        ((timed.execution.trace time).validatorState observer)).materialize
          candidate).sortedCommittedBlocks →
      ((timed.execution.trace time).validatorState observer).gcRound < block.round

namespace ValidatorLocalGcCutoffCausalSourceMap

/-- Every selected slot in one pending round is strictly above the local GC
cutoff. -/
theorem pending_selected_slot_above_gc
    {BlockId CommitId History Encoding PacketId LocalView : Type}
    [DecidableEq BlockId]
    {config : ValidatorEpochConfig CommitId}
    {faults : FixedFaultInterval config}
    ...
    {timed : ValidatorBoundedExecution (PacketId := PacketId) config faults
      protocolPacket network program}
    {source : LocalFlexCommitterSourceMap config functions context program}
    {causal : ExactAnchorCausalDataMap LocalView BlockId}
    (cutoff : ValidatorLocalGcCutoffCausalSourceMap
      (PacketId := PacketId) (faults := faults)
      (protocolPacket := protocolPacket) (network := network) (timed := timed)
      source causal)
    {time observer index : Nat}
    {slot : ReferenceSelectedSlotView BlockId}
    (indexInRange : index < (source.snapshot observer
      ((timed.execution.trace time).validatorState observer)).pending.roundCount)
    (slotIncluded : slot ∈ ((source.snapshot observer
      ((timed.execution.trace time).validatorState observer)).pending.rounds
        index).selectedSlots) :
    ((timed.execution.trace time).validatorState observer).gcRound <
      slot.slot.round := by
  have roundAbove := cutoff.pendingRoundsAboveGc time observer index indexInRange
  have slotRound := (source.tryCommitStateValid observer
    ((timed.execution.trace time).validatorState observer)).selectedSlotRoundsMatch
      index indexInRange slot slotIncluded
  omega

end ValidatorLocalGcCutoffCausalSourceMap
