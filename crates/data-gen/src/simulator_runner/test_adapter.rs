...
pub struct DataGenAdapter<'a> {
    pub(crate) executor: Simulacrum<StdRng>,
...
#[async_trait]
impl<'a> MoveTestAdapter<'a> for DataGenAdapter<'a> {
    ...
    async fn init(
        ...
        let executor = Simulacrum::new_with_rng(rng);
    ...
    async fn handle_subcommand(
        ...
        match command {
            ...
            SuiSubcommand::Serve(ServeCommand { url, port }) => {
                let addr: std::net::SocketAddr = format!("{}:{}", url, port).parse().unwrap();
                sui_rest_api::start_service(
                    addr,
                    std::sync::Arc::new(self.executor.store().clone()),
                    Some("/rest".to_owned()),
                )
                .await;
                Ok(Some(format!("Server running at {}", addr)))
            }
...
impl<'a> DataGenAdapter<'a> {
    ...
    async fn execute_txn(&mut self, transaction: Transaction) -> anyhow::Result<TxnSummary> {
        ...
        let effects = self.executor.execute_transaction(transaction)?;
        let _digest = effects.transaction_digest();
        // todo: capture exec error here
        let error_opt: Option<ExecutionError> = None;
        ...
        match effects.status() {
            ExecutionStatus::Success { .. } => {
                // todo: add support for events fetching
                let events = vec![];
                // let events = self
                //     .executor
                //     .query_events(
...
impl<'a> GetModule for &'a DataGenAdapter<'_> {
