...
use move_package_analyzer::{
    errors::PackageAnalyzerError, load_config, load_from_csv::load_csv,
    load_from_dir::load_from_directory, model::loader::build_environment,
    passes::pass_manager::run, query_indexer::query_packages,
};
...
#[clap(group(ArgGroup::new("input").required(true).args(&["db_url", "packages_dir", "csv_dir"])))]
struct Args {
    ...
    /// Path to a directory containing packages in a csv file as dumped from snowflakes
    /// `move_package_parquet`
    #[clap(long = "csv")]
...
fn main() -> Result<(), PackageAnalyzerError> {
    ...
    } else if let Some(packages_dir) = args.packages_dir {
        load_from_directory(packages_dir)?
    } else {
        load_csv(args.csv_dir.expect("a loading dir or db arg must exist"))?
    };
