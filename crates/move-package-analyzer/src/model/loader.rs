...
pub fn build_environment(packages: Vec<MovePackage>) -> GlobalEnv {
    ...
    let packages = clean_up_framework(packages);
    ...
    load_versions(&mut packages, &package_map, &modules); // discover package versions
    let (mut structs, struct_map) = load_structs(&mut identifier_map, &mut modules, &packages);
    module_dependencies(&packages, &mut modules);
...
impl TypeBuilder {
    fn make_type(&self, module: &Module, type_: &SignatureToken) -> Type {
        match type_ {
            ...
            SignatureToken::StructInstantiation(str_inst) => {
                let (struct_handle_idx, type_arguments) = &**str_inst;
                let idx = self
                    .get_struct_idx(module, *struct_handle_idx)
                    .unwrap_or_else(|| {
                        panic!(
                ...
                let type_arguments = type_arguments
                    ...
                    .collect::<Vec<_>>();
                Type::StructInstantiation(Box::new((idx, type_arguments)))
            }
    ...
    // Given a StructHandle return the index of the `Struct` in the `GlobalEnv`.
    // The index is that of where the type is first defined, where the type is introduced.
    fn get_struct_idx(
        &self,
        module: &Module,
        struct_handle_idx: StructHandleIndex,
    ) -> Option<StructIndex> {
        let module_package = &self.packages[module.package];
        ...
        // get the `PackageIndex` of the struct
        let struct_pkg_id = ObjectID::from(get_package_from_struct_handle(
            compiled_module,
            struct_handle_idx,
        ));
        let struct_pkg_idx = *self.package_map.get(&struct_pkg_id).unwrap_or_else(|| {
            panic!(
                "Failure getting the package index for {} in {}",
                struct_pkg_id, module_package.id,
            )
        });

        // if the package is in the dependencies list load that package,
        // otherwise use the current package
        let struct_package = match module_package.linkage_table.get(&struct_pkg_idx) {
            None => module_package,
            Some(pkg_idx) => &self.packages[*pkg_idx],
        };

        // get the package where the type is defined via the type origin table
        // of the package referenced by the struct
        let key = (module_name.to_string(), struct_name.to_string());
        let package_id = struct_package.type_origin.get(&key).unwrap_or_else(|| {
            panic!(
                "Failure getting type {}::{} in type origin table for package {}",
                module_name, struct_name, struct_package.id,
            )
        });

        // load the struct index of the original type definition
        let struct_key = format!("{}::{}::{}", package_id, module_name, struct_name);
        self.struct_map.get(&struct_key).copied()
    }
}

// Keep only latest framework and remove older versions of framework packages
fn clean_up_framework(
    packages: Vec<MovePackage>,
) -> Vec<MovePackage> {
    let mut framework_latest = BTreeMap::new();
    packages
        .iter()
        .filter(|package| FRAMEWORK.contains(&package.id()))
        .for_each(|package| {
            let version = framework_latest.entry(package.id()).or_insert(0u64);
    ...
    packages
        .into_iter()
        .filter(|package| {
            !FRAMEWORK.contains(&package.id())
                || framework_latest.get(&package.id()) == Some(&package.version().value())
        })
        .collect()
}

// Load all packages and return a vector of `Package`'s and a map of package id to `PackageIndex`'s.
// Return: all packages, map from package idx to package id, framework map
fn load_packages(
    packages: Vec<MovePackage>,
) -> (
    Vec<Package>,
    BTreeMap<ObjectID, PackageIndex>,
    BTreeMap<PackageIndex, ObjectID>,
) {
    let mut packages = packages
        ...
        .collect::<Vec<_>>();
    // load the `self_idx` which is the position in the vector
    packages.iter_mut().enumerate().for_each(|(idx, package)| {
        package.self_idx = idx;
    });

    // load the framework packages
    let framework = packages
        .iter()
        .filter_map(|package| {
            if FRAMEWORK.contains(&package.id) {
                Some((package.self_idx, package.id))
            } else {
                None
            }
        })
    ...
    let package_map = packages
        .iter()
        .map(|package| (package.id, package.self_idx))
        .collect::<BTreeMap<_, _>>();
    ...
    packages.iter_mut().for_each(|package| {
        ...
        package.linkage_table = move_package
            .linkage_table()
            .iter()
            .map(|(base, version)| {
            ...
            .collect::<BTreeMap<_, _>>();
        package.dependencies = package.linkage_table.values().copied().collect();
    });
...
fn load_modules(
    ...
    modules
        .iter_mut()
        .enumerate()
        .for_each(|(idx, module)| module.self_idx = idx);
    ...
    let module_map = modules
        ...
        .map(|module| {
            ...
            package.modules.push(module.self_idx);
            let package_id = package.id;
...
fn module_dependencies(packages: &[Package], modules: &mut [Module]) {
    packages.iter().for_each(|package| {
        let root = match package.root_version {
            None => package,
            Some(root_idx) => &packages[root_idx],
        };
        let versions = root
            .versions
            .iter()
            .map(|pkg_idx| packages[*pkg_idx].id)
            .collect::<BTreeSet<_>>();
        package.modules.iter().for_each(|module_idx| {
            ...
            let deps: BTreeSet<_> = compiled_module
                .module_handles
                .iter()
                .filter(|module_handle| *module_handle != compiled_module.self_handle())
                .map(|module_handle| {
                    ObjectID::from(
                ...
                .collect();
            assert_eq!(
                versions.intersection(&deps).count(),
                0,
                "dependency on previous package version for module {} in package {}",
    ...
    packages.iter_mut().for_each(|package| {
        ...
        package.modules.iter().for_each(|module_idx| {
            ...
            module.dependencies.iter().for_each(|dep| {
                let package_idx = package_map[dep];
                if package_idx != package.self_idx && Some(package_idx) != package.root_version {
                    let dep_idx = *package.linkage_table.get(&package_idx).unwrap_or_else(|| {
                        panic!(
                            "missing dependency for package {} on package {}",
                            package.id, dep,
                        )
                    });
                    direct_deps.insert(dep_idx);
                }
            });
        });
        package.direct_dependencies = direct_deps;
        assert!(
            package
                .direct_dependencies
                .iter()
                .all(|dep| package.dependencies.contains(dep)),
            "missing direct dependency for package {}",
            package.id,
        );
    });
}
...
fn load_versions(
    ...
    packages.iter().for_each(|package| {
        check_package(package, modules); // all modules have the same address/package id
        let package_id = package.id;
        // get the package id from the first module in the packagex.
        // Modules are compiled with the original package id, so when versioned
        // the package id and the address of the module will be different!
        let module = &modules[package.modules[0]];
        let origin = ObjectID::from(*module.module_id.address());
        if origin != package_id {
            // it's a versioned package (version > 1)
            let value = versions.entry(origin).or_insert(vec![]);
            value.push(package.self_idx);
        } else {
            // it's a package at version 1 (root), framework packages are at some version,
            // but they are not versioned packages (we overwrite them).
            // We do NOT push the root, but we create an entry in the map
            if !FRAMEWORK.contains(&package_id) {
                assert_eq!(
                    package.version, 1,
                    "package {} is not version 1",
                    package_id
                );
            }
    ...
    // sort packages according to version
    versions.iter_mut().for_each(|(_, v)| {
        v.sort_by(|pkg1, pkg2| packages[*pkg1].version.cmp(&packages[*pkg2].version))
    });
    // set the root version for all versioned packages
    versions.iter().for_each(|(id, pkg_versions)| {
        let root_idx = package_map[id];
        pkg_versions.iter().for_each(|pkg_idx| {
            packages[*pkg_idx].root_version = Some(root_idx);
        });
    });
    // set the list of versioned packages in the root package
    versions.into_iter().for_each(|(id, pkg_versions)| {
...
// Structs are keyed off the package they live in: <package_id>::<module_name>::<struct_name>
...
fn load_structs(
    ...
    let mut structs = modules
        ...
        .collect::<Vec<_>>();
    // load the `self_idx` which is the position in the vector
    structs.iter_mut().enumerate().for_each(|(idx, struct_)| {
    ...
    let struct_map = structs
        .iter()
        .map(|struct_| {
            let module = &mut modules[struct_.module];
            module.structs.push(struct_.self_idx);
...
fn load_functions(
    ...
    let function_map = functions
        ...
        .map(|function| {
            ...
            module.functions.push(function.self_idx);
            let compiled_module = module.module.as_ref().unwrap();
        ...
        if let Some(code_unit) = func_def.code.as_ref() {
            ...
            let code: Vec<Bytecode> = code_unit
                ...
                .map(|bytecode| match bytecode {
                    ...
                    MoveBytecode::LdU128(v) => Bytecode::LdU128(v.clone()),
                    MoveBytecode::LdU256(v) => Bytecode::LdU256(v.clone()),
...
// Build 2 maps from each function to the callers and to the function called (callees)
...
// All versioned packages (loaded from `versions` of the root) must have a `root_version` that
