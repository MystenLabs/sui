...
pub(crate) fn run(env: &GlobalEnv, output: &Path) {
    let file = &mut File::create(output.join("summary.txt"))
        .unwrap_or_else(|_| panic!("Unable to create file summary.txt in {}", output.display()));
    write_to!(file, "Environment Summary");
    package_summary(env, file);
    type_summary(env, file);
    function_summary(env, file);
}

//
// Package summary
//
fn package_summary(env: &GlobalEnv, file: &mut File) {
...
// Type summary
//
fn type_summary(env: &GlobalEnv, file: &mut File) {
...
// Function summary
//
fn function_summary(env: &GlobalEnv, file: &mut File) {
    ...
    count_calls(env, file);
    count_bytecodes(env, file);
}

fn count_calls(env: &GlobalEnv, file: &mut File) {
...
/// Count bytecodes
fn count_bytecodes(env: &GlobalEnv, file: &mut File) {
    ...
    write_to!(file, "** bytecodes: {count}");
    let mut entries: Vec<_> = bytecodes.into_iter().collect();
    entries.sort_by_key(|&(_, count)| count);
    write_to!(file, "\t{:<25}{:>8}", "Bytecode", "Count");
    for (name, count) in entries.iter().rev() {
        write_to!(file, "\t{:<25}{:>8}", name, count);
    }
}
