...
impl<T: std::marker::Sized + serde::Serialize, W: Write> StructuredLog<T, W> {
...
mod tests {
    ...
    async fn test_structured_log_writer_reader() {
        ...
        let mut writer = StructuredLog::new(&mut buffer);
    ...
    async fn test_structured_log_empty() {
        let mut buffer = Vec::new();
        let _writer = StructuredLog::<TestStruct, _>::new(&mut buffer);
