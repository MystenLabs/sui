...
use crate::{join_paths, FileMetadata, MaxCheckpointReader, ParquetSchema, Task, EPOCH_DIR_PREFIX};
...
impl<S: Serialize + ParquetSchema + 'static> Worker for AnalyticsProcessor<S> {
    ...
    async fn process_checkpoint(&self, checkpoint_data: &CheckpointData) -> Result<()> {
        ...
        let cut_new_files = (num_checkpoints_processed >= self.task.config.checkpoint_interval)
            || (state.last_commit_instant.elapsed().as_secs() > self.task.config.time_interval_s)
            || (state.num_checkpoint_iterations % CHECK_FILE_SIZE_ITERATION_CYCLE == 0
                && state.writer.file_size()?.unwrap_or(0)
                    > self.task.config.max_file_size_mb * 1024 * 1024);
        if cut_new_files {
            self.cut(&mut state).await?;
            self.reset(&mut state)?;
        }
...
impl<S: Serialize + ParquetSchema + 'static> AnalyticsProcessor<S> {
    pub async fn new(
        ...
        let local_store_config = ObjectStoreConfig {
            directory: Some(task.checkpoint_dir_path().to_path_buf()),
            object_store: Some(ObjectStoreType::File),
            ..Default::default()
        };
        let local_object_store = local_store_config.make()?;
        let remote_object_store = task.job_config.remote_store_config.make()?;
        let (kill_sender, kill_receiver) = oneshot::channel::<()>();
        let (sender, receiver) = mpsc::channel::<FileMetadata>(100);
        let name: String = handler.name().parse()?;
        let checkpoint_dir = task.checkpoint_dir_path();
        let cloned_metrics = task.metrics.clone();
        tokio::task::spawn(Self::start_syncing_with_remote(
            remote_object_store,
            local_object_store.clone(),
            checkpoint_dir.to_path_buf(),
            task.config.remote_store_path_prefix()?,
            receiver,
            kill_receiver,
            cloned_metrics,
            name.clone(),
    ...
    async fn cut(&self, state: &mut State<S>) -> anyhow::Result<()> {
            ...
            && state.writer.flush(state.current_checkpoint_range.end)?
        {
            let file_metadata = FileMetadata::new(
                self.task.config.file_type,
                self.task.config.file_format,
                state.current_epoch,
    ...
    fn epoch_dir(&self, state: &State<S>) -> Result<PathBuf> {
        let path = path_to_filesystem(
            self.task.checkpoint_dir_path().to_path_buf(),
            &self.task.config.file_type.dir_prefix(),
        )?
