...
struct State<S: Serialize + ParquetSchema + Send + Sync> {
    ...
    buffered_checkpoints: BTreeMap<u64, Box<dyn Iterator<Item = S> + Send + Sync>>,
}
...
impl<S: Serialize + ParquetSchema + Send + Sync + 'static> Worker for AnalyticsProcessor<S> {
    ...
    async fn process_checkpoint(&self, checkpoint_data: Arc<CheckpointData>) -> Result<()> {
        ...
        info!("Received checkpoint {checkpoint_num}, epoch {epoch}, timestamp {timestamp}");

        let row_iterator = self.handler.process_checkpoint(checkpoint_data).await?;

        let mut state = self.state.lock().await;

        // Buffer the checkpoint result
        state
            .buffered_checkpoints
            .insert(checkpoint_num, row_iterator);

        // Process buffered checkpoints in order
        while let Some(iterator) = {
            let next_checkpoint = state.current_checkpoint_range.end;
            state.buffered_checkpoints.remove(&next_checkpoint)
        } {
            ...
            state.writer.write(iterator)?;
            state.current_checkpoint_range.end += 1;
...
// Existing implementation methods remain unchanged (cut, reset, emit_file_size_metric, etc.).
...
impl<S: Serialize + ParquetSchema + Send + Sync + 'static> AnalyticsProcessor<S> {
    pub async fn new(
        ...
        // initialization unchanged, add buffered_checkpoints
        let state = State {
            current_epoch: 0,
    ...
    // keep existing private helper methods unchanged
