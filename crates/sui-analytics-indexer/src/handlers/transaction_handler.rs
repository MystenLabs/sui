...
impl Handler for TransactionHandler {
    ...
    type Batch = Vec<TransactionEntry>;
    ...
    async fn commit<'a>(
        ...
        let mut writer: ParquetWriter =
            ParquetWriter::new(temp_dir.path(), FileType::Transaction, first_checkpoint)?;

        // Collect into a vec to satisfy 'static lifetime requirement
        let rows: Vec<TransactionEntry> = batch.to_vec();
        AnalyticsWriter::<TransactionEntry>::write(&mut writer, Box::new(rows.into_iter()))?;
        AnalyticsWriter::<TransactionEntry>::flush(&mut writer, last_checkpoint + 1)?;

        // Build the object store path
        let file_path = FileType::Transaction.file_path(
        ...
        let local_file = temp_dir
            .path()
            .join(FileType::Transaction.dir_prefix().as_ref())
