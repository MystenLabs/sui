...
fn main() {
    ...
    let gas_cloned = gas.clone();

    // Setup the network
    let configs = Arc::new(test_and_configure_authority_configs(opts.committee_size));
    ...
    let _ = std::thread::spawn(move || {
        server_runtime.block_on(async move {
            // Setup the network
            let nodes: Vec<SuiNode> = spawn_test_authorities(gas_cloned, &configs).await;
            cloned_barrier.wait();
    ...
    client_runtime.block_on(async move {
        let clients = test_authority_aggregator(&cloned_configs);
        let quorum_driver_handler = QuorumDriverHandler::new(clients.clone());

        // publish package
        write("Publishing basics package".to_string());
        let package_ref = publish_counter_package(publish_module_gas, cloned_configs.validator_set()).await;
        gas.pop();
