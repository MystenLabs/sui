                    ...
                    if payload.is_batched() {
                        ...
                        let res = async move {
                            let bundle_results = if use_direct_batch_submission {
                                submit_transactions_directly(proxy, txs).await
                            } else {
                                submit_soft_bundles(proxy, txs, max_bundles, max_bundle_size).await
                            };
                            let latency = start.elapsed();
...
/// Submits every transaction in the batch independently through the normal transaction path,
/// concurrently. Each transaction is its own single-item result, so conflicting transactions
/// race against each other rather than being grouped into a bundle.
async fn submit_transactions_directly(
    proxy: Box<dyn ValidatorProxy + Send + Sync>,
    txs: Vec<Transaction>,
) -> BundleResults {
            ...
            async move {
                ...
                let result = proxy
                    ...
                    .unwrap_or_else(|error| {
                        vec![(
                            digest,
                            BundleItemResponse::DirectFailure(direct_submission_error_status(
                                error,
                            )),
                        )]
                    });
...
/// Partitions the batch into randomly sized soft bundles and submits each bundle concurrently.
/// Single-transaction bundles are randomly submitted through the normal transaction path instead
/// to exercise both code paths.
async fn submit_soft_bundles(
    proxy: Box<dyn ValidatorProxy + Send + Sync>,
    txs: Vec<Transaction>,
    max_bundles: NonZeroUsize,
    max_bundle_size: usize,
    ...
    let bundle_futures: Vec<_> = bundles
        ...
        .map(|(start_idx, bundle_txs)| {
            ...
            async move {
                ...
                let result = if bundle_txs.len() == 1 && rand::thread_rng().gen_bool(0.5) {
                    let tx = bundle_txs.into_iter().next().unwrap();
                    let digest = *tx.digest();
                    let exec_result = proxy.execute_transaction_block(tx).await;
