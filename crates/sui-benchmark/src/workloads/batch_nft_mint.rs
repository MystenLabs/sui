...
#[derive(Debug)]
pub struct BatchNFTMintTestPayload {
    package_id: ObjectID,
    num_recipients: u64,
...
impl std::fmt::Display for BatchNFTMintTestPayload {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "mint payload")
    }
}

impl Payload for BatchNFTMintTestPayload {
    ...
    fn make_transaction(&mut self) -> Transaction {
        ...
        TestTransactionBuilder::new(self.gas.1, self.gas.0, rgp)
            .call_nft_batch_mint(
                ...
                generate_recipients(self.num_recipients),
            )
...
#[derive(Debug)]
pub struct BatchNFTMintWorkloadBuilder {
    num_payloads: u64,
    nft_contents_size: u64,
    num_recipients: u64,
    rgp: u64,
...
impl BatchNFTMintWorkloadBuilder {
    pub fn from(
        workload_weight: f32,
        target_qps: u64,
        num_workers: u64,
        ...
        let workload_builder =
            Box::<dyn WorkloadBuilder<dyn Payload>>::from(Box::new(BatchNFTMintWorkloadBuilder {
...
#[async_trait]
impl WorkloadBuilder<dyn Payload> for BatchNFTMintWorkloadBuilder {
    ...
    async fn build(
        ...
        Box::<dyn Workload<dyn Payload>>::from(Box::new(BatchNFTMintWorkload {
...
#[derive(Debug)]
pub struct BatchNFTMintWorkload {
...
#[async_trait]
impl Workload<dyn Payload> for BatchNFTMintWorkload {
    async fn init(
        &mut self,
    ...
    async fn make_test_payloads(
        ...
        for g in self.payload_gas.iter() {
            mint_payloads.push(Box::new(BatchNFTMintTestPayload {
...
#[memoize]
fn generate_recipients(count: u64) -> Vec<SuiAddress> {
