...
pub enum FailurePayloadType {
    InvalidSignature,
    // NonexistentObject,
}
impl Copy for FailurePayloadType {}

impl TryFrom<u32> for FailurePayloadType {
    type Error = anyhow::Error;

    fn try_from(value: u32) -> Result<Self, Self::Error> {
        match value {
            0 => Ok(rand::random()),
            _ => FailurePayloadType::iter()
                .nth(value as usize)
                .ok_or_else(|| {
                    anyhow!(
                        "Invalid failure workload specifier. Valid options are 0 to {}",
                        FailurePayloadType::COUNT
                    )
                }),
        }
    }
...
impl FromStr for FailurePayloadType {
    type Err = anyhow::Error;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let v = u32::from_str(s).map(FailurePayloadType::try_from);
...
impl Distribution<FailurePayloadType> for Standard {
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> FailurePayloadType {
        // Exclude the "Random" variant
        let n = rng.gen_range(1..FailurePayloadType::COUNT);
        FailurePayloadType::iter().nth(n).unwrap()
    }
}

#[derive(Debug)]
pub struct FailureTestPayload {
...
impl Payload for FailureTestPayload {
...
impl std::fmt::Display for FailureTestPayload {
...
#[derive(Debug)]
pub struct FailureWorkloadBuilder {
    num_transfer_accounts: u64,
    num_payloads: u64,
}

impl FailureWorkloadBuilder {
    pub fn from(
        workload_weight: f32,
        target_qps: u64,
        num_workers: u64,
        ...
        if max_ops == 0 || num_workers == 0 {
            ...
            let workload_builder =
                Box::<dyn WorkloadBuilder<dyn Payload>>::from(Box::new(FailureWorkloadBuilder {
...
#[async_trait]
impl WorkloadBuilder<dyn Payload> for FailureWorkloadBuilder {
    ...
    async fn build(
        ...
        Box::<dyn Workload<dyn Payload>>::from(Box::new(FailureWorkload {
...
#[async_trait]
impl Workload<dyn Payload> for FailureWorkload {
    async fn init(
        &mut self,
