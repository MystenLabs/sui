                ...
                match event {
                    EthSuiBridgeEvents::TokensBridgedToSuiFilter(event) => {
                        let bridge_event = match EthToSuiTokenBridgeV1::try_from(&event) {
                            ...
                            Err(e) => {
                                tracing::error!(?eth_tx_hash, eth_event_index, "Failed to convert TokensBridgedToSui log to EthToSuiTokenBridgeV1. This indicates a bug in the code: {:?}. Err: {:?}", event, e);
                                return None;
                            }
                        };
