    ...
    P: JsonRpcClient,
{
    ...
    pub async fn get_finalized_bridge_action_maybe(
        ...
        let last_finalzied_block_id = self.get_last_finalized_block_id().await?;
        if receipt_block_num.as_u64() > last_finalzied_block_id {
        ...
        bridge_event
            ...
            .ok_or(BridgeError::BridgeEventNotAction)
    }
    ...
    async fn get_log_tx_details(&self, log: ethers::types::Log) -> BridgeResult<EthLog> {
        ...
        let log_index_in_tx = log_index_in_tx.ok_or(BridgeError::ProviderError(format!(
            "Couldn't find matched log {:?} in transaction {}",
            log, tx_hash
        )))?;
