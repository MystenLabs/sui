        ...
        while let Some((identifier, events)) = sui_events_rx.recv().await {
            ...
            for (sui_event, opt_bridge_event) in events.iter().zip(bridge_events) {
                ...
                if let SuiBridgeEvent::NewTokenEvent(e) = &bridge_event {
                    if let std::collections::hash_map::Entry::Vacant(entry) = latest_token_config.entry(e.token_id) {
...
mod tests {
    use crate::{
        events::{init_all_struct_tags, NewTokenEvent}, test_utils::{get_test_eth_to_sui_bridge_action, get_test_log_and_action}, types::BridgeActionDigest
    };
    use ethers::{
        abi::Hash,
    ...
    async fn test_sui_watcher_task() {
        ...
        let (token_type_tags_tx, token_type_tags_rx) = tokio::sync::watch::channel(HashMap::new());
        // start orchestrator
    ...
    async fn test_sui_watcher_task_add_new_token() {
        ...
        let (executor, executor_requested_action_rx) = MockExecutor::new();
        let (token_type_tags_tx, mut token_type_tags_rx) = tokio::sync::watch::channel(HashMap::new());
        // start orchestrator
        let _handles = BridgeOrchestrator::new(
            Arc::new(sui_client),
            sui_events_rx,
