...
/// Return the IP address of the current node handle that the thread is running on. This is currently
/// only used for spawning fullnodes in simtest in an adhoc way, where the fullnode isn't wrapped
/// inside a container and hence doesn't get its own IP address. It will just share the IP address
/// of the node handle that it's running on.
/// TODO: Move all fullnodes in simtest to run inside container with a unique IP.
#[cfg(msim)]
pub fn get_current_ip_for_testing() -> String {
    let node = msim::runtime::NodeHandle::current();
    node.ip()
        .expect("Current node should have an IP")
        .to_string()
}

/// In non-simtest, the current IP is always localhost.
#[cfg(not(msim))]
pub fn get_current_ip_for_testing() -> String {
    localhost_for_testing()
}
