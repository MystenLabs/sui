...
//! Prometheus metrics for the formal-snapshot
//! [`RestoreSource`](super::RestoreSource).
//!
//! Adapted from
//! `sui-indexer-alt-consistent-store::restore::metrics`. The
//! semantics match: "partitions" are per-bucket files; "objects"
//! are decoded live objects. Per-pipeline counters are dropped
//! because the [`RestoreDriver`](super::RestoreDriver) — not the
//! source — knows which pipelines see each object now.
