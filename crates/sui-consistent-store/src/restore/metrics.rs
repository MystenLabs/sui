...
/// cursors, so [`shards_done`](Self::shards_done) reflects cumulative
/// progress across a crash and resume — distinct from
/// [`FormalSnapshotMetrics::total_partitions_fetched`], which counts
/// only the current process's fetches.
