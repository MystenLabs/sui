...
//! Byte-blob storage abstraction used by
//! [`FormalSnapshot`](super::formal_snapshot::FormalSnapshot).
//!
//! Two implementations live here: a blanket [`Storage`] over
//! anything that implements [`object_store::ObjectStore`] (S3,
//! GCS, Azure, local filesystem) and a [`HttpStorage`] that
//! issues plain HTTP `GET`s for snapshot files served by an
//! arbitrary HTTP endpoint.
//!
//! Ported from
//! `sui-indexer-alt-consistent-store::restore::storage` and kept
//! crate-private since only the formal-snapshot source needs it.
...
/// Read-only byte-blob storage keyed by [`Path`].
#[async_trait::async_trait]
pub(super) trait Storage {
    /// Fetch the file at `path`.
    async fn get(&self, path: Path) -> anyhow::Result<Bytes>;
}

/// Knobs shared by every snapshot-store backend.
#[derive(clap::Args, Clone, Debug, Default)]
pub struct StorageConnectionArgs {
    /// How long to wait for a snapshot file download (defaults
    /// to no timeout).
    #[arg(long)]
    snapshot_timeout_ms: Option<u64>,

    /// How long to wait while establishing a connection to the
    /// snapshot store (defaults to no timeout).
    #[arg(long)]
    snapshot_connection_timeout_ms: Option<u64>,
}

/// HTTP storage backend: each [`Path`] is appended to a base
/// `endpoint` URL and fetched with `reqwest`.
...
impl HttpStorage {
    pub(super) fn new(endpoint: Url, args: StorageConnectionArgs) -> anyhow::Result<Self> {
