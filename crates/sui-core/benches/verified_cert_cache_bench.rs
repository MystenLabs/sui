...
use sui_core::batch_bls_verifier::{BatchCertificateVerifierMetrics, VerifiedDigestCache};
...
fn verified_cert_cache_bench(c: &mut Criterion) {
    ...
    let cache = VerifiedDigestCache::new(metrics);
                    ...
                    s.spawn(|| {
                        for digest in &**chunk {
                            if cache.is_cached(digest) {
                                ...
                                cache.cache_verified_digest(*digest);
                            }
                        }
                    })
                });
