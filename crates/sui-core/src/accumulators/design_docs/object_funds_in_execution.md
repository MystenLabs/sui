...
declared, executed, and settled, see [`write_path.md`](./write_path.md). For the version
machinery this check consumes (how the accumulator root gets a deterministic version on
...
- A root that has not caught up locally never produces a failure at all: execution blocks until
  the version is committed, so unavailability is invisible in effects.

## 6. Rollout

`check_object_funds_withdraw_in_execution` is enabled at protocol version 132 on devnet,
off on testnet and mainnet pending the replay reconstruction caveat above. It requires
