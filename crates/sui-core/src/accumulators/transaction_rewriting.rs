...
use sui_types::balance::{BALANCE_MODULE_NAME, BALANCE_REDEEM_FUNDS_FUNCTION_NAME};
use sui_types::coin::{COIN_FROM_BALANCE_FUNCTION_NAME, COIN_MODULE_NAME};
...
pub fn rewrite_transaction_for_coin_reservations(
    ...
    match transaction_kind {
        TransactionKind::ProgrammableTransaction(pt) => TransactionKind::ProgrammableTransaction(
            rewrite_programmable_transaction_for_coin_reservations(coin_reservation_resolver, pt),
        ),
        _ => transaction_kind,
    }
}
        ...
        match input {
                ...
                if coin_reservation::is_coin_reservation_digest(&object_ref.2) =>
            {
                let withdraw = coin_reservation_resolver
                    .resolve_funds_withdrawal(object_ref)
                    .unwrap();
                ...
                let balance_result =
                    builder.command(Command::MoveCall(Box::new(ProgrammableMoveCall {
                        ...
                        module: BALANCE_MODULE_NAME.to_string(),
                        function: BALANCE_REDEEM_FUNDS_FUNCTION_NAME.to_string(),
                        type_arguments: vec![balance_type_input.clone()],
                        arguments: vec![withdraw_arg],
                    })));

                // turn balance into a coin
                let coin_result =
                    builder.command(Command::MoveCall(Box::new(ProgrammableMoveCall {
                        package: SUI_FRAMEWORK_PACKAGE_ID,
                        module: COIN_MODULE_NAME.to_string(),
                        function: COIN_FROM_BALANCE_FUNCTION_NAME.to_string(),
                        type_arguments: vec![balance_type_input.clone()],
                        arguments: vec![balance_result],
                    })));
