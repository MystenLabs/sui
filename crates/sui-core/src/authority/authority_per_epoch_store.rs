...
impl AuthorityPerEpochStore {
    ...
    /// Clear all state hashes computed after the given checkpoint.
    /// This is used during startup to ensure validators start with identical state
    /// after fork recovery.
    pub fn clear_state_hashes_after_checkpoint(
        ...
        // Clear running root state hashes after the last committed checkpoint
        let mut keys_to_remove = Vec::new();
        for kv in tables
            .running_root_state_hash
            .safe_iter_with_bounds(
                Some(last_committed_checkpoint + 1),
                None,
            )
        {
        ...
        // Clear checkpoint state hashes after the last committed checkpoint
