...
pub struct AuthorityPerEpochStore {
    ...
    /// Batch verifier for certificates - also caches certificates and transactions that are known
    /// to have valid signatures. Lives in per-epoch store because the caching/batching is only
    /// valid within for certs within the current epoch.
...
impl AuthorityPerEpochStore {
    pub fn new(
        ...
        batch_verifier_metrics: Arc<SignatureVerifierMetrics>,
    ) -> Arc<Self> {
        ...
        let batch_verifier = SignatureVerifier::new(committee.clone(), batch_verifier_metrics);
        Arc::new(Self {
            committee,
            protocol_config,
            tables,
            ...
            signature_verifier: batch_verifier,
