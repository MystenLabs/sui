...
impl key_value_store::TransactionKeyValueStore for AuthorityStore {
        ...
        for key in keys {
            match key {
                key_value_store::Key::Tx(_) => {
                    results.push(tx_iter.next().unwrap().map(|tx| Box::new(tx.into())))
                }
                key_value_store::Key::Fx(_) => {
                    results.push(fx_iter.next().unwrap().map(|fx| Box::new(fx.into())))
                }
                key_value_store::Key::Events(_) => {
                    results.push(events_iter.next().unwrap().map(|e| Box::new(e.into())))
                }
            }
        }
    ...
    async fn multi_put(
        &self,
        keys: &[key_value_store::Key],
        values: &[key_value_store::Value],
    ) -> SuiResult {
        unimplemented!("TransactionKeyValueStore for AuthorityStore is read only");
    }
    ...
    async fn multi_get_tx(
        ...
        self.multi_get_transaction_blocks(keys)?
            .into_iter()
            .map(|t| t.into_inner())
            .collect()
    }
    ...
    async fn multi_get_fx(
        ...
        Ok(self.multi_get_effects(keys)?)
    }
