...
/// both generations, so the retained window oscillates in
/// `[generation_capacity, 2 * generation_capacity)`. FIFO is also a better fit
/// than LRU here: a transaction is finalized once, so being repeatedly voted on
/// should not extend how long it is considered "recently finalized".
///
...
impl FinalizedTransactionsCache {
    ...
    pub fn insert(&self, tx_digest: TransactionDigest) {
        // Hold the write lock only for the in-memory mutation; drop the aged-out
        // generation afterwards so readers don't wait on its deallocation.
        let _aged_out = {
...
mod tests {
    ...
    fn retains_across_one_rotation_then_evicts() {
        // generation_capacity = 1, so the live window is [1, 2) entries: an
        // entry survives exactly one rotation before being dropped.
