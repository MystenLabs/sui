...
impl SharedObjVerManager {
    ...
    pub fn assign_versions_for_certificate(
        ...
        assign_versions_for_certificate_impl(cert, shared_input_next_versions, cancelled_txns)
    }
}
...
// TODO: merge this function with assign_versions_for_certificate() above.
fn assign_versions_for_certificate_impl(
    cert: &VerifiedExecutableTransaction,
    shared_input_next_versions: &mut HashMap<ObjectID, SequenceNumber>,
    cancelled_txns: &BTreeMap<TransactionDigest, CancelConsensusCertificateReason>,
) -> Vec<(ObjectID, SequenceNumber)> {
