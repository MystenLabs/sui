...
impl AuthorityAPI for ConfigurableBatchActionClient {
    ...
    async fn handle_batch_stream(
        ...
        let mut seq = 0;
        let zero_batch = SignedBatch::new(self.state.epoch(), AuthorityBatch::initial(), &*secret, name);
        ...
        let _ = actions.iter().for_each(|action| {
            match action {
                BatchActionInternal::EmitUpdateItem(test_batch) => {
                    ...
                    items.push({
                        let item = SignedBatch::new(self.state.epoch(), last_batch.clone(), &*secret, name);
