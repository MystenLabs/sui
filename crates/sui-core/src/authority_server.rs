                ...
                second_status = consensus_tx_status_cache.notify_read_transaction_status_change(request.transaction_position, Some(cur_status)) => {
                    ...
                    match second_status {
                        NotifyReadConsensusTxStatusResult::Status(status) => {
                            ...
                            assert_eq!(status, ConsensusTxStatus::Finalized, "Transaction {} at position {:?} should now be finalized and has status {status:?} with cur_status {cur_status:?}", request.transaction_digest, request.transaction_position);
