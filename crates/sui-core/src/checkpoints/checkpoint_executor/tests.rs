...
pub async fn test_checkpoint_executor_crash_recovery() {
    ...
    let executor_handle =
        spawn_monitored_task!(
            async move { executor.run_epoch(epoch_store, RunWithRange::None).await }
        );
...
pub async fn test_checkpoint_executor_cross_epoch() {
    ...
    timeout(Duration::from_secs(5), async {
        executor
            .run_epoch(epoch_store.clone(), RunWithRange::None)
            .await;
    })
    .await
    .unwrap();
    ...
    timeout(Duration::from_secs(5), async {
        executor
            .run_epoch(new_epoch_store.clone(), RunWithRange::None)
            .await;
    })
    .await
    .unwrap();
