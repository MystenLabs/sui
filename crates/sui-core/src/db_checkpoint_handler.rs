...
impl DBCheckpointHandler {
                ...
                tokio::select! {
                    _now = interval.tick() => {
                        if let Ok(epochs) = self.find_all_missing_checkpoint_epochs().await {
                            ...
                            if let Err(err) = self.upload_db_checkpoint_to_object_store(epochs).await {
    ...
    async fn upload_db_checkpoint_to_object_store(&self, missing_epochs: Vec<u32>) -> Result<()> {
...
mod tests {
    ...
    async fn test_basic() -> anyhow::Result<()> {
        ...
        db_checkpoint_handler
            .upload_db_checkpoint_to_object_store(missing_epochs)
            .await?;
