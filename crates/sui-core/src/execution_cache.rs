...
pub trait ObjectCacheRead: Send + Sync {
    ...
    fn multi_get_objects_with_more_accurate_error_return(
        ...
        for (object_opt, object_ref) in objects.into_iter().checked_zip(object_refs) {
            match object_opt {
                None => {
    ...
    fn multi_input_objects_available(
        ...
        for ((idx, (id, version)), has_key) in move_object_keys.iter().checked_zip(
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    fn multi_get_executed_effects(
        ...
        for (i, effects) in fetch_indices.into_iter().checked_zip(effects.into_iter()) {
            results[i] = effects;
        }
