...
impl<V> CachedVersionMap<V> {
    ...
    pub fn get_last(&self) -> &(SequenceNumber, V) {
...
mod tests {
    ...
    fn remove_first_item() {
        ...
        assert!(map.values.iter().find(|(v, _)| *v == version1).is_none());
    }
    ...
    fn remove_second_item_deferred() {
        ...
        assert!(map.values.iter().find(|(v, _)| *v == version2).is_none());
    }
