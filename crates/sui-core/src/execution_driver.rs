...
use sui_types::{error::SuiError, executable_transaction::PendingCertificate};
...
pub async fn execution_process(
    ...
    loop {
        ...
        if epoch_store.epoch() != pending_cert.epoch() {
            info!(
                ?digest,
                cur_epoch = epoch_store.epoch(),
                cert_epoch = pending_cert.epoch(),
        ...
        if get_rng().gen_range(0.0..1.0) < QUEUEING_DELAY_SAMPLING_RATIO {
            let cert_ready_time = pending_cert.stats.ready_time.unwrap();
            authority
                .metrics
                .execution_queueing_latency
                .report(cert_ready_time.elapsed());
        ...
        spawn_monitored_task!(epoch_store.within_alive_epoch(async move {
            ...
            match authority.try_execute_immediately(
                &pending_cert.executable_certificate,
                &epoch_store_clone,
            ).await {
                Err(SuiError::ValidatorHaltedAtEpochEnd) => {
                    warn!(
                        "Could not execute transaction {:?} because validator is halted at epoch end. certificate={:?}",
                        digest,
                ...
                Err(e) => {
                    fatal!(
                        "Failed to execute certified transaction {:?}! error={e} certificate={:?}",
                        digest,
                        pending_cert.certificate(),
                    );
                }
