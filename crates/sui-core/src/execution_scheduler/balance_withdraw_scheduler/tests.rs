...
async fn test_basic_sufficient_balance() {
    ...
    let withdraw = TxBalanceWithdraw {
        ...
        reservations: BTreeMap::from([(account, Reservation::MaxAmount(50))]),
    };
...
async fn test_basic_insufficient_balance() {
    ...
    let withdraw = TxBalanceWithdraw {
        tx_digest: TransactionDigest::random(),
        reservations: BTreeMap::from([(account, Reservation::MaxAmount(150))]),
    };
...
async fn test_out_of_order_settlements() {
    ...
    let withdraw2 = TxBalanceWithdraw {
        tx_digest: TransactionDigest::random(),
        reservations: BTreeMap::from([(account, Reservation::MaxAmount(80))]),
    };
    let mut receivers = test
        .scheduler
...
async fn test_multi_accounts() {
    ...
    let reservations1 = BTreeMap::from([
        (account1, Reservation::MaxAmount(50)),
        (account2, Reservation::MaxAmount(50)),
    ]);
    let withdraw1 = TxBalanceWithdraw {
        tx_digest: TransactionDigest::random(),
    ...
    let reservations2 = BTreeMap::from([
        (account1, Reservation::MaxAmount(50)),
        (account2, Reservation::MaxAmount(60)),
    ]);
    let withdraw2 = TxBalanceWithdraw {
        tx_digest: TransactionDigest::random(),
...
async fn test_settlement_far_ahead_of_schedule() {
    ...
    let withdraw = TxBalanceWithdraw {
        tx_digest: TransactionDigest::random(),
        reservations: BTreeMap::from([(account, Reservation::MaxAmount(100))]),
    };
    let mut receivers = test
        .scheduler
...
async fn test_withdraw_entire_balance() {
    ...
    let withdraws2 = vec![
        ...
        TxBalanceWithdraw {
            tx_digest: TransactionDigest::random(),
            reservations: BTreeMap::from([(account, Reservation::MaxAmount(1))]),
        },
    ];
        ...
        let reservations = account_ids
            ...
            .map(|account_id| {
                (
                    *account_id,
                    if rng.gen_bool(0.8) {
                        Reservation::MaxAmount(rng.gen_range(1..10))
                    } else {
