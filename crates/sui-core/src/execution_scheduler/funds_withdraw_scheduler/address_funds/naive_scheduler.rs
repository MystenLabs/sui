...
use crate::{
    ...
    execution_scheduler::funds_withdraw_scheduler::{TxFundsWithdraw, WithdrawReservations},
};
...
impl NaiveFundsWithdrawScheduler {
    ...
    fn process_withdraws(
    ...
    ) -> Vec<(TransactionDigest, ScheduleStatus)> {
        ...
        for account_id in all_accounts {
            ...
            if version > accumulator_version {
                return withdraws
                    .into_iter()
                    .map(|withdraw| (withdraw.tx_digest, ScheduleStatus::SkipSchedule))
                    .collect();
            }
...
impl FundsWithdrawSchedulerTrait for NaiveFundsWithdrawScheduler {
    #[instrument(level = "debug", skip_all, fields(withdraw_accumulator_version = ?accumulator_version.value()))]
    fn schedule_withdraws(
        ...
        if cur_version == reservations.accumulator_version {
            let results = Self::process_withdraws(
                self.funds_read.as_ref(),
                reservations.accumulator_version,
                reservations.withdraws,
            );
            return results
                .into_iter()
                .map(|(tx_digest, status)| (tx_digest, ScheduleResult::ScheduleResult(status)))
                .collect();
        }

        let (senders, receivers): (Vec<_>, BTreeMap<_, _>) = withdraws
            .iter()
            .map(|withdraw| {
        ...
        tokio::spawn(async move {
            ...
            for ((_, result), sender) in results.into_iter().zip(senders) {
