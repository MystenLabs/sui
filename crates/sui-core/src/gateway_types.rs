...
impl SuiObject {
    ...
    pub fn try_from(o: Object, layout: Option<MoveStructLayout>) -> Result<Self, anyhow::Error> {
        ...
        let data = match o.data {
            ...
            Data::Package(p) => {
                let disassembled = p.disassemble()?;
                let (_, modules) = p.into();
                SuiData::Package(SuiMovePackage {
                    modules,
                    disassembled,
                })
            }
...
impl SuiData {
    ...
    pub fn try_as_move_object(&self) -> Option<&SuiMoveObject> {
        match self {
            SuiData::MoveObject(o) => Some(o),
            SuiData::Package(p) => None,
        }
    }

    pub fn into_move_object(self) -> Option<SuiMoveObject> {
        match self {
            SuiData::MoveObject(o) => Some(o),
            SuiData::Package(p) => None,
        }
...
impl SuiMovePackage {
    pub fn modules(&self) -> &BTreeMap<String, Vec<u8>> {
        &self.modules
    }
}
...
impl TryFrom<MoveModulePublish> for SuiMovePackage {
    ...
    // TODO: this is a bug--the disassembled code is going to have the wrong package ID (0x0 instead of the ID generated by the tx)
    fn try_from(m: MoveModulePublish) -> Result<Self, Self::Error> {
        let modules = unimplemented!("Parse from module publish result");
        Ok(Self {
            modules,
