    ...
    pub fn new(registry: &Registry) -> Arc<Self> {
        Arc::new(Self {
            ...
            key_error: register_int_counter_vec_with_registry!(
                "json_rpc_compaction_filter_key_not_found",
                "Compaction error",
                &["cf"],
                registry
            )
...
impl IndexStore {
    pub fn new_without_init(
        ...
        let table_options = DBMapTableConfigMap::new(BTreeMap::from([
            ...
            (
                "event_by_move_event".to_string(),
                compaction_filter_config_by_key(
                    "event_by_move_event",
                    compaction_metrics.clone(),
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, event_id): (StructTag, EventId)| event_id.0,
                ),
            ),
            (
    ...
    pub fn prune(&self, tx_digest: &TransactionDigest) -> Result<(), SuiError> {
        if let Some(watermark) = self.get_transaction_seq(tx_digest)? {
