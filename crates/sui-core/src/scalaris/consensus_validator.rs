...
impl ScalarisTxValidator {
    pub fn new(
        ...
        info!(
            "ScalarisTxValidator constructed for epoch {}",
            epoch_store.epoch()
        );
        Self {
            epoch_store,
...
/*
 * 20240510
 * Scalaris: Convert to the ConsensusTransactionWrapper
 */
fn tx_from_bytes(tx: &[u8]) -> Result<crate::ConsensusTransactionWrapper, eyre::Report> {
    bcs::from_bytes::<ConsensusTransaction>(tx).map_or_else(
        |e| {
            tracing::debug!("{:?}", e);
            bcs::from_bytes::<crate::ConsensusTransactionWrapper>(tx)
                .wrap_err("Malformed transaction (failed to deserialize)")
        },
        |transaction| Ok(crate::ConsensusTransactionWrapper::Consensus(transaction)),
    )
}

impl TransactionValidator for ScalarisTxValidator {
    ...
    fn validate_batch(
        ...
        /*
         * 20240510
         * Scalaris: filter out namespace transactions
         */
        let txs = b
            .transactions()
            .iter()
            .filter_map(|tx| {
                tx_from_bytes(tx)
                    .map(|item| match item {
                        crate::ConsensusTransactionWrapper::Namespace(_) => None,
                        crate::ConsensusTransactionWrapper::Consensus(transaction) => {
...
impl TransactionVerifier for ScalarisTxValidator {
    fn verify_batch(
        &self,
...
mod tests {
    ...
    use super::{ScalarisTxValidator, ScalarisTxValidatorMetrics};
    use crate::{
    ...
    async fn accept_valid_transaction() {
        ...
        let metrics = ScalarisTxValidatorMetrics::new(&Default::default());
        let validator = ScalarisTxValidator::new(
