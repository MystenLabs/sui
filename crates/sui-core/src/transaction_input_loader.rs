...
use mysten_common::{CheckedIteratorExt, checked_izip};
...
impl TransactionInputLoader {
    ...
    pub fn read_objects_for_signing(
        ...
        for (index, object) in fetch_indices.into_iter().checked_zip(objects.into_iter()) {
    ...
    pub fn read_objects_for_execution(
        ...
        for (object, key, (index, input)) in checked_izip!(
            objects.into_iter(),
