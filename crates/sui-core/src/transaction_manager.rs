                    ...
                    match lock_mode {
                        LockMode::Default => {
                            ...
                            if !(lock_queue.readonly_waiters.is_empty()
                                && lock_queue.readonly_holders.is_empty())
                            {
                                acquire = true;
                                assert!(lock_queue.default_waiters.insert(digest));
                            }
    ...
    pub(crate) fn objects_available(
        ...
        for input_key in input_keys {
            ...
            if !lock_queue.readonly_waiters.is_empty() {
                ...
                if lock_queue.readonly_holders.is_empty() {
    ...
    ) {
        {
            ...
            for (key, lock_mode) in &acquired_locks {
                ...
                assert!(
                    ...
                    "Certificate {:?} not found in default lock holders",
                    digest
                );
                if lock_queue.readonly_holders.is_empty() {
                    assert!(lock_queue.readonly_waiters.is_empty());
