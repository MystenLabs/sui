...
#[tokio::test]
async fn test_shared_object_metrics_exclude_implicit_registry() {
    ...
    let shared_transactions_before = authority.metrics.shared_obj_tx.get();
    let shared_objects_before = authority.metrics.num_shared_objects.get_sample_sum();
    let transfer = TestTransactionBuilder::new(
        sender,
    ...
    .build();
    let (_, effects) = submit_and_execute(
        &authority,
        to_sender_signed_transaction(transfer, &sender_key),
    )
    .await
    .unwrap();
    ...
    assert_eq!(
        ...
        shared_transactions_before,
    );
    assert_eq!(
        authority.metrics.num_shared_objects.get_sample_sum(),
        shared_objects_before,
    );

    // The same registry is a declared shared input when registering, and must still be counted.
    let gas = authority.get_object(&gas.id()).unwrap();
    ...
    let (_, effects) = submit_and_execute(
        &authority,
        to_sender_signed_transaction(builder.build(), &sender_key),
    )
    .await
    .unwrap();
    assert!(effects.data().status().is_ok(), "{effects:?}");
