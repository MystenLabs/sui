...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (SuiAddress, Vec<DynamicFieldInfo>) {
    ...
    let add_effects = authority_state
        .try_execute_for_test(&add_cert, ExecutionEnv::for_grpc_fastpath())
        .await
        .unwrap()
        .0
        .into_message();
...
async fn test_shared_object_transaction_no_shared_version_assignments() {
    ...
    let _ = authority
        .try_execute_for_test(&certificate, ExecutionEnv::for_grpc_fastpath())
        .await;
}
...
async fn test_insufficient_balance_for_withdraw_early_error() {
    ...
    let mut execution_env = ExecutionEnv::for_mysticeti_fastpath();
