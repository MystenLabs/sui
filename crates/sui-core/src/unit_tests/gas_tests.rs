...
async fn test_oog_full_budget_charge_exact_when_budget_indivisible_by_price() -> SuiResult {
    // Self-contained: force the v15+ pipeline (gas_model 15) regardless of which protocol version
    // activates it, so this test exercises gas_v3 independent of the activation PR.
    let _gas_v3 = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
        config.set_gas_model_version_for_testing(15);
        config
    });
    const GAS_PRICE: u64 = 1000;
    const MAX_UNIT_BUDGET: u64 = 5_000_000;
