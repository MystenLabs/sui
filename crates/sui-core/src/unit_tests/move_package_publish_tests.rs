...
use sui_move_build::{BuildConfig, check_unpublished_dependencies, gather_published_ids};
...
async fn test_custom_property_check_unpublished_dependencies() {
    ...
    let SuiErrorKind::ModulePublishFailure { error } = check_unpublished_dependencies(
