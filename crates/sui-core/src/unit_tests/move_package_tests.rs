...
fn test_new_initial() {
    ...
    let c_pkg = MovePackage::new_initial_for_testing(
    ...
    let b_pkg = MovePackage::new_initial_for_testing(
        OBJECT_START_VERSION,
    ...
    let a_pkg = MovePackage::new_initial_for_testing(
        OBJECT_START_VERSION,
...
fn test_upgraded() {
    ...
    let c_new = c_pkg
        .new_upgraded_for_testing(c_id2, build_test_modules("Cv2"), u64::MAX, [])
        .unwrap();
...
fn test_upgrade_upgrades_linkage() {
    ...
    let b_new = b_pkg
        .new_upgraded_for_testing(b_id2, build_test_modules("B"), u64::MAX, [&c_new])
        .unwrap();
...
fn test_upgrade_downngrades_linkage() {
    ...
    let b_id2 = ObjectID::from_single_byte(0xb2);
    let b_new = b_pkg
        .new_upgraded_for_testing(b_id2, build_test_modules("B"), u64::MAX, [&c_pkg])
        .unwrap();
...
fn package_digest_changes_with_dep_upgrades_and_in_sync_with_move_package_digest() {
    let c_v1 = MovePackage::new_initial_for_testing(
        OBJECT_START_VERSION,
        build_test_modules("Cv1"),
        u64::MAX,
        [],
    ...
    let b_v2 = MovePackage::new_initial_for_testing(
        OBJECT_START_VERSION,
...
fn test_panic_on_empty_package() {
    let _ = MovePackage::new_initial_for_testing(OBJECT_START_VERSION, vec![], u64::MAX, []);
}

#[test]
fn test_fail_on_missing_dep() {
    let err = MovePackage::new_initial_for_testing(
        OBJECT_START_VERSION,
        build_test_modules("B"),
        u64::MAX,
        [],
...
fn test_fail_on_upgrade_missing_type() {
    ...
    let c_id2 = ObjectID::from_single_byte(0xc2);
    let err = c_pkg
        .new_upgraded_for_testing(c_id2, build_test_modules("Cv1"), u64::MAX, [])
        .unwrap_err();
