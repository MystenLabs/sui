...
async fn test_pay_sui_failure_empty_recipients() {
    ...
    let coin1 = Object::with_id_owner_gas_for_testing(coin_id, sender, 2000);

    // an empty set of programmable transaction commands will still charge gas
    let res = execute_pay_sui(vec![coin1], vec![], vec![], sender, sender_key, 2000).await;
...
async fn test_pay_sui_failure_insufficient_total_balance_one_input_coin() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let coin1 = Object::with_id_owner_gas_for_testing(ObjectID::random(), sender, 1200);
    let recipient1 = dbg_addr(1);
    let recipient2 = dbg_addr(2);
...
async fn test_pay_sui_success_one_input_coin() -> anyhow::Result<()> {
    ...
    let coin_obj = Object::with_id_owner_gas_for_testing(object_id, sender, 100000);
    let recipient1 = dbg_addr(1);
    let recipient2 = dbg_addr(2);
    ...
    assert_eq!(
        ...
        100000 - 100 - 200 - 300 - gas_used as u64,
    );
...
async fn test_pay_sui_success_multiple_input_coins() -> anyhow::Result<()> {
    ...
    let coin_obj2 = Object::with_id_owner_gas_for_testing(object_id2, sender, 20000);
    let coin_obj3 = Object::with_id_owner_gas_for_testing(object_id3, sender, 20000);
    let recipient1 = dbg_addr(1);
    let recipient2 = dbg_addr(2);
    ...
    assert_eq!(
        GasCoin::try_from(&gas_object)?.value(),
        70000 - 500 - 1500 - gas_used as u64,
    );
...
async fn test_pay_all_sui_success_one_input_coin() -> anyhow::Result<()> {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let object_id = ObjectID::random();
    let coin_obj = Object::with_id_owner_gas_for_testing(object_id, sender, 70000);
    let recipient = dbg_addr(2);
    let res = execute_pay_all_sui(vec![&coin_obj], recipient, sender, sender_key, 50000).await;
    ...
    let gas_object = res.authority_state.get_object(&object_id).await?.unwrap();
    assert_eq!(GasCoin::try_from(&gas_object)?.value(), 70000 - gas_used,);
    Ok(())
}
...
async fn test_pay_all_sui_success_multiple_input_coins() -> anyhow::Result<()> {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let object_id1 = ObjectID::random();
    let coin_obj1 = Object::with_id_owner_gas_for_testing(object_id1, sender, 40000);
    let coin_obj2 = Object::with_id_owner_gas_for_testing(ObjectID::random(), sender, 30000);
    let coin_obj3 = Object::with_id_owner_gas_for_testing(ObjectID::random(), sender, 20000);
    let recipient = dbg_addr(2);
    ...
    let gas_object = res.authority_state.get_object(&object_id1).await?.unwrap();
    assert_eq!(GasCoin::try_from(&gas_object)?.value(), 90000 - gas_used,);
    Ok(())
}
