...
async fn test_shared_object_v2_denied() {
    ...
    // Test 2: Transaction with SharedObjectV2 should be rejected during validation
    {
        ...
        assert!(
            result.is_err(),
            "Transaction with SharedObjectV2 should fail validity check"
        );

        if let Err(e) = result {
            // Check that the error is a UserInputError about SharedObjectV2
            match &e {
                SuiError::UserInputError { error } => match error {
                    UserInputError::Unsupported(msg) => {
                        assert!(
                            msg.contains("SharedObjectV2"),
                            "Expected error about SharedObjectV2, got: {}",
                            msg
                        );
                    }
