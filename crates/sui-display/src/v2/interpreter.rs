...
use std::{borrow::Cow, fmt::Write as _, sync::atomic::AtomicUsize};
...
impl<'a> Interpreter<'a> {
    pub(crate) fn new(store: Box<dyn Store<'a>>, root: Slice<'a>) -> Self {
        Self { store, root }
    }

    /// Entrypoint to evaluate a single format string.
    ///
    /// The size of the output is bounded by `max_output_size`, and measured in a shared counter --
    /// `used_output`.
    pub(crate) async fn eval(
        &self,
        used_output: &AtomicUsize,
        max_output_size: usize,
        strands: &'a [P::Strand<'a>],
    ) -> Result<serde_json::Value, FormatError> {
        // TODO(amnn): Support nested display and JSON transform.
        let mut writer = BoundedWriter::new(used_output, max_output_size);
    ...
    async fn eval_alts(&self, alts: &'a [P::Chain<'a>]) -> Result<Option<Value<'a>>, FormatError> {
        for chain in alts {
    ...
    async fn eval_chain(&self, chain: &'a P::Chain<'a>) -> Result<Option<Value<'a>>, FormatError> {
        use Accessor as A;
        use Value as V;
        ...
        let accessors = try_join_all(chain.accessors.iter().map(|a| self.eval_accessor(a)));
        ...
        let Some(mut accessors) = accessors?.into_iter().collect::<Option<Vec<_>>>() else {
            return Ok(None);
        };
        ...
        while let Some(accessor) = accessors.last() {
            match (root, accessor) {
                ...
                // value. This can consume multiple accessors.
                (V::Slice(slice), _) => {
                    let Some(value) = Extractor::deserialize_slice(slice, &mut accessors)
                        .map_err(FormatError::Visitor)?
                    else {
                        return Ok(None);
                    };
    ...
    async fn eval_accessor(
        &self,
        acc: &'a P::Accessor<'a>,
    ) -> Result<Option<Accessor<'a>>, FormatError> {
        use Accessor as VA;
    ...
    async fn eval_literal(
        &self,
        lit: &'a P::Literal<'a>,
    ) -> Result<Option<Value<'a>>, FormatError> {
        use Value as V;
        use P::Literal as L;
    ...
    async fn eval_fields(
        &self,
        field: &'a P::Fields<'a>,
    ) -> Result<Option<Fields<'a>>, FormatError> {
        Ok(match field {
    ...
    async fn eval_chains(
        &self,
        chains: impl IntoIterator<Item = &'a P::Chain<'a>>,
    ) -> Result<Option<Vec<Value<'a>>>, FormatError> {
        let values = chains
            .into_iter()
            .map(|chain| Box::pin(self.eval_chain(chain)));

        Ok(try_join_all(values).await?.into_iter().collect())
    }
}
