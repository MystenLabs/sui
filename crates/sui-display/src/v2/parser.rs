...
impl<'s> Parser<'s> {
    ...
    fn parse_type<'b>(&mut self, meter: &mut Meter<'b>) -> Result<TypeTag, Error> {
        let meter = &mut meter.nest()?;
        Ok(match_token! { self.lexer;
            ...
            Lit(_, T::Ident, offset, "vector") => {
                ...
                match type_params.len() {
                    1 => {
                        // SAFETY: Bounds check on `type_params.len()` guarantees safety.
                        let inner = type_params.into_iter().next().unwrap();
    ...
    fn parse_datatype<'b>(&mut self, meter: &mut Meter<'b>) -> Result<StructTag, Error> {
    ...
    fn parse_type_params<'b>(&mut self, meter: &mut Meter<'b>) -> Result<Vec<TypeTag>, Error> {
    ...
    fn test_metering_field_access() {
        ...
        assert_eq!(
            ...
            vec![S::Expr(E {
                alternates: vec![C {
                    root: None,
                    accessors: vec![A::Field("foo"), A::Field("bar")],
                }],
                transform: None,
            })]
        );
