...
async fn test_accumulators_disabled() {
    ...
    let withdraw_tx = test_cluster
        .test_transaction_builder()
        .await
        .withdraw_funds_sui(1000, 1000, None)
        .build();
    let withdraw_tx = test_cluster.sign_transaction(&withdraw_tx).await;
    test_cluster
        .wallet
...
async fn test_address_balance_gas() {
    ...
    let deposit_tx = test_cluster
        .test_transaction_builder_with_gas_object(sender, gas)
        .await
        .send_funds_sui(&[(10_000_000, None)])
        .build();
...
async fn test_sponsored_address_balance_storage_rebates() {
    ...
    let sender_gas = test_cluster
        ...
        .unwrap()
        .unwrap();
    let deposit_tx_sender = test_cluster
        .test_transaction_builder_with_gas_object(sender, sender_gas)
        .await
        .send_funds_sui(&[(100_000_000, None)])
        .build();
    test_cluster
    ...
    let sponsor_gas = test_cluster
        .wallet
        .get_one_gas_object_owned_by_address(sponsor)
        .await
        .unwrap()
        .unwrap();
    let deposit_tx_sponsor = test_cluster
        .test_transaction_builder_with_gas_object(sponsor, sponsor_gas)
        .await
        .send_funds_sui(&[(100_000_000, None)])
        .build();
    test_cluster
...
// TODO: Add pay gas from address balance support in TestTransactionBuilder.
fn create_pay_gas_from_address_balance_transaction(
    sender: SuiAddress,
    rgp: u64,
    chain_id: ChainIdentifier,
    withdraw_amount: u64,
...
async fn test_address_balance_gas_cost_parity() {
    ...
    let deposit_tx = test_cluster
        .test_transaction_builder_with_gas_object(sender, gas_for_deposit)
        .await
        .send_funds_sui(&[(100_000_000, None)])
        .build();
    test_cluster.sign_and_execute_transaction(&deposit_tx).await;
...
async fn test_sponsor_insufficient_balance_charges_zero_gas() {
    ...
    let deposit_tx_sponsor = test_cluster
        .test_transaction_builder_with_gas_object(sponsor, sponsor_gas)
        .await
        .send_funds_sui(&[(sponsor_initial_balance, None)])
        .build();
    test_cluster
        .sign_and_execute_transaction(&deposit_tx_sponsor)
        .await;
...
async fn test_insufficient_balance_charges_zero_gas() {
    ...
    let deposit_tx = test_cluster
        .test_transaction_builder_with_gas_object(sender, gas_for_deposit)
        .await
        .send_funds_sui(&[(initial_balance, None)])
        .build();
    test_cluster.sign_and_execute_transaction(&deposit_tx).await;
    ...
    let tx1 =
        create_pay_gas_from_address_balance_transaction(sender, rgp, chain_id, withdraw_amount, 0);

    let tx2 =
        create_pay_gas_from_address_balance_transaction(sender, rgp, chain_id, withdraw_amount, 1);
...
async fn test_soft_bundle_different_gas_payers() {
    ...
    let sender1_gas = test_cluster
        .wallet
        .get_one_gas_object_owned_by_address(sender1)
        .await
        .unwrap()
        .unwrap();
    let deposit_tx1 = test_cluster
        .test_transaction_builder_with_gas_object(sender1, sender1_gas)
        .await
        .send_funds_sui(&[(10_000_000, None)])
        .build();
    test_cluster
    ...
    let sender2_gas = test_cluster
        .wallet
        .get_one_gas_object_owned_by_address(sender2)
        .await
        .unwrap()
        .unwrap();
    let deposit_tx2 = test_cluster
        .test_transaction_builder_with_gas_object(sender2, sender2_gas)
        .await
        .send_funds_sui(&[(10_000_000, None)])
        .build();
    test_cluster
