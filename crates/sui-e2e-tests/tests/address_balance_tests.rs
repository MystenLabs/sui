...
async fn wait_for_effects(
    ...
    for _ in 0..600 {
        let effects_opt = validator_handle.with(|node| {
            let state = node.state();
            state.get_transaction_cache_reader().get_executed_effects(digest)
        });
        if effects_opt.is_some() {
            return effects_opt.unwrap();
        }
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    }
    panic!("Transaction effects not found after 60 seconds for digest: {:?}", digest);
}
...
async fn test_sponsor_insufficient_balance_charges_zero_gas() {
    ...
    assert!(status_str.contains("InsufficientBalanceForWithdraw"), "Failed transaction should have InsufficientBalanceForWithdraw error");
    ...
    assert_eq!(final_sponsor_balance, expected_final_sponsor_balance, "Sponsor balance should reflect only the successful transaction");
...
async fn test_insufficient_balance_charges_zero_gas() {
    ...
    assert_eq!(final_sender_balance, expected_final_balance, "Final balance should reflect only the successful transaction");
