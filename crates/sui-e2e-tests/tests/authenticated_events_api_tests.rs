...
use sui_rpc_api::grpc::alpha::state_service_proto::state_service_client::StateServiceClient;
use sui_rpc_api::grpc::alpha::state_service_proto::GetObjectInclusionProofRequest;
...
fn proto_object_ref_to_sui_object_ref(
    object_ref_proto: &sui_rpc_api::grpc::alpha::state_service_proto::ObjectRef,
    ...
    let object_id =
        ObjectID::from_bytes(object_id_bytes).map_err(|e| format!("Invalid object_id: {}", e))?;
...
fn proto_ocs_inclusion_proof_to_light_client_proof(
    grpc_proof: &sui_rpc_api::grpc::alpha::state_service_proto::OcsInclusionProof,
...
async fn fetch_and_verify_event_stream_head(
    state_client: &mut StateServiceClient<tonic::transport::Channel>,
    ledger_client: &mut LedgerServiceClient<tonic::transport::Channel>,
    epoch_cache: &EpochCache,
    object_id: ObjectID,
    checkpoint: u64,
...
async fn list_authenticated_events_end_to_end() {
    ...
    let mut state_client = StateServiceClient::connect(test_cluster.rpc_url().to_owned())
        .await
        .unwrap();
