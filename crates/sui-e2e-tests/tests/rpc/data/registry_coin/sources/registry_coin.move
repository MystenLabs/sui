...
/// Example coin using the new create_currency_v2 API with CoinRegistry
module registry_coin::registry_coin {
	...
	/// Register the currency using create_currency_v2 API
	fun init(witness: REGISTRY_COIN, ctx: &mut TxContext) {
		let (treasury_cap, metadata_cap, init_coin_data) = coin::create_currency_v2<REGISTRY_COIN>(
			witness,
			6, // decimals
	...
	public entry fun update_name(
		...
		registry.set_name<REGISTRY_COIN>(metadata_cap, new_name.to_string());
	}
	...
	public entry fun register_supply(
		...
		coin::register_supply(registry, treasury_cap);
	}
}
