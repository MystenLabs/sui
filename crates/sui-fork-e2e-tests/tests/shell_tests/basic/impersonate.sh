...
mkdir victim
victim_config=victim/client.yaml
cp "$ISOLATED_LOCALNET_CONFIG_TEMPLATE" "$victim_config"
victim=$(sui client --client.config "$victim_config" new-address ed25519 --json | jq -r .address)
sui client --client.config "$victim_config" switch --address "$victim" > /dev/null
victim_envs=$(sui client --client.config "$victim_config" envs --json \
  | jq -r '[.[1], (.[0] | map(.alias) | join(","))] | join(" ")')
assert_eq "$victim_envs" "localnet localnet" "the victim client only knows the localnet"
