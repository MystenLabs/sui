...
///
/// Callers only need these fields for RPC responses and finality metadata; the
...
/// Shared context for the forked network: the simulacrum and the service
/// manager running the embedded indexer.
pub struct Context {
...
impl Context {
    ...
    /// before returning, so committed local checkpoints get indexed for RPC
    /// reads. The indexer's broadcast pipeline owns `checkpoint_sender` from
    /// here on: it, not this type, is what publishes to subscribers, so their
    ...
    /// Resolves when the embedded rpc-store indexer stops. Used as a liveness
    /// watchdog by the server loop, so an indexer failure surfaces immediately
    /// instead of as a publication timeout on the next executed transaction.
    ...
    /// advancement, and it returns only once `sui-rpc-store` has indexed the
    /// checkpoint.
    ///
    /// # Panics
    ///
    ...
    /// intentionally held through indexing so subscribers observe the same
    /// order that Simulacrum used to create checkpoints.
    ///
    /// # Panics
    ///
    /// Panics if the indexer cannot index the checkpoint before publishing.
