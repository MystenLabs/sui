...
//! GraphQL queries against the schema defined in
//! `crates/sui-indexer-alt-graphql/schema.graphql`, built in modules for epochs, transactions,
...
/// address, so it never appears in the address's owned-object enumeration. The field's object id
/// is derivable from `(address, coin type)`, but only once the coin type is known, and nothing
/// local knows which coin types an address has an accumulator balance in, including coin types it
/// holds no `Coin<T>` of. This query supplies exactly that list and nothing else. The balance
/// amounts are deliberately ignored, because the objects themselves are seeded and the stock
/// `Balance` restore pipeline derives the amounts from them.
pub(crate) mod address_balances_query {
    ...
    ///
    /// Rows with a zero or absent `addressBalance` are dropped, because the
    /// connection reports a row for every coin type the address has any
    /// balance in, including ones backed purely by coin objects, and those
    /// carry no accumulator field to seed.
    pub(crate) async fn query(
        address: SuiAddressType,
        checkpoint: CheckpointSequenceNumber,
        client: &GraphQLClient,
    ...
    /// "no balance" rather than an error, because the worst outcome is a
    /// field left unseeded, whereas failing here would take down seeding for
    /// the whole address over a single unparseable row.
...
pub(crate) mod object_seed_query {
    ...
    ///
    /// The owner check in [`query`] is a policy on what a user may name as a
    /// seed. This entry point is for ids the fork derived itself and already
    /// knows the shape of. Today those are the accumulator balance fields,
    /// which are owned by the accumulator root rather than by any address,
    /// and so would be rejected by that check. Objects absent at the
    /// checkpoint come back as `None` rather than an error, because an
    /// address may simply have no field for a given coin type.
