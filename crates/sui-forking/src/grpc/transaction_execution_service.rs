...
async fn simulate_transaction_impl(
    ...
    let execution::DryRunResult {
        effects, objects, ..
    } = execution::simulate_transaction(context, tx_data).await?;

    // Prefer object metadata returned by simulation and fall back to local store lookups.
    let object_type_by_key = build_object_type_lookup(&objects);
...
/// Builds a versioned object-type lookup from simulation object metadata.
fn build_object_type_lookup(objects: &ObjectSet) -> BTreeMap<ObjectKey, String> {
