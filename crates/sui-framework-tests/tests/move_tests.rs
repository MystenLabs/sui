...
use sui_move::unit_test::{MAX_GAS_COMPUTATION_BUCKET, TEST_GAS_PRICE, run_move_unit_tests};
...
pub(crate) async fn tests(path: &Path) -> datatest_stable::Result<()> {
    ...
    let mut testing_config =
        UnitTestingConfig::default_with_bound(Some(*MAX_GAS_COMPUTATION_BUCKET * TEST_GAS_PRICE));
