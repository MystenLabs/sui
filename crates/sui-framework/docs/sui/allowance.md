...
Native allowances: enable delegated, bounded, revocable spending from an address's
live balance.

A transaction declares its funding source as a (funder, allowance) pair.
Signing verifies that source against the shared <code><a href="../sui/allowance.md#sui_allowance_Allowance">Allowance</a></code> and hands the
transaction an <code><a href="../sui/allowance.md#sui_allowance_AllowanceWithdrawal">AllowanceWithdrawal</a></code>.

All logic checks (rate limits, lifetime spends etc) are enforced by this module..
...
-  [Function `settings`](#sui_allowance_settings)
-  [Function `current_spend`](#sui_allowance_current_spend)
-  [Function `proposed_settings`](#sui_allowance_proposed_settings)
...
Created by the runtime (adapter).

The inner <code>Withdrawal</code> is never handed out on its own: every spend path
consumes limits and redeems in one step, so an allowance's limits can never
be charged without the funds actually moving.
...
Enables withdrawing <code>T</code> from a funder's balance, within limits enforced.
...
<dt>
<code><a href="../sui/allowance.md#sui_allowance_settings">settings</a>: <a href="../sui/allowance.md#sui_allowance_Settings">sui::allowance::Settings</a></code>
</dt>
<dd>
</dd>
<dt>
<code><a href="../sui/allowance.md#sui_allowance_current_spend">current_spend</a>: u256</code>
</dt>
<dd>
...
<dd>
 An optional lifetime cap on withdrawals using this allowance.
...
<dd>
 Optional expiration time, in milliseconds. After which, the allowance
 cannot be used.
</dd>
<dt>
...
<dd>
 An optional recurring cap, applied on top of <code><a href="../sui/allowance.md#sui_allowance_lifetime_cap">lifetime_cap</a></code>. At least
 one of the two must be set, so no allowance is unbounded.
</dd>
<dt>
<code><a href="../sui/allowance.md#sui_allowance_name">name</a>: <a href="../std/string.md#std_string_String">std::string::String</a></code>
</dt>
<dd>
 A label not used in any check. It is intended for off-chain usage or
 bookkeeping. The <code><a href="../sui/allowance.md#sui_allowance_name">name</a></code> can be at most 128 bytes.
</dd>
</dl>
...
Created with the allowance and destroyed with it: <code><a href="../sui/allowance.md#sui_allowance_revoke">revoke</a></code> consumes both, so
...
A <code><a href="../sui/allowance.md#sui_allowance_SettingsPermit">SettingsPermit</a>&lt;A&gt;</code> authorizes changing what an allowance bound to <code>A</code> *is*
-- issuing one, or rotating its spender. It is issued from an
<code>internal::Permit&lt;A&gt;</code>.

Kept distinct from <code><a href="../sui/allowance.md#sui_allowance_SpendPermit">SpendPermit</a></code> so an app can hand out the right to spend
without also handing out the right to reconfigure, and vice versa.
...
An enum so that a limit with different mechanics -- a sliding window, or a
continuously refilling bucket -- can be added as a variant later, rather
than as a layout change to allowances that already exist.
...
<dd>
 At most <code>limit</code> per window. Windows only roll forward, so their
...
<dd>
 The most that can be spent within a window.
</dd>
</dl>
...
<dd>
 Amount spent so far within a window.
</dd>
</dl>
...
<dd>
 ...
 Every later window is counted from here.
</dd>
</dl>
...
<dd>
 Which window <code>spent</code> is accumulated in, numbered from the anchor
 (0 = first). A spend landing in a later one resets <code>spent</code>.
</dd>
</dl>
...
How windows are counted from a rate limit's anchor.
...
<dl>
<dt>
Variant <code>Periodic</code>
</dt>
<dd>
 Windows of exactly <code>period_ms</code>.
</dd>
...
<dt>
Variant <code>Calendar</code>
</dt>
<dd>
 Windows of <code>months</code> civil (UTC) months, renewing on the anchor's
 day-of-month at 00:00 UTC.
</dd>
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_periodic_rate_limit">periodic_rate_limit</a>(period_ms: u64, limit: u256): <a href="../sui/allowance.md#sui_allowance_RateLimit">RateLimit</a> {
    ...
    <a href="../sui/allowance.md#sui_allowance_new_rate_limit">new_rate_limit</a>(limit, Window::Periodic { period_ms })
}
</code></pre>
...
At most <code>limit</code> per <code>months</code> civil (UTC) months, counted from the first
charge: windows renew on its day-of-month at 00:00 UTC, clamped to shorter
months (a Jan 31 anchor renews Feb 28, then Mar 31).
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_calendar_rate_limit">calendar_rate_limit</a>(months: u8, limit: u256): <a href="../sui/allowance.md#sui_allowance_RateLimit">RateLimit</a> {
    ...
    <a href="../sui/allowance.md#sui_allowance_new_rate_limit">new_rate_limit</a>(limit, Window::Calendar { months })
}
</code></pre>
...
Creation is always <code><b>entry</b></code> to ensure only top-level PTB
(contracts cannot implicitly create allowances).
...
<pre><code><b>entry</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_new">new</a>&lt;T&gt;(
    ...
    <b>let</b> <a href="../sui/allowance.md#sui_allowance_settings">settings</a> = <a href="../sui/allowance.md#sui_allowance_new_settings">new_settings</a>(
        ctx.sender(),
    ...
    <a href="../sui/allowance.md#sui_allowance_share_new">share_new</a>&lt;T&gt;(<a href="../sui/allowance.md#sui_allowance_settings">settings</a>, ctx);
}
</code></pre>
...
Proposes an allowance bound to the controlling app <code>A</code>.

Unlike <code><a href="../sui/allowance.md#sui_allowance_new">new</a></code>, this does not create anything: <code>A</code>'s module must accept the
proposal via <code><a href="../sui/allowance.md#sui_allowance_issue">issue</a></code>, so an app always gets a say in the allowances that
name it and can vet the configuration.
...
<pre><code><b>entry</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_propose_for_app">propose_for_app</a>&lt;T, A&gt;(
    ...
    <a href="../sui/allowance.md#sui_allowance_AllowanceProposal">AllowanceProposal</a>(<a href="../sui/allowance.md#sui_allowance_settings">settings</a>)
}
</code></pre>
...
Accepts a proposal on <code>A</code>'s behalf, creating and sharing the allowance.
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_issue">issue</a>&lt;T, A&gt;(proposal: <a href="../sui/allowance.md#sui_allowance_AllowanceProposal">AllowanceProposal</a>&lt;T&gt;, _: <a href="../sui/allowance.md#sui_allowance_SettingsPermit">SettingsPermit</a>&lt;A&gt;, ctx: &<b>mut</b> TxContext) {
    <b>let</b> <a href="../sui/allowance.md#sui_allowance_AllowanceProposal">AllowanceProposal</a>(<a href="../sui/allowance.md#sui_allowance_settings">settings</a>) = proposal;
    <b>assert</b>!(<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_app">app</a>.contains(&type_name::with_defining_ids&lt;A&gt;()), <a href="../sui/allowance.md#sui_allowance_EWrongApp">EWrongApp</a>);
    <a href="../sui/allowance.md#sui_allowance_share_new">share_new</a>&lt;T&gt;(<a href="../sui/allowance.md#sui_allowance_settings">settings</a>, ctx);
}
</code></pre>
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_balance_spend">balance_spend</a>&lt;C&gt;(
    ...
    <b>assert</b>!(self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_app">app</a>.is_none(), <a href="../sui/allowance.md#sui_allowance_EHasApp">EHasApp</a>);
...
App path: authorized by a <code><a href="../sui/allowance.md#sui_allowance_SpendPermit">SpendPermit</a>&lt;A&gt;</code> matching the allowance's <code><a href="../sui/allowance.md#sui_allowance_app">app</a></code>;
the tx must still come from the spender.
...
App-only: for an app-bound allowance the app dictates who the spender is, so
it is the one that can change it.
For non-app allowances, the only way to rotate is leveraging address aliases.
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_rotate_spender">rotate_spender</a>&lt;T, A&gt;(
    ...
    self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_spender">spender</a> = option::some(new_spender);
}
</code></pre>
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_settings">settings</a>&lt;T&gt;(self: &<a href="../sui/allowance.md#sui_allowance_Allowance">sui::allowance::Allowance</a>&lt;T&gt;): &<a href="../sui/allowance.md#sui_allowance_Settings">sui::allowance::Settings</a>
</code></pre>
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_settings">settings</a>&lt;T&gt;(self: &<a href="../sui/allowance.md#sui_allowance_Allowance">Allowance</a>&lt;T&gt;): &<a href="../sui/allowance.md#sui_allowance_Settings">Settings</a> { &self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a> }
</code></pre>
...
<a name="sui_allowance_current_spend"></a>

## Function `current_spend`



<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_current_spend">current_spend</a>&lt;T&gt;(self: &<a href="../sui/allowance.md#sui_allowance_Allowance">sui::allowance::Allowance</a>&lt;T&gt;): u256
</code></pre>
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_current_spend">current_spend</a>&lt;T&gt;(self: &<a href="../sui/allowance.md#sui_allowance_Allowance">Allowance</a>&lt;T&gt;): u256 { self.<a href="../sui/allowance.md#sui_allowance_current_spend">current_spend</a> }
</code></pre>
...
<a name="sui_allowance_proposed_settings"></a>

## Function `proposed_settings`



<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_proposed_settings">proposed_settings</a>&lt;T&gt;(self: &<a href="../sui/allowance.md#sui_allowance_AllowanceProposal">sui::allowance::AllowanceProposal</a>&lt;T&gt;): &<a href="../sui/allowance.md#sui_allowance_Settings">sui::allowance::Settings</a>
</code></pre>
...
<pre><code><b>public</b> <b>fun</b> <a href="../sui/allowance.md#sui_allowance_proposed_settings">proposed_settings</a>&lt;T&gt;(self: &<a href="../sui/allowance.md#sui_allowance_AllowanceProposal">AllowanceProposal</a>&lt;T&gt;): &<a href="../sui/allowance.md#sui_allowance_Settings">Settings</a> { &self.0 }
</code></pre>
...
<code>(1970, 1, 1)</code>. This is Howard Hinnant's <code>civil_from_days</code>, where the
derivation of every constant here is documented:
https://howardhinnant.github.io/date_algorithms.html#civil_from_days

The unsigned-only form works because chain timestamps are never pre-epoch.
...
<pre><code><b>fun</b> <a href="../sui/allowance.md#sui_allowance_assert_app">assert_app</a>&lt;T, A&gt;(self: &<a href="../sui/allowance.md#sui_allowance_Allowance">Allowance</a>&lt;T&gt;) {
    ...
    <b>assert</b>!(self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_app">app</a>.contains(&<a href="../sui/allowance.md#sui_allowance_app">app</a>), <a href="../sui/allowance.md#sui_allowance_EWrongApp">EWrongApp</a>);
}
</code></pre>
...
<pre><code><b>fun</b> <a href="../sui/allowance.md#sui_allowance_consume">consume</a>&lt;T: store&gt;(
    ...
    <b>assert</b>!(self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_spender">spender</a>.contains(&ctx.sender()), <a href="../sui/allowance.md#sui_allowance_ENotSpender">ENotSpender</a>);
    // Defense-in-depth check <b>as</b> this should already be verified at signing.
    <b>assert</b>!(inner.owner() == self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_funder">funder</a>, <a href="../sui/allowance.md#sui_allowance_EWrongFunder">EWrongFunder</a>);
    <b>let</b> amount = inner.limit();
    <b>let</b> now = <a href="../sui/clock.md#sui_clock">clock</a>.timestamp_ms();
    self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_start_timestamp_ms">start_timestamp_ms</a>.do_ref!(|<a href="../sui/allowance.md#sui_allowance_start_timestamp_ms">start_timestamp_ms</a>| {
    ...
    self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_expiration_timestamp_ms">expiration_timestamp_ms</a>.do_ref!(|<a href="../sui/allowance.md#sui_allowance_expiration_timestamp_ms">expiration_timestamp_ms</a>| {
    ...
    self.<a href="../sui/allowance.md#sui_allowance_current_spend">current_spend</a> = self.<a href="../sui/allowance.md#sui_allowance_current_spend">current_spend</a> + amount;
    self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_lifetime_cap">lifetime_cap</a>.do_ref!(|<a href="../sui/allowance.md#sui_allowance_lifetime_cap">lifetime_cap</a>| {
        <b>assert</b>!(self.<a href="../sui/allowance.md#sui_allowance_current_spend">current_spend</a> &lt;= *<a href="../sui/allowance.md#sui_allowance_lifetime_cap">lifetime_cap</a>, <a href="../sui/allowance.md#sui_allowance_EExceedsLifetimeCap">EExceedsLifetimeCap</a>);
    });
    self.<a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_rate_limit">rate_limit</a>.do_mut!(|<a href="../sui/allowance.md#sui_allowance_rate_limit">rate_limit</a>| <a href="../sui/allowance.md#sui_allowance_rate_limit">rate_limit</a>.<a href="../sui/allowance.md#sui_allowance_charge">charge</a>(amount, now));
    inner
}
</code></pre>
...
<pre><code><b>fun</b> <a href="../sui/allowance.md#sui_allowance_index_at">index_at</a>(self: &<a href="../sui/allowance.md#sui_allowance_Window">Window</a>, anchor_ms: u64, now_ms: u64): u64 {
    match (self) {
        Window::Periodic { period_ms } =&gt; (now_ms - anchor_ms) / *period_ms,
        Window::Calendar { months } =&gt; <a href="../sui/allowance.md#sui_allowance_elapsed_windows">elapsed_windows</a>(anchor_ms, now_ms, *months),
    }
}
</code></pre>
...
<pre><code><b>fun</b> <a href="../sui/allowance.md#sui_allowance_elapsed_windows">elapsed_windows</a>(anchor_ms: u64, now_ms: u64, months: u8): u64 {
    ...
    // month ends: a Jan 31 anchor renews Feb 28.
...
Builds and validates a settings. Rejects allowances that are unbounded in
amount or in time: without both, a funder could hand out authority that
never stops growing or never expires.
...
<pre><code><b>fun</b> <a href="../sui/allowance.md#sui_allowance_share_new">share_new</a>&lt;T&gt;(<a href="../sui/allowance.md#sui_allowance_settings">settings</a>: <a href="../sui/allowance.md#sui_allowance_Settings">sui::allowance::Settings</a>, ctx: &<b>mut</b> <a href="../sui/tx_context.md#sui_tx_context_TxContext">sui::tx_context::TxContext</a>)
</code></pre>
...
<pre><code><b>fun</b> <a href="../sui/allowance.md#sui_allowance_share_new">share_new</a>&lt;T&gt;(<a href="../sui/allowance.md#sui_allowance_settings">settings</a>: <a href="../sui/allowance.md#sui_allowance_Settings">Settings</a>, ctx: &<b>mut</b> TxContext) {
    <b>let</b> <a href="../sui/allowance.md#sui_allowance_funder">funder</a> = <a href="../sui/allowance.md#sui_allowance_settings">settings</a>.<a href="../sui/allowance.md#sui_allowance_funder">funder</a>;
    <b>let</b> <a href="../sui/allowance.md#sui_allowance">allowance</a> = <a href="../sui/allowance.md#sui_allowance_Allowance">Allowance</a>&lt;T&gt; {
        ...
        <a href="../sui/allowance.md#sui_allowance_settings">settings</a>,
        <a href="../sui/allowance.md#sui_allowance_current_spend">current_spend</a>: 0,
    };
