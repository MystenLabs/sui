...
module bridge::bridge {
    use std::vector;
    use std::option::{Self, none, Option, some};
    ...
    fun load_inner(self: &Bridge): &BridgeInner {
        let version = versioned::version(&self.inner);
    ...
    fun claim_token_internal<T>(
        ...
        // get owner address
        let addr_bytes = message::target_address(&token_payload);
        assert!(vector::length(&addr_bytes) == 32, EInvalidAddress);
        let owner = address::from_bytes(addr_bytes);
        ...
        let target_chain = message::target_chain(&token_payload);
        ...
        // check token type
        assert!(treasury::token_id<T>() == message::type(&token_payload), EUnexpectedTokenType);
        // claim from treasury
        let token = treasury::mint<T>(&mut inner.treasury, message::amount(&token_payload), ctx);
        // Record changes
        record.claimed = true;
