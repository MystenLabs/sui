...
public fun append<Element>(lhs: &mut vector<Element>, other: vector<Element>) {
    let len = lhs.length();
    lhs.replace_range(len, len, other).destroy_empty();
}
...
public fun remove<Element>(v: &mut vector<Element>, i: u64): Element {
    ...
    let mut removed = v.remove_range(i, i + 1);
...
public fun insert<Element>(v: &mut vector<Element>, e: Element, i: u64) {
    ...
    v.replace_range(i, i, vector[e]).destroy_empty();
}
...
/// Retains only `v[start..end)`, moving the retained elements to the beginning of `v` and
/// dropping all others.
/// Aborts if `start > end` or `end > v.length()`.
public native fun keep_range<T: drop>(v: &mut vector<T>, start: u64, end: u64);

/// Retains the first `n` elements of `v`. If `n >= v.length()` the vector is left
/// unchanged.
public fun truncate<T: drop>(v: &mut vector<T>, n: u64) {
    if (n < v.length()) v.keep_range(0, n)
}

/// Removes `v[i..j)` and returns it, preserving the order of the remaining elements.
/// Aborts if `i > j` or `j > v.length()`.
public fun remove_range<Element>(v: &mut vector<Element>, i: u64, j: u64): vector<Element> {
    v.replace_range(i, j, vector[])
}

/// Copies `v[i..j)` into a new vector; `v` is untouched.
/// Aborts if `i > j` or `j > v.length()`.
public native fun copy_range<Element: copy>(v: &vector<Element>, i: u64, j: u64): vector<Element>;
...
/// by `other.length() - (j - i)`.
/// Aborts if `i > j` or `j > v.length()`.
public native fun replace_range<Element>(
