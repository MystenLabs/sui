...
module std::u8_tests {
    ...
    fun test_pow() {
        ...
        assert_eq!(0u8.pow(2), 0);
        assert_eq!(1u8.pow(255), 1);
        assert_eq!(2u8.pow(7), slow_pow!(2u8, 7));
    ...
    fun test_sqrt() {
        ...
        let cases: vector<u8> = vector[0, 2, 5, 8, 11, 14];
        ...
        let mut cases: vector<u8> = vector[2, 3, 4, 5, 6];
        while (!cases.is_empty()) {
            let case = cases.pop_back();
            let prev = case - 1;
