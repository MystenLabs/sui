...
/// Signing verifies that source against the shared `Allowance` and hands the
/// transaction an `AllowanceWithdrawal`.
///
...
public use fun proposed_settings as AllowanceProposal.settings;
...
///
/// The inner `Withdrawal` is never handed out on its own: every spend path
...
/// Enables withdrawing `T` from a funder's balance, within limits enforced.
...
public struct Settings has drop, store {
    ...
    /// An optional recurring cap, applied on top of `lifetime_cap`. At least
    ...
    /// bookkeeping. The `name` can be at most 128 bytes.
    name: String,
}
...
///
/// Created with the allowance and destroyed with it: `revoke` consumes both, so
...
public enum RateLimit has copy, drop, store {
    /// At most `limit` per window. Windows only roll forward, so their
    ...
    Windowed {
        ...
        /// Which window `spent` is accumulated in, numbered from the anchor
        /// (0 = first). A spend landing in a later one resets `spent`.
        index: u64,
...
public enum Window has copy, drop, store {
    /// Windows of exactly `period_ms`.
    Periodic { period_ms: u64 },
    /// Windows of `months` civil (UTC) months, renewing on the anchor's
...
/// A `SettingsPermit<A>` authorizes changing what an allowance bound to `A` *is*
/// -- issuing one, or rotating its spender. It is issued from an
/// `internal::Permit<A>`.
///
/// Kept distinct from `SpendPermit` so an app can hand out the right to spend
...
public fun periodic_rate_limit(period_ms: u64, limit: u256): RateLimit {
    ...
    new_rate_limit(limit, Window::Periodic { period_ms })
}

/// At most `limit` per `months` civil (UTC) months, counted from the first
...
public fun calendar_rate_limit(months: u8, limit: u256): RateLimit {
    ...
    new_rate_limit(limit, Window::Calendar { months })
}
...
/// Creation is always `entry` to ensure only top-level PTB
...
/// Proposes an allowance bound to the controlling app `A`.
///
/// Unlike `new`, this does not create anything: `A`'s module must accept the
/// proposal via `issue`, so an app always gets a say in the allowances that
...
/// Accepts a proposal on `A`'s behalf, creating and sharing the allowance.
...
/// App path: authorized by a `SpendPermit<A>` matching the allowance's `app`;
...
public fun settings<T>(self: &Allowance<T>): &Settings { &self.settings }

public fun current_spend<T>(self: &Allowance<T>): u256 { self.current_spend }

public fun proposed_settings<T>(self: &AllowanceProposal<T>): &Settings { &self.0 }
...
/// `(1970, 1, 1)`. This is Howard Hinnant's `civil_from_days`, where the
...
fun index_at(self: &Window, anchor_ms: u64, now_ms: u64): u64 {
    match (self) {
        ...
        Window::Calendar { months } => elapsed_windows(anchor_ms, now_ms, *months),
    }
}
...
/// Stands in for the runtime-issued reservation; the protocol creates these,
/// Move code cannot.
#[test_only]
