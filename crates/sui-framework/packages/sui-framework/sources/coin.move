...
module sui::coin {
	...
	public struct DenyCap<phantom T> has key, store { id: UID }
	...
	public fun create_currency<T: drop>(
		...
		(
			TreasuryCap {id: object::new(ctx), total_supply: balance::create_supply(witness)},
			CoinMetadata {
				id: object::new(ctx),
				decimals,
	...
	public fun mint<T>(cap: &mut TreasuryCap<T>, value: u64, ctx: &mut TxContext): Coin<T> {
		Coin {id: object::new(ctx), balance: cap.total_supply.increase_supply(value)}
	}
	...
	public fun deny_list_add<T>(
		...
		deny_list::add(deny_list, DENY_LIST_COIN_INDEX, `type`, addr)
	}
	...
	public fun deny_list_remove<T>(
		...
		let `type` = type_name::into_string(type_name::get_with_original_ids<T>()).into_bytes();
		deny_list::remove(deny_list, DENY_LIST_COIN_INDEX, `type`, addr)
	}

	/// Returns true iff the given address is denied for the given coin type. It will
	/// return false if given a non-coin type.
	public fun deny_list_contains<T>(freezer: &DenyList, addr: address): bool {
	...
	public fun create_treasury_cap_for_testing<T>(ctx: &mut TxContext): TreasuryCap<T> {
		TreasuryCap {id: object::new(ctx), total_supply: balance::create_supply_for_testing()}
	}
	...
	#[allow(unused_field)]
	public struct CurrencyCreated<phantom T> has copy, drop { decimals: u8 }
}
