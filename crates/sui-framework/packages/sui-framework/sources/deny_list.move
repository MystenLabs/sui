...
module sui::deny_list {
	...
	fun per_type_list_add(list: &mut PerTypeList, `type`: vector<u8>, addr: address) {
	...
	fun per_type_list_remove(list: &mut PerTypeList, `type`: vector<u8>, addr: address) {
		let denied_addresses = &mut list.denied_addresses[`type`];
	...
	fun per_type_list_contains(list: &PerTypeList, `type`: vector<u8>, addr: address): bool {
	...
	fun create(ctx: &mut TxContext) {
		...
		let deny_list_object = DenyList {id: object::sui_deny_list_object_id(), lists};
