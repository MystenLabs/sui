...
module sui::dynamic_field {
	...
	public fun remove<Name: copy + drop + store, Value: store>(
		...
		let object_addr = object.to_address();
		let hash = hash_type_and_key(object_addr, name);
		let Field { id, name: _, value } = remove_child_object<Field<Name, Value>>(
			object_addr,
			hash,
		);
		id.delete();
	...
	public(package) fun field_info<Name: copy + drop + store>(object: &UID, name: Name): (
		&UID,
		address,
	) {
		let object_addr = object.to_address();
	...
	public(package) fun field_info_mut<Name: copy + drop + store>(object: &mut UID, name: Name): (
		&mut UID,
		address,
	) {
		let object_addr = object.to_address();
