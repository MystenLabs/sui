...
public fun claim<OTW: drop>(otw: OTW, ctx: &mut TxContext): Publisher {
    ...
    let typename = type_name::get_with_original_ids<OTW>();

    Publisher {
        id: object::new(ctx),
        package: typename.get_address(),
...
public fun from_module<T>(self: &Publisher): bool {
    let typename = type_name::get_with_original_ids<T>();

    (typename.get_address() == self.package) && (typename.get_module() == self.module_name)
}
