...
module sui::token {
	...
	public fun new_policy<T>(_treasury_cap: &TreasuryCap<T>, ctx: &mut TxContext): (
		TokenPolicy<T>,
		TokenPolicyCap<T>,
	) {
		...
		let cap = TokenPolicyCap {id: object::new(ctx), `for`: object::id(&policy)};
	...
	public fun share_policy<T>(policy: TokenPolicy<T>) {
		event::emit(TokenPolicyCreated<T> {id: object::id(&policy), is_mutable: true});
	...
	public fun transfer<T>(t: Token<T>, recipient: address, ctx: &mut TxContext): ActionRequest<T> {
		...
		new_request(transfer_action(), amount, option::some(recipient), option::none(), ctx)
	}
	...
	public fun spend<T>(t: Token<T>, ctx: &mut TxContext): ActionRequest<T> {
		...
		new_request(spend_action(), balance.value(), option::none(), option::some(balance), ctx)
	}
	...
	public fun to_coin<T>(t: Token<T>, ctx: &mut TxContext): (Coin<T>, ActionRequest<T>) {
		...
		(
			...
			new_request(to_coin_action(), amount, option::none(), option::none(), ctx)
		)
	}
	...
	public fun from_coin<T>(coin: Coin<T>, ctx: &mut TxContext): (Token<T>, ActionRequest<T>) {
		...
		let token = Token {id: object::new(ctx), balance: coin.into_balance()};

		(token, new_request(from_coin_action(), amount, option::none(), option::none(), ctx))
	}
	...
	public fun confirm_request<T>(
		...
		let ActionRequest { name, approvals, spent_balance, amount, sender, recipient } = request;
	...
	public fun action<T>(self: &ActionRequest<T>): String {
		self.name
	}

	/// Amount of the `ActionRequest`.
	public fun amount<T>(self: &ActionRequest<T>): u64 {
		self.amount
	}

	/// Sender of the `ActionRequest`.
	public fun sender<T>(self: &ActionRequest<T>): address {
		self.sender
	}
	...
	fun key<Rule>(): RuleKey<Rule> {
		RuleKey {is_protected: true}
	}
