...
module sui::zklogin_verified_iss_tests {
    use sui::zklogin_verified_iss::check_zklogin_iss;
    use sui::address;
    use std::string::utf8;
    ...
    fun test_check_zklogin_iss() {
        ...
        assert!(!check_zklogin_iss(other_address, address_seed, &iss), 1);

        let other_address_seed = 1234u256;
        assert!(!check_zklogin_iss(address, other_address_seed, &iss), 2);

        let other_iss = utf8(b"https://other.issuer.com");
        assert!(!check_zklogin_iss(address, address_seed, &other_iss), 3);
    }
}