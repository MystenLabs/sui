...
public(package) fun new(
    ...
    init_active_validators.do_ref!(|validator| {
        staking_pool_mappings.add(validator.staking_pool_id(), validator.sui_address());
    });
...
public(package) fun request_add_validator_candidate(
    ...
    self
        ...
        .add(validator.sui_address(), validator_wrapper::create_v1(validator, ctx));
}
...
public(package) fun request_remove_validator_candidate(
    ...
    self.inactive_validators.add(staking_pool_id, validator_wrapper::create_v1(validator, ctx));
}
...
public(package) fun assert_no_pending_or_active_duplicates(
    ...
    let duplicates = self.active_validators.count!(|v| v.is_duplicate(validator));

    // Validator here must be active or pending, and thus must be identified as duplicate exactly once.
    assert!(
        duplicates + count_duplicates_tablevec(&self.pending_active_validators, validator) == 1,
        EDuplicateValidator,
    );
}
...
public fun derive_reference_gas_price(self: &ValidatorSet): u64 {
    let entries = self.active_validators.map_ref!(|v| {
...
public(package) fun is_duplicate_validator(
    ...
    validators.any!(|v| v.is_duplicate(new_validator))
}
...
/// Returns Some(index) if the validator is found; if not found, returns None.
...
fun get_active_or_pending_or_candidate_validator_mut(
    ...
    if (validator_index_opt.is_some()) {
        let validator_index = validator_index_opt.extract();
        let mut_ref = &mut self.active_validators[validator_index];
        return mut_ref
    };
    ...
    if (validator_index_opt.is_some()) {
        let validator_index = validator_index_opt.extract();
        let mut_ref = &mut self.pending_active_validators[validator_index];
        return mut_ref
    };
...
fun get_validator_ref(validators: &vector<Validator>, validator_address: address): &Validator {
    let index = find_validator(validators, validator_address).destroy_or!(abort ENotAValidator);
    &validators[index]
}
...
fun process_pending_removals(
    ...
    sort_removal_list(&mut self.pending_removals); // TODO: do we need to sort?
    self.pending_removals.destroy!(|index| {
    ...
    // destroy! macro does not work as expected on struct fields;
    self.pending_removals = vector[];
}
...
fun process_validator_departure(
    ...
    self.inactive_validators.add(validator_pool_id, validator_wrapper::create_v1(validator, ctx));
    removed_stake
}
...
fun clean_report_records_leaving_validator(
    ...
    reported_validators.do_ref!(|reported_validator_addr| {
...
fun process_pending_stakes_and_withdraws(validators: &mut vector<Validator>, ctx: &TxContext) {
    validators.do_mut!(|validator| validator.process_pending_stakes_and_withdraws(ctx));
}
...
fun compute_reward_adjustments(
    ...
    slashed_validator_indices.destroy!(|validator_index| {
        ...
        individual_storage_fund_reward_adjustments.insert(
            validator_index,
            storage_fund_reward_adjustment_u128 as u64,
        );
        total_storage_fund_reward_adjustment =
            total_storage_fund_reward_adjustment + (storage_fund_reward_adjustment_u128 as u64);
    });
...
fun compute_slashed_validators(
    ...
    let (addresses, reporters) = validator_report_records.into_keys_values();
    addresses.zip_do!(reporters, |validator_address, reporters| {
        assert!(
...
fun compute_unadjusted_reward_distribution(
    ...
    let storage_fund_reward_per_validator = total_storage_fund_reward / validators.length();
    validators.length().do!(|i| {
...
fun emit_validator_epoch_events(
    ...
    validators.length().do!(|i| {
        ...
        let is_slashed = slashed_validators.contains(&validator_address);
        let tallying_rule_global_score = if (is_slashed) 0 else 1;
...
public fun sum_voting_power_by_addresses(vs: &vector<Validator>, addresses: &vector<address>): u64 {
    ...
    addresses.do_ref!(|addr| {
        sum = sum + get_validator_ref(vs, *addr).voting_power();
    });
    sum
}
...
public(package) fun active_validator_addresses(self: &ValidatorSet): vector<address> {
    self.active_validators.map_ref!(|v| v.sui_address())
}
