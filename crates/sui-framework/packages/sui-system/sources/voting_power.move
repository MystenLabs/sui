...
public(package) fun set_voting_power(validators: &mut vector<Validator>) {
    ...
    let threshold = TOTAL_VOTING_POWER.min(MAX_VOTING_POWER.max(TOTAL_VOTING_POWER.divide_and_round_up(validators.length())));
...
fun update_voting_power(
    ...
    mut info_list: vector<VotingPowerInfoV2>,
) {
