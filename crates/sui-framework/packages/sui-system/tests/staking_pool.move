...
module sui_system::staking_pool_tests {
    ...
    fun test_process_pending_stake_withdraw_no_underflow() {
        ...
        assert!(distribute_rewards_and_advance_epoch(&mut staking_pool, &mut test, 0) == 1);
        ...
        assert!(staking_pool.sui_balance() == 0);
        assert!(staking_pool.pending_total_sui_withdraw() == 0);
        assert!(staking_pool.pool_token_balance() == 0);
        assert!(staking_pool.pending_pool_token_withdraw() == 0);
