...
module sui::hex {
    ...
    // Mapping of decimal versions of hex-relevant characters for base 16 hex conversion 48 = 0, 65 = A, 97 = a
    const BASE16: vector<u8> = vector[
    ...
    public fun decode(hex: vector<u8>): vector<u8> {
        ...
        while (i < l) {
            let decimal = (base_16_index(vector::borrow(&hex, i)) * 16) + 
                          base_16_index(vector::borrow(&hex, i + 1));
    ...
    fun base_16_index(base_16_value: &u8): u8 {
        let (found, base16) = vector::index_of(&BASE16, base_16_value);
        assert!(found, ENotValidHexCharacter);
        if (base16 >= 16) {
            base16 = base16 - 6;
        };
        (base16 as u8)
    ...
    #[test]
    fun test_hex_decode_string_literal_lowercase_and_uppercase() {
        assert!(x"ff" == decode(b"Ff"), 0);
        assert!(x"ff" == decode(b"fF"), 0);
        assert!(x"ff" == decode(b"FF"), 0);
    }
    ...
    fun test_hex_decode_hex_literal() {
        decode(x"ff");
    }

    #[test]
    #[expected_failure(abort_code = ENotValidHexCharacter)]
    fun test_hex_decode_invalid_string_literal() {
        decode(b"0g");
    }
}
