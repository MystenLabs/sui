...
pub fn ecvrf_verify(
    ...
    let public_key = match bincode::deserialize(public_key_bytes.as_bytes_ref().as_ref()) {
        Ok(pk) => pk,
        Err(_) => return Ok(NativeResult::err(cost, INVALID_ECVRF_PUBLIC_KEY)),
    };

    let proof: ECVRFProof = match bincode::deserialize(proof_bytes.as_bytes_ref().as_ref()) {
        Ok(p) => p,
        Err(_) => return Ok(NativeResult::err(cost, INVALID_ECVRF_PROOF)),
    };

    let result = proof.verify_output(alpha_string.as_bytes_ref().as_ref(), &public_key, hash);
    Ok(NativeResult::ok(
        cost,
