...
module sui::collection_tests {
    ...
    fun test_collection() {
        ...
        test_scenario::next_tx(scenario, &sender);
        {
            ...
            collection::add(&mut collection, obj1, test_scenario::ctx(scenario));
            collection::add(&mut collection, obj2, test_scenario::ctx(scenario));
    ...
    fun test_collection_bag_interaction() {
        ...
        test_scenario::next_tx(scenario, &sender);
        {
            ...
            let item = test_scenario::take_child_object<Collection<Object>, collection::Item<Object>>(scenario, &collection);

            let obj = collection::remove(&mut collection, item);
            let id = *id::id(&obj);
            bag::add(&mut bag, obj, test_scenario::ctx(scenario));
        ...
        test_scenario::next_tx(scenario, &sender);
        {
            let collection = test_scenario::take_owned<Collection<Object>>(scenario);
            let bag = test_scenario::take_owned<Bag>(scenario);
            let item = test_scenario::take_child_object<Bag, bag::Item<Object>>(scenario, &bag);

            let obj = bag::remove(&mut bag, item);
            let id = *id::id(&obj);
            collection::add(&mut collection, obj, test_scenario::ctx(scenario));
