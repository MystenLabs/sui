...
module sui::package_tests {
    ...
    fun check_ticket(cap: &mut UpgradeCap, policy: u8, digest: vector<u8>): UpgradeTicket {
        ...
        test_utils::assert_eq(package::get_upgrade_policy(&ticket), policy);
        test_utils::assert_eq(package::get_upgrade_digest(&ticket), digest);
        ticket
    }
    ...
    fun test_full_upgrade_flow() {
        ...
        test_utils::assert_eq(package::get_upgrade_policy(&ticket), package::dep_only_policy());
        let receipt = package::test_upgrade(ticket);
        package::commit_upgrade(&mut cap, receipt);
    ...
    fun test_failure_to_commit_upgrade_to_wrong_cap() {
        ...
        test_utils::assert_eq(package::get_upgrade_policy(&ticket1), package::dep_only_policy());
