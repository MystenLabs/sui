...
impl Service {
    ...
    pub async fn main(self) -> Result<(), Error> {
        self.run(GRACE, terminate).await
    }

    /// Runs the service, waiting for interrupt signals from the operating system to trigger
    /// graceful shutdown, within a `grace` period.
    pub async fn main_with_grace_period(self, grace: Duration) -> Result<(), Error> {
        self.run(grace, terminate).await
    }

    /// Run the service, waiting for the provided `terminate` future to complete to trigger
    /// graceful shutdown, within a `grace` period.
    async fn run<T: Future<Output = ()>>(
        mut self,
        grace: Duration,
...
mod tests {
    ...
    async fn test_completion() {
        ...
        atx.send(()).unwrap();
        svc.run(GRACE, std::future::pending).await.unwrap();
        assert!(btx.is_closed());
    }
    ...
    async fn test_failure() {
        ...
        let res = svc.run(GRACE, std::future::pending).await;
    ...
    async fn test_panic() {
        ...
        let _ = svc.run(GRACE, std::future::pending).await;
    }
    ...
    async fn test_graceful_shutdown() {
        ...
        let handle = tokio::spawn(svc.run(GRACE, move || srx.clone().notified_owned()));
    ...
    async fn test_multiple_tasks() {
        ...
        let handle = tokio::spawn(svc.run(GRACE, std::future::pending));
