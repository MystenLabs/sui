...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let tx: Vec<TransactionDigest> = client
        .read_api()
        .get_transactions_in_range(0, 10)
        .await
        .unwrap();
    assert_eq!(4, tx.len());
    ...
    let first_page = client
        .full_node_api()
        .get_transactions(TransactionQuery::All, None, Some(2))
        .await
        .unwrap();
    ...
    let second_page = client
        .full_node_api()
        .get_transactions(TransactionQuery::All, first_page.next_cursor, None)
        .await
        .unwrap();
    assert_eq!(2, second_page.data.len());
    ...
    // test get latest transactions paged
    let latest = client
        .full_node_api()
        .get_transactions(TransactionQuery::Latest, None, None)
        .await
        .unwrap();
    assert_eq!(4, latest.data.len());
    assert_eq!(all_txs_rev, latest.data);

    // test get_recent_transactions
    let tx = client.read_api().get_recent_transactions(10).await.unwrap();
    assert_eq!(4, tx.len());
