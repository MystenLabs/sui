...
impl GasInput {
    ...
    async fn gas_payment(
        ...
        // is not a Move object (i.e a package). Even though the transaction would fail to run, it
        // will still be indexed. On loading the transaction, if the return type of this field is a
        // `MoveObject`, then GraphQL will fail on the top-level with an internal error. Instead, we
        // return an `Object` here, so that the rest of the `TransactionBlock` will still be viewable.
