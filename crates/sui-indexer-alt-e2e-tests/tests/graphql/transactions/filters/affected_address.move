...
{ # A should be affected by all the transactions above
  affectsA: transactions(filter: { affectedAddress: "@{A}" }) {
    nodes { ...ObjectChanges, sender { address } }
  }

  # B was not affected by all the transactions
  affectsB: transactions(filter: { affectedAddress: "@{B}" }) {
    nodes { ...ObjectChanges }
  }

  # And neither was C
  affectsC: transactions(filter: { affectedAddress: "@{C}" }) {
    nodes { ...ObjectChanges }
  }
}

fragment ObjectChanges on Transaction {
  effects {
    objectChanges {
      edges {
        cursor
