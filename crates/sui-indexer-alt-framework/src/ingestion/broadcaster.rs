    ...
    S: StreamingService + Send + 'static,
{
    tokio::spawn(async move {
        ...
        'outer: while checkpoint_hi < end_cp {
            ...
            'streaming_decision: {
                let Some(streaming_service) = &mut streaming_service else {
                    // No streaming service configured, so we just use ingestion for the entire range.
                    break 'streaming_decision (dummy_streaming_handle!(end_cp), end_cp)
                };
                ...
                if tokio::time::Instant::now() < streaming_backoff_until {
                    ...
                    break 'streaming_decision (dummy_streaming_handle!(ingestion_end), ingestion_end)
                }
