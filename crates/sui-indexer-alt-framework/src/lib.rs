...
pub struct Indexer<S: Store> {
    ...
    /// The minimum next_checkpoint across all pipelines that participate in commit_hi feedback.
    /// This is used to initialize the regulator to prevent ingestion from running too far ahead.
...
impl<S: Store> Indexer<S> {
    ...
    /// The minimum next checkpoint across all pipelines that participate in commit_hi feedback.
    /// This value is used to initialize the ingestion regulator's high watermark to prevent
    /// ingestion from running too far ahead.
    pub fn initial_commit_hi(&self) -> Option<u64> {
        self.initial_commit_hi
    }
