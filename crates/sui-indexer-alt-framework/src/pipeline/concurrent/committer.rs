    ...
    Service::new().spawn_aborting(async move {
        ...
        let make_task = |BatchedRows {
            ...
            async move {
                ...
                let result = async {
                    // Double check that the commit actually went through, (this backoff
                    // should not produce any permanent errors, but if it does, we need to
        ...
        let result = if let Some((_, limit_rx)) = aimd {
            ...
            ReceiverStream::new(rx)
                .try_for_each_spawned(config.write_concurrency, make_task)
                .await
        };
