...
mod tests {
    ...
    pub struct StoredData {
        ...
        pub commit_failure_remaining: Arc<Mutex<usize>>,
    ...
    impl Default for StoredData {
        fn default() -> Self {
            Self {
                ...
                commit_failure_remaining: Arc::new(Mutex::new(0)),
                commit_delay_ms: 0,
            }
        }
    }
    ...
    impl Handler for DataPipeline {
        ...
        async fn commit<'a>(
            ...
            for value in values {
                ...
                {
                    let mut remaining = value.commit_failure_remaining.lock().unwrap();
                    if *remaining > 0 {
                        *remaining -= 1;
    ...
    async fn test_commit_with_retries_for_commit_failure() {
        ...
        let batch = BatchedRows {
            values: vec![StoredData {
                cp_sequence_number: 1,
                tx_sequence_numbers: vec![1, 2, 3],
                commit_failure_remaining: Arc::new(Mutex::new(1)),
