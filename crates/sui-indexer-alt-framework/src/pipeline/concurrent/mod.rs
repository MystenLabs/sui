...
mod integration_tests {
    ...
    impl Handler for DataPipeline {
        ...
        async fn commit<'a>(
            ...
            if conn.0.commit_delay_ms > 0 {
                tokio::time::sleep(Duration::from_millis(conn.0.commit_delay_ms)).await;
            }
            ...
            // Check for commit failure simulation
            {
                let prev = conn
                    ...
                    prev >= conn.0.commit_failures.failures,
                    "Transaction failed, remaining failures: {}",
                    conn.0.commit_failures.failures - prev
                );
            }
            ...
            if !values.is_empty() {
                ...
                for value in values {
                    data.entry(value.checkpoint).or_default().push(value.data);
                }
            }
            Ok(values.len())
        }
        ...
        async fn prune<'a>(
            ...
            // Check for prune failure simulation
            let should_fail =
                if let Some(failures) = conn.0.prune_failure_attempts.get(&(from, to_exclusive)) {
                    let prev = failures.attempts.fetch_add(1, Ordering::Relaxed);
