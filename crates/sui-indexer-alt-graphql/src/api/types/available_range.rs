...
impl AvailableRange {
    ...
    pub(crate) fn new(
        ...
        // Get the current watermark snapshot
        let checkpoint_hi = watermarks.high_watermark().checkpoint();
...
fn pipeline(retention_key: RetentionKey) -> Result<String, RpcError<Error>> {
    ...
    match (
        ...
        ("Query", "transactions", _) => Ok("tx_digests".to_string()),
        ("Query", "checkpoints", _) => Ok("tx_digests".to_string()),
        ("Query", "events", _) => Ok("tx_digests".to_string()),
