...
impl ExecutionError {
    ...
    async fn instruction_offset(&self) -> Result<Option<u64>, RpcError> {
        match &self.native {
            ExecutionFailureStatus::MoveAbort(location, _) => Ok(Some(location.instruction as u64)),
            ExecutionFailureStatus::MovePrimitiveRuntimeError(location_opt) => {
                Ok(location_opt.0.as_ref().map(|loc| loc.instruction as u64))
            }
            _ => Ok(None),
        }
    }
