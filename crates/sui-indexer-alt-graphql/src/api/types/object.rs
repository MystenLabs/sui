...
impl Object {
    ...
    pub(crate) async fn by_key(
        ...
        if bounds > 1 {
            ...
            Ok(Self::at_version(ctx, scope, key.address, v)
                .await
                .map_err(upcast)?)
        } else if let Some(v) = key.root_version {
            Ok(Self::version_bounded(ctx, scope, key.address, v)
                .await
                .map_err(upcast)?)
            ...
            Ok(Self::checkpoint_bounded(ctx, scope, key.address, cp)
                .await
                .map_err(upcast)?)
        } else {
            Ok(Self::latest(ctx, scope, key.address)
                .await
                .map_err(upcast)?)
        }
    }
    ...
    pub(crate) async fn latest(
        ...
        Self::checkpoint_bounded(ctx, scope, address, cp).await
    }
        ...
        self.contents
            .get_or_try_init(async || {
                ...
                let version = if let Some((version, _)) = self.version_digest {
                    ...
                    // For now, we'll use the checkpoint_bounded method to get the version.
                    let cp: UInt53 = self.super_.scope.checkpoint_viewed_at().into();
