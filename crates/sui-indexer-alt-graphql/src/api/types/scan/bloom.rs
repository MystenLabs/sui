...
/// Pre-computed probes to avoid repeated hashing in SQL.
...
/// Probe for per-checkpoint bloom filters.
struct CpBloomProbe {
    byte_offsets: Vec<usize>,
    bit_masks: Vec<usize>,
}
...
fn cp_bloom_block_probes(
    ...
    for cp_block_id in cp_block_lo..=cp_block_hi {
        ...
        for key in filter_values {
            let (idx, bits) = BlockedBloomFilter::hash(
                key,
                seed,
                NUM_BLOOM_BLOCKS,
                NUM_HASHES,
    ...
    let values = probes
        ...
        .map(|p| {
            format!(
                ...
                p.cp_block_id,
                p.bloom_block_idx,
                p.byte_offsets.iter().join(","),
                p.bit_masks.iter().join(",")
            )
        })
        .join(", ");
