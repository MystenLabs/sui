...
impl From<SystemParametersV1> for SystemParameters {
    fn from(value: SystemParametersV1) -> Self {
        Self {
            ...
            validator_very_low_stake_threshold: Some(
                value.validator_very_low_stake_threshold.into(),
            ),
...
impl From<SystemParametersV2> for SystemParameters {
    fn from(value: SystemParametersV2) -> Self {
        Self {
            duration_ms: Some(value.epoch_duration_ms.into()),
            stake_subsidy_start_epoch: Some(value.stake_subsidy_start_epoch.into()),
            // TODO min validator count can be extracted, but it requires some JSON RPC changes,
