...
pub(crate) async fn transactions(
    ...
    let candidate_cps = candidate_cps(ctx, &filter_values, cp_bounds, page).await?;
...
///
/// If filter values are empty, returns checkpoints in the bounds ordered by the page, without filtering.
async fn candidate_cps<C: CpBoundsCursor>(
    ctx: &Context<'_>,
    filter_values: &[[u8; 32]],
    cp_bounds: RangeInclusive<u64>,
    page: &Page<C>,
