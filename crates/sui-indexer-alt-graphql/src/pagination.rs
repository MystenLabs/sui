...
/// Ensures ASC order DESC order, then applies pagination limits with overhead for paginate_results to check
/// for has_prev and has_next page. Returns items in ascending order as GraphQL cursor syntax expects items
...
/// );
/// ```
pub(crate) fn ordered_results_with_limits<Result, SortKey, KeyFn, Cursor>(
    page: &Page<Cursor>,
...
where
    SortKey: Ord,
    KeyFn: Fn(&Result) -> SortKey,
{
    let mut results: Vec<Result> = results.into_iter().collect();

    if results.is_empty() {
        return results;
    }

    // Ensures items are in ASC or DESC order based on page.is_from_front().
    results.sort_by(|a, b| {
        let key_a = key_fn(a);
        let key_b = key_fn(b);
    ...
    // Truncate to limit with overhead
    results.truncate(page.limit_with_overhead());

    // Reverse if not from front to maintain ascending order for GraphQL cursor syntax.
    if !page.is_from_front() {
        results.reverse();
    }
...
mod tests {
    ...
    // Test cursor similar to EventCursor
    ...
    // Test item that contains cursor-like fields
    ...
    #[test]
    fn test_ordered_results_with_limits_empty() {
        let limits = PageLimits {
            default: 5,
            max: 100,
        };
    ...
    #[test]
    fn test_ordered_results_with_limits_front() {
        let limits = PageLimits {
            default: 2,
            max: 100,
        };
        let page: Page<JsonCursor<TestCursor>> =
            Page::from_params(&limits, Some(2), None, None, None).unwrap();
        let items = vec![
            TestItem {
            ...
            TestItem {
                tx_sequence_number: 5,
                ev_sequence_number: 0,
            },
            TestItem {
                tx_sequence_number: 5,
                ev_sequence_number: 2,
            },
            TestItem {
    ...
    #[test]
    fn test_ordered_results_with_limits_back() {
        let limits = PageLimits {
            default: 2,
            max: 100,
        };
        let page: Page<JsonCursor<TestCursor>> =
            Page::from_params(&limits, None, None, Some(2), None).unwrap();
        let items = vec![
            TestItem {
                tx_sequence_number: 5,
                ev_sequence_number: 1,
