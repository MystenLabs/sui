...
//!     LruPkgStore → PackageCache (shared) → DB
//! ```

mod checkpoint_stream_task;
mod lru_package_store;
...
pub(crate) use lru_package_store::LruPackageStore;
pub(crate) use package_eviction_task::EvictionQueue;
...
/// streamed index → LRU → shared PackageCache → DB.
pub(crate) type StreamingPackageStore = StreamedPackageStore<LruPackageStore<Arc<PackageCache>>>;
