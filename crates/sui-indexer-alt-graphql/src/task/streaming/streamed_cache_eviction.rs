...
mod tests {
    ...
    async fn evicts_up_to_its_pipeline_watermark() {
        let cache = MockCache::new("bigtable");
        let watermarks: WatermarksLock = Default::default();
        set_watermarks(&watermarks, &[("bigtable", 5)]).await;
    ...
    async fn flushes_each_cache_by_its_own_pipeline() {
        ...
        let transactions = MockCache::new("bigtable");
        let watermarks: WatermarksLock = Default::default();
        set_watermarks(&watermarks, &[("kv_packages", 4), ("bigtable", 2)]).await;
