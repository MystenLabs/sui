...
impl StreamedOverlays {
    ...
    /// Each overlay store as an [`EvictableCache`], for registration with the eviction task. Eviction
    /// stays per-store: each drains against its own watermark pipeline.
    pub(crate) fn evictable_caches(&self) -> Vec<Arc<dyn EvictableCache>> {
