...
use crate::task::watermark::BIGTABLE_PIPELINE;
use crate::task::watermark::KV_TRANSACTIONS_PIPELINE;
...
pub(crate) struct StreamedTransactionStore {
    ...
    /// The watermark pipeline of the KV backend `KvLoader` reads, whose progress decides when an
    /// entry is safe to drop.
    watermark_pipeline: &'static str,
}

impl StreamedTransactionStore {
    pub(crate) fn new(kv_loader: &KvLoader) -> Self {
        let watermark_pipeline = match kv_loader {
            KvLoader::Bigtable(_) => BIGTABLE_PIPELINE,
            KvLoader::LedgerGrpc(_) => LEDGER_GRPC_PIPELINE,
            KvLoader::Pg(_) => KV_TRANSACTIONS_PIPELINE,
        };
        Self {
...
mod tests {
    ...
    fn store() -> StreamedTransactionStore {
        StreamedTransactionStore {
            cache: StreamedStore::new(),
            watermark_pipeline: "bigtable",
        }
    }
