...
/// The OpenRPC document describing this service, served via `rpc.discover`.
///
/// The document was frozen from the last schema generated by the (since removed) `open_rpc`
/// macros. It must be updated by hand when a method's signature, documentation, or types change.
const OPENRPC_SCHEMA: &str = include_str!("../openrpc.json");
...
pub struct RpcService {
    ...
    /// The OpenRPC document served by this service, see [OPENRPC_SCHEMA].
    schema: Value,
}
...
impl RpcService {
    ...
    pub fn new(rpc_args: RpcArgs, registry: &Registry) -> anyhow::Result<Self> {
        ...
        let mut schema: Value =
            serde_json::from_str(OPENRPC_SCHEMA).context("Failed to parse OpenRPC schema")?;
        schema["info"]["version"] = json!(env!("CARGO_PKG_VERSION"));
    ...
    pub async fn run(self) -> anyhow::Result<Service> {
        ...
        // The frozen document describes every module this service can serve. Trim it down to
        // the methods that were actually registered (for example the write API is only present
        // when a fullnode is configured), so discovery does not advertise methods that would be
        // rejected as not found.
        let registered: BTreeSet<&str> = modules.method_names().collect();
        let described: BTreeSet<&str> = schema["methods"]
            .as_array()
            .into_iter()
            .flatten()
            .filter_map(|method| method["name"].as_str())
            .collect();
        for method in registered.difference(&described) {
            warn!("Method {method} is served but is not described by the OpenRPC schema");
        }
        if let Some(methods) = schema["methods"].as_array_mut() {
        ...
        modules
            .register_method("rpc.discover", move |_, _, _| schema.clone())
...
mod tests {
    ...
    async fn test_rpc_discovery() {
        ...
        // `Foo` is not described by the frozen schema, `FrozenCheckpoints` serves a method that
        // is, and the schema describes plenty of methods that neither of them serve.
        rpc.add_module(Foo).unwrap();
        rpc.add_module(FrozenCheckpoints).unwrap();
        ...
        let frozen: Value = serde_json::from_str(OPENRPC_SCHEMA).unwrap();
        let expected_method = frozen["methods"]
            .as_array()
            .unwrap()
            .iter()
            .find(|method| method["name"] == "sui_getCheckpoint")
            .expect("Frozen schema should describe sui_getCheckpoint");

        assert_eq!(resp["result"]["openrpc"], frozen["openrpc"]);
        assert_eq!(resp["result"]["info"]["title"], "Sui JSON-RPC");
        assert_eq!(
            resp["result"]["info"]["version"],
            env!("CARGO_PKG_VERSION"),
            "Served version should track the binary, not the frozen document",
        );
        assert_eq!(resp["result"]["methods"], json!([expected_method]));
        assert_eq!(
            resp["result"]["components"]["schemas"],
            frozen["components"]["schemas"],
        );
    ...
    /// Serves a method that is described by the frozen OpenRPC schema.
    #[rpc(server, namespace = "sui")]
    trait FrozenCheckpointsApi {
        #[method(name = "getCheckpoint")]
        fn get_checkpoint(&self) -> RpcResult<u64>;
    }
    ...
    impl FrozenCheckpointsApiServer for FrozenCheckpoints {
        fn get_checkpoint(&self) -> RpcResult<u64> {
            Ok(46)
        }
    }
    ...
    impl RpcModule for FrozenCheckpoints {
        fn into_impl(self) -> jsonrpsee::RpcModule<Self> {
            self.into_rpc()
        }
    }
