...
impl<V: Send> BatchStrategy<V> for SingleRowBatchStrategy<V> {
    ...
    fn take_from(&mut self, source: &mut Vec<V>) -> usize {
        if self.value.is_none() && !source.is_empty() {
            // Take from the front using split_off + swap pattern
            let mut remainder = source.split_off(1);
            std::mem::swap(source, &mut remainder);
            self.value = remainder.into_iter().next();
            return 1;
        }
        0
    }
