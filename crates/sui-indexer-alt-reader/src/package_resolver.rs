...
/// The shared LRU-cached package store. Boxed so the backend (Postgres or ledger gRPC) is picked
/// at start-up without changing the type consumers inject.
pub type PackageCache = PackageStoreWithLruCache<Box<dyn PackageStore>>;

pub struct DbPackageStore(Arc<DataLoader<PgReader>>);

/// A package store served by the ledger gRPC service (kv-rpc or fullnode). A package's storage ID
/// identifies exactly one immutable version, so a plain `BatchGetObjects` with no version answers
/// the fetch.
        ...
        for obj_result in batch_response.objects {
            ...
            let object: Object = object
                ...
                .ok_or_else(|| Error::Store {
                    store: GRPC_STORE,
                    error: "Missing bcs in object".to_string(),
                })?
                .deserialize()
                .map_err(|e| Error::Store {
                    store: GRPC_STORE,
                    error: format!("Failed to deserialize object: {e}"),
                })?;
            let Some(move_package) = object.data.try_as_package() else {
                return Err(Error::NotAPackage(object.id().into()));
            };
