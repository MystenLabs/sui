...
use sui_types::{effects::TransactionEffectsAPI, storage::ObjectKey};
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> Result<Vec<Self::Value>> {
        ...
        let created_objects = checkpoint.transactions.iter().flat_map(|txn| {
            txn.effects
                .all_changed_objects()
                .into_iter()
                .filter_map(move |(object_ref, _, _)| {
                    let id = object_ref.0;
                    let version = object_ref.1;
                    checkpoint.object_set.get(&ObjectKey(id, version)).map(|o| {
                        Ok(StoredObject {
                            object_id: id.to_vec(),
