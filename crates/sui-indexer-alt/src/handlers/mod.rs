...
pub(crate) fn checkpoint_input_objects(
    ...
    for tx in checkpoint.transactions.iter() {
        for change in tx.effects.object_changes() {
            ...
            // Look up input object from checkpoint.object_set
            // We need to find the object with this id and version
            let input_obj = checkpoint
                .object_set
                .iter()
                .find(|obj| obj.id() == id && obj.version() == version)
                .with_context(|| {
                    format!(
                    "Object {id} at version {version} referenced in effects not found in object_set"
                )
                })?;
