...
use crate::{
    db,
    models::objects::{StoredObjectUpdate, StoredOwnerKind, StoredSumObjType, UpdateKind},
    ...
    fn process(&self, checkpoint: &Arc<CheckpointData>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in transactions.iter().rev() {
            ...
            for change in tx.effects.object_changes() {
                ...
                let kind = match (change.input_version, change.output_version) {
                    // Unwrapped and deleted
                    (None, None) => continue,

                    // Unwrapped or created
                    (None, Some(_)) => UpdateKind::Insert,

                    // Wrapped or deleted
                    (Some(_), None) => UpdateKind::Delete,

                    // Mutated
                    (Some(_), Some(_)) => UpdateKind::Update,
                };
