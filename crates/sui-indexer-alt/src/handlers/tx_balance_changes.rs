...
fn balance_changes(
    ...
    for (object_id, version) in transaction.effects.modified_at_versions() {
        // We need to find the object with this id and version in the object_set
        if let Some(object) = checkpoint
            .object_set
            .iter()
            .find(|obj| obj.id() == object_id && obj.version() == version)
        {
    ...
    for (object_ref, _, _) in transaction.effects.all_changed_objects() {
        let (id, version, _digest) = object_ref;
        if let Some(object) = checkpoint.object_set.get(&ObjectKey(id, version)) {
            if let Some((type_, balance)) = Coin::extract_balance_if_coin(object)? {
                *changes.entry((object.owner(), type_)).or_insert(0i128) += balance as i128;
            }
        }
