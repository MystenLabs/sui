...
#[allow(dead_code)] // TODO: Remove this once we have a use for this struct.
...
impl ObjectFilterQueryBuilder {
    pub fn new(view_checkpoint: i64) -> Self {
        Self {
            package: None,
            module: None,
            name: None,
    ...
    pub fn with_type_filter(mut self, type_filter: TypeFilter) -> Self {
        match type_filter {
            TypeFilter::ByPackage(package) => self.package = Some(package),
            TypeFilter::ByModule(package, module) => {
                self.package = Some(package);
                self.module = Some(module);
            }
            TypeFilter::ByFullType(struct_tag) => {
    ...
    pub fn build(self, limit: usize, cursor_mode: Option<CursorMode>) -> String {
        ...
        format!(
            ...
            ON
                ...
                AND o.cp_sequence_number <= {view_checkpoint}
            WHERE
                o.object_id IS NULL
            ORDER BY
...
/// Bound the query by a view checkpoint number.
...
fn build_object_ids_query(
    ...
    let mut builder = ObjectFilterQueryBuilder::new(view_checkpoint);
            ...
            format!(
                "SELECT * FROM (
                    ...
                    WHERE object_id = '\\x{}'::bytea
                      AND cp_sequence_number <= {view_checkpoint_number}
...
mod tests {
    ...
    async fn test_objects_filter_query_plan_efficiency() {
        ...
        let filters = ObjectFilter {
            type_filter: Some(TypeFilter::ByFullType(StructTag {
        ...
        let query_without_cursor = build_object_ids_query(filters.clone(), 1000, None, 50);
    ...
    async fn test_latest_object_versions_query_plan_efficiency() {
        ...
        let view_checkpoint_number = 1000;
