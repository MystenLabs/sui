    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    async fn index_objects(
        ...
        let objects_size = latest_objects.len();
        info!(objects_size, "Got checkpoint object size");
        ...
        let history_objects: Vec<Object> = data
            ...
            .collect();
        let history_object_size = history_objects.len();
        info!(history_object_size, "checkpoint object history size");

        // make a map from struct_tag to move_struct_layout -- this has to be async
        let struct_tags = history_objects
            .iter()
            .filter_map(|o| {
            ...
            .collect::<Vec<_>>();
        // TODOggao: remove this before commit
        let struct_tags = struct_tags.into_iter().unique().collect::<Vec<_>>();
        let struct_tags_size = struct_tags.len();
        info!(struct_tags_size, "Got struct tags");
        info!("struct_tags are {:?}", struct_tags);
        let move_struct_layout_futures = struct_tags
            ...
            .map(|struct_tag| {
                ...
                async move {
                    ...
                    Ok::<(move_core_types::language_storage::StructTag, move_core_types::annotated_value::MoveStructLayout), IndexerError>((struct_tag, move_struct_layout))
                }
            })
            .collect::<Vec<_>>();
        ...
        let changed_objects = history_objects
            .into_iter()
            .map(|o| {
                let df_info = try_create_dynamic_field_info(&o, &move_struct_layout_map, &latest_objects);
    ...
    // TODOggao: see if indexing of objects and objects_history can share some execution, or at least some codes.
