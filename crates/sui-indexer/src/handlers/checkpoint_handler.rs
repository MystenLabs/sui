    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
        ...
        loop {
            ...
            let indexed_checkpoint_epoch_vec = join_all(downloaded_checkpoints.iter().map(
                |downloaded_checkpoint| async {
                    self.index_checkpoint(downloaded_checkpoint, /* index_epoch */ true).await
                },
            ))
            .await
            .into_iter()
    ...
    async fn start_download_and_index(&self) -> Result<(), IndexerError> {
        ...
        loop {
            ...
            let indexed_checkpoint_epoch_vec = join_all(downloaded_checkpoints.iter().map(
                |downloaded_checkpoint| async {
                    self.index_checkpoint(downloaded_checkpoint, /* index_epoch */ false).await
                },
            ))
            .await
            .into_iter()
    ...
    async fn index_checkpoint(
        ...
        // NOTE: Index epoch when object index has reached the same checkpoint,
        // because epoch info is based on the latest system state by the current checkpoint.
