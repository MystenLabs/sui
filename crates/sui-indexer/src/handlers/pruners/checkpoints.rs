...
impl Prunable for Checkpoints {
    const NAME: PrunableTable = PrunableTable::Checkpoints;

    const CHUNK_SIZE: u64 = 100_000;

    async fn data_lo(conn: &mut Connection<'_>) -> anyhow::Result<u64> {
        Ok(checkpoints::table
    ...
    async fn prune(
        ...
        execute_delete_range_query!(&mut conn, checkpoints, sequence_number, prune_lo, prune_hi)
