...
use crate::{
    ...
    schema::event_emit_package,
};

use super::Prunable;

pub struct EventEmitPackage;

#[async_trait::async_trait]
impl Prunable for EventEmitPackage {
    const NAME: PrunableTable = PrunableTable::EventEmitPackage;

    const CHUNK_SIZE: u64 = 100_000;

    async fn data_lo(conn: &mut Connection<'_>) -> anyhow::Result<u64> {
        Ok(event_emit_package::table
            .select(max(event_emit_package::tx_sequence_number))
