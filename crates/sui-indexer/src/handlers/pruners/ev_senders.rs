...
impl Prunable for EventSenders {
    const NAME: PrunableTable = PrunableTable::EventSenders;

    const CHUNK_SIZE: u64 = 100_000;

    async fn data_lo(conn: &mut Connection<'_>) -> anyhow::Result<u64> {
        Ok(event_senders::table
            .select(max(event_senders::tx_sequence_number))
