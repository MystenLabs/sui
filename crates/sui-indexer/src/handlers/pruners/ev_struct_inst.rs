...
use crate::{
    ...
    schema::event_struct_instantiation,
};

use super::Prunable;

pub struct EventStructInstantiation;

#[async_trait::async_trait]
impl Prunable for EventStructInstantiation {
    const NAME: PrunableTable = PrunableTable::EventStructInstantiation;

    const CHUNK_SIZE: u64 = 100_000;

    async fn data_lo(conn: &mut Connection<'_>) -> anyhow::Result<u64> {
        Ok(event_struct_instantiation::table
            .select(max(event_struct_instantiation::tx_sequence_number))
