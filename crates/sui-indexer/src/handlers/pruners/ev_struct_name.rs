...
use crate::{
    ...
    schema::event_struct_name,
};

use super::Prunable;

pub struct EventStructName;

#[async_trait::async_trait]
impl Prunable for EventStructName {
    const NAME: PrunableTable = PrunableTable::EventStructName;

    const CHUNK_SIZE: u64 = 100_000;

    async fn data_lo(conn: &mut Connection<'_>) -> anyhow::Result<u64> {
        Ok(event_struct_name::table
            .select(max(event_struct_name::tx_sequence_number))
