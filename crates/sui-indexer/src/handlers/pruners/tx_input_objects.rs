...
use crate::{
    ...
    schema::tx_input_objects,
};

use super::Prunable;

pub struct TxInputObjects;

#[async_trait::async_trait]
impl Prunable for TxInputObjects {
    const NAME: PrunableTable = PrunableTable::TxInputObjects;

    const CHUNK_SIZE: u64 = 100_000;

    async fn data_lo(conn: &mut Connection<'_>) -> anyhow::Result<u64> {
        Ok(tx_input_objects::table
            .select(max(tx_input_objects::tx_sequence_number))
