...
impl From<TxIndex> for StoredTxIndex {
    fn from(tx: TxIndex) -> Self {
        ...
        let flattened_input_objects: Vec<u8> = input_objects.into_iter().filter_map(|opt| opt).flatten().collect();
        let flattened_changed_objects: Vec<u8> = changed_objects.into_iter().filter_map(|opt| opt).flatten().collect();
        let flattened_packages: Vec<u8> = packages.into_iter().filter_map(|opt| opt).flatten().collect();
        let flattened_package_modules: Vec<u8> = package_modules.into_iter().filter_map(|opt| opt).map(|s| s.into_bytes()).flatten().collect();
        let flattened_package_module_functions: Vec<u8> = package_module_functions.into_iter().filter_map(|opt| opt).map(|s| s.into_bytes()).flatten().collect();
        let flattened_payers: Vec<u8> = payers.into_iter().filter_map(|opt| opt).flatten().collect();
        let flattened_senders: Vec<u8> = senders.into_iter().filter_map(|opt| opt).flatten().collect();
        let flattened_recipients: Vec<u8> = recipients.into_iter().filter_map(|opt| opt).flatten().collect();
