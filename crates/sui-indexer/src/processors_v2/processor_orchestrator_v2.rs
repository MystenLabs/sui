        ...
        loop {
            ...
            let network_metrics_handle = tokio::task::spawn(async move {
                ...
                if let Err(e) = network_metrics_res {
                    std::time::sleep(std::time::Duration::from_secs(5));
                    error!(
            ...
            let processor_orchestrator_res =
                try_join_all(vec![network_metrics_handle, addr_metrics_handle]).await;
            if let Err(e) = processor_orchestrator_res {
                std::time::sleep(std::time::Duration::from_secs(10));
                error!(
