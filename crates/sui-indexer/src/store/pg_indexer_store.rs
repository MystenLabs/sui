...
use crate::models::checkpoints::{IndexedCheckpoint, StoredCheckpoint};
use crate::models::epoch::{StoredEndOfEpochInfo, StoredEpochInfo};
use crate::models::events::{IndexedEvent, StoredEvent};
// use crate::models::network_metrics::{DBMoveCallMetrics, DBNetworkMetrics};
use crate::models::objects::{
    compose_object_bulk_insert_update_query, filter_latest_objects, IndexedObject,
};
...
use crate::models::tx_indices::{StoredTxIndex, TxIndex};
use crate::schema::{
    checkpoints,
    epochs, events, objects, transactions, tx_indices,
};
...
impl PgIndexerStore {
    ...
    fn get_checkpoint_ending_tx_sequence_number(
        ...
        .context(
            format!(
                "Failed reading checkpoint ending tx sequence number {:?} from PostgresDB",
                seq_num
            )
            .as_str(),
        )
    ...
    fn get_indexer_checkpoint(&self) -> Result<IndexedCheckpoint, IndexerError> {
        read_only_blocking!(&self.blocking_cp, |conn| {
            checkpoints::dsl::checkpoints
    ...
    fn get_checkpoints(
        ...
        read_only_blocking!(&self.blocking_cp, |conn| {
            let cp_vec: Vec<IndexedCheckpoint> = match cursor {
    ...
    fn get_event(&self, id: EventID) -> Result<StoredEvent, IndexerError> {
        unimplemented!();
        // read_only_blocking!(&self.blocking_cp, |conn| events::table
        //     .filter(events::dsl::transaction_digest.eq(id.tx_digest.base58_encode()))
        //     .filter(events::dsl::event_sequence_number.eq(id.event_seq as i64))
        //     .first::<Event>(conn))
    ...
    fn get_transaction_by_digest(&self, tx_digest: &str) -> Result<StoredTransaction, IndexerError> {
        read_only_blocking!(&self.blocking_cp, |conn| {
    ...
    fn multi_get_transactions_by_digests(
        ...
        //     .iter()
        //     .map(|s| TransactionDigest::from_str(s).map(|td| td.into_inner().to_vec()))
        //     .collect::<Result<Vec<_>, _>>()?;
    ...
    //             let digest = TransactionDigest::from_str(digest)?.into_inner().to_vec();
    ...
    fn persist_object_changes(
        &self,
        tx_object_changes: Vec<TransactionObjectChanges>,
        object_mutation_latency: Histogram,
        object_deletion_latency: Histogram,
        object_commit_chunk_counter: IntCounter,
    ) -> Result<(), IndexerError> {
        // FIXME: dedeup
        let (mutated_objects, deleted_objects) = Self::get_latest_objects(tx_object_changes);
    ...
    fn get_latest_objects(tx_object_changes: Vec<TransactionObjectChanges>) -> (Vec<IndexedObject>, HashSet<ObjectID>) {
    ...
    fn persist_events(&self, events: &[IndexedEvent]) -> Result<(), IndexerError> {
        transactional_blocking!(&self.blocking_cp, |conn| {
            let events = events
                .into_iter()
                .map(|event| StoredEvent::from(event))
                .collect::<Vec<_>>();
    ...
    fn persist_transaction_index_tables(
        ...
        transactional_blocking!(&self.blocking_cp, |conn| {
            let tx_indices = tx_indices
                .into_iter()
                .map(|ti| {
                    let (packages, modules_and_functions): (Vec<_>, Vec<_>) = ti
                        .move_calls
                        .iter()
                        .map(|(p, m, f)| {
                            (
                                p.to_vec(),
                                (
                                    format!("{}::{}", p.to_string(), m),
                                    format!("{}::{}::{}", p.to_string(), m, f),
                                ),
                            )
                        })
                        .unzip();

                    let (package_modules, package_module_functions): (Vec<_>, Vec<_>) =
                        modules_and_functions.into_iter().unzip();

                    StoredTxIndex {
                        tx_sequence_number: ti.tx_sequence_number as i64,
                        transaction_digest: ti.transaction_digest.into_inner().to_vec(),
                        input_objects: ti.input_objects.iter().map(|o| o.to_vec()).collect(),
                        changed_objects: ti.changed_objects.iter().map(|o| o.to_vec()).collect(),
                        senders: ti.senders.iter().map(|s| s.to_vec()).collect(),
                        recipients: ti.recipients.iter().map(|r| r.to_vec()).collect(),
                        packages,
                        package_modules,
                        package_module_functions,
                    }
                })
                .collect::<Vec<_>>();
            for tx_indices_chunk in tx_indices.chunks(PG_COMMIT_CHUNK_SIZE) {
                diesel::insert_into(tx_indices::table)
                    .values(tx_indices_chunk)
                    .on_conflict_do_nothing()
                    .execute(conn)
                    .map_err(IndexerError::from)
                    .context("Failed writing tx_indices to PostgresDB")?;
            }
    ...
    fn persist_epoch(&self, data: &TemporaryEpochStore) -> Result<(), IndexerError> {
        ...
        transactional_blocking!(&self.blocking_cp, |conn| {
            if let Some((epoch_total_transactions, end_of_epoch_info)) = &data.last_epoch {
                let last_epoch_id = data.new_epoch.epoch - 1;
                info!("Persisting at the end of epoch {}", last_epoch_id);
                let end_of_epoch = StoredEndOfEpochInfo {
                    epoch: last_epoch_id as i64,
                    epoch_total_transactions,
                    end_of_epoch_info: bcs::to_bytes(&end_of_epoch_info).unwrap(),
                };
                diesel::insert_into(epochs::table)
                    ...
                    .set((
                        ...
                        epochs::epoch_total_transactions.eq(excluded(epochs::epoch_total_transactions)),
                    ))
                    .execute(conn)?;
                info!("Persisted epoch {}", last_epoch_id);
            }
    ...
    fn get_epochs(
        ...
        let epoch_info: Vec<StoredEpochInfo> = read_only_blocking!(&self.blocking_cp, |conn| query
            .limit(limit as i64)
            .load(conn))
        .map_err(|e| {
    ...
    fn get_current_epoch(&self) -> Result<EpochInfo, IndexerError> {
        let epoch_info: StoredEpochInfo = read_only_blocking!(&self.blocking_cp, |conn| {
            epochs::dsl::epochs
                .order_by(epochs::epoch.desc())
...
impl IndexerStore for PgIndexerStore {
    ...
    // async fn get_checkpoint_starting_tx_sequence_number(&self) -> Result<u64, IndexerError> {
    //     self.spawn_blocking(move |this| {
    //         this.query_objects_history(filter, at_checkpoint, cursor, limit)
    //     })
    // }
...
fn persist_object_deletions(
    ...
    diesel::delete(objects::dsl::objects(objects::object_id.eq_any(deleted_objects)))
        .execute(conn)?;
