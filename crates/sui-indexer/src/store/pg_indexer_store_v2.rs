...
impl PgIndexerStoreV2 {
    pub fn new(blocking_cp: DbConnectionPool, metrics: IndexerMetrics) -> Self {
        ...
        let parallel_objects_chunk_size = std::env::var("PG_COMMIT_OBJECTS_PARALLEL_CHUNK_SIZE")
            ...
            .parse::<usize>()
            .unwrap();
        info!(parallel_chunk_size, "Using parallel chunk size for commits");
        info!(
    ...
    fn persist_objects_chunk(
        ...
        let _ = transactional_blocking_v2!(&self.blocking_cp, |mut conn| {
        ...
        transactional_blocking_v2!(&self.blocking_cp, |mut conn| {
            // Persist deleted objects
            for deleted_objects_chunk in deleted_object_ids.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX)
            {
                diesel::delete(
    ...
    fn persist_epoch(&self, data: &Vec<EpochToCommit>) -> Result<(), IndexerError> {
        ...
        transactional_blocking_v2!(&self.blocking_cp, |mut conn| {
            for epoch_data in data {
                ...
                let new_epoch = StoredEpochInfo::from_epoch_beginning_info(&epoch_data.new_epoch);
                diesel::insert_into(epochs::table)
                    .values(new_epoch)
                    .on_conflict_do_nothing()
                    .execute(&mut conn)?;
