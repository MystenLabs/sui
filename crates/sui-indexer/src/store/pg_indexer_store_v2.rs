...
use sui_json_rpc_types::TransactionBlocksPage;
...
use sui_json_rpc_types::{
    BalanceChange, DynamicFieldPage, ObjectChange, SuiTransactionBlock, SuiTransactionBlockEffects,
...
use crate::types_v2::{
    IndexedCheckpoint, IndexedEvent, IndexedObjectChange, IndexedPackage, IndexedTransaction,
    IndexerResult, OwnerType, TxIndex,
};
...
impl PgIndexerStoreV2 {
    ...
    fn persist_epoch(
        &self,
        data: &Vec<TemporaryEpochStoreV2>,
        metrics: IndexerMetrics,
    ) -> Result<(), IndexerError> {
        if data.is_empty() {
            return Ok(());
        }
        let guard = metrics.checkpoint_db_commit_latency_epochs.start_timer();
        transactional_blocking_with_retry!(
            &self.blocking_cp,
            |conn| {
                for epoch_data in data {
                    ...
                    let new_epoch = StoredEpochInfo::from(&epoch_data.new_epoch);
                    diesel::insert_into(epochs::table)
                        .values(new_epoch)
                        .on_conflict_do_nothing()
                        .execute(conn)?;
...
impl IndexerStoreV2 for PgIndexerStoreV2 {
    ...
    async fn get_dynamic_field_object(
        ...
        SuiObjectResponse::try_from((object_read, SuiObjectDataOptions::bcs_lossless())).map_err(
            |e| {
                IndexerError::DynamicFieldError(format!(
    ...
    async fn query_transaction_blocks(
        ...
        limit: usize,
        _descending_order: Option<bool>,
    ) -> IndexerResult<TransactionBlocksPage> {
        let mut results = read_only_blocking!(&self.blocking_cp, |conn| {
            transactions::dsl::transactions
                .order(transactions::dsl::tx_sequence_number.desc())
                .limit((limit + 1) as i64)
                .load::<StoredTransaction>(conn)
        })
        .context("Failed query transaction blocks")?
        .into_iter()
        .map(|stored_tx| {
            stored_tx.try_into_sui_transaction_block_response(self.module_cache.as_ref())
        })
        .collect::<IndexerResult<Vec<_>>>()?;

        let next_cursor = results.get(limit).map(|o| o.digest);
