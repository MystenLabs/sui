...
impl<T: R2D2Connection> PgPartitionManager<T> {
    ...
    pub fn advance_and_prune_epoch_partition(
        ...
        if last_partition == data.last_epoch {
            ...
            let sql = format!("ALTER TABLE {table_name} REORGANIZE PARTITION {table_name}_partition_{last_epoch} INTO (PARTITION {table_name}_partition_{last_epoch} VALUES LESS THAN ({next_epoch_start_cp}), PARTITION {table_name}_partition_{next_epoch} VALUES LESS THAN MAXVALUE)", table_name = table.clone(), last_epoch = data.last_epoch as i64, next_epoch_start_cp = data.next_epoch_start_cp as i64, next_epoch = data.next_epoch as i64);
            ...
            if let Some(epochs_to_keep) = epochs_to_keep {
                for epoch in first_partition..(data.next_epoch - epochs_to_keep + 1) {
                    ...
                    let sql = format!("ALTER TABLE {} DROP PARTITION partition_{}", table.clone(), epoch);
                    #[cfg(feature = "mysql-feature")]
                    #[cfg(not(feature = "postgres-feature"))]
                    transactional_blocking_with_retry!(
                        &self.cp,
