...
//! progress watermark from the floor. Leaves only ever advance at the floor
//! (peeked one bucket ahead, polled concurrently), so no branch can run ahead of
//! the others: the resume cursor stays within one sparse read of every leaf, and
//! there is no windowing/parking to get wrong. Consumed by streaming backends
//! such as BigTable; the synchronous [`super::eval_bitmap_query_bucket_iter`]
//! mirrors it and shares the per-bucket evaluation ([`eval_term_at_bucket`]).
