...
pub trait WriteApi {
    ...
    ///     Finality is guaranteed, but the client will not be able to read the objects
    ///     written by the transaction or see the transaction viauntil the full node locally executes the transaction.
    /// 2. WaitForLocalExecution: waits for TransactionEffectsCert *and* make sure the full node
    ...
    /// request_type is default to be `WaitForEffectsCert` unless options.show_balance_changes or options.show_balance_changes is true
