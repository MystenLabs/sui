...
use move_core_types::compressed::annotated::{BackendBuilder as _, LayoutHandle};
...
impl SuiJsonValue {
    ...
    pub fn to_bcs_bytes(&self, ty: &CA::MoveTypeLayout) -> Result<Vec<u8>, anyhow::Error> {
        let move_value = Self::to_move_value(&self.0, ty.as_ref())?;
    ...
    pub fn from_bcs_bytes(
        ...
        let json = if let Some(layout) = layout {
            ...
            fn try_parse_string(layout: CA::MoveTypeLayoutRef<'_>, bytes: &[u8]) -> Option<String> {
            ...
            if let Some(s) = try_parse_string(layout.as_ref(), bytes) {
                json!(s)
            } else {
                BoundedVisitor::deserialize_value(bytes, layout.as_ref()).map_or_else(
                    |_| {
    ...
    fn handle_inner_struct_layout(
        struct_layout: &CA::MoveStructLayout<'_>,
        val: &JsonValue,
        ty: CA::MoveTypeLayoutRef<'_>,
        s: &String,
        ...
        match inner_layout.as_view() {
            CA::MoveLayoutView::Vector(inner) => match inner.as_view() {
                ...
                CA::MoveLayoutView::Address => Ok(R::MoveValue::Struct(R::MoveStruct(vec![
                    Self::to_move_value(val, CA::MoveTypeLayout::address().as_ref())?,
                ]))),
                _ => bail!(
...
pub fn primitive_type(
    ...
    CA::MoveTypeLayoutBuilder::with_builder(|b| {
        primitive_type_(b, view, type_args, param).ok_or_else(|| anyhow!("not a primitive type"))
    })
    .ok()
}
