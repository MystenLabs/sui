...
//! Monotonic reads across load-balanced replicas.
//!
...
//! [`MonotonicReadLayer`] holds the request until this replica's view reaches that checkpoint, and
...
pub const X_SUI_MIN_CHECKPOINT: &str = "x-sui-min-checkpoint";

/// Applies the monotonic-read contract to every request reaching the services below it.
///
...
#[derive(Clone)]
pub(crate) struct MonotonicReadLayer {
    service_info: watch::Receiver<Option<GetServiceInfoResponse>>,
    timeout: Duration,
}

impl MonotonicReadLayer {
    pub(crate) fn new(
        service_info: watch::Receiver<Option<GetServiceInfoResponse>>,
        timeout: Duration,
    ) -> Self {
...
impl<S> Layer<S> for MonotonicReadLayer {
    type Service = MonotonicRead<S>;
...
/// See [`MonotonicReadLayer`].
#[derive(Clone)]
pub(crate) struct MonotonicRead<S> {
    inner: S,
    service_info: watch::Receiver<Option<GetServiceInfoResponse>>,
    timeout: Duration,
}

impl<S, ReqBody, ResBody> Service<http::Request<ReqBody>> for MonotonicRead<S>
where
...
async fn await_checkpoint(
    ...
    let Some(checkpoint) = extract_min_checkpoint(headers)? else {
        return Ok(());
    };
    ...
    Err(Status::unavailable(format!(
        "monotonic read at checkpoint {checkpoint} not available on this replica; retry"
    )))
}

/// The checkpoint `headers` asks the server to have reached, or `None` when the request does not
/// ask for a monotonic read.
fn extract_min_checkpoint(headers: &HeaderMap) -> Result<Option<u64>, Status> {
    let Some(value) = headers.get(X_SUI_MIN_CHECKPOINT) else {
        return Ok(None);
    };
    ...
    let invalid =
        |detail: String| Status::invalid_argument(format!("{X_SUI_MIN_CHECKPOINT}: {detail}"));
