...
fn object_reference_proto((object_id, version, digest): &ObjectRef) -> ObjectReference {
    let mut reference = ObjectReference::default();
    reference.object_id = Some(object_id.to_canonical_string(true));
    reference.version = Some(version.value());
    reference.digest = Some(digest.to_string());
    reference
}
...
fn package_item_response(object_ref: &ObjectRef, watermark: Watermark) -> ListPackagesResponse {
    let mut response = ListPackagesResponse::default();
    response.package = Some(object_reference_proto(object_ref));
    response.watermark = Some(watermark);
    response
}
