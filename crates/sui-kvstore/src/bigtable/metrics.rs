...
pub(crate) struct KvMetrics {
    ...
    pub kv_bt_flow_control_server_feedback_total: IntCounterVec,
    pub kv_bt_flow_control_latency_feedback_total: IntCounterVec,
...
impl KvMetrics {
    pub(crate) fn new(registry: &Registry) -> Arc<Self> {
        Arc::new(Self {
            ...
            .unwrap(),
            kv_bt_flow_control_server_feedback_total: register_int_counter_vec_with_registry!(
                "kv_bt_flow_control_server_feedback_total",
                "Valid Bigtable MutateRows RateLimitInfo messages by factor direction (decrease, neutral, or increase) and client handling outcome (queued or rejected)",
                &["client", "direction", "outcome"],
                registry,
            )
            .unwrap(),
            kv_bt_flow_control_latency_feedback_total: register_int_counter_vec_with_registry!(
                "kv_bt_flow_control_latency_feedback_total",
                "Latency-derived feedback generated by Bigtable adaptive batch-write flow control (decrease, reanchor, or increase)",
                &["client", "direction"],
                registry,
            )
            .unwrap(),
