...
impl GenerationWorker {
    ...
    async fn initialize(&mut self) -> Result<Option<GenerationLoopState>> {
        ...
        let startup_tx_hi = startup.tx_hi;
        ...
        let mut state =
            GenerationLoopState::new(startup_tx_hi, startup_bucket_start_cp, self.seal_fn);
        state.record_generation_started(watermark, framework_commit_time, self.seal_fn);
        Ok(Some(state))
    }
    ...
    async fn try_promote(&self, state: &mut GenerationLoopState) {
        ...
        while (self.seal_fn)(state.next_bucket_to_evict) <= watermark.tx_hi {
...
impl GenerationLoopState {
    fn new(startup_tx_hi: u64, startup_bucket_start_cp: u64, seal_fn: fn(u64) -> u64) -> Self {
        let current_bucket_id = super::bucket_of(startup_tx_hi, seal_fn);
        Self {
            current_bucket_id,
    ...
    fn record_generation_started(
        ...
        while seal_fn(self.current_bucket_id) <= watermark.tx_hi {
...
mod tests {
    ...
    fn bucket_start_cp_tracks_commit_that_crosses_bucket_boundary() {
        let mut state = GenerationLoopState::new(0, 0, seal_fn);

        state.record_generation_started(watermark(1, 5), Instant::now(), seal_fn);
        assert_eq!(state.current_bucket_id, 0);
        assert_eq!(state.generations.get(&1).unwrap().bucket_start_cp, 0);

        state.record_generation_started(watermark(2, 25), Instant::now(), seal_fn);
    ...
    async fn promote_dispatches_latest_contiguous_watermark_and_bucket_seal() {
        ...
        state.record_generation_started(watermark(1, 5), Instant::now(), seal_fn);
        state.record_generation_started(watermark(2, 25), Instant::now(), seal_fn);
        state.record_generation_started(watermark(3, 35), Instant::now(), seal_fn);
