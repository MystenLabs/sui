...
/// surface. `tx_hi` is the persisted watermark (un-clamped: it drives the
/// generation task's bucket identity, which must match `bucket_start_cp`).
...
impl Connection for BigTableConnection<'_> {
    async fn init_watermark(
        ...
        let (checkpoint_hi_inclusive, reader_lo, final_tx_hi, bucket_start_cp) = if write_happened {
            (
                initial.checkpoint_hi_inclusive,
                initial.reader_lo,
                initial.tx_hi,
                initial.bucket_start_cp,
            )
        } else {
            let row = self
                .client
...
mod tests {
    ...
    async fn test_init_watermark_returns_existing_on_conflict() {
        let (emulator, store) = store_conn().await;
        let mut conn = store.connect().await.unwrap();
        ...
        // Simulate a process restart by constructing a fresh `BigTableStore`
        // against the same emulator. `init_watermark` is memoized per-store,
        // so re-reading post-`set_committer_watermark` state in the same
        // process requires a new store handle — the same shape as the real
        // restart path.
        let fresh_client =
            BigTableClient::new_local(emulator.host().to_string(), INSTANCE_ID.into())
                .await
                .unwrap();
        let fresh_store = BigTableStore::new(fresh_client);
        let mut fresh_conn = fresh_store.connect().await.unwrap();
