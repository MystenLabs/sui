...
pub mod col {
    /// Schema version tag for the `v1` per-field cell layout. This cell was not present in v0.
...
/// Bails on unknown schema versions, and on missing required field cells when `v` is present
/// (which would indicate data corruption since all field cells are written atomically with `v`).
/// `chi` is the only optional field.
