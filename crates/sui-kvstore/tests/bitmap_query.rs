...
//! - `eval_bitmap_query` composes AND/OR/NOT/XOR correctly
...
/// Write a tx_seq_digest mapping entry: tx_seq → (digest, checkpoint_seq, event_count=0).
async fn write_tx_seq_digest(
    ...
    let entry = tables::make_entry(
        ...
        tables::tx_seq_digest::encode(digest, checkpoint_seq, 0),
        None,
    );
    client
...
/// Convenience: write tx_seq_digest rows for a run of consecutive tx_seqs all
/// in the same checkpoint.
async fn write_tx_seq_digests_for_checkpoint(
    client: &mut BigTableClient,
    checkpoint_seq: u64,
    tx_lo: u64,
    ...
    for (i, d) in digests.iter().enumerate() {
        write_tx_seq_digest(client, tx_lo + i as u64, d, checkpoint_seq).await?;
    }
    Ok(())
}
...
async fn test_eval_bitmap_query_and() -> Result<()> {
    ...
    let query = BitmapQuery::and(vec![BitmapQuery::scan(dim_a), BitmapQuery::scan(dim_b)]);
    let result = client.eval_bitmap_query(&query, 0..100_000).await?;
...
async fn test_eval_bitmap_query_or() -> Result<()> {
    ...
    let dim_b = encode_dimension_key(IndexDimension::Recipient, &[0xDD; 32]);

    write_bitmap(&mut client, &dim_a, 0, &[1, 2, 3]).await?;
    write_bitmap(&mut client, &dim_b, 0, &[3, 4, 5]).await?;

    let query = BitmapQuery::or(vec![BitmapQuery::scan(dim_a), BitmapQuery::scan(dim_b)]);
    let result = client.eval_bitmap_query(&query, 0..100_000).await?;
...
async fn test_eval_bitmap_query_not() -> Result<()> {
    ...
    let query = BitmapQuery::and(vec![
        BitmapQuery::scan(dim_a),
        BitmapQuery::complement(BitmapQuery::scan(dim_b)),
    ]);
    let result = client.eval_bitmap_query(&query, 0..100_000).await?;
...
#[tokio::test]
async fn test_eval_bitmap_query_xor() -> Result<()> {
    let (mut client, _emulator) = setup_emulator().await?;

    let dim_a = encode_dimension_key(IndexDimension::Sender, &[0x11; 32]);
    let dim_b = encode_dimension_key(IndexDimension::Sender, &[0x22; 32]);

    write_bitmap(&mut client, &dim_a, 0, &[1, 2, 3]).await?;
    write_bitmap(&mut client, &dim_b, 0, &[2, 3, 4]).await?;

    let query = BitmapQuery::xor(BitmapQuery::scan(dim_a), BitmapQuery::scan(dim_b));
    let result = client.eval_bitmap_query(&query, 0..100_000).await?;
...
/// Parity test: `eval_bitmap_query_stream` must yield the same tx_seqs as the
/// non-streaming `eval_bitmap_query` across operator combinations and buckets.
#[tokio::test]
async fn test_eval_bitmap_query_stream_parity() -> Result<()> {
    use futures::StreamExt;

    let (mut client, _emulator) = setup_emulator().await?;

    let dim_a = encode_dimension_key(IndexDimension::Sender, &[0xA1; 32]);
    let dim_b = encode_dimension_key(IndexDimension::MoveCall, &[0xB2; 32]);
    let dim_c = encode_dimension_key(IndexDimension::Recipient, &[0xC3; 32]);

    // Populate three dimensions across two buckets so the streaming
    // merge-join must cross bucket boundaries.
    write_bitmap(&mut client, &dim_a, 0, &[10, 20, 30, 40, 50]).await?;
    write_bitmap(&mut client, &dim_a, 1, &[5, 15]).await?;
    write_bitmap(&mut client, &dim_b, 0, &[20, 30, 40, 60]).await?;
    write_bitmap(&mut client, &dim_b, 1, &[5, 25]).await?;
    write_bitmap(&mut client, &dim_c, 0, &[30, 40, 70]).await?;
    write_bitmap(&mut client, &dim_c, 1, &[5]).await?;

    let range = 0u64..250_000u64;

    let queries: Vec<(&str, BitmapQuery)> = vec![
        (
            "and",
            BitmapQuery::and(vec![
                BitmapQuery::scan(dim_a.clone()),
                BitmapQuery::scan(dim_b.clone()),
            ]),
        ),
        (
            "or",
        ...
        (
            ...
            BitmapQuery::and(vec![
                BitmapQuery::scan(dim_a.clone()),
                BitmapQuery::complement(BitmapQuery::scan(dim_c.clone())),
            ]),
        ),
        (
            "standalone_not",
            BitmapQuery::complement(BitmapQuery::scan(dim_a.clone())),
        ),
        (
            "xor",
            BitmapQuery::xor(
    ...
    for (name, query) in queries {
        let expected = client.eval_bitmap_query(&query, range.clone()).await?;
        let streamed: Vec<u64> = client
            .eval_bitmap_query_stream(query, range.clone())
            .try_collect::<Vec<_>>()
            .await?;
        assert_eq!(streamed, expected, "streaming parity failed for {name}");
    }

    // Edge trimming: range that bisects a bucket.
    let trim_range = 15u64..100_020u64;
    let q = BitmapQuery::scan(dim_a.clone());
    let expected = client.eval_bitmap_query(&q, trim_range.clone()).await?;
    let streamed: Vec<u64> = client
        .eval_bitmap_query_stream(q, trim_range)
        .try_collect::<Vec<_>>()
        .await?;
    assert_eq!(streamed, expected, "streaming parity failed for edge trim");

    // Back-pressure: .take(2) on a large universe should only touch the first
    // few buckets. We can't observe that directly here, but we verify that
    // an early .take produces the expected prefix.
    let q = BitmapQuery::or(vec![
        BitmapQuery::scan(dim_a.clone()),
        BitmapQuery::scan(dim_b.clone()),
    ]);
    let expected_full = client.eval_bitmap_query(&q, 0..250_000).await?;
    let prefix: Vec<u64> = client
        .eval_bitmap_query_stream(q, 0..250_000)
        .take(3)
        .try_collect::<Vec<_>>()
        .await?;
    assert_eq!(prefix, expected_full[..3]);
...
async fn test_resolve_tx_digests() -> Result<()> {
    ...
    write_tx_seq_digests_for_checkpoint(&mut client, 0, 0, &[d0, d1, d2]).await?;
    write_tx_seq_digests_for_checkpoint(&mut client, 1, 3, &[d3, d4]).await?;

    // Resolve all 5
    let result = client.resolve_tx_digests(&[0, 1, 2, 3, 4]).await?;
    assert_eq!(result.len(), 5);
    assert_eq!(result[0], Some((d0, 0, 0)));
    assert_eq!(result[1], Some((d1, 0, 0)));
    assert_eq!(result[2], Some((d2, 0, 0)));
    assert_eq!(result[3], Some((d3, 1, 0)));
    assert_eq!(result[4], Some((d4, 1, 0)));

    // Resolve a subset
    let result = client.resolve_tx_digests(&[1, 4]).await?;
    assert_eq!(result.len(), 2);
    assert_eq!(result[0], Some((d1, 0, 0)));
    assert_eq!(result[1], Some((d4, 1, 0)));
...
async fn test_full_pipeline() -> Result<()> {
    ...
    write_tx_seq_digests_for_checkpoint(&mut client, 0, 0, &all_digests[0..3]).await?;
    write_tx_seq_digests_for_checkpoint(&mut client, 1, 3, &all_digests[3..5]).await?;
    ...
    let matching = client.eval_bitmap_query(&query, tx_range.clone()).await?;
    ...
    let by_seq: std::collections::HashMap<u64, _> = fetched
        .into_iter()
        .map(|(seq, _cp, tx)| (seq, tx))
        .collect();
    ...
    let matching = client.eval_bitmap_query(&query, tx_range).await?;
    assert_eq!(matching, vec![0, 2, 4]);

    Ok(())
}

/// AND where all children are NOT — materializes universe then subtracts.
#[tokio::test]
async fn test_eval_bitmap_query_all_not_and() -> Result<()> {
    let (mut client, _emulator) = setup_emulator().await?;

    let dim_a = encode_dimension_key(IndexDimension::Sender, &[0xAA; 32]);
    let dim_b = encode_dimension_key(IndexDimension::Sender, &[0xBB; 32]);

    write_bitmap(&mut client, &dim_a, 0, &[0, 2, 4]).await?;
    write_bitmap(&mut client, &dim_b, 0, &[1, 3]).await?;

    // AND(NOT(a), NOT(b)) in [0..6) → {5}
    let query = BitmapQuery::and(vec![
        BitmapQuery::complement(BitmapQuery::scan(dim_a)),
        BitmapQuery::complement(BitmapQuery::scan(dim_b)),
    ]);
    let matching = client.eval_bitmap_query(&query, 0..6).await?;
    assert_eq!(matching, vec![5]);

    Ok(())
}

/// Top-level NOT — materializes full complement.
#[tokio::test]
async fn test_eval_bitmap_query_standalone_not() -> Result<()> {
    let (mut client, _emulator) = setup_emulator().await?;

    let dim = encode_dimension_key(IndexDimension::Sender, &[0xCC; 32]);
    write_bitmap(&mut client, &dim, 0, &[2, 5]).await?;

    let query = BitmapQuery::complement(BitmapQuery::scan(dim));
    let matching = client.eval_bitmap_query(&query, 0..8).await?;
    assert_eq!(matching, vec![0, 1, 3, 4, 6, 7]);
...
async fn test_resolve_tx_digests_boundary_and_ordering() -> Result<()> {
    ...
    write_tx_seq_digests_for_checkpoint(&mut client, 0, 0, &all_digests[0..3]).await?;
    write_tx_seq_digests_for_checkpoint(&mut client, 1, 3, &all_digests[3..5]).await?;
    write_tx_seq_digests_for_checkpoint(&mut client, 2, 5, &all_digests[5..9]).await?;
    write_tx_seq_digests_for_checkpoint(&mut client, 3, 9, &all_digests[9..10]).await?;

    // Out-of-order input — results must preserve input order.
    let result = client.resolve_tx_digests(&[9, 0, 4, 7, 2]).await?;
    assert_eq!(result.len(), 5);
    assert_eq!(result[0], Some((all_digests[9], 3, 0)));
    assert_eq!(result[1], Some((all_digests[0], 0, 0)));
    assert_eq!(result[2], Some((all_digests[4], 1, 0)));
    assert_eq!(result[3], Some((all_digests[7], 2, 0)));
    assert_eq!(result[4], Some((all_digests[2], 0, 0)));

    // tx_seqs at exact checkpoint boundaries (tx_lo values).
    let result = client.resolve_tx_digests(&[0, 3, 5, 9]).await?;
    assert_eq!(result.len(), 4);
    assert_eq!(result[0], Some((all_digests[0], 0, 0)));
    assert_eq!(result[1], Some((all_digests[3], 1, 0)));
    assert_eq!(result[2], Some((all_digests[5], 2, 0)));
    assert_eq!(result[3], Some((all_digests[9], 3, 0)));

    // Single tx_seq.
    let result = client.resolve_tx_digests(&[6]).await?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0], Some((all_digests[6], 2, 0)));
