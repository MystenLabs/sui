...
async fn test_indexer_e2e() -> Result<()> {
    ...
    // Query the Sender dimension for our sender address — should find our txns.
    {
        ...
        let bitmap_resolved_digests = harness
            ...
            .await?;
        let known_sender_digests: std::collections::HashSet<_> = tx_digests
            .iter()
            .copied()
        ...
        let matching_tx_seqs = harness
            .bigtable_client()
            .eval_bitmap_query_stream(BitmapQuery::scan(sender_dim_key.clone()), tx_range.clone())
            .try_collect::<Vec<_>>()
            .await?;
        ...
        for (i, resolved) in bitmap_resolved_digests.iter().enumerate() {
            let Some((digest, _, _)) = resolved.as_ref().copied() else {
                continue;
            };
            let tx_seq = bitmap_tx_seqs[i];
            if known_sender_digests.contains(&digest) {
                assert!(
        ...
        let matching_event_seqs = harness
            .bigtable_client()
        ...
        for (i, resolved) in bitmap_resolved_digests.iter().enumerate() {
            let Some((digest, _, event_count)) = resolved.as_ref().copied() else {
                continue;
            };
            if !known_sender_digests.contains(&digest) {
                continue;
            ...
            let tx_seq = bitmap_tx_seqs[i];
            if digest == clock_event_digest {
                clock_event_count += event_count;
            }
