...
use sui_config::node::{DBCheckpointConfig, NodeType, RunWithRange};
...
/// A no-op consensus overload checker for observer nodes.
/// Observer nodes don't submit to consensus, so they never experience overload.
struct NoopConsensusOverloadChecker;
...
impl SuiNode {
    ...
    async fn start_epoch_specific_observer_components(
        ...
        let consensus_config = config
            .consensus_config()
            .ok_or_else(|| anyhow!("Observer nodes require consensus config"))?;

        let protocol_config = epoch_store.protocol_config();
        let consensus_adapter = Arc::new(ConsensusAdapter::new(
            Arc::new(UpdatableConsensusClient::new()),
            checkpoint_store.clone(),
            state.name,
            connection_monitor_status,
    ...
    async fn start_epoch_specific_validator_components(
        ...
        // Observer nodes don't participate in randomness generation (DKG or partial signature generation).
        // They only consume randomness values generated by validators.
        if epoch_store.randomness_state_enabled() && !matches!(config.node_type(), NodeType::Observer) {
                ...
                Some(ConsensusComponents::Observer(ObserverComponents {
                    ...
                    *hasher_guard = Some(new_hasher);
                    ...
                    match self.config.node_type() {
                        sui_config::NodeType::Observer => {
                            let weak_hasher = {
                                let hasher_guard = self.global_state_hasher.lock().await;
                                Arc::downgrade(hasher_guard.as_ref().unwrap())
                            };
