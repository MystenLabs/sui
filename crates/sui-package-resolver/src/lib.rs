...
impl<S: PackageStore> Resolver<S> {
    ...
    pub async fn type_layout(&self, mut tag: TypeTag) -> Result<CA::MoveTypeLayout> {
        ...
        CA::MoveTypeLayoutBuilder::with_builder(|b| {
            Ok(context.resolve_type_layout(b, &tag, max_depth)?.0)
        })
    ...
    fn resolve_datatype_signature(
        ...
        Ok(match &data_def.data {
            MoveData::Struct(fields) => {
                ...
                let resolved_field_refs: Vec<_> = resolved_fields
                    .iter()
                    .map(|(name, layout)| (name, *layout))
                    .collect();
                (
                    unable_to_construct_if_err(
                        builder.struct_layout(&type_, &resolved_field_refs),
                    )?,
                    field_depth + 1,
                )
            }
            MoveData::Enum(variants) => {
                ...
                let resolved_variant_field_refs: Vec<Option<Vec<_>>> = resolved_variants
                    .iter()
                    .map(|(_, _, fields)| {
                    ...
                    .collect();
                let resolved_variant_refs: Vec<_> = resolved_variants
                    .iter()
                    .zip_eq(resolved_variant_field_refs.iter())
                    .map(|((name, tag, _), fields)| (name, *tag, fields.as_deref()))
                    .collect();
                (
                    unable_to_construct_if_err(
                        builder.enum_layout(&type_, &resolved_variant_refs),
                    )?,
                    field_depth + 1,
                )
            }
