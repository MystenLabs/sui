...
pub struct ArtifactManager<'a> {
    ...
    /// `try_remove_artifact`, `clear_generated_artifacts`) become no-ops and the base directory is
    /// not created.
...
impl<'b> ArtifactManager<'b> {
    ...
    /// Remove generated artifacts and package bytecode while preserving source and unknown files.
    pub(crate) fn clear_generated_artifacts(&self) -> Result<()> {
        if self.skip_writes || !self.overrides_allowed {
            return Ok(());
        }
        ...
        for artifact in ARTIFACTS {
            let artifact_path = self.base_path.join(artifact.as_file());
...
mod tests {
    ...
    fn clears_generated_artifacts_and_preserves_user_files() {
        ...
        artifact_manager.clear_generated_artifacts().unwrap();

        assert!(artifact_path.exists());
        assert!(!bytecode_path.exists());
        assert_eq!(
            fs::read(source_path.join("package.move")).unwrap(),
            b"source"
        );
        assert_eq!(
