...
use crate::{
    ...
    replay_interface::{
        EpochData, EpochStore, ObjectKey, ObjectStore, StoreSummary, TransactionInfo,
...
impl TransactionStore for DataStore {
    fn transaction_data_and_effects(
        ...
        match block_on!(self.transaction(digest)) {
            Ok(Some((data, effects, checkpoint))) => Ok(Some(TransactionInfo {
                data,
                effects,
                checkpoint,
            }))
        ...
        match block_on!(self.objects(keys)) {
            Ok(results) => {
                for (key, res) in keys.iter().zip(results.iter()) {
                    match key {
                        ObjectKey {
                            version_query: VersionQuery::Version(_),
                            ..
                        } => {
                            if res.is_some() {
                        ...
                        ObjectKey {
                            version_query: VersionQuery::RootVersion(_),
                            ..
                        } => {
                            if res.is_some() {
                        ...
                        ObjectKey {
                            version_query: VersionQuery::AtCheckpoint(_),
                            ..
                        } => {
                            if res.is_some() {
                                self.metrics
