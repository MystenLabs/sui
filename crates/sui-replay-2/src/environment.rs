...
use std::{
    cmp::Ordering, collections::{BTreeMap, BTreeSet}, fmt::Debug, ops::Bound,
};
use sui_types::{
    base_types::ObjectID,
    ...
    object::Object, 
    supported_protocol_versions::{Chain, ProtocolConfig, ProtocolVersion}, 
};
use tracing::debug;
...
pub struct ReplayEnvironment {
    ...
    pub system_packages: BTreeMap<ObjectID, BTreeMap<u64, MovePackage>>,
    // all packages as pkg_id -> MovePackage
    pub packages: BTreeMap<ObjectID, MovePackage>,
    // all package objects as pkg_id -> Object
    pub package_objects: BTreeMap<ObjectID, Object>,
...
impl EpochStore {
    ...
    pub async fn new(data_store: &DataStore) -> Result<Self, ReplayError> {
        ...
        let (protocol_configs, rgps, epoch_info) =
            data_store.load_epochs().await?;
