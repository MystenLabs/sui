...
pub fn trace_simulated_transaction(
    ...
    clear_generated_artifacts(&artifact_path)?;
...
fn clear_generated_artifacts(path: &Path) -> Result<()> {
    let recovery = format!(
        "move or remove the replay artifact directory at {} manually, then retry",
        path.display()
    );
    let entries = match fs::read_dir(path) {
        ...
        Err(error) => {
            return Err(error).with_context(|| {
                format!(
                    "failed to inspect prior artifacts at {}; {recovery}",
                    path.display()
                )
            });
        }
    ...
    for entry in entries {
        ...
        let file_type = entry.file_type().with_context(|| {
            format!(
                "failed to inspect prior artifact at {}; {recovery}",
                entry.path().display()
            )
        })?;
        ...
        if !file_type.is_dir()
            || file_name
                .to_str()
                .and_then(|name| name.parse::<ObjectID>().ok())
                .is_none()
        {
            continue;
        }
        ...
        match fs::remove_dir_all(&bytecode_path) {
            ...
            Err(error) => {
                return Err(error).with_context(|| {
                    format!(
                        "failed to clear prior generated bytecode at {}; {recovery}",
    ...
    for artifact in ARTIFACTS {
        let artifact_path = path.join(artifact.as_file());
        match fs::remove_file(&artifact_path) {
            ...
            Err(error) => {
                return Err(error).with_context(|| {
                    format!(
                        "failed to clear prior artifact at {}; {recovery}",
...
mod tests {
    ...
    /// Cleanup removes generated artifacts and bytecode while preserving source and other user
    /// files. A missing digest directory is also accepted.
    #[test]
    fn clears_generated_artifacts_and_preserves_user_files() {
        ...
        std::fs::create_dir_all(&bytecode_path).unwrap();
        std::fs::create_dir_all(&source_path).unwrap();
        std::fs::create_dir_all(user_path.join(BCODE_DIR)).unwrap();
        std::fs::write(bytecode_path.join("stale.mv"), b"stale").unwrap();
        std::fs::write(source_path.join("package.move"), b"source").unwrap();
        std::fs::write(user_path.join(BCODE_DIR).join("notes.txt"), b"notes").unwrap();
        for artifact in ARTIFACTS {
            std::fs::write(artifact_path.join(artifact.as_file()), b"stale").unwrap();
        }

        clear_generated_artifacts(&artifact_path).unwrap();

        assert!(artifact_path.exists());
        assert!(!bytecode_path.exists());
        assert_eq!(
            std::fs::read(source_path.join("package.move")).unwrap(),
            b"source"
        );
        assert_eq!(
            std::fs::read(user_path.join(BCODE_DIR).join("notes.txt")).unwrap(),
            b"notes"
        );
        for artifact in ARTIFACTS {
            assert!(!artifact_path.join(artifact.as_file()).exists());
        }

        clear_generated_artifacts(&artifact_path).unwrap();
        clear_generated_artifacts(&temp.path().join("missing")).unwrap();
    }
}
