...
#[tokio::test]
async fn test_fungible_stake_balance_query() {
    telemetry_subscribers::init_for_testing();
    ...
    // Query FungibleStake balance before any staking — should be 0
    let response = rosetta_client
        .get_balance(
            network_identifier.clone(),
            sender,
            Some(SubAccountType::FungibleStake),
        )
        .await;
    assert_eq!(1, response.balances.len());
    assert_eq!(0, response.balances[0].value);
    ...
    assert!(!staked_sui_objects.is_empty(), "Expected at least 1 StakedSui object");
    ...
    // Now query FungibleStake balance via Rosetta — should have the staked amount
    let response = rosetta_client
        .get_balance(
            network_identifier.clone(),
            sender,
    ...
    assert!(
        ...
        "Expected positive FungibleStake balance, got {}",
        response.balances[0].value
    );

    // Verify sub_balances metadata is present with object details
    ...
    assert_eq!(
        ...
        "Expected exactly 1 FungibleStakedSui sub_balance"
    );
    assert!(
        metadata.sub_balances[0].value > 0,
        "Sub_balance value should be positive"
    );
}

#[tokio::test]
async fn test_fungible_stake_balance_query_empty() {
    telemetry_subscribers::init_for_testing();
    ...
    // Query FungibleStake balance for address with no fungible stakes
    let response = rosetta_client
        .get_balance(
            network_identifier,
            sender,
