...
#[tokio::test]
async fn test_redeem_fungible_staked_sui() {
    telemetry_subscribers::init_for_testing();
    ...
    assert!(
        ...
        "Expected at least 1 StakedSui object"
    );
    ...
    for changed in response.effects().changed_objects() {
        ...
        if object_type.contains("FungibleStakedSui") {
            fungible_id = Some(ObjectID::from_str(changed.object_id()).unwrap());
        }
    }
    let fungible_id = fungible_id.expect("FungibleStakedSui object should be created");

    // Now use Rosetta to redeem the FungibleStakedSui
    ...
    assert!(
        tx.effects().status().success(),
        "RedeemFungibleStakedSui transaction failed: {:?}",
        tx.effects().status().error()
    );
...
#[tokio::test]
async fn test_redeem_fungible_staked_sui_read_path() {
    telemetry_subscribers::init_for_testing();
    ...
    // Redeem via direct PTB (3-command: redeem -> from_balance -> TransferObjects)
    let fungible_ref = get_object_ref(&mut client.clone(), fungible_id).await.unwrap();
    let gas_object = get_random_sui(&mut client.clone(), sender, vec![fungible_id]).await;

    let mut ptb = ProgrammableTransactionBuilder::new();
    let system_state_arg = ptb.input(CallArg::SUI_SYSTEM_MUT).unwrap();
    let fungible_obj = ptb
        .obj(ObjectArg::ImmOrOwnedObject(fungible_ref.as_object_ref()))
        .unwrap();

    let balance_result = ptb.command(Command::move_call(
        SUI_SYSTEM_PACKAGE_ID,
        Identifier::new("sui_system").unwrap(),
        Identifier::new("redeem_fungible_staked_sui").unwrap(),
        vec![],
        vec![system_state_arg, fungible_obj],
    ));

    use sui_types::gas_coin::GAS;
    let coin_result = ptb.command(Command::move_call(
    ...
    ptb.command(Command::TransferObjects(vec![coin_result], sender_arg));
    ...
    assert!(
        response.effects().status().success(),
        "Redeem fungible staked sui failed: {:?}",
        response.effects().status().error()
    );

    let tx_digest: sui_types::digests::TransactionDigest =
        response.digest().parse().unwrap();

    wait_for_transaction(&mut client, &tx_digest.to_string())
        .await
        .unwrap();

    // Read back through Rosetta and verify
    let coin_cache = CoinMetadataCache::new(client.clone(), NonZeroUsize::new(2).unwrap());
    let ops = fetch_transaction_and_get_operations(&test_cluster, tx_digest, &coin_cache)
        .await
        .unwrap();

    // Find the RedeemFungibleStakedSui operation
    let redeem_ops: Vec<_> = ops
        .clone()
        .into_iter()
    ...
    assert_eq!(
        1,
        redeem_ops.len(),
        "Expected exactly 1 RedeemFungibleStakedSui operation"
    );
    let op = &redeem_ops[0];
    assert_eq!(op.status, Some(sui_rosetta::types::OperationStatus::Success));
    assert_eq!(
        op.account,
        Some(sui_rosetta::types::AccountIdentifier::from(sender))
    );
    assert!(op.amount.is_none(), "Redeem should have no amount in the operation itself");
}
