...
#[tokio::test]
async fn test_split_fungible_staked_sui() {
    telemetry_subscribers::init_for_testing();
    ...
    let fungible_staked_sui_id = fungible_id.expect("Expected a FungibleStakedSui object");

    let split_amount = 500_000_000u64;

    // Use Rosetta to split the FungibleStakedSui
    ...
    assert!(
        tx.effects().status().success(),
        "SplitFungibleStakedSui transaction failed: {:?}",
        tx.effects().status().error()
    );
...
#[tokio::test]
async fn test_split_fungible_staked_sui_read_path() {
    telemetry_subscribers::init_for_testing();
    ...
    let fungible_ref = fungible_ref.expect("Expected FungibleStakedSui object");
    let split_amount = 500_000_000u64;

    // Build the split PTB directly (not via Rosetta)
    let gas_object = get_random_sui(&mut client.clone(), sender, vec![fungible_ref.0]).await;

    let mut ptb = ProgrammableTransactionBuilder::new();
    let fungible_obj = ptb
        .obj(ObjectArg::ImmOrOwnedObject(fungible_ref))
        .unwrap();
    let amount_arg = ptb.pure(split_amount).unwrap();
    ...
    // Read back the transaction through Rosetta and verify the operation is recognized
    let coin_cache = CoinMetadataCache::new(client.clone(), NonZeroUsize::new(2).unwrap());
    ...
    // Construct the expected operation and verify it is found
    let expected_ops: Operations = serde_json::from_value(json!([{
        "operation_identifier": {"index": 0},
        "type": "SplitFungibleStakedSui",
        "account": { "address": sender.to_string() },
        "metadata": { "SplitFungibleStakedSui": {
            "fungible_staked_sui_id": fungible_ref.0.to_string(),
            "split_amount": split_amount
        }}
    }]))
    .unwrap();
    ...
    let split_op = ops_array
        .iter()
        .find(|op| op["type"] == "SplitFungibleStakedSui")
        .expect("Expected SplitFungibleStakedSui operation in parsed transaction");
    assert_eq!(split_op["account"]["address"], sender.to_string());
    assert!(split_op.get("amount").is_none() || split_op["amount"].is_null());
    assert_eq!(
        split_op["metadata"]["SplitFungibleStakedSui"]["fungible_staked_sui_id"],
        fungible_ref.0.to_string()
    );
    assert_eq!(
        split_op["metadata"]["SplitFungibleStakedSui"]["split_amount"],
        split_amount
    );
}
