...
pub fn get_object_inclusion_proof(
    ...
    let effects_refs: Vec<&_> = checkpoint_data
        .transactions
        .iter()
        .map(|tx| &tx.effects)
        .collect();
    ...
    // TODO: this logic is temporarily replicated from sui-light-client because of a circular dependency on sui-data-ingestion-core
    ...
    for (i, (id, _, _)) in leaves.iter().enumerate() {
        object_pos_map.insert(*id, i);
    }

    let leaf_index = object_pos_map.get(&object_ref.0).ok_or_else(|| {
        RpcError::new(
            tonic::Code::Internal,
    ...
    if leaves[*leaf_index] != object_ref {
        return Err(RpcError::new(
            tonic::Code::Internal,
            format!(
                "Object ref mismatch: expected {:?}, got {:?}",
                object_ref, leaves[*leaf_index]
            ),
        ));
    }
    ...
    let proto_inclusion_proof = OcsInclusionProof {
        ...
        object_ref: Some(ProtoObjectRef {
            object_id: Some(object_ref.0.to_vec()),
            version: Some(object_ref.1.value()),
            digest: Some(object_ref.2.into_inner().to_vec()),
        }),
    };

    let object = reader
        .get_object_by_key(&object_id, object_ref_from_checkpoint.0)
        .ok_or_else(|| {
            RpcError::new(
                tonic::Code::NotFound,
                format!(
                    "Object {} not found at version {}",
                    object_id, object_ref_from_checkpoint.0
                ),
            )
        })?;

    let object_data_bytes = bcs::to_bytes(&object)
        .map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;
