...
pub fn get_service_info(service: &RpcService) -> Result<GetServiceInfoResponse, RpcError> {
    ...
    message.set_lowest_available_checkpoint(lowest_available_checkpoint);
    // Set the old watermark for now, even though its deprecated
    message.set_lowest_available_checkpoint_objects(lowest_available_checkpoint);
