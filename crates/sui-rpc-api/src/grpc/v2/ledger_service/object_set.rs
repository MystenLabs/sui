...
pub(crate) fn build_object_sets(
...
) -> Result<(Vec<ObjectSet>, usize), RpcError> {
    let unique_keys = items
        ...
        .into_iter()
        .collect::<Vec<_>>();
    let fetched_key_count = unique_keys.len();
    ...
    Ok((object_sets, fetched_key_count))
}
...
pub(crate) fn fetch_object_sets_for_chunk(
    ...
    if !mask_requests_object_set(mask) {
        return Ok((vec![ObjectSet::default(); reads.len()], 0));
    }
...
pub(crate) fn fetch_transaction_object_set(
    ...
    let (mut object_sets, _) =
        fetch_object_sets_for_chunk(reader, std::slice::from_ref(read), mask)?;
...
mod tests {
    ...
    fn shared_keys_are_fetched_once_and_reconstructed_per_item() {
        ...
        let (sets, fetched_key_count) = build_object_sets(&items, |keys| {
    ...
    fn empty_inputs_do_not_fetch() {
        ...
        let (sets, fetched_key_count) = build_object_sets(&no_items, |_| {
            fetch_calls.set(fetch_calls.get() + 1);
            Vec::new()
        })
        .unwrap();
        ...
        assert_eq!(fetched_key_count, 0);

        let empty_items = vec![(digest(1), Vec::new()), (digest(2), Vec::new())];
        let (sets, fetched_key_count) = build_object_sets(&empty_items, |_| {
            fetch_calls.set(fetch_calls.get() + 1);
            Vec::new()
        })
        .unwrap();
