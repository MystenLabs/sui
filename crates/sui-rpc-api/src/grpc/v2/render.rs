...
mod tests {
    ...
    fn render_execution_error_metadata_respects_read_mask() {
        let metadata = ExecutionErrorMetadata::new("Loaded runtime object limit exceeded");
        ...
        assert_eq!(
            effects.status().error().metadata().message,
            metadata.message,
        );
    }
    ...
    fn render_execution_error_metadata_does_not_leak_into_other_error_masks() {
        let metadata = ExecutionErrorMetadata::new("hidden");
        let mask = FieldMaskTree::from(FieldMask::from_paths([TransactionEffects::path_builder()
            .status()
            .error()
            .abort()
        ...
        assert!(effects.status().error().metadata_opt().is_none());
    }
}
