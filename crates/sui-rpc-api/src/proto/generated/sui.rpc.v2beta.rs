...
pub mod sleep_service_client {
    ...
    use tonic::codegen::http::Uri;
    #[derive(Debug, Clone)]
    pub struct SleepServiceClient<T> {
        inner: tonic::client::Grpc<T>,
    }
    impl SleepServiceClient<tonic::transport::Channel> {
        /// Attempt to create a new client by connecting to a given endpoint.
        pub async fn connect<D>(dst: D) -> Result<Self, tonic::transport::Error>
        where
            D: TryInto<tonic::transport::Endpoint>,
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> SleepServiceClient<InterceptedService<T, F>>
        where
            F: tonic::service::Interceptor,
            T::ResponseBody: Default,
            T: tonic::codegen::Service<
            ...
            >>::Error: Into<StdError> + std::marker::Send + std::marker::Sync,
        {
            SleepServiceClient::new(InterceptedService::new(inner, interceptor))
        }
        /// Compress requests with the given encoding.
        ///
        /// This requires the server to support it otherwise it might respond with an
        ...
        pub async fn sleep(
            ...
            let codec = tonic::codec::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static(
                "/sui.rpc.v2beta.SleepService/Sleep",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(GrpcMethod::new("sui.rpc.v2beta.SleepService", "Sleep"));
            self.inner.unary(req, path, codec).await
        }
    }
}
/// Generated server implementations.
pub mod sleep_service_server {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
    ...
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with SleepServiceServer.
    #[async_trait]
    pub trait SleepService: std::marker::Send + std::marker::Sync + 'static {
        async fn sleep(
            &self,
    ...
    #[derive(Debug)]
    pub struct SleepServiceServer<T> {
        inner: Arc<T>,
        accept_compression_encodings: EnabledCompressionEncodings,
        send_compression_encodings: EnabledCompressionEncodings,
        max_decoding_message_size: Option<usize>,
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for SleepServiceServer<T>
    where
        T: SleepService,
        B: Body + std::marker::Send + 'static,
        B::Error: Into<StdError> + std::marker::Send + 'static,
    {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/sui.rpc.v2beta.SleepService/Sleep" => {
                    #[allow(non_camel_case_types)]
                    struct SleepSvc<T: SleepService>(pub Arc<T>);
                    impl<
                        T: SleepService,
                    > tonic::server::UnaryService<super::SleepRequest> for SleepSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SleepService>::sleep(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for SleepServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "sui.rpc.v2beta.SleepService";
    impl<T> tonic::server::NamedService for SleepServiceServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
/// A transaction.
