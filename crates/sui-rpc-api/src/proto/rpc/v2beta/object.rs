...
const PACKAGE_TYPE: &str = "package";

impl Object {
    const BCS_FIELD: &'static MessageField =
        &MessageField::new("bcs").with_message_fields(super::Bcs::FIELDS);
    const OBJECT_ID_FIELD: &'static MessageField = &MessageField::new("object_id");
    ...
    const OWNER_FIELD: &'static MessageField = &MessageField::new("owner");
    const OBJECT_TYPE_FIELD: &'static MessageField = &MessageField::new("object_type");
    const HAS_PUBLIC_TRANSFER_FIELD: &'static MessageField =
        &MessageField::new("has_public_transfer");
    const CONTENTS_FIELD: &'static MessageField = &MessageField::new("contents");
    const MODULES_FIELD: &'static MessageField = &MessageField::new("modules");
    const TYPE_ORIGIN_TABLE_FIELD: &'static MessageField = &MessageField::new("type_origin_table");
    const LINKAGE_TABLE_FIELD: &'static MessageField = &MessageField::new("linkage_table");
    const PREVIOUS_TRANSACTION_FIELD: &'static MessageField =
        &MessageField::new("previous_transaction");
    const STORAGE_REBATE_FIELD: &'static MessageField = &MessageField::new("storage_rebate");
}
...
impl MessageMerge<&sui_sdk_types::MoveStruct> for Object {
    fn merge(
        ...
        if mask.contains(Self::CONTENTS_FIELD.name) {
            self.contents = Some(source.contents().to_vec().into());
        }
    }
}
