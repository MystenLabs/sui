...
impl super::GetTransactionResult {
    pub fn new_transaction(transaction: super::ExecutedTransaction) -> Self {
        Self {
            result: Some(super::get_transaction_result::Result::Transaction(
                transaction,
            )),
        }
    }
    ...
    pub fn new_error(error: crate::proto::google::rpc::Status) -> Self {
        Self {
            result: Some(super::get_transaction_result::Result::Error(error)),
        }
    }

    pub fn to_result(
        self,
    ) -> Result<super::ExecutedTransaction, crate::proto::google::rpc::Status> {
        match self.result {
            Some(super::get_transaction_result::Result::Transaction(transaction)) => {
                Ok(transaction)
            }
            Some(super::get_transaction_result::Result::Error(error)) => Err(error),
    ...
    pub fn transaction(&self) -> Option<&super::ExecutedTransaction> {
        if let Some(super::get_transaction_result::Result::Transaction(transaction)) = &self.result
        {
            Some(transaction)
        } else {
            None
    ...
    pub fn error(&self) -> Option<&crate::proto::google::rpc::Status> {
        if let Some(super::get_transaction_result::Result::Error(error)) = &self.result {
            Some(error)
        } else {
            None
        }
