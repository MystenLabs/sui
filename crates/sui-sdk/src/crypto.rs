...
use sui_types::crypto::{
    derive_key_pair_at_index, get_key_pair_from_rng, EncodeDecodeBase64, PublicKey, Signature,
...
impl SuiKeystore {
    ...
    pub fn generate_new_key(
        ...
        match derive_key_pair_at_index(Seed::new(&mnemonic, "").as_bytes(), 0, &key_scheme) {
            Ok((address, keypair)) => {
    ...
    pub fn import_from_mnemonic(
        ...
        match derive_key_pair_at_index(seed.as_bytes(), 0, &key_scheme) {
            Ok((address, kp)) => {
                self.0.add_key(kp)?;
                Ok(address)
            }
