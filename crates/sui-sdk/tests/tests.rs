...
mod test {
    ...
    fn mnemonic_test() {
        ...
        let mut keystore = KeystoreType::File(keystore_path)
            .init_for_testing()
            .unwrap();

        let (address, phrase, scheme) =
            keystore.generate_new_key(SignatureScheme::ED25519).unwrap();

        let keystore_path_2 = temp_dir.path().join("sui2.keystore");
        let mut keystore2 = KeystoreType::File(keystore_path_2)
            .init_for_testing()
            .unwrap();
