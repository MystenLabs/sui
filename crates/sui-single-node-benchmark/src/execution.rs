...
use crate::move_tx_generator::MoveTxGenerator;
use crate::non_move_tx_generator::NonMoveTxGenerator;
use crate::root_object_create_tx_generator::RootObjectCreateTxGenerator;
...
pub async fn benchmark_simple_transfer(tx_count: u64) {
    let ctx = BenchmarkContext::new(tx_count, 1).await;
    let transactions = ctx.generate_transactions(NonMoveTxGenerator::new()).await;
...
pub async fn benchmark_move_transactions(
    ...
    let mut ctx = BenchmarkContext::new(tx_count, num_input_objects as u64).await;
    ...
    let transactions = ctx
        .generate_transactions(MoveTxGenerator::new(
            move_package.0,
            num_input_objects,
            computation,
            root_objects,
...
async fn preparing_dynamic_fields(
    ...
    let root_object_create_transactions = ctx
        .generate_transactions(RootObjectCreateTxGenerator::new(
            move_package,
            num_dynamic_fields,
        ))
        .await;
    let results = ctx
        .execute_transactions(root_object_create_transactions)
        .await;
...
async fn benchmark_transactions(
    ...
    let effects = ctx
        .validator()
        .execute_tx_immediately(sample_transaction)
        .await;
    info!("Sample effects: {:?}\n\n", effects);
