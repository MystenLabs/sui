...
pub async fn verify_package(
    ...
    let network_url = network_to_url(network);
...
impl CloneCommand {
    pub fn new(p: &RepositorySource, dest: &Path) -> anyhow::Result<CloneCommand> {
        ...
        let dest = dest
            .join(network_to_name(&p.network))
            .join(repo_name)
            .into_os_string();
...
pub fn network_to_url(network: &Network) -> &str {
    match network {
...
pub fn network_to_name(network: &Option<Network>) -> &str {
    match network {
        Some(Network::Mainnet) => "mainnet",
        Some(Network::Testnet) => "testnet",
        Some(Network::Devnet) => "devnet",
        Some(Network::Localnet) => "localnet",
        None => "mainnet",
    }
}
    ...
    for p in &config.packages {
        match p {
            PackageSources::Repository(r) => {
                ...
                let network_name = network_to_name(&r.network);
...
async fn api_route(
	...
	let error = format!("No source found for {symbol} at address {address} on network {}", network_to_name(&Some(network)));
