...
mod tests {
    ...
    fn metadata_serializes_with_camel_case_keys() {
        ...
        assert!(json["originalId"].as_str().unwrap().starts_with("0x"));
        assert!(json["publishedAt"].as_str().unwrap().starts_with("0x"));
        assert_eq!(json["toolchainVersion"], "1.71.1");
        assert_eq!(json["binaryPath"], "/cache/1.71.1/target/release/sui");
    }
}
