...
impl<'l, 's> Parser<'l, 's> {
    ...
    fn part(&mut self) -> Result<Cow<'s, str>, Error> {
        let mut copy = *self;
        if let Ok(s) = copy.lcurly() {
            *self = copy;
            return Ok(s);
        }

        let mut copy = *self;
        if let Ok(s) = copy.rcurly() {
            *self = copy;
            return Ok(s);
        }
    ...
    fn lcurly(&mut self) -> Result<Cow<'s, str>, Error> {
        use Token as T;
        self.eat(T::LCurl)?;
        self.eat(T::LCurl)?;
        Ok(Cow::Borrowed("{"))
    ...
    fn rcurly(&mut self) -> Result<Cow<'s, str>, Error> {
        use Token as T;
        self.eat(T::RCurl)?;
        self.eat(T::RCurl)?;
        Ok(Cow::Borrowed("}"))
