...
use crate::db_tool::db_dump::{print_objects, print_table_metadata};
use clap::Parser;
...
pub fn execute_db_tool_command(db_path: PathBuf, cmd: DbToolCommand) -> anyhow::Result<()> {
    match cmd {
        ...
        DbToolCommand::ObjectSummary(d) => print_objects(d.object_id, d.print_only_keys, db_path),
    }
}
