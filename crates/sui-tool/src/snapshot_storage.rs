...
pub struct SnapshotSourceArgs {
    ...
    #[arg(long, group = "source", value_name = "BUCKET")]
    ...
    #[arg(long, group = "source", value_name = "CONTAINER")]
    ...
    #[arg(long = "local-path", group = "source", value_name = "PATH")]
...
/// Connection arguments for snapshot downloads.
#[derive(Args, Clone, Debug, Default)]
...
impl From<&StorageConnectionArgs> for ClientOptions {
    fn from(args: &StorageConnectionArgs) -> ClientOptions {
        let mut opts = ClientOptions::new().with_pool_idle_timeout(Duration::from_secs(300));
...
pub struct LegacySnapshotArgs {
    ...
    #[arg(long = "snapshot-bucket", hide = true, conflicts_with = "source")]
    pub snapshot_bucket: Option<String>,

    /// [DEPRECATED] Use --s3, --gcs, --azure, --http, or --local-path instead.
    #[arg(long = "snapshot-bucket-type", hide = true, conflicts_with = "source")]
    pub snapshot_bucket_type: Option<ObjectStoreType>,

    /// [DEPRECATED] Use --local-path instead.
    #[arg(long = "snapshot-path", hide = true, conflicts_with = "source")]
    pub snapshot_path: Option<PathBuf>,

    /// [DEPRECATED] Use --http with the appropriate public endpoint instead.
    #[arg(long = "no-sign-request", hide = true, conflicts_with = "source")]
...
impl LegacySnapshotArgs {
    ...
    pub fn print_deprecation_warning(&self) {
        ...
        eprintln!(
            "  --snapshot-bucket, --snapshot-bucket-type, --snapshot-path, --no-sign-request"
        );
        eprintln!();
...
/// Note: endpoints must NOT have a trailing slash.
...
/// Strip trailing slash from URL string to avoid double-slash issues.
...
/// Returns Box<dyn ObjectStoreGetExt> for use with snapshot checks (get_latest_epoch, etc.).
pub async fn build_object_store(
    source: &SnapshotSourceArgs,
    connection: &StorageConnectionArgs,
) -> Result<Box<dyn ObjectStoreGetExt>> {
    ...
    if let Some(bucket) = &source.s3 {
        ...
        let store = AmazonS3Builder::from_env()
            ...
            .build()?;
        let store: Arc<DynObjectStore> = Arc::new(LimitStore::new(store, limit));
        Ok(Box::new(store))
        ...
        // Strip trailing slash to avoid double-slash issues (e.g., https://host//path)
        let endpoint_str = strip_trailing_slash(endpoint.as_str());
        info!(endpoint = %endpoint_str, "HTTP storage (unsigned)");
        // Use ObjectStoreConfig::make_http() for public HTTP endpoints.
        // This creates a simple HTTP client that handles unsigned requests.
        let config = ObjectStoreConfig {
            object_store: Some(ObjectStoreType::S3),
            aws_endpoint: Some(endpoint_str),
        ...
        config
            .make_http()
            .map(|s| Box::new(s) as Box<dyn ObjectStoreGetExt>)
...
pub fn resolve_source(
    ...
    if source.has_explicit_source() {
        // Validate: --http is not supported for DB snapshots (no free public endpoint)
        if matches!(snapshot_type, SnapshotType::Db) && source.http.is_some() {
            anyhow::bail!(
                "The --http flag is not supported for DB snapshots. \
                Free public endpoints are only available for formal snapshots. \
                Please use --s3 or --gcs with appropriate credentials instead. \
                For more information, see: https://docs.sui.io/guides/operator/snapshots"
            );
        }
        return Ok(source.clone());
    }

    // No explicit source specified - use defaults
    match snapshot_type {
        SnapshotType::Formal => {
            let defaults = NetworkDefaults::for_network(network).ok_or_else(|| {
                anyhow!(
                    "Cannot determine default snapshot location for unknown network. \
                    Please specify a source with --s3, --gcs, --azure, --http, or --local-path"
                )
            })?;
            Ok(SnapshotSourceArgs {
                http: Some(Url::parse(defaults.formal_http_endpoint)?),
                ..Default::default()
            })
        }
        SnapshotType::Db => {
            anyhow::bail!(
                "DB snapshots require an explicit source. Free public endpoints are only \
                available for formal snapshots. Please specify --s3 or --gcs with appropriate \
                credentials. For more information, see: https://docs.sui.io/guides/operator/snapshots"
            )
        }
    }
}
...
fn legacy_to_source(
    ...
    if legacy.no_sign_request {
        // --no-sign-request is only supported for formal snapshots
        if matches!(snapshot_type, SnapshotType::Db) {
            anyhow::bail!(
                "The --no-sign-request flag is not supported for DB snapshots. \
                Please use --s3 or --gcs with appropriate credentials instead. \
                For more information, see: https://docs.sui.io/guides/operator/snapshots"
            );
        }
    ...
    let bucket = legacy
        .snapshot_bucket
        .clone()
        .ok_or_else(|| anyhow!("Legacy mode requires --snapshot-bucket or --no-sign-request"))?;
