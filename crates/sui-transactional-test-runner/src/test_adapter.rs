...
impl SuiTestAdapter {
    ...
    async fn execute_txn(&mut self, transaction: Transaction) -> anyhow::Result<TxnSummary> {
        ...
        let accumulator_sort_keys: std::collections::HashMap<_, _> = accumulator_events
            .iter()
            .map(|event| {
                (
                    *event.accumulator_obj.inner(),
                    (event.write.address.address, event.write.address.ty.clone()),
                )
            })
            .collect();
        ...
        might_need_fake_id
            .sort_by_key(|id| self.get_object_sorting_key(id, &accumulator_sort_keys));
    ...
    fn get_object_sorting_key(
        &self,
        id: &ObjectID,
        accumulator_sort_keys: &std::collections::HashMap<
            ObjectID,
            (
                sui_types::base_types::SuiAddress,
                move_core_types::language_storage::TypeTag,
            ),
        >,
    ) -> String {
        // Check if this is an accumulator object (expected to not exist in storage yet)
        if let Some((address, ty)) = accumulator_sort_keys.get(id) {
            return format!(
                "{}~{}",
