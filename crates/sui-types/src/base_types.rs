...
/// Defines a `pub fn $name() -> &'static CA::MoveStructLayout` that lazily
...
macro_rules! cached_compressed_struct_layout {
    ($name:ident, $tree_fn:ident) => {
        pub fn $name() -> &'static CA::MoveStructLayout {
            static CELL: OnceLock<CA::MoveStructLayout> = OnceLock::new();
            CELL.get_or_init(|| CA::MoveStructLayout::try_from(&$tree_fn()).unwrap())
        }
    };
}
