...
impl SuiKeyPair {
    ...
    /// Encode a SuiKeyPair as `flag || privkey` in Bech32 starting with "suiprivkey". Note that the pubkey is not encoded.
    pub fn encode(&self) -> Result<String, eyre::Report> {
        Bech32::encode(&self.to_bytes(), SUI_PRIV_KEY_PREFIX)
    }

    /// Decode a SuiKeyPair from `flag || privkey` in Bech32 starting with "suiprivkey". The public key is computed directly from the private key bytes.
