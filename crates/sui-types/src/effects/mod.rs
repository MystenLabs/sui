...
pub trait TransactionEffectsAPI {
    ...
    /// declared as sequenced inputs — they are recorded as read-only so the read can be reproduced
    /// on replay, and are indistinguishable here from genuinely-sequenced read-only inputs. Callers
