...
impl ExecutedTransaction {
    pub fn input_objects<'a>(
        ...
        self.effects
            ...
            .into_iter()
            .filter_map(
                move |change| match (change.input_version, change.input_digest) {
                    (Some(version), Some(_digest)) => {
                        object_set.get(&ObjectKey(change.id, version))
                    }
                    _ => None,
                },
            )
    ...
    pub fn output_objects<'a>(
        ...
        self.effects
            ...
            .into_iter()
            .filter_map(
                move |change| match (change.output_version, change.output_digest) {
                    (Some(version), Some(_digest)) => {
                        object_set.get(&ObjectKey(change.id, version))
                    }
                    _ => None,
