...
impl<'a> SuiGasStatus<'a> {
    ...
    pub fn charge_storage_and_rebate(&self, storage_rebate_rate: u64) -> Result<GasCostSummary, ExecutionError> {
        let remaining_gas = u64::from(self.gas_status.remaining_gas());
        let storage_cost = u64::from(self.storage_gas_units);
        let storage_rebate = sender_rebate(u64::from(self.storage_rebate), storage_rebate_rate);
        let mut unit_computation_cost = u64::from(self.init_budget);
        unit_computation_cost = unit_computation_cost
            .checked_add(storage_rebate)
            .and_then(|val| val.checked_sub(remaining_gas))
            .and_then(|val| val.checked_sub(storage_cost))
            .ok_or(ExecutionErrorKind::InvariantViolation.into())?;
        let storage_cost = u64::from(self.storage_gas_units.mul(self.storage_gas_unit_price));
        let computation_cost = u64::from(unit_computation_cost.mul(self.computation_gas_unit_price));
        Ok(GasCostSummary {
            computation_cost,
            storage_cost,
            storage_rebate,
...
pub fn deduct_gas(gas_object: &mut Object, deduct_amount: u64, rebate_amount: u64) {
    ...
    assert!(deduct_amount <= rebate_amount || balance >= deduct_amount - rebate_amount);
