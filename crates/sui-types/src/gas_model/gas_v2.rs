...
mod checked {
    ...
    pub struct SuiCostTable {
        ...
        max_gas_price_aborted_transactions: u64,
    }
    ...
    impl SuiCostTable {
        pub(crate) fn new(c: &ProtocolConfig, gas_price: u64) -> Self {
            ...
            Self {
                ...
                max_gas_price_aborted_transactions: c.max_gas_price_aborted_transactions(),
            }
        }
        ...
        pub(crate) fn unmetered() -> Self {
            Self {
                ...
                max_gas_price_aborted_transactions: 0,
            }
        }
    }
    ...
    impl SuiGasStatusAPI for SuiGasStatus {
        ...
        fn bucketize_computation(&mut self, aborted: Option<bool>) -> Result<(), ExecutionError> {
            ...
            let effective_gas_price = if aborted.unwrap_or(false) {
                // For Move aborts (business logic failures), cap at max but don't exceed user's price
                // This minimizes the risk of competing for priority execution in the case that the txn may be aborted.
                self.gas_price
                    .min(self.cost_table.max_gas_price_aborted_transactions)
            } else {
