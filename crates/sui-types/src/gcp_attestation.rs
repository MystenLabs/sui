...
// JWT header; `alg` is required, `kid` is validated when an expected kid is supplied.
...
pub fn extract_kid_from_jwt(token: &[u8]) -> Result<String, GcpAttestationError> {
...
pub fn verify_gcp_attestation(
    ...
    // Bind the caller-supplied kid to the JWT header kid when provided.
