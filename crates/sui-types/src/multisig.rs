...
impl AuthenticatorTrait for MultiSig {
        ...
        T: Serialize,
    {
        ...
        for (sig, i) in self.sigs.iter().checked_zip(as_indices(self.bitmap)?) {
            let (subsig_pubkey, weight) =
                self.multisig_pk
                    .pk_map
                    .get(i as usize)
                    .ok_or(SuiErrorKind::InvalidSignature {
    ...
    pub fn combine(
        ...
        for s in full_sigs {
            ...
            let index = multisig_pk
                .get_index(&pk)
                .ok_or(SuiErrorKind::IncorrectSigner {
...
impl FromStr for MultiSig {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes = Base64::decode(s).map_err(|_| SuiErrorKind::InvalidSignature {
            error: "Invalid base64 string".to_string(),
        })?;
        let sig = MultiSig::from_bytes(&bytes).map_err(|_| SuiErrorKind::InvalidSignature {
...
impl MultiSigPublicKey {
    ...
    pub fn new(
        ...
        Ok(MultiSigPublicKey {
            pk_map: pks.into_iter().checked_zip(weights).collect(),
            threshold,
        })
    }
