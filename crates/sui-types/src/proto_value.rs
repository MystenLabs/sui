...
impl<'b, 'l> Visitor<'b, 'l> for ProtoVisitor {
    ...
    fn visit_vector(&mut self, driver: &mut VecDriver<'_, 'b, 'l>) -> Result<Value, Error> {
        ...
        let value = if driver.element_layout().is_type(&TypeTag::U8) {
            ...
            let bytes = BytesVisitor.visit_vector(driver)?;
    ...
    fn visit_struct(&mut self, driver: &mut StructDriver<'_, 'b, 'l>) -> Result<Value, Error> {
        ...
        let value = if ty == &move_ascii_str_layout().type_ || ty == &move_utf8_str_layout().type_ {
            // 0x1::ascii::String, 0x1::string::String
            let s = StringVisitor.visit_struct(driver)?;
            self.debit_string_value(s)?;
            Value::from(s)
        } else if ty == &UID::type_() {
            // 0x2::object::UID
            let uid = UidVisitor.visit_struct(driver)?.to_canonical_string(true);
            self.debit_string_value(&uid)?;
            Value::from(uid)
        } else if ty == &ID::type_() {
            // 0x2::object::ID
            let id = IdVisitor.visit_struct(driver)?.to_canonical_string(true);
            ...
            // 0x2::balance::Balance
            let balance = BalanceVisitor.visit_struct(driver)?.to_string();
            self.debit_string_value(&balance)?;
            Value::from(balance)
        } else if is_url_type(ty) {
            // 0x2::url::Url
            let url = UrlVisitor.visit_struct(driver)?;
            self.debit_string_value(url)?;
            Value::from(url)
        } else {
            // Arbitrary structs
...
impl<'b, 'l> Visitor<'b, 'l> for BytesVisitor {
    type Value = &'b [u8];
    type Error = Error;
    ...
    fn visit_vector(&mut self, driver: &mut VecDriver<'_, 'b, 'l>) -> Result<Self::Value, Error> {
            ...
            .get(driver.position()..(driver.position() + driver.len() as usize))
        {
            ...
            Err(Error::Visitor(annotated_visitor::Error::UnexpectedEof))
        }
    }
    ...
    fn visit_struct(&mut self, _: &mut StructDriver<'_, 'b, 'l>) -> Result<Self::Value, Error> {
        Err(Error::UnexpectedType)
    }
...
impl<'b, 'l> Visitor<'b, 'l> for AddressVisitor {
...
impl<'b, 'l> Visitor<'b, 'l> for IdVisitor {
    type Value = AccountAddress;
    type Error = Error;
    ...
    fn visit_struct(
        ...
        if driver.struct_layout() == &ID::layout() {
            driver
                .next_field(&mut AddressVisitor)?
                .map(|(_, id)| id)
...
impl<'b, 'l> Visitor<'b, 'l> for UidVisitor {
    type Value = AccountAddress;
    type Error = Error;
    ...
    fn visit_struct(
        ...
        if driver.struct_layout() == &UID::layout() {
            driver
                .next_field(&mut IdVisitor)?
                .map(|(_, id)| id)
                .ok_or(Error::UnexpectedType)
        } else {
            Err(Error::UnexpectedType)
...
impl<'b, 'l> Visitor<'b, 'l> for StringVisitor {
    type Value = &'b str;
    type Error = Error;
    ...
    fn visit_struct(
        ...
        if driver.struct_layout() == &move_ascii_str_layout()
            || driver.struct_layout() == &move_utf8_str_layout()
        {
            driver
                .next_field(&mut BytesVisitor)?
                .ok_or(Error::UnexpectedType)
                .and_then(|(_, bytes)| std::str::from_utf8(bytes).map_err(Into::into))
        } else {
            Err(Error::UnexpectedType)
        }
    }
...
fn is_url_type(s: &StructTag) -> bool {
    s.address == SUI_FRAMEWORK_ADDRESS
        && s.module.as_ident_str() == URL_MODULE_NAME
        && s.name.as_ident_str() == URL_STRUCT_NAME
}
...
impl<'b, 'l> Visitor<'b, 'l> for UrlVisitor {
    type Value = &'b str;
    type Error = Error;
    ...
    fn visit_struct(
        ...
        if driver.struct_layout() != &url_layout() {
            return Err(Error::UnexpectedType);
        }

        driver
            .next_field(&mut StringVisitor)?
            .map(|(_, s)| s)
            .ok_or(Error::UnexpectedType)
    }
...
impl<'b, 'l> Visitor<'b, 'l> for BalanceVisitor {
    type Value = u64;
    type Error = Error;
    ...
    fn visit_struct(
        ...
        if !is_balance(driver.struct_layout()) {
            return Err(Error::UnexpectedType);
        }
    ...
    fn visit_u64(&mut self, _: &ValueDriver<'_, 'b, 'l>, value: u64) -> Result<Self::Value, Error> {
        Ok(value)
    }
