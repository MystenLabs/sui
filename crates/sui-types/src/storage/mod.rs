...
pub trait ChildObjectResolver {
    ...
    /// Checks whether `owner` holds at least `amount` of `type_`, accounting for earlier withdrawals
    /// from the same account within this transaction and for the accumulator root having caught up
    /// to the version this transaction requires. Only `TemporaryStore`, the execution-time resolver,
    /// implements the real check; other resolvers are never the execution-time resolver, so the
    /// default reports `Sufficient` and never blocks. `deposited` is the amount already deposited to
    /// this account so far in the transaction, which offsets the withdrawals (the check is against
    /// the net withdrawn). When the accumulator root has not caught up, the implementation records a
    /// retry request and returns `Unknown`.
