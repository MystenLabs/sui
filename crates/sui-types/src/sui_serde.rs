...
/// A macro that helps serializing and deserializing a struct that implements:
/// 1. [`fn as_bytes(&self) -> Vec<u8>`] and [`fn from_bytes(bytes: &[u8])`]: This is used for non-human readable ser/de.
/// Note that this is different from [trait ToFromBytes] where as_bytes is required to return a reference to a byte slice.
///
/// 2. [trait EncodeDecodeBase64]: This is used for human readable ser/de.
#[macro_export]
        ...
        impl ::serde::Serialize for $type {
            fn serialize<S: ::serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
                match serializer.is_human_readable() {
                    ...
                    false => self.as_bytes().serialize(serializer),
                }
            }
        }
