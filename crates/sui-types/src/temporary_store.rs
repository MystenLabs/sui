...
#[derive(Clone)]
pub struct TemporaryLayoutResolver2<R> {
    temp_store: Arc<InnerTemporaryStore>,
    fallback: R,
}

impl<R> TemporaryLayoutResolver2<R> {
    pub fn new(temp_store: Arc<InnerTemporaryStore>, fallback: R) -> Self {
        Self {
            temp_store,
            fallback,
        }
...
impl<R> ModuleResolver for TemporaryLayoutResolver2<R>
where
    R: ModuleResolver<Error = SuiError>,
{
    type Error = SuiError;
...
impl<R: BackingPackageStore> BackingPackageStore for TemporaryLayoutResolver2<R> {
    fn get_package_object(&self, package_id: &ObjectID) -> SuiResult<Option<Object>> {
        let obj = self.temp_store.written.get(package_id);
        if let Some((_, o, _)) = obj {
            debug_assert!(
