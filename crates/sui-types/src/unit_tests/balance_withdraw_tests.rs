...
/// The allowance is included as a declared shared-object input.
...
#[test]
fn test_check_allowance_inputs() {
    ...
    let err = tx
        .check_allowance_inputs(&InputObjects::new(vec![]))
        .unwrap_err();
    ...
    let err = tx.check_allowance_inputs(&not_an_allowance).unwrap_err();
