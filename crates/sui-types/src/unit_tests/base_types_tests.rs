...
use crate::crypto::{get_key_pair, get_key_pair_from_bytes, AccountKeyPair, AuthorityKeyPair, AuthoritySignature, Signature, SuiAuthoritySignature, SuiSignature, get_key_pair_from_private_key_bytes};
use crate::id::{ID, UID};
...
const SAMPLE_ED25519_PRIV_KEY: [u8; 32] = [
    10, 112, 5, 142, 174, 127, 187, 146, 251, 68, 22, 191, 128, 68, 84, 13, 102, 71, 77, 57,
    92, 154, 128, 240, 158, 45, 13, 123, 57, 21, 194, 214,
];

const SAMPLE_ED25519_PUB_KEY: [u8; 32] = [
    189, 215, 127, 86, 129, 189, 1, 4, 90, 106, 17, 10, 123, 200, 40, 18, 34, 173, 240, 91, 213,
    72, 183, 249, 213, 210, 39, 181, 105, 254, 59, 163,
];


// Derive a sample address and key-pair tuple from KeyPair bytes.
fn derive_sample_address_from_keypair() -> (SuiAddress, AccountKeyPair) {
    let (address, keypair) = get_key_pair_from_bytes(
        &[SAMPLE_ED25519_PRIV_KEY, SAMPLE_ED25519_PUB_KEY].concat())
    .unwrap();
    (address, keypair)
}
...
fn test_address_backwards_compatibility() {
    let decode_error_msg = "If this test broke, then the algorithm for deriving addresses \
        from public keys has changed. If this was intentional, please compute a new sample address \
        in hex format from `derive_sample_address` and update the SAMPLE_ADDRESS const above with \
        the new derived address hex value. Note that existing deployments (i.e. devnet) \
        might also require updates if they use fixed values generated by the old algorithm.";

    let (address1, _) = get_key_pair_from_bytes(
        &[SAMPLE_ED25519_PRIV_KEY, SAMPLE_ED25519_PUB_KEY].concat());
    assert_eq!(
        address1.to_vec(),
        Hex::decode(SAMPLE_ADDRESS).expect("Decoding failed"), "{}", decode_error_msg);

    let (address2, _) = get_key_pair_from_private_key_bytes(&SAMPLE_ED25519_PRIV_KEY);
    assert_eq!(
        address2.to_vec(),
        Hex::decode(SAMPLE_ADDRESS).expect("Decoding failed"), "{}", decode_error_msg);
}
