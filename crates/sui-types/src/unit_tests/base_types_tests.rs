...
#[test]
fn test_signatures() {
    let (addr1, sec1) = KeyPair::get_key_pair();
    let (addr2, _sec2) = KeyPair::get_key_pair();
...
fn test_signature_serde_not_human_readable() {
    let (_, key) = KeyPair::get_key_pair();
...
fn derive_sample_address() -> (SuiAddress, KeyPair) {
    let (address, pub_key) = KeyPair::get_key_pair_from_bytes(&[
