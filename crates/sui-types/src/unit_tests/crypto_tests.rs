...
fn test_serde_suikeypair_roundtrip() {
    ...
    // the serialized bytes of SuiKeyPair should extend its member by 1 byte representing the scheme.
    assert_eq!(serialized_skp.len(), serialized_kp.len() + 1);
}
...
fn test_serde_public_key() {
    let kp: Ed25519KeyPair = get_key_pair().1;
    let skp: SuiKeyPair = SuiKeyPair::Ed25519(kp.copy());
    let pk = kp.public();
    ...
    // the serialized bytes of PublicKey should extend its member by 1 byte representing the scheme.
    assert_eq!(serialized_sui_pk.len(), serialized_pk.len() + 1);
}
