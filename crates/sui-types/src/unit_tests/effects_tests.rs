...
fn test_written_with_created_objects() {
    ...
    assert!(written.iter().any(|(id, _, digest)| {
        *id == obj1 && digest.is_alive()
    }));
    assert!(written.iter().any(|(id, _, digest)| {
        *id == obj2 && digest.is_alive()
    }));
    // Check gas object is included
    assert!(written.iter().any(|(id, _, digest)| {
        *id == gas_object_id && digest.is_alive()
    }));
}
...
fn test_written_with_combination_of_all_types() {
    ...
    assert!(written.iter().any(|(id, _, digest)| {
        *id == created1 && digest.is_alive()
    }));
    assert!(written.iter().any(|(id, _, digest)| {
        *id == created2 && digest.is_alive()
    }));
    
    // Verify mutated objects have alive digests
    assert!(written.iter().any(|(id, _, digest)| {
        *id == mutated1 && digest.is_alive()
    }));
    assert!(written.iter().any(|(id, _, digest)| {
        *id == mutated2 && digest.is_alive()
    }));
    ...
    assert!(written.iter().any(|(id, _, digest)| {
        *id == unwrapped1 && digest.is_alive()
    }));
    assert!(written.iter().any(|(id, _, digest)| {
        *id == unwrapped2 && digest.is_alive()
    }));
...
fn test_written_excludes_frozen_objects() {
    ...
    let effects = TestEffectsBuilder::new(tx.data())
        ...
        .with_mutated_objects(vec![
            (mutated, SequenceNumber::from(5), Owner::AddressOwner(sender)),
        ])
        .build();
...
fn test_written_version_numbers() {
    ...
    let effects = TestEffectsBuilder::new(tx.data())
        ...
        .with_mutated_objects(vec![
            (obj2, SequenceNumber::from(5), Owner::AddressOwner(sender)),
        ])
        .with_deleted_objects(vec![
            (obj3, SequenceNumber::from(10)),
        ])
        .build();
