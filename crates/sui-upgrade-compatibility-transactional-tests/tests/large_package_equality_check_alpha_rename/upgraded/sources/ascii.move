   ...
   spec Char {
    ...
    public fun all_characters_printable(my_string: &String): bool { // !!!! <<<<<<<< This has been renamed from "string" to "my_string"
       ...
       while ({
           spec {
               invariant i <= length;
               invariant forall j in 0..i: is_printable_char(my_string.bytes[j]);
           };
           i < length
       }) {
       ...
       spec {
           assert i == length;
           assert forall j in 0..length: is_printable_char(my_string.bytes[j]);
       };
       true
    }
    spec all_characters_printable {
        ensures result ==> (forall j in 0..len(my_string.bytes): is_printable_char(my_string.bytes[j]));
    }
