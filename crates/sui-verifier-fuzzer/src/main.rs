...
fn main() {
    ...
    fuzz!(|input: &[u8]| {
        if let Ok(m) = move_binary_format::file_format::CompiledModule::deserialize(&input) {
            if debug {
        ...
        if let Ok(m) = move_binary_format::file_format::CompiledModule::arbitrary(&mut unstructured)
        {
            ...
            match args[1].as_str() {
                ...
                "--sui-move-id-leak" => {
                    ...
                    if let Ok(()) = move_bytecode_verifier::verify_module(&m) {
                        if let Ok(()) = id_leak_verifier::verify_module(&m, &mut DummyMeter) {
                            process::exit(0);
                        }
                    }
                }
                "--sui-move-entry-points" => {
                    if let Ok(()) = move_bytecode_verifier::verify_module(&m) {
                        if let Ok(()) = entry_points_verifier::verify_module(
                ...
                "sui-move" | _ => {
                    ...
                    if let Ok(()) = move_result {
                        ...
                        if let Ok(()) = sui_result {
                            ...
                            dbg!("fails sui verifier");
                            dbg!(sui_result.err().unwrap());
                        }
                    } else {
                        if debug {
                            dbg!("fail move verifier");
