...
fn verify_init_function(module: &CompiledModule, fdef: &FunctionDefinition) -> Result<(), String> {
    ...
    if is_tx_context(view, &parameters[parameters.len() - 1]).unwrap_or_default() {
        Ok(())
    } else {
        Err(format!(
            "Expected last parameter for {0}::{1} to be &mut {2}::{3}::{4}. but found {5}",
            module.self_id(),
            INIT_FN_NAME,
...
fn verify_entry_function_impl(
    ...
    let all_non_ctx_params = match params.0.last() {
        Some(last_param) if is_tx_context(view, last_param).is_some() => {
...
// Returns Some(is_mut) if the type is a reference to the TxnContext. is_mut being true with
// a MutableReference, and false otherwise.
// Returns None for all other types
pub fn is_tx_context(view: &BinaryIndexedView, p: &SignatureToken) -> Option<bool> {
    match p {
        SignatureToken::MutableReference(m) | SignatureToken::Reference(m) => match &**m {
            SignatureToken::Struct(idx) => {
                ...
                if is_tx_context_type {
                    Some(matches!(p, SignatureToken::MutableReference(_)))
                } else {
                    None
                }
            }
