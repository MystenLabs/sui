...
pub struct TxProcessingArgs {
    ...
    /// Override the sender address for the transaction. This option must be used with
    /// --serialize-unsigned-transaction since this sender's private key
    /// might not be available in the keystore. The resulting unsigned transaction
    /// can then be signed externally using this sender's private key.
