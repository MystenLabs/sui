...
use sui_types::{messages::SenderSignedData, object::Owner};
...
pub async fn call_move(
    ...
    let data = context
        ...
        .await?;
    let tx_signature = context.keystore.sign(&sender, &data.to_bytes())?;
    let transaction = Transaction::new(SenderSignedData { data, tx_signature });
