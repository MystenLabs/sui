...
pub(crate) async fn compile_package(
    ...
    pkg_tree_shake(&root_pkg, &mut compiled_package).await?;
...
pub fn update_publication(
    ...
    match command {
        LockCommand::Publish => {
            let (upgrade_cap, version, _) = response
...
async fn publish_command(
    ...
    // if let Some(old_addresses) = old_addresses {
    //     let _ = set_original_id(&mut root_package, old_addresses)?;
    // }
...
// /// environment
// fn update_toolchain_version_in_lockfile(root_package: &mut RootPackage<SuiFlavor>) {
//     if let Some(published_data) = root_package.publication().as_mut() {
//         published_data.metadata.toolchain_version = Some(env!("CARGO_PKG_VERSION").into());
//     }
// }
