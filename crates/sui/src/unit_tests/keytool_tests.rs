...
fn test_addresses_command() -> Result<(), anyhow::Error> {
    ...
    let mut keystore = Keystore::from(InMemKeystore::new_insecure(3));
...
fn test_flag_in_signature_and_keypair() -> Result<(), anyhow::Error> {
    let mut keystore = Keystore::from(InMemKeystore::new_insecure(0));
...
fn test_sign_command() -> Result<(), anyhow::Error> {
    // Add a keypair
    let mut keystore = Keystore::from(InMemKeystore::new_insecure(1));
