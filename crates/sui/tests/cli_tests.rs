...
use sui_json_rpc_types::{
    get_new_package_obj_from_response, OwnedObjectRef, SuiObjectData, SuiObjectDataFilter,
...
async fn test_package_management_on_upgrade_command() -> Result<(), anyhow::Error> {
    ...
    let mut upgrade_lock_file_path = PathBuf::from(upgrade_pkg_path.clone());
    ...
    let content = std::fs::read_to_string(publish_lock_file_path.clone())?;
    eprintln!("content: {}", content);
    eprintln!(
        "copying from {} to {}",
        publish_lock_file_path.display(),
        upgrade_lock_file_path.display()
    );
    let content = std::fs::read_to_string(upgrade_lock_file_path.clone())?;
    eprintln!("DESTINATION CONTENT: {}", content);
    ...
    eprintln!("reading from {}", lock_file.display());
