...
{
    (
        lower_bound.map(|b| be_fix_int_ser(&b).unwrap()),
        upper_bound.map(|b| be_fix_int_ser(&b).unwrap()),
    )
}
    ...
    K: Serialize,
{
    let iterator_lower_bound = match range.start_bound() {
        Bound::Included(lower_bound) => {
            ...
            Some(be_fix_int_ser(&lower_bound).expect("Serialization must not fail"))
    ...
    let iterator_upper_bound = match range.end_bound() {
        ...
        Bound::Excluded(upper_bound) => {
            ...
            Some(be_fix_int_ser(&upper_bound).expect("Serialization must not fail"))
            // readopts.set_iterate_upper_bound(key_buf);
        }
        Bound::Unbounded => None,
    };
