...
The idea here is to enable the viewing of logging, traces, and metrics with industry-standard open source tools.  Thoughts:
- JSON logs from `SUI_JSON_SPAN_LOGS` could be fed directly to ElasticSearch or similar backends for easy indexing of fields and aggregation
- Numbers from the output could be fed into Prometheus
