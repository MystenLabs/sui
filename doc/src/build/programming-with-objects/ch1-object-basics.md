...
> :bulb: The `UID` type is internal to Sui, and you most likely won't need to deal with it directly. For curious readers, it contains the "unique ID" that defines an object on the Sui chain. It is unique in the sense that no two values of type `UID` will ever have the same underlying set of bytes.
...
```
This places `obj` in global storage along with metadata that records `recipient` as the owner of the object. In Sui, every object must have an owner, which can be either an address, another object, or "shared"--see [object ownership](https://github.com/MystenLabs/sui/blob/main/doc/src/build/move/sui-move-library.md) for more details.
