...
In Chapter 1 & 2, we learned how to create and use objects owned by an account address. In this chapter, we will demonstrate how to create and use immutable objects.

An object in Sui can have different types of [ownership](../objects.md#object-ownership), with two broad categories: owned object and shared object. There are two types of shared object: shared immutable and shared mutable. A shared immutable object is an object that can **never** be mutated, transferred or deleted. Because of the immutability, the object is not owned by anyone, and hence it can be used by anyone (thus shared). We will call this type of object "*immutable object*" for short. In a latter chapter we will talk about shared mutale object (which is usually what we refer to with the term *shared object*).

### Create Immutable Object

Given an object, regardless of whether it was just created or already owned by an account, to turn this object into an immutable object, we need to call this API in the [Transfer Library](../../../../sui_programmability/framework/sources/Transfer.move):
```rust
public native fun freeze_object<T: key>(obj: T);
```
After this call, the specified object will become permanently immutable. This is a non-reversible operation, hence only freeze an object when you are certain that it will never need to be mutated.
...
> :bulb: Note the `Transfer::freeze_object` API requires passing the object by value. Had we allowed passing the object by a mutable reference, we will then still be able to mutate the object after the `freeze_object` call, which contradicts to the fact that it should have become immutable.

Alternatively, you can also provide an API that creates an immutable object at born:
```rust
...
2. Anyone can use immutable objects. Immutable objects are considered as shared.
...
Since immutable objects can never be mutated, there will never be a data race even when multiple transactions are using the same immutable object at the same time. Hence the existence of shared immutable object does not pose any requirement on consensus.

### Test Immutable Object
Let's take a look at how we interact with immutable objects in unittests.
Previously, we used the `TestScenario::take_object` API to take an object from the global storage that's owned by the sender of the transaction in a unittest. Since immutable objects are not owned by anyone, `TestScenario::take_object` works for immutable objects as well! That is, if there exists an immutable object of type `T` in the global storage, `take_object<T>` will return that object.
...
```
In this test, we submit a transaction as `sender1`, which would create an immutable object. To show that this object is indeed shared and can be used by anyone, we start 2 new transactions, one with `sender1` and another with `sender2`. In both transactions, we are able to take the object.
...
```
Put the package object ID to `$PACKAGE` as we did in previous chapters. Create a new `ColorObject`:
```
$ wallet call --gas-budget 1000 --package $PACKAGE --module "ColorObject" --function "create" --args 0 255 0
```
Put the newly created object ID to `$OBJECT`. If we look at the list of objects in the current active account address's wallet:
```
$ wallet objects --address=$ADDR
```
There should be one more, with ID `$OBJECT`. Let's turn it to immutable:
```
...
```
It will complain that an immutable object cannot be passed to mutable argument.