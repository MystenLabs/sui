...
description: Details for developers on how to use Nautilus using the provided reproducible build template
---

The [Nautilus framework](https://github.com/MystenLabs/nautilus) helps you deploy an AWS Nitro Enclave with all the necessary scaffolding, such as reproducible builds, signature formatting, and HTTPS traffic forwarding, so you can focus on implementing the offchain computation logic inside the enclave.
...
Refer to the [Github repo](https://github.com/MystenLabs/nautilus) for the code structure and related usage instructions below.
