...
The proof of possession is a BLS signature created using the authority protocol private key, committed over the following message: `intent || pubkey || address || epoch`. Here `intent` is serialized to `[5, 0, 0]` representing an intent with scope as `Proof of Possession`, version as `V0`, and app_id as `Sui`. `pubkey` is the serialized public key bytes of the authority BLS protocol key. `address` is the account address associated with the authority account key. `epoch` is serialized to `[0, 0, 0, 0, 0, 0, 0, 0]`.
