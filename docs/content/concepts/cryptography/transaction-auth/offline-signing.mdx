...
The console responds with the resulting `<SIGNED_TX_BYTES>` value.

After you obtain the signed transaction bytes, you can submit it using the execute combined signed transaction command. This command takes `--signed-tx-bytes` as the signed transaction bytes to execute (see output of the previous `sui client transfer` command). This executes the signed transaction and returns the certificate and transaction effects if successful.
