...
- The backend service (mysten/zklogin:prover-stable) is compute-heavy. We recommend using at least 16 cores and 16GB RAM. Using weaker instances will likely lead to timeout errors with the message "Call to rapidsnark service took longer than 15s". Note that the environment variable `PROVER_TIMEOUT` can be adjusted to set a different timeout value, e.g., `PROVER_TIMEOUT=30` for a timeout of 30 seconds.
