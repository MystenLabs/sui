...
Compared to traditional signatures, zkLogin signatures take a longer time to generate. For example, the prover that Mysten Labs maintains currently takes about three seconds to return a proof, which runs on a machine with 16 vCPUs and 64 GB RAM. Using more powerful machines, such as those with physical CPUs or graphics processing units (GPUs), can reduce the proving time further.
...
The prover that Mysten Labs maintains is set to auto-scale to handle traffic peaks. If you are not sure whether Mysten Labs can handle a specific number of requests, please reach out to us on Discord. Our plan is to horizontally scale the prover to handle any RPS you have in mind.
