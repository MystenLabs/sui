...
2. Use the GraphQL endpoint directly: https://sui-[network].mystenlabs.com/graphql. See [documenetation](https://docs.sui.io/references/sui-api/sui-graphql/reference/api/queries/verify-zklogin-signature). This is recommended if you do not plan to run any server or handle any JWK rotations. 

3. Use Sui Keytool. This is recommended for debug usage. 

```bash
$SUI_BINARY keytool zk-login-sig-verify --sig $ZKLOGIN_SIG --bytes $BYTES --intent-scope 3 --network devnet --curr-epoch 3
```

4. Use a self hosted server endpoint and calls this endpoint. See instructions in [zklogin-verifier](https://github.com/MystenLabs/zklogin-verifier). This provides flexibility in the logic.
