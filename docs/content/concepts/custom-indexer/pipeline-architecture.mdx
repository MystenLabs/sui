...
Without regulation, missing checkpoints can cause deadlocks in situations like the following one:
...
1. [Commit Watermark](#con-commit-watermark)
1. [Reader Watermark](#con-reader-watermark)
1. [Pruner](#con-pruner)

#### `Processor` {#con-processor}
    
The `Processor` is the concurrent processing engine.
...
#### `Commit Watermark` {#con-commit-watermark}
...
#### `Reader Watermark` {#con-reader-watermark}
...
The `Watermarks` table manages all watermark coordination. It is critical for recovery, as the framework reads this table to resume from the correct checkpoint.

The framework automatically creates and manages a `Watermarks` table in your database when you first run your indexer. The table might have only one row per pipeline, allowing multiple indexers to share the same database.
