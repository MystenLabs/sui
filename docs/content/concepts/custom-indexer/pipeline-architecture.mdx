...
The `sui-indexer-alt-framework` provides two distinct pipeline architectures. Understanding their differences is crucial for choosing the right approach, but deciding which one is best for a project can sometimes be difficult.
...
- Complex application logic: You must handle data commits in pieces rather than just handling complete checkpoints.

## Decision framework

If your project details do not provide a clear-cut approach, start with a sequential pipeline as it's easier to implement and debug. Then, measure performance under a realistic load. If the sequential pipeline can't meet your project's requirements, then switch to a concurrent pipeline.

Specific scenarios where a sequential pipeline might not meet requirements:

- Your pipeline produces data per checkpoint that benefits from chunking and out-of-order commits. Individual checkpoints can produce lots of data or individual writes that can take a lot of time to process atomically.
- You're producing a lot of data and need pruning. In this case, you must use a concurrent pipeline.

Beyond the decision of which pipeline to use, you also need to consider scaling. If you're indexing multiple kinds of data, then consider using multiple pipelines and introduce the complexity of reads consulting watermarks.
...
For each pipeline, the indexer minimally tracks the `checkpoint_hi_inclusive`, or highest checkpoint where all data up to that point is committed. Optionally, the pipeline tracks the `reader_lo` and `pruner_hi` if pruning is enabled. These watermarks are particularly crucial for concurrent pipelines to enable out-of-order processing while maintaining data integrity. Both concurrent and sequential pipelines rely on the `checkpoint_hi_inclusive` committer watermark to understand where to resume processing on restarts, while the `reader_lo` and `pruner_hi` define safe lower bounds for reading and pruning operations.
...
The ingestion layer (`Regulator` + `Broadcaster`) and `Processor` components are identical to concurrent pipelines - same backpressure mechanisms, same `FANOUT` parallel processing, and same `processor()` implementation.

The key difference is the dramatically simplified pipeline core with just a single `Committer` component that handles ordering, batching, and database commits. Concurrent pipelines, in contrast, have four separate components (`Collector`, `Committer`, `Commit Watermark`, `Reader Watermark`, `Pruner`).

### Sequential pipeline components {#sequential-components}

As mentioned, there are two main components to sequential pipelines.
...
The sequential `Commiter` is the heart of the pipeline and your main customization point. At a high level, the `Commiter` performs the following actions:
...
To customize, your code provides two key functions that the committer calls at specific points:
...
Without regulation, missing checkpoints can cause deadlocks in the following situation:

1. Pipeline waits for checkpoint 100.
2. Checkpoint 100 gets dropped or delayed.
3. Ingestion fills buffer with 101, 102, 103...
4. Buffer becomes full and cannot refetch checkpoint 100 when available.
5. Pipeline becomes permanently stuck.

To avoid this pitfall, you can use watermark progress reporting by having each pipeline report progress. After successful commits, have the pipelines send `(pipeline_name, highest_committed_checkpoint)` to the regulator via the `ingest_hi_tx` channel.
...
Concurrent pipelines can process checkpoints out-of-order. Missing checkpoints only delay watermark updates but don't halt overall progress. Consequently, only sequential pipelines need this type of regulation.
...
The `Commiter` primarily writes batched data to the database using parallel connections with retry logic. It does this by receiving optimized batches from `Collector`, then spawns up to `write_concurrency` parallel database writers.
...
The `Committer` tasks don't actually perform database operations. Rather, it calls your handler's `commit()` method. You implement the actual database logic.

:::

Commit `Watermark` (progress tracker)

The primary responsibility of the commit `Watermark` is to track which checkpoints are fully committed and update `checkpoint_hi_inclusive` in the `Watermark` table.

The `Watermark` receives `WatermarkParts` from successful `Committer` writes.

{@inject: crates/sui-indexer-alt-framework/src/pipeline/mod.rs#struct=WatermarkPart}
    
The `Watermark` maintains an in-memory map of checkpoint completion status, advancing `checkpoint_hi_inclusive` only when there are no gaps in the sequence. Periodically, it writes the new `checkpoint_hi_inclusive` to the `Watermark` database.

Remember that `checkpoint_hi_inclusive` can advance only when all data up to that point is committed with no gaps. This component enforces that critical rule. See the [watermark system](#watermark-system) for details.
...
The primary responsibility of the reader `Watermark` is to calculate and update `reader_lo` to maintain the retention policy and provide safe pruning boundaries.

The reader `Watermark` polls the `Watermark` database periodically (`interval_ms`) to check current `checkpoint_hi_inclusive`. It then calculates the new `reader_lo = checkpoint_hi_inclusive - retention + 1` value and updates the `reader_lo` and `pruner_timestamp` in the `Watermark` database. This behavior provides the safety buffer that prevents premature pruning.

The `reader_lo` value represents the lowest checkpoint guaranteed to be available. This component ensures your retention policy is maintained. See the [watermark system](#watermark-system) for details.
...
The `Pruner` tasks doesn't actually delete data. Rather, it calls your handler's `prune()` method. You implement the actual cleanup logic.
...
## Backpressure mechanisms {#concurrent-backpressure}
...
1. **Initial state**: Your indexer is processing 100 checkpoints/second.
1. **Bottleneck appears**: Database becomes slow (maybe high load, maintenance, or similar) - can now handle only 50 commits/second
...
The following sections detail the configuration settings you can employ to get the best performance from your environment.
...
impl concurrent::Handler for MyHandler {
    ...
    // Backpressure threshold on commiter (default: 5000)
