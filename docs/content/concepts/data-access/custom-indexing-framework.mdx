...
The indexing framework supports multiple data sources to maximize flexibility. These fall into two categories:
...
- **gRPC streaming** receives checkpoint data pushed from fullnodes, offering lower latency than polling. Because streaming only provides the latest data, it requires a polling-based fallback source for historical checkpoints and reliability.

For detailed configuration examples and use cases, see [Checkpoint Data Sources](/guides/developer/advanced/custom-indexer/indexer-data-integration.mdx#checkpoint-data-sources).
...
`Broadcaster` is backpressured by the high watermarks reported by subscribers, so that the data flow rate doesn't overwhelm the slowest processing pipelines.
