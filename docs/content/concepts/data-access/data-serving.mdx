...
The [General-purpose Indexer](/concepts/data-access/custom-indexers.mdx) includes a performant and scalable implementation of the [custom indexing framework](/guides/developer/accessing-data/custom-indexing-framework.mdx). The underlying framework uses the remote checkpoint store and full node RPCs to ingest data.
