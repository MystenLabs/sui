...
- Run the Indexer pipelines and GraphQL service, while using the [Archival Service](/concepts/archival-store) and a full node from an RPC provider or indexer operator.
...
If you're running the GraphQL RPC + General-purpose Indexer stack as a service, here are a few key considerations for configuring your setup to offer builders a performant and cost-effective experience. Additionally, please refer to the [GraphQL and General-Purpose Indexer](/guides/operator/indexer-stack-setup) setup guide for detailed instructions on setting up and operating the full stack.
