...
| -------------------- | --------------------------------------------------------- | -------------------------- |
| **Consistent Store** | `Query.objects`, `Address.balance`, objects by owner/type | ~1 hour                    |
| **DB Store**         | `Query.transactions`, `Query.events`, `Query.checkpoints` | ~(30-90) days              |
| **Archival Service** | Point lookups (tx by digest, object by ID+version)        | Indefinite (if configured) |

:::tip Operators

Retention is configured by the stack operator. For production configuration examples, see [Indexer Stack Setup](/guides/operator/indexer-stack-setup).
...
- **`type`**: The GraphQL type name (e.g., `"Query"`, `"Address"`)
- **`field`**: The field on that type (e.g., `"transactions"`, `"events"`)
- **`filters`**: Filter field names from the filter input type (e.g., `["affectedAddress"]` for `TransactionFilter`, `["module"]` for `EventFilter`), or direct query parameter names (e.g., `["version"]`)
...
```graphql
# Check available range for basic transaction pagination
{
	serviceConfig {
...
```graphql
# Check available range for transactions filtered by affected address
{
	serviceConfig {
...
```graphql
# Check available range for events filtered by emitting module
{
	serviceConfig {
		availableRange(type: "Query", field: "events", filters: ["module"]) {
			first {
				sequenceNumber
			}
			last {
