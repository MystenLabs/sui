...
# Indexer-wide rate limiting
--max-rows-per-second 5000     # Cap total writes across all pipelines
```
...
- **Rate limiting:** Prevents aggregate write load from overwhelming the database when running many concurrent pipelines. Can be combined with per-pipeline `max_rows_per_second` in `CommitterConfig`.
...
During backfills the indexer can produce rows faster than the database can absorb them. Rate limiting is available at two levels:

- **Indexer-wide:** The `--max-rows-per-second` CLI flag sets a global cap shared across all concurrent pipelines. Every pipeline's committer acquires tokens from this shared limiter before writing.
- **Per-pipeline:** Each concurrent pipeline supports an optional `max_rows_per_second` setting in its `CommitterConfig` that caps write throughput for that pipeline alone.

When both are configured, each committer acquires tokens from both limiters concurrently, so the effective rate for a given pipeline is bounded by whichever limit is reached first. Both limiters use a smooth (GCRA) algorithm so writes are spread evenly rather than bursting. See [`CommitterConfig` optimization](/concepts/data-access/pipeline-architecture.mdx#committerconfig-optimization) for details.
...
The framework tracks extensive metrics across ingestion, pipeline processing, database operations, and watermark management. For a complete list of available metrics with descriptions, refer to the `IndexerMetrics` struct in [`sui-indexer-alt-framework/src/metrics.rs`](https://github.com/MystenLabs/sui/blob/main/crates/sui-indexer-alt-framework/src/metrics.rs).
