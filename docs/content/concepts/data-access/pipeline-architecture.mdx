...
- **`max_rows_per_second`:** Caps the combined write rate across all `write_concurrency` workers for this pipeline. Useful during backfills to prevent the indexer from overwhelming the database. The limit uses a smooth (GCRA) rate limiter so writes are spread evenly rather than bursting. Set to `None` (the default) for unlimited throughput. For an indexer-wide cap across all pipelines, use the `--max-rows-per-second` CLI flag (see [Rate limiting writes](/concepts/data-access/indexer-runtime-perf.mdx#rate-limiting-writes)).
