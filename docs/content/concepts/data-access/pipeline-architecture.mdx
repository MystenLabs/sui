...
Each pipeline enforces a per-pipeline row limit via the `MAX_PENDING_ROWS` constant on its `Handler` trait. Before pulling the next checkpoint from the ingestion channel, the pipeline's processor waits until its own uncommitted row count drops below this threshold.
