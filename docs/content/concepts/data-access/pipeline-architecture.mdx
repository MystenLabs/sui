...
- **Broadcaster → Processor:** `checkpoint_channel_size` slots (defaults to `checkpoint_buffer_size`).
...
**`Broadcaster` to `Processor`**: `checkpoint_channel_size` slots (defaults to `checkpoint_buffer_size`) → `Broadcaster` blocks, upstream pressure.
...
By default the per-pipeline channel is sized to `checkpoint_buffer_size`, coupling channel depth with the lag regulation window. Setting `checkpoint_channel_size` independently lets you keep a large lag window (so the ingestion service can fetch far ahead) while using a smaller channel per pipeline to limit memory. This can be helpful to increase backfill throughput.
...
Each concurrent pipeline enforces a per-pipeline row limit via the `MAX_PENDING_ROWS` constant on its `Handler` trait. Before pulling the next checkpoint from the ingestion channel, the pipeline's processor waits until its own uncommitted row count drops below this threshold. Sequential pipelines do not use row-count backpressure because their in-order commit requirement can create deadlocks: the committer may need future checkpoints to make progress, but row-count backpressure prevents the processor from delivering them.

<ImportContent mode="code" source="crates/sui-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="MAX_PENDING_ROWS" />

This is useful when pipelines produce highly variable numbers of rows per checkpoint. Channel-based backpressure counts checkpoints, not rows, so a single checkpoint with tens of thousands of rows can spike memory even though only one checkpoint is buffered. Because the limit is per-pipeline, pipelines that produce large rows (such as `kv_objects`) can be tuned independently from pipelines that produce small rows (such as `tx_digests`). When a pipeline's processor pauses, its ingestion channel naturally fills up, creating backpressure on the broadcaster without the broadcaster needing to know about row counts.
...
impl concurrent::Handler for MyHandler {
    ...
    // Backpressure threshold on processor (default: 5000)
...
- **`MAX_PENDING_ROWS`:** Controls how much data can accumulate before the processor pauses. Higher values provide more buffer space but use more memory during bottlenecks.
