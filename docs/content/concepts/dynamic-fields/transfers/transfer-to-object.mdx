...
You should also always be aware of both the type of the object you are transferring to and the object that is being transferred since the object that is transferred to (i.e., the parent) can _always_:
...
And, if the object being transferred is `key` only then:
- The module that defines the object that is being transferred will also need to implement a custom receive function for it in a similar manner to custom transfer functions. Just as with custom transfer functions, custom receivership function may have arbitrary restrictions they may enforce and that you should be aware of, or they may not exist.
- Once sent, the object will not be able to be accessed or used unless the parent's (object being sent-to) module has defined a function to receive objects _and_ the child's (object being sent) module has defined a function to receive the object, and the restrictions that _both_ functions define are met. 
...
module sui::transfer {
    ...
    ///
    /// This function has custom rules enforced by the Sui Move bytecode verifier that ensures
...
Just like with [custom transfer policies](./custom-rules.mdx), Sui allows for the definition of custom receivership rules for `key`-only objects. In particular, the `sui::transfer::receive` function can only be used on objects defined in the same module as the call to `sui::transfer::receive`--just like the `sui::transfer::transfer` function can only be used on objects defined in the module where it is being used.

Similarly for objects that also have the `store` ability, they can be received from by anyone using the `sui::transfer::public_receive` function--just like `sui::transfer::public_transfer` can transfer any objects that have the `store` ability on them.

This coupled with the fact that the parent object can always define custom rules around receivership means that we have the following matrix of permissions to consider around receiving objects and the abilities of the object being sent based on the child object's abilities:
...
Just like with custom transfer policies there's a lot of power that can be expressed with these restritions, and coupled with each other they can be quite expressive. For example, you can implement [soul-bound objects](#soul-bound-example) using both custom transfer and receivership rules.
