...
This page explores the key features of Sui for gaming, real-world use cases, and essential tools for builders. Whether you're building an RPG, a racing game, or a digital card game, Sui provides the Web3 infrastructure to power your next-generation gaming projects.

## Player experience

Video games are less fun when there is a large barrier to entry. Sui provides primitives that allow you to create a simple pathway from player account and profile creation, to actually playing your game.

### Simplified onboarding

Most players are familiar with using some sort of single sign-on (SSO) or social login to create their game accounts. On Sui, you can leverage [zkLogin](/concepts/cryptography/zklogin.mdx) to simplify player account creation through a Web2-like onboarding process, removing the friction of wallet setup.

### Sponsored transactions

With the rising cost of AAA titles, modern players are often opting for free-to-play (F2P) games that do not require an upfront cost. Sponsored transactions allow you, or a designated entity, to cover the required gas fees for a player, removing the friction of purchasing SUI tokens.

## Fair and transparent gameplay

Trusting that a game is fair is essential to a positive gaming experience and player retention. Modern game mechanics such as loot boxes, procedural world generation, and player versus player (PVP) matchmaking, will often drive players away from a game if they perceive the systems as rigged or predetermined.

### On-chain randomness and verification

Sui provides [native randomness](/guides/developer/advanced/randomness-onchain.mdx), allowing games to operate without requiring players to trust a centralized operator.

Additionally, all transactions on the Sui network are viewable and verifiable. Sui leverages threshold cryptography and Distributed Key Generation (DKG), which generates secreate shares of a distributed key at the beginning of each epoch, and continuously uses them to produce randomness. 

Off-chain computation can also be verified on-chain by using [Nautilus](/concepts/cryptography/nautilus.mdx). 
...
In-game currencies allow users to purchase game features like items, upgrades, and premium content. In Web2 games, these currencies are exclusively in-game, with their values set by the developers. 
...
[Non-fungible tokens (NFTs)](/guides/developer/nft.mdx) are able to represent many traditional game features. 

For example, in-game objects, battle passes, rewards, skins, game cards or keys, and loot boxes are just a few of the features that you can create using NFTs.

On Sui, everything is an object and all objects are NFTs. For gaming discussions, you can also think of these NFTs as game assets that can be generally categorized as either [dynamic assets](#dynamic-assets) or [soulbound assets](#soulbound-assets).
...
The [Sui Object Display](/standards/display.mdx) standard is a tool that helps define how objects appear in apps and interfaces. It works like a template where you can insert data from an object to control how it's displayed. Use it to manage how different types of data are shown, even if the data itself is stored on-chain while the display happens off-chain (in apps or websites).
