...
We recommend retaining **30 to 90 days** of recent checkpoint data in your Postgres-compatible database. This provides a strong default for most apps without incurring the high storage costs of full historical indexing.
...
You can configure your indexing pipelines to scope which data you include (e.g. events, objects, transactions), and disable any components that aren’t needed.
...
We strongly encourage all production deployments to pair Postgres with the [Archival Service](./archival-store.mdx) to support points lookups of transactions, objects, and checkpoints when relevant data does not exist in Postgres.
...
- While not strictly required, we strongly recommend using the Archival Service in any production setup that aims to support `high-retention` GraphQL or gRPC workloads.

Current implementation supports [GCP Bigtable](https://cloud.google.com/bigtable) which is a highly scalable and performant data store. If you plan to operate your own archival store, see [sui-kvstore](https://github.com/MystenLabs/sui/blob/main/crates/sui-kvstore/src/main.rs) and [sui-kv-rpc](https://github.com/MystenLabs/sui/tree/main/crates/sui-kv-rpc) for indexer setup and RPC service implementation respectively. For the indexer setup, make sure to use the [custom indexing framework](../guides/developer/advanced/custom-indexer). If you're interested in contributing support for other scalable data stores, reach out on GitHub.

### Deployment strategies and trade-offs

You don’t need to index everything to provide a reliable and performant GraphQL RPC service. In fact, many developers may only need the latest object and transaction data plus a few weeks to months of history. You can reduce operational overhead and improve query performance by:

- Configuring a clear retention window (e.g., 30–90 days) in Postgres
...
- Postgres-only with short-retention = lower storage cost and faster performance, but limited historical coverage
- Postgres-only with high retention = broader data coverage, but relatively higher storage cost and slower performance at scale
- Postgres with short-retention + Archival Service = optimized for cost and completeness, ideal for production deployments
...
- Consider offering different tiers of service to meet different developer needs. For example:
    - A basic tier that serves recent data (e.g., 30 days) via GraphQL RPC or gRPC.
    - A premium tier with full GraphQL / gRPC + Archival Service access, suited to apps that need historical lookups.
    - You can also offer region-specific instances or throughput-based pricing to support diverse client footprints.
