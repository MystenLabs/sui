...
Switch any apps that still use the GraphQL Alpha endpoints (`https://sui-mainnet.mystenlabs.com/graphql` and `https://sui-testnet.mystenlabs.com/graphql`) to the GraphQL Beta endpoints as soon as possible to avoid disruption of service.
...
The GraphQL service does not support consistent pagination for arbitrarily old cursors. A cursor can grow stale, if the checkpoint it is from is no longer in the **available range**. Data outside of the available range can also be pruned away. This range might be different depending on the specific query and filters used. You can query the upper- and lower-bounds of that range for your query as follows:
