...
gRPC offers a high-performance, efficient communication protocol that uses [Protocol Buffers](https://protobuf.dev/overview/) for fast, compact data serialization. Its strongly typed interfaces reduce runtime errors and simplify client/server development across multiple languages. With built-in support for code generation, you can quickly scaffold clients in Typescript, Go, Rust, and more. This makes it ideal for scalable backend systems like indexers, blockchain explorers, and data-intensive decentralized apps.

In addition to request-response calls, gRPC supports server-side streaming, enabling real-time data delivery without constant polling. This is especially useful in environments where events and transactions need to be tracked live. gRPC's binary format is significantly faster and lighter than JSON, saving bandwidth and improving latency.
...
## Sui gRPC API overview

The gRPC interface is defined using Protocol Buffers. You can find the relevant beta `.proto` files at [sui-rpc-api on Github](https://github.com/MystenLabs/sui/tree/main/crates/sui-rpc-api/proto), which apart from the gRPC messages (request and response payloads) include the following services and types:
...
You will find some proto files in the folder `sui/rpc/v2alpha` as well. Those are are in alpha i.e. they are early experimental versions that are subject to change and not recommended for any serious use.

:::

`TransactionExecutionService` provides a single RPC to begin with, to execute a transaction request - `ExecuteTransaction(ExecuteTransactionRequest)`. Whereas the `LedgerService` includes the core lookup queries for Sui data. Some of the RPCs in that service are:
...
- `GetFullCheckpoint(GetCheckpointRequest)`: Fetches information about a particular checkpoint.
...
In the Sui gRPC API, FieldMasks are used in requests like `GetTransaction` to control which parts of the transaction (e.g., `effects`, `events`, `input`) are included in the response. Field paths must match the structure of the response message. This selective querying is especially useful for building efficient applications and tools.

### Encoding

In the Sui gRPC API, identifiers such as digest, object id, address etc. are represented using [Base58](https://learnmeabitcoin.com/technical/keys/base58/) encoding. This format is compact, URL-safe, and avoids visually ambiguous characters, making it ideal for blockchain identifiers. When constructing gRPC requests, clients may need to decode Base58 strings into raw bytes before sending them in fields like `digest` or `object_id`, as the gRPC protocol expects binary data. Similarly, when receiving responses, clients may need to encode byte fields back into Base58 for display or logging. This encoding convention ensures consistency across SDKs, tools, and the broader Sui ecosystem.

## Sample Sui gRPC clients
...
Your results may be different from the examples shared below, depending on the breadth and maturity of the gRPC APIs available on Sui full nodes.
...
This is an example to build a typescript client for Sui gRPC API. Feel free to use another set of tools or modules that you’re comfortable with.
...
In your `go.mod`, add the following (make sure to change the versions depending on when you may be following this example):
...
- You may need to adjust `__init__.py` files or PYTHONPATH to ensure proper module resolution.
- The example assumes that gRPC is available on port `443` which requires SSL.
- Digest in the request is directly provided in the `Base58` format, but check if you need to decode from your source format.
