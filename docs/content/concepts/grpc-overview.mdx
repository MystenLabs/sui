...
- `TransactionExecutionService`
  ...
  - Purpose: Submit and execute signed transactions on the Sui network. Wallets and apps use the service to send user actions to the network.
- `LedgerService`
...
- `LiveDataService`
  ...
  - Purpose: Query up-to-date on-chain data like balances, coin metadata, dynamic fields, or owned objects. Also supports dry-run simulations for transactions.
- `SubscriptionService`
...
- `SignatureVerificationService`
  ...
  - Purpose: Validate signatures outside transaction execution. Helps pre-verify payloads that may include [zklogin](./cryptography/zklogin.mdx) or other signatures, simulate authentication, or build custom signing workflows.
...
In the Sui gRPC API, `FieldMask`s are used in requests like `GetTransaction`, `GetObject`, and so on to control which parts are included in the response (such as, `effects`, `events` of a transaction).
...
To serve builders using the gRPC API, you must enable gRPC on your Full node by indexing gRPC-specific data. Be aware that during the initial indexing phase, your Full node may not be able to handle other traffic — including existing JSON-RPC requests. Plan your rollout carefully and communicate any downtime to your customers and partners in advance. The time needed for this index sync depends on your node’s hardware and software specifications.

Running gRPC alongside JSON-RPC will increase your Full node’s storage usage. While this is expected in the short term, you will be able to reclaim this storage later by disabling JSON-RPC once it is phased out. Builders are expected to migrate to either gRPC directly, or to [GraphQL RPC](./graphql-rpc.mdx) which would rely on gRPC under the hood in future.

By default, your Full node will retain `14` days of gRPC index data. You can configure this retention window using the following settings in your Full node config:
...
You can adjust this window based on your infrastructure and data availability requirements, but make sure to benchmark performance and scalability before extending retention and providing to your customers and partners.

To support the ecosystem-wide transition away from JSON-RPC, we strongly recommend that you enable gRPC beta support on your Full node as early as possible. This will give builders sufficient time to test and prepare for full production integration post the gRPC GA release. Refer to [Sui's data serving roadmap](/guides/developer/getting-started/data-serving.mdx#grpc-api) for more details.
