...
Party objects can be used by multiple inflight transactions at the same time. This contrasts with address-owned objects, which only allow a single inflight transaction. Many applications may benefit from the ability to pipeline multiple transactions on the same party object.

:::info

`Coin`s can be party objects, including `Coin<SUI>`. However, it is not possible to use a party object `Coin<SUI>` for gas payment. If you want to use a party object `Coin<SUI>` for gas, it must first be transferred back to address-owned.
