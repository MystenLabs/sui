...
Another common solution is to send your object to a pool, such as an NFT marketplace or a staking pool, and perform the swap in the pool, either right away, or later when there is demand. Transactions using only owned objects are faster and less expensive (in terms of gas) than using shared objects, since they do not require consensus in Sui.
