...
On Sui, an _epoch_ is a certain period of time on the network. During this time, the Sui [validator](/guides/operator/validator/validator-config.mdx) set and their stakes remain unchanged. On both [Mainnet](/concepts/sui-architecture/networks.mdx) and [Testnet](/concepts/sui-architecture/networks.mdx), an epoch is about 24 hours. This timeframe allows validators to process transactions efficiently without worrying about ad hoc validator changes.
