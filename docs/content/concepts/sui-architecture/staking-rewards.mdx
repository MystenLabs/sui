...
The Sui staking mechanism includes features that may differ from other blockchains:
...
- **Performance:** If a validator does not operate performantly, other validators may report them using the tallying rule. The validator’s staking rewards for that epoch are then slashed.  
