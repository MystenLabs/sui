...
The Sui network is operated by a [set of validators](/concepts/sui-architecture/consensus.mdx) that process [transactions](/guides/developer/transactions/txn-overview.mdx). They reach agreement on which transactions submitted and processed on the network are valid and should be finalized through [_consensus_](/concepts/sui-architecture/consensus.mdx).
