...
| Access control | Identity/role-based access control via `Ownable` and `AccessControl` contract. | Mostly [capability based access control](https://move-book.com/programmability/capability.html) via owned objects. Identity/role-based access control is possible. |
...
| Mutate contract state | Sending transactions via compile time ABI interface. | Sending transaction via runtime [programmable transaction block](/concepts/transactions/prog-txn-blocks.mdx) (PTB) construction. | 
...
| Common standards (coin, token, nft, and so on) | Currency, Token | ERC-20, ERC-721, ERC-1155 |
...
| Indexing | High level tx data + objects, coins, and so on. | High level tx data. |
...
| Contract immutability | Native mutable/immutable support using upgrade capabilities. | Not native, requires auditing the Solidity code deployed. Can be discerned by some operation codes. |
