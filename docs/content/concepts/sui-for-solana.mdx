...
The biggest difference between Sui and Solana development lies in the programming language. Sui uses [**Move**](/concepts/sui-move-concepts.mdx), while Solana relies on **Rust**, typically paired with the **Anchor** framework that simplifies dApp development through Rust macros.
...
| Data storage | Data is stored in programs, stateless acounts that contain instructions. | Data is stored in Move objects. |
...
| Asset/Token accessibility | Tokens stored in Program Derived Addresses (PDAs) or Associated Token Accounts. Access controlled through program logic and signer verification. | Anyone can access shared objects. Owned objects can only be accessed by object owner. |
| Access control | Signer-based access control. Programs verify transaction signers and account ownership. PDAs enable program-controlled accounts. | Mostly [capability based access control](https://move-book.com/programmability/capability.html) through owned objects. Identity/role-based access control is possible. |
| Contract upgrades | Programs can be marked as upgradeable or immutable at deployment. Upgrades replace the program binary but maintain the same program ID. | New contracts must be [layout-compatible](/guides/developer/packages/upgrade#upgrade-requirements) with the old one. Need to consider versioning shared objects. |
| Development environment | Anchor framework. | [Move VSCode extension](https://marketplace.visualstudio.com/items?itemName=mysten.move). |
...
| Common standards (coin, token, NFT, and so on) | SPL Token, Metaplex Token Metadata, Metaplex Certified Collections | Coin Standard, Closed-Loop Token |
...
| L1/L2 | No L2, relies on fast L1 with parallel execution | No L2, relies on fast L1 with horizontal scaling |
...
| Parallel execution | Sealevel runtime enables parallel execution when transactions don't overlap in account access. Accounts must be declared upfront for scheduling. | Transactions that can be parallel are run in paralell. |
...
| Token royalties | Not enforced on-chain, relies on marketplace cooperation and Metaplex standard implementation | Enforced by the chain through Kiosk and transfer policies |
