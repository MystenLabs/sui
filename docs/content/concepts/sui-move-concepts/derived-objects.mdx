...
Most objects on Sui get an ID assigned to them upon object creation that is guaranteed to be unique to every other object on the network. Derived objects, on the other hand, have an ID that is claimed from the mapping of a parent object and a key. This `parent-key` mapping ensures that the claimed ID is a singular namespace because the objects derived from the parent object (unique ID that exists on-chain) are paired with individual keys. It's this mapping that makes the resulting ID of derived objects deterministic.

When you claim an ID from the `derived_object` module, you pass the parent object and key. The parent object already exists on-chain, either at package publish time or through a transaction with the smart contract. The parent object can be [shared, owned, party, or even a wrapped UID](../object-ownership.mdx).  
