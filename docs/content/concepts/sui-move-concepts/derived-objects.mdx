...
Most Sui objects get a unique ID assigned to them upon object creation. However, a **derived object** does not technically have an assigned ID; instead it has a _claimed_ ID created by mapping a parent object to a key. The parent object's unique ID that exists on-chain is mapped to an individual key, ensuring that the derived object's claimed ID is both deterministic and unique.

Derived objects enable you to deterministically compute object IDs from a parent object ID and a key, both on- and off-chain. This means you can compute the ID of a derived object before you ever actually create it on the network. 
...
If you know the parent ID, you can compute the ID of the derived object through off-chain logic using the [TypeScript](https://github.com/MystenLabs/ts-sdks/blob/642ff2784ffb89b1bce4a43a6ceae91c04486669/packages/typescript/src/utils/derived-objects.ts) or [Rust SDK](https://mystenlabs.github.io/sui/sui_types/derived_object/index.html) helper functions. This functionality means your client logic can treat unclaimed IDs as if they already exist. 
...
Because the ID of a derived object can exist before the object does, derived objects can also receive transfers before they exist. Send assets to a derived address, then create the object later to claim them. This enables patterns like:
