...
When you publish or upgrade your package on multiple chains (Mainnet, Testnet, Devnet), automated address management tracks separate addresses for each chain. Tracking is based on your active environment (run `sui client active-env` if unsure). For example, if your active environment is set to an RPC connected to `testnet` and you publish a package, the `Move.lock` records the address for that package and associates it with `testnet`.  

Automated address management works for one package published to one or more chains. If a package is republished to a chain, tracked addresses for that chain are overwritten.
