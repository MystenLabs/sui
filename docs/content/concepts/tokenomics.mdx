...
The tokenomics subject includes more than just the rules that govern SUI. Subtopics in this section also examine tokenomics on the Sui network that might define your own token projects on the Sui blockchain.

## Related links

- [The Sui Smart Contracts Platform: Economics and Incentives](/paper/tokenomics.pdf): Whitepaper that details Sui tokenomics.
- [Gas in Sui](./tokenomics/gas-in-sui.mdx): Gas fees are charged on all network operations and used to reward participants of the proof-of-stake mechanism and prevent spam and denial-of-service attacks.
- [Token Vesting Strategies](./tokenomics/vesting-strategies.mdx): Vesting strategies to consider when dealing with your own token on the Sui network.
- [Token Launches on Sui](./tokenomics/token-launches.mdx): Best practices to follow when launching a token on Sui.
