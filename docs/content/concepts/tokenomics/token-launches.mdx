...
Following best practices when launching tokens on Sui can help  best practices for launching new tokens on Sui. The Multisig Authentication guide includes an example of using a [multisig account to support the launch of a token on Sui](../../guides/developer/cryptography/multisig.mdx#multisig-token-launch).
