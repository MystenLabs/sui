...
Account-based currencies define only the first three levels, while UTXO-based currencies add `change` and `address` level definitions. Because Sui's object-oriented data model is neither UTXO nor account-based (it in fact combines both), it employs all five levels for maximum compatibility.
...
Sui's [collection of validators](/guides/operator/validator-index.mdx) holds three distinctive key pairs for different purposes.
