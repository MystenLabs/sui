...
1. Clients can collect a supermajority of validator responses, assemble them into an effects certificate, and use it as proof of the transaction's settlement.
1. Subsequently, Sui forms [checkpoints] for every consensus commit, which it uses to drive the [reconfiguration protocol].
