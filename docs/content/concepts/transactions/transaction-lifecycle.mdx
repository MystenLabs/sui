...
If a app stores the signed transaction locally before sending it to the full node, which is best practice, when the app restarts it tries to verify if the transaction has been finalized first. If it has, then no other steps are necessary. If it has not, then the app needs to resubmit the transaction.
