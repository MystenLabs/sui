...
A [`TransferPolicy`](https://github.com/MystenLabs/sui/blob/65895261feba7a1241cc5342d44cf72386c5ebe0/crates/sui-framework/docs/sui/transfer_policy.md) on Sui is a customizable primitive that enables the owner of a type to create custom rules that define how the type can be transferred. A `TransferPolicy` can be used within a Sui Kiosk marketplace or any other system that integrates the `TransferPolicy` primitive. The creator can set any number of rules, such as paying a royalty or commission, and all must be satisfied in a single transaction for the transfer to succeed.
...
This system gives creators maximum freedom and flexibility. By removing transfer policy logic from the trading primitive, only creators can set policies, and they control all enforcement as long as the primitive is used.
...
The rule logic is simple: someone can publish a new rule module, such as a "fixed fee," and add it to the `TransferPolicy`. Once the rule is added, `TransferRequest` needs to collect a `TransferReceipt` marking that the requirement specified in the rule was completed.

### Example rule 

For example, to implement VAT fees on every merchant transaction, you must introduce a rule.
...
1. A set function which adds the rule to the `TransferPolicy`. This must be performed by the `TransferPolicyCap` holder.
...
The `TransferPolicy` module allows removing any rule at any time, as guaranteed by the constraints on the rule's configuration. The example module above has no configuration and accepts a `Coin<SUI>` of any value.
...
- Item ID
- Amount paid (SUI)
- From ID: The object which was used for selling, such as the kiosk
...
Rules apply to more than just payments and fees. Some rules may permit trading only before or after a specific time. Because rules are not standardized, developers can encode logic with any object.
...
By adding type parameters to a rule, it is possible to create a generic rule which will not only vary by configuration but also by the type of the rule.
...
The `witness_rule` is generic and can be used to require a custom witness depending on the settings. It is a simple way to link custom marketplace or trading logic to the `TransferPolicy`. With a slight modification, the rule can be turned into a generic capability requirement for any object, even a `TransferPolicy` for a different type or a `TransferRequest`. 
