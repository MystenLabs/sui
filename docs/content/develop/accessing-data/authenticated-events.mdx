...
The reference client implementation lives in [`MystenLabs/sui` → `crates/sui-light-client`](https://github.com/MystenLabs/sui/tree/main/crates/sui-light-client) (module `sui_light_client::authenticated_events`). The APIs and client implementation are **experimental**: once they stabilize, the client will move to [`sui-rust-sdk`](https://github.com/MystenLabs/sui-rust-sdk) and early adopters will be expected to migrate at that time.
...
This enables cryptographic verification of event completeness and correctness without trusting the RPC provider, and is lightweight enough to run in a browser or on mobile.
...
Add the client as a git dependency in `Cargo.toml`:

```toml
[dependencies]
sui-light-client = { git = "https://github.com/MystenLabs/sui", branch = "main" }
sui-types = { git = "https://github.com/MystenLabs/sui", branch = "main" }
futures = "0.3"
tokio = { version = "1", features = ["full"] }
```

Connect to testnet using the bundled trust anchor and the gRPC archive:

```rust
use sui_light_client::authenticated_events::{
    AuthenticatedEventsClient, ClientConfig, bundled,
};
...
const TESTNET_ARCHIVE_URL: &str = "https://archive.testnet.sui.io:443";

// `bundled::testnet_start_committee()` is a trust anchor shipped with the
// crate (committee for the first testnet epoch where authenticated events
// were enabled). The client trust-ratchets forward from there.
let client = Arc::new(
    AuthenticatedEventsClient::new_with_config(
        TESTNET_RPC_URL,
        Some(TESTNET_ARCHIVE_URL),
        bundled::testnet_start_committee(),
...
let config = ClientConfig::default()
    .with_page_size(page_size)                       // Events per RPC call (max 1000)
    .with_poll_interval(poll_interval)               // How often to poll for new events
    .with_max_pagination_iterations(max_iters)       // Max pages before forcing checkpoint boundary
    .with_rpc_timeout(rpc_timeout);                  // RPC call timeout

let client = AuthenticatedEventsClient::new_with_config(
    rpc_url,
    Some(archive_url),  // or None; e.g. "https://archive.testnet.sui.io:443"
    starting_committee,
    config,
).await?;
```
