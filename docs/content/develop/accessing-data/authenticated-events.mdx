...
goal:
  description: >-
    Reader understands cryptographically verifiable Move events that can be
    verified by a light client without trusting any intermediary, suitable for
    trustless event consumption
  requires:
    - has_frontmatter:
        - title
        - description
    ...
    - headings:
        - pattern: Why authenticated events\?
        - pattern: Quick start
