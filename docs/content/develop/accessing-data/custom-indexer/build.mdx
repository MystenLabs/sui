...
description: >-
  Build a custom indexer using the `sui-indexer-alt-framework` module. The
  example indexer demonstrates a sequential pipeline that extracts transaction
  digests from Sui checkpoints and stores them in a local PostgreSQL database,
...
goal:
  description: >-
    Reader can build a custom indexer using the `sui-indexer-alt-framework`
    module.
  requires:
    - has_frontmatter:
        - title
...
builder_paths:
  ...
  - path_id: walrus-agentic
    ...
    stage: Ship
questions:
  - How do I build a custom indexer?
  - What is Build a Custom Indexer in Sui?
answer: Build a custom indexer using the `sui-indexer-alt-framework` module.
---

Build a custom indexer using the `sui-indexer-alt-framework` module.
