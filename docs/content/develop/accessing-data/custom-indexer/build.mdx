...
goal:
  description: >-
    ...
    module. The example indexer demonstrates a sequential pipeline that extracts
    transaction digests from Sui checkpoints and stores them in a local
    PostgreSQL database, the default example storage backend
  requires:
    - has_frontmatter:
        - title
        - description
