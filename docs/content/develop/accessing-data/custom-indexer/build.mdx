...
- Connects to a Sui checkpoint source: Uses a GCS checkpoint bucket as the production-recommended source for full checkpoint history, or the public HTTPS endpoint for simpler access to recent checkpoints only (most recent 30 days).
...
Sui provides checkpoint sources for both Mainnet and Testnet.

For production use and full-retention backfill, use the network-specific GCS bucket with Requester Pays enabled:

- Testnet: `gs://mysten-testnet-checkpoints-use4`
- Mainnet: `gs://mysten-mainnet-checkpoints-use4`

For simpler access to recent data only (most recent 30 days), use the public HTTPS endpoints:
...
**Recommended: GCS checkpoint bucket (full history):**

```sh
$ cargo run -- --remote-store-gcs mysten-testnet-checkpoints-use4
```

**Recommended with gRPC streaming from a full node (with GCS fallback):**

```sh
$ cargo run -- --remote-store-gcs mysten-testnet-checkpoints-use4 --streaming-url https://fullnode.testnet.sui.io:443
```

**Using the public HTTPS endpoint (recent checkpoints only, most recent 30 days):**

```sh
$ cargo run -- --remote-store-url https://checkpoints.testnet.sui.io
```

The GCS checkpoint buckets (`gs://mysten-testnet-checkpoints-use4` for Testnet, `gs://mysten-mainnet-checkpoints-use4` for Mainnet) provide full checkpoint history and are the recommended source for production indexers and backfills. These buckets are Requester Pays enabled. See [Use Requester Pays](https://cloud.google.com/storage/docs/using-requester-pays) and [Running a Remote Store](/operators/data-management/remote-store-setup) for more information.

The public HTTPS checkpoint endpoints (`https://checkpoints.testnet.sui.io` and `https://checkpoints.mainnet.sui.io`) retain only the most recent 30 days of checkpoints and are suitable for simpler setups that only need recent data.
