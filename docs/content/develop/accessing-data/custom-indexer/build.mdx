...
**Recommended: GCS bucket for full-retention backfill (production):**

```sh
$ cargo run -- --remote-store-gcs mysten-testnet-checkpoints-use4
```

**Recommended with gRPC streaming: GCS bucket fallback with full node streaming:**

```sh
$ cargo run -- --remote-store-gcs mysten-testnet-checkpoints-use4 --streaming-url https://fullnode.testnet.sui.io:443
```

**Simpler option for recent data only (development or last 30 days):**

```sh
$ cargo run -- --remote-store-url https://checkpoints.testnet.sui.io
```

**HTTPS store with gRPC streaming (recent data only):**
