...
description: >-
  The `sui-indexer-alt-framework` is a powerful Rust framework for building
  high-performance, custom blockchain indexers on Sui. It provides customizable,
  production-ready components for data ingestion, processing, and storage.
keywords:
  - indexer
  - custom indexer
  - data processing
...
goal:
  description: >-
    Reader understands what custom indexers are, when to use them, and how the
    sui-indexer-alt-framework ingestion, processing, and storage layers fit
    together
  requires:
    - has_frontmatter:
        - title
...
builder_paths:
  ...
  - path_id: walrus-general
    ...
    step: Custom indexer framework
    stage: Ship
questions:
  - What Are Custom Indexers?
  - What is Custom Indexers in Sui?
  - How does custom indexers work on Sui?
  - How do I build custom indexers?
answer: >-
  The `sui-indexer-alt-framework` is a powerful Rust framework for building
  high-performance, custom blockchain indexers on Sui.
---
...
- **Remote stores** can connect to public checkpoint repositories such as [`https://checkpoints.mainnet.sui.io`](https://checkpoints.mainnet.sui.io/) and [`https://checkpoints.testnet.sui.io`](https://checkpoints.testnet.sui.io/), but these HTTPS endpoints retain only the most recent 30 days of checkpoints and should be used only as fallback sources. For full-retention backfills, use `gs://mysten-mainnet-checkpoints-use4` or `gs://mysten-testnet-checkpoints-use4` with `--remote-store-gcs` and Requester Pays enabled.
...
To build your own pipelines for application-specific data, see [Building Your First Custom Indexer](/develop/accessing-data/custom-indexer/build). You can also reach out to a [data indexer operator](/references/awesome-sui) that might already have set one up for your needs.
