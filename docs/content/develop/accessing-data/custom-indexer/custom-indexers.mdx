...
- **Remote stores** can connect to public checkpoint repositories such as [`https://checkpoints.mainnet.sui.io`](https://checkpoints.mainnet.sui.io/), but these HTTPS endpoints retain only the most recent 30 days of checkpoints and should be used only as fallback sources. For Mainnet backfills older than 30 days, use `gs://mysten-mainnet-checkpoints-use4` with `--remote-store-gcs`.
