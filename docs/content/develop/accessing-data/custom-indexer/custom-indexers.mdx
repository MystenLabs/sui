...
goal:
  description: >-
    Reader understands the `sui-indexer-alt-framework` is a powerful Rust
    framework for building high-performance, custom blockchain indexers on Sui
  requires:
    - has_frontmatter:
        - title
        - description
    ...
    - headings:
        - pattern: When to use
        - pattern: How custom indexers fit into the applica
        - pattern: Building custom indexers
...
To build your own pipelines for application-specific data, see [Building Your First Custom Indexer](/develop/accessing-data/custom-indexer/build). You can also reach out to a [data indexer operator](/references/awesome-sui#indexers--data-services) that might already have set one up for your needs.
