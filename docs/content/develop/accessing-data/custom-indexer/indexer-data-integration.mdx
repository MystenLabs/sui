...
**Custom HTTP headers:** You can pass arbitrary HTTP headers to remote store requests using the `--remote-store-http-header` flag. This is useful when accessing authenticated or private remote stores that require API keys, authorization tokens, or other custom headers. You can specify the flag multiple times to include more than one header:

```sh
$ cargo run -- --remote-store-url https://my-private-store.example.com \
    --remote-store-http-header "Authorization: Bearer <token>" \
    --remote-store-http-header "X-Custom-Header: value"
```
