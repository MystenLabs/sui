...
description: >-
  The `sui-indexer-alt-framework` provides two distinct pipeline architectures.
  Understand the differences between the sequential and concurrent pipelines
  that the `sui-indexer-alt-framework` provides to decide which best suits your
  project needs.
keywords:
  - indexer
  - index data
...
goal:
  description: >-
    Reader understands the `sui-indexer-alt-framework` provides two distinct
...
questions:
  - What is Indexer Pipeline Architecture in Sui?
  - When to use each pipeline?
  - How does indexer pipeline architecture work on Sui?
answer: >-
  The `sui-indexer-alt-framework` provides two distinct pipeline architectures.
...
let config = SequentialConfig {
    ...
    // fanout: Some(ConcurrencyConfig::fixed(20)),
...
let config = ConcurrentConfig {
    committer: CommitterConfig {
        ...
        write_concurrency: ConcurrencyConfig::fixed(10),
