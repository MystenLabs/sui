...
- GraphQL returns structured, typed data. For example, `effects.checkpoint.sequenceNumber` gives you the checkpoint directly without needing a separate lookup.
...
- Select fields from `executionResult` in the same mutation to read execution results immediately without waiting for a separate indexed query.
...
See [issue #23832](https://github.com/MystenLabs/sui/issues/23832) for the recommended computation and the legacy implementation as a reference.
