...
description: >-
  Learn the core concepts for working with GraphQL on Sui RPC, including request
  headers, query composition with variables and fragments, pagination
  strategies, query scope, service limits, and the components of the GraphQL and
  General-purpose Indexer stack.
keywords:
  - graphql
  - graphql headers
  - x-sui-rpc-version
  ...
  - sui rpc calls
goal:
  description: Reader can use GraphQL to make Sui RPC calls
  requires:
    - has_frontmatter:
        - title
        - description
...
GraphQL RPC Server and the General-purpose Indexer are generally available. JSON-RPC is deprecated. Sui Foundation disabled it on Mainnet full nodes the week of July 27, 2026, and plans full decommission for mid-October 2026. See the [JSON-RPC Migration Guide](/develop/accessing-data/json-rpc-migration) for the complete timeline.
...
**Tables** : `tx_digests`, `tx_kinds`, `tx_calls`, `tx_affected_addresses`, `tx_affected_objects`
