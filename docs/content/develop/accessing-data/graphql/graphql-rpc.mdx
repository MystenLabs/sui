...
GraphQL RPC Server and the General-purpose Indexer are generally available. JSON-RPC is deprecated. JSON-RPC is disabled on Sui Foundation mainnet full nodes the week of July 27, 2026, with full decommission in mid-October 2026. See the [JSON-RPC Migration Guide](/develop/accessing-data/json-rpc-migration) for the complete timeline.
