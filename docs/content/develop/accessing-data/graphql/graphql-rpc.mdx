...
GraphQL provides a flexible, generally available way to query the Sui network, submit transactions, and simulate transactions. The GraphQL schema is versioned and generally available; do not treat it as a beta API or expect routine breaking schema changes. This page covers the core concepts for working with GraphQL on Sui RPC, including request headers, query composition with variables and fragments, pagination strategies, query scope, and service limits.
...
The GraphQL RPC Service reads data from the General-purpose Indexer's Postgres-compatible database, Consistent Store, Archival Store and Service, and a full node. GraphQL RPC is an alternative to the gRPC API and provides a broad query surface for reads, transaction submission, and transaction simulation. The General-purpose Indexer is a scalable implementation of the [custom indexing framework](/develop/accessing-data/custom-indexer/custom-indexers). The framework ingests data using the remote checkpoint store and full node RPCs. It lets you configure it to load different types of Sui network data into Postgres-compatible tables in parallel, improving data ingestion performance. You can also configure pruning for different tables to balance performance and cost.
...
<details>
	<summary>Full list of tables and their schemas</summary>
...
<details>
	<summary>`lib.rs` in `sui-kvstore`</summary>
	<ImportContent source="crates/sui-kvstore/src/lib.rs" mode="code" />
</details>

<details>
	<summary>`main.rs` in `sui-kv-rpc`</summary>
