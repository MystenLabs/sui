...
This guide provides practical examples for querying the Sui network using GraphQL. For core concepts like headers, variables, fragments, pagination, scope, and limits, see the [corresponding concepts page](/develop/accessing-data/graphql/graphql-rpc).
...
Look for changes where `outputState.asMoveObject.contents.type.repr` is `0x2::package::Publisher`. The object change `address` is the Publisher object ID.
