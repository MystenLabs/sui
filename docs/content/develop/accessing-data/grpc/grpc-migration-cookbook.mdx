...
- [x] Install the `@mysten/sui` TypeScript SDK by running `npm install @mysten/sui`.
...
Subscriptions begin at the current tip of the chain. The first frame is a progress-only frame (no transaction payload) that establishes the stream's start position. To avoid gaps between historical data and the live stream, see [Backfill and subscribe without gaps](#backfill-and-subscribe-without-gaps).
...
For sponsored transactions, the sponsor sets `setGasPayment` and `setGasOwner` on the transaction, builds the kind-only bytes with `tx.build({ client, onlyTransactionKind: true })`, and provides an additional signature. See [Sponsored Transactions](/develop/transaction-payment/sponsor-txn) for the full pattern.
...
To combine historical backfill with a live subscription without missing data, open the subscription first, note its starting watermark, then backfill up to that point. The general sequence is:

1. Open the subscription. It starts at the current tip and sends a progress-only first frame with no payload.
2. Read the first frame to get the starting watermark (a checkpoint number or opaque cursor depending on the API).
3. Backfill historical data from your last processed checkpoint up to the subscription's starting watermark using the corresponding `List*` API.
4. After backfill completes, consume from the already-open subscription.

This pattern works with all three subscription and list API pairs: `SubscribeCheckpoints` with `ListCheckpoints`, `SubscribeTransactions` with `ListTransactions`, and `SubscribeEvents` with `ListEvents`. The subscription and list APIs accept the same filter types, so you can reuse the same filter for both.

The following example demonstrates this pattern with `SubscribeCheckpoints` (which the TypeScript SDK supports today) and `GetCheckpoint` for backfill:

```ts
// 1. Open the subscription (starts at the current tip)
const { responses } = client.subscriptionService.subscribeCheckpoints({
...
// 2. Read the first frame to get the starting checkpoint cursor
const firstFrame = await iterator.next();
const startCursor = firstFrame.value?.cursor ?? 0n;
console.log('Subscription starts at checkpoint:', startCursor);

// 3. Backfill from your last processed checkpoint up to the start cursor
for (let seq = YOUR_LAST_PROCESSED_CHECKPOINT + 1n; seq < startCursor; seq++) {
  const { response } = await client.ledgerService.getCheckpoint({
...
// 4. Consume from the already-open subscription
let frame = firstFrame;
while (!frame.done) {
  if (frame.value?.checkpoint) {
    processCheckpoint(frame.value.checkpoint);
  }
  frame = await iterator.next();
}
```

For `SubscribeTransactions` and `SubscribeEvents`, apply the same pattern using the proto client (`client.subscriptionService.subscribeTransactions()` or `client.subscriptionService.subscribeEvents()`). The subscription response carries a `watermark` with an opaque `cursor` and a `checkpoint` field. Pass the `cursor` to the corresponding `List*` API's `options.before` to set the backfill upper bound.
...
**Server-side timeout:** The server applies a configurable timeout (default: 60 seconds) as a deadline for unary requests. For streaming `List*` APIs, this timeout bounds the time to the first response. If a sparse filter scans a large range before finding its first match, the server may terminate the stream. Retry with a narrower checkpoint range or a less selective filter.

**Public endpoint limitations:** The public URLs at `https://fullnode.<network>.sui.io` are behind load balancers that impose their own connection timeouts and rate limits beyond what the Sui node configures. Long-running streams are more likely to be interrupted on public endpoints. For production streaming workloads, run your own [Sui full node](/operators/full-node/sui-full-node) or use a provider with a dedicated gRPC endpoint.
