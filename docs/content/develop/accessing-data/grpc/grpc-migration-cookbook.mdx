...
// Subscribe to events with a server-side filter
const { responses } = client.subscriptionService.subscribeEvents({
  filter: {
    terms: [{
      literals: [{
        predicate: {
...
- Subscriptions always begin at the current tip of the chain. They do not support resumption from a prior point. To backfill historical events before subscribing, use `LedgerService.ListEvents` with the same filter. See [Subscriptions for streaming data](/develop/accessing-data/grpc/what-is-grpc#subscriptions-for-streaming-data).
...
<TabItem value="grpc-sdk" label="gRPC (TypeScript SDK)">
```ts
// List events of a specific type within a checkpoint range
...
- Subscriptions do not support resumption. To backfill missed checkpoints, use `ListCheckpoints` with the appropriate checkpoint range before resubscribing.
