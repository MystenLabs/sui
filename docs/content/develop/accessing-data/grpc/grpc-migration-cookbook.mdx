...
To combine historical backfill with a live subscription without missing data, open the subscription first, start consuming it concurrently, then backfill up to the subscription's starting point. The general sequence is:
...
3. Start consuming the subscription concurrently into a buffer or queue so the server does not evict your client as a slow consumer (the server drops subscribers whose 256-item buffer fills).
4. Backfill historical data from your last processed checkpoint up to the subscription's starting watermark using the corresponding `List*` API (not per-item `Get*` calls, which are too slow for large gaps).
5. After backfill completes, drain the buffer and continue consuming from the live subscription.
...
The subscription's per-subscriber buffer holds 256 items. If backfill takes long enough for the live stream to queue 256 frames, the server evicts your subscriber. Always consume the subscription concurrently during backfill, or use a short enough backfill range that the buffer cannot overflow.
...
async function startStream() {
  ...
  // Reconnect on both errors and normal termination (slow-consumer
  // eviction and source-lag drops end the stream without an error).
  console.log('Stream ended, reconnecting from checkpoint', lastProcessed);
  startStream();
}
```
...
**Server-side timeout:** The server applies a configurable timeout (default: 60 seconds) as a deadline for unary requests. For streaming `List*` APIs, this timeout covers the entire stream duration, not just time to first response. A large query can terminate after returning partial results. Always resume from the last `watermark.cursor` rather than retrying from scratch.
