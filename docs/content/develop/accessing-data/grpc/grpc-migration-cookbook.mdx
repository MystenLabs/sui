...
The `@mysten/sui` TypeScript SDK (v2.23.1+) exposes `client.listTransactions()` as a high-level wrapper. You can also use the proto client directly through `client.ledgerService.listTransactions()`. The examples below show the TypeScript proto client and raw protobuf JSON request shapes.
...
```ts
// Use the proto client for full control over the streaming response
...
for await (const response of stream.responses) {
  if (response.transaction) {
    console.log('Digest:', response.transaction.digest);
    console.log('Effects:', response.transaction.effects);
  }
  // Track the watermark for resume
  if (response.watermark) {
    console.log('Scanned to checkpoint:', response.watermark.checkpoint);
  }
  // Check why the stream ended
  if (response.queryEnd) {
    console.log('Ended:', response.queryEnd.reason);
    // Save response.watermark.cursor to resume from this point
  }
}
```
...
The `@mysten/sui` TypeScript SDK (v2.23.1+) exposes `SubscribeTransactions` through the proto client at `client.subscriptionService.subscribeTransactions()`. The examples below show the TypeScript proto client and raw protobuf JSON request shapes.
...
for await (const response of stream.responses) {
  if (response.transaction) {
    console.log('Transaction:', response.transaction.digest);
    console.log('Status:', response.transaction.effects?.status);
  }
}
```
