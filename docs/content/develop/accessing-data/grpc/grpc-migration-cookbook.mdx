...
Implement `backfillCheckpoints` with `ListCheckpoints`, treating its end checkpoint as exclusive and resuming partial responses from the latest watermark cursor. If the durable spool reaches its configured capacity, stop both workers and restart from `lastProcessed` rather than dropping live frames.
