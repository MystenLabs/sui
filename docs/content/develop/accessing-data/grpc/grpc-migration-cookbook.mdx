...
async function commit(checkpoint) {
  // Make this operation idempotent and atomically persist the cursor with
  // the application updates.
  await processAndPersistCheckpoint(checkpoint);
  lastProcessed = checkpoint.sequenceNumber;
}

async function backfillBefore(endCheckpoint: bigint) {
  if (lastProcessed + 1n >= endCheckpoint) return;

  const { responses } = client.ledgerService.listCheckpoints({
    startCheckpoint: lastProcessed + 1n,
    endCheckpoint, // Exclusive.
    readMask: { paths: ['sequenceNumber', 'transactions'] },
  });

  for await (const response of responses) {
    if (response.checkpoint) await commit(response.checkpoint);
  }
}
...
for (let attempt = 0; ; attempt++) {
  try {
    ...
    for await (const response of responses) {
      ...
      const seq = checkpoint.sequenceNumber;
      if (seq > lastProcessed + 1n) await backfillBefore(seq);
      if (seq > lastProcessed) await commit(checkpoint); // Skip replayed data.
      attempt = 0;
    }

    throw new Error('checkpoint subscription ended');
  } catch (err) {
    console.error('Stream ended; reconnecting from checkpoint', lastProcessed, err);
...
**Server-side timeout:** The server applies a configurable timeout (default: 60 seconds) to the full execution of unary requests. For streaming `List*` APIs, it applies only to request setup and time to first response; after the first frame, it does not limit the stream's lifetime. A later network, proxy, or GOAWAY interruption can still leave a query partially consumed. Resume from the last `watermark.cursor` while preserving the original upper bound instead of restarting from scratch.
