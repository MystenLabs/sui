...
The `@mysten/sui` TypeScript SDK (v2.23.1+) exposes `client.listEvents()` for common paginated event queries. For checkpoint-bounded `ListEvents` queries, use the proto client directly through `client.ledgerService.listEvents()`. The `grpcurl` example in the next tab shows the raw protobuf JSON request shape.
