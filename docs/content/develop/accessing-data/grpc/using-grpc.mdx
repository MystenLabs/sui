...
The following tables list commonly used `read_mask` field paths for each service method. Field paths follow the proto message structure and support dot notation for nested fields. Pass `*` to request all fields, or omit `read_mask` entirely (defaults to `*`). For the complete field definitions, see the [proto source](https://github.com/MystenLabs/sui-apis/tree/main/proto).
...
| `type` | The Move type of the object |
| `owner` | Ownership information (address-owned, shared, immutable, or wrapped) |
| `content` | The object's BCS-encoded or JSON content |
...
| `object_changes` | Object mutations (created, mutated, deleted, wrapped, unwrapped) |
...
| `start_timestamp` | Epoch start timestamp |
| `end_timestamp` | Epoch end timestamp (set after epoch ends) |
...
Rate limits depend on the endpoint operator. The public Sui Foundation endpoints at `fullnode.<network>.sui.io` enforce the following default limits:

| Limit type | Default value |
| --- | --- |
| Requests per second (unary) | 100 RPS per IP |
| Concurrent streams | 10 per IP |
| Max message size (request) | 4 MB |
| Max message size (response) | 50 MB |
| Connection max age | 4 hours (server sends GOAWAY) |
| Subscription buffer | 256 items per subscriber |

Private providers and self-hosted full nodes can configure different limits. Contact your provider for their specific rate limits, or check the full node's `grpc` configuration section if you operate your own node.
