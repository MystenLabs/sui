...
The following tables list commonly used `read_mask` field paths for each service method. Field paths follow the proto message structure and support dot notation for nested fields. Pass `*` to request all fields. When `read_mask` is omitted, the default varies by method (for example, `GetObject` defaults to `object_id,version,digest`). Check the [proto source](https://github.com/MystenLabs/sui-apis/tree/main/proto) for method-specific defaults.
