...
To combine historical and real-time data without gaps, open the subscription first, note the first `Watermark` in the stream, then backfill with the corresponding list API (`ListCheckpoints`, `ListTransactions`, or `ListEvents`) up to that watermark. The list APIs accept `start_checkpoint` and `end_checkpoint` bounds and return results as a server-side stream with cursor-based pagination through `QueryOptions`. Once the backfill is complete, begin consuming from the already-open subscription.
