...
### Negative filters (MatchNone) have no direct equivalent

JSON-RPC object queries supported `MatchAll`, `MatchAny`, and `MatchNone` filter combinators to include or exclude results by type. Neither gRPC nor GraphQL supports negation filters.

- **gRPC** uses a Disjunctive Normal Form (DNF) filter structure: `terms` (OR) containing `literals` (AND). You can express "match A or B" and "match A and B", but not "match everything except C". Filter client-side for exclusion logic, or restructure the query to list only the types you want.
- **GraphQL** uses flat filter fields (`type`, `sender`, `emittingModule`) without boolean combinators. Use multiple queries or client-side filtering for exclusion patterns.
...
- **dApp Kit** (`@mysten/dapp-kit-react`, `@mysten/dapp-kit-core`): Replace `SuiClient` with `SuiGrpcClient` in the `createClient` factory. All hooks and transaction methods work identically with either client.
...
- **Kiosk SDK** (`@mysten/kiosk`): No changes needed. Kiosk operations are PTB move calls that you build with the `Transaction` class and execute through your app's Sui client. Swap the app's client to `SuiGrpcClient` and all Kiosk code works unchanged.
