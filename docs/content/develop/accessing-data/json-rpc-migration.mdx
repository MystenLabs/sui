...
Follow these steps to migrate from JSON-RPC to gRPC and/or GraphQL:

1. **Audit your JSON-RPC usage.** List every JSON-RPC method your application calls. Use the [method mapping](#method-mapping) table to identify the gRPC or GraphQL replacement for each call.

2. **Choose your replacement API.** Use the [decision criteria](#choose-your-replacement) to decide whether each call migrates to gRPC, GraphQL, or both. Most applications use gRPC for the live data path and transaction submission, and GraphQL for ad hoc queries and joined reads.

3. **Set up a gRPC client.** Install the `@mysten/sui` TypeScript SDK or generate a client from the [proto definitions](https://github.com/MystenLabs/sui-apis/tree/main/proto). See [Querying Data with gRPC](/develop/accessing-data/grpc/using-grpc) for setup instructions in TypeScript, Go, and Python.

4. **Migrate call by call.** Use the [gRPC Migration Cookbook](/develop/accessing-data/grpc/grpc-migration-cookbook) for side-by-side code samples. Start with read-only calls (object lookups, balance queries) before migrating transaction execution.

5. **Replace WebSocket subscriptions.** Migrate any `sui_subscribeEvent` or `sui_subscribeTransaction` WebSocket usage to `SubscriptionService` gRPC streaming. See [Subscriptions for streaming data](/develop/accessing-data/grpc/what-is-grpc#subscriptions-for-streaming-data).

6. **Test against Testnet.** Verify each migrated call against a Testnet gRPC endpoint before switching production traffic.

7. **Update your production endpoint.** Point your application at a production gRPC endpoint. Do not use the public `fullnode.*.sui.io` URLs for production workloads.

### Related pages

| Page | Purpose |
| --- | --- |
| [What is gRPC?](/develop/accessing-data/grpc/what-is-grpc) | gRPC concepts, service table, and streaming patterns |
| [Querying Data with gRPC](/develop/accessing-data/grpc/using-grpc) | Client setup, field masks, pagination, and language-specific examples |
| [gRPC Migration Cookbook](/develop/accessing-data/grpc/grpc-migration-cookbook) | Side-by-side JSON-RPC to gRPC code samples for every common call |
| [gRPC Method Reference](/references/fullnode-protocol) | Auto-generated reference for all gRPC methods |
| [gRPC Message Definitions](/references/fullnode-protocol-messages) | Proto message field definitions |
| [gRPC Enum and Scalar Types](/references/fullnode-protocol-types) | Enum values and scalar type mappings |
...
| `sui_getCheckpoints` | `LedgerService.ListCheckpoints` for paginated, filtered queries over a checkpoint range. Use `SubscriptionService.SubscribeCheckpoints` for an ordered live stream, or `LedgerService.GetCheckpoint` for single-checkpoint lookups. | `Query.checkpoints` |
...
| `suix_queryTransactionBlocks` | `LedgerService.ListTransactions` with a `TransactionFilter` for paginated, filtered queries over a checkpoint range. Use `SubscriptionService.SubscribeTransactions` with the same filter for a live stream of matching transactions. | `Query.transactions` with a `TransactionFilter` |
...
Not all tools in the Sui ecosystem require manual migration:

- **Sui CLI**: No changes needed. The CLI already uses gRPC internally. The `rpc` field in `~/.sui/sui_config/client.yaml` points at the same `fullnode.*.sui.io:443` URLs, which serve both JSON-RPC and gRPC.

- **dApp Kit**: The currently published `@mysten/dapp-kit` client factory requires `SuiJsonRpcClient` and does not accept `SuiGrpcClient`. Continue using its JSON-RPC client until a dApp Kit release explicitly adds transport-agnostic client support; do not substitute `SuiGrpcClient` in `createClient`.

- **Kiosk SDK** (`@mysten/kiosk`): Continue using a supported JSON-RPC or GraphQL client for Kiosk object and event queries. The currently published Kiosk SDK does not accept `SuiGrpcClient`; migrate only after a Kiosk release explicitly adds gRPC support.

- **Walrus site-builder**: The site-builder manages its own Sui connection through `sites-config.yaml`. Check the [Walrus documentation](https://docs.wal.app/) for gRPC migration status, as the transport is controlled by the Walrus team.
