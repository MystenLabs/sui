...
### Combining event filters

To combine filter conditions (for example, a specific module and time range), use a GraphQL query with multiple filter fields or nest conditions in an `All` filter when using the Rust SDK:

```graphql
{
  events(
    filter: {
      eventType: "0xPACKAGE::swap::SwapEvent"
    }
    after: null
    first: 50
  ) {
...
A single transaction can emit a maximum of 1,000 events. If your Move code exceeds this limit, the transaction aborts.
...
For archival data beyond the recent window, see [Accessing archival data](/develop/accessing-data/archival-data).
