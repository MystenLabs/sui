...
:::info
The `@mysten/sui` TypeScript SDK (v2.23.1+) exposes `client.listEvents()` as a high-level wrapper and `client.subscriptionService.subscribeEvents()` through the proto client. The `GetTransaction` example below uses the SDK's `client.core.getTransaction()` method.
:::
