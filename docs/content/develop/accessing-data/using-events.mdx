...
goal:
  description: >-
    Reader can use events to notify onchain assets of activity your smart
    contracts initiate and query events from other packages to trigger logic
  ...
  requires:
    ...
    - headings:
        - pattern: Defining event struct
        - pattern: Using the `emit` function
