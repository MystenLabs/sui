...
gRPC provides three ways to access events:

- **`LedgerService.ListEvents`**: Paginated, filtered queries over a checkpoint range. Use an `EventFilter` to match by event type, sender, or emitting module. This is the direct replacement for `suix_queryEvents`.
...
To backfill historical events and then continue in real time, use `ListEvents` to catch up to the current checkpoint, then switch to `SubscribeEvents` with the same filter. See the [Event indexer example](/getting-started/examples/event-indexer) for a worked example.
...
// Query historical events by type over a checkpoint range
const { responses } = client.ledgerService.listEvents({
...
These rate-limited endpoints are for development and public-good access. For production, run your own [full node](/operators/full-node/sui-full-node) or use a dedicated provider. Both gRPC and GraphQL endpoints are available for each network.
