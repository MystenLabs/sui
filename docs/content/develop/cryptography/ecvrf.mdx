...
goal:
  description: >-
    Reader understands elliptic curve verifiable random function is a
    cryptographic algorithm that enables you to generate a random number and
    provide proof that the number used a secret key for generation
  requires:
    - has_frontmatter:
        - title
        - description
