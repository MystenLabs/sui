...
title: Groth16
description: >-
  Use the Sui Move API to verify Groth16 zk-SNARK proofs over BN254 or BLS12-381
  curves, with examples using the Circom circuit language and Arkworks Rust
  library.
keywords:
  - groth16
  - zero knowledge
...
goal:
  description: >-
    Reader understands zero-knowledge proofs are used to validate statements
    without revealing information about the proof's inputs
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Groth16 in Sui?
  - How does groth16 work on Sui?
  - Why should I use groth16?
answer: >-
  A zero-knowledge proof allows a prover to prove that a statement is true
  without revealing any information about the inputs.
---
...
Groth16 requires a trusted setup for each circuit to generate the verification key. The API is not pinning any particular verification key and each user can generate their own parameters or use an existing verification key for their apps.
