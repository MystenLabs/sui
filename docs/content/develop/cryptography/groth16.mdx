...
The randomly generated proving and verifying keys used in the following example (via `generate_random_parameters_with_reduction` with a fixed seed) are for testing only and are **not secure**. In production, proving and verifying keys must come from a proper trusted setup ceremony. Because the Sui API does not pin any verification key, your contract must pin or verifiably identify the expected verifying key it accepts; otherwise a proof generated against an attacker-chosen key could be accepted as valid.
