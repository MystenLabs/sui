...
Use `sui client chain-identifier` to find the chain ID for your network. For local or ephemeral deployments, use `sui move test-publish` instead.
...
When you run `sui move build` or `sui move test`, the system ensures you have all dependencies cached in your `~/.move` directory. It first checks whether the lockfile is current, and if it is not, it repins all of the dependencies for your build environment. It then checks whether the packages are cached, and if not it downloads them.
...
1. Publish with test-publish (no permanent record): `sui move test-publish`
1. After each code change, re-run `sui move test-publish` for a fresh deployment.
