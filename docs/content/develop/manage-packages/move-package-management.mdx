...
If you see `Your active environment is not present in Move.toml`, add an `[environments]` section:
...
Find the chain ID for your network with `sui client chain-identifier`. For local or ephemeral deployments, use `sui move test-publish` instead.
