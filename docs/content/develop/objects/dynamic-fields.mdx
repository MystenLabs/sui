...
A transaction that attempts to add a field with the same `<Name>` type and value as one that is already defined fails. You can modify fields in-place by borrowing them mutably, overwrite them safely by removing the old value first, or use the `replace` function to swap out the value directly.
...
## Replacing a dynamic field value

Use the `replace` function to swap out the value of an existing dynamic field and receive the old value in return:

<ImportContent source="crates/sui-framework/packages/sui-framework/sources/dynamic_field.move" mode="code" fun="replace" noComments />

This function requires that a field with the given `name` already exists on `object`. If no such field exists, the transaction aborts. The old value is returned, so you can inspect, delete, or transfer it as needed.

:::info

`sui::dynamic_object_field` has an equivalent `replace` function for object fields, with the added constraint `Value: key + store`.

:::

Use `replace` when you want to atomically swap a field's value without first removing it. Compared to the alternatives:

- Use `borrow_mut` when you want to update fields of the existing value in-place without replacing the value itself.
- Use `remove` followed by `add` when you need to replace a value and the field might not already exist, or when you want to change the value type.
- Use `replace` when the field is guaranteed to exist and you want to directly substitute the stored value, receiving the old value back.
