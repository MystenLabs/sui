...
### Checking field existence and optional removal

To check whether a dynamic field exists before accessing it, use `sui::dynamic_field::exists`:

<ImportContent source="crates/sui-framework/packages/sui-framework/sources/dynamic_field.move" mode="code" fun="exists" noComments />

To remove a field that may or may not be present (returning an `Option`), use `sui::dynamic_field::remove_opt`:

<ImportContent source="crates/sui-framework/packages/sui-framework/sources/dynamic_field.move" mode="code" fun="remove_opt" noComments />

`sui::dynamic_object_field` provides equivalent `exists` and `remove_opt` functions for object fields.

:::warning

`sui::dynamic_field::exists_` is deprecated; use `sui::dynamic_field::exists` instead.

`sui::dynamic_field::remove_if_exists` is deprecated; use `sui::dynamic_field::remove_opt` instead.

The same applies to the equivalent `sui::dynamic_object_field` functions: `exists_` is deprecated in favor of `exists`, and `remove_opt` has been added as the preferred way to optionally remove an object field.
