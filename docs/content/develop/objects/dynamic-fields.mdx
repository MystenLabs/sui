...
A transaction fails if it attempts to borrow a field that does not exist. Use `borrow_or_add` or `borrow_mut_or_add` to avoid this abort when a default value is acceptable.
...
### Additional access utilities

The `sui::dynamic_field` and `sui::dynamic_object_field` modules also provide higher-level convenience functions for common patterns:

- **`borrow_or_add`** / **`borrow_mut_or_add`**: Borrows (or mutably borrows) a field by name, inserting a provided default value first if the field does not yet exist. This avoids the abort that `borrow`/`borrow_mut` would cause on a missing field.
- **`get_do`** / **`get_mut_do`**: If a field with the given name exists, applies a provided function to its value. Does nothing if the field is absent.
- **`get_fold`** / **`get_mut_fold`**: Applies a provided function to a field's value if it exists, or returns a default value if the field is absent.
