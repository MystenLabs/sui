...
The multi-member model exists in the type today, but the public transfer functions currently accept only single-owner parties, so `single_owner` is the only constructor you can use. Support for multi-member parties is planned.
