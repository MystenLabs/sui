...
A party object is owned by a `Party`: a permission set that determines which addresses can access the object and how. Like [shared](/develop/objects/object-ownership/shared) objects, party objects are sequenced and versioned by [consensus](/develop/sui-architecture/consensus), so several transactions can use them concurrently. Unlike shared objects, only the owning party can access a party object, and you can transfer it to and from other ownership types and wrap it.
