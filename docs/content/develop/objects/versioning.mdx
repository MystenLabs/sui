...
goal:
  description: >-
    Reader understands sui provides `Table` and `Bag` collections built using
    dynamic fields, but with additional support to count the number of entries
    they contain to protect against accidental deletion when non-empty
  requires:
    - has_frontmatter:
        - title
        - description
    ...
    - headings:
        - pattern: Object versioning paths
        - pattern: Fastpath objects
        - pattern: Consensus objects
