...
Sui tracks every onchain object by a unique (ID, version) pair. Each time a transaction modifies an object, Sui writes the updated contents to the same ID at a new, strictly higher version. Versions are assigned using Lamport timestamps, meaning no (ID, version) pair is ever reused. Objects follow one of two versioning paths: fastpath (for address-owned and immutable objects) or consensus (for shared and party objects). The path determines how versions are assigned and what ownership models are available.
...
The following Move example shows how you can read an object's current version at runtime using the `sui::object` module:
...
module example::versioning {
    ...
    /// Returns the current version of the object as a u64.
    public fun current_version(obj: &MyObject): u64 {
        object::version(&obj.id)
    }

    /// Mutates the object, causing its version to increment after the transaction.
...
After `increment_value` runs in a transaction, Sui increments the object's version according to the Lamport timestamp rule. The new version is always `1 + max(version of all input objects in that transaction)`.
...
## Why does object versioning matter?

Understanding object versioning helps you design applications correctly and avoid common mistakes:

- **Avoid stale version references.** If your client code caches an object version and another transaction modifies that object before yours executes, validators reject your transaction. Always fetch the latest version before submitting.
- **Choose the right versioning path.** Fastpath reduces latency for single-owner workflows. Consensus handles multi-party coordination without requiring offchain locking.
- **Prune safely.** Because (ID, version) pairs are never reused, node operators can safely prune old versions without risking collision with future writes.
- **Reason about history.** The strictly increasing version sequence gives every object a linear, auditable history. Archival nodes can serve any historical version to clients that need it.