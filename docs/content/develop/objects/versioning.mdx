...
description: >-
  Sui versions every onchain object by (ID, version) pair using Lamport
  timestamps, with objects versioned either through the fastpath or through
  consensus depending on their ownership type.
keywords:
  - object versioning
  - Lamport timestamps
  - fastpath
...
Move code cannot read an object's version. The runtime assigns the new version after execution finishes, so no value exists to read while your function is running, and `sui::object` exposes no accessor for it. Your Move code sees an object's ID through `object::id` and its contents through its fields, and that is all.

Read versions from a client instead. Every object the SDK returns carries its version:
...
const { object } = await client.getObject({ objectId: '0xOBJECT_ID' });

console.log(object.version); // for example, "6"
```

The `version` field is a string, because versions are `u64` values that can exceed the range JavaScript numbers represent exactly.
