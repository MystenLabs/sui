...
Declare functions that consume randomness as `entry` functions that are not `public`. A non-`public` `entry` function can be called directly from a transaction but not from another Move module. This prevents other modules from composing your randomness-consuming function into a larger call that enables a test-and-abort attack.
