...
- Protect the `UpgradeCap` with the same care as an admin capability. Whoever holds it can change your package.
...
- Call `make_immutable` on the `UpgradeCap` when you no longer need upgrades. This permanently freezes the package.
...
Declare functions that consume randomness as private `entry` functions. A private `entry` function can be called directly from a transaction but not from another Move module. This prevents other modules from composing your randomness-consuming function into a larger call that enables a test-and-abort attack.
...
Design functions so all outcomes consume similar gas. Otherwise an attacker can set a gas budget that only succeeds on favorable outcomes.
