...
You are trying to use a named address, such as `std` or similar, within your code or a dependency, but you have not assigned that address a value in the `Move.toml` file. 
...
You have defined two or more modules with the same name or address in your project or its dependencies. You must give each module a unique name and address combination. 
...
A module in your package does not have the address `0x0`. In Move, you must set all modules to have their self-address as `0x0` in the source code. When you publish the package, Sui automatically assigns it the correct address. 
...
A function you are calling in your Move module expects 1 signer argument, yet you provided 2. On Sui, the number of signer arguments in your function's signature must match the number of signers you provide. 
...
Your Move code fails either during bytecode verification or during deserialization. 
...
Your transaction is trying to use one or more Sui objects that another transaction already uses. When you use an object in a transaction, the system locks that object to prevent multiple, possibly conflicting modifications to an object. For DeFi objects, this prevents double-spending.
...
An object you are trying to use is in an equivocated state, meaning the network cannot reach consensus about its state due to conflicting transactions. The network freezes the object and you cannot use it until the next epoch.
...
The sender address either has insufficient SUI balance or the gas coin is already used as a transaction input object.

##### Solution

- Verify the wallet has enough SUI to cover the gas budget.
- Do not pass the same coin object as both a gas payment and a transaction input.
...
You wrapped a value already returned by a transaction command (such as `tx.splitCoins()` or `tx.moveCall()`) inside `tx.object()` again. Values returned by transaction commands are already `TransactionArgument` objects.
...
The Sui CLI cannot find the environment alias you are using (such as `devnet` or `testnet`) in the `[environments]` section of your `Move.toml` file.
...
Look up the chain ID for your network with `sui client chain-identifier`. For local or ephemeral deployments, use `sui move test-publish` instead.
...
| -32604 | Transaction Execution Error | No        | On-chain execution failed (MoveAbort).    |
| -32050 | Transient Error             | Yes       | Overloaded node; retry with exponential backoff. |
| -32002 | Server Busy                 | Yes       | Rate limited; slow down request frequency. |
...
| Execution errors  | `MoveAbort`, `InsufficientGas`        | On-chain VM      | Fix code or parameters.|
