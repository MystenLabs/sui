...
Indicates the sender address either has insufficient SUI balance or the gas coin is already used as a transaction input object.
...
Indicates that you wrapped a value already returned by a transaction command (such as `tx.splitCoins()` or `tx.moveCall()`) inside `tx.object()` again. Values returned by transaction commands are already `TransactionArgument` objects.
...
Indicates that the Sui CLI cannot find the environment alias you are using (such as `devnet` or `testnet`) in the `[environments]` section of your `Move.toml` file.
