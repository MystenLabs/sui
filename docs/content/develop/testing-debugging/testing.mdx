...
- **Dry runs:** Use `sui client call --dry-run` (or `--dev-inspect`) from the CLI, or the TypeScript SDK's `simulateTransaction` client method, to simulate a transaction against the current network state without executing it. This catches issues like incorrect object versions or insufficient gas before you submit.
