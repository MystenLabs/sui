...
goal:
  description: >-
    Reader understands a Sui transaction must pay for both the computational
    cost of execution and the long-term cost of storing the objects a
    transaction creates or mutates
  requires:
    - has_frontmatter:
        - title
        - description
    ...
    - headings:
        - pattern: Gas fee structure
        - pattern: 'Computation units \{#computation\}'
        - pattern: 'Storage units \{#storage\}'
...
The $StoragePrice$ is set exogenously through the governance proposal with the goal of targeting the offchain dollar cost of data storage. In the long run, as the costs of storage fall due to technological improvements and the dollar price of the SUI token evolves, governance proposals update the price in order to reflect the new dollar target price.
