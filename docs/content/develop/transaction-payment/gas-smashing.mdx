...
Gas smashing applies only to real `Coin<SUI>` objects that you supply as gas payment. When you provide multiple `Coin<SUI>` objects, the network merges them into a single coin and deducts the gas fee from that coin.
...
The compatibility layer can present compatibility coin reservations to older clients. A compatibility coin reservation is a synthetic object reference that reserves address balance funds, not a persisted `Coin<SUI>` object. Gas smashing does not merge these reservations, because the network resolves an address balance reservation as a withdrawal instead of smashing it into a coin.
