...
keywords: [ usdc, stablecoins, sui dollar, usdsui, usdy, ausd, fdusd, buck, ysld, gasless, free tier, free ]
---
...
Stablecoins qualified for gasless stablecoin transfers can be found [here](https://github.com/MystenLabs/sui/pull/26417).
...
To send an allowlisted stablecoin gas-free, build a PTB that calls `0x2::balance::send_funds<T>` with a `Balance<T>` from `tx.balance()`, and submit it through `SuiGrpcClient`. The SDK detects eligibility during simulate, sets `gasPrice = 0` and `gasBudget = 0`, and pays gas from the sender's address balance.
