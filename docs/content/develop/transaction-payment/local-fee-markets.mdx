...
description: >-
  Object-based local fee markets limit the rate of transactions writing to a
  single shared object, preventing the network from becoming overloaded with
  checkpoints that take too long to execute.
keywords: [fee markets, gas price, congestion, shared objects, transaction deferral]
goal:
  description: >-
    Reader understands object-based local fee markets limit the rate of
    transactions writing to a single shared object, preventing the network from
    becoming overloaded with checkpoints that take too long to execute
  requires:
    - has_frontmatter:
        - title
        - description
