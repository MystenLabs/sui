...
Use `sui client ptb` to build and execute PTBs from the command line. Prefer `sui client ptb` for examples that build transactions, including simple transfers and coin operations, because it exposes the same PTB commands used by SDKs.
