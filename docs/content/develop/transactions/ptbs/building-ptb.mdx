...
This guide explores creating a [programmable transaction block (PTB)](/develop/transactions/ptbs/prog-txn-blocks) on Sui using the [TypeScript SDK](https://sdk.mystenlabs.com/typescript/install).
...
- For pure values: Use the `tx.pure(type, value)` function to construct an input for a non-object input.
  - If value is a `Uint8Array`, the function treats the value as raw bytes and uses it directly: `tx.pure(SomeUint8Array)`.

  - Otherwise, the function uses it to generate the BCS serialization layout for the value.
...
Build a PTB using the Sui Rust SDK:
