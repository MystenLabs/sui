...
description: >-
  Programmable transaction blocks accept two types of inputs (objects and pure
  values) and produce results that subsequent commands can use.
keywords:
  - PTB inputs
  - PTB results
  - pure values
...
goal:
  description: >-
    Reader understands programmable transaction blocks accept two types of
...
builder_paths:
  ...
  - path_id: walrus-agentic
    ...
    stage: Agent Write
questions:
  - What is Inputs and Results in Sui?
  - How does inputs and results work on Sui?
  - Why should I use inputs and results?
answer: >-
  Programmable transaction blocks accept two types of inputs (objects and pure
  values) and produce results that subsequent commands can use.
---
...
- If the object is owned by an address or it is immutable, then use `ObjectArg::ImmOrOwnedObject(ObjectID, SequenceNumber, ObjectDigest)`. Note that if the object is immutable, it cannot be transferred to another object. The triple respectively specifies the object's ID, its sequence or version number, and the digest of the object's data.
