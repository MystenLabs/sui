...
This primitive is specified in [SIP-19: Soft Bundle API](https://github.com/sui-foundation/sips/blob/main/sips/sip-19.md), which has a status of Draft. Sui enables the feature through the `soft_bundle` protocol configuration flag.
...
The SIP originally specified stricter bundle-level rules, such as requiring every transaction to access a shared object. The current implementation does not enforce those as rejection rules. The same gas price still matters for ordering, as described in [How soft bundles work](#how-soft-bundles-work).
