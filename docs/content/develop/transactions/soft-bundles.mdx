...
This primitive is specified in [SIP-19: Soft Bundle API](https://github.com/sui-foundation/sips/blob/main/sips/sip-19.md), which has a status of Final. Sui enables the feature through the `soft_bundle` protocol configuration flag.
...
Unable to determine exact correction without access to SIP-19 document; this claim requires verification against the actual SIP-19 specification. The current implementation does not enforce those as rejection rules. The same gas price still matters for ordering, as described in [How soft bundles work](#how-soft-bundles-work).
