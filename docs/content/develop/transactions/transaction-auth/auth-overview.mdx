...
description: >-
  Understanding cryptographic keys, addresses, and signatures on Sui. Sui
  supports multiple cryptography algorithms and primitives with rapid switching
  between them.
keywords:
  - authentication
  - keys
...
goal:
  description: 'Reader understands cryptographic keys, addresses, and signatures on Sui'
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  ...
  - path_id: agentic-payments
    ...
    stage: Authorization
questions:
  - How do I sign and send transactions on Sui?
  - Which signature schemes does Sui support?
  - What is Signing and Sending Transactions in Sui?
answer: >-
  Understanding cryptographic keys, addresses, and signatures on Sui. Sui
  supports multiple cryptography algorithms and primitives with rapid switching
  between them.
...
For managing wallets that support the Ed25519 (EdDSA) signing scheme, Sui follows [BIP-32](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki) or [SLIP-0010](https://github.com/satoshilabs/slips/blob/master/slip-0010), which enables wallets to derive child private keys from parent private keys using key derivation paths.
...
If you want to use a specific gas coin, first find the gas coin object ID to be used to pay for gas and explicitly use that in the PTB. If there is no gas coin object, use the [splitCoin](/develop/transactions/ptbs/building-ptb) transaction to create a gas coin object. The split coin transaction should be the first transaction call in the PTB.
...
```
```sh
$ sui client ptb --transfer-objects "[gas]" @$SUI_ADDRESS --gas-coin @$GAS_COIN_ID --gas-budget $GAS_BUDGET --serialize-unsigned-transaction
```
```sh
