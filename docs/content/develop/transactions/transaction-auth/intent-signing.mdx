...
The authority signature is created using the protocol key. Like a user signature, it commits to the Blake2b-256 hash of the BCS serialization of an `IntentMessage`, not to the raw message bytes. Authority signatures use the internal intent scopes, such as `TransactionEffects`, `ProofOfPossession`, and `SenderSignedTransaction`. For all available intent scopes, see the `IntentScope` enum in [`crates/shared-crypto/src/intent.rs`](https://github.com/MystenLabs/sui/blob/main/crates/shared-crypto/src/intent.rs).
