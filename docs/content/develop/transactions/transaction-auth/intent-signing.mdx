...
To create a user signature, construct an intent message first, then sign the 32-byte Blake2b-256 hash of its BCS serialization. The signer does not sign the raw transaction bytes. It signs the hash of the structured `IntentMessage`, whose BCS serialization is the 3-byte intent followed by the BCS bytes of the transaction data. The signed digest therefore commits to both the intent and the message.
