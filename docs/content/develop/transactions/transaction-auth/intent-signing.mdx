...
questions:
  ...
  - What are the intent scopes and app IDs in Sui signing?
answer: >-
  Intent signing provides a compact domain separator to data signed by both user
  and authority signatures.
---

An intent is a compact struct that serves as the domain separator for a message that a signature commits to. The data that the signature commits to is an intent message. All user signatures in Sui must commit to an intent message rather than the message itself.
