...
description: >-
  ...
  local network using the Sui CLI.
keywords:
  - multisig
  - multi-sig
  - multisig auth
...
goal:
  description: >-
    ...
    submit it against a local network using the Sui CLI
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  ...
  - path_id: agentic-payments
    ...
    stage: Authorization
questions:
  - What is Multisig Authentication in Sui?
  - How does multisig authentication work on Sui?
answer: >-
  Guide on how to create a multisig transaction and then submit it against a
  local network using the Sui CLI.
---

Sui supports multi-signature (multisig) transactions, which require multiple keys for authorization rather than a single key signature. Sui supports `k` out of `n` multisig transactions, where `k` is the threshold and `n` is the total weight of all participating parties. The maximum number of parties is 10. To learn more about supported single key signatures, see [Signatures](/develop/transactions/transaction-auth/auth-overview).
...
The following steps demonstrate how to create a multisig transaction and then submit it against a network using the [Sui CLI](/references/cli). A transaction can be the transfer of an object, the publish or upgrade of a package, the payment of SUI, and so on.
...
The topic also assumes that your active environment is Testnet (`sui client active-env`). You can perform these steps on Devnet or a Localnet as well, but you must adjust the commands appropriately.
