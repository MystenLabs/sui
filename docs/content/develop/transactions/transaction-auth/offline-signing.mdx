...
goal:
  description: >-
    ...
    when a device is not connected to a Sui network, or if the signing wallet
    uses a different programming language than that of the transaction being
    signed
  requires:
    - has_frontmatter:
        - title
    ...
    - headings:
        ...
        - pattern: 'Sign the serialized data \{#sign\}'
        - pattern: 'Execute the signed transaction \{#execut'
