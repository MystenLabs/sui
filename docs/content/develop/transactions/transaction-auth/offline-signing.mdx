...
description: >-
  Offline signing can be used to sign a Sui transaction when a device is not
  connected to a Sui network, or if the signing wallet uses a different
...
goal:
  description: >-
    Reader understands offline signing can be used to sign a Sui transaction
    when a device is not connected to a Sui network
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  ...
  - path_id: agentic-payments
    ...
    stage: Authorization
questions:
  - What is Offline Signing in Sui?
  - How do I sign the serialized data sign?
  - How does offline signing work on Sui?
answer: >-
  Offline signing is the process of signing transactions using a device that is
  not connected to a Sui network or using a wallet implemented in a different
  programming language that doesn't rely on Sui Keystore.
---
