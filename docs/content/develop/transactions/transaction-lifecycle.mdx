...
1 SUI = 1,000,000,000 MIST (10^9). All SDK and RPC amounts use MIST. Multiply your SUI amount by `1_000_000_000` before passing it to the SDK.
...
1. The client sends the transaction to each [validator](/operators/validator), often through a [full node](/operators/full-node/sui-full-node). Validators perform a series of validity and safety checks, sign the transaction, and return the signed transaction to the client.

1. The client collects the responses from a set of validators that account for at least 2/3 of the [validator committee](/develop/sui-architecture/consensus) to form a transaction certificate. As a result, Sui validators do not need to propagate signatures on a best-effort basis (gossip signatures) or aggregate certificates. This effort is now the responsibility of the client or gateway.
...
1. After assembling the certificate, the client sends it back to all validators, who check its validity and acknowledge its receipt to the client.
    ...
    1. If the transaction involves [owned objects](/develop/objects/object-ownership/address-owned) or immutable objects exclusively, Sui can process the transaction certificate immediately and execute it without waiting for the [consensus engine](/develop/sui-architecture/consensus).
...
1. Subsequently, Sui forms [checkpoints](/develop/sui-architecture/checkpoint-verification) for every consensus commit, which it uses to drive the [reconfiguration protocol](/develop/sui-architecture/epochs).
...
Periodically (~24 hours), the Sui [Testnet](/develop/sui-architecture/networks) and [Mainnet](/develop/sui-architecture/networks) networks enter the process of [epoch](/develop/sui-architecture/epochs) change. During epoch change, the network calculates and distributes [staking rewards](/develop/sui-architecture/tokenomics-overview), validator metadata takes effect, and other network processes are performed. The network delays all user transactions until the new epoch begins.

If your transaction is submitted at an epoch boundary, in rare timing a quorum of validators might have already decided to not accept new transaction certificates in the current epoch. This means your transaction is not checkpointed either. For any transactions that are certified but not finalized, the network reverts them if executed or unlocks their owned objects if not yet executed.
...
console.log(`${Number(balance.totalBalance) / 1e9} SUI`);
```
...
// Returns the base gas price in MIST
```

### Get checkpoint from a transaction
...
console.log('Checkpoint:', txn.checkpoint);
```

:::info Minimum balance

Sui does not enforce a minimum wallet balance. However, you must have enough SUI to cover gas fees for any transaction you submit.
