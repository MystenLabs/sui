...
A struct with `key` but without `store` must be explicitly transferred or destroyed. This is the foundation of resource safety in Move. See [The Move Book: Abilities](https://move-book.com/move-basics/abilities-introduction.html).
