...
title: Move Concepts
description: >-
  Move is an open source language for writing safe packages to manipulate
  onchain objects.
keywords: - move on sui, move on sui
  - move language
  - what is move
  - move objects
...
goal:
  description: >-
    Reader understands move is an open source language for writing safe packages
    to manipulate onchain objects
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Move Concepts in Sui?
  - How does move concepts work on Sui?
  - Why should I use move concepts?
answer: >-
  Move is an open source language for writing safe packages to manipulate
  onchain objects.
---
...
You can use Move to define, create, and manage programmable Sui objects representing assets and smart contracts. Sui's [object system](/develop/sui-architecture/object-model) is implemented by adding new functionality to Move while also imposing additional restrictions.
...
See the [address section](https://move-book.com/reference/primitive-types/address) in The Move Book for an overview on addresses and refer to <UnsafeLink href="/references/framework/sui_sui/object">object.move</UnsafeLink> in the Sui framework for implementation details.
...
The `entry` keyword has a specific use case in Move on Sui. Use `entry` when you want some functionality to be usable by anyone on chain, but not be wrapped around other Move logic. In other words, a function can be called in [programmable transaction blocks (PTBs)](/develop/transactions/ptbs/prog-txn-blocks) but not in other Sui packages. For example, this is utilized when using Sui's onchain randomness standard to prevent other smart contract engineers from creating logic to essentially front-run or back-run the randomness generation. Find more information about this in the [on-chain randomness page](/sui-stack/on-chain-primitives/randomness-onchain#use-non-public-entry-functions).
