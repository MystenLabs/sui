...
questions:
  - What is Developer Cheat Sheet in Sui?
  - How do I sign?
  - How does developer cheat sheet work on Sui?
answer: Quick reference on best practices for Sui Network developers.
---
...
- For optimal performance and data consistency, submit writes and reads to the same full node. In the TS SDK, use dApp Kit's [`useSignAndExecuteTransaction`](https://sdk.mystenlabs.com/dapp-kit) hook, which prompts the wallet only for a signature and then executes the transaction through the client your app configures in `SuiClientProvider`. Alternatively, sign with the [`useSignTransaction`](https://sdk.mystenlabs.com/dapp-kit) hook and submit the signed transaction yourself with your client's `core.executeTransaction` method (decode the returned base64 `bytes` with `fromBase64` from `@mysten/sui/utils` first). Avoid wallet APIs that execute the transaction on the wallet's own full node. This ensures read-after-write consistency. Reads from your app's full node reflect writes from the transaction right away instead of waiting for a checkpoint.
