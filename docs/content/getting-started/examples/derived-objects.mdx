...
entry_functions:
  - module: objects
    name: create_incremental
    file: C5/contracts/derived_objects/sources/objects.move
  - module: objects
    name: create_address
    file: C5/contracts/derived_objects/sources/objects.move
  - module: objects
...
[Derived objects](/develop/objects/derived-objects) are Sui objects with deterministic IDs computed from a parent object and a key. You can predict a derived object's ID offchain before the transaction that creates it runs, which enables indexer-free lookups, batch queries, and state verification.
...
1. The TypeScript test computes the expected object ID offchain using `deriveObjectID(parentObjectId, keyType, serializedKey)`. This requires no network call.
...
[Learn more about derived objects](/guides/developer/objects/derived-objects).
...
<ImportContent source="C5/contracts/derived_objects/sources/parent.move" mode="code" org="MystenLabs" repo="sui-move-bootcamp" ref="solution" struct="ParentObject" />
...
<ImportContent source="C5/contracts/derived_objects/sources/objects.move" mode="code" org="MystenLabs" repo="sui-move-bootcamp" ref="solution" fun="create_incremental" />
...
<ImportContent source="C5/contracts/derived_objects/sources/objects.move" mode="code" org="MystenLabs" repo="sui-move-bootcamp" ref="solution" fun="create_address" />
...
<ImportContent source="C5/ts/src/helpers/deriveObjectID.ts" mode="code" org="MystenLabs" repo="sui-move-bootcamp" ref="solution" fun="deriveObjectIDFromParent" />
...
<ImportContent source="C5/ts/tests/derive.test.ts" mode="code" org="MystenLabs" repo="sui-move-bootcamp" ref="solution" fun="reverse search pagination" />
...
**Cause:** A previous transaction already created a derived object with the same parent + key combination. Derived IDs are unique per parent-key pair.
...
**Cause:** The `PARENT_OBJECT_ID` in `.env` is wrong, or you published the package on a different network than the client targets.

**Fix:** Verify the parent object ID from the publish transaction output. Confirm `NETWORK` in `.env` matches the network you published the package on.
