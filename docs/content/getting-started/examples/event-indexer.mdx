...
description: >-
  Build a real-time event indexer that subscribes to Sui checkpoints through
  gRPC, filters for specific Move events, and decodes BCS-encoded event data in
  TypeScript.
keywords:
  - example
  - indexer
...
goal:
  description: >-
    Reader can build a real-time event indexer that subscribes to Sui
    checkpoints through gRPC
  requires:
    - headings:
    ...
    - min_words: 800
      label: Needs more explanation depth
    - pattern: mermaid|sequenceDiagram|graph|flowchart
      min: 1
      label: Has architecture diagram
    - headings:
...
questions:
  - How do I build a real-time event indexer for Sui?
  - How do I subscribe to Sui checkpoints via gRPC?
  - How do I decode BCS-encoded Move events in TypeScript?
answer: >-
  Build a real-time event indexer that subscribes to Sui checkpoints through
  gRPC, filters for specific Move events, and decodes BCS-encoded event data in
  TypeScript.
...
- **gRPC checkpoint streaming:** The `SuiGrpcClient.subscriptionService.subscribeCheckpoints` method opens a persistent stream that yields each new checkpoint as it finalizes. The `readMask` parameter filters the response to only include the fields you need (in this case, `transactions.events`).

- **BCS event decoding:** Move encodes events with BCS. The TypeScript SDK provides `bcs.struct` (from `@mysten/sui/bcs`) to define a schema that matches the Move struct layout. Parsing the raw bytes with this schema produces a typed JavaScript object.
...
4. The indexer filters the checkpoint's transactions for events matching the `UserRegistered` type string (`PACKAGE_ID::indexer_sample::UserRegistered`).
...
The `subscriptionService.subscribeCheckpoints` call opens a persistent gRPC stream starting from the latest checkpoint. The `readMask` limits the response to `transactions.events`, reducing bandwidth. The `for await` loop processes each checkpoint as it arrives, calling `processCheckpoint` to filter and decode events.
...
The `USER_REGISTERED_EVENT_BCS` schema defines the BCS layout: `bcs.Address` for the owner, `bcs.string()` for the name, and `bcs.u64()` for the user ID. The function extracts the raw bytes from the event, converts them to a `Uint8Array`, and parses them with the schema. The function converts `user_id` to a string because JavaScript cannot safely represent all u64 values as numbers.
...
**Cause:** The `FULL_EVENT_NAME` string does not match the deployed contract's event type. The format must be `PACKAGE_ID::MODULE_NAME::UserRegistered` with the exact package ID and module name.

**Fix:** Verify `PACKAGE_ID` and `MODULE_NAME` in `.env` match the deployed contract. The module name in the Move source is `indexer_sample` (from `module indexer_sample::indexer_sample`).
...
**Fix:** Verify the TypeScript `bcs.struct` field order matches the Move struct exactly. `bcs.Address` maps to `address`, `bcs.string()` maps to `String`, and `bcs.u64()` maps to `u64`.
