...
- **Public vs entry visibility:** An `entry` function can only be called directly from a programmable transaction block (PTB), not from another Move module. A `public` function can be called from anywhere, including other modules that inspect the result and conditionally abort.
...
Sui also enforces `PostRandomCommandRestrictions`: after a PTB command that uses `Random`, subsequent commands in the same PTB are restricted. Your wrapper must handle the entire flow (open and extract) in a single Move function call, not as separate PTB commands.
...
**Fix:** Obtain Testnet USDC from a faucet. You must obtain Testnet USDC on the Sui Testnet. Obtaining Testnet USDC on other chains will not work on Sui.
