...
The example has three actors that interact with one onchain package. The following diagram shows the components and the calls between them.
...
The `plinko` Move package holds game state, enforces betting rules, and computes payouts. The Random module is a Sui framework object that provides verifiable random bytes. The contract uses these bytes to determine ball paths, so outcomes are reproducible onchain rather than decided offchain.
