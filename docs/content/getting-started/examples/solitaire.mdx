---
title: Solitaire
description: Play solitaire onchain where every card shuffle uses Sui's [Random module](/sui-stack/on-chain-primitives/randomness-onchain) and every move is a [sponsored transaction](/develop/transaction-payment/sponsor-txn) verified on the blockchain.
...
6. If the transaction fails (the move was invalid), the frontend calls `handleFailedTransaction` to refetch the full onchain game state and reset the board.
...
The function validates that the target placement follows Klondike rules (alternating color, descending rank, Kings on empty columns). When a face-down card is revealed, it calls `reveal_card` with `&Random` and emits a `CardRevealed` event that the frontend reads.
...
- **Change card visuals:** Replace the SVG card images in `cardMappings.ts` with custom artwork. The card IDs (0-51) map deterministically to suits and ranks.
...
**Cause:** The frontend's local validation (`useSolitaireGameMoves`) and the Move contract disagree on the board state. This happens when a previous transaction's `CardRevealed` event was missed.
...
**Cause:** The game state is fetched from the chain on load. If the previous session's game was deleted or finished, no game object exists.
