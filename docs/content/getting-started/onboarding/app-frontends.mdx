...
The example frontend uses [`@mysten/dapp-kit`](https://sdk.mystenlabs.com/dapp-kit), which provides React hooks and components for wallet connection, transaction signing, and onchain data queries. The key components used in this example are:
...
The `dapp-kit.ts` file in the example project configures the Sui dApp Kit provider with network settings. For a deeper walkthrough of building a frontend with Sui dApp Kit, see the [dApp Kit starter app](https://github.com/MystenLabs/sui-move-bootcamp/tree/solution/E2/my-first-sui-dapp).
