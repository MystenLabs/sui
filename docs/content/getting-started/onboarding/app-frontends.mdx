...
builder_paths:
  - path_id: p2p-payments
    ...
    stage: Setup
questions:
  - How do I connect a React frontend to a Sui Move package?
  - How do I call a Move function from a web app?
  - What is the Sui dApp Kit and how do I use it?
answer: >-
...
The example frontend uses [`@mysten/dapp-kit`](https://sdk.mystenlabs.com/dapp-kit), the official React library for building Sui apps. Sui dApp Kit provides React hooks and components for wallet connection, transaction signing, and onchain data queries. The key components used in this example are:
...
- **`useCurrentAccount`:** A React hook that returns the currently connected wallet account, including the user's Sui address. Use it to check whether a user is connected and to read their address for transaction construction.
- **`useSignAndExecuteTransaction`:** A React hook that signs and submits a transaction through the connected wallet.

The `dapp-kit.ts` file in the example project configures the Sui dApp Kit provider with network settings. For a deeper walkthrough of building a frontend with Sui dApp Kit, see the [dApp Kit Frontend](/getting-started/examples/dapp-kit-frontend) example.
