...
description: >-
  You need an address on the Sui network before you can build packages and own
  objects.
keywords: - create an address, create an address
  - sui address
...
goal:
  description: >-
    Reader can create a Sui address, understand recovery phrases, and manage
    multiple addresses
  requires:
    - pattern: '```'
      min: 2
...
questions:
  - How do I create a Sui address?
  - What is a recovery phrase and how do I back it up?
  - How do I manage multiple Sui addresses?
answer: >-
  An address is a unique, anonymous identity on a blockchain network. On Sui, an
...
Every Sui address is a unique 32-byte identifier and appears in hexadecimal encoding with a `0x` prefix, unless you refer to it by an [alias](#address-aliases). For example:
