...
description: >-
  Build and publish your first Move package using a basic 'Hello, World!'
  example.
keywords:
  - first dapp
  - first package
  ...
  - publish package
  - call package
  - make call to package
  - how to call package
pagination_prev: null
pagination_next: null
goal:
  description: >-
    Reader can clone, build, publish, and call a Move package, understanding
...
questions:
  - How do I build and publish a package?
  - What is Move?
  - How do I build the move package?
  - How do I publish the move package?
answer: >-
  You'll build a "Hello, World!" program to learn the fundamentals of
  programming on Sui. You create programs on Sui by writing and deploying smart
...
- All resources must be either moved into global storage or explicitly destroyed by the end of a transaction. 

- Resources cannot be copied. 

In the "Hello, World!" example, the struct `Greeting` is a resource type. 

To satisfy the requirement that all resources must be moved or destroyed by the end of a transaction, `new_greeting` is assigned to `Greeting`, which the call to `transfer::share_object(new_greeting)` then moves into global storage.
...
[Learn more](https://github.com/MystenLabs/sui/blob/main/external-crates/move/historical-versions/v1/crates/move-bytecode-verifier/README.md) about the Move Bytecode Verifier.
