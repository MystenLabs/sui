...
This page walks you through installing the [Sui CLI](/references/cli) and verifying it works. The CLI creates and manages address balances, builds and publishes smart contracts, and queries information from the network.

:::tip Looking for something else?

This page covers **installation only**. For other common tasks:

- **Connect to a network** → [Configure a Sui Client](/getting-started/onboarding/configure-sui-client)
- **Get test tokens** → [Get SUI from Faucet](/getting-started/onboarding/get-coins)
- **Build a Move package** → [Hello, World!](/getting-started/onboarding/hello-world)
- **Run a full node** → [Sui Full Node](/operators/full-node/sui-full-node)
- **Choose an SDK** → [Data Access](/develop/accessing-data/data-serving)
...
Homebrew and Chocolatey might compile dependencies from source if prebuilt binaries are not cached. This can take several minutes. For a faster installation, use `suiup` instead:
