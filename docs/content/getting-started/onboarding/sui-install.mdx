...
If no path is printed, the install directory is not in your `PATH`. Add `export PATH="$HOME/.local/bin:$PATH"` to your shell profile (`~/.bashrc`, `~/.zshrc`, or `~/.bash_profile`), then restart your terminal.
