...
Open a **new terminal window** after installing. If the issue persists, verify that `~/.local/bin` (Linux/macOS) or the suiup install directory is in your `PATH`:

```sh
$ echo $PATH | tr ':' '\n' | grep -i sui
```

If no path appears, add `export PATH="$HOME/.local/bin:$PATH"` to your shell profile (`~/.bashrc`, `~/.zshrc`, or `~/.bash_profile`), then restart your terminal.
