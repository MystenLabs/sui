---
title: Ethereum -> Sui
description: Build your first app on Sui if you have EVM experience
keywords:
  - ethereum
  - solidity
  - EVM
...
goal:
  description: >-
    Ethereum developer can map their existing mental model to Sui's
    object-centric equivalents and start building
  requires:
    - has_tables: true
      min: 2
      label: Has comparison tables
    - headings:
    ...
    - pattern: '```move'
      min: 1
      label: Has Move code example
    - min_words: 800
      label: Needs more comparison depth
    - has_frontmatter:
        - title
        - description
        - keywords
    ...
    - pattern: Ethereum
      min: 3
      label: References Ethereum for comparison
    - has_questions: true
...
questions:
  ...
  - How do I migrate from Ethereum to Sui?
  - How do I transition from Ethereum to Sui?
answer: >-
  If you have worked with Ethereum Virtual Machine (EVM) before, the biggest
  difference when developing on Sui is the programming language. Sui uses
  **Move** and EVM uses **Solidity**.
---
...
The following example shows a basic PTB that splits a coin and transfers the result to another address using the TypeScript SDK:

{/* TODO: Import code from MystenLabs/sui-typescript — show a basic PTB example using Transaction, splitCoins, and transferObjects */}
{/* Description: TypeScript PTB that splits a coin from the gas coin and transfers the split amount to a recipient address */}

The `splitCoins` command creates a new coin from the gas coin, and `transferObjects` sends it to the recipient. Both commands execute atomically in a single transaction. If either fails, the entire transaction reverts.
