...
Unlike Solana CPIs (limited to 4 levels deep with explicit account declarations), PTBs chain up to 1,024 commands in a single atomic transaction without depth limits. The `splitCoins` command creates a new coin, and `transferObjects` sends it to the recipient. Both execute atomically.
