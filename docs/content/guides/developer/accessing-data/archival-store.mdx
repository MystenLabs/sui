...
If you are using a different storage backend, build a compatible indexer using the [custom indexing framework](/guides/developer/accessing-data/custom-indexing-framework.mdx) and ensure it emits data in a structure that is compatible with the [gRPC API's](/concepts/data-access/grpc) `LedgerService` endpoints.
