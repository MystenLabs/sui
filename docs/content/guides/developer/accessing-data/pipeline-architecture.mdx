...
**`Collector` to `Committer`**: `COMMITTER_CHANNEL_SIZE` slots (default 10, override via env var) → `Collector` stops accepting.
...
<ImportContent mode="code" source="crates/sui-indexer-alt-framework/src/ingestion/broadcaster.rs" tag="bound" /> 
