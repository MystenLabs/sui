...
The TypeScript SDK provides methods to query events. For getting events from a specific transaction, use the `getTransaction` method with `include: { events: true }`:
...
// Define a type-safe query for filtering events by type
...
- Incorporate a [custom indexer](/guides/developer/accessing-data/custom-indexing-framework.mdx) to take advantage of Sui's micro-data ingestion framework.
