...
[TypeScript SDK - CoinWithBalance](https://sdk.mystenlabs.com/sui/transaction-building/intents#coinwithbalance):
...
- `withdrawFrom`: The source of funds. Use `{ $kind: 'Sender', Sender: true }` for the transaction sender.

Note: Explicit `FundsWithdrawal` inputs currently only support withdrawing from the sender's address balance. Sponsor withdrawals (`withdrawFrom: Sponsor`) are supported for gas payments (empty `gas_data.payment`) but not yet for explicit PTB withdrawal inputs.
...
[TypeScript SDK - Transaction.moveCall](https://sdk.mystenlabs.com/sui/transaction-building/basics#movecall):
