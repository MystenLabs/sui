...
Note: `tx.withdrawal()` creates a `Withdrawal<Balance<T>>` capability that must be redeemed. For portable code that works across different gas payment methods, prefer `coinWithBalance` instead.
