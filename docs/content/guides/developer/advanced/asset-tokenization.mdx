...
@Stefanos Pleros Todo: write an example flow of how the contract could be used in order to achieve a hypothetical scenario. Include diagrams.
...
```
**Tokenized Asset Architecture Diagram**
...
flowchart LR
    ...
    %% Q[Modules];
    %% X([Objects]);
    A[Template] -->|On new Asset type creation<br> the creator receives the Publisher for type T| B([Publisher&ltT&gt]);
    C[Proxy] -->|On init, the Publisher of the Asset Tokenization module<br> is created and added inside the Registry shared object | D([Registry]);
    D -->|Input to| E{Proxy Methods};
    B -->|Input to| E;
    E --> F([ProtectedTP]);
    E --> G([TransferPolicy&ltTokenizedAsset&ltT&gt&gt]);
    E --> H([TransferPolicyCap&ltTokenizedAsset&ltT&gt&gt]);

    E --> I([Display&ltTokenizedAsset&ltT&gt&gt]);
```
...
- `ProtectedTP`
    This is a shared object utilized to store an empty transfer policy. It is required to create one per type `<T>` generated by a user.
...
- `setup_tp`
    ...
    This function leverages the Publisher nested within the registry and the sender's Publisher to generate and return a Transfer Policy and the associate Transfer Policy Cap specific to the `TokenizedAsset<T>`, where the type `T` is derived from the Publisher object.
    It invokes the internal function `create_protected_tp()` as part of its process to accomplish its objective
...
- `created_protected_tp`

    Invoked within `setup_tp()`, this method is an internal function responsible for generating an empty transfer policy and sharing the `ProtectedTP<T>` object. This functionality can be utilized under specific conditions to bypass the kiosk lock rule.
    
    ```rust
    fun create_protected_tp<T: drop>(
                registry: &Registry, 
                ctx: &mut TxContext) {}
    ```
...
The following is a sequence diagram presenting how the unlock flow would take place. The following flow assumes that:

- Tokenized assets A & B have already been minted by the creator of their type.
- Tokenized assets A & B are already placed and locked inside the user's Kiosk.
- Everything is executed in the same programmable transaction block (PTB).

```mermaid
sequenceDiagram
    participant Tokenized Asset A;
    participant Tokenized Asset B;
    participant Kiosk;
    participant Proxy Module;
    participant Transfer Policy Module;
    participant Tokenized Asset Module;
    
    Tokenized Asset B ->> Kiosk: Listed on user's Kiosk for 0 SUI;
    Kiosk ->>+ Tokenized Asset B: Purchased from Kiosk, also produces Transfer Request;
    Kiosk ->> Transfer Policy Module: Transfer Request resolved via the ProtectedTP with confirm_request;
    Kiosk ->> Tokenized Asset A: Borrowed from user's Kiosk;
    Tokenized Asset A ->> Tokenized Asset Module: Inputs of Join function;
    Tokenized Asset B ->>- Tokenized Asset Module:  Inputs of Join function;
    Tokenized Asset Module ->> Kiosk: Assets A & B are joined & A's borrow promise is resolved;
    Tokenized Asset Module ->> Kiosk: Asset A is returned to the user's Kiosk;
```
...
- `asset_from_kiosk_to_burn`
    ...
    This helper function is intended to facilitate the burning of tokenized assets locked in kiosk. It assists in their unlocking while ensuring a promise that the total supply will be reduced, achieved by returning a `BurnPromise`.
...
- `prove_burn`
    ...
    This function ensures that the total supply of the asset cap has indeed been reduced by the balance of the burned tokenized asset.
...
### fnft_template package

An example package utilizing the launchpad approach serves as the template package utilized whenever a new asset requires representation as a `tokenized_asset`. This package should implement two essential modules, each catering to distinct functionalities or aspects required for asset tokenization.
...
- **Modules**
    - fnft_template
        ...
        When there's a necessity to represent a new asset as a fractional asset, this module will undergo modification to `fnft_<asset name>`, with the `<asset name>` (in capitals) being the OTW of this new asset.
...
Burning on the user side requires for them to access the shared `Treasury` object. 
...
public fun burn_with_ticket<T>(
  ...
  let TokenizedAsset { id, balance, metadata: _, image_url: _} = self;
...
At this stage, you will need to update your .env file. You can see a .env.template for reference:

```
PACKAGE_ID_ASSET_TOKENIZATION = Created by publishing `asset_tokenization` package
REGISTRY = Created by publishing `asset_tokenization` package

PACKAGE_ID_FNFT_TEMPLATE = Created by publishing `fnft_template` package
ASSET_CAP_ID = Created by publishing `fnft_template` package
ASSET_METADATA_ID = Created by publishing `fnft_template` package
ASSET_PUBLISHER = Created by publishing `fnft_template` package

PROTECTED_TP = Created by calling `setup_tp` function
TRANSFER_POLICY = Created by calling `setup_tp` function

OWNER_MNEMONIC_PHRASE = your mnemonic
...
<TabItem label="fnft_template" value="fnft_template">

In a terminal or console at the `move/fnft_template` directory of the project enter:
...
- **Select Kiosk**
    ...
    You must place the tokenized assets within a kiosk. Subsequently, you can list and sell them to other users. It's imperative to lock the objects in the kiosk to prevent any future unauthorized usage outside the defined policy that you set.
...
- **Join**
    ...
    When you execute the command `npm run call join`, two specified tokenized assets of the FT type are merged together.
    
- **Burn**
    
    When you intend to burn a tokenized asset, execute the command `npm run call burn`. Following this action, the specified asset is destroyed.
