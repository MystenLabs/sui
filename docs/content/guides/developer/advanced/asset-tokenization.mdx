...
- `created_protected_tp`
    ...
    Invoked within `setup_tp()`, this method is an internal function responsible for generating an empty transfer policy and sharing thge `ProtectedTP<T>` object. This functionality can be utilized under specific conditions to bypass the kiosk lock rule.
