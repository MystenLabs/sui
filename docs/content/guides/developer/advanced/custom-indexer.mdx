...
If you're unsure whether your system has the necessary softare properly installed, you can verify installation with the following commands.
...
This guide demonstrates how to create an indexer that:
...
First, open your terminal or console to the directory you want to store your indexer project. Use the `cargo new` command to create your Rust project and then navigate your terminal or console to its directory.
...
Replace your `Cargo.toml` with the following configuration and save. You can use an editor of your choice, but the instructions use Visual Studio Code (VS Code) for demonstration. Replace those instructions as needed for your environment.
...
- `diesel/diesel-async`: Type-safe database ORM with async support.
...
If successful your terminal or console should display a message similar to the following:
...
Before you start coding, you need to set up a local PostgreSQL database (created in the previous step) for the indexer to store the extracted transaction data.

The following steps have you:
...
<SubStep title="Configure Diesel">
...
<SubStep title="Create database table using Diesel migrations">
...
Use the `diesel setup` command to create the necessary directory structure for the next command, passing your database URL with the `--database-url` argument.
...
```sh
code ./migrations/YYYY-MM-DD-HHMMSS_transaction_digests/up.sql
```
...
This example uses the `TEXT` data type for `tx_digest`, but the best practice for a production indexer is to use `BYTEA` data type.
...
Save `up.sql`, then open `down.sql` to edit (using the actual folder name).

```sh
code ./migrations/YYYY-MM-DD-HHMMSS_transaction_digests/down.sql
```
...
<SubStep title="Apply migration and generate Rust schema">
...
<Step title="Create data structure">
...
<Step title="Define the Handler struct in handler.rs">
...
and define a concrete struct to implement the `Processor` and `Handler` traits:
...
<Step title="Implement the Processor">
...
- **type Value**: Defines what data flows through the pipeline - ensures type safety.
...
The `Handler` trait defines how to commit data to the database. Add the dependencies and code after `Processor`.
...
<Step title="Create main function">

Now to tie everything together in the main function. Open your `main.rs` file:

```sh
code ./src/main.rs
```

Replace the default code with the following and save the file:
...
Check how many transaction digests were indexed:
...
🎉 You've built a working custom indexer! The key concepts covered here apply to any custom indexer: define your data structure, implement the `Processor` and `Handler` traits, and let the framework handle the infrastructure.
