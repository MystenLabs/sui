...
Building a custom indexer on Sui lets you take full control of data ingestion, storage, and processing. You can choose from multiple [checkpoint data sources](#checkpoint-data-sources)—remote store, local files, or direct RPC API access—depending on whether you're indexing Mainnet data, testing against known checkpoints, or working on a local network. 
...
The `sui-indexer-alt-framework` supports three data sources for accessing Sui blockchain data. For all data sources, indexing framework is using a polling technique to check for new checkpoints, as explained in the "What is a Custom Indexer?" section. Data sources are configured through command-line arguments.

### Remote store

The most straightforward stream source is to subscribe to a remote store of checkpoint contents. Sui provides the following endpoints:
...
For a complete reference, study the `sui-pg-db` implementation on `Store`:

<ImportContent source="crates/sui-pg-db/src/store.rs" mode="code" impl="Store" />
...
<RelatedLink to="/guides/developer/advanced/custom-indexer/indexer-runtime-perf.mdx" />
