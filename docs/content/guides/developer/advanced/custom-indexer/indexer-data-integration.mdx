...
The `sui-indexer-alt-framework` supports multiple data sources for accessing Sui blockchain data. Data sources are configured through command-line arguments and fall into two categories: **polling-based** and **push-based**.

### Polling-based data sources

Polling-based sources periodically check for new checkpoints using the [indexing framework](/concepts/data-access/custom-indexing-framework.mdx) polling mechanism.
...
Push-based sources provide real-time checkpoint data as it becomes available, offering lower latency than polling.

#### gRPC streaming

gRPC streaming provides real-time checkpoint data pushed from fullnodes for the latest checkpoints. Because streaming only provides the latest data, it requires a polling-based fallback source (remote store, local path, or RPC API) for historical checkpoints and reliability:
...
- Production indexers requiring minimal latency
- Real-time data processing pipelines
- Applications that need immediate checkpoint updates

**Important notes:**

- Always provide a polling-based fallback source (like `--remote-store-url`) for reliability and historical data
- The streaming connection automatically falls back to the polling source if streaming is unavailable or for historical checkpoints
