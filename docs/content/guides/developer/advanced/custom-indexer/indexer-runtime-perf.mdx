...
The indexing framework provides multiple levels of configuration to optimize performance for different use cases. This section covers basic configuration options, while complex pipeline-specific tuning is covered in dedicated sections within each pipeline architecture.
...
Tokio Console is particularly useful for:
...
1. Launch the console dashboard
    ...
    You should then observe this cool dashboard with information about every running Tokio tasks:
...
`Tokio-console` introduces runtime overhead and should be used carefully in production. While it's safe to use regularly during development and staging, production usage requires careful evaluation. Before enabling in production, you should run performance benchmarks with and without console enabled to measure the impact on your specific workload, and consider enabling only during maintenance windows or targeted troubleshooting sessions while monitoring system resources.
...
[https://github.com/MystenLabs/sui/blob/becbf213a55228879a8dededc21c5a0fb27194be/crates/sui-indexer-alt-framework/src/metrics.rs#L46](https://github.com/MystenLabs/sui/blob/becbf213a55228879a8dededc21c5a0fb27194be/crates/sui-indexer-alt-framework/src/metrics.rs#L46)
...
This prefixes all default metrics (e.g., my_custom_indexer_indexer_total_ingested_checkpoints).
...
[Collect Prometheus metrics |  Grafana Alloy documentation](https://grafana.com/docs/alloy/latest/collect/prometheus-metrics/)
...
Partition size is the key tradeoff: you need `N` full partitions plus one trailing _active_ partition that cannot be dropped until it falls outside the read window. Larger partitions mean fewer total partitions to scan, reducing read amplification. Smaller partitions reduce retention overhead but increase the number of partitions queried. For example, keeping 10M checkpoints could use one 10M partition plus one active partition (querying at most 2 partitions) or ten 1M partitions plus one active partition (querying 11 partitions). In practice, per-epoch partitioning tends to produce too many small partitions, and not partitioning at all leads to performance degradation from repeated `DELETE`s creating dead tuples and fragmentation.
