...
When you delete metadata on chain, the `walrus_blob` table records this transaction by updating the existing entry’s `cp_sequence_number` value and setting `deleted = TRUE`. Conversly, the `walrus_blob_historical` table writes a new record with most of the fields set to `NULL`. These records serve as tombstone rows that the pruner deletes.
