...
To provide users with a list of posts written by a publisher, your service would first query the
database on `publisher`, yielding a result like the below, then use the `blob_obj_id` to fetch the
`Blob` NFT contents. From there, the actual Walrus content can be retrieved.
