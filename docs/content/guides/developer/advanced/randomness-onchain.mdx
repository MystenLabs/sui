---
title: Access On-Chain Randomness
...
See <a href="/references/framework/sui-framework/random" data-noBrokenLinkCheck='true'>random.move</a> for the Move APIs for accessing randomness on Sui.
...
Move compiler enforces this behavior by rejecting `public` functions with `Random` as an argument.
...
The Move compiler prevents defining `public` functions with `RandomGenerator` as an argument.
