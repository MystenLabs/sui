...
Developers must be aware that some resources that are available to transactions are limited.
If a function that reads `Random` consumes more resources in an unhappy path flow than in a happy path flow, an attacker can use that difference to revert the transaction in the unhappy flow.
Concretely, gas is such a resource. Consider the following code:
...
Sui does not prevent this kind of attack, and developers must be aware of this subtlety when designing their contracts.

:::

In many cases this attack is not an issue, like when selecting a raffle winner, or lottery numbers, as the code running is independent of the randomness.
...
The first function, called by transaction `tx1`, fetches a random value and stores it in an object that is unreadable by other commands in `tx1` (e.g., by transferring the object to the caller, or, by storing the tx digest and checking it is different on read).
...
Also, it is important to gracefully handle the case in which the second step is never completed (e.g., charge a fee in the first step).
...
Other limited resources per transaction that developers should consider are:
...
Consider for example a betting game that uses randomness for rolling a dice:
