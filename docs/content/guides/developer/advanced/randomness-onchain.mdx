...
keywords: [ raffle, randomness, random, ]
mdx:
  format: md
---
...
For many use cases this attack is not an issue, like when selecting a raffle winner, or lottery numbers, as the code running is independent of the randomness. However, in the cases where this attack can be problematic, consider using one of the following approaches:

### Option 1: Divide logic

Split the logic into two functions that different transactions must call.

The `tx1` transaction calls the first function, which fetches a random value and stores it in an object that is unreadable by other commands in `tx1`. For example, by transferring the object to the caller or by storing the transaction digest and checking that it's different on read.

The `tx2` transaction calls a second function, which reads the stored value and completes the operation.
`tx2` might fail, but now the random value is fixed and cannot be modified using repeated calls.
...
It's also important to gracefully handle the case in which the second step is never completed. You could accomplish this, for example, by charging a fee in the first step.

<details>
<summary>
Example implementation.
</summary>
{@inject: examples/move/random/random_nft/sources/example.move#reveal_alternative2_step1,reveal_alternative2_step2}
</details>
...
Write the function so the main processing path does the heavy work, while early-exit paths return quickly. Keep the following in mind:
  ...
  - Use [profile-transaction](../../../references/cli/client.mdx#profile-a-transaction) to profile the costs of a transaction.
...
- [Sui Framework Reference](../../../references/framework): The Sui framework libraries include Move modules that provide the logic for Sui and its standards. A Rust process creates the documentation for the modules directly from comments in the code.
  - <UnsafeLink href="/references/framework/sui-framework/random">random.move</UnsafeLink>: Documentation for the `random` module.
- [Raffle example](https://github.com/MystenLabs/sui/blob/main/examples/move/random/raffles): GitHub repo containing a raffle smart contract demonstrating randomness.
- [Sui Client CLI](/references/cli/client): Sui provides command line tools to interact with the network, its features, and the Move programming language. Individual command groups are referred to as Sui Client CLI, Sui Keytool CLI, Sui Move CLI, and Sui Validator CLI.
