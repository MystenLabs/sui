...
Although `Random` is a shared object, it is inaccessible for mutable operations, and any transaction attempting to modify it will be unsuccessful.
...
See [random_nft.move](https://github.com/MystenLabs/sui/blob/main/sui_programmability/examples/nfts/sources/random_nft.move) for examples.
...
If one wants to call `roll_dice(r: &Random, ctx: &mut TxContext)` in module `example`, the following code snippet can be used:
```typescript
...
- [Randomness example](https://github.com/MystenLabs/sui/blob/main/sui_programmability/examples/nfts/sources/random_nft.move)
- [Raffle example](https://github.com/MystenLabs/sui/blob/main/sui_programmability/examples/games/sources/raffles.move)
