---
title: Coin Flip
description: Learn Sui through a coin flip app that covers the full end-to-end flow of building a Sui Move module and connecting it to a React Sui app.
...
To use a [programmable transaction block](/guides/developer/transactions/prog-txn-blocks.mdx) (PTB) in Sui, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
Now sign and execute the transaction block. dApp Kit provides the `signAndExecuteTransaction` method on the dAppKit instance (accessed via `useDAppKit()`) to streamline this process. When called, it prompts the UI for you to approve, sign, and execute the transaction block. The `HouseData` object is important as you use it as input for later Move calls, so save its ID somewhere.
...
Event subscriptions via WebSocket (`subscribeEvent`) require configuring dApp Kit with `SuiJsonRpcClient`. Alternatively, for SDK 2.0 with the gRPC client, you can poll for events using GraphQL. See [Using Events](/guides/developer/accessing-data/using-events.mdx) for more information.

:::

All of this logic is in `HouseFinishGame.tsx` (using JSON-RPC subscription):
...
export function HouseFinishGame() {
  ...
  }, [housePrivHex, houseDataId, suiClient, dAppKit]);

  return null;
}
```

To get the underlying client instance from the SDK, use `useCurrentClient()`. You want to subscribe to events whenever the `HouseFinishGame` component loads. To do this, use the React hook `useEffect()` from the core React library.

The client exposes a method called `subscribeEvent()` that enables you to subscribe to a variety of event types. This is actually a thin wrapper around the RPC method [`suix_subscribeEvent`](/sui-api-ref#suix_subscribeevent).
