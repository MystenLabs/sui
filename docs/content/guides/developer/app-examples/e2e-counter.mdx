...
[Example source code](https://github.com/MystenLabs/sui/blob/c617f8dced6c61b7c2070a51999bc202d43f7375/examples/distributed-counter)
...
{@inject: examples/distributed-counter/counter/sources/counter.move#struct=Counter noComments}

- The `Counter` type stores the address of its `owner`, its current `value`, and its own `id`.

### Creating `Counter`

{@inject: examples/distributed-counter/counter/sources/counter.move#fun=create noComments}
...
{@inject: examples/distributed-counter/counter/sources/counter.move#fun=increment noComments}
The `increment` function accepts a mutable reference to any shared `Counter` object and will increments its `value` field.

{@inject: examples/distributed-counter/counter/sources/counter.move#fun=set_value noComments}
...
{@inject: examples/distributed-counter/counter/sources/counter.move noComments}
...
To skip building the frontend and test out your newly deployed package, create this example using the following template and [connect your deployed package](#connecting-your-package): 
...
{@inject: examples/distributed-counter/counter-app/src/constants.ts noComments}
...
{@inject: examples/distributed-counter/counter-app/src/App.tsx noComments}
