...
This app will use the `lucide-react` package for icons and `tailwindcss` for styling. Install them by running the following command:

<Tabs groupId="packagemanager">
  <TabItem label="PNPM" value="pnpm">
    ```bash
    pnpm add lucide-react tailwindcss@latest postcss@latest autoprefixer@latest
    ```
  </TabItem>
  <TabItem label="Yarn" value="yarn">
    ```bash
    yarn add lucide-react tailwindcss@latest postcss@latest autoprefixer@latest
    ```
  </TabItem>
</Tabs>
...
<details>
  ...
  {@inject: sdk/create-dapp/templates/react-e2e-counter/tailwind.config.js}
</details>
...
<details>
  ...
  {@inject: sdk/create-dapp/templates/react-e2e-counter/src/styles/base.css}
</details>

<details>
  <summary>
    Import the `base.css` file in your `src/main.tsx` file:
  </summary>
  {@inject: sdk/create-dapp/templates/react-e2e-counter/src/main.tsx}
</details>
...
<details>
  ...
  ```tsx
    ...
    import { LoaderCircle } from '@radix-ui/react-loader';
...
The `create` function now creates a new Sui `Transaction` and calls the `create` function from your Move module. The Programmable Transaction Block (PTB) is then signed and executed via the `useSignAndExecuteTransaction` hook. The `onCreated` callback is called with the new counter's ID when the transaction is successful.
...
Currently, the `Counter` component doesn't exist, so the app will just display an empty page if you navigate to a counter ID.
