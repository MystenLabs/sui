  ...
  ```tsx
    ...
    export function Counter({ id }: { id: string }) {
      ...
      const executeMoveCall = (method: "increment" | "reset") => {
        // TODO - will be implemented below
      };
      ...
      const ownedByCurrentAccount =
        data.owner.$kind === "AddressOwner" && data.owner.AddressOwner === currentAccount?.address;
      ...
      return (
        <>
          ...
          <Flex direction="column" gap="2">
            <Text>Count: (see full implementation for BCS parsing)</Text>
...
Note that the gRPC API returns object content as BCS-encoded bytes. The full template (shown below) includes BCS parsing to extract the counter's value and owner fields.

The code also adds an `executeMoveCall` function that still needs implementing. The full implementation uses `useMutation` from React Query to handle the transaction execution and state management.
