...
[trading-api-repo-link]: https://github.com/MystenLabs/sui/examples/trading/api
[sui-ts-sdk-link]: https://sdk.mystenlabs.com/typescript
[express-link]: https://expressjs.com/

This is a continuation of the Escrow smart contract guide to demonstrate how to implement a service responsible for indexing chain state in order to discover trades.

In most cases where we want to enhance our dApp and ensure it is production ready, we need to have an indexing service (indexer) listening to the blockchain for on-chain data, shaping the data to fit our application needs, storing the transformed data into the local off-chain database so we can query them in the most efficient way. Joining hands with an indexer, we will expose an API allowing frontend to query the indexed data and update the screen as escrows are made and swaps are fulfilled.

Architecturally, we will have the service doing the heavy-lifting indexing the data while the other service will expose the data through an API convention for external consumption.

## Prerequisites

:::info

The full source code for the indexing service available at the [Trading API example directory]([trading-api-repo-link]).

:::

Before getting started, make sure you:
- Understand [the mechanism behind the Escrow smart contract backend](/guides/developers/app-examples/escrow).
- Check out the [indexer's README]([trading-api-repo-link]) to setup the development environment.
- Check out [Prisma]([prisma-link]) to have an overall sense on what it is as we will use it to facilitate all the database interactions.
- Check out [Express]([express-link]) to learn how to quickly setup a Web server application in Node.js as we will use it to bootstrap our API service
- Check out [Sui Typescript SDK]([sui-ts-sdk-link]) for its usage.

:::note

For technology that is fairly new you, don't worry, we will walk you through the basics of these along the way.
...
In most cases, when we're working with a database directly from a backend service, we may want to use some sort of database libraries and toolings to abstract us away from database creation and management complexity. In our case, we use [Prisma]([prisma-link]) to manage all the database interactions such as defining data models and database migrations.

First of all, we want to design what data to index
...
These data models represents the `Locked` and `Escrow` object. Compared to their on-chain version which contains much less attributes due to initial smart contract design, they have additional fields providing extra information that will help us to facilitate any queries at later stage.
...
Most indexing service will need to implement some sort of checkpoint mechanism to ensure it will pick up the progress where it left off even when it is crashed. `Cursor` is our checkpoint, and we ensure it long-lives and unaffected by any incident by storing it in our persistent database.

Next, let's explore the logic keeping the service listens to our blockchain signals and reacts accordingly.

### `event-indexer.ts`

Let us first examine `event-indexer.ts`
...
As we mentioned earlier, we should expose the indexed data for external consumption through an API Service. Particularly, we will use [Express]([express-link]) to build a Node.js HTTP API.

## API Design

### WHERE Parameters

We want our API to accept the query string in the URL as the parameters for database WHERE query. Hence, we want utility that can extract and parse the URL query string into valid query parameters for Prisma. With the `parseWhereStatement()` function, we allow the callers to filter the set of keys from URL query string and transform those corresponding key-value pairs into the correct format for Prisma.
...
Pagination is another crucial part to ensure our API will return sufficient or/and ordered chunk of information instead all the data which might be the vector for DDOS attack. Similar to **WHERE parameters**, we define a set of keys in the URL query string to be accepted as valid pagination parameters. `parsePaginationForQuery()` utility function helps us to achieve this by filtering the pre-determined keys `sort`, `limit`, `cursor` and parsing corresponding key-value pairs into `ApiPagination` which can be consumed by Prisma.

In this demo, we use the `id` field of the model in the database as the cursor to allow clients to continue subsequent queries with the next page.
...
```
### API Endpoints
All the endpoints are defined in `server.ts`, particularly, we have 2 endpoints:
- `/locked` to query `Locked` objects.
- `/escrows` to query `Escrow` objects.

The implementation for both endpoints is pretty straightforward. We will define a list of valid query keys, such as `deleted`, `creator`, `keyId`, `objectId` for `Locked` data and `cancelled`, `swapped`, `recipient`, `sender` for `Escrow` data, and pass the URL query string into our pre-defined utilities which will output the correct parameters that we can use with Prisma.
