...
As we mentioned earlier, we should expose the indexed data for external consumption through an API Service. Particulary, we will use [Express]([express-link]) to build a Node.js HTTP API.
...
The implementation for both endpoints is pretty straightforward. We will define a list of valid query keys, such as `deleted`, `creator`, `keyId`, `objectId` for `Locked` data and `cancelled`, `swapped`, `recipient`, `sender` for `Escrow` data, and pass the URL query string into our pre-defined utilties which will output the correct parameters that we can use with Prisma.
