...
The full source code for the indexing service available at the [Trading API example directory](https://github.com/MystenLabs/sui/examples/trading/api).
...
- Check out [Prisma](https://www.prisma.io/) to have an overall sense on what it is as we will use it to facilitate all the database interactions.
- Check out [Express](https://expressjs.com/) to learn how to quickly setup a Web server application in Node.js as we will use it to bootstrap our API service
- Check out [Sui Typescript SDK](https://sdk.mystenlabs.com/typescript) for its usage.
...
In most cases, when we're working with a database directly from a backend service, we may want to use some sort of database libraries and toolings to abstract us away from database creation and management complexity. In our case, we use [Prisma](https://www.prisma.io/) to manage all the database interactions such as defining data models and database migrations.
...
As we mentioned earlier, we should expose the indexed data for external consumption through an API Service. Particularly, we will use [Express](https://expressjs.com/) to build a Node.js HTTP API.
