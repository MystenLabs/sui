...
This guide shows you how to write and deploy a smart contract in Move that verifies the users are human and not bots (using CAPTCHA) before they interact with the contract.
...
As with all SUI dApps, a Move package on chain powers the logic of reCAPTCHA module. The following instruction walks you through creating and publishing the module.
...
1. The forth line declares the module name as `recaptcha` within the package `recaptcha`.
1. The next eight lines use the use keyword to import types and functions from other modules, such as `sui::clock::Clock`, `sui::dynamic_field as df`, `sui::ed25519::ed25519_verify`, `sui::event::emit,sui::math`, `sui::object::{Self, UID}`, `sui::transfer`, and `sui::tx_context::{sender, TxContext}`.
These moduless are needed for the implementation of the reCAPTCHA verification and the interaction logic.
...
The `verify` function is a public function that allows anyone to register themselves as non-bot by calling. The function takes five parameters:
...
Now that you have implemented the `verify`, you can move on to the next step, which is to demonstrate how someone can interact with the contract, let’s write an `interact` function that checks whether the user is verified or not.
...
And with that, your `recaptcha.move` code is complete. We hope you enjoyed this guide and learned how to create a smart contract in Move that verifies the users are human and not bots.
...
The package should successfully deploy. Next, we need to set up a backend server that verifies whether the user has successfully completed the reCAPTCHA challenge and then signs a message that should be passed to the `verify` function.

## Backend

To implement the backend for the reCAPTCHA, you will need to create an `express` app that can handle HTTP requests and responses. You will also need to install some dependencies, such as `@noble/ed25519`, `axios`, `cors`, `helmet`, `morgan`, and `dotenv`. These packages will help you with cryptography, HTTP requests, cross-origin resource sharing, security, logging, and environment variables.
...
3. Create a file named `app.ts` and paste the code you have provided.
...
4. Let's go through the code and see what it does.

- First, we import the modules that we need for our app.
- Next, we create an express app and use some middlewares to enhance its functionality. We use `morgan` for logging, `helmet` for security, `cors` for cross-origin resource sharing, and `express.json` for parsing JSON data.
- Then, we define a `GET` route for the root path (`/`) that returns a simple JSON message.
- After that, we define an interface for the reCAPTCHA API response. This is the data that we will receive from Google when we verify the user's response token. It contains some fields such as `success`, `challenge_ts`, `hostname`, and `error-codes`. It also has some optional fields that we will add later, such as `signature`, `pubKey`, and `message`.
- Next, we define a `POST` route for the `/verify-token` path that handles the verification of the user's response token. This is the main logic of our backend. Here are the steps that we follow in this route:
  - We get the current time in milliseconds and store it in a variable named `now`.
  - We get the secret key from the environment variable `SK` and store it in a variable named `privKey`. This is the key that we will use to sign our message and verify our identity to the smart contract.
  - We use the @noble/ed25519 module to get the public key from the private key and store it in a variable named `pubKey`. This is the key that we will share with the smart contract and the user.
  - We get the response token, the secret key, and the user's address from the request body and store them in variables named `response`, `secret`, and `userAddress`.
  - We log the values of these variables for debugging purposes.
  - We create a message that consists of the user's address (without the 0x prefix) and the current time, and convert it to a hexadecimal string. We store it in a variable named `message`.
  - We use the @noble/ed25519 module to sign the message with the private key and store the signature in a variable named `signature`.
  - We use the @noble/ed25519 module to verify the signature with the message and the public key and store the result in a variable named `isValid`.
  - We log the values of these variables for debugging purposes.
  - We use the axios module to send a POST request to the reCAPTCHA API with the secret key and the response token as parameters. We store the response in a variable named `axiosResponse`.
  - We check if the response data has a success field and if it is true. If so, we return a JSON object with the following fields:
    - success: true
  ...
  - If not, we catch the error and return a JSON object with the following field:
    - success: false
- Next, we define a helper function named `stringToHex` that takes a string as an input and returns a hexadecimal string as an output. This function is used to convert the message to a hexadecimal format.
- Finally, we use some custom middlewares to handle not found and error cases, and export the app as a default module.
...
To implement the frontend for the reCAPTCHA, you will need to create a react app that can render a user interface and interact with the backend and the smart contract. You will also need to install some dependencies, such as `@mysten/wallet-kit`, `@mysten/sui.js`, `axios`, and `react-google-recaptcha`. These packages will help you with wallet integration, transaction execution, HTTP requests, and reCAPTCHA rendering.
...
5. Let's go through the code and see what it does.

- First, we import the modules that we need for our app.
- Next, we use the useWalletKit hook from `@mysten/wallet-kit` to get access to the current wallet, the current account, and the `signAndExecuteTransactionBlock` function. These will help us to connect to the wallet and execute transactions on the blockchain.
- Then, we define some constants for the site key, the secret key, the package ID, the registry ID, and the module ID. These are the values that we will use to interact with the reCAPTCHA API and the smart contract.
- After that, we define some state variables to store the status of the reCAPTCHA validation, the verification result, and the message, the public key, and the signature that we will get from the backend. We use the useState hook from react to manage these state variables.
- Next, we define a function named onChange that takes a token as an input and handles the change of the reCAPTCHA component. This function is triggered when the user completes the reCAPTCHA challenge. Here are the steps that we follow in this function:
  - We check if the token is null. If so, we set the reCAPTCHA validation state to false.
  - If not, we call the verifyToken function with the token as an argument and store the result in a variable named recaptchaApiResponse. This function will send a POST request to the backend and get the verification result and the data that we need to interact with the smart contract.
  - We set the reCAPTCHA validation state to true.
  - We check if the verificationPassedOneTime state is false. If so, we set it to true. This state is used to enable the interact button only once after the user passes the verification.
  - We check if the recaptchaApiResponse has the message, the pubKey, and the signature fields. If so, we set the corresponding state variables with the values from the response.
- Next, we define a function named verifyToken that takes a token as an input and returns a promise of the reCAPTCHA API response. This function is used to communicate with the backend. Here are the steps that we follow in this function:
  - We try to send a POST request to the backend URL with the token, the secret key, and the user's address as the body parameters. We use the axios module to send the request and store the response in a variable named response.
  - We return the data field of the response as the reCAPTCHA API response.
  - We catch any error and log it to the console.
  - We return an empty object as the default reCAPTCHA API response.
- Next, we use the useEffect hook from react to run some code when the currentWallet state changes. In this case, we don't do anything, but we could add some logic here if we want to.
- Finally, we return a JSX element that renders the app. The app consists of the following components:
  - A ConnectButton component from `@mysten/wallet-kit` that allows the user to connect to their wallet.
  - A button that allows the user to interact with the smart contract. This button is disabled unless the user passes the verification at least once. When the user clicks this button, we create a new TransactionBlock object from `@mysten/sui.js` and add a moveCall action that calls the interact function of the smart contract with the registry ID and the clock object ID as arguments. Then, we use the signAndExecuteTransactionBlock function from `@mysten/wallet-kit` to sign and execute the transaction block on the blockchain. We also log the result to the console.
  - A button that allows the user to verify their identity to the smart contract. This button is disabled unless the user passes the reCAPTCHA challenge. When the user clicks this button, we create a new TransactionBlock object from `@mysten/sui.js` and add a moveCall action that calls the verify function of the smart contract with the registry ID, the signature, the public key, and the message as arguments. Then, we use the signAndExecuteTransactionBlock function from `@mysten/wallet-kit` to sign and execute the transaction block on the blockchain. We also log the result to the console.
  - A ReCAPTCHA component from `react-google-recaptcha` that renders the reCAPTCHA widget. We pass the site key and the onChange function as props to this component.

That's it! You have implemented the frontend for the reCAPTCHA. To run the app, you can use `pnpm run dev`. To test the app, you can open the browser and go to the localhost:5173 URL. You should see the app and be able to interact with the reCAPTCHA and the smart contract.