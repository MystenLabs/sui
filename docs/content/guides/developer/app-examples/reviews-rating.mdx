...
Moderators monitor content of the reviews and can delete any reviews that contain inappropriate content. Moderators also keep an eye on the number of reviews registered for a service to ensure that they do not grow indefinitely.
...
All the submitted reviews are stored in `reviews` field, and top rated reviews are stored in `top_reviews` field, which has `MultiMap<ID>` type. The [multimap module](#multimap) defines the type.
