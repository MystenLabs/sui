...
All the submitted reviews are stored in ***reviews*** field which has ***MultiMap<ID\>*** type. MultiMap is our implementation of Multimap data structure (https://en.wikipedia.org/wiki/Multimap) which keeps all the reviews sorted by their. Using MultiMap, all the reviews are ordered such that the high rated reviews come first.
...
## Backend
No backend service is required to run this platform. All data is stored onchain and reviews are rated onchain!
