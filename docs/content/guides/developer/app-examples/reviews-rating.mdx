...
Unlike traditional review rating platforms, that often do not disclose the algorithm used to rate reviews, the on-chain platform uses an algorithm that is published on-chain for everyone to see and verify. All submitted reviews are scored and ordered on-chain, which the low gas cost of computation on Sui makes financially feasible.

## Personas

There are four actors in the typical workflow of the Reviews Rating example.  

- Service: Review requestor.
...
In other words, anyone can go to a transaction explorer and find a review object by its `object_id` but they won't be able to use a review as an input to a transaction by its `object_id`.
