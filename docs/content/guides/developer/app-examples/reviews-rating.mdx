...
Moderators also keep an eye on the number of reviews registered for a service to ensure that they do not exceed the maximum number allowed.
When the maximum number of reviews is reached, moderators can prune old reviews to make room for a new review.
...
```
`SERVICE OWNERS` are given `AdminCap`, and `MODERATORS` are given `Moderator` to allow them to perform privileged operations.
...
Note that `vector` is used to store the contents of the `MultiMap`. This is acceptable in our use case because the maximum number of reviews for a service is 1000.
As reviews are sorted by `total_score` on-chain, it is necessary to keep a manageable number of reviews for a service.
...
```
`Binary Search` is implemented to insert a new entry in the `MultiMap`. The `insert` function is used to insert a new entry in the `MultiMap` along with `insert_recursive` helper function.
