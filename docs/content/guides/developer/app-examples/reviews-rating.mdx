...
```
Whenever a user casts a vote on a review, the `total_score` of the review is updated, and the `top_reviews` field can be updated by calling the `update_top_reviews` function.
...
After `total_score` is updated, the [`update_top_reviews`](#servicemove) function can be called to update the `top_reviews` field of the `Service` object.
