...
Use the `useDAppKit()` hook from the dApp Kit to access the Sui client and wallet functionality configured in `DAppKitProvider`. The `signAndExecuteTransaction()` function signs the transaction block using the currently connected wallet and executes it in a single call. It displays the UI for users to review and sign their transactions with their selected wallet. Use `react-hot-toast` as another dependency to toast transaction status to users.
