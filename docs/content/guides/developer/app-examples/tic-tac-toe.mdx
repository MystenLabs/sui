...
This guide covers two different implementations of the game tic-tac-toe on Sui. The first example utilizes a shared object that both users can mutate. The second example utilizes a owned object that is controlled by a 1-of-2 [multisig](https://docs.sui.io/guides/developer/cryptography/multisig) account generated from the two players' public keys.

Shared objects don't required a third party for management of the game board and only require one transaction per move. However, these transactions require consensus since the game board is not owned by a single account. Owned objects require a third party to manage the game board and require two transactions per move. However, these transactions don't require consensus and utilize the fast path for transactions. 

The guide is divided into two parts that each cover a different implementation of the tic-tac-toe game board:

1. [Shared game board](#shared): A shared object that allows players to directly update the game board.
...
- **Fast Path Consensus:** The guides introduces [Fast Path Consensus](https://move-book.com/object/fast-path-and-consensus.html), a feature of Sui that allows transactions to bypass extensive consensus. In this case, the owned object version of the game board uses Fast Path Consensus to allow the game admin to update the game board quicker. 
- **Dynamic object fields:** The guide teaches you how to use [dynamic object fields](https://move-book.com/programmability/dynamic-object-fields.html), in this case to transfer the actions of the players to the game board, which will be retrieved by the game admin. 
...
- `scripts` to hold the scripts that publish packages and update configs used while building the front-end and CLI.
...
<details>
  <summary>
    Add `publish.sh` to `tic-tac-toe/scripts/`
  </summary>
  {@inject: examples/tic-tac-toe/scripts/publish.sh}
</details>
...
- You have created a `publish.sh` file in the `tic-tac-toe/scripts/` directory.

:::

## Frontend setup

This guide uses a single React frontend to interact with both the shared and multisig versions of the game. The frontend is built on top of `create-react-dapp`, using the TS SDK and `dapp-kit`. 



## Shared game board {#shared}

### Contract

In this version, the game object is a shared object, allowing both players to access and modify it directly, enabling them to place markers in just one transaction. However, using a shared object generally incurs extra costs because Sui needs to sequence the operations from different transactions. In the context of this game, where players are expected to take turns, this shouldn't significantly impact performance. Overall, this shared object approach simplifies the implementation compared to the multisig version.

Create a new file in `tic-tac-toe/move/sources` titled `shared.move`.

In this first version of tic-tac-toe, the `Game` object, including the game board, is shared between the two players.
...
Once a `Game` object is shared, players can place their markers on the board using the `place_mark` function:
...
## Multisig operated game board {#multisig}

### Contract

Create a new file in `tic-tac-toe/move/sources` titled `owned.move`.

In this second version of tic-tac-toe, the `Game` object, including the game board, is controlled by a single account. 

{@inject: examples/tic-tac-toe/move/sources/owned.move#struct=Game noComments}

In this case, the game board is owned by a 1-of-2 multisig account generated from the two players' public keys. The `admin` field in the `Game` object stores the public key of the multisig account that controls the game board.
...
When a mark is sent by a player (`send_mark`), the admin (the same player via the multisig) places the marker on the board (`place_mark`). Each move requires two steps (thus two transactions): one from the player and one from the admin.
