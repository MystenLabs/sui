...
This pattern requires configuring dApp Kit with `SuiJsonRpcClient` (see `config.ts` in the example) to use the `executeTransactionBlock` method.
...
function OwnedGame({
  ...
  // Cast to SuiJsonRpcClient since the app is configured to use JSON-RPC client
  ...
  const { mutate: multiSignAndExecute } = useExecutor({
    execute: ({ bytes, signature }) => {
      ...
      return client.executeTransactionBlock({
        ...
        // gas object, because the player is sponsoring the transaction.
