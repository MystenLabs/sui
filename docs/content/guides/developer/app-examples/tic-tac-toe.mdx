...
On this page, we will cover three different implementations of the game tic tac toe on Sui. The first example will utilize a centralized admin that marks the board on the users’ behalf. The second example will utilize a shared object that both users can mutate. And the third example will utilize a multisig, where instead of sharing the game board, it will be in a 1-of-2 multisig of both users’ accounts. This guide will compare and contrast the design philosophies behind the three different games, as well as the pros and cons of each.
...
```
The main logic of the game is in the `create_game` function below.
...
- Both players get 5 `MarkMintCap`s each, giving them the capability to place a maximum of 5 marks each
...
To view the entire source code, see [here](https://github.com/MystenLabs/sui/blob/main/sui_programmability/examples/games/sources/tic_tac_toe.move). The rest of the logic, including how to check for a winner, as well as deleting the gameboard once the game has concluded, can be found here.

An alternative version of this game, shared tic tac toe, is mentioned below, which uses shared objects for a simpler implementation and without a centralized service. This comes at a slightly increased cost, as using shared objects is more expensive than transactions involving wholly owned objects.

## shared_tic_tac_toe.move

In the previous version, the admin owned the game object, preventing players from directly changing the gameboard, as well as requiring two transactions for each marker placement. In this version, the game object is a shared object, allowing both players to access and modify it directly, enabling them to place markers in just one transaction. However, using a shared object generally incurs extra costs since Sui needs to sequence the operations from different transactions. In the context of this game, where players are expected to take turns, this shouldn't significantly impact performance. Overall, this shared object approach simplifies the implementation compared to the previous method.

As you can see below, the `TicTacToe` object in this example is almost identical to one before it, the only difference being that the `gameboard` is represented as `vector<vector<u8>>` instead of `vector<vector<Option<Mark>>>`. We’ll discuss why this is the case below.
...
Let’s take a look at the `create_game` function below:
...
The full source code can be found [here](https://github.com/MystenLabs/sui/blob/main/sui_programmability/examples/games/sources/shared_tic_tac_toe.move)
multisig_tic_tac_toe.move
In this implementation of the game, the game is in a 1-of-2 multisig account that acts as the game admin. In this particular case, since there are only two players, a much simpler implementation can be found in the previous example. However, this example is to illustrate that in some cases, a multisig can replace shared objects, thus allowing transactions to bypass consensus when using such an implementation.

Let us examine the two main objects in this game, `TicTacToe`, and `Mark`:
...
Let’s now take a look at `send_mark_to_game` and `place_mark`:
...
The first function is straightforward. The player sends the location of the mark to the multisig’s account. Then in the next function, the multisig actually places down the mark the player requested, as well as all the logic to check to see if there is a winner, end the game and award a player a trophy if so, or to advance to the next player’s turn if not. See the full repo for this game [here](https://github.com/MystenLabs/multisig_tic-tac-toe).
