...
Create a new file in `tic-tac-toe/move/sources` titled `owned.move`. This file will contain the Move code for the game board in the centralized (and multisig) version of tic-tac-toe.
...
- The `Game` object is created and returned by this function, it is up to its creator to send it to the game admin to own.
...
`useTransactions.ts` also contains functions to place, send, and receive marks, end the game, and burn ended games. These functions all return a `Transaction` object which is used in the React frontend to execute the transaction with the appropriate signer.
