...
Functions responsible for various aspects of the escrow's lifecycle emit these events.  so that the custom indexer can subscribe to transactions that emit these events, and only process those, rather than the whole chain state:
...
- Understand [the mechanism behind the Escrow smart contract backend](#escrow).
- Check out [indexing service guide](#backend) to learn how we index on-chain data and API endpoints exposed to serve data query requests.
