...
export function LockOwnedObjects() {
  ...
  const { data, fetchNextPage, isFetchingNextPage, hasNextPage, refetch } =
    useInfiniteQuery({
      ...
      getNextPageParam: (lastPage) =>
        lastPage.hasNextPage ? lastPage.cursor : null,
      enabled: !!account,
      select: (data) =>
  ...
  return (
    ...
    >
      {data?.map((obj) => (
        <SuiObjectDisplay key={obj.objectId} object={obj}>
        </SuiObjectDisplay>
      ))}
    </InfiniteScrollArea>
  );
...
Use the `useCurrentClient()` hook from dApp Kit to retrieve the Sui client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from dApp kit that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, use the `executeTransaction()` on the client instance of the Sui TypeScript SDK.
...
This component uses TanStack Query's `useInfiniteQuery` since the data is being fetched from the example's API rather than directly from Sui. 
