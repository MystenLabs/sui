...
This is the third in a [three-part guide](./intro.mdx) on how to build an app for trustless atomic swaps on Sui.
...
- Understand [the mechanism behind the Escrow smart contract backend](./intro.mdx).
- Check out [indexing service guide](./backend.mdx) to learn how we index on-chain data and API endpoints exposed to serve data query requests.
