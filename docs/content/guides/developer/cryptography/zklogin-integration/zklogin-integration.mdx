...
zkLogin uses the user salt to compute the zkLogin Sui address (see [definition](/guides/developer/cryptography/zklogin-integration/)). The salt must be a 16-byte value or a integer smaller than `2n**128n`. There are several options for the application to maintain the user salt:
