...
1. **OTW creation:** Use `new_currency_with_otw<T>()` with a One-Time Witness for uniqueness proof. Note: that for proper creation and RPC support it requires a second transaction to promote the currency to the registry.
