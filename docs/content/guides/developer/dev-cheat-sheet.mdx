...
- Since the addition of [programmable transaction blocks](https://docs.sui.io/concepts/transactions/prog-txn-blocks), it is now possible to call `public` functions in PTBs. As a result, `entry` functions have been made redundant, and even limiting. `public` functions can return anything, but `entry` functions can only return `T: drop`. Thus it is recommended to only use `public` functions.
