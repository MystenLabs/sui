...
For the full specification, see [SIP-58: Sui Address Balances](https://github.com/sui-foundation/sips/blob/main/sips/sip-58.md). For implementation details on sending, withdrawing, and paying gas from address balances, see [Using Address Balances](/guides/developer/digital-assets/address-balances/using-address-balances).
...
Anyone operating a wallet (custodial, non-custodial, or exchange wallets) might be impacted by this change. Users can now send funds to your address through `send_funds()` instead of transferring a coin object. If a wallet receives funds this way but cannot see or access them, the user might believe the funds have been lost. The funds cannot actually be lost, but this can cause confusion for affected users.

Initially, this does not happen often because most of the ecosystem continues to transfer coin objects. However, because funds can arrive at any wallet as an address balance transfer, wallet implementors should be prepared.
...
Previously, you computed balance changes by comparing input and output coin objects. With address balances, the algorithm adds a third step:
...
The Sui framework provides a helper that handles all three steps:
...
Contracts that accept `Coin<T>` or `Balance<T>` remain callable without changes. The `redeem_funds` functions convert withdrawals to the expected types within the PTB.
