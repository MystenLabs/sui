...
To use address balance funds in a transaction, you withdraw them and redeem the withdrawal into a `Coin<T>` or `Balance<T>`.

### TypeScript: `coinWithBalance`

If you are not explicitly selecting coin objects for gas or as transaction arguments, the TypeScript SDK automatically checks both coin objects and address balances and draws from the correct sources. The [`coinWithBalance`](https://sui-typescript-docs-git-mh-address-balance-docs-mysten-labs.vercel.app/sui/transactions/intents#coinwithbalance) method draws from address balances first, falling back to coin objects only when needed:
...
The [TypeScript SDK Core API](https://sdk.mystenlabs.com/sui/clients/core) RPC methods include address balance information:
...
Migration is optional. The SDK automatically selects coins or address balances as needed. Contracts that accept `Coin<T>` or `Balance<T>` remain callable. The `redeem_funds` functions convert withdrawals to the expected types within the PTB.

To consolidate coins into address balances using [Sui CLI](/references/cli/ptb):
...
Or using [`coinWithBalance`](https://sui-typescript-docs-git-mh-address-balance-docs-mysten-labs.vercel.app/sui/transactions/intents#coinwithbalance) in TypeScript:
