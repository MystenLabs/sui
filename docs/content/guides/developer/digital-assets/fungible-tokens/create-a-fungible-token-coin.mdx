...
See [Address Balances Migration Guide](/concepts/coin-mgt/address-balances.mdx) for the full reference on using address balances, including withdrawal patterns, gas payment, sponsored transactions, and balance queries.
...
You can convert coin objects into an address balance at any time by calling `coin::send_funds` with your own address as the recipient. See [Converting Existing Coins to Address Balances](/concepts/coin-mgt/address-balances.mdx#converting-existing-coins-to-address-balances) for instructions.
...
Address balances work for any coin type, not just SUI. You can send and withdraw non-SUI coins through `send_funds` and `redeem_funds` the same way you do with SUI.

Many existing Move contracts still accept `Coin<T>` as a function argument. If you hold funds as an address balance and need to call such a contract, withdraw from your address balance and redeem to a `Coin<T>` within your programmable transaction block (PTB). The `coinWithBalance` intent in the TypeScript SDK handles this automatically.
...
Concurrency in transaction execution is not the only performance consideration. Discovering and fetching objects, and dry running transactions, each require round trips to a full node. These round trips can affect performance significantly in high-throughput scenarios.
