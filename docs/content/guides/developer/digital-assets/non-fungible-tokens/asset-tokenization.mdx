...
The `tokenized_asset` module is the core of the implementation. It handles asset creation, minting, splitting, joining, and burning. It operates similarly to the [`coin` library](/references/framework/sui-framework/coin) on Sui:
...
- Claim the [`Publisher`](/references/framework/sui-framework/package) object using a One-Time Witness and store it in a shared `Registry`
- Create a [`TransferPolicy`](/standards/kiosk) and `TransferPolicyCap` for a given `TokenizedAsset<T>` type using `setup_tp`
- Create an empty [`Display`](/references/framework/sui-framework/display) for a `TokenizedAsset<T>` type using `new_display`
