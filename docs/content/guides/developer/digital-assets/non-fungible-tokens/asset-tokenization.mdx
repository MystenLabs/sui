...
description: Represent real-world assets as on-chain fractional tokens using the tokenized_asset module on Sui.
...
The core idea behind asset tokenization is fractionalization: dividing a high-value asset into smaller units, each representing a share of the whole. This makes assets more accessible by allowing investors to hold a fraction rather than the entire asset.
...
The `tokenized_asset` module supports both NFT and fungible token (FT) fractions. The distinction is determined at mint time by whether per-token metadata is provided:

- **NFT**: If additional metadata (key-value pairs) is provided when minting, the token is unique. Its balance is fixed at 1, and it represents a single distinct instance of the asset.
- **FT**: If no additional metadata is provided, the token is fungible. Its balance can exceed 1, and multiple identical instances can exist.

FTs can be split into smaller units or joined together when the balance is greater than 1. NFTs cannot be split or joined.
...
If a fraction is burned, the circulating supply decreases by that fraction's balance. The total supply remains unchanged, which means you can mint new fractions up to the original total supply again if needed.
...
:::info
This reference implementation uses the [Kiosk standard](/standards/kiosk.mdx) to ensure that tokenized assets operate within their defined transfer policy. Use the implementation as presented to support marketable tokenized assets with rules like royalties and commissions.
