...
- [x] [Install Sui](/guides/developer/getting-started/sui-install)
- [x] [Configure a Sui client](/guides/developer/getting-started/configure-sui-client)
- [x] [Get SUI from faucet](/guides/developer/getting-started/get-coins)
...
The `.env.template` file defines all required variables. Use the following as a reference:
...
You can publish the `asset_tokenization` package manually or automatically using the provided bash script.
...
Store the `package ID` and `registry ID` from the created objects in your `.env` file. Then modify the `Move.toml` file: under the `[addresses]` section, replace `0x0` with the same `package ID`. Optionally, under the `[package]` section, add `published-at = <package ID>` (this step is not needed if you see a `Move.lock` file after running `sui client publish`).
...
After publishing, edit the `Move.toml` file as described in the manual flow above.
 
For more details, see the setup folder's [README](https://github.com/MystenLabs/asset-tokenization/tree/main/setup).
 
#### `template` package
 
You can publish the `template` package manually or automatically through the WASM library.
 
##### Manually
 
In a terminal or console at the `move/template` directory of the project, run:
...
The automatic deployment for the `template` package publishes a new asset through the WASM library. Before running:
 
- Ensure that the `asset_tokenization` package address is set in the `[addresses]` section of `asset_tokenization/Move.toml` and matches the original deployment.
- If a `Move.lock` file exists after running `sui client publish`, proceed to the next step. If not, ensure the `asset_tokenization/Move.toml` file has its `published-at` field populated with the address of the latest package deployment.
- Make any changes to the template fields by updating the input parameters of the `publishNewAsset` function.
 
```sh
$ npm run publish-template
```
 
Store the `Template Package ID`, `asset metadata ID`, `asset cap ID`, and `publisher ID` from the created objects in your `.env` file.
