...
In the output we can see the value of the `swords_created` field of the `Forge` change after the increment. The stack trace shows the bytecode instructions that have been executed so far, and the next few instructions to be executed. (**Note**: the specific bytecode instructions the values of the locals may vary depending on the version of the Sui toolchain.)
