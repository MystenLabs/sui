...
Custom indexers are also a part of the [Future data access interfaces](#future-data-access-interfaces), and are a good choice for app or protocol-specific logic and data layout.
...
- Indexer 2.0 will include a performant and scalable implementation of the [Sui indexer framework](../../../concepts/custom-indexing-framework.mdx) and is currently available in alpha. You can use it to load data at scale from Full nodes into a Postgres relational database.
...
- It does not have implicit fallback on the previously mentioned [Sui Foundation-managed key-value store for historical transaction data](/guides/operator/data-management.mdx#sui-full-node-key-value-store-backup). Full node operators, RPC providers, and data indexer operators are encouraged to run their own instance of a similar archival store, which you can explicitly define a dependency on to get the relevant historical data. Learn more at [Archival Store and Service](#archival-store-and-service).
...
| September-October 2025 | GA release of polling-based and streaming APIs. | You can start migration and cutover of your application in the production environment. **JSON-RPC will be deprecated at this point and migration notice period will start.** |
...
As mentioned, Indexer 2.0 will include a performant and scalable implementation of the [indexer framework](../../../concepts/custom-indexing-framework.mdx). The underlying framework uses the full node RPCs to ingest the data, initially using the current generally available JSON-RPC, and later using the gRPC API.
...
As Full nodes enforce limited retention for scalability, long-term access to historical on-chain data becomes essential for developers. The [Archival Store and Service](../../../concepts/archival-store.mdx) enable access to this data through a pluggable storage backend (like [Bigtable](https://cloud.google.com/bigtable)) and a [gRPC interface compatible with the `LedgerService`](../../../concepts/grpc-overview.mdx).
...
Learn more at [Archival Store and Service](../../../concepts/archival-store.mdx).
