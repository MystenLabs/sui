...
If you are using the devnet or testnet networks, or you spinned up a local network, you can simply use the [Sui CLI](https://docs.sui.io/guides/developer/getting-started/sui-install) to request tokens for your address. Use the `--url` option to provide a custom faucet endpoint.
```bash
sui client faucet 		=> will use the active network and active address
sui client faucet --address[..] => will use the address corresponding to this address (or its alias), and the active network
sui client faucet --url [..]	=> will use the active address and the faucet server at that endpoint
sui client faucet --address [..] --url https://myfaucet.mysui.io/v5/gas => will use that address and the faucet server at that endpoint.
```
