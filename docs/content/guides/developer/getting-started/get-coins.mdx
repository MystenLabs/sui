...
Use the [Sui CLI](../../../references/cli/client.mdx) to request tokens for your address on the Devnet, Testnet, or local network. `sui client faucet` uses the active network and active address that is currently set in the Sui CLIt, but you can specify custom data through the following two arguments:
