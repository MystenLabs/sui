...
If you are using the devnet or testnet networks, or you spinned up a local network, you can simply use the [Sui CLI](https://docs.sui.io/guides/developer/getting-started/sui-install) to request tokens for your address. The `sui client faucet` will by default use the [active network](references/cli/client#list-available-network-environments) and [active address](references/cli/client#get-current-active-address) that is currently set in the Sui CLI, but you can specify custom data through the following two arguments:
