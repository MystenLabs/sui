...
Point look-ups (acessing an object by its ID and version, a transaction by its digest, a checkpoint by its sequence number etc) are generally unpruned, if GraphQL has access to an Archival Service.
...
As there is no longer a dedicated API for fetching the coins owned by an address, whenever coins are returned by an owned object API (such as `Query.objects` with an `owner` filter or `IAddress.objects`), they are grouped by coin type and sorted in descending balance order.
