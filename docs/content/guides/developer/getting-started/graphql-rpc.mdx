...
As well as being renamed to `simulateTransaction`, dry-run now supports the same input schema as `gRPC`'s `SimulateTransaction` -- [Transaction](/references/fullnode-protocol#sui-rpc-v2-Transaction) -- in JSON form. This means that it can also accept transactions where the inputs have not been fully resolved or serialized, perform gas estimation, and coin selection.
