...
This query can be used to paginate over the dynamic fields of an object. This works even when the object in question is [wrapped](/concepts/object-ownership/wrapped), by using the [owner](/references/sui-graphql/reference/queries/owner) query, so can be used for iterating over the elements of on-chain data structures, like [Tables and Bags](/concepts/dynamic-fields/tables-bags).
