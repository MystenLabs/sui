...
- Linux - Ubuntu version 22.04 (Jammy Jellyfish) or later
    - Binaries from AWS only support version 22.04
...
The `sui-node` binaries for Ubuntu 22.04 are available for download from AWS. You can use either the commit sha for the version of Sui you want to retrieve or its version tag. Using either of these values, you can construct the correct URL to download the binary from AWS.
