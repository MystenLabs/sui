...
The most effective way to install the Sui CLI is to use `suiup`, as it allows you to easily install and switch between different versions of not only the Sui CLI but also other Sui stack components like [`walrus`](https://docs.wal.app/usage/setup.html) and [`mvr`](https://github.com/MystenLabs/mvr). 
...
If Sui is already installed from a previous development environment, be sure to upgrade to the [latest version](/guides/developer/getting-started/upgrade-sui.mdx).
...
Before you can create and publish a smart contract, you must [configure your Sui client](/guides/developer/getting-started/configure-sui-client.mdx) and [obtain testnet SUI](/guides/developer/getting-started/get-coins.mdx).
...
Now that you have Sui installed, you must [configure your Sui client](/guides/developer/getting-started/configure-sui-client.mdx).
