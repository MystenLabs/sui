...
1. Open the expanded folder. Navigate to the `target/release` directory and double-click the appropriate binary to install, beginning with `sui-ubuntu-x86_64`:

    - `sui-data-ingestion-ubuntu-x86_64`: Massage Full node data for indexer to store in a database.
    - `sui-faucet-ubuntu-x86_64`: Local faucet to mint coins on local network.
    - `sui-indexer-ubuntu-x86_64`: An indexer for a local Sui network.
    - `sui-ubuntu-x86_64`: Main Sui binary.
    - `sui-node-ubuntu-x86_64`: Run a local node.
    - `sui-test-validator-ubuntu-x86_64`: Run test validators on a local network for development.
    - `sui-tool-ubuntu-x86_64`: Provides utilities for Sui.
...
1. In the **Assets** section of the release, select the .tgz compressed file that corresponds to your operating system.
1. Double-click the downloaded file. If the file doesn't automatically expand, manually unzip the file.
1. Open the expanded folder. Navigate to the `target/release` directory and double-click the appropriate binary to install, beginning with `sui-macos-<ARCHITECTURE>`:

    - `sui-data-ingestion-macos-<ARCHITECTURE>`: Massage Full node data for indexer to store in a database.
    - `sui-faucet-macos-<ARCHITECTURE>`: Local faucet to mint coins on local network.
    - `sui-indexer-macos-<ARCHITECTURE>`: An indexer for a local Sui network.
    - `sui-macos-<ARCHITECTURE>`: Main Sui binary.
    - `sui-node-macos-<ARCHITECTURE>`: Run a local node.
    - `sui-test-validator-macos-<ARCHITECTURE>`: Run test validators on a local network for development.
    - `sui-tool-macos-<ARCHITECTURE>`: Provides utilities for Sui.
...
1. Navigate to the `target\release` directory of the expanded folder. Rename each file to remove the `-windows-x86_64` suffix from each file name. For example, rename `sui-windows-x86_64.exe` to `sui.exe`. If you don't do this step, you must type `sui-windows-x86_64` to execute commands instead of simply `sui`. When complete, you should have the following .exe files:
    ...
    - `sui-data-ingestion`: Massage Full node data for indexer to store in a database.
