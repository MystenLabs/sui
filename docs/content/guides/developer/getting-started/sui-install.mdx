...
1. Open the expanded folder. Navigate to the `target/release` directory and double-click the appropriate binary to install, beginning with `sui-linux-<ARCHITECTURE>`:

    - sui-faucet-`linux`-`<ARCHITECTURE>`: Local faucet to mint coins on local network.
    - sui-indexer-`linux`-`<ARCHITECTURE>`: An indexer for a local Sui network.
    - sui-`linux`-`<ARCHITECTURE>`: Main Sui binary.
    - sui-node-`linux`-`<ARCHITECTURE>`: Run a local node.
    - sui-test-validator-`linux`-`<ARCHITECTURE>`: Run test validators on a local network for development.
    - sui-tool-`linux`-`<ARCHITECTURE>`: Provides utilities for Sui.
...
1. Navigate to the `target\release` directory of the expanded folder. Rename each file to remove the `-windows-<ARCHITECTURE>` suffix from each file. For example, rename `sui-windows-x86_64` to `sui`. If you don't do this step, you must type `sui-windows-x86_64` to execute commands instead of simply `sui`. When complete, you should have the following .exe files:
