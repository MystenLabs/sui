...
These tools are developed and maintained by community members and do not come with any security guarantees. Do your own due diligence if you decide to use these in a production environment.
...
It transforms Trusted Execution Environment (TEE) access into a transparent and decentralized marketplace. A Sui Move contract enables you to submit jobs, pay with stablecoins, and expose workloads to Oyster operators. Builds are deterministic, meaning enclave integrity and identity are fully verifiable.
...
Nautilus-Ops is a command-line tool you can use to build and deploy enclave images, register attestations on chain, and verify enclave signatures.

[Learn more on the project GitHub](https://github.com/Ashwin-3cS/nautilus-ops).

## nautilus-ts

nautilus-ts is a TypeScript framework for building apps inside AWS Nitro Enclaves using SDKs such as `@mysten/sui` and `@mysten/seal`. The framework handles VSOCK networking, NSM attestation, traffic forwarding, and key management.
