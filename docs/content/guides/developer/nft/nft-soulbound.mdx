...
A soulbound NFT is an NFT that is non-transferable. Once a NFT is minted to a Sui account, the NFT is "bounded" to that account and cannot be transferred. This implementation leverages the custom logic of the Sui framework's transfer functions. The [`sui::transfer` module](/references/framework/sui-framework/transfer.md) contains two functions that transfers objects: [`transfer::transfer`](/references/framework/sui-framework/transfer.md#function-transfer) and [`transfer::public_transfer`](/references/framework/sui-framework/transfer.md#function-public_transfer). 
...
It is also possible to create custom transfer logic for NFTs on Sui. The `transfer::transfer` function has custom rules performed by the Sui Move bytecode verifier that ensures that the transferred objects is an object defined in the module where transfer is invoked. While removing the `store` ability from a struct definition makes `transfer::public_transfer` unusable, `transfer::transfer` can still be used as long as it's used in the module that defined that object's type. This allows for the module owner to provide custom transfer logic for their soulbound NFTs. 
...
This `TestnetSoulboundNFT` struct is defined with the `key` ability but without the `store` ability meaning it cannot be transferred with `transfer::public_transfer` and can only be transferred with `transfer::transfer`, which requires custom transfer logic to be implemented in the same module. 

This example also includes an example of how to provide custom transfer logic using the `transfer::transfer` function. This is where you can add additional logic such as reset the NFT's stats, or requiring a payment. Don't provide this functionality if the NFT is fully soulbound. 
