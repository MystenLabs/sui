...
The value that is returned can be interacted with just like any other value. For example, removed dynamic object field values can then be deleted or transfered back to the sender:
...
Sui provides `Table` and `Bag` collections built using dynamic fields, but with additional support to count the number of entries they contain to protect against accidental deletion when non-empty. To learn more, see [Tables and Bags](/guides/developer/objects/tables-bags.mdx).
...
<RelatedLink to="/guides/developer/objects/dynamic-fields/tables-bags" />