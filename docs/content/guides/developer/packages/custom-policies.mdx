...
To address these risks while still providing the opportunity to [upgrade live packages](/guides/developer/packages/upgrade.mdx), Sui offers custom upgrade policies. These policies protect `UpgradeCap` access and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.
...
Finally, execute that transaction and display its effects to the console. The following snippet assumes that you're running your examples against a local network. Change the `baseUrl` and `network` parameters to run on Devnet, Testnet, or Mainnet respectively:
