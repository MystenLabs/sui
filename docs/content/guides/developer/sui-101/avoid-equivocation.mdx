...
Use the `SerialTransactionExecutor` when you process transactions 1 after another. The executor grabs all the coins from the sender and combines them into a single coin that is used for all transactions.
