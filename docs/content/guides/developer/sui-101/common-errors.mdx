...
description: Find solutions to common issues with installation, transactions, and building Move packages.
...
- Must start with `0x` and only contain hexadecimal characters (0-9, a-f, A-F).
...
- [Create a Sui Address](https://docs.sui.io/guides/developer/getting-started/get-address)
...
- [`sui::url`](https://docs.sui.io/references/framework/sui_sui/url)
...
Indicates that your environment or tooling cannot find a Move package at the specified address. This might be caused by:
...
Remove any explicit address blocks from your Move source files. Do not wrap your modules in an `address ... { ... }` block.
...
- `FIELD_MISSING_TYPE_ABILITY`: Your struct is missing a required ability such as key, store, etc. Ensure all fields have the necessary abilities. 
...
Wait for other transactions to complete, then retry your transaction. If transactions are stuck or fail, you may need to wait for the next epoch before you can retry the transaction.

Make sure your code does not use multiple objects simultaneously by mistake.

**Resources**

- [Signing and Sending Transactions](https://docs.sui.io/guides/developer/sui-101/sign-and-send-txn)
...
- [Avoiding Equivocation](https://docs.sui.io/guides/developer/sui-101/avoid-equivocation)

---

#### `Unable to process transaction. Unexpected status code: 403`

Indicates the Sui network responds to an RPC or API request with `HTTP 403: Forbidden` status. This may happen if an RPC provider requires authentication and you do not provide any, the provider has restricted your region or IP address, or the provider has rate limited you by sending too many requests in a given time period.
