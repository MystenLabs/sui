...
To execute a transaction to Sui, a signature is a required to be submitted along with the transaction. The signature is provided with the private key owned by the wallet, and its public key must be consistent with the transaction sender Sui address. 

A `SuiKeyPair` is used to produce the signature, which commits to the Blake2b hash digest of the intent message (i.e. `intent || bcs bytes of tx_data`). The signature schemes currently supported Ed25519 Pure, ECDSA Secp256k1, ECDSA Secp256r1, Multisig and ZkLogin. 

Ed25519 Pure, ECDSA Secp256k1, ECDSA Secp256r1 can be instantiated using `SuiKeyPair` and used to sign transactions. Note that this guide does not apply to Multisig and ZkLogin, please refer to their own pages for instructions. 

# Examples

### Typescript SDK

First we demonstrate various ways to instantiate a keypair and to derive its public key and Sui address.
...
First we demonstrate various ways to instantiate a `SuiKeyPair` and to derive its public key and Sui address. The complete runnable code can be found under [sui-sdk](~/sui/crates/sui-sdk/examples/sign_tx_guide.rs). 
...
Next we demonstrate how to sign a transaction data, constructed using an example programmable transaction with default gas coin, gas budget and gas price. See [more](./building-ptb.mdx) on how to construct other types of programmable transactions. 
```
...
A signature is committed to the Blake2b hash digest of the intent message (i.e. `intent || bcs bytes of tx_data`). 

```Rust
    // derive the digest that the keypair should sign on. 
    let intent_msg = IntentMessage::new(Intent::sui_transaction(), tx_data);
    let raw_tx = bcs::to_bytes(&intent_msg).map_err(|_| TransactionError::ParsingError)?;
    let mut hasher = sui_types::crypto::DefaultHash::default();
    hasher.update(raw_tx.clone());
    let digest = hasher.finalize().digest;

    // use SuiKeyPair to sign the digest. 
    let sui_sig = skp.sign(&digest);

    // verify the signature against the transaction locally. if this fails the transaction probably will fail submitting. 
...
CLI initiate a local file in `~/.sui/keystore` on your machine with a list of private keys (encoded as Base64 encoded `flag || 32-byte-private-key`) and can be used to sign transactions by specifying its address.

There are 3 following ways to intialize a key.
```bash
# generate randomly. 
...
# import the mneomonics with derivation path to keystore. 
sui keytool import "$MNEMONICS" ed25519
sui keytool import "$MNEMONICS" secp256k1
sui keytool import "$MNEMONICS" secp256r1
```
Create a transfer transaction in CLI. Set the $SUI_ADDRESS to the one corresponding to the keypair used to sign. Then sign with the private key corresponding to the sender address. 

```bash
sui client transfer-sui --to $SUI_ADDRESS --sui-coin-object-id <COIN-OBJECT-ID> --gas-budget $GAS_BUDGET --serialize-unsigned-transaction
...
### Notes
1. The guide here demonstrates how to sign with a single private key. Refer to [Multisig] when it is preferred to set up more complex signing policies.
2. Similarly, native zkLogin does not follow the above steps, see [zkLogin] see how to derive a zkLogin address, produce a zkLogin signature with an ephemeral keypair. 
3. If you decided to implement your own signing mechanisms instead of using the above tools, see [Sui Signatures](../concepts/cryptography/transaction-auth/signatures) on the accepted signature specs for each scheme. 
4. Flag is one byte that differentiates signature schemes. See supported schemes and its flag in [Sui Signatures](../concepts/cryptography/transaction-auth/signatures).
5. The `execute_transaction_block` endpoint takes a list of signatures, it should contain exactly one user signature, unless you are using sponsored transaction that a second signature for the gas object can be provided. See [sponsored transaction] page for more. 