...
The Sui RPC provides a [queryEvents](/sui-api-ref#suix_queryEvents) method to query on-chain packages and return available events. The following `curl` command queries the Deepbook package on Mainnet:
...
Both the TypeScript and Rust SDKs provide wrappers for the `suix_queryEvents` method.

- TypeScript SDK [`client.queryEvents`](https://sdk.mystenlabs.com/typedoc/classes/_mysten_sui.client.SuiClient.html#queryEvents).
- Rust SDK [`query_events`](https://mystenlabs.github.io/sui/sui_sdk/apis/struct.EventApi.html#method.query_events).
...
The [TypeScript SDK](https://sdk.mystenlabs.com/typedoc/modules/_mysten_sui.graphql.html) and [Rust SDK](https://mystenlabs.github.io/sui/sui_graphql_rpc_client/index.html) provide functionality to interact with the Sui GraphQL service. 
...
Firing events is not very useful in a vacuum. You also need the ability to respond to those events. There are to methods from which to choose when you need to monitor on-chain events:
...
The following examples demonstrate how to query on-chain events.
