...
A transaction on Sui requires a gas payment to execute. The payment can be a list of `0x2::coin::Coin<0x2::sui::SUI>` objects or drawn from the sponsor address balance.
...
Address balances simplify sponsored transactions by eliminating gas coin management. The sponsor does not need to select or lock gas coins, and the user can sign before the sponsor. See [Sponsored Transactions with Address Balances](/guides/developer/digital-assets/address-balances/using-address-balances#sponsored-transactions) for details.
...
Address balance gas payments eliminate equivocation risk for the gas payment itself because no owned objects are involved. If your sponsored transactions do not use other owned objects as inputs, address balance sponsorship removes this risk entirely.
