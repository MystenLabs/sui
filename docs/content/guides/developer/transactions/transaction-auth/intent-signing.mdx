...
1.  Pass the digest to the signing API for each corresponding scheme of the signer. The supported signature schemes are pure Ed25519, ECDSA Secp256k1, and ECDSA Secp256r1. See [Sui Signatures](/guides/developer/transactions/transaction-auth/auth-overview#signature-requirements) for requirements of each scheme.
