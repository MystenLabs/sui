...
If the information about the transaction digest, effects, events, or checkpoints is not available locally, a Full node automatically retrieves the historical data from a cloud-based key-value store managed by Sui Foundation. The current key-value store implementation keeps historic transactional data only, but future releases might support retrieving the historic object versions.
