...
GraphQL server instances read data from the indexer db, the consistent store and the archival store. The number of instances is determined by the required read throughput.
...
GraphQL server supports two ways to access the archival store: self-hosted Bigtable instance, or a managed archival service that exposes a LedgerService gRPC interface. See [Archival Store](/concepts/archival-store) for more information.
