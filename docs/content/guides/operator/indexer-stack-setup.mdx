...
GraphQL's configuration file is optional and if it is not provided, all default values are used. To customize the configuration, a file with the default settings can be output using the following command, and then edited:
