...
Internally at Mysten, indexer instances were provisioned with 8 CPUs and 16 GiB of memory for backfill, and scaled down to 0.5 CPU and 1 GiB of memory for normal operation at network tip.
...
Once caught up to network tip, you could manually set the max concurrency to a lower value. Mysten-internal indexers are capped at 20.
:::
...
To restart and index from a different initial checkpoint, you must delete the watermark record for that pipeline. We also recommend deleting the existing data for that pipeline to avoid any data inconsistencies.

For the general-purpose indexer writing to Postgres, we recommend using the template below:
```sql
BEGIN;
...
The following autovacuum settings are used by Mysten for a 32-vCPU, 256 GiB Postgres instance, and should be adjusted proportionally for your setup:
