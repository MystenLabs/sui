...
The general-purpose indexer writes to a Postgres database. The bulk of the storage is consumed by `obj_versions` at 8.2 TB. A pruning strategy is in development.
A 30-day retention adds 1.6 TB on top, while a 90-day retention contributes an additional 3.1 TB.
