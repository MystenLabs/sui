...
GraphQL RPC server reads data from the general-purpose indexer's database (Postgres), the consistent store, and the [archival service](/concepts/archival-store).
...
GraphQL RPC server can access the [Archival Service](https://github.com/concepts/archival-store) by connecting either to a self-hosted service backed by your own Bigtable cluster or to a managed service operated by an RPC provider.

#### Run command using Bigtable

Each GraphQL instance is run using the same command below:
```sh
GOOGLE_APPLICATION_CREDENTIALS=<PATH_TO_BT_CREDENTIALS_JSON> sui-indexer-alt-graphql rpc \
    --config CONFIG_FILE \
...
#### Run command using LedgerService gRPC

:::info

If you use Sui Foundation's public good archival service at archive.testnet.sui.io and archive.mainnet.sui.io, you may experience performance issues which will be addressed before GA release.
:::
...
| `BIGTABLE_INSTANCE`              | GCP Bigtable instance id                                                             |
| `PATH_TO_BT_CREDENTIALS_JSON`    | Path to GCP Bigtable credentials JSON file                                       |
...
The default GraphQL API configuration is provided in the `examples` directory of the Sui repo.

<ImportContent source="examples/prod-config/graphql.toml" mode="code" />
|-----------------------------------------------------------------------------------------------|-----------------------------------|
| [graphql.toml](examples/prod-config/graphql.toml) | default GraphQL API configuration |
