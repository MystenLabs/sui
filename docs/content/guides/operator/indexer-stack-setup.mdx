...
1. Determine the most recently indexed checkpoint (`recent-checkpoint`) by doing one of the following (an approximate value is ok to use because the new pipeline will catch up):
