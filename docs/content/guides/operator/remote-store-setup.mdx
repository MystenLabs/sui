...
The **remote store** is an object store (S3 or GCS) containing protobuf-encoded checkpoint blobs. Fullnodes read from it for state sync and indexers read from it for backfills. The `sui-checkpoint-blob-indexer` populates this store by reading checkpoints from a source and writing compressed protobuf blobs to a destination.

## Architecture overview

```mermaid
graph LR
    A[Source: Existing Remote Store / Fullnode gRPC] --> B[sui-checkpoint-blob-indexer]
...
- Access to a checkpoint source:
   ...
   - **Your own fullnode** with gRPC enabled (for steady-state streaming).
...
## Alternative: backfill from a fullnode

If you have an unpruned fullnode, you can use it as the checkpoint source instead of a bucket:
...
Once the backfill has caught up to the network tip, switch to fullnode gRPC streaming for the lowest latency. Use `--rpc-api-url` for checkpoint fetching and `--streaming-url` for live streaming:
...
No special configuration is needed for steady-state. The framework defaults are sufficient. The fullnode only needs gRPC enabled — JSON-RPC is not required.

Writes are idempotent, so you can run multiple indexer instances for redundancy.
...
| `--rpc-api-url <URL>` | Fetch checkpoints from a fullnode gRPC endpoint. |
...
| `--streaming-url <URL>` | — | Fullnode gRPC URL for streaming live checkpoints. Used alongside `--rpc-api-url` for lowest latency at the network tip. |
...
Framework defaults with adaptive concurrency controls handle tuning automatically. In most cases you do not need a config file at all — just use `--pipeline` to run only the two recommended pipelines and exclude the deprecated BCS pipeline:

```sh
--pipeline checkpoint_blob --pipeline epochs
```

If you do want to override defaults, pass a TOML file via `--config`. The configuration options (`[ingestion]`, `[committer]`, `[pipeline.<name>]`) are the same as the indexer framework's standard settings. See [Pipeline architecture — Performance tuning](/concepts/data-access/pipeline-architecture#performance-tuning) for the full reference.
...
<RelatedLink to="/guides/operator/archival-stack-setup" />
<RelatedLink to="/guides/operator/sui-full-node" />
<RelatedLink
 href="https://github.com/MystenLabs/sui/tree/main/crates/sui-checkpoint-blob-indexer"
 label="sui-checkpoint-blob-indexer"
 desc="The checkpoint blob indexer crate in the Sui repo."
/>
