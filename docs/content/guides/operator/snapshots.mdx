...
1. When using `sui-tool download-db-snapshot` the db is copied to the location you've passed to `--path`, in a directory named `epoch_[NUM]`. You'll need to move this directory to the `live/` fullnode db dir, eg: `/opt/sui/db/authorities_db/full_node_db/live`.
