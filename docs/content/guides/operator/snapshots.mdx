...
Changing the configs for a full node is done by the following steps:

1. Stop your node, if it's running.
2. Open your fullnode.yaml config file and apply config updates.
...
Add an entry to the config file for `db-checkpoint-config`. Using Amazon's S3 service as an example:
   ...
   ```
   The configuration settings show in the example are specific to AWS S3, but GCS, Azure Storage, and Cloudflare R2 are all supported.
...
To restore using a Formal snapshot, you can use the `sui-tool` binary. `sui-tool` can be downloaded along with other sui binaries.
See [Install Sui](./getting-started/sui-install.mdx) for more details.
...
2. Run the command:
   ...
   sui-tool download-db-snapshot --epoch "<EPOCH-NUMBER>" --genesis "<PATH-TO-GENESIS-BLOB>" \
        --formal --network "<NETWORK>" --path <PATH-TO-NODE-DB> --num-parallel-downloads 50 --no-sign-request --snapshot-bucket-type=s3
   ```
   - `--epoch`: The epoch that you want to download, eg. `200`. Mysten Labs hosted buckets will only keep the last 90 epochs. 
...
Mysten is providing two tiers of snapshot storage access. *High throughput, Requester Pays enabled buckets*, and *Free, permissionless buckets*.
