...
2. Run the command:
   ...
   sui-tool download-formal-snapshot --latest --genesis "<PATH-TO-GENESIS-BLOB>" \
        --network <NETWORK> --path <PATH-TO-NODE-DB> --num-parallel-downloads 50
   ```

   **Snapshot source options** (mutually exclusive - use one):
   - `--s3 <BUCKET>`: Fetch from AWS S3. Provide the bucket name.
   - `--gcs <BUCKET>`: Fetch from Google Cloud Storage. Provide the bucket name.
   - `--azure <CONTAINER>`: Fetch from Azure Blob Storage. Provide the container name.
   - `--http <URL>`: Fetch from an HTTP endpoint (unsigned/public access).
   - `--local-path <PATH>`: Fetch from local filesystem.

   If no source is specified, defaults to the public HTTP endpoint for the selected network.
   ...
   **Other options:**
   ...
   - `--latest`: Rather than explicitly passing an epoch via `--epoch`, you can pass the `--latest` flag, which will automatically select the latest snapshot.
   ...
   **Environment variables for cloud storage credentials:**
   ...
   When using `--s3`, `--gcs`, or `--azure`, credentials are read from standard cloud provider environment variables:
   * *AWS S3*: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_DEFAULT_REGION`, `AWS_ENDPOINT`
...
* To use the free buckets, either omit the source flag (defaults to the public endpoint) or use `--http` explicitly:
  ```
  sui-tool download-formal-snapshot --latest --genesis genesis.blob --path /data
  # or explicitly:
  sui-tool download-formal-snapshot --http https://formal-snapshot.mainnet.sui.io --latest --genesis genesis.blob --path /data
  ```
* The free public endpoints are only available for _formal_ snapshots. You must use one of the requester pays (S3 or GCS) buckets if downloading a db snapshot
