...
There are many ways to run a Sui Full node (bare metal, virtual machine, Kubernetes statefulset, etc.), and the solution that you choose depends on your specific needs as well as the infrastructure that you have available.

There are some specific considerations to keep in mind when running a Sui Full node which apply to all environments:
...
* [Updates](./updating-fullnode.mdx): Sui Full nodes must be updated to the latest version to remain in sync with the network.
...
Once you are ready to run `sui-node` in your production environment, you can set up your Full node by completing the following steps:
