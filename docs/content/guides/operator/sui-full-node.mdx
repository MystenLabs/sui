...
You can configure the data retention window for the gRPC index data on a Full node using the following config:
...
Adjust the gRPC data retention period as needed, based on your Full node’s resource profile and depending on if you've disabled JSON-RPC or not. In any case, be sure to test the longer retention period for performance and scalability before using gRPC in your application or offering it to other developers.
