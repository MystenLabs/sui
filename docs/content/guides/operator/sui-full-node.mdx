...
Today, a Sui Full node has the potential to maintain the full history of the chain. But that will change with gRPC and as we phase out JSON-RPC.
...
Enable gRPC indexing on your Full node using the following configuration:
...
Running gRPC alongside JSON-RPC will increase your Full node’s storage usage. While this is expected in the short term, you can reclaim this storage in the future by disabling JSON-RPC after it is phased out. Developers must gradually migrate to gRPC or [GraphQL RPC](/concepts/graphql-rpc.mdx) (which will internally rely on gRPC). You can also run gRPC and JSON-RPC on separate nodes to optimize resource usage and provide a performant plus resilient RPC experience to applications. Use the following config to selectively disable JSON-RPC on a Full node:
...
By default, your Full node will retain `14` days of gRPC index data. You can customize this retention window with the following config:
...
Adjust the gRPC retention period as needed based on your Full node’s resource profile and depending on if you've disabled JSON-RPC or not. In any case, be sure to test the longer retention period for performance and scalability before using gRPC in your application or offering it to other developers.

We strongly recommend enabling gRPC beta support on your Full nodes as early as possible to help developers evaluate and integrate the API before general availability. Refer to [Sui's data serving roadmap](/guides/developer/getting-started/data-serving.mdx#grpc-api) for guidance on the upcoming transition from JSON-RPC.
