...
2. **Verify balance changes.** For basic coin transfers, check `balanceChanges` to confirm the correct amount reached the correct recipient.
3. **Verify Payment Kit events.** If you use the [Payment Kit](/onchain-finance/payment-kit), verify the emitted `PaymentReceipt` event. Match the full event type against your network's audited Payment Kit package ID, and match the nonce, exact amount, coin type, and receiver.
...
If `waitForTransaction` times out, the transaction might still succeed later. Query the stored digest directly before retrying.
