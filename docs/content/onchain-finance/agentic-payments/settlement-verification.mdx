...
<ImportContent
	source="examples/onchain-finance/settlement/src/verify.ts"
	mode="code"
	tag="wait-for-finality"
/>
...
<ImportContent
	source="examples/onchain-finance/settlement/src/verify.ts"
	mode="code"
	tag="verify-balance-changes"
/>

### Verify Payment Kit events

If you are using the [Payment Kit](/onchain-finance/payment-kit), verify the emitted `PaymentReceipt` event instead of relying only on raw balance changes. Match the full event type against your network's audited Payment Kit package ID, and match the nonce, exact amount, coin type, and receiver. Also require a successful transaction status before releasing goods.

<ImportContent
	source="examples/onchain-finance/settlement/src/verify.ts"
	mode="code"
	tag="verify-payment-kit-events"
/>
...
In the PTB, chain `pay_and_prove` into `consume_proof`:

<ImportContent
	source="examples/onchain-finance/settlement/src/verify.ts"
	mode="code"
	tag="onchain-settlement-ptb"
/>
