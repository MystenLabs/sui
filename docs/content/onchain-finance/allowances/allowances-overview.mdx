...
1. **The funder grants it.** In one transaction, the funder names the spender, the coin, and the limits. Sui records the allowance as a [shared object](/develop/objects/object-ownership/shared) and gives the funder an `AllowanceCap`, a revocation capability that cannot be transferred.
