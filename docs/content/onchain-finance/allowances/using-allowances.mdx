...
- [x] [Install Sui](/getting-started/onboarding/sui-install) v1.80 or later for Devnet and Testnet, or v1.81 or later for Mainnet.
- [x] A funded [address balance](/onchain-finance/asset-custody/address-balances) on the funder's address.
- [x] For the TypeScript examples, `@mysten/sui` 2.31 or later: `npm install @mysten/sui`.
...
The allowances demo issues an allowance, spends against it, and revokes it with a connected wallet, so you can see the whole lifecycle before writing any code. It runs on Testnet and opens in a new tab.

<a href="https://mystenlabs.github.io/allowances-app/" target="_blank" rel="noreferrer">Open the allowances demo</a>
...
1. A funds withdrawal input whose source is `SenderAllowance { funder, allowance }`. This declares the amount, the funder whose balance Sui debits, and the allowance that authorizes it.
