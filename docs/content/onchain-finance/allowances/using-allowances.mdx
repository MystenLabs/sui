...
answer: >-
  ...
  The spender then spends with tx.balance({ allowance, balance }) in the
  TypeScript SDK, or builds a funds withdrawal whose source is SenderAllowance
  and passes it to sui::allowance::balance_spend. The funder revokes with
  sui::allowance::revoke and the AllowanceCap.
---
...
For full control over the transaction, build the withdrawal input yourself with `from: 'allowance'` and call `balance_spend` directly:

```typescript title='spend-allowance-manual.ts'
...
Both forms take the coin type, while the allowance was issued with the balance type `Balance<0x2::sui::SUI>`. The spender pays gas.
