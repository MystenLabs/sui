...
questions:
  ...
  - How do I enable and manage address aliases on Sui?
answer: >-
  Address aliases let you authorize additional Sui addresses to act as
  your account. Enable aliases, then add, remove, or replace alias
  addresses. Aliases have full sender authority, enabling key rotation
...
A multisig account can add a single-key alias for routine operations. The alias has full sender authority — it can do anything the multisig can, so use this pattern only when you trust the alias holder completely. The benefit is operational speed: day-to-day transactions can be signed by the alias without gathering multiple signatures.
