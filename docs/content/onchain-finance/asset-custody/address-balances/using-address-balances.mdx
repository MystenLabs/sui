...
description: >-
  ...
  to Sui addresses. Learn how to send, withdraw, and query address balances.
keywords:
  - address balances
  - coin objects
  - funds withdrawal
...
goal:
  description: >-
    Reader can send, withdraw, pay gas from, and query address balances using
    the TypeScript SDK, CLI, and Move
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  - path_id: p2p-payments
    ...
    step: Using address balances
    stage: Wallets & Funding
questions:
  - What is Using Address Balances in Sui?
  - How do I send funds?
  - How does using address balances work on Sui?
answer: >-
  Address balances introduce a canonical balance system for fungible assets tied
  to Sui addresses. Learn how to send, withdraw, and query address balances.
---

Address balances provide a canonical balance for each coin type tied directly to a Sui address. Funds sent through `sui::coin::send_funds` and `sui::balance::send_funds` merge automatically into a single balance per coin type, with no object management required. For the conceptual model and a comparison with coin objects, see [Address Balances overview](/onchain-finance/asset-custody/address-balances). For the full specification, see [SIP-58: Sui Address Balances](https://github.com/sui-foundation/sips/blob/main/sips/sip-58.md).
...
- The sponsor needs to manage address balance inventory.
...
// Option A: Send the transaction bytes and user signature to the sponsor service.
// The sponsor signs and executes the transaction on their end.
// Replace with your own sponsor communication logic (e.g., an API call).
await sendToSponsor(userSignature, bytes);
...
Use [`GetBalance`](/references/fullnode-protocol) and [`ListBalances`](/references/fullnode-protocol) from `StateService`:
