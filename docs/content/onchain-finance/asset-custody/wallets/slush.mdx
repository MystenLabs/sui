...
description: >-
  ...
  wallet or app using the Slush Wallet SDK and Sui dApp Kit.
keywords:
  - slush
  - slush wallet
  - wallet
...
questions:
  - What is Slush in Sui?
  - How does slush work on Sui?
  - Why should I use slush?
answer: >-
  Slush is the official Sui wallet built by Mysten Labs, available as a Chrome
...
The [Sui dApp Kit](https://sdk.mystenlabs.com/dapp-kit) provides built-in opt-in support for Slush Wallet. Pass `slushWalletConfig` to `createDAppKit` to enable it. The `name` field is required, but it is not shown to the user during the connection flow.
