...
keywords:
  ...
  - connect
goal:
  description: Reader can use SuiLink to connect wallets across different chains
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is SuiLink in Sui?
  - How do I create a suilink?
  - How do I integrate suilink?
answer: Use SuiLink to connect wallets across different chains.
---
...
This returns every SuiLink NFT owned by the specified Sui address, across both packages. `listOwnedObjects` returns one page at a time, so `listAllOwnedObjects` follows `hasNextPage` and `cursor` to the end. An address with more links than a single page holds otherwise looks like it has fewer.
...
The `type` field on each object indicates which blockchain network the wallet is linked to:
