...
description: >-
  Learn how zkLogin wallets work on Sui, explore examples like Enoki and the
  Playtron wallet, and integrate zkLogin into your app using the Sui TypeScript
  SDK.
keywords:
  - zkLogin
...
goal:
  description: >-
    Reader can learn how zkLogin wallets work on Sui, explore examples like
...
questions:
  - What is zkLogin Wallets in Sui?
  - How does zklogin wallets work on Sui?
...
4. The app uses the JWT, a [per-user salt](/sui-stack/zklogin-integration/zklogin#user-salt), and the issuer URL to [derive a stable Sui address](/sui-stack/zklogin-integration/zklogin#address-definition) for the user. The same credential always produces the same address for a given issuer and salt.
5. Transactions are signed with the [ephemeral private key](/sui-stack/zklogin-integration/zklogin#ephemeral-private-key) and submitted alongside the zero-knowledge proof. Validators verify the proof and execute the transaction.
...
The `computeZkLoginAddressFromSeed` function normalizes the `iss` (issuer) value before computing the address through the internal [`normalizeZkLoginIssuer` helper](https://github.com/MystenLabs/ts-sdks/blob/main/packages/sui/src/zklogin/utils.ts). This normalization handles a specific case: Google's OpenID configuration returns the issuer as `accounts.google.com` (without a protocol prefix), but the zkLogin address computation expects `https://accounts.google.com`. The SDK prepends `https://` automatically for Google. For all other providers, the `iss` value passes through unchanged.
...
- **Mysten Labs prover:** A publicly accessible proving service maintained by Mysten Labs. Suitable for Testnet and Devnet development. See the [zkLogin integration guide](/sui-stack/zklogin-integration/integration-guide) for the endpoint and request format.
