...
When using Enoki with dApp Kit, the `useCurrentAccount` hook returns a Wallet Standard account populated by the Enoki wallet. The account exposes `address`, `chains`, `features`, `publicKey`, and `icon` — the standard fields defined by the Wallet Standard, not OAuth-specific identity fields.
