...
goal:
  description: >-
    Reader understands tokens do not allow transfers, conversions, or spends by
    default. To allow these actions, use an `ActionRequest`
  requires:
    - has_frontmatter:
        - title
        - description
    ...
    - headings:
        - pattern: Protected actions
        - pattern: ActionRequest structure
        - pattern: 'Request confirmation \{#request-confirma'
