...
title: Spending
description: >-
  Closed-Loop Tokens lack the `store` ability, so they use a `spend` method that
  consumes the token and moves its balance into an `ActionRequest` for
  confirmation or burning.
keywords:
  - token
  - store
  - spend
