...
Import the DeepBook Margin SDK and initialize the client before calling any margin manager functions.
...
After initializing the client, use `marginClient.marginContract` to access margin manager functions and add them to a `Transaction` object.
...
Use `managerState` to check a manager's current risk ratio, balances, and debt before executing trades or withdrawals.
...
readManagerState = async (managerKey: string) => {
	...
	const state = await marginClient.marginContract.managerState(managerKey)(tx);
	console.log('Risk ratio:', state.riskRatio);
	console.log('Base balance:', state.baseBalance);
	console.log('Quote balance:', state.quoteBalance);
	console.log('Borrowed base shares:', state.borrowedBaseShares);
	console.log('Borrowed quote shares:', state.borrowedQuoteShares);
	return state;
};