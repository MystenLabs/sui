...
## How do I set up the Margin Manager SDK?

Install the DeepBook Margin SDK and import the margin manager functions before calling any of the methods described on this page.
...
const marginClient = new DeepBookMarginClient({
  suiClient,
  // Additional config options: poolKeys, managerKeys, etc.
});
```

After you create the client, you can call any of the margin manager functions by passing a `Transaction` object to the returned function.

## What methods does the Margin Manager SDK provide?

The SDK exposes transaction-building functions and read-only query functions. Transaction functions return a callback that accepts a `Transaction` object. You add that callback to the transaction using `tx.add(...)`. Read-only functions query onchain state without modifying it.
...
Use these functions to repay borrowed assets. If no amount is specified, the function repays the maximum available balance up to the total debt. The call returns a function that takes a `Transaction` object.
...
Use `newMarginManagerWithInitializer` when you want to deposit collateral in the same transaction that creates the manager. Call `depositDuringInitialization` before `shareMarginManager` to fund the account before it becomes publicly accessible.

```tsx
import { Transaction } from '@mysten/sui/transactions';

createManagerWithDeposit = async (tx: Transaction) => {
	const poolKey = 'SUI_DBUSDC';

	// Step 1: Create the manager and get the initializer
	const { manager, initializer } =
		this.marginContract.newMarginManagerWithInitializer(poolKey)(tx);

	// Step 2: Deposit SUI collateral before sharing
	tx.add(
		this.marginContract.depositDuringInitialization({
			manager,
			poolKey,
			coinType: 'SUI',
			amount: 50,
	...
	// Step 3: Share the manager so it becomes accessible onchain
	tx.add(this.marginContract.shareMarginManager({ poolKey, manager, initializer }));
};
```

### Query manager state

Use `managerState` to read the current risk ratio, balances, and debt information for a margin manager before executing a trade or withdrawal.

```tsx
import { Transaction } from '@mysten/sui/transactions';

queryManagerState = async (managerAddress: string, poolKey: string) => {
	const tx = new Transaction();
	tx.add(this.marginContract.managerState(managerAddress, poolKey));
