...
Place limit and market orders through a margin manager. Orders can be placed whether or not margin trading is enabled for the pool.
...
Place reduce-only orders that can only decrease your debt position. These orders are useful when margin trading is enabled and you need to close existing positions.
