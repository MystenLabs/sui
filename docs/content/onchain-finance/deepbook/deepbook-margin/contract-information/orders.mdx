...
Place reduce-only orders that can only decrease your debt position. Regular limit and market orders can only be placed if margin trading is enabled for the pool, but reduce-only orders remain available when margin trading is disabled, so you can still close existing positions.
