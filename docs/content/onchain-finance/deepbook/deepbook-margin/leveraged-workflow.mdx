...
:::caution[Testnet status as of 2026-08-07]

You cannot complete this walkthrough on Testnet right now. Every Pyth feed the Testnet margin pools depend on (SUI, DBUSDC, DEEP, DBTC) is roughly 73 hours stale, and the margin protocol caps `max_price_age` at 1 hour, so every call that values a position reverts on the age check. Refreshing does not help, because the update Hermes serves carries the same old publish time. Steps up to and including creating a manager still work; deposit, borrow, order placement, and risk reads do not. Recheck the feed age before you start, as [Keep the price feeds fresh](#keep-the-price-feeds-fresh) shows.
