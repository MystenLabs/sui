...
:::info[Testnet support]

The setup steps run on Testnet today: reading the pool's risk parameters, checking borrow liquidity, and creating a `MarginManager` all work.

The steps that value a position (deposit, borrow, order placement, and risk reads) additionally need fresh Pyth prices, and that support is not yet in place on Testnet. The margin protocol caps `max_price_age` at 1 hour, and the Testnet feeds the margin pools depend on currently publish less often than that, so those calls revert on the age check rather than returning a stale value. Check the current feed age before you start, as [Keep the price feeds fresh](#keep-the-price-feeds-fresh) shows, and run the oracle-dependent steps once the feeds are publishing inside the pool's window.
...
## Keep the price feeds fresh

Every margin operation that values a position (borrow, withdraw, order placement, risk read) reads the pool's Pyth `PriceInfoObject` for both the base and the quote coin, and reverts if that price is older than the pool's maximum age. The [`pool_proxy`](https://github.com/MystenLabs/deepbookv3/blob/main/packages/deepbook_margin/sources/pool_proxy.move) entry points take both price objects as arguments. Spot trading has no such dependency, so this is the step that most often surprises people coming from the spot workflow.

The SDK refreshes the feeds for you as part of the transaction that depends on them. [`getPriceInfoObjects`](https://github.com/MystenLabs/ts-sdks/blob/main/packages/deepbook-v3/src/queries/priceFeedQueries.ts) returns the existing object IDs when the onchain price is still fresh, and appends the Pyth verify-and-update commands to your transaction when it is stale:

<ImportContent
	source="packages/deepbook-v3/src/queries/priceFeedQueries.ts"
	mode="code"
	org="MystenLabs"
	repo="ts-sdks"
	fun="getPriceInfoObjects"
	signatureOnly
/>

Call it on the same `Transaction` you are about to add the margin command to, and call it first. Because it fetches from Pyth over the network it is async, so it has to run before you build the rest of the transaction.

To decide whether a read-only call is about to revert, check the age directly. Read-only calls cannot append an update, because they do not build a transaction:
...
Liquidation is permissionless, and two entry points reach it. `margin_manager::liquidate` is the direct call: the liquidator supplies the repay coin and receives your collateral back, including the user liquidation reward. `liquidation_vault::liquidate_base` and its quote counterpart wrap that call, funding the repayment from a shared vault and returning every proceed to the vault rather than to whoever called it. Neither takes a capability. See [Liquidation paths and who gets paid](/onchain-finance/deepbook/deepbook-margin/margin-integration#liquidation-paths-and-who-gets-paid) for the full contrast.
...
Both routes end in the inner `margin_manager::liquidate`, which does the settlement: it repays part of your debt and pays the caller and the pool a reward out of your collateral, sized to restore your risk ratio to the target liquidation ratio rather than closing the whole position. Note that the vault entry returns silently when the position is not liquidatable, whereas the direct call aborts with `ECannotLiquidate`.
...
- **Valuation reverts on a stale price:** the Pyth price is older than the pool's maximum price age. Append `getPriceInfoObjects` to the same transaction, as in [Keep the price feeds fresh](#keep-the-price-feeds-fresh). See [Oracle risk](/onchain-finance/deepbook/deepbook-margin/margin-risks#oracle-risk).
