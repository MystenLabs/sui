...
The SDK refreshes the feeds for you as part of the transaction that depends on them. `getPriceInfoObjects` returns the existing object IDs when the onchain price is still fresh, and appends the Pyth verify-and-update commands to your transaction when it is stale:
