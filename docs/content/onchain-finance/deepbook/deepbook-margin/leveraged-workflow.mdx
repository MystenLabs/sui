...
Refresh the feeds in the same transaction as the operation that depends on them:

<ImportContent source="examples/deepbook-margin/src/price-feeds.ts" mode="code" tag="refresh-prices" />

The call is a no-op when the onchain price is still fresh, so it is safe to run before every margin transaction. To combine it with an order:

<ImportContent source="examples/deepbook-margin/src/price-feeds.ts" mode="code" tag="order-with-fresh-prices" />

Read-only calls such as `getMarginManagerState` cannot append an update, because they do not build a transaction. Check the age before you call one:

<ImportContent source="examples/deepbook-margin/src/price-feeds.ts" mode="code" tag="price-age" />
