...
The client references pools, coins, and margin pools by SDK key. On Testnet it auto-loads the margin package IDs and Pyth config. Read-only calls work without a manager; all trading operations require the margin manager to be associated with the correct DeepBook pool.
