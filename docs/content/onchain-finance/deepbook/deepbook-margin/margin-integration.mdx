...
answer: >-
  ...
  Anyone can liquidate positions at the pool's liquidation risk ratio through
  the margin liquidation vault. Index the MarginManager and MarginPool events to
  track positions, loans, and liquidations.
builder_paths:
...
Every pool proxy order, and every operation that values a position (borrow, withdraw, risk read), takes Pyth `PriceInfoObject` arguments and reverts on a price older than the pool's maximum age. Spot orders take no oracle. This is the largest day-to-day difference: a margin integration keeps price feeds fresh, and any valuing call fails if they are stale. Append the refresh to the same transaction with `getPriceInfoObjects`, as in [Keep the price feeds fresh](/onchain-finance/deepbook/deepbook-margin/leveraged-workflow#keep-the-price-feeds-fresh). See [Oracle risk](/onchain-finance/deepbook/deepbook-margin/margin-risks#oracle-risk).
...
## Liquidation paths and who gets paid

Anyone can liquidate any position your integration opens once its risk ratio falls to the pool's liquidation risk ratio. Two entry points do this, in two different packages, and they differ in who funds the repayment and who keeps the proceeds. Choose deliberately, because the economics are opposite.

| **Aspect** | **`margin_manager::liquidate`** | **`liquidation_vault::liquidate_base` and `_quote`** |
| --- | --- | --- |
| Package | Margin package | Separate `margin_liquidation` package |
| Repay capital | You supply a `Coin<DebtAsset>` | The shared liquidation vault supplies it |
| Proceeds | Returned to you as base, quote, and leftover debt coins, including the user liquidation reward | Deposited back into the vault. The caller receives nothing |
| Caller cost | Gas plus the repay capital you commit | Gas only |
| Not liquidatable | Aborts with `ECannotLiquidate` | Returns silently, emitting no event and changing no state |

Both are permissionless in the sense that neither takes a capability. The vault functions gate only their `swap_*` siblings on an authorized trader, not the liquidation calls themselves. But permissionless to call is not the same as profitable to call: a third party who calls the vault path pays gas and hands every proceed to the vault. The direct call is the one with keeper economics.

For an integrator this means two things. First, no grace period protects the positions you build, so surface the risk ratio and its distance to the liquidation threshold to your users, and act early. Second, if you run a liquidator, use `margin_manager::liquidate`, hold the debt asset in size, and keep price feeds fresh, because the same oracle staleness that blocks a valuation blocks a liquidation. See [Monitoring Runbook](/onchain-finance/deepbook/deepbook-margin/monitoring-runbook) for watching positions approach that threshold. The [Leveraged Position Workflow](/onchain-finance/deepbook/deepbook-margin/leveraged-workflow#what-happens-if-you-do-nothing-and-the-price-moves-against-you) shows the onchain entry point, and [Margin Risks](/onchain-finance/deepbook/deepbook-margin/margin-risks#how-liquidation-works) works the consequences through with numbers.
