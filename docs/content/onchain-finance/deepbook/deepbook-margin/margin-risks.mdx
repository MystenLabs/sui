...
builder_paths:
  - path_id: defi-deepbook
    ...
    stage: Margin
questions:
  - What is Margin Risks in Sui?
  - How does margin risks work on Sui?
...
- **Price delays:** Oracle prices might lag behind real market prices during extremely volatile periods. The protocol mitigates this by rejecting prices older than the pool's maximum price age, but brief delays within that window can still occur.
...
- **Staleness protection:** Prices older than the pool's maximum price age are automatically rejected, preventing liquidations based on stale data. That age is a per-pool governance parameter, not a constant: it defaults to 5 minutes and governance can set it anywhere between 30 seconds and 1 hour. Read the value for your pool onchain rather than assuming one, as [Keep the price feeds fresh](/onchain-finance/deepbook/deepbook-margin/leveraged-workflow#keep-the-price-feeds-fresh) shows
