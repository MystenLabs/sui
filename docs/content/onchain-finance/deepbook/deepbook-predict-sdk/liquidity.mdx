...
`predict` version 3 adds `plp::add_usdc_to_plp`, which donates USDC into the pool without minting shares, so the whole amount accrues to existing holders. The SDK has no builder for it as of 2.6.1, and the generated `plpMoveCalls` bindings do not carry it either, because they were generated before that version.

To call it, build the `moveCall` yourself against the Testnet version 3 call target on [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#testnet). It takes the `PoolVault`, the `ProtocolConfig`, and a `Coin<USDC>` of at least 10 USDC. [Add USDC without minting shares](/onchain-finance/deepbook/deepbook-predict/contract-information/vault#add-usdc) covers the admission rules, including the price-ceiling bound that rejects a contribution large enough to push the mark outside the executable band. Mainnet is at version 2 and does not carry the call.
