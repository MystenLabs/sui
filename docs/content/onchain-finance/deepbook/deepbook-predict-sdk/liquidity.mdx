...
Both floors default to none. A request without `minPlpOut` or `minUsdcOut` fills at whatever mark the next flush computes, so set a floor at the price where you would rather take the fill than the refund.
