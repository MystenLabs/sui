...
answer: >-
  ...
  Predict package version 2, which is live on Testnet only. `tx.redeem` closes a live position with no
...
`tx.mintCost` and `read.quoteMintCost` call an entrypoint that arrived in Predict package version 2. That version is live on Testnet only, so both calls abort on Mainnet, which is still at version 1. See [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids).
