...
The snippet below still carries the SDK's own comment saying Testnet only, which the Mainnet upgrade has since overtaken. One genuine lag remains, in the other direction: 2.6.1 resolves Testnet `predict` at its version 2 address, and Testnet is now at version 3. That call target keeps working while the Testnet `ProtocolConfig` watermark stays at or below 2, and stops when an admin raises it. See [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids).
