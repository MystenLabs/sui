...
verified_against: publication records at deepbookv3 36783242 (Mainnet predict/sessions v2; Testnet predict v3, sessions v2); '@mysten/deepbook-v3' 2.6.1 at ts-sdks 56d51645
goal:
  description: >-
...
answer: >-
  ...
  and 5m cadences. Every package is past version 1, so each one's Move call
  target is its latest address while its type tags keep its original one. The
  networks are not in lockstep: Testnet predict is at version 3, which adds
  add_usdc_to_plp, and Mainnet predict is at version 2. The
  @mysten/deepbook-v3 SDK ships both deployments and reads every value without
  an indexer, though 2.6.1 still resolves Testnet predict at its version 2
  address.
---
...
Testnet `predict` then went one further, to version 3, from source commit [`153fe3d6`](https://github.com/MystenLabs/deepbookv3/tree/153fe3d6535cdfe79e34272baeec503c5eeda14e/packages/predict), which adds [`plp::add_usdc_to_plp`](/onchain-finance/deepbook/deepbook-predict/contract-information/vault#add-usdc). So the networks carry the same surface except for that one call:

| **Package** | **Mainnet** | **Testnet** |
| --- | --- | --- |
| `predict` | 2 | 3 |
| `sessions` | 2 | 2 |
...
`@mysten/deepbook-v3` 2.6.1 predates the Testnet `predict` version 3 publication and still resolves `0x30a03c33…`, the version 2 address, for that network. Calls through it keep working while the Testnet `ProtocolConfig` watermark stays at or below 2, and stop the moment an admin raises it to 3, because the version gate on every [mint](/onchain-finance/deepbook/deepbook-predict/contract-information/predict#mint-a-position) and pool call requires the running package to be at or above the watermark. Mainnet's IDs in that release are current.
...
| Upgrade source commits | `8d7d42a0f48bc6434bc282d45d2403809b4e492c`, `153fe3d6535cdfe79e34272baeec503c5eeda14e` |
| `predict` package version | 3 |
| `sessions` package version | 2 |
...
`predict` is at version 3 here and `sessions` at version 2, so each carries 2 addresses. Call the **call target**; build every type tag from the **original ID**. The other 5 packages are at version 1, where the 2 columns coincide:

| **Package** | **Call target** | **Original ID** | **Contents** |
| --- | --- | --- | --- |
| `predict` | `0xb5155d4cd83f27ac2fad2c2ef339988b115ffef7830283c9acd68359afeba5b8` | `0x59d71119e990573a738dd3ff9c4c7d28d6893af69c87c1a7f3a2e90e280ce2f4` | Markets, trading, protocol configuration, registry, and the PLP pool |
...
The entrypoint surface is identical across the networks except for one call. `expiry_market::mint_exact_cost`, `expiry_market::quote_mint_exact_cost_for_account`, and the `sessions::mint_exact_cost` wrapper all arrived at package version 2, which both networks run. `plp::add_usdc_to_plp` arrived at `predict` version 3, which only Testnet runs, so it is not callable on Mainnet.
...
- **Package version:** the networks are not in lockstep. Testnet `predict` is at version 3 and Mainnet's at version 2, and the 2 packages were not upgraded together either. Read a package's live version rather than assuming the networks match, and remember that an older call target keeps working only until the `ProtocolConfig` watermark passes it.
