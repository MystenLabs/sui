...
verified_against: deepbook-predict-mainnet 7b169bde (predict v1); deepbook-predict-testnet 4d752fb8 upgraded to predict/sessions v2 at 8d7d42a0; deployment records at deepbookv3 4e9bfe90
goal:
  description: >-
...
answer: >-
  ...
  and 5m cadences. Testnet runs predict and sessions at package version 2, so
  its Move call targets are the upgraded package IDs while its type tags keep
  the original ones; Mainnet is still at version 1, where the two coincide. The
...
The SDK carries both: `packages.predict` and `sessionsPackageId` are the call targets, `packages.predictV1` and `sessionsPackageIdV1` the original IDs. On Mainnet, where both packages are still at version 1, each pair holds the same value.
...
| Network | Mainnet |
| Chain ID | `35834a8a` |
| Source commit | `7b169bde2c2a25fee3ac9aa4686e346309bfe570` |
| `predict` package version | 1 |
| Indexing start checkpoint | `321804009` |

### Mainnet packages {#packages}

Predict is 7 packages rather than one. Move call targets, event type prefixes, and the PLP coin type all derive from these IDs. Every Mainnet package is still at version 1, so each one's call target and original ID are the same address and the distinction in [Upgraded packages carry 2 IDs](#two-ids) does not bite here. On Mainnet the `usdc` entry is Circle's native USDC package, which Predict links against rather than publishes:

| **Package** | **ID** | **Contents** |
| --- | --- | --- |
| `predict` | `0x89aea622e7bb3bdd598bde87dde40ee31c9eed4971b9546e23ec83de3c48bbba` | Markets, trading, protocol configuration, registry, and the PLP pool |
| `propbook` | `0xa6c8f32015b5b41d34ee09995a2e9d7a21cdecf0e1910a50b264fd252cd33831` | Oracle registry, Pyth feeds, and Block Scholes stores |
| `account` | `0x4e1dd01465713c9d832313fed5f45c222a4d5c62a533d6da96764c8b2a245d58` | Account custody, balances, and the `Auth` authorization type |
| `fixedMath` | `0x52ec2d263bb9ad545d1be1c00f6779cc577ed7c0408cdea8d589c2664adc22db` | Fixed-point arithmetic used by the pricing math |
| `deepbookCoreAccount` | `0xd71b5a341dc8dc7e187517849edf61e54670b60fe496e07186c09a65fa7afdb7` | DeepBook core account app type |
| `sessions` | `0x9a068beffa019ae756f4a6d2611e9899d3c6dd888812dd9eca87535d2ac55e2a` | Session support |
...
The entrypoint surface is no longer identical across the networks. `expiry_market::mint_exact_cost`, `expiry_market::quote_mint_exact_cost_for_account`, and the `sessions::mint_exact_cost` wrapper exist only at package version 2, so they are callable on Testnet and not on Mainnet. Every other entrypoint behaves the same on both.
