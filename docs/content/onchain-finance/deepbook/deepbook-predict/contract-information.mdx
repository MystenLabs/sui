...
verified_against: publication records at deepbookv3 abb4cfd9 (Mainnet predict publish 3, Testnet predict publish 4, sessions publish 2 on both; predict protocol version 3, sessions 2); '@mysten/deepbook-v3' 2.6.1 at ts-sdks 56d51645
goal:
  description: >-
...
answer: >-
  ...
  and 5m cadences. Every package has been upgraded at least once, so each one's
  Move call target is its latest published address while its type tags keep its
  original one. Both networks run the same code today: predict protocol version
  3 and sessions protocol version 2. The @mysten/deepbook-v3 SDK ships both
  deployments and reads every value without an indexer, though 2.6.1 predates
  the latest predict publication on each network and still resolves the
  superseded address for both.
---

DeepBook Predict runs on Sui Mainnet and Sui Testnet as 2 separate deployments, `deepbook-predict-mainnet` and `deepbook-predict-testnet`. Version 2.6.1 or later of [`@mysten/deepbook-v3`](https://www.npmjs.com/package/@mysten/deepbook-v3) carries both, so `getConfig` returns the identifiers on this page for either network. The Mainnet publish comes from DeepBookV3 source commit [`7b169bde`](https://github.com/MystenLabs/deepbookv3/tree/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/predict), and the original Testnet publish from source commit [`4d752fb8`](https://github.com/MystenLabs/deepbookv3/tree/4d752fb82d909a821c85bcc5d3963725efb546f4/packages/predict), whose Move sources are byte-identical to the Mainnet ones.

Both networks have since upgraded `predict` and `sessions` to version 2 from source commit [`8d7d42a0`](https://github.com/MystenLabs/deepbookv3/tree/8d7d42a0f48bc6434bc282d45d2403809b4e492c/packages/predict), which adds the [`mint_exact_cost`](/onchain-finance/deepbook/deepbook-predict/contract-information/predict#mint-a-position) all-in budget entrypoint and its session wrapper, callable on either network.

Both have since taken a further `predict` upgrade, to protocol version 3, which adds [`plp::add_usdc_to_plp`](/onchain-finance/deepbook/deepbook-predict/contract-information/vault#add-usdc). So the 2 networks run the same code and expose the same entrypoints.
...
### Publish count is not protocol version {#version-numbers}

Two different numbers get called "version", and they have come apart:

| **Package** | **Publishes** | **Protocol version** |
| --- | --- | --- |
| Mainnet `predict` | 3 | 3 |
| Testnet `predict` | 4 | 3 |
| `sessions`, both networks | 2 | 2 |

- **Publishes** is what `Published.toml` records: how many times that package has been published on that network. It only counts events, so a Testnet-only rehearsal of an upgrade leaves Testnet permanently a publish ahead.
- **Protocol version** is `constants::current_version!()`, compiled into the code. It gates behavior: [`assert_version`](#version-gate) aborts unless it is at or above the `ProtocolConfig` watermark, and it is what determines whether an entrypoint exists.

Testnet `predict` is on its 4th publish and Mainnet's on its 3rd, yet both run protocol version 3 and behave identically. Compare protocol versions, never publish counts, when you want to know what a network can do.

### The version gate {#version-gate}

Every version-gated call runs `assert_version`, which aborts with `EPackageVersionDisabled` unless `constants::current_version!() >= ProtocolConfig.version_watermark`. An older package therefore keeps working until an admin raises the watermark past it, at which point every call through it stops. That is what makes a stale call target in a client library a deferred failure rather than an immediate one.
...
The SDK carries both: `packages.predict` and `sessionsPackageId` are the call targets, `packages.predictV1` and `sessionsPackageIdV1` the original IDs. All 4 now differ from their call targets, because every package is past version 1 on both networks.

:::caution

`@mysten/deepbook-v3` 2.6.1 predates the latest `predict` publication on both networks. It resolves `0x1cacb9bf…` on Mainnet and `0x30a03c33…` on Testnet, and both are superseded addresses running protocol version 2. Its `sessions` IDs are current on both.

Those predict targets still work, because protocol version 2 is at or above each network's current watermark, but they cannot reach anything added at protocol version 3, so `plp::add_usdc_to_plp` is unreachable through the SDK on either network. They stop working outright once an admin raises a watermark to 3. Take the current call targets from the tables below until a release ships them.
...
| Upgrade source commits | `8d7d42a0f48bc6434bc282d45d2403809b4e492c`, `9a32854c` |
| `predict` publishes / protocol version | 3 / 3 |
| `sessions` publishes / protocol version | 2 / 2 |
| Indexing start checkpoint | `321804009` |

### Mainnet packages {#packages}

Predict is 7 packages rather than one. Move call targets, event type prefixes, and the PLP coin type all derive from these IDs. `predict` and `sessions` have both been upgraded here, so each carries 2 addresses: call the **call target**, and build every type tag from the **original ID**. The other 5 packages have never been upgraded, so the 2 columns coincide. The `usdc` entry is Circle's native USDC package, which Predict links against rather than publishes:

| **Package** | **Call target** | **Original ID** | **Contents** |
| --- | --- | --- | --- |
| `predict` | `0x08fa3ef1b047d87b0b4ce1c7e5f8b42d8bfdea9a70547efe3cce5d8d6e47ee53` | `0x89aea622e7bb3bdd598bde87dde40ee31c9eed4971b9546e23ec83de3c48bbba` | Markets, trading, protocol configuration, registry, and the PLP pool |
...
| Upgrade source commits | `8d7d42a0f48bc6434bc282d45d2403809b4e492c`, `153fe3d6535cdfe79e34272baeec503c5eeda14e`, `9a32854c` |
| `predict` publishes / protocol version | 4 / 3 |
| `sessions` publishes / protocol version | 2 / 2 |
...
`predict` is on its 4th publish here and `sessions` on its 2nd, so each carries 2 addresses. Call the **call target**; build every type tag from the **original ID**. The other 5 packages have never been upgraded, so the 2 columns coincide:

| **Package** | **Call target** | **Original ID** | **Contents** |
| --- | --- | --- | --- |
| `predict` | `0x6c2c2d3c2394cf282f4b8462a99c2e814bda0c223796de10b47b36d35fd878f5` | `0x59d71119e990573a738dd3ff9c4c7d28d6893af69c87c1a7f3a2e90e280ce2f4` | Markets, trading, protocol configuration, registry, and the PLP pool |
...
As on Mainnet, the PLP type carries `predict`'s original ID, `0x59d71119…`, not its current call target. Deriving a coin type by string-concatenating the call target produces a type that does not exist.
...
The entrypoint surface is identical across the networks. `expiry_market::mint_exact_cost`, `expiry_market::quote_mint_exact_cost_for_account`, and the `sessions::mint_exact_cost` wrapper arrived at protocol version 2; `plp::add_usdc_to_plp` arrived at `predict` protocol version 3. Both networks run both versions, so every entrypoint is callable on either.
...
- **Package version:** the networks run the same protocol versions today, but they have not been upgraded in lockstep and need not be. Compare protocol versions rather than publish counts, as [Publish count is not protocol version](#version-numbers) explains, and remember that an older call target keeps working only until the `ProtocolConfig` watermark passes it.
