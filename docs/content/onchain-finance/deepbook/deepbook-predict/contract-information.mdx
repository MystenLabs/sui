...
verified_against: deepbook-predict-mainnet 7b169bde, predict upgraded to v2 (sessions still v1); deepbook-predict-testnet 4d752fb8, predict and sessions upgraded to v2; upgrade source 8d7d42a0; publication records at deepbookv3 d0913647
goal:
  description: >-
...
answer: >-
  ...
  and 5m cadences. The predict package is at version 2 on both networks and
  sessions at version 2 on Testnet only, so an upgraded package's Move call
  ...
  an indexer, though version 2.6.0 still addresses Mainnet predict at its
  version 1 address.
---
...
So `expiry_market::mint_exact_cost` is callable on both networks, while the `sessions::mint_exact_cost` wrapper is callable only on Testnet, because Mainnet's `sessions` package has not been upgraded.
...
The SDK carries both: `packages.predict` and `sessionsPackageId` are the call targets, `packages.predictV1` and `sessionsPackageIdV1` the original IDs. A pair holds the same value only where the package is still at version 1, which today means Mainnet `sessions`.

:::caution

`@mysten/deepbook-v3` 2.6.0 predates the Mainnet `predict` upgrade and still carries `0x89aea622…`, the version 1 address, as its Mainnet call target. Until a release ships the version 2 address, the facade's `tx.mintCost` and `read.quoteMintCost` address a package that has no `mint_exact_cost`, so they fail on Mainnet. A `moveCall` you build yourself against the version 2 call target below works. Every other Mainnet call is unaffected, because an original ID stays a valid call target for the functions that existed at version 1.
...
| `sessions` package version | 1 |
| Indexing start checkpoint | `321804009` |

### Mainnet packages {#packages}

Predict is 7 packages rather than one. Move call targets, event type prefixes, and the PLP coin type all derive from these IDs. `predict` is at version 2 here, so it carries 2 addresses: call the **call target**, and build every type tag from the **original ID**. The other 6 packages are at version 1, where the 2 columns coincide. The `usdc` entry is Circle's native USDC package, which Predict links against rather than publishes:
...
| `sessions` | `0x9a068beffa019ae756f4a6d2611e9899d3c6dd888812dd9eca87535d2ac55e2a` | Same | Session support, still at version 1 |
...
The upgrade kept `predict`'s original ID, so every object, position, and account created before it stays valid, and the PLP coin type below is unchanged.
...
The PLP type carries `predict`'s original ID, not its v2 call target. Deriving a coin type by string-concatenating the call target produces a type that does not exist. Take it from `getConfig(network).coinTypes.plp`.
...
The entrypoint surface is almost identical across the networks. `expiry_market::mint_exact_cost` and `expiry_market::quote_mint_exact_cost_for_account` arrived at `predict` version 2, which both networks now run, so both are callable on either. The `sessions::mint_exact_cost` wrapper arrived at `sessions` version 2, which only Testnet runs, so a session key can size a mint by all-in budget on Testnet and not on Mainnet. Every other entrypoint behaves the same on both.
...
- **Package version:** `predict` is at version 2 on both networks and `sessions` at version 2 on Testnet only, so the `mint_exact_cost` session wrapper exists on Testnet alone. Read a package's live version from the `ProtocolConfig` watermark rather than assuming the 2 networks match.
