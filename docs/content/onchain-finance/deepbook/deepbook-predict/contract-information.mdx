...
answer: >-
  ...
  publishes seven packages and shares its own Registry, ProtocolConfig,
...
DeepBook Predict runs on Sui Mainnet and Sui Testnet as two separate deployments, `deepbook-predict-mainnet` and `deepbook-predict-testnet`. Version 2.3.0 or later of [`@mysten/deepbook-v3`](https://www.npmjs.com/package/@mysten/deepbook-v3) carries both, so `getConfig('mainnet')` and `getConfig('testnet')` each return the identifiers on this page. Both deployments are built from the [`deepbook-predict-mainnet` branch](https://github.com/MystenLabs/deepbookv3/tree/deepbook-predict-mainnet/packages/predict) of the DeepBookV3 repository. The Testnet sources are identical to the Mainnet source commit apart from two lines of doc comment and attribute in `packages/propbook/sources/feeds/pyth_feed.move`, so one source pin serves both networks.

:::caution

Every identifier on this page belongs to one of the two current deployments. Earlier Testnet deployments are superseded, and their accounts, positions, and markets are not visible on the current ones, so ignore identifiers from them in local configs and scripts.

As of 2026-09-10 the Mainnet deployment is published and configured, its pool holds only the bootstrap minimum, and no market has been tradable there yet. Read `read.markets()`, `read.pool()`, and the `ProtocolConfig` object rather than assuming either network's state.
...
Predict is seven packages rather than one. Move call targets, event type prefixes, and the PLP coin type all derive from these IDs. On Mainnet the `usdc` entry is Circle's native USDC package, which Predict links against rather than publishes:
...
| `protocolConfig` | `0x55e8800bcb31b792683ca4385bd533716e2a9721364236a69ee2c3bee61dd8f2` | `deepbook_predict::protocol_config::ProtocolConfig`: tunables, fee rates, freshness windows, the no-trade and valuation windows, pause and freeze flags, and the version watermark |
...
| `deepbookRegistry` | `0xaf16199a2dff736e9f07a845f23c5da6df6f756eddb631aed9d24a93efc4549d` | DeepBook core registry referenced by the `deepbookCoreAccount` app authorization |
...
| `usdc` | `0xc028557a1ed49e42ed091e115aedefd70a442b184c18fbec5c48d5b6c0b8c184` | Test USDC, published by the deployment |
...
Each `ExpiryMarket` is its own shared object, created per underlying and expiry. Market IDs are not deployment configuration: read them from `registry::expiry_market_id` onchain or from `read.markets()` at runtime. The PLP type is defined by each network's Predict package, so it differs per network and changes with every redeployment. The quote coin type differs per network too, so always take it from `getConfig(network).quoteCoinType` rather than from a symbol.
...
Passing an oracle object that is not the current canonical binding aborts rather than pricing against the wrong feed, so resolve the three oracle IDs from `OracleRegistry` at runtime instead of trusting a cached copy. See [Oracle](/onchain-finance/deepbook/deepbook-predict/contract-information/oracle) for the binding accessors, the freshness windows, and which call needs which object.
...
On both networks' `AccountRegistry`, `PredictApp`, `SessionsApp`, and `DeepbookCoreAccountApp` were authorized as of 2026-09-10. That is live registry state, so read it with `account_registry::is_app_authorized<App>` rather than assuming it.

The DeepBook core registry is a separate authorization, and it differs by network as of 2026-09-10. On Testnet, `DeepbookCoreAccountApp` is authorized on the DeepBook registry `0x7c256edbda983a2cd6f946655f4bf3f00a41043993781f8674a7046e8c0e11d1`. On Mainnet it is not authorized on the DeepBook registry `0xaf16199a2dff736e9f07a845f23c5da6df6f756eddb631aed9d24a93efc4549d`, so the session spot wrappers cannot work there yet. Neither state is permanent, and the authorization belongs to that registry's administrator rather than to Predict. Check it with `deepbook::registry::assert_app_is_authorized<DeepbookCoreAccountApp>`, which aborts when the app is not authorized. [Sessions](/onchain-finance/deepbook/deepbook-predict/contract-information/sessions#spot-wrappers) covers what that authorization gates.

## Units and constants

Predict mixes two scales, and confusing them is the fastest way to misprice a trade. Probabilities, prices, and rates are fixed-point integers, while coin amounts use the coin's own decimals. The units are identical on both networks:
...
Both deployments ship the same initial configuration, and both `ProtocolConfig` objects matched it field for field when read on chain on 2026-09-10. Configuration is mutable protocol state, so read the live values from `ProtocolConfig` and `registry::cadence_configs` rather than pinning the ones below.

### Cadences

Six cadence slots exist per underlying, addressed by cadence ID. A cadence is enabled when its window size is greater than zero, and only the 1m and 5m cadences are enabled:
...
- **Max expiry allocation:** the cadence cap on cash allocated to one expiry market, in USDC base units: 10,000 USDC. The market's inventory-impact curve is scaled by this same value.
- **Initial expiry cash:** the cash the vault seeds a new market with, in USDC base units: 2,000 USDC.
- **Window size:** how many expiries of this cadence can be live at once, so four markets are live at any time across the two enabled cadences. Zero disables the cadence.
...
| Block Scholes SVI freshness | `60000` ms | The SVI surface changes more slowly |
...
| `max_valuation_window_ms` | `300000` | A pool flush must finish within 5 minutes of its snapshot or be restarted |
| `max_lp_pool_value` | `500000000000` | The pool accepts at most 500,000 USDC of liquidity |
| `lp_request_limit_flush_attempts` | `1` | A queued request whose floor the flush mark misses is cancelled and refunded at the first miss |
| PLP supply fee | `0` | |
...
| EWMA congestion penalty | disabled | |

The strike-exposure template is unchanged from earlier deployments: base fee 10 percent, minimum fee 2.2 percent, entry probability band 1 percent to 99 percent, backing buffer lambda 0.31, inventory impact rate 0, and a 1 day expiry fee window with a 1x maximum multiplier. Every package is at version 1 on both networks.

## Public read APIs for the previous Testnet deployment

As of 2026-09-10 no indexed read service exists for `deepbook-predict-testnet` or for `deepbook-predict-mainnet`. The three public read services below still index the previous-generation `predict-8-21` Testnet deployment, so their responses carry that deployment's package and object IDs and none of the identifiers above. Nothing on the SDK path needs them: every `client.predict.read` method runs against a full node, and the event decoders parse a transaction result locally.

The tables stay here for readers who still work against `predict-8-21`, and because the shape of these services is what a future service for the current deployments is likely to follow. Each exposes read-only JSON `GET` endpoints with permissive browser CORS, and none accepts writes:
...
Market discovery returns the creation record for each live market. The following sample is trimmed to the market's identity and cadence terms, and its values are the previous deployment's:
...
The two deployments run the same contracts and the same initial configuration. What differs is around them:
...
- **Take decimals and scale from the configured deployment.** An amount scaled by a hardcoded exponent silently misprices every mint.
- **Read the admission grid from the market.** Admission tick size is mutable protocol state.
- **Treat timing you measure on Testnet as an observation rather than a guarantee.** Oracle update frequency and settlement latency are properties of the relayers, not of the contracts.

## Live onchain events

Stream events directly from a full node when a client needs them as they land, and back them with your own store for history. Predict emits its events from four modules under the Predict package: `config_events` for lifecycle and configuration, `order_events` for trading, `vault_events` for the pool and its queues, and `builder_code_events` for builder attribution. The following table lists the ones an integrator subscribes to most often, not every event each module declares:
...
| `vault_events::FlushExecuted` | The liquidity queues are drained at one frozen NAV |
...
Swap the method for `sui.rpc.v2.LedgerService/ListEvents` to page the same events historically. The filter is disjunctive normal form, so each `terms` block is ORed: the request above matches either event type and scopes both to one deployment.
...
Every module below is pinned to the Mainnet source commit `14a7e8f822e0397df2d61fbfbce3ef21086891c2`, which the [`deepbook-predict-mainnet`](https://github.com/MystenLabs/deepbookv3/tree/deepbook-predict-mainnet) branch carries. The Testnet deployment's sources are identical apart from two lines in `pyth_feed.move`, so the same links serve both networks. There is no `entry fun` in any Predict package: every integrator call is a `public fun` invoked as a `moveCall`, and several of them return values that a later command in the same transaction must consume.
...
Several tables were renumbered or extended on these deployments, so a code memorized from an earlier deployment can point at the wrong constant:
