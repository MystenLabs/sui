...
DeepBook Predict runs on Sui Mainnet and Sui Testnet as 2 separate deployments, `deepbook-predict-mainnet` and `deepbook-predict-testnet`. Version 2.5.0 or later of [`@mysten/deepbook-v3`](https://www.npmjs.com/package/@mysten/deepbook-v3) carries both, so `getConfig` returns the identifiers on this page for either network. The Mainnet publish comes from the [`deepbook-predict-mainnet` branch](https://github.com/MystenLabs/deepbookv3/tree/deepbook-predict-mainnet/packages/predict) of the DeepBookV3 repository, and the Testnet publish from the [`deepbook-predict-testnet` branch](https://github.com/MystenLabs/deepbookv3/tree/deepbook-predict-testnet/packages/predict). The 2 source commits carry byte-identical Move sources, so one source pin serves both networks.
...
Both deployments were published on 2026-09-12. As of 2026-09-14 neither runs a market service: the 4 markets each network created at deployment expired unsettled on 2026-09-12, no market has opened since, and no oracle update has landed since deployment. The Mainnet pool holds only the bootstrap minimum. Read `read.markets()`, `read.pool()`, and the `ProtocolConfig` object rather than assuming either network's state.
...
| Network | Testnet |
| Chain ID | `4c78adac` |
| Source commit | `4d752fb82d909a821c85bcc5d3963725efb546f4` |
...
| **Package** | **ID** | **Contents** |
| --- | --- | --- |
| `predict` | `0x59d71119e990573a738dd3ff9c4c7d28d6893af69c87c1a7f3a2e90e280ce2f4` | Markets, trading, protocol configuration, registry, and the PLP pool |
| `propbook` | `0xa83f9d7651de09672a40cea371c387bf954e0f3092947670d71428d4fbc9edd9` | Oracle registry, Pyth feeds, and Block Scholes stores |
| `account` | `0x1e57d6554b99e4ca68330322c3e5c409ba1b681642726b603bf1be92c1840ca9` | Account custody, balances, and the `Auth` authorization type |
| `fixedMath` | `0xf3a38ba24fd40173af692cef151a43850f5935a3e026ae037fc19bb81974c6a8` | Fixed-point arithmetic used by the pricing math |
| `deepbookCoreAccount` | `0x2a3a5f51e1c566e0a77c667596a8212d036b44aeac32790991c9eb97580fabb1` | DeepBook core account app type |
| `sessions` | `0x1908eee49d7a08d74a537d7f23766b363a145517fe0d3e0d85635d1682831ffd` | Session support |
...
- **Activity:** As of 2026-09-14 neither network runs a market service, so no market opens or settles on either until the services run again. The Mainnet pool holds only the bootstrap minimum. `read.markets()` lists live-and-unsettled markets, which includes an expired market nobody has settled, so check each entry's expiry before you quote against it.
...
The links below pin every module to the Mainnet source commit `7b169bde2c2a25fee3ac9aa4686e346309bfe570`, which the [`deepbook-predict-mainnet`](https://github.com/MystenLabs/deepbookv3/tree/deepbook-predict-mainnet) branch carries. The Testnet deployment's Move sources are byte-identical, so the same links serve both networks. There is no `entry fun` in any Predict package: every integrator call is a `public fun` invoked as a `moveCall`, and several of them return values that a later command in the same transaction must consume.

| **Area** | **Source** |
| --- | --- |
| Markets, quoting, trading, and settlement | [`packages/predict/sources/expiry_market.move`](https://github.com/MystenLabs/deepbookv3/blob/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/predict/sources/expiry_market.move) |
| Market catalog, cadences, and capabilities | [`packages/predict/sources/registry/registry.move`](https://github.com/MystenLabs/deepbookv3/blob/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/predict/sources/registry/registry.move) |
| Protocol configuration and gating | [`packages/predict/sources/config/protocol_config.move`](https://github.com/MystenLabs/deepbookv3/blob/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/predict/sources/config/protocol_config.move) |
| PLP pool, liquidity queues, and valuation | [`packages/predict/sources/plp/plp.move`](https://github.com/MystenLabs/deepbookv3/blob/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/predict/sources/plp/plp.move) |
| Pool valuation capability | [`packages/predict/sources/capabilities/pool_valuation_cap.move`](https://github.com/MystenLabs/deepbookv3/blob/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/predict/sources/capabilities/pool_valuation_cap.move) |
| Account custody and authorization | [`packages/account/sources/account.move`](https://github.com/MystenLabs/deepbookv3/blob/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/account/sources/account.move) |
| Oracle registry and bindings | [`packages/propbook/sources/registry.move`](https://github.com/MystenLabs/deepbookv3/blob/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/propbook/sources/registry.move) |
| Pyth and Block Scholes feeds | [`packages/propbook/sources/feeds`](https://github.com/MystenLabs/deepbookv3/tree/7b169bde2c2a25fee3ac9aa4686e346309bfe570/packages/propbook/sources/feeds) |
