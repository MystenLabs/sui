...
The registry also holds the 1h, 1d, 1w, and 1mo cadences in a disabled state, and a disabled cadence reads back as an all-zero configuration. A chain read on 2026-09-14 confirmed those values on both networks. Read the live values with `registry::cadence_config` or `registry::cadence_configs`, or as `tickSize` and `admissionTickSize` from `read.markets()`, rather than hardcoding them, because an admin can change a cadence and each market keeps the terms it received at creation.

## A position is a tick pair {#range-key}

Every mint function takes `lower_tick` and `higher_tick` as separate `u64` arguments, and every order event carries them the same way. Only the durable order ID packs the pair into one integer.
...
A one-sided range wins on an unbounded outcome, so it costs more per contract and wins more often. A two-sided range wins on a bounded outcome, so it costs less per contract and wins less often. Ranges suit a view on where a price settles rather than on which direction it moves. A two-sided range also carries 2 finite boundaries, so the protocol charges a trading fee at each and applies the entry-probability band at each, in addition to the band on the range itself.
...
`mint_exact_quantity`, `mint_exact_amount`, and `mint_exact_cost` all return a `u256` order ID. That integer carries the durable contract terms and nothing else, so the ID deliberately omits mint-only inputs such as entry probability, premium, and fee policy. Tightening admission policy in a later version therefore cannot retroactively invalidate an existing ID.
