...
Both the Mainnet and the Testnet deployment run one underlying, BTC, with two enabled cadences, and both cadences use the same grids:
...
The 1h, 1d, 1w, and 1mo cadences are configured and disabled, and a disabled cadence reads back as an all-zero configuration. Those values were verified on both networks on 2026-09-10. Read the live values with `registry::cadence_config` or `registry::cadence_configs`, or as `tickSize` and `admissionTickSize` from `read.markets()`, rather than hardcoding them, because an admin can change a cadence and each market keeps the terms it was created with.
