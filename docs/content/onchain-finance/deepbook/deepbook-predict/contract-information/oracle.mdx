...
Live pricing runs through a `Pricer`, which you build once per transaction from all four objects:
...
Loading a pricer first checks all three feed objects against the registry's current bindings, then checks that the market has not expired. A mismatch aborts before Predict reads any price:
