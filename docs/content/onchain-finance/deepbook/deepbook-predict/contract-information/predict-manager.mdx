...
- **Both IDs derive from the registry.** The wrapper address and the account identity are derived objects claimed from it, so they are deterministic and computable before the account exists.
...
Field order in the table is declaration order. Creation writes the two referrer fields once, and they never change, which is why you cannot add or move a referral relationship afterward.
...
Two consequences follow. You can always retrieve funds that are not committed to a live position. Risk tooling must not treat a Predict pause as a custody halt.
...
You can compose creating an account, funding it, and trading into one programmable transaction block, because the wrapper's object ID is derivable before the account exists. Two layers reach that, and they are not interchangeable.
...
All three constructors emit it, once per account:
