...
verified_against: deepbookv3 d0913647; predict v2 on both networks, upgraded from 8d7d42a0
---
...
`mint_exact_cost` arrived in `predict` package version 2, which both networks now run, so call it against the version 2 call target on either. The `sessions` wrapper of the same name is a separate matter: Mainnet's `sessions` package is still at version 1, so a session key cannot use this shape there. See [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids).
