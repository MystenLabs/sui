...
answer: >-
  ...
  mint_exact_cost, redeem_live, or redeem_settled with an account Auth.
  mint_exact_cost, new in package version 2 and now live on both networks,
  sizes the fill inside an all-in budget so fees come out of the spend rather
  than on top of it. Live trades close in the last no_trade_window_ms before expiry, and
  settlement is the permissionless try_settle.
last_verified: 2026-09-23
verified_against: deepbookv3 abb4cfd9; predict protocol version 3 and sessions 2 on both networks
---
...
The module lives at [`packages/predict/sources/expiry_market.move`](https://github.com/MystenLabs/deepbookv3/blob/76b11922f3144be5bdaacb66c38c7c3524a02ff3/packages/predict/sources/expiry_market.move) on DeepBookV3 `main`. The Move snippets on this page pin DeepBookV3 `main` at `76b11922`, whose sources match both deployments. For the package and object IDs of each network, see [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information).
...
`quote_mint_exact_cost_for_account`, new in package version 2 and callable on both networks, is the read-only twin of [`mint_exact_cost`](#mint-a-position): it runs the same budget search against the same state and returns the fill that budget buys, along with that fill's cost decomposition. It takes the account for the same reasons `quote_mint_for_account` does, plus one more: the chain caps the budget at the account's available USDC before sizing, and the quote reproduces that cap. It is the natural source of a `min_quantity` floor for the mint that follows. Like the other 2 quotes, it does not preflight expiry cash backing or exposure-index capacity.

Both quotes run the entry-probability band before they size anything. For a two-sided range the band applies 3 times: to the range probability, to the probability that settlement ends above the lower strike, and to the probability that it ends at or below the higher strike. An open end contributes no check, and the finite end of a directional position has the same probability as the position itself. The quoted `trading_fee` sums one fee for each finite boundary, as [Fee components](/onchain-finance/deepbook/deepbook-predict/design#fee-components) describes.
...
Positions open through 3 functions, one per sizing shape: an exact payout quantity, a premium budget, or an all-in budget. All 3 take the strike range as the tick pair `(lower_tick, higher_tick]`, consume an account `Auth`, and return the minted order ID as a `u256`. A `u256` has `drop`, so the transaction is free to ignore that return value; recover the ID from the `OrderMinted` event instead when you do not chain another command onto it:
...
`mint_exact_cost` arrived in `predict` package version 2, which both networks now run or exceed. Call it against the network's current call target, not the original package ID, which has no such function. See [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids).
...
`mint_exact_cost` fixes the **all-in** budget, which is the difference that matters. Where `mint_exact_amount` sizes on the pre-fee premium and then charges the trading fee, builder add-on, congestion surcharge, and inventory-impact charge on top of it, `mint_exact_cost` sizes inside the total: it caps `max_cost` at the account's available USDC, then searches for the largest lot-rounded quantity whose complete withdrawal fits that figure. The search probes the same cost computation the mint charges, against the fee, subsidy, congestion, and book state at execution, so the debit can never exceed `max_cost` and no off-chain fee estimate or padding is needed. Passing `std::u64::max_value!()` sizes against the whole balance. There is no `max_cost` requirement to satisfy here, because the budget *is* the sizing input; a budget too small to clear the minimum premium aborts with `EPremiumBelowMinimum`, and zero is such a budget.

`min_quantity` is this shape's slippage guard. The spend is fixed, so an adverse move in price, congestion surcharge, sponsor subsidy, or inventory impact between building the transaction and executing it arrives as fewer contracts rather than a larger debit, and a fill below the floor aborts with `EMintQuantityBelowMin`. It bounds the all-in price per contract at `max_cost / min_quantity`, which is why this shape carries no `max_probability` either. Zero accepts any fill the budget buys.

Three limits qualify the fill, and a caller sizing a spend should know all 3:

- **The one-lot remainder holds only when the budget binds.** If the budget is what limits the fill, what goes unspent is less than the incremental all-in cost of one more lot. A fill limited by something else can leave much more.
- **The maximum-payout bound can step the fill down.** The marginal inventory-impact rate rises with liability, so the largest fill a budget could pay for is sometimes one that would cost more than its own maximum payout. Sizing then falls back to a smaller fill. That fallback is conservative: integer rounding can make it miss a larger admissible fill, or abort on `min_quantity` or the minimum premium where another size would have succeeded. When no smaller fill clears the bound, the mint reports `EMintCostAboveMaxPayout`.
- **Backing is not a sizing input.** Insufficient expiry cash backing aborts the mint rather than shrinking it, and the read-only quote does not preflight backing or exposure-index capacity.

`quote_mint_exact_cost_for_account` is the read-only twin. It returns the fill the same budget would buy, and that fill's cost decomposition, which is the natural source for a `min_quantity` floor.

All 3 calls run the same gates, in this order, before anything moves:
...
<ImportContent
	...
	repo="deepbookv3"
	branch="abb4cfd93b2e51f7925941fe5d53b3fe70fc140c"
	fun="mint_exact_quantity,mint_exact_amount,mint_exact_cost"
	noComments
/>

</details>

The TypeScript SDK wraps all 3 calls, resolves the market from an underlying and expiry, and converts raw strikes to ticks for you. `client.predict.tx.mint` builds `mint_exact_quantity`, `client.predict.tx.mintAmount` builds `mint_exact_amount`, and `client.predict.tx.mintCost` builds `mint_exact_cost`, with `client.predict.read.quoteMintCost` as the matching dry run. Omitting the SDK's `maxCost` or `maxProbability` options sends `u64::MAX`, which caps nothing, so read a quote first and set a cap. `tx.mintCost` has no such hole, because its budget is required.
...
`redeem_live` closes an unsettled position at the current range probability, in full or in part. A partial close removes the closed slice and returns a replacement order ID in the `Option<u256>`, and a full close returns `none`. `Option<u256>` has `drop`, so the transaction can ignore the return value; the replacement ID is also on the `LiveOrderRedeemed` event as `replacement_order_id`. `min_probability` floors the quoted per-contract probability and `min_proceeds` floors the net USDC credited after the trading fee, the builder fee, and the congestion surcharge, with inventory-impact rebate included. Pass `0` to disable either floor. The close charges the trading fee once for each finite boundary, the same way a mint does, and caps it at the gross close value. It applies no entry-probability band, so a position whose probability has left the band stays closable. A live redeem in the same `clock.timestamp_ms()` as the mint aborts with `EMintRedeemSameTimestamp`, which blocks an atomic mint, oracle update, and redeem inside one transaction. A live redeem inside the no-trade window aborts with `ETradeWindowClosed`.
...
| `9` | `EMintCostCapRequired` | A caller ran `mint_exact_amount` with `max_cost` set to zero. `mint_exact_cost` never raises it: there the budget is the sizing input, and a zero budget aborts `EPremiumBelowMinimum` instead. |
...
| `11` | `EMintCostAboveMaxPayout` | The quoted `all_in_cost` exceeds `quantity`, the position's maximum payout. Every quote and every mint raises it. On `mint_exact_cost` it means no admissible size cleared the bound, because that entrypoint first steps a breaching fill down. |
...
The 2 ordinary failure modes of a first mint are not in the table above. Both live in `deepbook_predict::strike_exposure_config`, the module that holds each market's snapshot of entry-probability and fee policy and runs `assert_range_mint_probability_policy` and `assert_mint_admission` before any state moves:
...
A two-sided range meets the band at each finite boundary as well: the probability that settlement ends above the lower strike and the probability that it ends at or below the higher strike must each sit inside the band. A range with a 0.995 probability of settling above its lower strike and a 0.5 probability of settling above its higher strike has a range probability of 0.495, inside the band, yet the mint aborts with `EEntryProbabilityOutOfBounds`, because 0.995 exceeds the `0.99` maximum.
