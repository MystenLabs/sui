...
The module lives at [`packages/predict/sources/expiry_market.move`](https://github.com/MystenLabs/deepbookv3/blob/deepbook-predict-mainnet/packages/predict/sources/expiry_market.move) on the `deepbook-predict-mainnet` branch. The Move snippets on this page are pinned to the Mainnet source commit, which serves both deployments. For the package and object IDs of each network, see [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information).
...
- **`Pricer` has `copy` and `drop` but no `store`.** It cannot be saved in an object or carried between transactions, so every transaction that prices anything must call `load_live_pricer` first. Because it has `drop`, a block that ends up not using the value is still valid. The storable `FrozenPricer` that a pool flush keeps between transactions is constructed and thawed only inside the package, and no trade function accepts it.
...
Freshness is judged against each input's own source timestamp: the Pyth feed timestamp and, for the Block Scholes price, forward, and SVI inputs, the provider's per-update timestamp. Both deployments run the Pyth spot and Block Scholes price windows at 2,000 ms and the SVI window at 60,000 ms.
...
Two Move functions price a prospective mint without changing market state. Both apply the live-mint and admission gates, including the no-trade window, so a quote that succeeds tells you the trade is admissible at the current oracle state. Neither is a full preflight, and the section below is careful about which layer each claim belongs to:
...
`MintQuote` has `copy` and `drop`, and it reports amounts in USDC base units except for `entry_probability`, which uses the 1e9 fixed-point scale. Read it through the nine getters, each of which takes `&MintQuote` and returns `u64`:
...
The result is then split out of the USDC payment the trader has already made. It is carved out of fees already paid, is never an extra debit, and never changes `all_in_cost`, so a trader with a referrer and a trader without one pay exactly the same total for the same trade. The amount is zero unless the account was created with `new_with_referrer`.
...
`mint_exact_amount` fixes the premium budget instead. The market first caps `max_premium` by the account's available USDC, computes the largest lot-rounded quantity whose premium fits, and aborts when that quantity falls below `min_quantity`. Fees, the builder add-on, and the congestion surcharge are charged on top of the budget, so `max_cost` is the only bound on the total withdrawal. `max_cost` is required here: zero aborts with `EMintCostCapRequired`, and no value disables it. This variant has no `max_probability` argument, because `min_quantity` against a fixed budget already bounds the price paid per contract.

Both calls run the same gates, in this order, before anything moves: the running package version must be at or above the protocol version watermark, no pool-flush snapshot stage may be open in this transaction, the pricer must be bound to this market, the market must be outside its [no-trade window](#no-trade-window), global trading must be enabled, and this market's `mint_paused` flag must be false. The quote then has to satisfy `all_in_cost <= quantity`, and the expiry must hold enough cash to back the post-mint payout liability. A pool flush that is in flight but past its snapshot transaction does not block a mint.
...
The gate applies to `quote_mint`, `quote_mint_for_account`, `mint_exact_quantity`, `mint_exact_amount`, `redeem_live`, and their `sessions` wrappers. It does not apply to `redeem_settled`, `redeem_settled_permissionless`, `try_settle`, `set_reference_tick`, `load_live_pricer`, or any read accessor, so a position that is open when the window closes is neither stranded nor force-closed: it settles at expiry and is redeemed through the settled path.
...
Both settled paths run the version gate and refuse to be composed into a pool flush's snapshot transaction, and they abort with `EMarketNotSettled` before settlement. A flush past its snapshot transaction does not block them.
...
Five properties matter when you compose it:
...
- **A pool flush never blocks it.** A market snapshotted into an in-flight flush settles the instant it can, because the flush reads only frozen figures that settlement does not touch. The call also discards a stale flush stamp left on the market by a superseded flush.
...
Four of the accessors behave differently from the rest:
...
- **`is_pending_valuation` takes `&ProtocolConfig` as well as the market.** It returns `true` while the market is snapshotted into the in-flight pool flush and its `value_expiry` has not run. It gates nothing: trading and settlement both proceed regardless, so do not defer a settlement attempt on it.
...
`current_nav` is free expiry cash minus the marked liability of the exposure book, floored at zero. `live_order_value` is one order's full-close range value before fees. `settled_order_payout` is one order's terminal payout and aborts with `EMarketNotSettled` before settlement. None of the three proves that the caller holds `order_id`. The pool flush values a market with the package-internal `snapshot_nav`, which has `current_nav`'s exact shape over the values frozen at the snapshot instant.
...
`expiry_market` declares twelve abort codes:
...
| `11` | `EMintCostAboveMaxPayout` | The quoted `all_in_cost` exceeds `quantity`, the position's maximum payout. Raised by both quotes and both mints. |

Aborts from other modules reach these calls too. A mint can abort inside `strike_exposure_config` on entry-probability or premium policy, covered in the next section. A pricer load can abort inside `pricing` when an oracle object does not match the registry binding, when a contributing observation is stale or was written in the same transaction, or with `ELivePricingExpired` at or after expiry. A gate can abort inside `protocol_config` with `ETradingPaused` (`0`), `EPackageVersionDisabled` (`3`), `EProtocolFrozen` (`5`), `ESnapshotInProgress` (`6`) when a trade is composed into a pool flush's snapshot transaction, or `ETradeWindowClosed` (`7`) inside the no-trade window. An account borrow can abort inside `account` with `EInvalidOwner` or `EBalanceTooLow`.
...
`EEntryProbabilityOutOfBounds` is the band. A strike far enough out of the money prices below `min_entry_probability`, which is `0.01` on both deployments, and is rejected before the premium is computed.
