...
`mint_exact_cost` arrived in `predict` package version 2, which both networks now run. Call it against the version 2 call target, not the original package ID, which has no such function. See [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids).
