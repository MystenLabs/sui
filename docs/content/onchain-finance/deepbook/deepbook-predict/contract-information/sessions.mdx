...
verified_against: deepbookv3 2378336b; '@mysten/deepbook-v3' 2.6.0 plus the Mainnet id bump at ts-sdks b0422900; sessions v2 on both networks
goal:
  description: >-
...
- `deepbook_sessions::sessions`: Grant storage, the lifecycle calls, and every trading wrapper ([source](https://github.com/MystenLabs/deepbookv3/blob/2378336bbe49d9de5cc55d6208ab38fd78f444f0/packages/sessions/sources/sessions.move)).
- `deepbook_sessions::session_config`: The shared `SessionsConfig` object and the package version floor ([source](https://github.com/MystenLabs/deepbookv3/blob/2378336bbe49d9de5cc55d6208ab38fd78f444f0/packages/sessions/sources/session_config.move)).

Sui Mainnet and Sui Testnet each run a separate deployment of the package, and the Move snippets on this page pin DeepBookV3 `main` at `2378336b`. Both deployments are at `sessions` version 2, which adds the `mint_exact_cost` wrapper, so every wrapper on this page is callable on either network. Because the package has been upgraded, it carries a call target and an original ID that differ; the table below gives both. In TypeScript, `getSessionsConfig(network)` from `@mysten/deepbook-v3/sessions` returns the package call target and its original ID, the shared config, the account registry that authorizes the app, the Predict protocol config, and the 2 extra IDs the DeepBook spot wrappers take. Version 2.6.0 predates the Mainnet upgrade and still returns the version 1 Mainnet call target, so take Mainnet's from the table below until a later release ships it. The full tables are on [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#packages):

| **Object** | **Mainnet** | **Testnet** |
|---|---|---|
| `sessions` package, call target | `0xec678aee98cd161bdce62ff3dcf4893574df29d927dc8b573f6b4563ec960a1a` | `0xe9daa1e3f98659f45f0e589c0367c7c13877af9a6a1bf1eda40e33e944816b40` |
| `sessions` package, original ID | `0x9a068beffa019ae756f4a6d2611e9899d3c6dd888812dd9eca87535d2ac55e2a` | `0x1908eee49d7a08d74a537d7f23766b363a145517fe0d3e0d85635d1682831ffd` |
| `SessionsConfig` | `0xb2fba483748aac7508676020d4e691d68239b66f8bf7c5e7634dd1a76a329317` | `0xc096b97289821b27badfdf758d614bbbfa386f93d3faa409ecc1fcc6075d1186` |

Call the call target; build `SessionGrant` and every dynamic-field key, including `DataKey<…::sessions::SessionsApp>`, from the original ID. The 2 differ on both networks now. [Upgraded packages carry 2 IDs](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids) explains the rule.
...
<ImportContent
	...
	repo="deepbookv3"
	branch="2378336bbe49d9de5cc55d6208ab38fd78f444f0"
	fun="mint_exact_quantity,mint_exact_amount,mint_exact_cost,redeem_live,redeem_settled"
	noComments
/>
