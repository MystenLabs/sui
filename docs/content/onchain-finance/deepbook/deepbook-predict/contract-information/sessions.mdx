...
verified_against: deepbookv3 4e9bfe90; '@mysten/deepbook-v3' 2.6.0 (ts-sdks df494cdb); deepbook-predict-mainnet sessions v1; deepbook-predict-testnet sessions v2
goal:
  description: >-
...
- `deepbook_sessions::sessions`: Grant storage, the lifecycle calls, and every trading wrapper ([source](https://github.com/MystenLabs/deepbookv3/blob/4e9bfe905fa1f4ba1ce666f70d9eb3e50eb4f950/packages/sessions/sources/sessions.move)).
- `deepbook_sessions::session_config`: The shared `SessionsConfig` object and the package version floor ([source](https://github.com/MystenLabs/deepbookv3/blob/4e9bfe905fa1f4ba1ce666f70d9eb3e50eb4f950/packages/sessions/sources/session_config.move)).

Sui Mainnet and Sui Testnet each run a separate deployment of the package, and the Move snippets on this page pin DeepBookV3 `main` at `4e9bfe90`. The 2 deployments are no longer at the same version: Testnet was upgraded to `sessions` version 2, which adds the `mint_exact_cost` wrapper, while Mainnet is still at version 1 and does not carry it. Mainnet's `predict` package *has* been upgraded to version 2, so the owner-signed `expiry_market::mint_exact_cost` works there; it is only the session wrapper that Mainnet lacks. Everything else on this page behaves identically on both. In TypeScript, `getSessionsConfig(network)` from `@mysten/deepbook-v3/sessions` resolves the current Mainnet and Testnet deployments from version 2.6.0 and returns the package call target and its original ID, the shared config, the account registry that authorizes the app, the Predict protocol config, and the 2 extra IDs the DeepBook spot wrappers take. The full tables are on [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#packages):

| **Object** | **Mainnet** | **Testnet** |
|---|---|---|
| `sessions` package, call target | `0x9a068beffa019ae756f4a6d2611e9899d3c6dd888812dd9eca87535d2ac55e2a` | `0xe9daa1e3f98659f45f0e589c0367c7c13877af9a6a1bf1eda40e33e944816b40` |
...
Call the call target; build `SessionGrant` and every dynamic-field key from the original ID. The 2 coincide on Mainnet, where `sessions` is still at version 1. [Upgraded packages carry 2 IDs](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids) explains the rule.
...
An active session can call 5 Predict wrappers on Testnet, and 4 on Mainnet:
...
| `mint_exact_cost` | `expiry_market::mint_exact_cost` | The new order ID, as `u256`. | Testnet, `sessions` v2 |
