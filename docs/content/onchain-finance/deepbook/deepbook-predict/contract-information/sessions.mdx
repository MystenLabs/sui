...
verified_against: deepbookv3 abb4cfd9; '@mysten/deepbook-v3' 2.6.1 (ts-sdks 56d51645); sessions v2 on both networks
goal:
  description: >-
...
- `deepbook_sessions::sessions`: Grant storage, the lifecycle calls, and every trading wrapper ([source](https://github.com/MystenLabs/deepbookv3/blob/abb4cfd93b2e51f7925941fe5d53b3fe70fc140c/packages/sessions/sources/sessions.move)).
- `deepbook_sessions::session_config`: The shared `SessionsConfig` object and the package version floor ([source](https://github.com/MystenLabs/deepbookv3/blob/abb4cfd93b2e51f7925941fe5d53b3fe70fc140c/packages/sessions/sources/session_config.move)).

Sui Mainnet and Sui Testnet each run a separate deployment of the package, and the Move snippets on this page pin DeepBookV3 `main` at `abb4cfd9`. Both deployments run `sessions` protocol version 2, which adds the `mint_exact_cost` wrapper, so every wrapper on this page is callable on either network. Because the package has been upgraded, it carries a call target and an original ID that differ; the table below gives both. In TypeScript, `getSessionsConfig(network)` from `@mysten/deepbook-v3/sessions` returns the package call target and its original ID, the shared config, the account registry that authorizes the app, the Predict protocol config, and the 2 extra IDs the DeepBook spot wrappers take. Version 2.6.1 returns the current call target for `sessions` on both networks. The full tables are on [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#packages):
