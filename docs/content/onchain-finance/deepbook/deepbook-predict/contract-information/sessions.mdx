...
verified_against: deepbookv3 36783242; '@mysten/deepbook-v3' 2.6.1 (ts-sdks 56d51645); sessions v2 on both networks
goal:
  description: >-
...
Sui Mainnet and Sui Testnet each run a separate deployment of the package, and the Move snippets on this page pin DeepBookV3 `main` at `36783242`. Both deployments are at `sessions` version 2, which adds the `mint_exact_cost` wrapper, so every wrapper on this page is callable on either network. Because the package has been upgraded, it carries a call target and an original ID that differ; the table below gives both. In TypeScript, `getSessionsConfig(network)` from `@mysten/deepbook-v3/sessions` returns the package call target and its original ID, the shared config, the account registry that authorizes the app, the Predict protocol config, and the 2 extra IDs the DeepBook spot wrappers take. Version 2.6.1 returns the current call target for `sessions` on both networks. The full tables are on [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#packages):
