...
| Add USDC without minting shares | `add_usdc_to_plp` | Permissionless, and blocked while a flush is in flight. Needs `predict` version 3, so Testnet only |
...
`add_usdc_to_plp` arrived in `predict` package version 3, which only Testnet runs. Mainnet is at version 2 and does not carry it. See [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids).
...
3. **The contribution must not push the mark out of the executable band.** A flush fills only at a mark within `executable_price_band_factor` of unit parity in each direction, which is 0.01 to 100 USDC per PLP; both coins use 6 decimals, so unit parity is raw-unit parity. A donation raises value without raising supply, so a large enough one puts the mark above the top of that band and every later flush declines as [not executable](#redemption-value). The call restates the upper half of that test against idle, requiring `ceil((idle + amount) / 100) <= total_supply`, and otherwise aborts with `EContributionExceedsPriceCeiling`.
