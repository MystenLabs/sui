...
A flush in progress blocks the call, which aborts with the valuation guard rather than changing the pool under a snapshot. Success emits [`UsdcAddedToPlp`](#events).
