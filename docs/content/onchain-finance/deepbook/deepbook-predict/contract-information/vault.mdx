...
| `fee_incentive_reserve` | `Balance<USDC>` | Sponsor-funded USDC for taker fee subsidies, excluded from PLP net asset value (NAV). |
...
| `add_usdc_to_plp` | Blocked with `EValuationInProgress` | Blocked with `EValuationInProgress` |
...
<details>
<summary>Source for the 5 flush calls</summary>
...
You have 4 operations outside the ordinary supply and withdraw path:
...
| Add USDC without minting shares | `add_usdc_to_plp` | Permissionless, and blocked while a flush is in flight. Needs `predict` protocol version 3 |
...
### Add USDC without minting shares {#add-usdc}

`add_usdc_to_plp` donates USDC into the pool's idle balance and mints nothing. Supply is unchanged and the payer receives no claim, so the whole amount accrues to existing holders as a rise in the mark. That is the difference from `sponsor_fee_incentives`, whose USDC sits in the fee incentive reserve and is excluded from NAV: a contribution here *is* NAV.

:::info

`add_usdc_to_plp` arrived in `predict` protocol version 3, which both networks run. Reach it through the current call target, not a superseded one: a client pinned to an older `predict` address cannot see it. See [Contract Information](/onchain-finance/deepbook/deepbook-predict/contract-information#two-ids).

:::

Three admission rules apply, in this order:

1. **The pool must be bootstrapped.** A contribution before `lock_capital` aborts with `ENotBootstrapped`, because a donation into a zero-supply pool has no holders to accrue to.
2. **At least 10 USDC.** Below `min_usdc_contribution`, which is `10_000_000` base units, the call aborts with `EBelowMinUsdcContribution`. It is the same floor as a supply request but a separate constant, so the 2 policies can diverge.
3. **Pool cash after the contribution must stay under 10 USDC per PLP.** The call requires `ceil(cash_after / 10) <= total_supply`, where `cash_after` is idle plus `deployed_expiry_cash` plus the contribution, and otherwise aborts with `EContributionExceedsPriceCeiling`. The next 2 subsections explain the 2 halves of that: why the bound counts deployed cash, and why the ceiling is 10 rather than the band's own 100.

#### Why the bound counts deployed cash {#contribution-cash}

A flush marks every dollar the pool owns, including the cash `rebalance_expiry_cash` has moved out of idle and into a market. A bound that read idle alone could therefore be walked around: rebalancing is permissionless, so a contributor could fill the idle ceiling, park the cash in a market, and contribute again — or simply contribute into the idle headroom of a pool whose markets are already funded.

So the guard tests pool cash: idle, plus `pool_accounting::deployed_expiry_cash`, which sums the net USDC funded into each active expiry, sent minus received and floored at zero per expiry. Moving idle into a market leaves that sum unchanged, so parking reopens no headroom. Cash is an upper bound on the pool's own contribution to the mark, because a market's marked liabilities only pull its NAV below its cash.

#### Why the ceiling is a tenth of the band {#contribution-ceiling}

A flush fills only at a mark inside the executable band, within `executable_price_band_factor` of unit parity in each direction, which is 0.01 to 100 USDC per PLP. Both coins use 6 decimals, so unit parity is raw-unit parity.

Contributions are held to `contribution_price_ceiling_factor`, which is `executable_price_band_factor / 10`, so 10 USDC per PLP. The gap is not slack. Every LP fill after a contribution pushes the price up and never down: `quote_supply_shares` floors the shares it mints, and the supply and withdraw fees stay in the pool. A pool filled exactly to the band would be carried out of it by the very next uneven or fee-charged fill, and once the mark is above the band every LP request refunds and `total_supply` can never grow to bring it back down. The tenth leaves LP activity 9 times the pool's cash of room.

A flush in progress blocks the call with `EValuationInProgress`, rather than letting a donation move the pool under a snapshot, as [What a flush blocks](#flush-gates) lists. Success emits [`UsdcAddedToPlp`](#events).

<details>
<summary>Source for `add_usdc_to_plp`</summary>
...
`vault_events` declares 16 events, all with `copy`, `drop`, and `store`. The table lists fields in declaration order, which is also their Binary Canonical Serialization (BCS) order:
...
| `UsdcAddedToPlp` | `pool_vault_id`, `contributor`, `amount` |
...
| **Value** | **Meaning** |
|---|---|
| `0` | The account canceled the request. |
...
`@mysten/deepbook-v3` version 2.6.1 queues and cancels supply and withdraw requests, reads the pool and a PLP balance, and decodes request receipts on its `/predict` subpath, running every read through the Sui client's core API. See [Predict Liquidity SDK](/onchain-finance/deepbook/deepbook-predict-sdk/liquidity) for each function, the units each floor takes, and examples.
...
| `12` | `EBelowMinUsdcContribution` | `add_usdc_to_plp` ran with less than 10 USDC. |
| `13` | `EContributionExceedsPriceCeiling` | `add_usdc_to_plp` would push the mark above the executable price band. |
...
| `3` | `ENotRequestOwner` | The canceling account is not the recorded recipient. |
