...
Predict is deployed on Sui Mainnet as `deepbook-predict-mainnet` and on Sui Testnet as `deepbook-predict-testnet`, with identical package sources: Testnet was published from the `deepbook-predict-testnet` branch, and Mainnet from the [`deepbook-predict-mainnet` branch](https://github.com/MystenLabs/deepbookv3/tree/deepbook-predict-mainnet/packages/predict) of the DeepBookV3 repository. Version 2.3.0 or later of the [`@mysten/deepbook-v3`](https://www.npmjs.com/package/@mysten/deepbook-v3) package ships a `/predict` subpath that carries both deployments, builds every transaction, and answers every read, so trading needs no indexer. The examples on these pages select a network with one constant and default to Testnet.

:::caution

These pages describe the two current deployments. As of 2026-09-10 the Mainnet deployment is published and configured, but its pool holds only the bootstrap minimum and no market has been tradable there yet. Testnet creates and settles markets every minute. Read `read.markets()`, `read.pool()`, and the `ProtocolConfig` object on the network you target rather than assuming its state, and treat the package IDs and object IDs on these pages as pinned to those two deployments.

:::

## Quickstart: mint a position

The following steps install the DeepBook TypeScript SDK, create and fund a Predict account, discover a live market, quote a position, and mint it under a cost cap and a fill price cap. They run on Testnet, where the quote coin is a mintable test coin. The samples live in the `examples/deepbook-predict` package, which type-checks with `npm run build` (`tsc --noEmit`) against `@mysten/deepbook-v3` version 2.3.0 or later and `@mysten/sui` version 2.30.0 or later. They pass compile checks but do not run against Testnet during the docs build, so work through [Verify on Testnet](#verify-on-testnet) before you rely on them.
...
On Mainnet the quote asset is Circle's native USDC, and there is no faucet. Read the exact type for either network from `getConfig(network).quoteCoinType` rather than from a symbol: the two coins share the `usdc::USDC` module path but live in different packages.
...
Set up a client next. The following example uses the [gRPC](/develop/accessing-data/grpc) client, which matches the rest of the DeepBook SDK docs, and extends it with the Predict client for the selected network.
...
Predict creates markets on a fixed cadence, one object per underlying and expiry. Both networks configure BTC markets on the 1 minute and 5 minute cadences, with two expiries of each live at once. `client.predict.read.markets()` returns the active ones with their expiry, tick size, admission tick size, mint pause flag, and reference price. Active means live and not yet settled, so an expired market nobody has settled is still in the list, and quoting against it aborts.
...
- **Per-expiry markets:** Each market settles once, at one timestamp, from one of two sources: the exact Pyth price at the expiry timestamp, or, when that is unavailable 30 seconds after expiry, the exact Block Scholes minute-boundary spot. Six cadence slots exist from 1 minute to 1 month, and both deployments enable the 1 minute and 5 minute cadences. Trading closes 2 seconds before expiry, so a mint or live redeem inside that no-trade window aborts rather than filling at a converged price.
