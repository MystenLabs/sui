...
The walkthrough sends every transaction and every read through the `/predict` subpath of `@mysten/deepbook-v3`, version 2.6.0 or later. The SDK carries both the `deepbook-predict-mainnet` and `deepbook-predict-testnet` deployments, and the samples select one with a single `NETWORK` constant that defaults to `'testnet'`. The walkthrough runs on Testnet, where the quote coin is a mintable test coin. Set `NETWORK` to `'mainnet'` to point the same code at Mainnet, where the quote coin is native USDC. The SDK builds transactions and never signs them, and every read it exposes runs against the Sui client core API, so no indexer or server sits on the critical path. The samples live in the `examples/deepbook-predict` package, which type-checks with `npm run build` (`tsc --noEmit`). This document does not execute them, so run the [manual verification steps](#verify-on-testnet) before you rely on the write paths.

:::caution

Both deployments went live on 2026-09-12. As of 2026-09-14 the markets created at deployment have expired unsettled, no market has opened since, and no oracle update has landed since deployment, so the walkthrough's quotes and mints abort on either network. The Mainnet pool holds only the bootstrap minimum. Read `read.markets()`, `read.pool()`, and the `ProtocolConfig` object on the network you target rather than assuming its state. Package IDs, object IDs, and function signatures on these pages belong to the 2 current deployments.

:::

<Tabs className="tabsHeadingCentered--small">
<TabItem value="prereq" label="Prerequisites">
- [x] Node.js 22 or later, with `@mysten/deepbook-v3` 2.6.0 or later and its `@mysten/sui` 2.30.0 or later peer dependency installed. See the [Sui TypeScript SDK](https://sdk.mystenlabs.com/typescript) for client setup.
...
Both networks enable 2 cadences for BTC, 1m and 5m, with 2 expiries of each live at once. The configuration also lists the 1h, 1d, 1w, and 1mo cadences, all disabled. The 1m cadence spaces expiries 1 minute apart, and a market-lifecycle call creates each market. `tickSize` is `$0.01` and `admissionTickSize` is `$1` on both enabled cadences, so every finite strike lands on a whole dollar.
...
| `read.quoteMint`, `read.quoteMintCost`, `read.quoteRedeem` | Exact dry runs of the matching builders |
...
`maxCost` is optional on `tx.mintAmount`, and that is the most dangerous default of the 3 builders. The protocol charges fees on top of `spend`, so `maxCost` is the only bound on the whole withdrawal. Omit it and the call builds fine, sends `U64_MAX`, and leaves the debit unbounded. The onchain guard does not save you: `mint_exact_amount` aborts with `EMintCostCapRequired` only when `max_cost` is exactly 0, which `U64_MAX` is not. The SDK does reject a `maxCost` at or below zero before it builds anything, but nothing rejects an absent one. Always pass a `maxCost` derived from a fresh quote.

:::

### Spend an exact total instead

Where `tx.mintAmount` treats `spend` as a pre-fee premium budget and charges the fees on top, `tx.mintCost` treats `spend` as the whole withdrawal and sizes the fill inside it. The chain caps the budget at the account's available USDC, then picks the largest lot-rounded fill whose complete debit fits, probing the same cost computation it charges. The debit cannot exceed `spend`, so there is no cost cap to pass and none to forget, and no fee estimate to pad:
...
const tx = await client.predict.tx.mintCost(owner, descriptor, {
	spend: 25,
	minQuantity: preview.quantity * 0.99,
});
```

`minQuantity` is this shape's only slippage guard: the spend is fixed, so an adverse move arrives as fewer contracts, and the floor is what rejects a fill that moved too far. `0` accepts whatever the budget buys.

This entrypoint arrived in `predict` package version 2, which both networks run onchain. The published `@mysten/deepbook-v3` 2.6.0 predates the Mainnet upgrade and still addresses Mainnet `predict` at its version 1 address, so on that release these 2 calls work on Testnet and fail on Mainnet until the next patch ships. [Predict Positions SDK](/onchain-finance/deepbook/deepbook-predict-sdk/positions#mint-cost) covers the sizing limits, and [Predict Cost SDK](/onchain-finance/deepbook/deepbook-predict-sdk/cost#mint-cost-for-budget) runs the same search locally with no chain call.
...
At mint, the chain treats each finite boundary of a two-sided range as its own leg. It charges a trading fee on each leg, so `fees.trading` in the quote sums 2 boundary fees, each with its own floor, where a directional position pays 1. It also applies the entry-probability band to each leg as well as to the range, so a very wide range aborts with `EEntryProbabilityOutOfBounds` when the probability that settlement lands above its lower bound, or at or below its upper bound, exceeds the band's maximum, even when its range probability sits inside the band. `pricer.range` returns only the range probability, which sets the premium, so take the fee from `read.quoteMint`. [Design](/onchain-finance/deepbook/deepbook-predict/design#fee-components) works through the fee on an example range.
