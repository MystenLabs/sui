...
Both deployments were published on 2026-09-12. As of 2026-09-14 neither runs a market service: the markets created at deployment expired unsettled on 2026-09-12, no market has opened since, and no oracle update has landed since deployment, so the walkthrough's quotes and mints abort until those services run again. The Mainnet pool holds only the bootstrap minimum. Read `read.markets()`, `read.pool()`, and the `ProtocolConfig` object on the network you target rather than assuming its state. Package IDs, object IDs, and function signatures on these pages belong to the 2 current deployments.
...
Both networks enable 2 cadences for BTC, 1m and 5m, with 2 expiries of each live at once. The configuration also lists the 1h, 1d, 1w, and 1mo cadences, all disabled. While the market service runs, each network gains a new 1m market every minute. `tickSize` is `$0.01` and `admissionTickSize` is `$1` on both enabled cadences, so every finite strike lands on a whole dollar.
