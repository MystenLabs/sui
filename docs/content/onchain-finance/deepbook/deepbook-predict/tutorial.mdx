...
This entrypoint arrived in `predict` package version 2. Both networks run that version onchain, but `@mysten/deepbook-v3` 2.6.0 still addresses Mainnet `predict` at its version 1 address, so these 2 calls work on Testnet and fail on Mainnet until a release ships the new call target. [Predict Positions SDK](/onchain-finance/deepbook/deepbook-predict-sdk/positions#mint-cost) covers the sizing limits, and [Predict Cost SDK](/onchain-finance/deepbook/deepbook-predict-sdk/cost#mint-cost-for-budget) runs the same search locally with no chain call.
