...
As of 2026-09-10 the Mainnet deployment is published and configured, but its pool holds only the bootstrap minimum and no market has been tradable there yet. Read `read.markets()`, `read.pool()`, and the `ProtocolConfig` object on the network you target rather than assuming its state. Package IDs, object IDs, and function signatures on these pages are pinned to the two current deployments.
...
`predict({ network })` lands the extension on `client.predict`, which exposes three groups. `tx` builds ready-to-sign transactions, `read` runs simulations and object reads, and `decode` parses receipts out of an executed transaction's events with no network call at all.
...
Both networks enable two cadences for BTC, 1m and 5m, with two expiries of each live at once. The 1h, 1d, 1w, and 1mo cadences are configured but disabled. A new 1m market is created every minute on Testnet. `tickSize` is `$0.01` and `admissionTickSize` is `$1` on both enabled cadences, so every finite strike lands on a whole dollar.

Three things make the soonest expiry a poor default, and the sample's `tradeableMarket(minTtlMs = 30_000)` handles the first two:

- **Trading closes before expiry.** `no_trade_window_ms`, 2 seconds on both deployments, is a hard stop: a quote, mint, or live redeem inside the last 2 seconds before expiry aborts in `protocol_config` with code `7`, `ETradeWindowClosed`. An expired market that nobody has settled is still in the active list, and quoting against it aborts too.
...
| `read.pool()` | Pool aggregates: `plpTotalSupply`, `idleUsdc`, and the two queue depths |
...
The lot size is the granularity of a quantity, not a size you can trade. `strike_exposure_config::assert_mint_admission` rejects any mint whose premium falls below 1 USDC with code `3`, `EPremiumBelowMinimum`, and the premium is `quantity` multiplied by the quoted `entryProbability`. Because `max_entry_probability` caps that probability at 0.99, no market accepts a quantity below about 1.02, and the floor climbs as the strike moves out of the money: a strike quoting near a coin flip needs roughly 2 contracts, and a strike quoting at 0.08 needs 12.5. Size by the product, not by the grid, and quote before you pick a quantity.
...
To find positions worth closing, `read.positions(owner)` walks the account's dynamic fields and returns `marketId` and `orderId` pairs. An indexed service, where one exists for your deployment, returns the same set only when it is keyed on the canonical account ID.
...
You cannot start a flush. Only a holder of a pool valuation capability starts one, through `start_pool_valuation`, and that transaction also snapshots every live market's pricer with `snapshot_expiry_pricer` and seals the snapshot. Anyone can then run `value_expiry` per market and `finish_flush`, which drains the queues at the frozen mark, and the whole flush must finish within 5 minutes of its snapshot or be restarted. That single frozen mark is what stops anyone from timing an entry or exit against a self-supplied oracle update. Trading is not blocked while a flush is in flight, and neither is queuing a new request, but a request queued after the snapshot waits for the next flush, and cancelling a queued request is blocked until the flush finishes. Plan on filling at a price you do not control and cannot preview, bounded only by the floor you set.

:::

### Supply liquidity

`tx.supplyPlp(owner, amountUsdc, { minPlpOut })` queues a supply request and escrows the quote coin. The minimum request is 10 USDC. `minPlpOut` is a floor on the shares the flush must mint for the request, in raw six-decimal shares, and it is optional: omitted, the request accepts whatever the next flush quotes.
...
- **Exits are bounded by idle cash:** The flush fills supplies before withdrawals, so quote coin supplied in a flush can pay that same flush's withdrawals. Cash already funded into an expiry is not directly redeemable until a rebalance or settlement returns it, so a large exit can fill across several flushes.
...
- **A queued request can come back refunded:** A request the flush cannot execute at the frozen mark, or whose floor that mark misses, is cancelled and refunded rather than held, so handle a refund as a normal outcome rather than an error.
