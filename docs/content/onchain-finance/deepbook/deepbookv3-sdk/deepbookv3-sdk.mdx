...
// Valid values: 0.1, 0.2, 0.3, ... up to 10.0
client.deepbook.deepBook.mintReferral('SUI_DBUSDC', 0.1)(tx);

// Update the multiplier for an existing referral
client.deepbook.deepBook.updatePoolReferralMultiplier('SUI_DBUSDC', referralId, 0.2)(tx);

// Claim accumulated referral rewards (returns base, quote, and DEEP coins).
// This call returns an object rather than a command, so apply the thunk to `tx`
// directly instead of passing it to tx.add.
const { baseRewards, quoteRewards, deepRewards } = client.deepbook.deepBook.claimPoolReferralRewards(
	'SUI_DBUSDC',
	referralId,
)(tx);

// Get the current balances for a referral
client.deepbook.deepBook.getPoolReferralBalances('SUI_DBUSDC', referralId)(tx);

// Get the multiplier for a referral
client.deepbook.deepBook.poolReferralMultiplier('SUI_DBUSDC', referralId)(tx);
```
