...
The package root covers DeepBook spot and margin. Three subpaths ship in the same package, each as a separate module graph, so importing one does not load the others:
