...
Placing an order requires a trade proof and a long argument list. The TypeScript SDK generates the proof and encodes the arguments for you, so it is the recommended path. Deposit into the `BalanceManager` and place the order in the same transaction so both settle atomically.
...
// Order calls live on the `deepBook` contract namespace, not on the client.
tx.add(
...
Placing an order from the CLI is impractical. [`pool::place_limit_order`](https://github.com/MystenLabs/deepbookv3/blob/main/packages/deepbook/sources/pool.move) takes 12 value arguments and 2 type arguments: the pool, the `BalanceManager`, a `TradeProof` from `balance_manager::generate_proof_as_owner`, then the client order ID, order type, self-matching option, price, quantity, `is_bid`, `pay_with_deep`, expiration, and the `0x6` clock, all under the pool's base and quote types. Use the TypeScript SDK for order placement.
