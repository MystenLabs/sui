...
Build common DeepBook [programmable transaction blocks (PTBs)](/develop/transactions/ptbs/prog-txn-blocks) from copy-paste templates for both the [`sui client ptb` CLI](/references/cli/ptb) and the TypeScript SDK. These templates start with the syntax rules that cause the most CLI errors, then cover coin operations, generic Move calls, and DeepBook flows.
...
## Fix the 3 most common CLI errors first
