...
title: Orders
description: >-
  Use DeepBookV3 to create limit and market orders, modify orders, or cancel
  orders.
keywords:
  - limit orders
  - market orders
...
goal:
  description: >-
    Reader can use DeepBookV3 to create limit and market orders, modify orders,
    or cancel orders
  requires:
    - has_frontmatter:
        - title
...
Cancel multiple orders within a vector. The orders must be owned by the `balance_manager`. The orders are removed from the book and the `balance_manager` open orders. If any order fails to cancel, the system cancels none of them (atomic operation).
...
The system emits this event when a taker fills a maker order.
...
The system emits this event when you cancel a maker order.
...
The system emits this event when you modify a maker order.
...
The system emits this event when you place a maker order into the order book.
