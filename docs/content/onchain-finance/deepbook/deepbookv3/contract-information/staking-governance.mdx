...
5. At the end of the epoch, the winning proposal's parameters take effect in the following epoch.

Voting power is proportional to your active stake in the pool. You cannot split voting power across multiple proposals.
...
Call `stake` before the epoch ends so that your stake is active in the following epoch. Voting and submitting proposals require nonzero active stake. Claiming rebates requires that your `BalanceManager` has accumulated rewards; calling `claim_rebates` with no rewards results in an error.