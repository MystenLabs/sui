...
answer: >-
  ...
  25% higher rate. The DEEP amount floats with an onchain price, so read it with
...
Sign and execute the returned transaction with your keypair, then wait for it before building the next one. See [Sign and execute](/onchain-finance/deepbook/deepbookv3/spot-workflow#sign-and-execute) for that step, and [BalanceManager](/onchain-finance/deepbook/deepbookv3-sdk/balance-manager) for creating and sharing a manager.
...
If neither route works, or you want a book with reliable liquidity, run DeepBook locally instead. The [DeepBook Sandbox](https://github.com/MystenLabs/deepbook-sandbox) deploys the protocol to a private localnet with its own DEEP and USDC faucet and a market maker seeding the pools. See [Run the same flow locally](/onchain-finance/deepbook/deepbookv3/spot-workflow#run-the-same-flow-locally).
...
- `version unavailable for consumption`: you built a transaction from stale state before a previous one finalized. Call `client.core.waitForTransaction({ digest })` before building the next transaction, as in [Sign and execute](/onchain-finance/deepbook/deepbookv3/spot-workflow#sign-and-execute). Do not blindly retry a rejected transaction, because it can leave the gas coin equivocation-locked.
