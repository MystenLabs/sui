...
Every builder on this page returns an unsigned `Transaction`. Every later step submits one through a helper like this one, taken from the [DeepBook Sandbox](https://github.com/MystenLabs/deepbook-sandbox). Nothing in it is network-specific, so it works against Testnet exactly as it does against the sandbox's localnet:

<ImportContent
	source="examples/sandbox/setup.ts"
	mode="code"
	org="MystenLabs"
	repo="deepbook-sandbox"
	fun="signAndExecute"
/>

:::caution

Wait for each transaction before building the next one. Execution returns as soon as the transaction finalizes, but the node you read from can still serve the pre-transaction state, and a transaction built against that stale state fails with a version-unavailable error. Every step in this workflow depends on the one before it, so `waitForTransaction` is not optional here.
...
`createAndShareBalanceManager` returns nothing you can capture, so read the new manager's ID out of the transaction effects instead. Ask for `objectTypes` alongside `effects` when you execute, then find the created object whose type contains `BalanceManager`. The sandbox's `setupWithBalanceManager` in [Run the same flow locally](#run-the-same-flow-locally) does exactly this, in its step 3.

**Observe:** the effects contain a created shared object whose type ends in `BalanceManager`. Save that ID and pass it to the client as a named manager.
...
If a later deposit fails for insufficient funds even though you funded the key, the funds are usually sitting in another manager, not lost, because each manager is a shared object that keeps what you deposit. List the managers you have created and check their balances. Persist each ID when you create it. If you did not, scan your creation transactions with the gRPC `LedgerService.ListTransactions` method and a `sender` filter, or the GraphQL `Query.transactions` field with a `sentAddress` filter. Both replace the deprecated `suix_queryTransactionBlocks` JSON-RPC method with its `FromAddress` filter. See [Migrating from JSON-RPC](/develop/accessing-data/json-rpc-migration) for the full method mapping, and the [gRPC](/develop/accessing-data/grpc/grpc-migration-cookbook#list-and-paginate-transactions) and [GraphQL](/develop/accessing-data/graphql/graphql-migration-cookbook#query-filtered-transactions) cookbooks for request shapes.
...
If you cannot obtain Testnet DEEP, or you want a book with reliable liquidity to trade against, run this workflow against a local deployment instead. The [DeepBook Sandbox](https://github.com/MystenLabs/deepbook-sandbox) deploys the full protocol (token, order book, margin, oracle, and a test USDC) to a private localnet in one command, seeds the pools with a market maker, and exposes a faucet for SUI, DEEP, and USDC:
...
The sandbox ships runnable versions of the steps on this page. Its `place-limit-order.ts` covers manager creation, deposit, and order placement end to end, and shows one detail this walkthrough leaves to you: reading the mid price, then rounding your price down to the pool's tick size before placing, so the order cannot fail the tick check.

<ImportContent
	source="examples/sandbox/place-limit-order.ts"
	mode="code"
	org="MystenLabs"
	repo="deepbook-sandbox"
/>

Its `setupWithBalanceManager` covers manager creation end to end, including the effects read that recovers the new manager's ID:
...
<ImportContent
	...
	org="MystenLabs"
	repo="deepbook-sandbox"
	fun="setupWithBalanceManager"
/>

Because the sandbox deploys its own packages, its client reads package, coin, and pool IDs from a generated manifest rather than the SDK's Testnet constants, and it funds a fresh keypair from a local faucet. Those two steps are localnet-only. Everything else, including the effects extraction and the `waitForTransaction` gate, is the same SDK surface this page uses.
