...
Every builder on this page returns an unsigned `Transaction`. Every later step submits one through this helper.

<ImportContent source="examples/deepbook-spot/src/execute.ts" mode="code" tag="execute" />
...
`createAndShareBalanceManager` returns nothing you can capture, so read the new manager's ID out of the transaction effects:

<ImportContent source="examples/deepbook-spot/src/execute.ts" mode="code" tag="created-object" />
...
Because the sandbox deploys its own packages, its client reads package, coin, and pool IDs from a generated manifest rather than the SDK's Testnet constants. Everything after client construction is the same SDK surface this page uses.
