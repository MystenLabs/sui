...
Every builder on this page returns an unsigned `Transaction`. Every later step submits one through a helper like this one, from the DeepBookV3 SDK's own examples:
...
`createAndShareBalanceManager` returns nothing you can capture, so read the new manager's ID out of the transaction effects instead. Ask for `objectTypes` alongside `effects` when you execute, then find the created object whose type contains `BalanceManager`.
