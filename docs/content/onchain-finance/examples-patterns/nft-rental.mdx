...
5. **Use the rented NFT.** The borrower accesses the NFT through `borrow` (an immutable reference) or `borrow_val` (the item itself, returned with a `Promise` that must be resolved in the same transaction, for use cases where the NFT state changes during rental).
