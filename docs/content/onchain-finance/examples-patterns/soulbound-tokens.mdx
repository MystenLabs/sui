...
A soulbound non-fungible token (NFT) is an NFT that is non-transferable. After you mint an NFT to a Sui account, the NFT belongs to that account and no one can transfer it. This implementation uses the custom logic of the Sui framework `transfer` functions. The <UnsafeLink href="/references/framework/sui_sui/transfer">`sui::transfer` module</UnsafeLink> contains 2 functions that transfer objects: <UnsafeLink href="/references/framework/sui_sui/transfer#sui_transfer_transfer">`transfer::transfer`</UnsafeLink> and <UnsafeLink href="/references/framework/sui_sui/transfer#sui_transfer_public_transfer">`transfer::public_transfer`</UnsafeLink>.
