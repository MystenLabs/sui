...
description: >-
  Implement a generic staking pool where users deposit stake tokens and earn
  reward tokens proportionally over time, using the Synthetix reward-per-token
  accumulator pattern for O(1) gas cost regardless of staker count.
...
- [x] [Install the Sui CLI](/getting-started/onboarding/sui-install).

- [x] [Create an address and fund it with SUI](/getting-started/onboarding/get-coins) for gas.
...
A complete, tested implementation lives in [`staking-contract-template`](https://github.com/MystenLabs/staking-contract-template). Build it, run its tests, and publish it as-is, or lift the accumulator logic into a package of your own:
...
Every entry point is a `public fun` that returns its objects instead of transferring them internally. That makes the module composable in programmable transaction blocks, but it also means a plain `sui client call` fails with `UnusedValueWithoutDrop`. Use `sui client ptb` and transfer the results explicitly.
...
For a hands-on exercise exploring what happens when a staking contract gets the accumulator pattern wrong, see the [staking challenge in the CTF repository](https://github.com/MystenLabs/CTF).
...
**Fix:** Use `sui client ptb` and explicitly transfer the returned objects. See [Calling the contract](#calling-the-contract) for the correct syntax.
