...
async function handleProviderWebhook(req: Request): Promise<Response> {
  ...
  const tx = await suiClient.core.getTransaction({
    digest: txDigest,
    include: { effects: true, balanceChanges: true },
  });

  const depositConfirmed = tx.Transaction?.balanceChanges?.some(
    (change) =>
      change.owner === payload.recipientAddress &&
...
tx.moveCall({
  ...
  arguments: [
    tx.balance({ balance: refundAmount }),
