...
The following sections cover the integration patterns, lifecycle flows, and error handling for both on-ramps (fiat to crypto) and off-ramps (crypto to fiat).
...
<ImportContent source="examples/regulated-payments/src/verify-deposit.ts" mode="code" tag="verify-deposit" />
...
<ImportContent source="examples/regulated-payments/src/off-ramp.ts" mode="code" tag="off-ramp-send" />
...
- **Absorb the loss.** If the onchain assets have already moved beyond your control, you might need to absorb the chargeback cost. Factor this into your pricing model.
