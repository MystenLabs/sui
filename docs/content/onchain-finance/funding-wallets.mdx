...
For agents using spending mandates, the mandate's owner funds the mandate with the tokens the agent can spend. The agent needs gas only if it is not using a gas sponsor. See [Agent Wallet Setup](/onchain-finance/agentic-payments/agent-wallet-setup) for details.
