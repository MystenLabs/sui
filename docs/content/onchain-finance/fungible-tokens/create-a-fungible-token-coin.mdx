...
See [Address Balances Migration Guide](/onchain-finance/asset-custody/address-balances/migrate-address-balances) for the full reference on using address balances, including withdrawal patterns, gas payment, sponsored transactions, and balance queries.
...
You can convert coin objects into an address balance at any time by calling `coin::send_funds` with your own address as the recipient. See [Converting Existing Coins to Address Balances](/onchain-finance/asset-custody/address-balances/migrate-address-balances#converting-existing-coins-to-address-balances) for instructions.
