...
class GasCoinPool {
  ...
  async initialize(client: SuiGrpcClient, sponsorAddress: string) {
    const ownedCoins = await client.core.listOwnedObjects({
      owner: sponsorAddress,
      type: '0x2::coin::Coin<0x2::sui::SUI>',
      limit: 50,
    });
    for (const coin of ownedCoins.objects) {
      const id = coin.objectId;
      this.coins.set(id, {
        objectId: id,
...
app.post('/sponsor', async (req, res) => {
  try {
    ...
    const { txBytes, sender } = req.body; // txBytes: Base64-encoded kind bytes; sender: user's Sui address
