...
The recommended approach is [`@mysten-incubation/sponsor`](https://www.npmjs.com/package/@mysten-incubation/sponsor), which handles gas coin management, transaction validation, and the dual-signature flow out of the box. For custom requirements (non-standard validation, self-managed coin pools), see the [self-hosted gas station](#self-hosted-gas-station) section below.
