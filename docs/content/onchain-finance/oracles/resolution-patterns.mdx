...
For a production worked example, DeepBook Predict settles each market through a permissionless [`try_settle`](/onchain-finance/deepbook/deepbook-predict/contract-information/predict) transition that anyone can call after expiry. It reads the exact price stamped at the expiry timestamp, records that price and the payout liability once, and returns `false` rather than inventing a mark when no exact price is available yet. Every later read, including settled redemption, consumes the recorded price instead of reading an [oracle](/onchain-finance/deepbook/deepbook-predict/contract-information/oracle). The mechanism, resolve once at expiry and read recorded state afterward, is the same.
