...
goal:
  description: >-
    ...
    restricted asset movement on Sui through Accounts
  requires:
    - has_frontmatter:
        - title
        - description
...
answer: >-
  On Sui, any object with the `store` ability can be freely transferred by its
...
This claim should be removed or corrected, as Sui uses a unique ID model (not derived addresses) where every entity has a globally unique ID, as stated in onchain-finance/types-of-assets.mdx line 152.
