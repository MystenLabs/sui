...
description: >-
  Learn how to move assets through the Permissioned Asset Standard (PAS) on Sui
...
goal:
  description: >-
    Reader can move assets through the Permissioned Asset Standard (PAS) on Sui
    using Accounts, Requests, and Policies
  requires:
    - has_frontmatter:
        - title
...
questions:
  - What is Permissioned Assets Actions in Sui?
...
The unrestricted path is `Balance<C>`-specific. It exists for assets like SUI that might accidentally end up in an Account. It aborts if a `Policy<T>` exists for the type because you cannot bypass managed asset controls.
