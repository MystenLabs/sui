...
The unrestricted path is `Balance<C>`-specific. It exists for assets like SUI that might accidentally end up in an Account. It aborts if a `Policy<C>` exists for the type because you cannot bypass managed asset controls.
