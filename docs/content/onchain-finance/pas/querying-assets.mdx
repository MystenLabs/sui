...
goal:
  description: >-
    ...
    using the PAS SDK and standard Sui APIs
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  ...
  - What is Querying Balances and Assets in Sui?
answer: >-
  How to read PAS-managed balances for a wallet address using the PAS SDK and
  standard Sui APIs.
---
...
From your perspective, both belong to the same address. PAS balances are held in a derived Account object, and transfers always target the Account address, not the wallet address. The SDK resolves the underlying Account transparently.
...
- **Deterministic:** The same wallet address always resolves to the same Account derived address.
