...
From your perspective, both belong to the same address. PAS balances are held in a derived Account object, but transfers always target the Account address, not the wallet address. The SDK resolves the underlying Account transparently.
...
- **Deterministic:** The same wallet address always resolves to the same derived Account address.
