...
A `.sui` name can expire and be re-registered by a different user. The `resolveNameServiceAddress` method returns `null` for expired names. If you cache resolved addresses, re-resolve before each payment to ensure the name still points to the intended recipient.
...
<ImportContent source="examples/onchain-finance/recipient-resolution/src/queries.graphql" mode="code" tag="resolve-graphql" />

For reverse lookup, query the `defaultNameRecord` field on an address:

<ImportContent source="examples/onchain-finance/recipient-resolution/src/queries.graphql" mode="code" tag="reverse-graphql" />
