...
description: >-
  Publish the asset_tokenization and template packages on Sui and interact with
...
keywords:
  ...
  - CLI
goal:
  description: Reader can publish the asset_tokenization and template packages on Sui
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I deploy a tokenized asset on Sui?
...
The WASM feature enables Move bytecode serialization and deserialization in a browser environment, but you still need to build packages relative to a build environment that determines dependencies, addresses, and network-specific configuration. For asset tokenization, this lets you create and publish new asset types directly from the browser.
