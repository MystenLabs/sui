...
PAS uses Sui [Address Balances](/onchain-finance/asset-custody/address-balances/address-balance-migration). PAS balances are managed by PAS on behalf of the wallet address. They are held in a derived Account object, however, transfers always target the wallet address, not the Account address.
