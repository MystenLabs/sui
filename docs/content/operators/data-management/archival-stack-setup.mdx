...
| Option | Default | Description |
|--------|---------|-------------|
| `write-concurrency` | adaptive (initial 5, range 1–1024) | Integer for fixed concurrency, or an adaptive configuration table. |
...
The default adaptive writer concurrency starts at 5. Integer values select fixed concurrency, so the explicit values above reserve more concurrency for the highest-throughput pipelines (`objects`, `transactions`, `checkpoints`, `checkpoints_by_digest`). To opt in elsewhere, use an adaptive table such as `{ kind = "adaptive", initial = 5, min = 1, max = 100 }`.
