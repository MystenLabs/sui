...
- Access to the Sui checkpoint buckets for your target network:
   ...
   - **Mainnet full-history backfill bucket:** `gs://mysten-mainnet-checkpoints-use4` via `--remote-store-gcs mysten-mainnet-checkpoints-use4`. This bucket is Requester Pays enabled.
   - **Testnet endpoint:** `https://checkpoints.testnet.sui.io` with full history.
...
The public HTTPS checkpoint endpoints retain only the latest 30 days of checkpoints. For Mainnet backfill beyond 30 days, use `--remote-store-gcs mysten-mainnet-checkpoints-use4`. This bucket is Requester Pays enabled. Using `--remote-store-gcs` with valid credentials and a billing project avoids throttling and is the supported path for full-history backfill.
