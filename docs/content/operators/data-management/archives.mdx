...
Mainnet archival fallback requires AWS credentials for the S3 Requester Pays bucket. Supply credentials through the AWS default credential chain, preferably with an IAM role attached to the node. Do not put access keys in `fullnode.yaml`: the node configuration can be exposed through administrative diagnostics. State sync explicitly rejects `checkpoints.mainnet.sui.io` as an archive source. Testnet uses a public HTTPS endpoint that does not require credentials.
...
| `gs://mysten-mainnet-checkpoints-use4` | Full history | GCS credentials | Mainnet checkpoint ingestion for indexers (GCS Requester Pays) |
| `gs://mysten-testnet-checkpoints-use4` | Full history | GCS credentials | Testnet checkpoint ingestion for indexers (GCS Requester Pays) |

:::caution

Do not use `https://checkpoints.mainnet.sui.io` for Mainnet archival fallback. State sync rejects this endpoint. Use the S3 or GCS bucket instead.
