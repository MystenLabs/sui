...
Mainnet archival fallback requires the S3 Requester Pays bucket with AWS credentials. State sync explicitly rejects `checkpoints.mainnet.sui.io` as an archive source. Testnet uses a public HTTPS endpoint that does not require credentials.
