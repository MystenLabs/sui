...
| `--remote-store-header` | Arbitrary HTTP header to include in requests to the remote store, in `Key: Value` format. Can be specified multiple times for multiple headers. |
...
For remote stores requiring custom authentication or other HTTP headers, use `--remote-store-header "Header-Name: value"` to pass arbitrary headers with each request to the remote store. You can specify this flag multiple times to include multiple headers.
