...
If a pipeline's `checkpoint_hi_inclusive` is significantly behind the network tip, that pipeline is lagging. Compare against the current network checkpoint (visible on [SuiVision](https://suivision.xyz/) or through `curl -s http://localhost:9184/metrics | grep watermark_checkpoint`). The [`watermark_checkpoint` metric](https://github.com/MystenLabs/sui/blob/main/crates/sui-indexer-alt-framework/src/metrics.rs) tracks each pipeline's current position; `watermark_checkpoint_in_db` shows the last position persisted to the database.

### Diagnose a stalled pipeline

If a pipeline's checkpoint stops advancing:

1. Check the indexer logs for errors related to that pipeline. Filter by pipeline name: `grep "PIPELINE_NAME" indexer.log`.
2. Verify the Postgres connection is healthy. A connection timeout or lock contention can stall all pipelines.
3. Check for autovacuum contention. Long-running autovacuum on a pipeline's table can block writes. Query `pg_stat_activity` for vacuum processes.
4. Restart the indexer. It resumes from the last recorded watermark automatically.
...
To fix, ensure `cp_sequence_numbers` is included in your indexer's pipeline list. If it was not included during initial setup, follow the [adding a new pipeline](#adding-a-new-pipeline) procedure to add it.
...
| Watermark not advancing after restart | `--first-checkpoint` flag ignored because watermark exists | Delete the watermark record for that pipeline (see [re-indexing](#re-indexing-from-a-different-checkpoint)). |
