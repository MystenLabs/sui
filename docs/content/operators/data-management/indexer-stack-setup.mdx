...
| `--http`, `--gcs`, `--azure`, `--s3`, or `--local` | Formal snapshot source. Specify exactly one. See [managed snapshots](/operators/snapshots#mysten-labs-managed-snapshots). |
...
You can run the GraphQL RPC server with a configuration file to customize settings. To customize settings, generate a configuration file using the following command and edit it as needed:
