...
Each TOML file can include an optional `[ingestion]` section to tune checkpoint ingestion. The most common setting is `min-cohort-boundary`, which controls how pipelines are grouped into ingestion cohorts based on their distance from the tip. The default is `25000` checkpoints. A higher value prevents nearly caught-up pipelines from being fragmented into many small cohorts. You do not need to set this for most deployments.
