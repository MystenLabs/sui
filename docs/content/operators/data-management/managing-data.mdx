...
2. **Checkpoints:** Sui groups committed transactions in checkpoints, and then uses those checkpoints to achieve state synchronization. Checkpoints keep transaction digests that contain additional integrity metadata. Sui full nodes require data from checkpoints to achieve state synchronization, so you cannot prune this data.
