...
| `--remote-store-http-header <KEY:VALUE>` | Set an arbitrary HTTP header on requests to the remote store. Can be repeated to set multiple headers. Use this when the remote store requires authentication or custom headers, for example `Authorization: Bearer <token>`. |
...
To set custom HTTP headers for requests to the remote store (for example, when the store requires authentication), add them to the `[ingestion]` section:

```toml
[ingestion]
remote-store-http-headers = [["Authorization", "Bearer <token>"], ["X-Custom-Header", "value"]]
```
