...
description: >-
  Setup instructions for running the checkpoint blob indexer to populate a
  remote store with protobuf checkpoint blobs.
keywords:
  - remote store
  - checkpoint
  - blob
...
goal:
  description: >-
    Operator can run the checkpoint blob indexer to populate a remote store with
    protobuf checkpoint blobs
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I run a remote store?
  - How do I set up running a remote store?
  - What are the requirements for running a remote store?
answer: >-
  Setup instructions for running the checkpoint blob indexer to populate a
  remote store with protobuf checkpoint blobs.
---
...
The easiest way to populate your own remote store on Mainnet or Testnet is to backfill from Mysten's requester-pays GCS buckets. Use `--remote-store-gcs` as the source and a destination flag (`--s3` or `--gcs`) to write to your store.
