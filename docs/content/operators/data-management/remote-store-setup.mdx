...
The most straightforward way to populate your own remote store is to backfill from Mysten's public GCS buckets. Use `--remote-store-gcs` as the source and a destination flag (`--s3` or `--gcs`) to write to your store.
...
You can also use `--remote-store-url https://checkpoints.mainnet.sui.io` instead of `--remote-store-gcs`, but the GCS option is faster for backfill because it uses the GCS API directly rather than HTTPS.
