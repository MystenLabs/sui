...
:::info
The JSON-RPC API used in some examples on this page is deprecated. For new integrations, use the gRPC or GraphQL APIs. See the [Sui API documentation](/sui-api-ref) for migration guidance.
:::
...
You can set up and configure a [Sui full node](/operators/full-node/sui-full-node) using Docker or directly from source code in the Sui GitHub repository.

## Set up Sui addresses

Sui addresses do not require onchain initialization, you can spend from an address if it corresponds to your private key. You can derive a 32-byte Sui address by hashing the signature scheme flag byte concatenated with public key bytes `flag || pubkey` using the [BLAKE2b](https://www.blake2.net/) (256 bits output) hashing function.
...
The following bash example demonstrates how to use `suix_getBalance` for address `0x849d63687330447431a2e76fecca4f3c10f6884ebaa9909674123c6c662612a3`. Replace the value for `rpc` with the URL to the appropriate full node for your target network.

```sh
rpc="https://fullnode.mainnet.sui.io:443"
address="0x849d63687330447431a2e76fecca4f3c10f6884ebaa9909674123c6c662612a3"
data="{\"jsonrpc\": \"2.0\", \"method\": \"suix_getBalance\", \"id\": 1, \"params\": {\"owner\": \"$address\"}}"
...
The following example demonstrates using `suix_getBalance` in Rust. This example uses the JSON-RPC Rust SDK, which is deprecated. For new integrations, use the gRPC SDK (`sui-rust-sdk` with `SuiGrpcClient`) or GraphQL instead.
...
async fn main() -> Result<(), anyhow::Error> {
   ...
   let balance = sui.read_api().get_balance(address).await?;
   println!("{:?}", balance);
   Ok(())
}
```
...
rpc="https://fullnode.mainnet.sui.io:443"
address="0x849d63687330447431a2e76fecca4f3c10f6884ebaa9909674123c6c662612a3"
data="{\"jsonrpc\": \"2.0\", \"id\":1, \"method\": \"suix_queryEvents\", \"params\": [{\"Sender\": \"$address\"}, null, null, true ]}"
curl -X POST -H 'Content-type: application/json' --data-raw "$data" $rpc
```
...
Sui is a DAG-based blockchain and uses checkpoints for node synchronization and global transaction ordering. Checkpoints differ from blocks in the following ways:
 - Sui creates checkpoints and adds finalized transactions. Transactions are finalized even before they are included in a checkpoint.
 - Checkpoints do not fork, roll back, or reorganize.
 - Sui creates approximately four checkpoints every second. Find the most up-to-date statistic on the [Sui public dashboard](https://metrics.sui.io/public-dashboards/4ceb11cc210d4025b122294586961169).
...
Sui Checkpoint API operations include:
 ...
 - [`sui_getCheckpoints`](/sui-api-ref#sui_getCheckpoints): Retrieves a paginated list of checkpoints that occurred during the specified interval. Available in a future release.
...
Sui supports the following API operations related to transferring SUI between addresses:
 ...
 - [`sui_payAllSui`](/sui-api-ref#sui_payAllSui): This method accepts an array of SUI token IDs. It merges all existing tokens into one, deducts the gas fee, then sends the merged token to the recipient address.
   ...
   The method is especially useful if you want to transfer all SUI from an address. To merge together all coins for an address, set the recipient as the same address. This is a native Sui method and does not produce additional onchain transaction objects for the internal merge operation.
...
See [Sui Signatures](/develop/transactions/transaction-auth/auth-overview) for more details on signature validity requirements.
