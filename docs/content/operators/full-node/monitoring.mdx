...
Nodes expose metrics on `localhost:9184/metrics` by default. During formal snapshot restoration (`sui-tool download-formal-snapshot`), the restore process exposes metrics on `localhost:9185/metrics` instead. If port 9185 is unavailable, pass `--metrics-port <PORT>`; the restore process exits if it cannot bind its metrics port.
