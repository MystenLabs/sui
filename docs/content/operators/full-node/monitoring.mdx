...
Nodes expose metrics on `localhost:9184/metrics` by default. During formal snapshot restoration (`sui-tool download-db-snapshot`), the restore process exposes metrics on `localhost:9185/metrics` instead.
