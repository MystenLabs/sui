...
Nodes expose metrics on `localhost:9184/metrics` by default. During formal snapshot restoration (`sui-tool download-formal-snapshot`), the restore process exposes metrics on `localhost:9185/metrics` instead.
