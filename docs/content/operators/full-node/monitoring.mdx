...
- **Stale checkpoint:** `last_executed_checkpoint_timestamp_ms > 0 and time() - last_executed_checkpoint_timestamp_ms / 1000 > 120` sustained for 2 minutes indicates the node has stopped executing checkpoints. The `> 0` guard avoids false alerts during startup, when the timestamp has not yet been populated.
