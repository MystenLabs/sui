...
1. Stop the node.
2. Download the latest snapshot using `sui-tool download-formal-snapshot --latest` or by copying a RocksDB snapshot from the Mysten Labs managed buckets.
3. Place the snapshot at the correct `db_path` (see your `fullnode.yaml` configuration).
4. Restart the node.
