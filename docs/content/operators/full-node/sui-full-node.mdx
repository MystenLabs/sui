...
A full node creates the following directories under its configured `db-path` (inside the `live/` subdirectory):

| Directory | Purpose | When created |
|-----------|---------|--------------|
| `store/` | Core chain state (objects, transactions, effects). Do not delete. | Always |
| `epochs/` | Committee (validator set) data per epoch. Do not delete. | Always |
| `checkpoints/` | Checkpoint summaries and contents. Do not delete. | Always |
| `rpc_store/` | gRPC index data (derived indexes and ledger history). Required for gRPC filtered queries and list APIs. | When `rpc.enable-indexing: true` |
| `indexes/` | Legacy JSON-RPC indexes (transaction lookups by address, object queries). Safe to delete after migrating to gRPC. | When `enable-index-processing: true` |
| `rpc-index/` | Predecessor to `rpc_store/`. Automatically deleted on startup when `rpc.enable-indexing: true`. | Never (legacy) |
