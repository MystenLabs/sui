...
1. Stop the node.
2. Download and restore the latest snapshot. See [Sui Snapshots](/operators/snapshots) for the complete command, which requires `--genesis`, `--path`, and network flags in addition to `--latest`.
3. Verify the snapshot is placed at the correct `db_path` (see your `fullnode.yaml` configuration).
4. Restart the node.
