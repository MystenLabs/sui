...
Your full node creates the following directories under its configured `db-path` (inside the `live/` subdirectory):

| **Directory** | **Purpose** | **When your node creates it** |
|---------------|-------------|-------------------------------|
| `store/` | Stores core chain state (objects, transactions, effects). Do not delete it. | Always |
| `epochs/` | Stores committee (validator set) data for each epoch. Do not delete it. | Always |
| `checkpoints/` | Stores checkpoint summaries and contents. Do not delete it. | Always |
| `rpc_store/` | Stores gRPC index data (derived indexes and ledger history). gRPC filtered queries and list APIs require it. | When you set `rpc.enable-indexing: true` |
| `indexes/` | Stores legacy JSON-RPC indexes (transaction lookups by address and object queries). You can delete it after migrating to gRPC. | When you set `enable-index-processing: true` |
| `rpc-index/` | Stores the predecessor to `rpc_store/`. Your node deletes it on startup when you set `rpc.enable-indexing: true`. | Never (legacy) |

To reclaim disk space after migrating to gRPC, disable legacy JSON-RPC indexing (see below), then stop the node and delete the `live/indexes/` directory. If `rpc-index/` exists and you have `rpc.enable-indexing: true`, the node removes it automatically on the next startup. Otherwise, delete it manually while the node is stopped.

### Disable legacy JSON-RPC indexing
