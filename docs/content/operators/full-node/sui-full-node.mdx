...
| `9184` | TCP | Prometheus metrics | No (localhost only) |
| `1337` | TCP | Admin interface | No (localhost only) |
...
- **Block:** `9184` (metrics) and `1337` (admin) should never be reachable from untrusted networks. Bind them to `127.0.0.1`.
...
3. **Confirm checkpoint execution.** Verify that `last_executed_checkpoint` advances over time and `checkpoint_exec_errors` stays at 0.

A node that syncs checkpoints, executes them without errors, and reports the correct chain identifier is ready to serve read traffic. You do not need to execute a test transaction unless the node is expected to submit transactions for clients.
