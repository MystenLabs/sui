...
| `rpc-index/` | Predecessor to `rpc_store/`. Automatically deleted on startup. No action needed. | Never (legacy, auto-removed) |

To reclaim disk space after migrating to gRPC, disable legacy JSON-RPC indexing (see below), then stop the node and delete the `live/indexes/` directory. The `rpc-index/` directory is removed automatically on startup.
