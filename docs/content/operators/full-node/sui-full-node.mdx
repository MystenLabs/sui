...
## State synchronization 
Sui full nodes sync through the state sync protocol, which disseminates certified [checkpoints](/develop/sui-architecture/checkpoint-verification) between nodes. A full node does not need to follow individual validators to learn about committed transactions.
