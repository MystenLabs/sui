...
description: >-
  Database snapshots of the Sui network enable full node operators a way to
...
goal:
  description: >-
    Operator understands database snapshots of the Sui network enable full node
...
questions:
  - How do I use Sui database snapshots?
  - How do I restore a Sui node from a snapshot?
  - Where can I download Sui node snapshots?
answer: >-
  Database snapshots of the Sui network enable full node operators a way to
  bootstrap a full node without having to execute all the transactions that
  occurred after genesis.
...
Formal snapshots provide a mechanism for a node to restore to a canonical state (the state of a full pruned and compacted node at the end of an epoch) without having to execute all the transactions that have occurred since that epoch. Unlike existing database snapshots, these formal snapshots have the following properties:
...
1. Download the snapshot to a temporary directory using cloud object store tooling. For example, if the `db-path` value points to `/opt/sui/db/authorities_db/full_node_db` and you want to restore from epoch 10, use one of these commands:
...
See [Install Sui](/getting-started/onboarding/sui-install) for more details.
