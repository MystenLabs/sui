...
* The free buckets (for example, `sui-tool download-formal-snapshot --no-sign-request`) are only available for formal snapshots. You must use one of the requester pays (S3 or GCS) buckets if downloading a db snapshot.
