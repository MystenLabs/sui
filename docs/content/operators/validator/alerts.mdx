...
sum(
  ...
  and
  histogram_quantile(0.95, sum by (le, host) (
    rate(sequencing_certificate_latency_bucket{network="$network", processed_method="processed_via_consensus", submitted="submitted", tx_type=~".*user_transaction.*|soft_bundle"}[2m])
  )) < 2
) < 5000
```
