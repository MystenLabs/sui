...
:::caution
The `tx_type` metric classification labels `shared_certificate` and `owned_certificate` have been collapsed into `_deprecated_certificate`. If you have existing alert rules or dashboards that filter on `shared_certificate` or `owned_certificate`, update those queries to use `_deprecated_certificate` instead.
:::
