...
:::caution
The `shared_certificate` and `owned_certificate` label values for `tx_type` are deprecated. The `classify()` metric now emits `_deprecated_certificate` in place of both. Update any existing queries that reference `shared_certificate` or `owned_certificate` to use `_deprecated_certificate` instead, or those matchers will silently match nothing.
:::
