...
The `classify()` metric no longer emits `shared_certificate` or `owned_certificate` as `tx_type` label values. These have been collapsed into `_deprecated_certificate`. Similarly, `UserTransaction` (v1) now emits `_deprecated_user_transaction`. If you have existing alert rules deployed in your monitoring system that use `tx_type=~"shared_certificate|owned_certificate"`, those rules will silently stop matching active data and will no longer fire. Update your deployed alert rules to use the new label names shown in the query below.
