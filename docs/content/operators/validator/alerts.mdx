...
:::caution
The `tx_type` label values `shared_certificate` and `owned_certificate` are deprecated. The `ConsensusAdapter` metric classifier no longer emits these labels. They have been replaced by `_deprecated_certificate`. If you have copied this query into your own monitoring system, update the `tx_type` filter accordingly.
:::
