...
questions:
  - What is Validator Node Management in Sui?
  - How do I set up validator node management?
  - What are the requirements for validator node management?
answer: >-
  As a validator on Sui, there are some processes you need to perform to ensure
...
- **Revoke after the task completes.** Once a delegated task is done, rotate the cap with `sui_system::rotate_operation_cap` to invalidate the old one, and rotate immediately if a delegatee key is lost or compromised.
