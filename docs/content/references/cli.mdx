...
goal:
  description: >-
    Reader can reference sui provides command line tools to interact with the
    network, its features, and the Move programming language. Individual command
    groups and utilities are referred to as Sui Client CLI, Sui Keytool CLI, Sui
    Move CLI, Sui Completion CLI, and Sui Validator CLI
  requires:
    - has_frontmatter:
        - title
        - description
