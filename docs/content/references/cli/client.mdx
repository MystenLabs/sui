...
In this example, let's see how to transfer SUI or transfer an object from one address to another. First all, there two main commands for sending SUI or transfering objects: Pay and Transfer. 
Each of these commands, has some sister commands: PaySui, PayAllSui, and TransferSui. The main difference between Pay and Transfer is that Pay allows you to send coin to multiple recipients, whereas Transfer only accepts one recipient. Another important difference is that Pay and Transfer require a gas object that is different than the one used to send the coins, which is used to pay gas fees. 
In contrast, PaySui, PayAllSui, and TransferSui, do not require a separate gas object. The input coin(s) will be used as the gas object.

Let's assume we have two addresses:
...
We want to send 0.5 SUI to `eloquent-amber`. Given that we have a few gas coins, we can use Pay. If we would have had only one gas coin, then we could have only used TransferSui, or PaySui, or we would have needed to split the coin first to have another coin to use for paying gas. In this case, let's use the TransferSui command as we will not need to provide a gas coin to be used for the gas fees. We need to pass the address (or its alias) of the address to which SUI is sent, which coin to use to transfer SUI from, and the amount of SUI to transfer.

```
sui client transfer-sui --to eloquent-amber --sui-coin-object-id 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --amount 500000000 --gas-budget 5000000
```
...
We can check the gas for the sender and the recipient's coins using `sui client gas <address>`. The sender now has 199.49 SUI for that gas coin that was used. 0.5 SUI was transferred, and the rest 0.01 SUI were the gas fees.
```
sui client gas
╭────────────────────────────────────────────────────────────────────┬────────────────────┬──────────────────╮
│ gasCoinId                                                          │ mistBalance (MIST) │ suiBalance (SUI) │
...
If you'd like to send coins to multiple addresses, use one of the `Pay*` commands.

If you'd like to transfer the whole object, you can use `sui client transfer-sui` and do not pass an amount:
```
...
Then check again the gas for `eloquent-amber`
```
sui client gas eloquent-amber
╭────────────────────────────────────────────────────────────────────┬────────────────────┬──────────────────╮
│ gasCoinId                                                          │ mistBalance (MIST) │ suiBalance (SUI) │
