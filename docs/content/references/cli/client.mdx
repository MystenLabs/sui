...
In this example, you learn how to transfer SUI or transfer an object from one address to another. There are two main commands for sending SUI or transferring objects: `pay` and `transfer`.
...
- commands that end in `-sui` deal with Sui's native coin, and they use the input coins to pay for gas and for transferring SUI or the object.
...
You want to send 0.5 SUI to `eloquent-amber`. Given that you have a few gas coins, you can use `pay`. If only one gas coin exists, then you need to use `transfer-sui` or `pay-sui`, or you would need to split the coin first to have another coin to use for paying gas. In this case, use the `pay-sui` command because you do not need to provide a separate gas coin for the gas fees. In the command below, you set the recipient to be `eloquent-amber`, which coin to use to transfer SUI from, and the amount of SUI to transfer.
...
### Send funds using send-funds

Use `sui client send-funds` to transfer a specific amount of SUI to a recipient address in a single command. This is a convenience wrapper around the programmable transaction block (PTB) pattern for simple SUI transfers. For more detail on the underlying PTB approach, see [Using Address Balances](/concepts/transactions/prog-txn-blocks).

```sh
$ sui client send-funds --to eloquent-amber --amount 500000000 --gas-budget 5000000
```

### Transfer an object using party-transfer

Use `sui client party-transfer` to transfer an object under party ownership. This command is used when an object has been set up with party ownership semantics.

```sh
$ sui client party-transfer --object-id 0xfffbb30ccb631f15f6cd36700589fc9c31cb04af28a95f3ed26d62daf3acb57f --to eloquent-amber --gas-budget 5000000
```

### Serialize a transaction for offline signing

Use `sui client serialized-tx` to build and serialize a transaction without executing it. This is useful for offline signing workflows where you construct the transaction on one machine and sign it on another.

```sh
$ sui client serialized-tx --to eloquent-amber --sui-coin-object-id 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --amount 500000000 --gas-budget 5000000
```

Use `sui client serialized-tx-kind` to serialize only the transaction kind (without gas data), which is useful when a sponsor will provide the gas payment separately.

```sh
$ sui client serialized-tx-kind --to eloquent-amber --sui-coin-object-id 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --amount 500000000
```

### Test publish and upgrade locally

Use `sui client test-publish` to simulate a package publish without submitting the transaction to the network. This is useful for verifying that a package builds and passes all checks before publishing.

```sh
$ sui client test-publish
```

Use `sui client test-upgrade` to simulate a package upgrade without submitting the transaction to the network.

```sh
$ sui client test-upgrade --upgrade-capability 0xde670ae990c8f20fd53e9f597a0da056bdb634175176602658e9da2c2ec9cb93
```

For more detail on publishing and upgrading packages, see the [package management guide](/guides/developer/app-examples/publish).

### Execute a combined signed transaction

Use `sui client execute-combined-signed-tx` to submit a transaction that has been signed by multiple parties. This command is useful in multi-party signing workflows where each party signs the serialized transaction bytes offline before submission.

```sh
$ sui client execute-combined-signed-tx --tx-bytes <BASE64-TX-BYTES> --signatures <SIG1> <SIG2>
```
