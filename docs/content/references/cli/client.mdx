...
description: >-
  The Sui Client CLI provides command-level access to interact with the Sui
  network.
keywords:
  - sui cli
  - sui client cli
...
questions:
  - What is Sui Client CLI in Sui?
  - How do I publish a move package?
  - What are the available sui client cli commands?
answer: >-
  The Sui CLI `client` command provides command-level access to interact with
  the Sui network.
---
...
Use the `sui client chain-identifier` command to query the chain identifier of the current network. The command prints it in both encodings: the full Base58-encoded genesis checkpoint digest (the format returned by the gRPC and GraphQL APIs) and the legacy hex short form (its first 4 bytes). Either value can be used as a chain ID in the `[environments]` section of `Move.toml`.

```sh
$ sui client chain-identifier
```
```sh
Base58: 4btiuiMPvEENsttpZC7CZ53DruC3MAgfznDbASZ7DR6S
Hex: 35834a8a
```

Use the `--format` option to print only one encoding:

```sh
$ sui client chain-identifier --format base58
```
```sh
