---
title: Sui Completion CLI
description: Generate shell completion scripts for the Sui CLI.
keywords: [sui completion, sui cli completion, shell completion, bash completion, zsh completion, fish completion]
---

The Sui CLI `completion` command generates shell completion scripts for the `sui` command. Install the generated script for your shell to complete Sui commands, subcommands, flags, and values from your terminal.
...
Usage: sui completion --generate <GENERATOR>
...
Install the generated script in a directory loaded by your shell. Restart your shell after installation, or source the completion file directly when your shell supports it.

### Bash

For Bash, install the generated `sui.bash` file in a Bash completion directory. Common locations include:

- Linux: `/etc/bash_completion.d/sui` for all users, or `~/.local/share/bash-completion/completions/sui` for one user.
...
After copying or updating the completion file, you might need to remove the Zsh completion cache and start a new shell:
