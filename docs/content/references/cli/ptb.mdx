...
CLI PTBs requires in some cases to specify the type of a specific value / variable. For example, in the example below the `"<u64>"` type is required when calling the `0x1::option::is_none` function.
```bash
sui client ptb \
...
If you need to pass in multiple types, you can simply do so by delimiting them with a comma:
```bash
