...
:::
:::tip
If you include multiple PTBs from files and want to check the order of your transactions, use the `--preview` flag to display the PTB transaction list instead of executing it.
:::
...
The following examples demonstrate how to use the `client ptb` command. Most of these examples can be copied to a file and executed using `sui client ptb --file to_filepath`.
...
:::info
You can also pass an alias instead of an address.
:::
