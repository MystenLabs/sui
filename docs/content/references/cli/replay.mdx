...
---
description: The Sui Replay CLI provides a command to replay transactions and generate traces for the Move debugger.
---

Replay is a tool that allows you to replay a given transaction to check and analyze the transaction effects. In addition, you can enable tracing to generate traces that can be used
with the Move debugger to debug that transaction locally.
...
After running such a command, the tool generates a directory corresponding to the digest in the current directory: `<cur_directory>/.replay/<digest>`, unless the `--output-dir` flag is set.
A typical response after running the command above is:
```
