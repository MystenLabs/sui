...
With Replay, you can pass the `--trace` flag to generate execution traces that can be used with the Move debugger to debug that transaction locally. For more information about the Move debugger, see the [Move Debugger](../ide/debugger.mdx) page. You can additionally run different [analyses on the transaction traces](./trace-analysis.mdx) such as [gas profiling](./trace-analysis.mdx#profiling-a-transaction).
