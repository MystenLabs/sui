...
You can provide default values for the replay tool flags by specifying them in the `~/.sui/sui_config/replay.config` file, one flag per line. The file format supports comments starting with `#`. Flags specified on the command line take priority over flags specified in the config file.