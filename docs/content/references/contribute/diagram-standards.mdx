...
1. Download the latest release from the [Sui Docs Diagrams releases page](https://github.com/jon-mysten/sui-docs-diagrams/releases/latest). Two files are needed:
   - `sui-docs-claude-design.zip`, the design system content
   - `sui-docs-fonts.zip`, the brand fonts bundle (uploaded through a separate path)
...
4. When Claude Design displays the Missing brand fonts banner after onboarding, click the **Upload fonts** button and drop in `sui-docs-fonts.zip`. Claude Design's font analyzer is a separate ingestion path from the design system upload.
...
The Sui Docs Diagrams package ships with four static audits the package author runs before any diagram ships:
