...
goal:
  description: >-
    Contributor can use the custom MDX components available in Sui docs, such as
...
questions:
  - What is MDX Components in Sui?
  - Where can I find the mdx components reference?
  - How does mdx components work on Sui?
answer: Extensions to the Sui docs rendering engine.
---
...
When using multiple instances of `<Tabs>` within a page, use `groupId` to persist selections across sections. The `value` of each `<TabItem>` across `<Tabs>` components using `groupId` must match for the tabs to function properly.
