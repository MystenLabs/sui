...
When creating process documents it's best to use an ordered list with each step consisting of a single instruction for the user. On occassion, however, you might want to have a group of instructions for each step. The step titles enable the use of numbered steps without having to maintain the proper number order. 

### When to use

Use when you detail a long process that deals with multiple technologies or otherwise requires lengthy contextual content. You don't, for instance, want to create a 30-step ordered list or one where list item 1 is three full page scrolls to get to list item 2. Having steps in the title of longer processes helps users recall where they left off and more easily navigate back using the ToC if they don't complete the task set.

### How to use

To create step headings, add a `step` or `substep` suffix to the usual heading levels. 

For instance, to create a level 2 heading, you'd write `##step Buy some peanuts`. The end result of this would be the same as typing `## Step 1: Buy some peanuts`.

The feature supports one level of nesting using the `substep` suffix in the same manner. Don't forget to use the proper number of hashes in relation to the parent step. Continuing the example, you might have `###substep Buy some Cracker Jacks`. Which would be the same as using `### Step 1.1: Buy some Cracker Jacks`.