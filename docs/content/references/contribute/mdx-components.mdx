...
When creating process documents it's best to use an ordered list with each step consisting of a single instruction for the user. On occasion, however, you might want to have a group of instructions for each step. The step titles enable the use of numbered steps without having to maintain the proper numbered step order. 
