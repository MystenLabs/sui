...
If a guide or tutorial uses the following prerequisites, a reusable snippet can be used:
...
The reusable snippet should be used with:

```jsx
<ImportContent source="prerequisites.mdx" mode="snippet" />
```

For guides and tutorials that have their own unique prerequisites, the following `Tabs` component with the `className="tabsHeadingCentered--small"` class should be used:
