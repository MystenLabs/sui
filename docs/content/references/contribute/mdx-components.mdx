...
Sui documentation uses a custom Docusaurus plugin to include code from the Sui repo in its pages. 
...
To include the entire source file, use the `{@inject: ...}` directive with a path to the file, based on the Sui repo root. For example, to include the source file at `sui/examples/move/hero/sources/example.move`, write:
...
For code located outside of the Sui repo, you can reference it via its public GitHub URL.

### When to use 

Whenever code located outside the Sui monorepo is to be embedded as a code block in the Sui documentation.

### How to use 

Use a code block, plus the syntax highlight type and 'reference' keyword, followed by the public GitHub URL:

    ```move reference 
    https://github.com/MystenLabs/deepbookv3/blob/master/packages/deepbook/sources/pool.move
    ```

To reference only specific lines in the code, add #LX-LX to the end of the URL, such as: 

    ```move reference 
    https://github.com/MystenLabs/deepbookv3/blob/master/packages/deepbook/sources/pool.move#L105-L134
    ```
