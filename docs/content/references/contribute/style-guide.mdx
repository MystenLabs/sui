...
| ---------------------- |---------------------------------------------- | -------------- |
| **Always capitalized** | [Proper nouns](#proper-nouns), [Product names](#product-names), Archival Store and Service, Archival Store, Archival Service, DeepBook Indexer, DeepBookV3, Devnet, ID, Localnet, Kiosk (when referring to the standard), Mainnet, Mysticeti, Operation Cap, Sui, Sui CLI, Sui Client PTB CLI, Sui Closed-Loop Token / Closed-Loop Token, Sui dApp Kit, Sui Explorer, SuiJSON, Sui Keystore, Sui Keytool, SuiLink, Sui Object Display, SuiPlay0X1, SUI, SUI token, Testnet, Wallet Standard, Web2, Web3, zkSend SDK | Use capitalization consistently for branding and product-specific references |
| **Always lowercase**   | archival node, bridge node, dApp, kiosk (when referring to an instance), object, oracle, recovery passphrase (mnemonic), soulbound, Sui framework, Sui object, validator node, wallet | Use lowercase when referring to general concepts rather than branded terms   |
...
When referencing nodes (full, archive, validator, and so on), use:
...
- [Object and Package Versioning](/concepts/versioning): Versioning provides the ability to upgrade packages and objects on the Sui network.
