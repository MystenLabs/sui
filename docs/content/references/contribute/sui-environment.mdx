...
You can interact with the Sui network through two official SDKs (TypeScript SDK and Rust SDK), and by using the Sui CLI. For more details about using the Sui CLI, see [Install Sui](../../guides/developer/getting-started/sui-install.mdx) and the [Sui CLI](/references/cli.mdx) reference.
