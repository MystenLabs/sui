...
title: Gaming on Sui
description: >-
  Sui offers features like dynamic NFTs, Kiosk, soulbound assets, and onchain
  randomness to provide builders with the tools to create immersive,
  transparent, and fair gaming experiences.
keywords:
  - gaming
  - game components
  - game features
...
goal:
  description: >-
    Game developer can learn how to use Sui features such as dynamic NFTs,
    Kiosk, soulbound assets, and onchain randomness to build games
  requires:
    - has_frontmatter:
        - title
        - description
    ...
    - has_tables: true
      min: 1
      label: Has reference table
    - has_questions: true
      label: Needs questions for AI search visibility
    - has_answer: true
      label: Needs answer summary for AI citation
questions:
  - What gaming tools and features does Sui provide?
  - How do I build onchain games with Sui?
  - 'What are dynamic NFTs, Kiosk, and soulbound assets?'
answer: >-
  Sui offers features like dynamic NFTs, Kiosk, soulbound assets, and onchain
  randomness to provide builders with the tools to create immersive,
  transparent, and fair gaming experiences.
...
Sui provides [native randomness](/sui-stack/on-chain-primitives/randomness-onchain), allowing games to operate without requiring players to trust a centralized operator.
...
Offchain computation can also be verified onchain by using [Nautilus](/sui-stack/nautilus/nautilus-design). 
...
Using Sui, players can now have [true ownership of their in-game currency](/onchain-finance/examples-patterns/in-game-currency). The currency exists as a [token on the Sui blockchain](/onchain-finance/fungible-tokens/currency), where players can conceivably buy, sell, or swap that currency for anything else that also lives onchain. If you create an ecosystem of games, players can purchase currency in one of your games but spend it in another one that also uses that same currency. The possibilities are vast, but you must first learn how to create the currency.
...
The [Sui Object Display](/develop/objects/display/display-overview) standard is a tool that helps define how objects appear in apps and interfaces. It works like a template where you can insert data from an object to control how it's displayed. Use it to manage how different types of data are shown, even if the data itself is stored onchain while the display happens offchain (in apps or websites).
...
On Sui, owned objects are either freely transferable or non-transferable. To ensure royalties, Sui provides a standard called [Kiosk](/onchain-finance/kiosk). A kiosk is a shared object that restricts access to a single address or user.
...
Enoki transactions can be signed without requiring confirmation from the player to approve the transaction. However, players must still approve transactions through their wallet or authentication method.
...
Dynamic NFTs on the Sui network allow vehicles to securely update attributes and metadata over time. Every upgrade and customization is recorded onchain, ensuring each vehicle's uniqueness and authenticity.
...
Onchain logic enables secure and transparent management of events and betting systems. The Sui network ensures fairness, with immutable records of bets and outcomes.
...
By leveraging Sui's onchain randomness, ArcaneBattles introduces unpredictable elements such as random card draws, critical hit chances, and random effects from certain cards. This randomness is verifiable and secure, preventing manipulation by any party and maintaining fairness across all gameplay aspects.
