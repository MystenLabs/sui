...
Move source code is not stored on chain and a debugging session for an onchain transaction will start in disassembly mode by default. To enable source-level debugging, you can point the debugger at a local package directory containing package sources. The package must be built locally to also include debugging metadata required to enable source-level debugging.
...
Using the newest version of the package sources might still work, particularly for packages whose existing functionality is largely stabilized and its source code rarely changes, such as Sui framework package. However, it will not work if the package was already upgraded on chain after the debugged transaction executed and metadata in its package directory reflects this (in which case you need to obtain package sources from before the upgrade).

Use the button on top of the window displaying the trace file to add sources to the debugger before starting the debugging session. The explanation on how this machinery works under the hood and how you can add sources to the debugger manually is provided below but is intended only for the advanced users.

## Advanced source-level debugging instructions

:::info
    These instructions are intended for advanced users and should be followed only if the default method of adding sources to the debugger via the debugger's UI does not work.
:::


Data downloaded from the network for debugging a transaction does not contain the source code for Move calls executed throughout the transaction. You can, however, provide the sources along with additional debugging metadata by hand to enable source-level debugging, if you have access to the source code of the exact versions of packages used throughout the transaction. Below are the instructions on how to handle this simplified case, followed by instructions on how to locate precise versions of packages.
