...
**II. Start debugging**
   1. Open the subdirectory containing the transaction data downloaded for a transaction with the given digest. Open the trace file `trace.json.zst`.
...
<span id="metadata-generation">**Metadata generation**</span>
   ...
   1. Clone the repository, unless you already have the Sui source code repository available.

        ```bash
        git clone https://github.com/MystenLabs/sui.git
        ```

   1. Build Sui framework package source code.

        ```bash
        cd sui/crates/sui-framework/packages/sui-framework; sui move build
        ```

   1. Copy the debugging metadata to the right location.
...
1. Go to the `deepbook` package directory.

    ```bash
    cd deepbookv3/packages/deepbook
    ```

1. Check the version tags.
...
1. Build the package and copying the debugging metadata.
