---
title: Manifest Reference
description: Explore examples of package manifest files.
keywords:
  - move
  - move reference
  - package management
  - package manager
  - move packages
  - move package management
pagination_prev: null
goal:
  description: Reader can look up manifest reference syntax and configuration options
  requires:
    - has_frontmatter:
        - title
        - description
      label: Has required frontmatter fields
    - min_words: 100
      label: Needs more reference content
    - has_questions: true
      label: Needs questions for AI search visibility
    - has_answer: true
      label: Needs answer summary for AI citation
questions:
  - What fields are available in Move.toml?
  - How do I configure Move.toml for different networks?
  - What is the Move package manifest format?
answer: Explore examples of package manifest files.
---

Explore examples of package manifest files.

This section provides sample manifest files for you to use as templates in your project.

For a basic project that only depends on `sui` and `std`, the minimal lockfile is:

```toml
[package]
name = "example"
```

For a package that has a `mvr` dependency and a git dependency with different Mainnet and Testnet branches:

<!-- TODO: usdc doesn't actually have different tags; find a package that does -->

```toml
[package]
name = "example"

[dependencies]
ascii = { r.mvr = "@potatoes/ascii" }
usdc = { git = "https://github.com/circlefin/stablecoin-sui.git", subdir = "packages/usdc", rev = "releases/testnet" }

[dep-replacements.mainnet]
usdc = { git = "https://github.com/circlefin/stablecoin-sui.git", subdir = "packages/usdc", rev = "releases/mainnet" }
```

For a package that defines a `testnet_alpha` environment:

```toml
[package]
name = "example"

[environments]
testnet_alpha = "4c78adac"

[dependencies]
ascii = { r.mvr = "@potatoes/ascii" }
usdc = { git = "https://github.com/circlefin/stablecoin-sui.git", subdir = "packages/usdc", rev = "releases/testnet" }

[dep-replacements.mainnet]
usdc = { git = "https://github.com/circlefin/stablecoin-sui.git", subdir = "packages/usdc", rev = "releases/testnet" }

[dep-replacements.testnet_alpha]
ascii = { use-environment = "ascii" }
usdc = { use-environment = "testnet" }
```

The chain ID on the right-hand side of an `[environments]` entry can be specified in either the legacy hex short form (for example, `4c78adac`) or the full Base58 form (for example, `69WiPg3DAQiwdxfncX6wYQ2siKwAe6L9BZthQea3JNMD`). Both formats are accepted interchangeably. Run `sui client chain-identifier` to find the chain ID for your network in both formats.
