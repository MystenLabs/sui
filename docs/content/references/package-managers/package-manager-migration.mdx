...
The new system provides several benefits:
 ...
 - **Designed for multiple environments**: you should never have to have multiple `Move.toml` files for the same package
 ...
 - **Other conveniences**: faster git downloads, stored upgrade capabilities, tooling for local publications
...
## TLDR: Migrating Move.toml

Although the new package system will continue to work with existing packages, we encourage you to migrate to the new format as soon as possible. The steps to migrate are:
...
For example, if your old-style `Move.toml` was
...
Then your new-style `Move.toml` would be
...
## TLDR: Publishing on devnet and localnet
...
Instead of writing to `Published.toml`, the `test-publish` command writes the addresses to an ephemeral publication file (e.g. `Pub.localnet.toml`). You can select the file to use with the `--pubfile-path` option. If you want to publish multiple packages, you should use the same ephemeral publication file for all of the packages, so that the dependency addresses will be set correctly. You can also automate this with the `--publish-unpublished-deps` flag [TODO].

:::caution
If you were using `--with-unpublished-deps`, you should almost certainly use `--publish-unpublished-deps` instead.
:::

The `test-publish` command requires a "build environment" (testnet or mainnet by default, although you can [define your own][TODO]) to determine which dependencies to use for the ephemeral publication. You specify the build environment by passing the `--build-env <env>` flag.
...
For [ephemeral publication](TODO), the package system will use ephemeral files (often named `Pub.<env>.toml`); these files should not be committed to source control. We recommend adding `Pub.*.toml` to your `.gitignore` file.
