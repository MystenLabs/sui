...
title: Sui RPC
description: >-
  SuiJSON is a JSON-based format with restrictions that enable Sui to align JSON
  inputs more closely with Move call arguments, including type coercion rules
  for Move types.
keywords:
  - SuiJSON
  - json
...
goal:
  description: Reader can look up how SuiJSON aligns JSON inputs with Move call arguments
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Sui RPC in Sui?
  - Where can I find the sui rpc reference?
  - How does sui rpc work on Sui?
answer: >-
  SuiJSON is a JSON-based format with restrictions that allow Sui to align JSON
