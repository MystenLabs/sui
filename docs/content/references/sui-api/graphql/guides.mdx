...
The service accepts as input for each request the following header(s):
* `x-sui-rpc-version` to specify which RPC version to use (note that this is currently not available)
* `x-sui-rpc-show-usage` will return the query complexity information. 
...
For example, the following query retrieves the first 20 transaction blocks (along with the sender's address, the gas object used to pay for the transaction, the gas price, and the gas budget) after a specific one, at epoch `97`. Previously, in JSON RPC, this required multiple steps.
...
Fragments are reusable units that can be included in the queries as needed. The official [docs](https://graphql.org/learn/queries/#fragments) contain more information about fragments. Below you can find an advanced example of how fragments are used for querying a dynamic field of an owner.
