...
Include the `query` information in the `data` parameter of the cURL `POST` request to query the endpoint:
```shell
# basic query
...
```
GraphQL supports passing in [variables](#working-with-variables) for ease of reuse, and these need to be added in their own `variables` field as below.
```bash
