...
goal:
  description: Reader can look up unfamiliar Sui terms and concepts by name
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Glossary in Sui?
  - Where can I find the glossary reference?
  - How does glossary work on Sui?
answer: Find terms used in Sui defined below.
---
...
A long-term storage system that holds checkpoint-indexed Sui data, that is, Bigtable.
...
Single-writer objects are owned by one address. In Sui, transactions affecting only single-writer objects owned by the same address might proceed with only a verification of the sender's address, greatly speeding transaction times. These are basic transactions. See Single-Writer Apps for example applications of this basic transaction model.
...
A transaction in Sui is a change to the blockchain. This might be a basic transaction affecting only single-writer, single-address objects, such as minting an NFT or transferring an NFT or another token. These transactions might bypass the consensus protocol in Sui.
...
A transfer is switching the owner address of a token to a new one through a command in Sui. This is accomplished through the Sui CLI client command line interface. It is one of the more common of many commands available in the CLI client.
