...
A gRPC API layer that exposes access to this store, enabling point lookups of historical data, i.e., LedgerService
...
A token that can only be used for a specific service, by an authorized account, or a token that certain accounts can be blocked from using.
