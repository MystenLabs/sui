...
- "Queries" lists all top-level queries for reading the chain state, from reading details about addresses and objects to <UnsafeLink href="/references/sui-api/sui-graphql/beta/reference/operations/queries/simulate-transaction">dryRunTransactionBlock</UnsafeLink>, which has an execution-like interface but does not modify the chain, and <UnsafeLink href="/references/sui-api/sui-graphql/beta/reference/operations/queries/verify-signature">verifySignature</UnsafeLink>, which can be used to verify cryptographic signatures against a message and public key.
- <UnsafeLink href="/references/sui-api/sui-graphql/beta/reference/types/objects/object">Object</UnsafeLink> is the type representing all onchain objects (Move values and packages).
