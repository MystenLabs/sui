...
- "Queries" lists all top-level queries for reading the chain state, from reading details about addresses and objects to <UnsafeLink href="/sui-api-ref#sui_dryruntransactionblock">dryRunTransactionBlock</UnsafeLink>, which has an execution-like interface but does not modify the chain.
- "Mutations" lists operations that can modify chain state, like <UnsafeLink href="/sui-api-ref#sui_executetransactionblock">executeTransactionBlock</UnsafeLink>.
