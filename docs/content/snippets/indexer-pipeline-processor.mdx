The `Processor` handles multiple tasks running concurrently for maximum throughput. It's primary responsibility is to convert raw checkpoint data into database-ready rows using parallel workers.
