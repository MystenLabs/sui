...
description: Closed Loop Token is a standard token implementation on Sui which allows its creator to limit applications where the token is used, set up custom policies for transfers, spending and conversions.
---
