...
Some actions are disabled by default. Token creator can enable them by creating a "TokenPolicy" and specifying conditions (Rules) for their execution.

Token has 4 protected actions:

| Function           | Action Name | Description                     |
| ------------------ | ----------- | ------------------------------- |
| `token::transfer`  | "transfer"  | Transfer a Token to a recipient |
| `token::from_coin` | "from_coin" | Convert a Coin into a Token     |
| `token::to_coin`   | "to_coin"   | Convert a Token into a Coin     |
| `token::spend`     | "spend"     | Spend a Token                   |
...
- `spent_balance` - the balance of a spent Token in the ["spend" action](./spending) (can be utilized in custom actions)
...
```move
module example::custom_request {
    use std::string;
    use std::option;
    use sui::event;
    use sui::tx_context::TxContext;
    use sui::token::{Self, ActionRequest};

    /// See "Basics" for more details on creating a Token
    use example::my_token::MY_TOKEN;

    /// An event emitted when user; event already contains the "sender"
    struct UserSignedUp has copy, drop {}

    /// Actions can be freely created anywhere and used as an authorization tool
    public fun sign_up(ctx: &mut TxContext): ActionRequest<MY_TOKEN> {

        event::emit(UserSignedUp {}); // emit an event to notify the backend
        token::new_request(
            string::utf8(b"sign_up"), // name
            0, // amount
            option::none(), // recipient
            option::none(), // spent_balance
            ctx
        )
    }
}
...
For the example above to work, the TokenPolicy for the `MY_TOKEN` type must allow the `sign_up` action. See "TokenPolicy" section for more details.
...
2. With a [`TokenPolicy`](./token-policy) - token creator can create a shared `TokenPolicy` and set up allowed actions and requirements for each action. This way any application or a wallet would know which actions can be considered "public" and would be able to perform them.
3. With a `TokenPolicyCap` - the capability managing the `TokenPolicy` can also be used to confirm requests. This can be useful for applications that have the `TreasuryCap` wrapped and inaccessable; and some admin action needs to be authorized by the Token creator.

*TokenPolicyCap cannot be used to confirm "spend" requests (see [Spending](./spending))*
