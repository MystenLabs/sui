---
title: Token Policy
description: A `TokenPolicy` is a shared object that defines a token's allowed actions and conditions. `TokenPolicies` are often used by wallets and other financial services.
