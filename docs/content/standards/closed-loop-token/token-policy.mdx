...
`TokenPolicy` is a shared object which can be created by the Token owner using the TreasuryCap. Here's an example of how a policy can be created (with `transfer` operation allowed):
