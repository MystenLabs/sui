...
To add a Rule for an action, use the `token::add_rule_for_action` function. The function takes a `TokenPolicy` and `TokenPolicyCap` as arguments. The Rule must be specified by its type (eg `0x0...::denylist::Denylist`) and the Rule module must be imported in the TokenPolicy module.
