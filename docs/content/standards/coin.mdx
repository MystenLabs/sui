...
The list of addresses that aren't able to use a particular regulated coin is held within a system-created `DenyList` shared object. If you have access to the `DenyCap`, then you can use the `coin::deny_list_v2_add` and `coin::deny_list_v2_remove` functions to add and remove addresses. You can also use the `coin::deny_list_v2_contains_current_epoch` and `coin::deny_list_v2_contains_next_epoch` functions to check if an address is already on the list.

### Global pause switch

Regulated coin objects include an `allow_global_pause` Boolean field. When set to `true`, the bearer of the `DenyCapV2` object for the coin type is able to pause coin activity indefinitely. Immediately upon the bearer initiating the pause, the network disallows the coin type as input for any transactions. At the start of the next epoch (epochs last ~24 hours), the network disallows all addresses from receiving the coin type. 

When the bearer of the `DenyCapV2` object for the coin type removes the pause, the coins are immediately available to use again as transaction inputs. Addresses cannot receive the coin type, however, until the following epoch.  
...
Use the `coin::deny_list_v2_contains_current_epoch` and `coin::deny_list_v2_contains_next_epoch` functions to check if an address exists on the deny list for your coin in the current or next epoch, respectively.

{@inject: crates/sui-framework/packages/sui-framework/sources/coin.move#fun=deny_list_v2_contains_current_epoch noComments}

{@inject: crates/sui-framework/packages/sui-framework/sources/coin.move#fun=deny_list_v2_contains_next_epoch noComments}

The functions return `true` if the address is found on the coin's list, otherwise they return `false`.
