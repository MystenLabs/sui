...
Coins on Sui are fungible tokens by definition, which means there are many of a specific type (`Coin`) and each token of that type is the same as the next. If you created a regulated coin and named it REGCOIN, then you could mint as many of those tokens as you wanted and each REGCOIN would have the same properties as the next.
...
The `TreasuryCap` object is transferrable, so a third party can take over the management of a coin that you create if you transfer the `TreasuryCap`. After transferring the ability, however, you are no longer able to mint and burn tokens yourself.  
