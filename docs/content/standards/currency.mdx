...
Currency created with the `new_currency` function is immediately complete after the `finalize` function is called. However, for OTW created currencies, you must call `finalize_registration` to promote the currency to the registry.
