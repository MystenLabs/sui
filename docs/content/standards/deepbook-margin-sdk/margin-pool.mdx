...
```tsx
getId(coinKey: string);
deepbookPoolAllowed(coinKey: string, deepbookPoolId: string);
supplyCap(coinKey: string);
maxUtilizationRate(coinKey: string);
protocolSpread(coinKey: string);
minBorrow(coinKey: string);
```
...
```tsx
totalSupply(coinKey: string);
supplyShares(coinKey: string);
totalBorrow(coinKey: string);
borrowShares(coinKey: string);
lastUpdateTimestamp(coinKey: string);
```
...
```tsx
userSupplyShares(coinKey: string, supplierCapId: string);
userSupplyAmount(coinKey: string, supplierCapId: string);
```
