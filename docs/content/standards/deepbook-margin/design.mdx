...
- **Max utilization rate**: Upper bound on how much of the pool can be borrowed (e.g., 80%)
...
The `ProtocolFees` component manages the distribution of interest earned. When borrowers pay interest, the protocol spread (e.g., 10%) is taken as fees, with the remaining portion (e.g., 90%) distributed to suppliers based on their shares.
...
$$\text{Risk Ratio} = \frac{\text{Total Assets (in debt currency)}}{\text{Total Debt}}$$

Different risk ratio thresholds determine allowable actions:

- **Borrow threshold** (e.g., 1.5): Minimum ratio to take out new loans
- **Withdraw threshold** (e.g., 1.3): Minimum ratio to withdraw collateral
- **Target liquidation ratio** (e.g., 1.25): Target ratio after partial liquidation
- **Liquidation threshold** (e.g., 1.15): Below this, the position can be liquidated

### Liquidation Process

When a `MarginManager`'s risk ratio falls below the liquidation threshold, anyone can liquidate the position:

1. The liquidator provides repayment for the debt
2. The liquidator receives collateral plus a liquidation reward (e.g., 5%)
3. The margin pool may also receive a reward (e.g., 3%)
...
- Below the kink (e.g., 80% utilization): Linear growth at a moderate rate
