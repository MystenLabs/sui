...
Margin pools use a kinked interest rate model where the borrow rate increases gradually up to an optimal utilization point, then rises sharply to discourage excessive borrowing. The formula for the borrow interest rate (APR) is:
...
| Asset | Base Rate | Base Slope | Optimal Utilization | Excess Slope |
| ----- | --------- | ---------- | ------------------- | ------------ |
| USDC  | 10%       | 15%        | 80%                 | 500%         |
| SUI   | 10%       | 20%        | 80%                 | 500%         |
| DEEP  | 15%       | 20%        | 80%                 | 500%         |
...
For example, at 50% utilization in the USDC pool:
- `borrowRate = 10% + 50% × 15% = 10% + 7.5% = 17.5% APR`

At 90% utilization (above optimal):
- `borrowRate = 10% + 80% × 15% + (90% - 80%) × 500% = 10% + 12% + 50% = 72% APR`
