---
title: DeepBookV2 Design
sidebar_label: Design
description: Learn about DeepBookV2's design, such as its pool and order book structures. 
keywords: [ pool structure, object structure, shared object structure, shared object pools, order book structure, store orders, pool storing orders, how pools store orders, orderbook, placing orders, deepbookv2, how to place orders, deepbook design ]
---
...
For each base and quote asset trading pair, a globally shared pool is created for bookkeeping open orders on the order book and handle placement, cancellation, and settlement of orders. Under this architecture, you can parallelize transactions involving different trading pairs to maximize throughput.
...
struct Pool<phantom BaseAsset, phantom QuoteAsset> has key {
    ...
    // The difference between taker_fee_rate and maker_rebate_rate goes to the protocol.
    // 10^9 scaling
    taker_fee_rate: u64,
    // 10^9 scaling
