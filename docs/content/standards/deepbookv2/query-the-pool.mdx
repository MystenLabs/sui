...
description: DeepBook pools are queried for order status, pool status, user account status, and account balance information. 
keywords: [ order status, deepbookv3, pool status, query pool, query order, user account status, account balance info, query account info, query account balance, query account status ]
---
...
/// Returns the list of all valid prices and corresponding depths
