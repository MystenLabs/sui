...
description: Construct and execute token trades or swaps using DeepBookV3. 
keywords: [ trade tokens, swap tokens, create a pool, construct trade, construct swap, make trade, make swap, typescript sdk, scaling precision, limit order, place order, place market order, deposit quote, cancel order, cancel single order, cancel batch order, create batch order ]
---
 ...
 * @param overrides: overrides for blockchain params. Like gas budget, transaction block and the provider, default we will set to the max gas budget, and create a new transaction block
