...
```http
/historical_volume/:pool_names?start_time=<UNIX-TIMESTAMP>&end_time=<UNIX-TIMESTAMP>&volume_in_base=<BOOLEAN>
```
...
```http
/all_historical_volume?start_time=1731260703&end_time=1731692703&volume_in_base=true
```
...
By default, this endpoint retrieves the last 24-hour trading volume in the base asset. If you want to query the quote asset instead, set `volume_in_base` to `false`.
...
```
/historical_volume_by_balance_manager_id/:pool_names/:balance_manager_id?start_time=<UNIX-TIMESTAMP>&end_time=<UNIX-TIMESTAMP>&volume_in_base=<BOOLEAN>
```
...
```
/historical_volume_by_balance_manager_id_with_interval/:pool_names/:balance_manager_id?start_time=<UNIX-TIMESTAMP>&end_time=<UNIX-TIMESTAMP>&interval=<UNIX-TIMESTAMP>&volume_in_base=<BOOLEAN>
```
...
```json
{
	...
	"last_price": integer,
	"lowest_price_24h": integer,
	"highest_bid": integer,
	"base_volume": integer,
	"price_change_percent_24h":integer,
	"quote_volume": integer,
	"lowest_ask": integer,
	"highest_price_24h": integer,
