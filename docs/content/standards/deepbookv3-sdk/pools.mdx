...
```tsx
updatePoolAllowedVersions(poolKey: string);
```
...
```tsx
createPermissionlessPool(params: CreatePermissionlessPoolParams);
```
...
```tsx
async getBalanceManagerIds(owner: string) {}
```
...
```tsx
mintReferral(poolKey: string, multiplier: number);
```
...
```tsx
updateReferralMultiplier(poolKey: string, referral: string, multiplier: number);
```
...
```tsx
claimReferralRewards(poolKey: string, referral: string);
```
...
```tsx
async getReferralBalances(poolKey: string, referral: string) {}
```
