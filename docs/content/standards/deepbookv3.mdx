...
The DEEP token is used to pay for trading fees on the exchange. Users can pay trading fees using DEEP token or input token, but owning, using, and staking DEEP will continue to provide benefits to active DeepBook traders on the Sui network.
