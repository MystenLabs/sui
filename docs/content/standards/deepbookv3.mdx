...
As mentioned, the DEEP token facilitates the operations of DeepBook. Initially, the tokens are required to pay for fees on the service. Future iterations will allow other forms of payment, like SUI, but owning, using, and staking DEEP will continue to provide benefits to active DeepBook traders on the Sui network.  

Staked takers (participants that have DEEP tokens staked on Sui and place orders matched on the CLOB), for instance, can trade with fees as low as 0.25 basis points (bps) on stable pairs and 2.5 bps on volatile pairs. Staked makers (participants that have DEEP tokens staked on Sui and place limit orders on the CLOB) can earn rebates based on their staking share.
