...
The `new()` function creates a `BalanceManager` hot potato. It must be combined with `share`, or else the transaction will fail. The transaction can be combined with deposit calls, allowing the user to create, deposit, then share the balance manager in one transaction.
...
The owner of a `BalanceManager` can mint a `TradeCap` and send it to another address. That address will have the capability to place orders with this `BalanceManager`. It will not be able to deposit or withdraw funds however. The maximum number of `TradeCap` that can be assigned for a BalanceManager is 1000. If this limit is reached, one or more existing `TradeCap` must be revoked before minting new ones.
