...
Mint a new `DeepBookPoolReferral` object for a specific pool with a specified multiplier. The multiplier determines the additional trading fees allocated to the referrer. The multiplier must be a multiple of the base referral multiplier and cannot exceed the maximum allowed multiplier. Returns the ID of the created referral.
...
Update the multiplier for an existing pool referral. Only the referral owner can update the multiplier. The new multiplier must meet the same requirements as when minting a referral.
