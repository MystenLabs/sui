...
Once claimed, the holder of `DisplayCap<T>` can update the display fields (`set`, `unset`, `clear`). Anyone who holds a V1 `Display<T>` can call `delete_legacy(display, legacy)` to burn their legacy object, no need to be the cap holder.
