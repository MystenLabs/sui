...
Use a `Publisher` object that you own to set `sui::display` for a type. For more information about `Publisher` objects, see [Publisher Authority](https://move-book.com/programmability/publisher.html) topic in *The Move Book*.
