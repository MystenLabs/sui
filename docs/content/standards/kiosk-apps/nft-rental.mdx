...
The `Promise` struct is created for borrowing by value. The `Promise` operates as the hot potato (a struct that has no capabilities that you can only pack and unpack it in its module) that can only be resolved by returning the item back to the extension's `Bag`.
