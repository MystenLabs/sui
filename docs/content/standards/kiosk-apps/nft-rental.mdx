...
NFT renting is a mechanism that allows individuals without ownership or possession of a specific NFT to temporarily utilize or experience it. The implementation of this process leverages the [Kiosk Apps standard](../kiosk-apps.mdx) to establish an infrastructure conducive to facilitating rental transactions. This approach closely aligns with the [Etherium ERC-4907](https://eips.ethereum.org/EIPS/eip-4907) renting standard, making it a suitable choice for Solidity-based use cases intended for implementation on Sui.
...
The `Rented` struct represents a rented item. The only field the struct includes is the ID of the object. It is used as the DF key in the borrower's Bag <key, value> entry when an item is actively being borrowed by someone.
...
The `Listed` struct represents a listed item. The only field the struct includes is the ID of the object. It is used as the DF key in the renter's Bag <key, value> entry once an item is listed for renting. 
...
It also lacks the `drop` ability since it can only be consumed by the `return_val` function.
...
This struct requires the `store` ability since it stores a value `T` that will definitely also have `store`.
...
The `RentalPolicy` struct is a shared object that every creator mints. Defines the royalties the creator receives from each rent invocation.
...
The `ProtectedTP` object is a shared object that every creator, that would like to enable renting, mints, regardless of their intent to enforce royalties. The object provides authorized access to an empty `TransferPolicy`.
A protected empty transfer policy is required to facilitate the rental process so that the extension can transfer the asset without any additional rules to be resolved (like lock rule, loyalty rule, etc).
If the creator desires to enforce royalties on rentals, they can do so via the `RentalPolicy` above.
...
The `install` function enables installation of the `Rentables` extension in a Kiosk. 
The party facilitating the rental process is responsible to ensure that the extension has been installed in the user's Kiosk.

```move
public fun install(kiosk: &mut Kiosk, cap: &KioskOwnerCap, ctx: &mut TxContext)
```

The `remove` function enables the owner (and only the owner) of the kiosk to remove the extension. The extension storage must be empty for the transaction to succeed.
The extension storage empties once the user is no longer borrowing or renting any items.
...
It is important to note that the `reclaim` functionality is manually invoked and the rental service provider is responsible for ensuring that the renter is reminded to `reclaim`. As such this can cause the borrower to hold the asset for longer than the rental period. This can be mitigated if we modified the current contract by adding an assertion in the `borrow` & `borrow_val` functions to check if the rental period has expired.
...
- For a new type that a creator would like to allow to be rented
    - Involves invoking `setup_renting` & TransferPolicy creation with optional lock rule
