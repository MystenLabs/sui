...
A protected empty transfer policy is required to facilitate the rental process so that the extension can transfer the asset without any additional rules to be resolved (like lock rule, loyalty rule, and so on).
...
The extension storage empties after the user is no longer borrowing or renting any items.
