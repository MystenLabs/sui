...
The `Rented` struct represents a rented item. The only field the struct includes is the ID of the object. It is used as the DF key in the borrower's `Bag` entry when someone is actively borrowing an item.
...
The `Listed` struct represents a listed item. The only field the struct includes is the ID of the object. It is used as the DF key in the renter's `Bag` entry after an item is listed for renting. 
...
public fun setup_renting<T>(publisher: &Publisher, amount_bp: u64, ctx: &mut TxContext) {
    ...
    // Stores inside the ProtectedTP the policy_cap with no way to access it
    ...
    // Creates a RentalPolicy that will be used to attribute royalties, if desired, to the 
    // item's T publisher.
...
public fun list<T: key + store>(
    ...
    // This is also crucial to ensure the correct person will receive the payment.
    // Prevents unexpected results in cases where the kiosk could have been transferred between users without the owner being updated.
...
public fun rent<T: key + store>(
    ...
    // Updates the rentables start_date.
...
public fun return_val<T: key + store>(
    ...
    // Destructures the Promise and reconstructs the Rentable, placing it in the borrower's Rentables extension bag.
    let Promise {    
        item,        
        duration,
        start_date,
