...
let mut request = from_account.send_balance<MyCoin>(
    &auth,
    &to_account,
    amount,
    ctx,
...
let mut request = from_account.clawback_balance<MyCoin>(amount, ctx);
...
```move
let auth = account::new_auth(ctx);
let mut request = account.unlock_balance<MyCoin>(&auth, amount, ctx);
