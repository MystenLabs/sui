...
Transfer balance of permissioned assets from one Account to another. Requires the owner's `Auth` proof.
...
`resolve_balance` sends the balance directly. The caller does not get it back. It goes straight to the destination Account.
...
Forcibly withdraw tokens from an Account. No `Auth` is required because this is an admin action. The policy must have `clawback_allowed: true`.
...
Deposit tokens into an Account. No request is needed:

```move
account.deposit_balance(balance);
```

Or send to an Account address directly. This is useful before the Account object is available in the transaction:
...
```
PTB:
  1. account::new_auth()                        → auth
  2. account.send_balance(auth, to, amount)     → request
...
|---|---|---|
| [Loyalty](https://github.com/MystenLabs/pas/tree/main/packages/examples/loyalty) | `send_funds` (permissionless), `unlock_funds` (per-address registry) | Transfers are open, points stay in the system. Redemptions are gated by a `RedeemRegistry` and resolved in Move. Once unlocked, funds leave the managed system entirely. |
