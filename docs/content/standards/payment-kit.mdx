...
The destination address that will receive the payment funds. Must be a valid Sui address.
...
The full type identifier of the coin to be transferred (for example, `0x2::sui::SUI`).
...
The Object ID, or ascii represented `name` of the `PaymentRegistry`, to use for processing the payment. If provided, the payment will be processed through the registry with duplicate prevention. If omitted, the payment will be processed as an ephemeral payment.
