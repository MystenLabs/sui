...
Use the [Sagat web interface](https://sagat.mystenlabs.com/) for [signing transactions](#transaction-signer) and [analyzing signatures](#signature-analyzer). Use the [Sagat SDK](https://github.com/MystenLabs/sagat/tree/main/sdk) for creating and managing multisigs, accepting or rejecting multisig invitations, and creating, voting on, and executing proposals. 

## What is multisig?

[Multisig](/guides/developer/cryptography/multisig) is a type of authentication that requires multiple signatures from different parties before a transaction can be executed. Several addresses can be invited to a multisig group, and all invited parties must accept for the multisig to be created.
...
To mitigate the risks, you can self host Sagat. To do so, first download the Github repository:
...
To run the prewritten tests stored at `/api/test`, first navigate to that directory and install the necessary dependencies:
...
Then start the bun server again:

```
$ bun run dev
```

To test proposals, you need a Sui Localnet instance running. To run a Localnet instance, you must have the [Sui CLI] installed, then run:
...
Through the Sagat [web interface](https://sagat.mystenlabs.com/) you can use the signature analyzer and transaction signer tools.

### Connecting Slush 

First, connect your [Slush](https://slush.app/) wallet to the Sagat web interface:

![Sagat connect Slush](./images/sagat-1.png "Screenshot showing how to connect Slush wallet to Sagat.")

:::tip 

The message "No wallets found. Install a wallet (e.g. Slush Wallet) to continue." indicates that you must [install the Slush extension](https://chromewebstore.google.com/detail/slush-%E2%80%94-a-sui-wallet/opcgpfmipidbgpenhmajoajpbobppdil) before continuing. 

:::

Unlock your wallet and approve the initial transaction. The Sagat web interface prompts you to generate a second transaction used to authenticate your wallet. 

![Sagat authenticate Slush](./images/sagat-2.png "Screenshot showing how to authenticate transaction in Slush.")

Follow the steps in your Slush wallet to sign and approve the transaction, then verify account ownership.
...
The [signature analyzer tool](https://sagat.mystenlabs.com/tools/signature-analyzer) is used to analyze and decode 64-base encoded signatures. It supports both multisig and single signature schemes.

Insert the signature into the input box. The decoded signature and its details are returned:

![Sagat signature analyzer](./images/sagat-3.png "Screenshot showing signature analysis details using Sagat signature analyzer tool.")
...
Sagat can be used through a [TypeScript SDK](https://github.com/MystenLabs/sagat/tree/main/sdk) or the [Sagat API](https://github.com/MystenLabs/sagat/tree/main/api). The examples below demonstrate test scenarios for using the Sagat API. 
...
To create and execute multisig transactions, first a multisig must be created. Multisigs are groups of users who must vote and approve of transactions before they can be executed. If a proposed transaction does not receive majority approval by the members of the multisig, the transaction is cancelled. 
...
Only verified multisig members can create and vote on proposals. Each multisig member can vote once. A proposal must reach a certain voting threshold to be executed. Before a proposal has been executed, it can be [cancelled by the user](https://github.com/MystenLabs/sagat/blob/313624a9d3b4b971b81b80be851b49e055067250/sdk/src/client.ts#L197-L208) that created the proposal. 
