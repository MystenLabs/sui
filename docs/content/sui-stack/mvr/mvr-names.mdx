...
goal:
  description: >-
    Reader can create MVR applications, set metadata, and attach packages to
    MVR names on Mainnet and other networks
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I create a new MVR application?
  - How do I attach a package to an MVR name?
  - How do I set metadata on an MVR application?
answer: >-
  You create an MVR application by calling move_registry::register with a SuiNS
  name, then attach PackageInfo objects for Mainnet or set network pointers for
  other networks, and configure metadata like description and icon URL.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

MVR names let you register Move packages under human-readable names in the Move Registry (MVR). Each name maps to a SuiNS name and a package name. You manage names by creating applications, attaching packages, and setting metadata. On Mainnet, package attachment is permanent. On other networks, you attach a pointer you can update later.

:::caution

The TypeScript examples assume that you use the [MVR plugin](./tooling/typescript-sdk) for the Sui TypeScript SDK. The Sui CLI examples do not use the MVR plugin.
...
The `register` call returns an `AppCap` object. You can use the `AppCap` immediately to attach packages, or transfer it to a safe address and register packages later.
...
You can call `set_metadata` multiple times with different keys to configure all fields. Each call updates a single key-value pair.
...
## How do I attach a Mainnet package to an application?
...
## How do I attach a non-Mainnet package to an application?
...
The `set_network` call requires a chain ID string. Use `4c78adac` for Testnet. The `app_info::new` call accepts three optional fields: the `PackageInfo` object ID on the external network, the package address on the external network, and a third optional field you can set to `null`.
...
**AppCap:** The capability object returned when you register an application. You must present the `AppCap` to attach packages or set metadata. Store it in a safe address or a shared object your team controls.

**PackageInfo:** An object created alongside a published package on Mainnet. Attaching it to an application creates a permanent, verifiable link between the MVR name and that package.

**Network pointer:** For non-Mainnet networks, MVR stores a pointer (chain ID plus package address) instead of a strict binding. You can update this pointer at any time using `unset_network` followed by `set_network`.

**Registry object:** The shared Move Registry object at `0x0e5d473a055b6b7d014af557a13ad9075157fdc19b6d51562a18511afd397727`. All calls pass this object as the first argument. You can also resolve it as `registry-obj@mvr` from Mainnet SuiNS.