...
A TEE is not magic: it attests to what code runs, not that the code is correct or safe. Minimize the code and dependencies inside the enclave to shrink the trusted computing base, audit those dependencies, rate-limit and guard the endpoints that reach the enclave, and alert on PCR drift so unexpected changes to enclave measurements are caught immediately. See the [Nautilus](/develop/security/best-practices#nautilus) section of the Security Best Practices guide.
