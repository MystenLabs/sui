...
goal:
  description: >-
    Reader understands overview of the design aspects of Nautilus, including its
    trust model and knows when to use it
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Nautilus Design in Sui?
  - What happens inside the enclave?
  - How does nautilus design work on Sui?
answer: '![image](./images/nautilus-overview_nautilus-design_v1.png).'
---
...
It generates a new ephemeral keypair inside its isolated memory. The private key never leaves the enclave while the public key is returned through `/get_public_key`.
...
As an app developer, follow these steps to deploy a self-managed TEE. Alternatively, you can use the [Marlin Oyster marketplace](/sui-stack/nautilus/community-dev-tools) for Docker-based simplicity.

1. Create a Nautilus offchain server with a reproducible build using the [provided template](https://github.com/MystenLabs/nautilus).
