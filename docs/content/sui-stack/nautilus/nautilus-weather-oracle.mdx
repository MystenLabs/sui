...
goal:
  description: >-
    Reader can build a verified weather oracle on Sui using Nautilus, where a
    Trusted Execution Environment fetches weather data, signs it
    cryptographically, and the Move contract mints an NFT only after verifying
    the signature
  requires:
    - has_frontmatter:
        - title
