...
description: >-
  Build a verified weather oracle on Sui using Nautilus, where a Trusted
...
concepts:
  ...
  - enclave_attestation
goal:
  description: Reader can build a verified weather oracle on Sui using Nautilus
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I build a weather oracle with Nautilus on Sui?
...
- [x] Work through [Using Nautilus](/sui-stack/nautilus/using-nautilus) and confirm you can deploy an enclave to an AWS Nitro instance.
...
A complete implementation lives in [`sui-move-bootcamp/K4`](https://github.com/MystenLabs/sui-move-bootcamp/tree/solution/K4). Only 2 files change for any Nautilus application:
...
Everything else (enclave runtime, attestation, key management) comes from the [Nautilus repo](https://github.com/MystenLabs/nautilus). Your Move package depends on the enclave module:
