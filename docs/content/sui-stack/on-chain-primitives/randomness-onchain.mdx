...
goal:
  description: >-
    ...
    but can also expose flaws in your logic. Understanding the vulnerabilities
    and accounting for them can mitigate the threat exposure for your smart
    contracts and knows when to use it
  requires:
    - has_frontmatter:
        - title
        - description
    ...
    - headings:
        - pattern: Limited resources and `Random` dependent
        - pattern: Use \(non-public\) `entry` functions
        - pattern: Programmable transaction block \(PTB\) r
