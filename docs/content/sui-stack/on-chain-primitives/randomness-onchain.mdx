...
- Account for the PTB restriction: only `TransferObjects` and `MergeCoins` commands may follow a `MoveCall` that consumes `Random`, so complete the draw-and-resolve in a single Move call.

- For high-stakes flows, use a commit-reveal pattern (commit and pay in one transaction, reveal the outcome in a later transaction) so outcomes cannot be inspected and reverted atomically.
