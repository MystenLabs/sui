...
questions:
  - What is Onchain Randomness in Sui?
...
`Random` has a reserved address `0x8`. See <UnsafeLink href="/references/framework/sui_sui/random">`random.move`</UnsafeLink> for the Move APIs that access randomness on Sui.
...
The attack takes advantage of the atomic nature of PTBs and reverts the entire transaction if the guess was incorrect, avoiding the fee payment. Sending multiple transactions can repeat the attack, each one executed with different randomness and reverted if the guess is incorrect.
