...
description: Learn how to encrypt Walrus blobs and enforce onchain access control using Seal.
...
OnlyFins uses a purchase-gated pattern, but Seal's `seal_approve` mechanism is fully generic. Other common patterns include:

- **Allowlist:** Maintain a shared object containing a list of authorized addresses. The `seal_approve` function checks whether the caller's address is in the list.
- **Subscription:** A user purchases a subscription NFT valid for a time period. The `seal_approve` function checks that the subscription object is owned by the caller and has not expired.
- **Time-lock:** Encrypt data that can only be decrypted after a specific timestamp. The `seal_approve` function checks the Sui `Clock` object.

The Seal repository's [`examples/move`](https://github.com/MystenLabs/seal/tree/main/examples) directory contains complete implementations of the allowlist and subscription patterns.
