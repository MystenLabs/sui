...
This guide continues from [Data Storage Using Walrus](../walrus/data-storage.mdx) and builds on the same [OnlyFins](https://github.com/MystenLabs/onlyfins-example-app) app. The Walrus guide covered how OnlyFins stores and reads images. This guide covers how it protects them: encrypting images before upload and enforcing onchain access control so only users who have paid can decrypt them.

By the end of this guide you will understand how to design a Seal access control policy in Move, how to encrypt data before uploading it to Walrus, and how to implement the full client-side decryption flow in a browser app.
...
- **Onchain access policies:** Move functions you write that define who is allowed to decrypt. When a user requests decryption key shares, key servers evaluate your policy by running a `dry_run_transaction_block` on Sui. If the policy approves, the key shares are returned.
- **Key servers:** Off-chain services that hold IBE (identity-based encryption) master secret keys. Each server returns derived decryption key shares only when the associated onchain policy approves the request. You configure a threshold, so decryption requires at least `t` out of `n` servers to agree.
- **Client-side encryption:** Data is encrypted and decrypted locally in the user's environment. Key servers never see plaintext.

In OnlyFins, the access policy is: the user must own a `ViewerToken` object for the specific post they want to unlock. A `ViewerToken` is a Sui object the user receives after paying for a post onchain. Seal key servers check this ownership before issuing key shares.
...
- **`SessionKey`:** Represents a user-approved session that allows a browser app to fetch decryption keys without triggering a wallet popup on every request. A session has a TTL and is scoped to a specific package ID.
...
:::info
OnlyFins uses `SuiClient` from `@mysten/sui/client` to back `SealClient`. `SuiGrpcClient` from `@mysten/sui/grpc` is the recommended transport for new integrations. The `SealClient` constructor and all encryption and decryption APIs are identical regardless of which client you pass in. This also applies to `encryptImages.ts` in the backend, which constructs its own `SuiClient` the same way.
:::
...
- It must not be called from other PTB commands during Seal evaluation. Only `seal_approve*` functions are invoked directly.
...
A `ViewerToken` is minted onchain when a user pays for a post. The `usePayForContent` hook builds and executes this transaction:
...
The most basic policy: only the wallet address encoded in the identity can decrypt. The entire `seal_approve` function is a single assertion comparing the BCS-encoded caller address to the `id` bytes:
...
Data can only be decrypted after a specific timestamp has passed. The `id` encodes a `u64` unlock time in milliseconds. The `seal_approve` function peeks the timestamp from the identity bytes and checks it against the Sui `Clock` object:
...
A shared `Allowlist` object holds a list of authorized addresses managed by an admin. The `seal_approve` function checks that the caller is a member. Because the allowlist is a shared object, the admin can add or remove members at any time without re-encrypting the content:
...
:::info
This script uses `SuiJsonRpcClient` and `getJsonRpcFullnodeUrl` from `@mysten/sui/jsonRpc`. The JSON RPC transport is deprecated. For new integrations use `SuiGrpcClient` from `@mysten/sui/grpc` instead. The Seal `SealClient` setup and the encrypt/decrypt flow are identical regardless of which transport you use.
:::

## Decrypting

Decryption in OnlyFins is entirely client-side. The browser fetches the encrypted blob from the Walrus aggregator, requests key shares from Seal key servers, derives the decryption key, and decrypts the image locally. Key servers never see the ciphertext or the plaintext.
