...
goal:
  description: >-
    Reader understands how to join, create, and manage SuiNS Communities,
    including WHO and FEE requirements and subname faucets
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What are SuiNS Communities?
  - How do I join a SuiNS community?
  - How do I create a SuiNS community?
answer: >-
  SuiNS Communities is a social identity layer on Sui where you join
  ecosystem-specific communities by claiming subnames, with configurable WHO
  (asset ownership) and FEE (payment) requirements.
---
...
Community creators and administrators can define what onchain assets you must own before you can join their community. The community minter sets these requirements, but the community administrator can update them at any time.
...
The system verifies asset ownership onchain, and potential members only need 1 qualifying asset.
...
Community creators and administrators have the option to allow potential members to join their community through digital payment. This option allows users that do not meet the WHO requirements to still join. Each community administrator sets the fee amount, so SuiNS Communities does not control it. Most of the fees (90%) go to the address that owns the community, with the rest going to SuiNS Communities as a fee.
...
If you want to charge a fee to create a subname, enable the **Fee for Subname Creation** toggle. The address to receive the proceeds automatically uses the owner's address and cannot be changed. Set the fee amount in the final field.

### Finalize community creation

Review the information you set. When satisfied, select the **Create Community** button.

You can always update the community settings after creation. If you disable the subname faucet later, you do not receive a refund for the setup fee.
