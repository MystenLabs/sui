...
:::info
This example mirrors the Sui Messenger app, which uses the older `SuiClient` transport from `@mysten/sui/client` and client extension APIs from earlier `@mysten/seal` and `@mysten/messaging` releases. `SuiGrpcClient` from `@mysten/sui/grpc` is the recommended transport for all new integrations. See the [Sui SDK v2 migration guide](https://sdk.mystenlabs.com/sui/migrations/sui-2.0) for upgrade instructions.
:::
