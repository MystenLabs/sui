...
- Resolve a SuiNS name to an address onchain in Move and offchain through the SuiNS SDK.
...
- [x] Node.js 18+ with `@mysten/sui` and `@mysten/suins` installed: `pnpm add @mysten/sui @mysten/suins`
...
flowchart LR
    ...
    subgraph offchain ["Offchain resolution (SDK/RPC)"]
        RPC["SUINS SDK\ngetNameRecord"]
        GQL["GRAPHQL\nresolveSuinsAddress"]
    end
    ...
    USER -->|"getNameRecord(name)"| RPC
    RPC -->|"target address"| USER
...
Both resolution types are available onchain in Move and offchain through the SuiNS SDK and Sui APIs.
...
- **`@mysten/suins` SDK extension:** The SuiNS SDK adds higher-level methods like `getNameRecord` to any client that provides the core API through the `$extend` pattern. Use this when you need name record details beyond the target address (expiration, metadata, avatar).
- **Core client name service method:** Any client that provides the core API, such as `SuiGrpcClient`, exposes `core.defaultNameServiceName` for reverse lookup without installing an additional package.
...
The `@mysten/suins` package provides the `suins()` extension function. Attach it to any client that provides the core API through `$extend` to add SuiNS methods. The extension uses `ClientWithCoreApi` and `PackageInfo` internally, and automatically loads the correct constants for Mainnet and Testnet:
...
// nameRecord is null if the name is not registered
```
...
### Reverse lookup (address to default name)

Reverse lookup resolves an address to its default name through the core API:

```ts
const { data } = await client.core.defaultNameServiceName({
  address: '0x2',
});
// data.name is 'example.sui', or null if no default name is set
```
...
When you display a name for an address in your UI, use reverse lookup (`core.defaultNameServiceName`) rather than scanning all names that point to that address. The reverse lookup returns only the name that the address owner has explicitly set as their default through a signed transaction. This guarantees that the address owner consented to being identified by that name ([SuiNS developer reference](https://docs.suins.io/developer)).
...
Set up resolution so the subname can be looked up. The subname is resolvable immediately after creation through onchain `lookup` and offchain `getNameRecord`. If the target address owner wants the subname as their default name, they must sign a separate set-default transaction. Both node and leaf subnames support reverse lookup registration.
...
:::info
This example mirrors the Sui Messenger app, which uses the older `SuiClient` transport from `@mysten/sui/client`, the `SealClient.asClientExtension` API that current `@mysten/seal` releases have removed, and `SuiStackMessagingClient.experimental_asClientExtension`, which `@mysten/messaging` has deprecated in favor of `messaging()`. `SuiGrpcClient` from `@mysten/sui/grpc` is the recommended transport for all new integrations. See the [Sui SDK v2 migration guide](https://sdk.mystenlabs.com/sui/migrations/sui-2.0) for upgrade instructions.
:::
...
| Wrong network | Mainnet name queried on Testnet client or reverse | Match the client's network and URL to the network where the name is registered |
...
**Wrong network.** `getNameRecord` returns `null` on Mainnet for a name registered on Testnet, or the reverse. Confirm that the client's network and URL match the network where the name is registered.
