...
goal:
  description: >-
    Reader can register and manage SuiNS names, update avatars, link addresses,
    and create sui.id websites
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  ...
  - How do I renew a SuiNS name?
  - How do I link a wallet address to my SuiNS name?
answer: >-
  Register a SuiNS name at suins.io by searching for an available name and
  purchasing it for 1 year, then manage it through the dashboard to renew,
  update avatars, link addresses, or create a sui.id website.
---

Build your onchain identity in Web3 with Sui Name Service (SuiNS). Using SuiNS, you can register a name (any available name) and use it for onchain transactions.
...
To enjoy the benefits of SuiNS, you must first register names. When you register available names, you purchase the rights to those names for 1 year. After registration, you can [renew your names](#renew-names) for multiple years.
...
1. Type a name of interest in the search field between 3 and 63 characters, inclusive. Press <kbd>Enter</kbd> or tap or click the magnifying glass icon to initiate the search. Search results show whether the name is available and the price in SUI for 1 year of registration.
...
1. Approve the transaction using your connected wallet. SuiNS updates the address the name points to, but the address that registered the name retains control of the SuiNS name and the SuiNS NFT remains in the wallet of the registered address, as well.
