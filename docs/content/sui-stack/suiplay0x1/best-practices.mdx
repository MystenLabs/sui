...
keywords: [ SuiPlay0X1, beamable, shinami, data management, handling transactions, onchain storage, offchain storage ]
---
...
Follow wallet and session security practices to protect players. Do not cache wallet state between sessions, since assets and balances can change externally and stale state can mislead the player. Avoid blind signing: present clear, human-readable transaction details so players know what they are approving. Enforce session expiry rather than holding long-lived sessions, and rate-limit and authorize sponsored transactions so an attacker cannot drain your gas budget through abuse. When you operate custodial wallets on a player's behalf, clearly disclose the associated custodial risk to users. See the [Security Best Practices](/develop/security/best-practices) guide.
