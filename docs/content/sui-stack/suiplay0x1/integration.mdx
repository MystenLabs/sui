...
The following example detects the Playtron environment with the `OS` class and retrieves the Playtron wallet address with the `Sui` class.

```ts
import { OS, Sui } from '@playtron/sdk';

async function main() {
  // Check whether the game is running on GameOS
  ...
  try {
    // Read the address from the user's Playtron wallet
...
`OS.isPlaytron()` returns `true` when the game runs on GameOS. `Sui.getWalletAddress()` then returns the address of the user's Playtron wallet without any additional setup. The `Sui` class also exposes `signMessage()`, `signTransaction()`, and `signAndExecuteTransaction()` for transaction flows. Refer to the [Playtron GameOS Node.js SDK reference](https://github.com/playtron-os/playtron-sdk/blob/main/node/README.md) for the full API and to the [SDK repository](https://github.com/playtron-os/playtron-sdk) for C++ and C# examples.
...
Create a single dApp Kit instance for your game:
...
// The hooks require this global type registration
...
Wrap your game's UI in `DAppKitProvider` and drop in the `ConnectButton` component:
