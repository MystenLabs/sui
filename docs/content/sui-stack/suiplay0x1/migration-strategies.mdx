...
Use Sui dApp Kit to add wallet connection support to the off-device (PC or web) version of your game. Create a dApp Kit instance, wrap your game UI in `DAppKitProvider`, and render a `ConnectButton`:
...
const dAppKit = createDAppKit({
  ...
  createClient(network) {
    return new SuiGrpcClient({ network, baseUrl: 'https://fullnode.mainnet.sui.io:443' });
  },
});
...
`ConnectButton` lists every wallet that dApp Kit detects, plus any wallet you register through the `walletInitializers` option on `createDAppKit`. The Playtron zkLogin wallet is not a browser extension, so it needs that explicit registration before players can select it. For the full setup, see [SDK Integrations](/sui-stack/suiplay0x1/integration).

:::info

`@mysten/dapp-kit` without a suffix is the deprecated 1.x package. Older examples that wrap your app in `SuiClientProvider` and `WalletProvider` target that version.
...
The following example resolves the linked addresses from a player's Sui-to-Sui SuiLink NFTs, then checks each one for a membership asset. Both queries page through their results, because `listOwnedObjects` returns one page at a time.
...
// Sui-to-Sui SuiLink package on Mainnet
const SUILINK_PACKAGE_ID = '0x73f5ab2461c5993408fff21354fa9831d4f4a66cc81382419ec29e3c80c384b5';
const MEMBERSHIP_TYPE = '0xPACKAGE_ID::membership::Membership';

// Walk every page, because a player can own more objects than one page holds
...
async function hasMembership(playtronAddress: string) {
  // The Playtron wallet address is the one the player connected, so check it directly
  const owned = await listAllOwnedObjects(playtronAddress, MEMBERSHIP_TYPE);
  if (owned.length > 0) return true;

  // Then check each address the player has proven they own through SuiLink
  const links = await listAllOwnedObjects(
    playtronAddress,
    `${SUILINK_PACKAGE_ID}::suilink::SuiLink`,
  );

  for (const link of links) {
    const linkedAddress = link.json.network_address as string;
    const linkedAssets = await listAllOwnedObjects(linkedAddress, MEMBERSHIP_TYPE);
