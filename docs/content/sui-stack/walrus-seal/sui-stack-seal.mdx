...
This guide continues from [Data Storage Using Walrus](/sui-stack/walrus-seal/sui-stack-walrus) and builds on the same [OnlyFins](https://github.com/MystenLabs/onlyfins-example-app) app. The Walrus guide covered how OnlyFins stores and reads images. This guide covers how it protects them: encrypting images before upload and enforcing onchain access control so only authorized users can decrypt them.

By the end of this guide you will understand how to design a [Seal](https://seal-docs.wal.app/) access control policy in [Move](/develop/write-move/sui-move-concepts), how to encrypt data before uploading it to [Walrus](https://docs.wal.app/), and how to implement the full client-side decryption flow in a browser app.
...
Seal is a decentralized secrets management (DSM) service that uses access control policies defined and validated on Sui. It lets you encrypt sensitive data before storing it on Walrus, onchain, or in any other storage, and then issue decryption keys only to users who satisfy your onchain access conditions.

Seal has 3 core components:

- **Onchain access policies:** Move functions you write that define who is allowed to decrypt. When a user requests decryption key shares, [key servers](https://seal-docs.wal.app/ServerOverview) evaluate your policy by running a `dry_run_transaction_block` on Sui. If the policy approves, the key shares are returned.
- **Key servers:** Off-chain services that hold [IBE (identity-based encryption)](https://seal-docs.wal.app/Design) master secret keys. Each server returns derived decryption key shares only when the associated onchain policy approves the request. You configure a threshold, so decryption requires at least `t` out of `n` servers to agree.
- **Client-side encryption:** Data is encrypted and decrypted locally in the user's environment. Key servers never see plaintext.

In OnlyFins, the access policy is: the user must own a `ViewerToken` object for the specific post they want to unlock. A `ViewerToken` is a [Sui object](/develop/sui-architecture/object-model) the user receives after requesting access onchain. Seal key servers check this ownership before issuing key shares.
...
3. When a user wants to view a post, they build a transaction that proves they own a `ViewerToken`.
...
Two tools support Seal integration: the CLI for local testing and the TypeScript SDK for application development.
...
The `@mysten/seal` package exports two primary classes:
...
:::info
OnlyFins uses `SuiClient` from `@mysten/sui/client` to back `SealClient`. `SuiGrpcClient` from `@mysten/sui/grpc` is the recommended transport for new integrations. The `SealClient` constructor and all encryption and decryption APIs are identical regardless of which client you pass in. This also applies to `encryptImages.ts` in the backend, which constructs its own `SuiClient` the same way. See the [Sui SDK v2 migration guide](https://sdk.mystenlabs.com/sui/migrations/sui-2.0) for upgrade instructions.
:::
...
The identity tied to each encrypted object is the value passed as `id` at encryption time. The same `id` must be used in the `seal_approve` PTB at decryption time.
...
This function receives the Seal ID, the `Post` object, and the user's `ViewerToken`. It checks that the `ViewerToken` was issued for the correct post and that the encryption ID matches. If either check fails, it aborts and the key server denies the request.

### Acquiring a ViewerToken

A `ViewerToken` is minted onchain when a user requests access to a post. The `usePayForContent` hook builds and executes this transaction:
...
After the transaction succeeds, the `ViewerToken` object appears in the user's wallet. Seal key servers will now approve decryption requests for that post from that wallet address.
...
The most basic policy: only the wallet address encoded in the identity can decrypt. The entire `seal_approve` function is a single assertion comparing the [Binary Canonical Serialization (BCS)](https://sdk.mystenlabs.com/sui/bcs)-encoded caller address to the `id` bytes:
...
Data can only be decrypted after a specific timestamp has passed. The `id` encodes a `u64` unlock time in milliseconds. The `seal_approve` function peeks the timestamp from the identity bytes and checks it against the Sui [`Clock`](/sui-stack/on-chain-primitives/access-time) object:
...
Encryption happens before upload, entirely on the client side. The data never leaves the client in plaintext.
...
The `encrypt` call returns the `encryptedObject` (the ciphertext) and a backup `key`. The backup key can be used for disaster recovery with the `seal-cli symmetric-decrypt` command. Store or discard it according to your recovery requirements.
...
:::info
This script uses `SuiJsonRpcClient` and `getJsonRpcFullnodeUrl` from `@mysten/sui/jsonRpc`. The JSON RPC transport is deprecated. For new integrations use `SuiGrpcClient` from `@mysten/sui/grpc` instead. The Seal `SealClient` setup and the encrypt/decrypt flow are identical regardless of which transport you use. See the [Sui SDK v2 migration guide](https://sdk.mystenlabs.com/sui/migrations/sui-2.0) for upgrade instructions.
:::

## Decrypting

Decryption in OnlyFins is entirely client-side. The browser fetches the encrypted blob from the Walrus [aggregator](https://docs.wal.app/docs/operator-guide/aggregators/operating-aggregator), requests key shares from Seal key servers, derives the decryption key, and decrypts the image locally. Key servers never see the ciphertext or the plaintext.

### Session keys

A `SessionKey` is a short-lived credential that authorizes a browser app to fetch decryption keys from Seal key servers without triggering a wallet popup on every request. The user approves a session once per package, signing a personal message in their wallet. The session is valid for a configurable TTL.

OnlyFins manages session keys through `useSessionJWT.ts` and a `SessionKeyProvider` context. The session key is created once when the user connects their wallet and reused across all decryption requests during its lifetime:

<ImportContent
  source="frontend/src/hooks/useSessionJWT.ts"
  mode="code"
  org="MystenLabs"
  repo="onlyfins-example-app"
  language="ts"
...
The `usePostDecryption` hook implements the complete client-side decryption flow for all unlocked posts the user has access to:
