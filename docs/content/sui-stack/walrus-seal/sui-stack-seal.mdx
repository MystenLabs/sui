...
The `@mysten/seal` package exports 2 primary classes:
...
After the transaction succeeds, the `ViewerToken` object appears in the user's wallet. Seal key servers now approve decryption requests for that post from that wallet address.
