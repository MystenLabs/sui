...
Encryption happens before upload, entirely on the client side. The client never sends the data in plaintext.
...
OnlyFins manages session keys through `useSessionJWT.ts` and a `SessionKeyProvider` context. OnlyFins creates the session key once when the user connects their wallet and reuses it across all decryption requests during its lifetime:
