...
A [publisher](https://docs.wal.app/docs/operator-guide/aggregator) is an HTTP server that accepts blob data over a REST API and handles the entire Walrus write flow on your behalf. Publishers are the most direct way to integrate uploads in server-side applications without using the TS SDK.
...
To reduce the number of requests from ~2,200 to a single relay request, configure the upload relay when extending the client:
...
:::info
These examples use `SuiClient` from `@mysten/sui/client`. `SuiGrpcClient` from `@mysten/sui/grpc` is the recommended transport for new integrations. The Walrus SDK setup and `writeBlob` API are identical regardless of which client you use. The same note applies to the `downloadWithErrorHandling` example later in this guide, which is from the same module. See the [Sui SDK v2 migration guide](https://sdk.mystenlabs.com/sui/migrations/sui-2.0) for upgrade instructions.
:::
...
:::info
`createPosts.ts` uses `SuiClient` from `@mysten/sui/client`. `SuiGrpcClient` from `@mysten/sui/grpc` is the recommended transport for new integrations. The `Transaction` building and `signAndExecuteTransaction` patterns are identical regardless of which client you use. See the [Sui SDK v2 migration guide](https://sdk.mystenlabs.com/sui/migrations/sui-2.0) for upgrade instructions.
:::
...
:::info
This example uses `SuiClient` from `@mysten/sui/client`. `SuiGrpcClient` from `@mysten/sui/grpc` is the recommended transport for new integrations. The `writeFiles` API and `WalrusFile.from()` patterns are identical regardless of which client you use. See the [Sui SDK v2 migration guide](https://sdk.mystenlabs.com/sui/migrations/sui-2.0) for upgrade instructions.
:::
...
:::info
This example uses `SuiClient` from `@mysten/sui/client`. `SuiGrpcClient` from `@mysten/sui/grpc` is the recommended transport for new integrations. The `getBlob` and `blob.files()` patterns are identical regardless of which client you use. See the [Sui SDK v2 migration guide](https://sdk.mystenlabs.com/sui/migrations/sui-2.0) for upgrade instructions.
:::
...
OnlyFins queries a fixed hardcoded list of post objects rather than dynamically discovering blobs by owner. Adding a view that displays all blobs owned by the connected wallet can be done by deploying a version of [Sui dApp Kit](https://sdk.mystenlabs.com/dapp-kit)'s `react-client-dapp` starter with an object type filter added to the owned-objects query. The result would look like this:
