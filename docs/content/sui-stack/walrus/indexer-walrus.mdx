...
goal:
  description: >-
    Reader understands walrus is a content-addressable storage protocol, where
    data is retrieved using a unique identifier derived from the content itself,
    rather than from a file path or location. Integrating a custom Sui Indexer
    with Walrus can provide novel user experiences and knows when to use it
  requires:
    - has_frontmatter:
        - title
        - description
    ...
    - headings:
        - pattern: Blog platform using Walrus
        - pattern: Data modeling
        - pattern: Custom indexer implementation
