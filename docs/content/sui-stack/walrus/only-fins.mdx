...
- **Walrus storage:** The frontend stores encrypted image blobs offchain on Walrus and references them by blob ID. Walrus blobs are publicly addressable, so encryption is required to restrict access.
...
Walrus is a content-addressed storage protocol where data is retrieved by blob ID. All Walrus blobs are publicly addressable, so anyone who knows the blob ID can fetch the raw bytes. To restrict access, the frontend encrypts content before uploading to Walrus.
...
- [x] A Sui wallet ([Slush Wallet](https://slush.app/) or another compatible wallet) or willingness to use [Enoki](https://portal.enoki.mystenlabs.com/) zkLogin
...
The first script creates encrypted image files in `backend/encrypted/` and outputs JSON mapping files:

```bash
$ pnpm encrypt-images
```

Upload the encrypted and unencrypted images to Walrus, then populate the blob IDs in the generated JSON files.

The second script publishes the posts onchain using the configured author key pairs:
