...
Your site's static assets (HTML, CSS, JavaScript, images, fonts, and so on) are stored as blobs on Walrus. The site-builder groups them into a single quilt, a Walrus storage format that reduces upload cost and improves speed. Each file in the quilt is assigned a `QuiltPatchID` the site object uses to locate it. See [Walrus Sites Components](https://docs.wal.app/docs/sites/introduction/components) for a full breakdown of the architecture.
...
On Testnet, 1 epoch lasts 1 day. Use a larger number to keep the site available longer.
...
`ws-resources.json` is a configuration file `site-builder` reads during deployment to control how your site is built and served. The file itself is not uploaded to Walrus and is not visible to visitors.
...
- **`object_id`:** Written automatically after the first `deploy`. Subsequent deploys update this site.
...
When you update a site, all resources are deleted and re-uploaded together as a single quilt, even resources that have not changed. To update the Walrus Snake example, edit `examples/snake/index.html` and run `deploy` again:
