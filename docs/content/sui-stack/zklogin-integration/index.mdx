...
goal:
  description: >-
    Reader can implement the zkLogin flow — ephemeral keys, JWT, user salt, and
...
builder_paths:
  ...
  - path_id: agentic-payments
    ...
    stage: Authorization
questions:
  - What is zkLogin integration on Sui?
  - How do I install and use the zkLogin TypeScript SDK?
  - How do I manage user salts for zkLogin?
answer: >-
  This guide covers the zkLogin flow for creating ephemeral keys, obtaining JWTs
  and proofs, managing user salts, assembling signatures, and submitting
  transactions.
---
...
1. For the next step, you need two Docker images from the [mysten/zklogin repository](https://hub.docker.com/r/mysten/zklogin) (tagged as `prover` and `prover-fe`). A docker compose file is available that automates this process. Run `docker compose` with the downloaded zkey from the same directory as the YAML file.
