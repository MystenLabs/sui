...
This page covers zkLogin on its own. Consumer apps usually combine it with sponsored transactions so a new user never has to fund an address before their first action. The same user with the same salt always gets the same address, enabling persistent wallet identity across sessions. For that combination end to end, see the [Consumer App Cookbook](/sui-stack/zklogin-integration/consumer-app-zklogin).
...
The `maxEpoch` controls how long this ephemeral key can authorize transactions. The address itself does not expire; only the session does. When the epoch passes `maxEpoch`, generate a new key pair and zero-knowledge proof. See [Does my address expire?](/sui-stack/zklogin-integration/zklogin#address-and-session-lifetime) for details.
...
The Mysten Labs hosted proving service generates zero-knowledge proofs on your behalf. Access depends on the environment:

- **Devnet and Testnet:** The public prover endpoint is available without registration. Use it for development and testing.
- **Mainnet:** Mainnet access requires allowlisting through the [Enoki platform](https://docs.enoki.mystenlabs.com/). Register your app's client ID in the Enoki Portal and contact Mysten Labs to enable Mainnet proving.

#### Hosted prover allowlisting

Before your app can use the Mysten Labs prover on Mainnet, complete the following:

1. Register your app in the [Enoki Portal](https://portal.enoki.mystenlabs.com) and obtain an API key.
2. Add your OAuth client ID to the Enoki Portal configuration. The prover only accepts JWTs from allowlisted client IDs.
3. Contact Mysten Labs through the [Enoki docs](https://docs.enoki.mystenlabs.com/) to request Mainnet prover access for your registered app.

The hosted prover auto-scales for traffic surges. If you expect a spike in prover requests, reach out on [Discord](https://discord.gg/sui).
...
Self-host the proving service when you need full control over proof generation, want to avoid the hosted prover allowlisting process, or have compliance requirements that prevent sending JWTs to a third-party service. The self-hosted prover uses the same request and response format as the Mysten Labs service, so you can switch between hosted and self-hosted without changing your client code.
