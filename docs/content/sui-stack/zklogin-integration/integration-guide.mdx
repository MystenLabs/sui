...
The zkLogin address is deterministic. The same OAuth identity (`sub` + `iss` + `aud`) and salt always produce the same address, so the user can receive tokens from an on-ramp provider across multiple sessions without re-deriving.
