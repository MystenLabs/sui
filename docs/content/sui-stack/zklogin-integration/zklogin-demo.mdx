...
Replace `YOUR_GOOGLE_CLIENT_ID` with the OAuth client ID from your Google Cloud project. The `VITE_SALT` can be any large random number; it determines the derived Sui address. Use the same salt consistently to get the same address. You can also set `VITE_NETWORK=devnet` and update `VITE_SUI_GRPC_URL` accordingly if you need access to the most cutting-edge features, but keep in mind that Devnet resets periodically.
