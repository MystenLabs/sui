...
description: >-
  Add zkLogin to a React application so users authenticate with OAuth providers
  and get a derived Sui address, with no seed phrase to manage.
keywords:
  - example
  - zklogin
  - authentication
...
last_verified: 2026-04-30T00:00:00.000Z
...
goal:
  description: >-
    Reader can add zkLogin authentication to an existing React application
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I add zkLogin to my React app?
  - What does the zkLogin authentication example demonstrate?
  - How do I integrate OAuth login with Sui transactions?
answer: >-
  Add zkLogin to a React application so users authenticate with OAuth providers
  and get a derived Sui address, with no seed phrase to manage.
---
...
- Derive a deterministic Sui address from an OAuth identity and a salt so users always get the same address.
...
- **Address derivation:** `jwtToAddress(jwt, salt, false)` deterministically maps an OAuth identity to a Sui address. The third argument (`legacyAddress`) is required; pass `false` for new integrations. The same user with the same salt always gets the same address, enabling deterministic address derivation; however, the ephemeral key pair changes per session, so the actual signing capability is session-specific.
...
- [x] An existing [React](https://react.dev/) app you can add routes and hooks to.

- [x] [Install the Sui CLI](/getting-started/onboarding/sui-install) and [fund an address with Testnet SUI](/getting-started/onboarding/get-coins) for testing transfers.

- [x] [Node.js](https://nodejs.org/) 18 or later.

- [x] A [Google OAuth client](https://console.cloud.google.com/) whose authorized JavaScript origin and redirect URI both match your dev server origin.
...
A complete React implementation lives in [`sui-move-bootcamp/K2`](https://github.com/MystenLabs/sui-move-bootcamp/tree/solution/K2). Clone it to see the hooks below wired together, or read on for the pieces you need to port into your own app.

The app reads its configuration from environment variables, and the same values are what your integration needs:

| Variable | Purpose |
| --- | --- |
| `VITE_SUI_GRPC_URL` | Fullnode gRPC endpoint used to fetch the current epoch and execute transactions. |
| `VITE_SALT` | The per-user salt. A fixed value is fine for a demo, but production needs a real salt service. |
| `VITE_EPHEMERAL_KEY_DURATION_EPOCHS` | How many epochs past the current one the ephemeral key stays valid. |
| `VITE_OAUTH_CLIENT_ID` | Your OAuth client ID. Determines which identities map to which addresses. |

The salt and the OAuth client ID together determine every derived address. Changing either one moves all of your users to different addresses, so treat both as permanent configuration.
...
**Fix:** Use the [Sui faucet](https://faucet.sui.io/) to request Testnet SUI for the derived address. Copy the address from the Step 3 panel and paste it into the faucet.
