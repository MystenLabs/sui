...
- **Address derivation:** `jwtToAddress(jwt, salt, false)` deterministically maps an OAuth identity to a Sui address. The third argument (`legacyAddress`) is required; pass `false` for new integrations. The same user with the same salt always gets the same address, enabling persistent wallet identity across sessions.
...
`jwtToAddress` maps the JWT's `sub` claim and the salt to a Sui address. `genAddressSeed` produces the address seed needed later for the zkLogin signature. The same JWT `sub` claim + salt always produces the same address, so the user's wallet persists across sessions.
