...
- **Address derivation:** `jwtToAddress(jwt, salt, false)` deterministically maps an OAuth identity to a Sui address. The third argument (`legacyAddress`) is required; pass `false` for new integrations. Consumer apps usually combine it with sponsored transactions so a new user never has to fund an address before their first action.
