...
2. `kc_name_F = hashBytesToField(kc_name, maxKCNameLen)` — key claim name (e.g., `sub`) mapped to a BN254 field element.
...
The salt is required for both ZK proof generation and address derivation. A leaked salt does not enable fund theft, but it allows an attacker to link the user's `sub` to their Sui address. The impact depends on whether the provider uses pairwise identifiers (e.g., Facebook — unique per app, low risk) or public reusable identifiers (e.g., Google, Twitch — globally unique `sub`, higher privacy risk).
