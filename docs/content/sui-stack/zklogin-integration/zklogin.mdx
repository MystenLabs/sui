...
The master secret is the single most sensitive value in a zkLogin deployment. If you lose it, no user can recover their salt. If an attacker compromises it, they can derive any user's salt.
...
A zkLogin session binds to an ephemeral key pair that remains valid until a specific epoch (`maxEpoch`). After that epoch, validators reject any transaction signed with the ephemeral key.
...
The user's Sui address does not change. The system derives it from `(iss, aud, sub, salt)`, none of which change during renewal.
...
4. Any in-flight transactions signed with the old key complete normally as long as the network processes them before the epoch ends.
