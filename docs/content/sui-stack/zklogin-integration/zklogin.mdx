...
description: >-
  zkLogin is a Sui primitive that enables you to send transactions from a Sui
...
goal:
  description: >-
    Reader understands zkLogin is a Sui primitive that enables you to send
    transactions from a Sui address using an OAuth
  requires:
    - has_frontmatter:
        - title
        - description
...
answer: >-
  zkLogin is a Sui primitive that allows you to send transactions from a Sui
  address using an OAuth credential without publicly linking the two.
---
...
- No. zkLogin does not provide a way to recover a user's email from a Sui address, a zero-knowledge proof, or onchain data. A zkLogin address is derived from `sub`, `iss`, `aud`, and `user_salt`, and it exposes no identifying information. Your app can read the user's email only during the OAuth login flow, and only if it requests the `email` scope and the provider includes an `email` claim in the [JWT](#json-web-token-jwt). If you need the email, read it from the JWT claims at login time and store it yourself. You cannot derive it later from the address.
...
1. Use the [Sui Keytool CLI](/references/cli/keytool). This is recommended for debug usage.
