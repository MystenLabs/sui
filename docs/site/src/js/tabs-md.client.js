...
function getTabs(container) {
  ...
  const panels = Array.from(container.querySelectorAll('.margin-top--md > [role="tabpanel"]'));
...
function activate(container, value) {
  ...
  panels.forEach((p) => (p.dataset.value === value ? p.removeAttribute('hidden') : p.setAttribute('hidden', '')));
}
...
function initContainer(container) {
  ...
  const qs = querySync ? new URL(window.location.href).searchParams.get(group || 'tab') : null;
  const initial =
    qs ?? (tabs.find((t) => t.classList.contains('tabs__item--active'))?.dataset.value || tabs[0].dataset.value);
...
function setAllInGroup(groupId, value) {
  document.querySelectorAll(`.tabs-container[data-group="${groupId}"]`).forEach((c) => activate(c, value));
}
...
function bindDelegatedHandlers() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowRight' || e.key === 'ArrowLeft') {
      e.preventDefault();
      const tgt = e.key === 'ArrowRight' ? tabs[(idx + 1) % tabs.length] : tabs[(idx - 1 + tabs.length) % tabs.length];
      tgt?.focus();
...
export function onRouteDidUpdate() {
  ...
  document.querySelectorAll('.tabs-container[data-tabs]').forEach(initContainer);
}
