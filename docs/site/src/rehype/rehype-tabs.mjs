...
function getFromMdx(node, ...names) {
  ...
  for (const n of names) {
    ...
    if (!a) continue;
    if (a.value == null) return ''; // boolean attribute present
...
function getClassMdx(node) {
  const v = getFromMdx(node, 'className', 'class'); // allow 'class'
  return v ? classes(v) : [];
}
...
export default function rehypeTabsMd() {
  return (tree) => {
    visit(tree, (node, index, parent) => {
      ...
      // Collect <tabitem> children in either form
      ...
      const items = tabNodes.map((child, i) => {
        if (child.type === 'element') {
          ...
          return {
            ...
            label: getFromHast(p, 'label') || getFromHast(p, 'value') || `item-${i + 1}`,
            itemClass: classes(p.className),
            content: cloneChildren(child.children),
          };
        } else {
          const value =
            getFromMdx(child, 'value') || `item-${i + 1}`;
      ...
      const liEls = items.map((it) => {
        ...
        return {
          ...
          properties: {
            ...
            'data-value': it.value,
            className: classes('tabs__item', selected && 'tabs__item--active', it.itemClass),
          },
      ...
      const replacement = {
        ...
        tagName: 'div',
        properties: {
          className: classes('tabs-container', containerExtraClass),
          'data-md-tabs': '',      // important to keep your script scoped!
          'data-group': groupId,
        ...
        children: [
          {
            ...
            properties: {
              ...
              className: classes('tabs', block && 'tabs--block'),
            },
            children: liEls,
          },
          {
