...
async function consolidateReleaseNotes() {
  ...
  // A failed fetch must not skip writing the page. The page is generated on
  // every build and is not in version control, so producing nothing here means
  // the build fails later on a missing doc rather than here on the real cause.
  let githubReleases = [];
  ...
  try {
    ...
    console.error('   Writing the page from local release notes only.');
  }

  {
    ...
    // Emit the full frontmatter, including the description, keywords, and GEO
    // fields. They were previously added to the committed page by the audit
    // tooling and overwritten on every build.
...
{/* This page is generated by docs/site/src/shared/js/convert-release-notes.cjs. */}
{/* Edit the long-form notes in release-notes/ instead of editing this file. */}
...
`;
    ...
    if (fetchFailed) {
      consolidatedContent += `:::info

Published release history could not be retrieved when this page was built. See [GitHub Releases](${GITHUB_RELEASES_URL}) for the full list.

:::

`;

      // Local notes normally hang off the release they belong to. With no
      // releases fetched they would be dropped, so render them on their own.
      const versions = Array.from(localNotesByVersion.keys()).sort((a, b) =>
        b.localeCompare(a, undefined, { numeric: true }),
      );

      versions.forEach((versionDir) => {
        const notes = localNotesByVersion.get(versionDir);
        if (!notes || notes.length === 0) return;

        consolidatedContent += `## ${versionDir.replace('_', '.')}\n\n`;
        notes.forEach((note) => {
