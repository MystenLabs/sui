...
module raffles::tests {
    ...
    fun test_example1() {
        ...
        let end_time = 100;
        example1::create(end_time, 10, scenario.ctx());
        scenario.next_tx(user1);
        let mut game = scenario.take_shared<example1::Game>();
    ...
    fun test_example2() {
        ...
        mint(user1, 1000, scenario);
        let end_time = 100;
        example2::create(end_time, 10, scenario.ctx());
        scenario.next_tx(user1);
        let mut game = scenario.take_shared<example2::Game>();
        assert!(game.cost_in_sui() == 10, 1);
        assert!(game.participants() == 0, 1);
        assert!(game.end_time() == end_time, 1);
        assert!(game.balance() == 0, 1);
