...
import {
  InvalidateTrophyQuery,
  Trophy,
  useTrophyQuery,
} from "hooks/useTrophyQuery";
...
export default function Game({ id }: Props): ReactElement {
  ...
  if (trophy.status === "pending") {
    ...
    return (
      <Error title="Error loading game">
        Could not check win for{" "}
        <IDLink id={id} size="2" display="inline-flex" />:
        <br />
...
function OwnedGame({
  ...
  const adminKey = game.admin
    ? new MultiSigPublicKey(new Uint8Array(game.admin))
    : null;
  ...
  if (adminKey == null) {
    return (
      <Error title="Error loading game">
        Could not load game at{" "}
        <IDLink id={game.id} size="2" display="inline-flex" />.
        <br />
        Game has no admin.
      </Error>
    );
    ...
    <AlertDialog.Root>
      ...
      <AlertDialog.Content>
        ...
        <AlertDialog.Description>
          Are you sure you want to delete this game? This will delete the object
          from the blockchain and cannot be undone.
        </AlertDialog.Description>
