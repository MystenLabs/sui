...
#[test]
fn invalid_variant_jump_table_in_script() {
    use Bytecode::*;

    let skeleton = empty_script();
    let jt_index = VariantJumpTableIndex(skeleton.code.jump_tables.len() as u16);
    let mut m = skeleton.clone();
    m.code.code = vec![VariantSwitch(jt_index)];
    assert_eq!(
        BoundsChecker::verify_script(&m).unwrap_err().major_status(),
        StatusCode::INDEX_OUT_OF_BOUNDS
    );
}
