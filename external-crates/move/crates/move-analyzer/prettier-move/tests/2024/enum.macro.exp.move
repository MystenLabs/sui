module foo::bar {
    ...
    // Types
    public struct X {}
    public struct Old<T: key + copy + store> has key { x: u64 }
    ...
    macro fun for($start: u64, $stop: u64, $body: |u64|) {
        let mut i = $start;
        let stop = $stop;
        while (i < stop) { $body(i);i = i + 1 }
    }
    macro fun for_each<$T>($v: &vector<$T>, $body: |&$T|) {
        let v = $v;
        let mut i = 0;
        let n = v.length();
        while (i < n) { $body(v.borrow(i));i = i + 1 }
    }
    ...
    fun t() { None().ignore!() }
    entry fun main() { let v = new!(10, |i| i);assert!(sum!(&v) == 45, 0); }
