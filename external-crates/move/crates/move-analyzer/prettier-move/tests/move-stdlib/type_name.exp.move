...
module std::type_name {
    ...
    public fun is_primitive(self: &TypeName): bool {
        ...
        bytes ==
        &b"bool" || bytes == &b"u8" || bytes == &b"u16" || bytes == &b"u32" ||
        bytes == &b"u64" ||
        bytes == &b"u128" ||
        bytes == &b"u256" ||
        bytes == &b"address" || (
            bytes.length() >= 6 && bytes[0] == ASCII_V && bytes[1] == ASCII_E &&
            bytes[2] == ASCII_C &&
            bytes[3] == ASCII_T &&
            bytes[4] == ASCII_O &&
            bytes[5] == ASCII_R,
    ...
    public fun borrow_string(self: &TypeName): &String { &self.name }
    ...
    public fun get_address(self: &TypeName): String {
        ...
        while (i < len) { addr_bytes.push_back(str_bytes[i]);i = i + 1; };
    ...
    public fun into_string(self: TypeName): String { self.name }
}
