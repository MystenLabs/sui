...
module std::vector {
    ...
    public macro fun find_index<$T>(
        ...
        'find_index: {
            v
                .length()
                .do!(|i| if ($f(&v[i])) return 'find_index option::some(i));
            option::none()
        }
    }
    ...
    public macro fun zip_do_reverse<$T1, $T2>(
        $v1: vector<$T1>,
        $v2: vector<$T2>,
        $f: |$T1, $T2|,
    ) {
    ...
    /// Aborts if the vectors are not of the same length.
    /// The order of elements in the vectors is preserved.
    public macro fun zip_do_ref<$T1, $T2>(
        $v1: &vector<$T1>,
        $v2: &vector<$T2>,
        $f: |&$T1, &$T2|,
    ) {
