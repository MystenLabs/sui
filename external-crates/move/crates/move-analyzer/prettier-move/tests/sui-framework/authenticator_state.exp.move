...
module sui::authenticator_state {
    ...
    public struct AuthenticatorState has key { id: UID, version: u64 }
    ...
    public struct JwkId has store, drop, copy { iss: String, kid: String }
    #[allow(unused_field)]
    ...
    fun string_bytes_lt(a: &String, b: &String): bool {
        let a_bytes = a.bytes();
        let b_bytes = b.bytes();
        if (a_bytes.length() < b_bytes.length()) { true } else if (
            ...
            while (i < a_bytes.length()) {
                ...
                if (a_byte < b_byte) { return true } else if (a_byte > b_byte) {
                    return false
                };
                i = i + 1;
            };
    ...
    fun update_authenticator_state(
        ...
        while (i < active_jwks_len && j < new_active_jwks_len) {
            ...
            if (active_jwk_equal(old_jwk, new_jwk)) {
                ...
                res.push_back(*old_jwk);
                i = i + 1;
            } else { res.push_back(*new_jwk);j = j + 1; }
        };
    ...
    fun deduplicate(jwks: vector<ActiveJwk>): vector<ActiveJwk> {
        ...
        while (i < jwks.length()) {
            let jwk = &jwks[i];
            if (prev.is_none()) { prev.fill(jwk.jwk_id); } else if (
                jwk_id_equal(prev.borrow(), &jwk.jwk_id)
            ) {
                // skip duplicate jwks in input
                i = i + 1;
                continue
            } else { *prev.borrow_mut() = jwk.jwk_id; };
            res.push_back(*jwk);
            i = i + 1;
        };
        res
    ...
    fun expire_jwks(
        ...
        while (i < len) {
            ...
            if (prev_issuer.is_none()) { prev_issuer.fill(*cur_iss); } else if (
                cur_iss != prev_issuer.borrow()
            ) { *prev_issuer.borrow_mut() = *cur_iss;j = j + 1; };
    ...
    #[test_only]
    public fun create_for_testing(ctx: &TxContext) { create(ctx); }
    #[test_only]
    ...
    public fun get_active_jwks_for_testing(
        self: &AuthenticatorState,
        ctx: &TxContext,
    ): vector<ActiveJwk> { self.get_active_jwks(ctx) }
}
