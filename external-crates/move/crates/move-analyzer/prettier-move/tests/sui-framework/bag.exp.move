...
module sui::bag {
    ...
    public fun add<K: copy + drop + store, V: store>(
        ...
        k: K,
        v: V,
    ) { field::add(&mut bag.id, k, v);bag.size = bag.size + 1; }
    #[syntax(index)]
    ...
    public fun borrow_mut<K: copy + drop + store, V: store>(
        bag: &mut Bag,
        k: K,
    ): &mut V { field::borrow_mut(&mut bag.id, k) }
    ...
    public fun remove<K: copy + drop + store, V: store>(
        bag: &mut Bag,
        k: K,
    ): V { let v = field::remove(&mut bag.id, k);bag.size = bag.size - 1;v }
    ...
    public fun contains_with_type<K: copy + drop + store, V: store>(
        bag: &Bag,
        k: K,
    ): bool { field::exists_with_type<K, V>(&bag.id, k) }
    /// Returns the size of the bag, the number of key-value pairs
    public fun length(bag: &Bag): u64 { bag.size }
    /// Returns true iff the bag is empty (if `length` returns `0`)
    public fun is_empty(bag: &Bag): bool { bag.size == 0 }
