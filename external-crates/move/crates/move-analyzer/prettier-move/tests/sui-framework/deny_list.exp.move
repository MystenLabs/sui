...
module sui::deny_list {
    ...
    public(package) fun v2_contains_current_epoch(
        ...
        let per_type_config = deny_list.borrow_per_type_config(
            per_type_index,
            per_type_key,
        );
        let setting_name = AddressKey(addr);
        config::read_setting(
            object::id(per_type_config),
            setting_name,
            ctx,
        ).destroy_or!(false)
    }
    ...
    public(package) fun v2_contains_next_epoch(
        ...
        let setting_name = AddressKey(addr);
        per_type_config
            .read_setting_for_next_epoch(setting_name)
            .destroy_or!(false)
    }
    ...
    public(package) fun migrate_v1_to_v2(
        ...
        let elements = if (
            !bag_entry.denied_addresses.contains(per_type_key)
        ) vector[]
        ...
        elements.do!(
            |addr| {
                let setting_name = AddressKey(addr);
                let next_epoch_entry = per_type_config.entry!<
                    _,
                    AddressKey,
                    bool,
                >(
