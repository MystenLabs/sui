...
module sui::deny_list {
    ...
    fun per_type_list_remove(
        ...
        let denied_count = &mut list.denied_count[addr];
        *denied_count = *denied_count - 1;
        if (*denied_count == 0) { list.denied_count.remove(addr); }
    }
    ...
    #[test_only]
    public fun reserved_addresses(): vector<address> { RESERVED }
    #[test_only]
    public fun create_for_test(ctx: &mut TxContext) { create(ctx); }
    #[test_only]
