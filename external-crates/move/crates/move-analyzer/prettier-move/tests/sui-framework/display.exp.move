...
module sui::display {
    ...
    public entry fun add_multiple<T: key>(
        ...
        assert!(len == values.length(), EVecLengthMismatch);
        let mut i = 0;
        while (i < len) { self.add_internal(fields[i], values[i]);i = i + 1; };
    }
    ...
    public entry fun edit<T: key>(
        ...
        name: String,
        value: String,
    ) { let (_, _) = self.fields.remove(&name);self.add_internal(name, value) }
...
module sui::display_tests {
    ...
    public struct Capy has key { id: UID, name: String }
