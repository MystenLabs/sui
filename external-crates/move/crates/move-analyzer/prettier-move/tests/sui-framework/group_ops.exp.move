...
module sui::group_ops {
    ...
    public struct Element<phantom T> has store, copy, drop { bytes: vector<u8> }
    ...
    public(package) fun from_bytes<G>(
        type_: u8,
        bytes: &vector<u8>,
        is_trusted: bool,
    ): Element<G> {
    ...
    public(package) fun multi_scalar_multiplication<S, G>(
        ...
        Element<G> {
            bytes: internal_multi_scalar_mul(
                type_,
                &scalars_bytes,
                &elements_bytes,
            ),
    ...
    public(package) fun set_as_prefix(
        x: u64,
        big_endian: bool,
