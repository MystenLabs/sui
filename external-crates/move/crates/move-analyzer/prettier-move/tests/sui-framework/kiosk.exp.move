...
module sui::kiosk {
    ...
    public fun set_owner(
        ...
        cap: &KioskOwnerCap,
        ctx: &TxContext,
    ) { assert!(self.has_access(cap), ENotOwner);self.owner = ctx.sender(); }
    ...
    public fun place<T: key + store>(
        ...
        cap: &KioskOwnerCap,
        item: T,
    ) { assert!(self.has_access(cap), ENotOwner);self.place_internal(item) }
    ...
    public fun lock<T: key + store>(
        ...
        item: T,
    ) { assert!(self.has_access(cap), ENotOwner);self.lock_internal(item) }
    ...
    public fun take<T: key + store>(
        ...
        df::remove_if_exists<Listing, u64>(
            &mut self.id,
    ...
    public fun return_purchase_cap<T: key + store>(
        ...
        df::remove<Listing, u64>(
            &mut self.id,
            Listing { id: item_id, is_exclusive: true },
        );
        id.delete()
    }
    ...
    public fun uid_mut_as_owner(
        self: &mut Kiosk,
        cap: &KioskOwnerCap,
    ): &mut UID { assert!(self.has_access(cap), ENotOwner);&mut self.id }
    /// [DEPRECATED]
