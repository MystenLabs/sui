...
module sui::linked_table {
    ...
    public fun borrow<K: copy + drop + store, V: store>(
        table: &LinkedTable<K, V>,
        k: K,
    ): &V { &field::borrow<K, Node<K, V>>(&table.id, k).value }
    #[syntax(index)]
    ...
    public fun borrow_mut<K: copy + drop + store, V: store>(
        table: &mut LinkedTable<K, V>,
        k: K,
    ): &mut V { &mut field::borrow_mut<K, Node<K, V>>(&mut table.id, k).value }
    ...
    public fun prev<K: copy + drop + store, V: store>(
        table: &LinkedTable<K, V>,
        k: K,
    ): &Option<K> { &field::borrow<K, Node<K, V>>(&table.id, k).prev }
    ...
    public fun next<K: copy + drop + store, V: store>(
        table: &LinkedTable<K, V>,
        k: K,
    ): &Option<K> { &field::borrow<K, Node<K, V>>(&table.id, k).next }
    ...
    public fun contains<K: copy + drop + store, V: store>(
        table: &LinkedTable<K, V>,
        k: K,
    ): bool { field::exists_with_type<K, Node<K, V>>(&table.id, k) }
    ...
    public fun drop<K: copy + drop + store, V: drop + store>(
        table: LinkedTable<K, V>,
    ) { let LinkedTable { id, size: _, head: _, tail: _ } = table;id.delete() }
}
