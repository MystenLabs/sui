...
module sui::object_bag {
    ...
    public fun add<K: copy + drop + store, V: key + store>(
        ...
        k: K,
        v: V,
    ) { ofield::add(&mut bag.id, k, v);bag.size = bag.size + 1; }
    #[syntax(index)]
    ...
    public fun borrow_mut<K: copy + drop + store, V: key + store>(
        bag: &mut ObjectBag,
        k: K,
    ): &mut V { ofield::borrow_mut(&mut bag.id, k) }
    ...
    public fun remove<K: copy + drop + store, V: key + store>(
        bag: &mut ObjectBag,
        k: K,
    ): V { let v = ofield::remove(&mut bag.id, k);bag.size = bag.size - 1;v }
    ...
    public fun contains_with_type<K: copy + drop + store, V: key + store>(
        bag: &ObjectBag,
        k: K,
    ): bool { ofield::exists_with_type<K, V>(&bag.id, k) }
    /// Returns the size of the bag, the number of key-value pairs
    public fun length(bag: &ObjectBag): u64 { bag.size }
    /// Returns true iff the bag is empty (if `length` returns `0`)
    public fun is_empty(bag: &ObjectBag): bool { bag.size == 0 }
    ...
    public fun value_id<K: copy + drop + store>(
        bag: &ObjectBag,
        k: K,
    ): Option<ID> { ofield::id(&bag.id, k) }
}
