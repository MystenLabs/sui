...
module sui::object_table {
    ...
    public fun new<K: copy + drop + store, V: key + store>(
        ctx: &mut TxContext,
    ): ObjectTable<K, V> { ObjectTable { id: object::new(ctx), size: 0 } }
    ...
    public fun add<K: copy + drop + store, V: key + store>(
        ...
        k: K,
        v: V,
    ) { ofield::add(&mut table.id, k, v);table.size = table.size + 1; }
    #[syntax(index)]
    ...
    public fun borrow_mut<K: copy + drop + store, V: key + store>(
        table: &mut ObjectTable<K, V>,
        k: K,
    ): &mut V { ofield::borrow_mut(&mut table.id, k) }
    ...
    public fun contains<K: copy + drop + store, V: key + store>(
        table: &ObjectTable<K, V>,
        k: K,
    ): bool { ofield::exists_<K>(&table.id, k) }
    ...
    public fun destroy_empty<K: copy + drop + store, V: key + store>(
        table: ObjectTable<K, V>,
    ) {
    ...
    public fun value_id<K: copy + drop + store, V: key + store>(
        table: &ObjectTable<K, V>,
        k: K,
    ): Option<ID> { ofield::id(&table.id, k) }
}
