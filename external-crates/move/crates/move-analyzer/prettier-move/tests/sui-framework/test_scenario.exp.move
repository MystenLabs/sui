...
module sui::test_scenario {
    ...
    /// ```
    public struct Scenario { txn_number: u64, ctx: TxContext }
    ...
    public fun sender(scenario: &Scenario): address { scenario.ctx.sender() }
    ...
    public fun transferred_to_account(
        effects: &TransactionEffects,
    ): VecMap<ID, address> { effects.transferred_to_account }
    /// Accessor for `transferred_to_object` field of `TransactionEffects`
    public fun transferred_to_object(
        effects: &TransactionEffects,
    ): VecMap<ID, ID> { effects.transferred_to_object }
    ...
    /// been taken
    public native fun most_recent_id_for_address<T: key>(
        account: address,
    ): Option<ID>;
    ...
    public fun take_from_address<T: key>(
        scenario: &Scenario,
        account: address,
    ): T {
    ...
    public fun most_recent_id_for_sender<T: key>(
        scenario: &Scenario,
    ): Option<ID> { most_recent_id_for_address<T>(sender(scenario)) }
    ...
    /// Aborts if there is no object of type `T` in the inventory with ID `id`
    public native fun take_immutable_by_id<T: key>(
        scenario: &Scenario,
        id: ID,
    ): T;
    ...
    public fun receivable_object_ids_for_owner_id<T: key>(
        object: ID,
    ): vector<ID> { ids_for_address<T>(object::id_to_address(&object)) }
    ...
    /// test scenario.
    public fun return_receiving_ticket<T: key>(
        ticket: sui::transfer::Receiving<T>,
    ) {
