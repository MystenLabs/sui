...
module sui::transfer {
    ...
    public struct Receiving<phantom T: key> has drop { id: ID, version: u64 }
    ...
    public fun freeze_object<T: key>(obj: T) { freeze_object_impl(obj) }
    /// Freeze `obj`. After freezing `obj` becomes immutable and can no longer be transferred or
    /// mutated.
    ...
    public(package) fun make_receiver<T: key>(
        id: ID,
        version: u64,
    ): Receiving<T> { Receiving { id, version } }
    #[test_only]
    public(package) fun receiving_id<T: key>(r: &Receiving<T>): ID { r.id }
}
