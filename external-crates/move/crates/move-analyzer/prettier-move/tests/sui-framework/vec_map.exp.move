...
module sui::vec_map {
    ...
    /// An entry in the map
    public struct Entry<K: copy, V> has copy, drop, store { key: K, value: V }
    ...
    public fun try_get<K: copy, V: copy>(
        self: &VecMap<K, V>,
        key: &K,
    ): Option<V> {
        if (self.contains(key)) { option::some(*get(self, key)) } else {
            option::none()
        }
    }
    ...
    public fun into_keys_values<K: copy, V>(
        self: VecMap<K, V>,
    ): (vector<K>, vector<V>) {
    ...
    public fun get_idx_opt<K: copy, V>(
        self: &VecMap<K, V>,
        key: &K,
    ): Option<u64> {
        let mut i = 0;
        let n = size(self);
        while (i < n) {
            if (&self.contents[i].key == key) { return option::some(i) };
            i = i + 1;
        };
        option::none()
    }
    ...
    public fun get_entry_by_idx<K: copy, V>(
        self: &VecMap<K, V>,
        idx: u64,
    ): (&K, &V) {
    ...
    /// Note that map entries are stored in insertion order, *not* sorted by key.
    /// Aborts if `idx` is greater than or equal to `size(self)`
    public fun get_entry_by_idx_mut<K: copy, V>(
        self: &mut VecMap<K, V>,
        idx: u64,
    ): (&K, &mut V) {
        assert!(idx < size(self), EIndexOutOfBounds);
    ...
    /// Aborts if `idx` is greater than or equal to `size(self)`
    public fun remove_entry_by_idx<K: copy, V>(
        self: &mut VecMap<K, V>,
        idx: u64,
    ): (K, V) {
        assert!(idx < size(self), EIndexOutOfBounds);
