...
use crate::symbols::{
    add_fun_use_def, add_struct_use_def, def_info_to_type_def_loc, expansion_mod_ident_to_map_key,
    type_def_loc, LocalDef,
};
...
use move_compiler::{
    ...
    parser::ast as P,
    shared::Identifier,
    typing::visitor::TypingVisitorContext,
    typing::{ast as T, visitor::LValueKind},
};
...
use crate::utils::{get_start_position_opt, ignored_function, to_lsp_position};
...
impl<'a> TypingVisitorContext for TypingSymbolicator<'a> {
                    ...
                    match kind {
                        LValueKind::Bind => {
                            self.add_local_def(
                                ...
                                &var.value.name,
                                *ty.clone(),
                                !for_unpack, // with_let (only for simple definition, e.g., `let t = 1;``)
    ...
    fn visit_exp_custom(&mut self, _exp: &mut T::Exp) -> bool {
        ...
        self.visit_type(&mut _exp.ty);
        match &mut _exp.exp.value {
            TE::Move { from_user: _, var }
            | TE::Copy { from_user: _, var }
            | TE::Use(var)
            | TE::BorrowLocal(_, var) => {
                self.add_local_use_def(&var.value.name, &var.loc);
                true
            }
