...
exports.TraceEffectKind = exports.TraceEventKind = exports.TraceInstructionKind = void 0;
exports.readTrace = readTrace;
exports.traceEventsToString = traceEventsToString;
const fs = __importStar(require("fs"));
const utils_1 = require("./utils");
var JSONTraceRefType;
(function (JSONTraceRefType) {
    JSONTraceRefType["Mut"] = "Mut";
    JSONTraceRefType["Imm"] = "Imm";
})(JSONTraceRefType || (JSONTraceRefType = {}));
// Runtime data types.
/**
 ...
 */
var TraceInstructionKind;
(function (TraceInstructionKind) {
    /**
     * Call instruction.
     */
    TraceInstructionKind[TraceInstructionKind["CALL"] = 0] = "CALL";
    /**
     * Generic call instruction.
     */
    TraceInstructionKind[TraceInstructionKind["CALL_GENERIC"] = 1] = "CALL_GENERIC";
    // for now we don't care about other kinds of instructions
    TraceInstructionKind[TraceInstructionKind["UNKNOWN"] = 2] = "UNKNOWN";
})(TraceInstructionKind || (exports.TraceInstructionKind = TraceInstructionKind = {}));
/**
...
(function (TraceEventKind) {
     ...
     */
    TraceEventKind[TraceEventKind["ReplaceInlinedFrame"] = 0] = "ReplaceInlinedFrame";
    TraceEventKind[TraceEventKind["OpenFrame"] = 1] = "OpenFrame";
    TraceEventKind[TraceEventKind["CloseFrame"] = 2] = "CloseFrame";
    TraceEventKind[TraceEventKind["Instruction"] = 3] = "Instruction";
    TraceEventKind[TraceEventKind["Effect"] = 4] = "Effect";
})(TraceEventKind || (exports.TraceEventKind = TraceEventKind = {}));
/**
 * Kind of an effect of an instruction.
 */
var TraceEffectKind;
(function (TraceEffectKind) {
    TraceEffectKind["Write"] = "Write";
    TraceEffectKind["ExecutionError"] = "ExecutionError";
    // TODO: other effect types
})(TraceEffectKind || (exports.TraceEffectKind = TraceEffectKind = {}));
/**
 ...
 */
function readTrace(traceFilePath, sourceMapsModMap, sourceMapsHashMap, filesMap) {
    ...
    // the loop
    const localLifetimeEnds = new Map();
    const localLifetimeEndsMax = new Map();
    const tracedLines = new Map();
    ...
    for (const event of traceJSON.events) {
        if (event.OpenFrame) {
            ...
            const differentFileVirtualFramePop = processInstructionIfMacro(sourceMapsHashMap, events, frameInfoStack, event.Instruction.pc, instLoc);
            if (differentFileVirtualFramePop) {
                ...
                processInstructionIfMacro(sourceMapsHashMap, events, frameInfoStack, event.Instruction.pc, instLoc);
            }
            ...
            const lines = tracedLines.get(filePath) || new Set();
            ...
            for (let i = 0; i < lifetimeEnds.length; i++) {
                if (lifetimeEnds[i] === undefined || lifetimeEnds[i] === utils_1.FRAME_LIFETIME) {
 ...
 */
function processInstructionIfMacro(sourceMapsHashMap, events, frameInfoStack, instPC, instLoc) {
    let frameInfo = frameInfoStack[frameInfoStack.length - 1];
    const fid = frameInfo.ID;
 ...
 */
function JSONTraceTypeToString(baseType, refType) {
 ...
 */
function JSONTraceAddressToHexString(address) {
    try {
 ...
 */
function processJSONLocalLocation(traceLocation, localLifetimeEnds) {
    if ('Local' in traceLocation) {
        ...
        if (localLifetimeEnds) {
            ...
            lifetimeEnds[localIndex] = utils_1.FRAME_LIFETIME;
 ...
 */
function traceRuntimeValueFromJSON(value) {
    if (typeof value === 'boolean'
        ...
        const fields = Object.entries(value.fields).map(([key, value]) => [key, traceRuntimeValueFromJSON(value)]);
        const compoundValue = {
            fields,
            type: value.type,
 ...
 */
function traceEventsToString(trace) {
 ...
 */
function instructionKindToString(kind) {
    switch (kind) {
...
//# sourceMappingURL=trace_utils.js.map