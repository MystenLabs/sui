...
use crate::{
    file_format::{
        ...
        VariantJumpTableIndex, Visibility, VariantHandle, VariantInstantiationHandle, VariantHandleIndex, VariantInstantiationHandleIndex, JumpTableInner,
    },
...
impl VariantHandleState {
    ...
    fn add_variant_handle(&mut self, vh: VariantHandle) -> Option<VariantHandleIndex> {
        debug_assert!(self.variant_handles.len() < TableSize::max_value() as usize);
...
impl VariantInstantiationHandleState {
    ...
    fn add_variant_instantiation_handle(&mut self, vh: VariantInstantiationHandle) -> Option<VariantInstantiationHandleIndex> {
        debug_assert!(self.variant_instantiation_handles.len() < TableSize::max_value() as usize);
        if let Some(idx) = self.variant_map.get(&vh) {
            return Some(*idx);
        }
        ...
        let bytecode = match self {
            ...
            BytecodeGen::PackVariant(idx, tag) => {
                    ...
                    .is_empty()
                {
                    ...
                    let handle = state.variant_instantiation_handles.add_variant_instantiation_handle(
