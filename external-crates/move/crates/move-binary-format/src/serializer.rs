...
fn serialize_variant_instantiation_handle_index(binary: &mut BinaryData, idx: &VariantInstantiationHandleIndex) -> Result<()> {
...
fn serialize_jump_table(binary: &mut BinaryData, jump_table: &VariantJumpTable) -> Result<()> {
    ...
    match &jump_table.jump_table {
        JumpTableInner::Full(offsets) =>  {
...
impl ModuleSerializer {
    ...
    fn serialize_tables(&mut self, binary: &mut BinaryData, module: &CompiledModule) -> Result<()> {
        ...
        self.serialize_variant_instantiation_handles(
            binary,
    ...
    fn serialize_variant_handles(
        ...
        if !variant_handles.is_empty() {
            ...
            self.variant_handles.1 =
                checked_calculate_table_size(binary, self.variant_handles.0)?;
        }
        Ok(())
    }
