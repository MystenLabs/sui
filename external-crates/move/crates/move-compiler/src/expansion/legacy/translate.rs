...
use crate::{
    ...
    expansion::{
        ...
        legacy::aliases::{
            AliasMap, AliasMapBuilder, AliasSet, OldAliasMap, ParserExplicitUseFun, UseFunsBuilder,
        },
        translate::{ModuleMemberKind, NameCase},
    },
    parser::ast::{
...
pub struct Context<'env, 'map> {
    module_members: UniqueMap<ModuleIdent, ModuleMembers>,
    pub named_address_mapping: Option<&'map NamedAddressMap>,
    address_conflicts: BTreeSet<Symbol>,
    address: Option<Address>,
    aliases: AliasMap,
    pub is_source_definition: bool,
...
fn name_access_chain(
    ...
    let tn_ = match (access, ptn_) {
        ...
        (_, PN::Two(sp!(eloc, LN::AddressName(n1)), n2)) => {
