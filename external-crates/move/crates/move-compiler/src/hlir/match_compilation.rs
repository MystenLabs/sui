            ...
            TP::At(x, inner) => {
                ...
                match (x_in_rhs_binders, inner_has_rhs_binders) {
                    ...
                    (true, true) => {
                        ...
                        seq.push_back(make_copy_binding(x, bind_entry.ty.clone(), bind_entry));
                        queue.push_front((entry, *inner));
                    }
                    // no unpack needed, just move the value to the x
                    (true, false) => seq.push_back(make_move_binding(x, entry.ty.clone(), entry)),
...
fn match_pattern_has_binders(pat: &T::MatchPattern, rhs_binders: &BTreeSet<Var>) -> bool {
    match &pat.pat.value {
        TP::Binder(x) => rhs_binders.contains(x),
        TP::At(x, inner) => {
