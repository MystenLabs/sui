...
impl Context<'_> {
    ...
    fn check_redundant_ref_deref(&mut self, exp: &Exp) {
        ...
        // to handle paths.
        if exp.exp.loc == deref_exp.exp.loc { return; }
        match &deref_exp.exp.value {
            TE::TempBorrow(_, inner) if is_simple_deref_ref_exp(inner) => {
            ...
            TE::Borrow(_, _, _) | TE::BorrowLocal(_, _) => {
                self.env.add_diag(diag!(
                    StyleCodes::RedundantRefDeref.diag_info(),
                    (
                        exp.exp.loc,
...
fn is_simple_deref_ref_exp(exp: &Box<Exp>) -> bool {
    match &exp.exp.value {
        TE::Value(_) => true,
