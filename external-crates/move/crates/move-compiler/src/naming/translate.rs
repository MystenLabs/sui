...
fn function(
    ...
    if macro_.is_some() {
        check_macro_unused_let_mut(context, &f);
    }
    context.local_count = BTreeMap::new();
...
fn check_macro_unused_let_mut(context: &mut Context, f: &N::Function) {
...
/// Collect `let mut` bindings from a sequence, skipping parameters ($-prefixed) and underscore.
fn macro_mut_collect_seq(mut_vars: &mut BTreeMap<N::Var_, (Loc, Loc)>, seq: &N::Sequence) {
    for sp!(_, item_) in &seq.1 {
        match item_ {
            N::SequenceItem_::Bind(sp!(_, lvalues), e) => {
                macro_mut_collect_lvalues(mut_vars, lvalues);
                macro_mut_collect_exp(mut_vars, e);
            }
            N::SequenceItem_::Declare(sp!(_, lvalues), _) => {
                macro_mut_collect_lvalues(mut_vars, lvalues);
            }
            N::SequenceItem_::Seq(e) => macro_mut_collect_exp(mut_vars, e),
        }
    }
}
        ...
        match lv_ {
            ...
            N::LValue_::Unpack(_, _, _, fields) => {
                for (_, _, (_, inner)) in fields {
                    macro_mut_collect_lvalues(mut_vars, std::slice::from_ref(inner));
                }
            }
            _ => (),
        }
...
fn macro_mut_collect_exp(mut_vars: &mut BTreeMap<N::Var_, (Loc, Loc)>, sp!(_, e_): &N::Exp) {
    match e_ {
        N::Exp_::Block(N::Block { seq, .. }) => macro_mut_collect_seq(mut_vars, seq),
        N::Exp_::Lambda(N::Lambda {
            ...
            macro_mut_collect_exp(mut_vars, body);
        }
        N::Exp_::IfElse(econd, et, ef) => {
            macro_mut_collect_exp(mut_vars, econd);
            macro_mut_collect_exp(mut_vars, et);
            if let Some(ef) = ef {
                macro_mut_collect_exp(mut_vars, ef);
            }
        }
        N::Exp_::Match(esubject, arms) => {
            macro_mut_collect_exp(mut_vars, esubject);
            for arm in &arms.value {
                if let Some(guard) = &arm.value.guard {
                    macro_mut_collect_exp(mut_vars, guard);
                }
                macro_mut_collect_exp(mut_vars, &arm.value.rhs);
            }
        }
        N::Exp_::While(_, econd, ebody) => {
            macro_mut_collect_exp(mut_vars, econd);
            macro_mut_collect_exp(mut_vars, ebody);
        }
        _ => (),
    }
}

/// Walk expressions to find mutations, removing variables from mut_vars when found.
fn macro_mut_check_seq(mut_vars: &mut BTreeMap<N::Var_, (Loc, Loc)>, seq: &N::Sequence) {
    for sp!(_, item_) in &seq.1 {
        match item_ {
            N::SequenceItem_::Seq(e) | N::SequenceItem_::Bind(_, e) => {
                macro_mut_check_exp(mut_vars, e)
            }
...
fn macro_mut_check_exp(mut_vars: &mut BTreeMap<N::Var_, (Loc, Loc)>, sp!(_, e_): &N::Exp) {
    if mut_vars.is_empty() {
        return;
    }
    match e_ {
        ...
        N::Exp_::Assign(sp!(_, lvalues), rhs) => {
            for lv in lvalues {
                macro_mut_check_assign_lvalue(mut_vars, lv);
            }
            macro_mut_check_exp(mut_vars, rhs);
        }
        // Field mutation: `x.field = e`
        N::Exp_::FieldMutate(dotted, rhs) => {
            macro_mut_check_dotted_base(mut_vars, dotted);
            macro_mut_check_exp(mut_vars, rhs);
        }
        // Dereference mutation: `*x = e`
        N::Exp_::Mutate(lhs, rhs) => {
            macro_mut_check_exp(mut_vars, lhs);
            macro_mut_check_exp(mut_vars, rhs);
        }
        ...
        N::Exp_::Block(N::Block { seq, .. }) => macro_mut_check_seq(mut_vars, seq),
        N::Exp_::Lambda(N::Lambda { body, .. }) => macro_mut_check_exp(mut_vars, body),
        N::Exp_::IfElse(econd, et, ef) => {
            macro_mut_check_exp(mut_vars, econd);
            macro_mut_check_exp(mut_vars, et);
            if let Some(ef) = ef {
                macro_mut_check_exp(mut_vars, ef);
            }
        }
        N::Exp_::Match(esubject, arms) => {
            macro_mut_check_exp(mut_vars, esubject);
            for arm in &arms.value {
                if let Some(guard) = &arm.value.guard {
                    macro_mut_check_exp(mut_vars, guard);
                }
                macro_mut_check_exp(mut_vars, &arm.value.rhs);
            }
        }
        N::Exp_::While(_, econd, ebody) | N::Exp_::BinopExp(econd, _, ebody) => {
            macro_mut_check_exp(mut_vars, econd);
            macro_mut_check_exp(mut_vars, ebody);
        }
        N::Exp_::Loop(_, e)
        | N::Exp_::Return(e)
        | N::Exp_::Abort(e)
        ...
        | N::Exp_::UnaryExp(_, e)
        | N::Exp_::Cast(e, _)
        | N::Exp_::Annotate(e, _) => macro_mut_check_exp(mut_vars, e),
        N::Exp_::ModuleCall(_, _, _, _, sp!(_, es))
        | N::Exp_::VarCall(_, sp!(_, es))
        | N::Exp_::Builtin(_, sp!(_, es))
        | N::Exp_::Vector(_, _, sp!(_, es))
        ...
        N::Exp_::ExpDotted(_, dotted) => macro_mut_check_exp_dotted(mut_vars, dotted),
        N::Exp_::Value(_)
        | N::Exp_::Var(_)
        | N::Exp_::Constant(_, _)
        | N::Exp_::Continue(_)
...
fn macro_mut_check_assign_lvalue(
    mut_vars: &mut BTreeMap<N::Var_, (Loc, Loc)>,
    sp!(_, lv_): &N::LValue,
) {
    match lv_ {
        ...
        N::LValue_::Unpack(_, _, _, fields) => {
            for (_, _, (_, inner)) in fields {
                macro_mut_check_assign_lvalue(mut_vars, inner);
            }
        }
...
fn macro_mut_check_dotted_base(
    mut_vars: &mut BTreeMap<N::Var_, (Loc, Loc)>,
    ...
    match dotted_ {
        ...
        N::ExpDotted_::Dot(inner, _, _) | N::ExpDotted_::DotAutocomplete(_, inner) => {
            macro_mut_check_dotted_base(mut_vars, inner);
        }
...
fn macro_mut_check_exp_dotted(
    mut_vars: &mut BTreeMap<N::Var_, (Loc, Loc)>,
    sp!(_, dotted_): &N::ExpDotted,
) {
    match dotted_ {
        N::ExpDotted_::Exp(e) => macro_mut_check_exp(mut_vars, e),
        N::ExpDotted_::Dot(inner, _, _) | N::ExpDotted_::DotAutocomplete(_, inner) => {
            macro_mut_check_exp_dotted(mut_vars, inner)
        }
