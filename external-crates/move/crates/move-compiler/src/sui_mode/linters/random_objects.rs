...
use super::{
    LinterDiagCategory, LINTER_DEFAULT_DIAG_CODE,
    LINT_WARNING_PREFIX, SUI_PKG_NAME, RANDOM_MOD_NAME, RANDOM_GENERATOR_STRUCT_NAME, RANDOM_STRUCT_NAME
};
...
impl TypingVisitor for RandomObjectsVisitor {
    fn visit(
        ...
        for (_, _, mdef) in program.modules.iter() {
            ...
            mdef.functions.iter()
                .filter(|(_, _, fdef)| !fdef.attributes.is_test_or_test_only() && matches!(fdef.visibility, Visibility::Public(_)))
