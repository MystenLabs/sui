...
use crate::{
    ...
    compressed::{annotated as CA, runtime as CR},
...
fn intern_shuffled_runtime(
    ...
    match t {
        ...
        T::Enum(e) => {
            ...
            let variants: Vec<Option<Vec<CR::LayoutHandle>>> =
                handles.into_iter().map(Option::unwrap).map(Some).collect();
            b.enum_layout(variants).unwrap()
        }
    }
}
...
fn intern_shuffled_annotated(
    ...
    match t {
        ...
        T::Struct(s) => {
            ...
            b.struct_layout(s.type_.clone(), handles).unwrap()
        }
        T::Enum(e) => {
            ...
            let variants: Vec<(Identifier, u16, Option<Vec<(Identifier, CA::LayoutHandle)>>)> =
                variants.into_iter().map(Option::unwrap).collect();
            b.enum_layout(e.type_.clone(), variants).unwrap()
        }
    }
}
...
fn runtime_view_mismatched_kinds_not_equivalent() {
    let v1 = compress_runtime(&R::MoveTypeLayout::U8).as_view();
    let v2 = compress_runtime(&R::MoveTypeLayout::U16).as_view();
    ...
    let v_u8 = compress_runtime(&R::MoveTypeLayout::U8).as_view();
...
fn runtime_unknown_variant_equivalence() {
    ...
    let u8_h1 = b1.u8();
    let h1 = b1.enum_layout(vec![None, Some(vec![u8_h1])]).unwrap();
    let l1 = b1.build(h1);

    let mut b2 = CR::MoveTypeLayoutBuilder::new();
    let u8_h2 = b2.u8();
    let h2 = b2.enum_layout(vec![None, Some(vec![u8_h2])]).unwrap();
    let l2 = b2.build(h2);
    ...
    let u8_h3 = b3.u8();
    let h3 = b3
        .enum_layout(vec![Some(vec![]), Some(vec![u8_h3])])
        .unwrap();
    let l3 = b3.build(h3);
...
fn annotated_unknown_variant_equivalence() {
    ...
    let h1 = b1
        .enum_layout(
            struct_tag("0x0::foo::E"),
            vec![
                (ident("V"), 0u16, None),
                (ident("W"), 1u16, Some(vec![(ident("x"), u8_h1)])),
            ],
        )
        .unwrap();
    let l1 = b1.build(h1);
    ...
    let h2 = b2
        .enum_layout(
            ...
            vec![
                (ident("V"), 0u16, None),
                (ident("W"), 1u16, Some(vec![(ident("x"), u8_h2)])),
            ],
        )
        .unwrap();
    let l2 = b2.build(h2);
    ...
    let h3 = b3
        .enum_layout(
            struct_tag("0x0::foo::E"),
            vec![
                (ident("V"), 0u16, Some(vec![])),
                (ident("W"), 1u16, Some(vec![(ident("x"), u8_h3)])),
            ],
        )
        .unwrap();
    let l3 = b3.build(h3);
