...
fn annotated_shared_subtree_dedup() {
    ...
    // 4 nodes: Bool, U64, Inner struct, Wrapper struct (Inner deduped)
    assert_eq!(compressed.node_count(), 4);
...
fn runtime_struct_view_navigate() {
    ...
    let compressed = RC::MoveTypeLayout::from(&layout);
    let view = compressed.as_view();
    let s = match view.current().unwrap() {
        RC::MoveTypeNode::Struct(s) => s,
        _ => panic!("expected struct"),
    };
    assert_eq!(s.fields.len(), 2);

    let field0 = view.view_at(s.fields[0]);
    assert_eq!(format!("{}", field0.inflate().unwrap()), "u64");
    let field1 = view.view_at(s.fields[1]);
...
fn runtime_enum_view_navigate() {
    ...
    let compressed = RC::MoveTypeLayout::from(&layout);
    let view = compressed.as_view();
    let e = match view.current().unwrap() {
        RC::MoveTypeNode::Enum(e) => e,
        _ => panic!("expected enum"),
    };
    assert_eq!(e.variants.len(), 2);

    assert_eq!(e.variants[0].len(), 1);
    assert_eq!(
        format!("{}", view.view_at(e.variants[0][0]).inflate().unwrap()),
        "u8"
    );

    assert_eq!(e.variants[1].len(), 2);
}
...
fn runtime_view_at_navigation() {
    ...
    let inner_idx = match view.current().unwrap() {
        RC::MoveTypeNode::Vector(idx) => *idx,
        _ => panic!("expected vector"),
    };
    let s = match view.view_at(inner_idx).current().unwrap() {
        RC::MoveTypeNode::Struct(s) => s,
        _ => panic!("expected struct"),
    };
    assert_eq!(s.fields.len(), 2);
...
fn annotated_view_struct_navigate() {
    ...
    let s = match view.current().unwrap() {
        AC::MoveTypeNode::Struct(s) => s,
        _ => panic!("expected struct"),
    };
    assert_eq!(view.resolve_tag(s.type_).unwrap().name.as_str(), "Foo");
    assert_eq!(s.fields.len(), 2);

    let (name_idx, layout_idx) = &s.fields[0];
    assert_eq!(view.resolve_string(*name_idx).unwrap().as_str(), "x");
    assert_eq!(
        view.view_at(*layout_idx).inflate().unwrap(),
        A::MoveTypeLayout::U64
    );
}
...
fn annotated_view_enum_navigate() {
    ...
    let view = compressed.as_view();
    let e = match view.current().unwrap() {
        AC::MoveTypeNode::Enum(e) => e,
        _ => panic!("expected enum"),
    };
    assert_eq!(view.resolve_tag(e.type_).unwrap().name.as_str(), "MyEnum");

    let (name_idx, tag, fields) = e.variants.iter().find(|(_, t, _)| *t == 1).unwrap();
    assert_eq!(*tag, 1);
    assert_eq!(view.resolve_string(*name_idx).unwrap().as_str(), "Some");
    assert_eq!(fields.len(), 1);
    assert_eq!(view.resolve_string(fields[0].0).unwrap().as_str(), "value");
    assert_eq!(
        view.view_at(fields[0].1).inflate().unwrap(),
        A::MoveTypeLayout::U64
    );

    let (name_idx, _, fields) = e.variants.iter().find(|(_, t, _)| *t == 0).unwrap();
    assert_eq!(view.resolve_string(*name_idx).unwrap().as_str(), "None");
