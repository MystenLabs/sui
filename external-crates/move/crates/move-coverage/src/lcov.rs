...
impl BranchInfo {
    ...
    pub fn hit_branch(&mut self, branch: BranchNumber) {
        self.branches.get_mut(&branch).map(|h| *h += 1);
    }
}
...
impl PackageRecordKeeper {
    ...
    pub fn calculate_coverage<'a, R: Read>(&mut self, trace: MoveTraceReader<'a, R>) {
            ...
            match event.unwrap() {
                ...
                TraceEvent::Instruction { pc, .. } => {
                    ...
                    if let Some(from) = coming_from {
                        record
                            .branches
                            .get_mut(&(*current_fn_index, from))
                            .map(|info| info.hit_branch(pc));
                    }
...
impl FileRecordKeeper {
    ...
    pub fn to_lcov_records(&self) -> Vec<LRecord> {
        ...
        records.push(LRecord::SourceFile {
            path: source_file_path.clone(),
        });

        records.push(LRecord::FunctionsFound {
            found: functions_found.len() as u32,
        });
        records.push(LRecord::FunctionsHit {
            hit: functions_hit.len() as u32,
        });
        records.push(LRecord::LinesFound {
            found: instrumented_lines.len() as u32,
        });
        records.push(LRecord::LinesHit {
            hit: line_entries.len() as u32,
        });
        records.push(LRecord::BranchesFound {
            found: branches
                .values()
                .map(|info| info.branches.len())
                .sum::<usize>() as u32,
        });
        records.push(LRecord::BranchesHit {
            hit: branches
                .values()
