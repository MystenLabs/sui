...
fn structure_loop(
    ...
    let needs_dispatch_decision = owned_succs.len() > 1;
    if needs_dispatch_decision {
    ...
    let dispatch_mode = needs_dispatch_decision;
    ...
    let reaching_body: Option<D::Structured> = if !dispatch_mode {
    ...
    if dispatch_mode {
        ...
        // Reserved-prefix naming. The leading `__` keeps the synthesized local outside of any
        // user-writable identifier space, and the `dispatch_` prefix is greppable so a reader
        // of the decompiled source knows it's an artifact of multi-succ-loop dispatch synthesis
        // rather than an original Move local.
        ...
        let dispatch_map: HashMap<NodeIndex, crate::ast::DispatchTag> = owned_succs
            ...
            .collect();
        // Dense 0..N-1 tag range is a load-bearing precondition of `inline_dispatch_cascade`
        // (which requires `if (sel <= 0); if (sel <= 1); ...; if (sel <= N-1)`). The
        // `.enumerate()` above establishes it by construction; the debug_assert catches a
        // future edit that introduces gaps or duplicates.
        ...
        // exits (a CFG fork inside the cascade tail), the non-min candidates surface as raw
        // `Jump`s the dispatch arms don't fold; downstream refinements
        ...
        // deterministic without a separate sort, so the `flat_map(neighbors_directed) ->
        // sort_by_key -> dedup -> first` pattern is determinism-by-construction.
