        ...
        match instr {
            ...
            SI::AssignReg {
                ...
                let unpack_fields = fields
                    .iter()
                    .zip(lhs.iter())
                    .map(|(f, r)| (f.0.clone(), format!("{r}")))
                    .collect::<Vec<_>>();
                let module_id = ty.struct_.defining_module.clone();
                let name = ty.struct_.name.clone();
            ...
            SI::AssignReg {
                ...
                let unpack_fields = fields
                    ...
                    .map(|(f, r)| (f.0.clone(), format!("{r}")))
                    .collect::<Vec<_>>();
                let module_id = ty.enum_.defining_module.clone();
                let enum_ = ty.enum_.name.clone();
                let variant = ty.variant.name.clone();
                let rhs = Box::new(trivials(&mut map, args.clone()).remove(0));
