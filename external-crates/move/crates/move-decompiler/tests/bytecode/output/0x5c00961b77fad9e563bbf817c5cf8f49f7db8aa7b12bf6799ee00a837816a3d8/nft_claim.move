// Auto-generated by Move decompiler

module 0x5c00961b77fad9e563bbf817c5cf8f49f7db8aa7b12bf6799ee00a837816a3d8::nft_claim;

// -- uses -- 

use 0x1::string;
use 0x2::display;
use 0x2::object;
use 0x2::package;
use 0x2::transfer;
use 0x2::tx_context;
use 0x2::vec_set;
use 0x5c00961b77fad9e563bbf817c5cf8f49f7db8aa7b12bf6799ee00a837816a3d8::nft_claim;
use 0x1::string::String;
use 0x2::object::UID;
use 0x2::tx_context::TxContext;
use 0x2::vec_set::VecSet;

// -- structs -- 

public struct DEEPClaimNFT has key, store { id: UID, name: String, description: String, image_url: String }

public struct ClaimCap has key, store { id: UID, claimed: VecSet<address> }

public struct NFT_CLAIM has drop { dummy_field: bool }

// -- constants -- 

const C0: vector<u8> = vector[110u8, 97u8, 109u8, 101u8];

const C1: vector<u8> = vector[100u8, 101u8, 115u8, 99u8, 114u8, 105u8, 112u8, 116u8, 105u8, 111u8, 110u8];

const C2: vector<u8> = vector[105u8, 109u8, 97u8, 103u8, 101u8, 95u8, 117u8, 114u8, 108u8];

const C3: vector<u8> = vector[68u8, 69u8, 69u8, 80u8, 32u8, 67u8, 108u8, 97u8, 105u8, 109u8, 32u8, 78u8, 70u8, 84u8];

const C4: vector<u8> = vector[84u8, 104u8, 97u8, 110u8, 107u8, 32u8, 121u8, 111u8, 117u8, 32u8, 102u8, 111u8, 114u8, 32u8, 98u8, 101u8, 105u8, 110u8, 103u8, 32u8, 97u8, 32u8, 83u8, 117u8, 105u8, 32u8, 117u8, 115u8, 101u8, 114u8, 46u8, 32u8, 84u8, 104u8, 105u8, 115u8, 32u8, 101u8, 120u8, 99u8, 108u8, 117u8, 115u8, 105u8, 118u8, 101u8, 32u8, 78u8, 70u8, 84u8, 32u8, 105u8, 115u8, 32u8, 121u8, 111u8, 117u8, 114u8, 32u8, 99u8, 108u8, 97u8, 105u8, 109u8, 32u8, 116u8, 111u8, 32u8, 68u8, 69u8, 69u8, 80u8, 32u8, 116u8, 111u8, 107u8, 101u8, 110u8, 115u8, 46u8];

const C5: vector<u8> = vector[104u8, 116u8, 116u8, 112u8, 115u8, 58u8, 47u8, 47u8, 105u8, 46u8, 105u8, 109u8, 103u8, 117u8, 114u8, 46u8, 99u8, 111u8, 109u8, 47u8, 90u8, 65u8, 100u8, 113u8, 101u8, 90u8, 75u8, 46u8, 112u8, 110u8, 103u8];

// -- functions -- 

public fun get_description(l0: &DEEPClaimNFT): &String {
    return &l0.description
}

public fun get_id(l0: &DEEPClaimNFT): address {
    return object::uid_to_address(&l0.id)
}

public fun get_image_url(l0: &DEEPClaimNFT): &String {
    return &l0.image_url
}

public fun get_name(l0: &DEEPClaimNFT): &String {
    return &l0.name
}

fun init(l0: NFT_CLAIM, l1: &mut TxContext) {
    let l3 = vector[string::utf8(C0), string::utf8(C1), string::utf8(C2)];
    let l5 = vector[string::utf8(C3), string::utf8(C4), string::utf8(C5)];
    let l4 = package::claim(l0, l1);
    let l2 = display::new_with_fields(&l4, l3, l5, l1);
    display::update_version(&mut l2);
    transfer::public_transfer(l4, tx_context::sender(freeze(l1)));
    transfer::public_transfer(l2, tx_context::sender(freeze(l1)));
    transfer::transfer(ClaimCap { id: object::new(l1), claimed: vec_set::empty() }, tx_context::sender(freeze(l1)))
}

public entry fun mint_batch(l0: &mut ClaimCap, l1: vector<address>, l2: &mut TxContext) {
    let l3 = 0u64;
    let l4 = &l1.len();
    while (l3 < l4) {
        let l5 = *(&(&l1)[l3]);
        if (!(vec_set::contains(&l0.claimed, &l5))) {
            nft_claim::mint_single(l0, l5, l2)
        };
        l3 = l3 + 1u64;
    }
}

public fun mint_single(l0: &mut ClaimCap, l1: address, l2: &mut TxContext) {
    if (!(vec_set::contains(&l0.claimed, &l1))) {
        let l3 = DEEPClaimNFT { id: object::new(l2), name: string::utf8(C3), description: string::utf8(C4), image_url: string::utf8(C5) };
        vec_set::insert(&mut l0.claimed, l1);
        transfer::transfer(l3, l1)
    } else {
        abort 0u64
    }
}

