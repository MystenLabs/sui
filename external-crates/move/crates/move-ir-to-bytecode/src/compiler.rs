...
pub fn compile_module<'a>(
    ...
    let version = if enum_defs.is_empty()
        && enum_def_instantiations.is_empty()
        && variant_handles.is_empty()
        && variant_instantiation_handles.is_empty()
        && function_defs.iter().all(|f| {
            f.code
                .as_ref()
