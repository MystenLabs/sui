...
pub struct GlobalEnv {
    ...
    global_invariants_for_memory: BTreeMap<QualifiedInstId<DatatypeId>, BTreeSet<GlobalId>>,
...
impl GlobalEnv {
    ...
    pub fn is_wellknown_event_handle_type(&self, ty: &Type) -> bool {
        if let Type::Datatype(mid, sid, _) = ty {
    ...
    pub fn add(
        ...
        mut struct_data: BTreeMap<DatatypeId, StructData>,
        ...
        let struct_idx_to_id: BTreeMap<StructDefinitionIndex, DatatypeId> = struct_data
            ...
            .map(|(id, data)| match &data.info {
                ...
                StructInfo::Generated { .. } => panic!("generated struct not expected"),
            })
            .collect();
        ...
        for (svar_id, svar) in &spec_vars {
            ...
            struct_data.insert(DatatypeId::new(data.name), data);
        }
...
impl<'env> FunctionEnv<'env> {
    ...
    pub fn get_modify_targets(&self) -> BTreeMap<QualifiedId<DatatypeId>, Vec<Exp>> {
        ...
        let mut modify_targets: BTreeMap<QualifiedId<DatatypeId>, Vec<Exp>> = BTreeMap::new();
