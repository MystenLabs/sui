...
use crate::{
    ...
    jsonrpc::Endpoint,
    schema::{
        EnvironmentID, EnvironmentName, ManifestDependencyInfo, PackageName, ResolveRequest,
        ResolveResponse, ResolverDependencyInfo, ResolverName,
    },
};

use super::{Dependency, DependencySet, Parsed, Resolved};
...
pub async fn resolve(
    deps: DependencySet<Dependency<Parsed>>,
    ...
    for (env, pkg, dep) in deps.iter() {
        if let Parsed::External(ext) = &dep.dep_info {
            requests.entry(ext.resolver.clone()).or_default().insert(
                env.clone(),
                pkg.clone(),
                ResolveRequest {
                    env: dep.source_environment.clone(),
                    data: ext.data.clone(),
                },
            );
        }
    ...
    for (env, pkg, dep) in deps.into_iter() {
        ...
        result.insert(
            ...
            dep.map(|info| match info {
                Parsed::Local(loc) => Resolved::Local(loc),
                Parsed::Git(git) => Resolved::Git(git),
                Parsed::OnChain(onchain) => Resolved::OnChain(onchain),
                Parsed::External(_) => ext.expect("resolve_single outputs same keys as input"),
            }),
        );
    }
    Ok(result)
