...
use crate::{
    dependency::{pin, DependencySet, ManifestDependencyInfo, PinnedDependencyInfo},
    errors::{with_file, Located, LockfileError, PackageError, PackageResult},
    flavor::MoveFlavor,
};

use super::{manifest::Manifest, EnvironmentName, PackageName};
...
impl<F: MoveFlavor + fmt::Debug> Lockfile<F> {
    ...
    async fn update_lockfile(
        ...
        flavor: &F,
        manifest: Manifest<F>,
        // manifest_deps: DependencySet<ManifestDependencyInfo<F>>,
    ) -> PackageResult<()> {
            ...
            match env {
                ...
                Some(env) => {
                    self.unpublished
                        .dep_overrides
                        .get_mut(&env)
                        .map(|p| p.pinned.get_mut(&pkg).map(|x| *x = dep));
                }
                // update default pinned deps
                None => {
                    if let Some(p) = self.unpublished.dependencies.pinned.get_mut(&pkg) {
                        *p = dep;
                    }
                }
            }
