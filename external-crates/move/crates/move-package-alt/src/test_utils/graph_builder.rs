...
pub struct TestPackageGraph {
    ...
    inner: DiGraph<PackageBuilder, DepBuilder>,
...
impl TestPackageGraph {
    ...
    pub fn new(node_names: impl IntoIterator<Item = impl AsRef<str>>) -> Self {
        ...
        for node in node_names {
            let index = inner.add_node(PackageBuilder::new(node.as_ref()));
    ...
    pub fn add_package(
        mut self,
        node: impl AsRef<str>,
        build: impl FnOnce(PackageBuilder) -> PackageBuilder,
    ) -> Self {
        let builder = PackageBuilder::new(&node);
    ...
    pub fn add_dep(
        ...
        build: impl FnOnce(DepBuilder) -> DepBuilder,
    ) -> Self {
        let source_idx = self.nodes[source.as_ref()];
        let target_idx = self.nodes[target.as_ref()];
        let dep_spec = build(DepBuilder::new(target));
    ...
    pub fn build(self) -> Scenario {
        ...
        for (package_id, node) in self.nodes.iter() {
            ...
            debug!("Generated test Move.toml for {package_id}:\n\n{manifest}");
            project = project.file(dir.join("Move.toml"), manifest);

            if let Some(published) = &self.format_published(*node) {
                debug!("Generated test Move.published for {package_id}:\n\n{published}");
                project = project.file(dir.join("Move.published"), published);
            }

            if let Some(local_pub) = &self.format_local_pubs(*node) {
                debug!("Generated test Move.published.local for {package_id}:\n\n{local_pub}");
                project = project.file(dir.join("Move.published.local"), local_pub);
            }
        }
    ...
    fn format_legacy_dep(&self, dep: &DepBuilder, target: &PackageBuilder) -> String {
