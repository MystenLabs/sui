...
pub fn project_in(dir: impl AsRef<Path>) -> ProjectBuilder {
    ProjectBuilder::new(root().join(dir).join("foo"))
}
...
pub fn project_in_home(name: impl AsRef<Path>) -> ProjectBuilder {
    ProjectBuilder::new(home().join(name))
}
...
pub fn symlink_supported() -> bool {
    ...
    let src = root().join("symlink_src");
    fs::write(&src, "").unwrap();
    let dst = root().join("symlink_dst");
