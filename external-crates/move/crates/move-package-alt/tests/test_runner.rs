...
use move_package_alt::{
    ...
    package::{RootPackage, lockfile::Lockfiles, paths::PackagePath},
    ...
    fn output(&self) -> anyhow::Result<String> {
        Ok(match self.kind {
            "parsed" => {
                ...
                let manifest = ParsedManifest::read_from_file(file_handle);
...
async fn run_pinning_tests(input_path: &Path) -> datatest_stable::Result<String> {
    ...
    let env = vanilla::default_environment();
    let manifest = ParsedManifest::read_from_file(file_handle).unwrap();

    let deps = CombinedDependency::combine_deps(file_handle, &env, &manifest).unwrap();
