...
impl<Progress: Write> DependencyGraphBuilder<Progress> {
    ...
    pub fn get_graph(
        ...
        let root_pkg_id = custom_resolve_pkg_id(&root_manifest, lock_string.as_deref())
            .with_context(|| {
                format!(
    ...
    fn new_for_dep(
        ...
        match dep {
            PM::Dependency::Internal(d) => {
                ...
                let resolved_pkg_id = custom_resolve_pkg_id(&manifest, lock_string.as_deref())
                    .with_context(|| format!("Resolving package name for '{}'", dep_pkg_name))?;
                let resolved_version = resolve_version(&manifest, lock_string.as_deref())
