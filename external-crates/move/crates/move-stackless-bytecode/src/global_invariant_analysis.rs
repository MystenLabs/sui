...
use move_model::{
    ast::ConditionKind,
    model::{DatatypeId, FunId, FunctionEnv, GlobalEnv, GlobalId, QualifiedId, QualifiedInstId},
...
impl PerFunctionRelevance {
    ...
    fn calculate_invariant_relevance<'a>(
        env: &GlobalEnv,
        mem_related: impl Iterator<Item = &'a QualifiedInstId<DatatypeId>>,
...
fn get_callee_memory_usage_for_invariant_instrumentation(
    ...
    BTreeSet<QualifiedInstId<DatatypeId>>, // memory constitute to entry-point assumptions
    BTreeSet<QualifiedInstId<DatatypeId>>, // memory constitute to in-line or exit-point assertions
) {
    let inv_analysis = env
        .get_extension::<InvariantAnalysisData>()
        .expect("Verification analysis not performed");
