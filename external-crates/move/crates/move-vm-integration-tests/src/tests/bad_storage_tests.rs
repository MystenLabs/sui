...
use crate::compiler::{as_module, compile_units, serialize_module};
...
fn test_malformed_module() {
    ...
    let mut blob = vec![];
    serialize_module(&m, &mut blob).unwrap();
...
fn test_missing_module_dependency() {
    ...
    let mut blob_m = vec![];
    serialize_module(&m, &mut blob_m).unwrap();
    let mut blob_n = vec![];
    serialize_module(&n, &mut blob_n).unwrap();
