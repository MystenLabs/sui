...
impl GasProfiler {
    pub fn write_profile_from_trace<R: Read>(
        ...
        let mut prof = GasProfiler {
            ...
            start_gas: *gas_left,
            config: Some(VMProfilerConfig {
                full_path: config.output_path,
                use_long_function_name: config.use_long_function_name,
        ...
        for event in trace.into_iter() {
            match event? {
                ...
                TraceEvent::OpenFrame { frame, gas_left } => {
                    let fn_name = if self.config.as_ref().unwrap().use_long_function_name {
    ...
    pub fn dump_file(&self) -> anyhow::Result<()> {
