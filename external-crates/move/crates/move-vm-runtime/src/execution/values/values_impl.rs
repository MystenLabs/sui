...
impl IntegerValue {
    ...
    // Signed division truncates toward zero (Rust semantics), not toward negative infinity.
    ...
    // Signed right shift is arithmetic (sign-extending), matching Rust's `checked_shr` semantics.
    shift_op!(
        shl_checked,
        safe_shift_left,
