...
use move_binary_format::{
    ...
    file_format::{
        ...
        VariantJumpTable, VariantTag, VariantHandleIndex, VariantHandle, VariantInstantiationHandle, VariantInstantiationHandleIndex,
    },
    IndexKind,
};
...
impl ModuleCache {
    ...
    fn add_module(
        ...
        for datatype in self.types.binaries.iter().rev().take(field_types_len) {
...
impl<'a> Resolver<'a> {
    ...
    pub(crate) fn instantiate_enum_type(
        ...
        let handle = self.binary.loaded.variant_instantiation_handle_at(vidx);
        let enum_inst = self.binary.loaded.enum_instantiation_at(handle.enum_def);
    ...
    pub(crate) fn variant_field_count_and_tag(&self, vidx: VariantHandleIndex) -> (u16, VariantTag) {
...
impl LoadedModule {
    ...
    fn variant_instantiation_handle_at(&self, vidx: VariantInstantiationHandleIndex) -> &VariantInstantiationHandle {
    ...
    fn variant_instantiantiation_field_count_and_tag(
        ...
        (enum_inst.variant_count_map[handle.variant as usize], handle.variant)
    }
}
