...
use crate::{
    ...
    dbg_println, invariant_violation, jit,
...
fn load_packages(
    ...
    let pkgs = match store.get_packages(ids) {
        ...
        Err(err) => {
            return Err(invariant_violation!("Unexpected storage error: {:?}", err)
...
pub(crate) fn jit_and_cache_package(
    ...
    cache.cached_package_at(version_id).ok_or_else(|| {
        invariant_violation!("Package not found in cache after loading")
