    ...
    for (_, module) in &pkg.modules {
        ...
        for (_, function) in &module.functions {
            if let Some(code) = &function.code {
                ...
                assert!(code.code.len() >= 1, "Functions should have at least one basic block");
        ...
        for (func_idx, unopt_func) in &unopt_module.functions {
            ...
            if let (Some(unopt_code), Some(opt_code)) = (&unopt_func.code, &opt_func.code) {
                ...
                for (label, block) in &opt_code.code {
                    assert!(!block.is_empty(), "Optimized block {} should not be empty", label);
                }
            }
        }
    }
...
fn test_complex_control_flow() {
    ...
    for (_, module) in &pkg.modules {
        for (_, function) in &module.functions {
            if let Some(code) = &function.code {
                let total_instructions = code.code.values()
                    .map(|block| block.len())
                    .sum::<usize>();

                // Complex functions should have reasonable number of instructions
                if code.code.len() > 2 {
                    assert!(total_instructions > 0, "Complex functions should have instructions");
                }
            }
        }
    }
