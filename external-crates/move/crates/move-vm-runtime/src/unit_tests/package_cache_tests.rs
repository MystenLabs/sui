...
fn cache_and_evict_packages() {
    ...
    assert!(adapter.runtime().cache().remove_package(&package2_address), "Package 2 not found");
    assert!(adapter.runtime().cache().package_cache().read().len() == 2);

    assert!(!adapter.runtime().cache().remove_package(&package2_address), "Package 2 double-evicted");
    assert!(adapter.runtime().cache().package_cache().read().len() == 2);
