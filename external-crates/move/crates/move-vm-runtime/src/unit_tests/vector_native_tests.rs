...
use crate::natives::move_stdlib::vector::{
    CopyRangeGasParameters, KeepRangeGasParameters, ReplaceRangeGasParameters,
    ReverseGasParameters, keep_range_gas_count, native_copy_range, native_keep_range,
    native_replace_range, native_reverse, replace_range_gas_count,
};
use crate::{
...
#[test]
fn keep_range_gas_count_counts_dropped_and_compacted_elements() {
    // Keeping a prefix drops only the suffix; no retained element is moved.
    assert_eq!(keep_range_gas_count(5, 0, 3), (2, 0));
    // Keeping a middle range drops both sides and compacts the retained elements.
    assert_eq!(keep_range_gas_count(5, 1, 4), (2, 3));
    // Keeping a suffix compacts the retained elements to the front.
    assert_eq!(keep_range_gas_count(5, 2, 5), (2, 3));
    assert_eq!(keep_range_gas_count(5, 0, 5), (0, 0));
    assert_eq!(keep_range_gas_count(5, 2, 2), (5, 0));
}

#[test]
fn keep_range_gas_count_is_zero_for_invalid_ranges() {
    assert_eq!(keep_range_gas_count(5, 4, 3), (0, 0));
    assert_eq!(keep_range_gas_count(5, 0, 6), (0, 0));
    assert_eq!(keep_range_gas_count(5, u64::MAX, u64::MAX), (0, 0));
}

#[test]
fn replace_range_gas_count_covers_valid_shapes() {
    // Equal-size replacement does not move the tail.
    assert_eq!(replace_range_gas_count(8, 2, 5, 3), 3);
    // Growing and shrinking both move the tail when the sizes differ.
    assert_eq!(replace_range_gas_count(8, 2, 4, 5), 9);
    assert_eq!(replace_range_gas_count(8, 2, 6, 1), 3);
    ...
    assert_eq!(replace_range_gas_count(8, 8, 8, 3), 3);
    assert_eq!(replace_range_gas_count(8, 3, 8, 0), 0);
    // Draining from the middle moves only the tail. The returned value is charged separately.
    assert_eq!(replace_range_gas_count(8, 2, 5, 0), 3);
}

#[test]
fn replace_range_gas_count_is_zero_for_invalid_ranges() {
    assert_eq!(replace_range_gas_count(8, 5, 4, 3), 0);
    assert_eq!(replace_range_gas_count(8, 0, 9, 3), 0);
    assert_eq!(replace_range_gas_count(8, u64::MAX, u64::MAX, 3), 0);
}
...
fn successful_native_costs_match_work_formulas() {
    ...
    let keep_params = KeepRangeGasParameters {
        base: 5.into(),
        per_dropped_elem: 3.into(),
        per_moved_elem: 7.into(),
    };
    let keep_result = with_native_context(100, |context| {
        native_keep_range(&keep_params, context, vec![Type::U8], keep_args).unwrap()
    });
    ...
    let splice_params = ReplaceRangeGasParameters {
        base: 5.into(),
        per_elem: 3.into(),
    };
    let splice_result = with_native_context(100, |context| {
        native_replace_range(&splice_params, context, vec![Type::U8], splice_args).unwrap()
    });
    ...
    let slice_result = with_native_context(100, |context| {
        native_copy_range(
            &CopyRangeGasParameters { base: 5.into() },
            context,
            vec![Type::U8],
            slice_args,
        )
...
fn invalid_ranges_charge_only_base() {
    ...
    let splice_result = with_native_context(100, |context| {
        native_replace_range(
            &ReplaceRangeGasParameters {
                base: 5.into(),
                per_elem: 3.into(),
            },
            context,
...
#[test]
fn keep_range_full_range_charges_only_base() {
    ...
    let params = KeepRangeGasParameters {
        base: 5.into(),
        per_dropped_elem: 3.into(),
        per_moved_elem: 7.into(),
    };
    ...
    let result = with_native_context(100, |context| {
        native_keep_range(&params, context, vec![Type::U8], args).unwrap()
    });
...
#[test]
fn keep_range_out_of_gas_does_not_mutate() {
    let holder = MemBox::new(Value::vector_u8([1, 2, 3, 4, 5]));
    let inspect: VectorRef = VMValueCast::cast(holder.as_ref_value()).unwrap();
...
#[test]
fn replace_range_out_of_gas_does_not_mutate() {
    let holder = MemBox::new(Value::vector_u8([1, 2, 3, 4]));
    let inspect: VectorRef = VMValueCast::cast(holder.as_ref_value()).unwrap();
    ...
    let params = ReplaceRangeGasParameters {
        base: 5.into(),
        per_elem: 3.into(),
    };

    let result = with_native_context(16, |context| {
        native_replace_range(&params, context, vec![Type::U8], args).unwrap()
    });
