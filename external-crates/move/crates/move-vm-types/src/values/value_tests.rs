...
fn leagacy_ref_abstract_memory_size_consistency() -> PartialVMResult<()> {
    ...
    let r = locals.borrow_loc(0)?;
    assert_eq!(
        r.abstract_memory_size(&SIZE_CONFIG).ok(),
        Some(r.legacy_size())
    );
...
fn legacy_struct_abstract_memory_size_consistenty() -> PartialVMResult<()> {
    ...
    for s in &structs {
        assert_eq!(
            s.abstract_memory_size(&SIZE_CONFIG).ok(),
            Some(s.legacy_size())
        );
    }
...
fn legacy_val_abstract_memory_size_consistency() -> PartialVMResult<()> {
    ...
    for (idx, val) in vals.iter().enumerate() {
        ...
        let val_size_new = val.abstract_memory_size(&SIZE_CONFIG).ok();
        let val_size_old = Some(val.legacy_size());
