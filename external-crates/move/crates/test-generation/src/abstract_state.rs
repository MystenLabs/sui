...
use move_binary_format::file_format::{
    ...
    FunctionInstantiationIndex, Signature, SignatureIndex, SignatureToken, StructDefInstantiation,
    StructDefInstantiationIndex, StructDefinitionIndex, TableIndex, empty_module,
};
...
impl InstantiableModule {
    ...
    /// Ensure the module has an `address` constant and return its index. Modules generated by the
    /// `module-generation` crate have an empty constant pool, but inhabiting an `address` value
    /// requires a constant to `LdConst`. The constant is added directly to the underlying module's
    /// constant pool (which `instantiate` preserves) and reused on subsequent calls.
    pub fn add_address_constant(&mut self) -> ConstantPoolIndex {
        if let Some(index) = self
            .module
            .constant_pool
            .iter()
            .position(|c| c.type_ == SignatureToken::Address)
        {
            return ConstantPoolIndex(index as TableIndex);
        }
        let index = ConstantPoolIndex(self.module.constant_pool.len() as TableIndex);
        self.module.constant_pool.push(Constant {
            type_: SignatureToken::Address,
            data: AccountAddress::ZERO.to_vec(),
        });
        index
    }
...
pub struct AbstractState {
    ...
    /// A map from local indices to `AbstractValue`s and `BorrowState`s. A `BTreeMap` (rather than
    /// a `HashMap`) so that iteration order is deterministic, which keeps seeded generation
    /// reproducible.
    locals: BTreeMap<usize, (AbstractValue, BorrowState)>,
...
impl AbstractState {
    ...
    pub fn get_locals(&self) -> &BTreeMap<usize, (AbstractValue, BorrowState)> {
        &self.locals
    }
