...
pub trait ModuleAccess: Sync {
    ...
    fn struct_name(&self, idx: StructDefinitionIndex) -> &IdentStr {
        ...
        let handle = self.declared_type_handle_at(struct_def.struct_handle);
        self.identifier_at(handle.name)
    }

    fn enum_name(&self, idx: EnumDefinitionIndex) -> &IdentStr {
        let enum_def = self.enum_def_at(idx);
        let handle = self.declared_type_handle_at(enum_def.enum_handle);
        self.identifier_at(handle.name)
    }
    ...
    fn declared_type_handle_at(&self, idx: DeclaredTypeHandleIndex) -> &DeclaredTypeHandle {
        let handle = &self.as_module().declared_type_handles[idx.into_index()];
    ...
    fn declared_type_handles(&self) -> &[DeclaredTypeHandle] {
        &self.as_module().declared_type_handles
    }
    ...
    fn find_struct_def(&self, idx: DeclaredTypeHandleIndex) -> Option<&StructDefinition> {
    ...
    fn find_struct_def_by_name(&self, name: &IdentStr) -> Option<&StructDefinition> {
        self.struct_defs().iter().find(|def| {
            let handle = self.declared_type_handle_at(def.struct_handle);
...
pub trait ScriptAccess: Sync {
    ...
    fn declared_type_handle_at(&self, idx: DeclaredTypeHandleIndex) -> &DeclaredTypeHandle {
        &self.as_script().declared_type_handles[idx.into_index()]
    }
    ...
    fn declared_type_handles(&self) -> &[DeclaredTypeHandle] {
        &self.as_script().declared_type_handles
    }
