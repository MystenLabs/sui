...
use crate::{
    ...
    file_format::{
        ...
        DeclaredTypeHandle, DeclaredTypeHandleIndex, EnumDefInstantiation,
        EnumDefInstantiationIndex, EnumDefinition, EnumDefinitionIndex, FieldHandle,
        ...
        StructDefinition, StructDefinitionIndex, VariantDefinition, VariantTag,
    },
    CompiledModule,
};
use move_core_types::{
...
impl<'a> BinaryIndexedView<'a> {
    ...
    pub fn declared_type_handles(&self) -> &[DeclaredTypeHandle] {
        match self {
            BinaryIndexedView::Module(module) => module.declared_type_handles(),
            BinaryIndexedView::Script(script) => script.declared_type_handles(),
        }
    }
    ...
    pub fn declared_type_handle_at(&self, idx: DeclaredTypeHandleIndex) -> &DeclaredTypeHandle {
        match self {
            BinaryIndexedView::Module(module) => module.declared_type_handle_at(idx),
            BinaryIndexedView::Script(script) => script.declared_type_handle_at(idx),
        }
    }
    ...
    pub fn abilities(
        ...
        match ty {
            ...
            DeclaredType(idx) => {
                let sh = self.declared_type_handle_at(*idx);
                Ok(sh.abilities)
            }
