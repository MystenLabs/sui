...
impl<'a> BinaryIndexedView<'a> {
    ...
    pub fn data_type_handles(&self) -> &[DataTypeHandle] {
        match self {
            BinaryIndexedView::Module(module) => module.data_type_handles(),
            BinaryIndexedView::Script(script) => script.data_type_handles(),
        }
    }
    ...
    pub fn data_type_handle_at(&self, idx: DataTypeHandleIndex) -> &DataTypeHandle {
        match self {
            BinaryIndexedView::Module(module) => module.data_type_handle_at(idx),
            BinaryIndexedView::Script(script) => script.data_type_handle_at(idx),
        }
    }
    ...
    pub fn abilities(
        ...
        match ty {
            ...
            DataType(idx) => {
                let sh = self.data_type_handle_at(*idx);
                Ok(sh.abilities)
            }
