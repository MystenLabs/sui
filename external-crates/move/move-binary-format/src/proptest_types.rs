...
impl CompiledModuleStrategyGen {
    ...
    pub fn generate(self) -> impl Strategy<Value = CompiledModule> {
        ...
        //
        let data_type_handles_strat = vec(
            DataTypeHandleGen::strategy(self.data_type_params.clone()),
            1..=self.size,
        );

        let struct_defs_strat = vec(
            StructDefinitionGen::strategy(self.field_count.clone(), self.data_type_params.clone()),
            1..=self.size / 2,
        );
        ...
        (
            ...
            .prop_map(
                |(
                    ...
                    //
                    // struct handles
                    let mut data_type_handles = vec![];
                    if module_handles_len > 1 {
                        let mut data_type_handles_set = BTreeSet::new();
                        for data_type_handle_gen in data_type_handle_gens.into_iter() {
                            let sh = data_type_handle_gen.materialize(
                                self_module_handle_idx,
                                module_handles_len,
                                identifiers_len,
                            );
                            if data_type_handles_set.insert((sh.module, sh.name)) {
                                data_type_handles.push(sh);
                            }
                        }
                    }
                    ...
                    let mut signatures: Vec<_> = random_sigs_gens
                        .into_iter()
                        .map(|sig_gen| sig_gen.materialize(&data_type_handles))
                        .collect();
