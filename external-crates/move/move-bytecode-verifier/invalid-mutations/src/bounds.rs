...
use move_binary_format::{
    ...
    file_format::{
        AddressIdentifierIndex, CompiledModule, DeclaredTypeHandleIndex, FunctionHandleIndex,
...
impl PointerKind {
    ...
    pub fn pointers_from(src_kind: IndexKind) -> &'static [PointerKind] {
        ...
        match src_kind {
            ...
            DeclaredTypeHandle => &[One(ModuleHandle), One(Identifier)],
            FunctionHandle => &[
                One(ModuleHandle),
                One(Identifier),
                One(Signature),
...
impl ApplyOutOfBoundsContext {
    ...
    fn set_index(
        ...
        match (src_kind, dst_kind) {
            ...
            (DeclaredTypeHandle, ModuleHandle) => {
                self.module.declared_type_handles[src_idx].module = ModuleHandleIndex(new_idx)
            }
            (DeclaredTypeHandle, Identifier) => {
                self.module.declared_type_handles[src_idx].name = IdentifierIndex(new_idx)
            }
            ...
            (StructDefinition, DeclaredTypeHandle) => {
                self.module.struct_defs[src_idx].struct_handle = DeclaredTypeHandleIndex(new_idx)
            }
            ...
            (Signature, DeclaredTypeHandle) => {
                ...
                self.module.signatures[src_idx].0[arg_idx]
                    .debug_set_sh_idx(DeclaredTypeHandleIndex(new_idx));
            }
...
fn struct_handle(token: &SignatureToken) -> Option<DeclaredTypeHandleIndex> {
    use SignatureToken::*;

    match token {
        DeclaredType(sh_idx) => Some(*sh_idx),
        DeclaredTypeInstantiation(sh_idx, _) => Some(*sh_idx),
