...
use move_binary_format::{
    ...
    file_format::{
        AbilitySet, Bytecode, CodeOffset, CompiledModule, CompiledScript, DeclaredTypeHandleIndex,
...
struct Context<'a, 'b> {
    ...
    struct_id_to_handle_map: BTreeMap<(ModuleId, Identifier), DeclaredTypeHandleIndex>,
...
fn verify_imported_structs(context: &Context) -> PartialVMResult<()> {
    let self_module = context.resolver.self_handle_idx();
    for (idx, struct_handle) in context.resolver.declared_type_handles().iter().enumerate() {
        ...
        {
            Some(def_idx) => {
                let def_handle = owner_module.declared_type_handle_at(*def_idx);
...
fn compare_types(
    ...
    match (handle_type, def_type) {
        ...
        (SignatureToken::DeclaredType(idx1), SignatureToken::DeclaredType(idx2)) => {
            compare_structs(context, *idx1, *idx2, def_module)
        }
        (
            SignatureToken::DeclaredTypeInstantiation(idx1, inst1),
            SignatureToken::DeclaredTypeInstantiation(idx2, inst2),
        ) => {
            compare_structs(context, *idx1, *idx2, def_module)?;
        ...
        | (SignatureToken::DeclaredType(_), _)
        | (SignatureToken::DeclaredTypeInstantiation(_, _), _)
...
fn compare_structs(
    context: &Context,
    idx1: DeclaredTypeHandleIndex,
    idx2: DeclaredTypeHandleIndex,
    def_module: &CompiledModule,
) -> PartialVMResult<()> {
    // grab ModuleId and struct name for the module being verified
    let struct_handle = context.resolver.declared_type_handle_at(idx1);
    let module_handle = context.resolver.module_handle_at(struct_handle.module);
    let module_id = context.resolver.module_id_for_handle(module_handle);
    let struct_name = context.resolver.identifier_at(struct_handle.name);

    // grab ModuleId and struct name for the definition
    let def_struct_handle = def_module.declared_type_handle_at(idx2);
