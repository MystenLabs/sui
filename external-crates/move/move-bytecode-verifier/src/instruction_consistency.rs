...
use move_binary_format::{
    ...
    file_format::{
        Bytecode, CodeOffset, CodeUnit, CompiledModule, CompiledScript, DeclaredTypeHandleIndex,
...
impl<'a> InstructionConsistency<'a> {
    ...
    fn check_type_op_(
        &self,
        offset: usize,
        declared_type_handle_index: DeclaredTypeHandleIndex,
        generic: bool,
    ) -> PartialVMResult<()> {
        let declared_type_handle = self
            .resolver
            .declared_type_handle_at(declared_type_handle_index);
        if declared_type_handle.type_parameters.is_empty() == generic {
            return Err(
