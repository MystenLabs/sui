...
use move_binary_format::{
    ...
    file_format::{
        CompiledModule, DeclaredTypeHandleIndex, SignatureToken, StructDefinitionIndex, TableIndex,
    },
...
struct StructDefGraphBuilder<'a> {
    ...
    handle_to_def: BTreeMap<DeclaredTypeHandleIndex, StructDefinitionIndex>,
}
...
impl<'a> StructDefGraphBuilder<'a> {
    ...
    fn add_signature_token(
        ...
        match token {
            ...
            T::DeclaredTypeInstantiation(sh_idx, inners) => {
                if let Some(struct_def_idx) = self.handle_to_def.get(sh_idx) {
                    neighbors
                        .entry(cur_idx)
                        .or_insert_with(BTreeSet::new)
