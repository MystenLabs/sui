...
use move_binary_format::{
    ...
    file_format::{
        AbilitySet, Bytecode, CodeOffset, DeclaredTypeHandleIndex, EnumDefinition,
        FieldHandleIndex, FunctionDefinitionIndex, FunctionHandle, JumpTable, LocalIndex,
        Signature, SignatureToken, SignatureToken as ST, StructDefinition, StructDefinitionIndex,
        StructFieldInformation, VariantDefinition,
    },
    safe_unwrap_err,
};
...
// * The type of the reference is the same as the type of the enum
...
fn variant_switch(
    ...
    // Check: type of reference is the same as the type of the enum
    let inner_type = match operand {
        ST::Reference(inner) if jump_table.head_signature.0 == *inner => inner,
    ...
    let enum_def = match *inner_type {
        SignatureToken::DeclaredType(handle)
        | SignatureToken::DeclaredTypeInstantiation(handle, _) => {
    ...
    // Check: Exhaustivity and uniqueness of the jump table.
    let mut seen_tags = BTreeSet::new();
    for (tag, _) in jump_table.jump_table.iter() {
        // Found duplicate tag
        if !seen_tags.insert(tag) {
            return Err(verifier.error(StatusCode::DUPLICATE_ENUM_SWITCH_TAG, offset));
        }
    }

    // No duplicates, so cardinality check is sufficient to guarantee exhaustivity.
    if seen_tags.len() != enum_def.variants.len() {
...
fn pack_enum_variant(
    ...
    let field_sig = variant_def
        .fields
        .iter()
        .map(|field_def| instantiate(&field_def.0, type_args));
...
fn materialize_type(
    struct_handle: DeclaredTypeHandleIndex,
    type_args: &Signature,
) -> SignatureToken {
    if type_args.is_empty() {
        ST::DeclaredType(struct_handle)
    } else {
        ST::DeclaredTypeInstantiation(struct_handle, type_args.0.clone())
    }
}
...
fn instantiate(token: &SignatureToken, subst: &Signature) -> SignatureToken {
    ...
    match token {
        ...
        DeclaredType(idx) => DeclaredType(*idx),
        DeclaredTypeInstantiation(idx, struct_type_args) => DeclaredTypeInstantiation(
            *idx,
            struct_type_args
                .iter()
