...
fn exp_list_items_to_vec(
    ...
    // Because we previously froze subpoints of _all_ ExpList forms as a hoisted binding, we need
    // to recreate that behavior to avoid shifting gas costs and generated bytecode. We do that
    // here, sometimes hoisting the vector to a `Multiple` and then yielding the results back.
    let final_es = if let Some(expected_ty @ sp!(tloc, HT::Multiple(_))) = ty {
...
fn exp_impl(
    ...
    let res = match e_ {
        ...
        TE::ExpList(items) => HE::Multiple(exp_list_items_to_vec(context, result, None, eloc, items)),
        TE::Borrow(mut_, te, f) => {
            let e = exp(context, result, None, *te);
