    ...
    fn instruction_effect(&self, instruction: &Bytecode) -> PartialVMResult<(u64, u64)> {
        Ok(match instruction {
            ...
            Bytecode::PackVariant(vidx) => {
                let handle = self.resolver.variant_handle_at(*vidx)?;
                let variant_definition = self.resolver.variant_def_at(handle.enum_def, handle.variant)?;
            ...
            Bytecode::PackVariantGeneric(vidx) => {
                ...
                let enum_def_instantiation = self.resolver.enum_instantiation_at(handle.enum_def)?;
