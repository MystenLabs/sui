...
impl<S: MoveResolver> DataStore for TransactionDataCache<S> {
    ...
    fn load_package(&self, _package_id: &AccountAddress) -> VMResult<Vec<Vec<u8>>> {
        unreachable!("TransactionDataCache::load_package should never be called in v2")
    }
    ...
    fn all_package_dependencies(&self) -> VMResult<BTreeSet<AccountAddress>> {
        unreachable!("TransactionDataCache::all_package_dependencies should never be called in v2")
    }
}
