...
impl Frame {
    ...
    fn execute_instruction(
        ...
        match instruction {
            ...
            Bytecode::MutBorrowGlobal(sd_idx) | Bytecode::ImmBorrowGlobal(sd_idx) => {
                ...
                interpreter.borrow_global(data_store, addr, &ty)?;
            }
            ...
            Bytecode::Exists(sd_idx) => {
                let addr = interpreter.operand_stack.pop_as::<AccountAddress>()?;
                let ty = resolver.get_struct_type(*sd_idx);
                interpreter.exists(data_store, addr, &ty)?;
            }
            ...
            Bytecode::MoveFrom(sd_idx) => {
                let addr = interpreter.operand_stack.pop_as::<AccountAddress>()?;
                let ty = resolver.get_struct_type(*sd_idx);
                interpreter.move_from(data_store, addr, &ty)?;
            }
            ...
            Bytecode::MoveTo(sd_idx) => {
                ...
                interpreter.move_to(data_store, addr, &ty, resource)?;
            }
    ...
    fn pre_charge_gas(
        ...
        match instruction {
            Bytecode::Pop => {
                let popped_val = interpreter.peek()?;
            ...
            Bytecode::Pack(sd_idx) => {
                ...
                Self::check_depth_of_type(resolver, &struct_type)?;
                gas_meter
                    .charge_pack(false, interpreter.peek_last_n(field_count as usize)?.iter())?;
            }
            Bytecode::PackGeneric(si_idx) => {
                let field_count = resolver.field_instantiation_count(*si_idx);
                let ty = resolver.instantiate_generic_type(*si_idx, ty_args)?;
            ...
            Bytecode::Eq => {
                let values = interpreter.peek_last_n(2)?;
                let lhs = &values[0];
                let rhs = &values[1];
            ...
            Bytecode::VecPack(si, num) => {
                ...
                gas_meter.charge_vec_pack(
                    make_ty!(&ty),
                    interpreter.peek_last_n(*num as usize)?.iter(),
                )?;
            }
            Bytecode::VecLen(si) => {
                let ty = &resolver.instantiate_single_type(*si, ty_args)?;
