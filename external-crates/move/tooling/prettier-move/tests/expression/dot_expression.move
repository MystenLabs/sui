...
fun dot_name() {
    ...
    &board.elements.something.else.and_then;

    // in assignment we want to try and keep
    // the dot expression on the same line as
    // the `=` sign
    *some_identifier.get_mut(a, b).next = next;
...
fun dot_expression() {
    ...
    board.add_element(Element { id: object::new(ctx) });

    // same as above, but with an arguments list
    board.add_fields(field_one, field_two, field_three);
...
fun dot_with_lambda_lists() {
    ...
    vector.length().do_range!(num, |el| {
        el.destroy_empty();
    });

    // should break and indent correctly
    option.map!(|e| option::some(10)).destroy_or!({
        let _ = 1;
        1 + 2
    });
