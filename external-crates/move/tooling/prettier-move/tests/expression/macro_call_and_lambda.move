...
fun f() {
    animation.do_ref!(|el| {
        contents = contents
            ...
            .or!(vector[el.to_string()]);
    });

    stack.other!(argument, |x| x.do!());
    stack.another!(argument, argument, argument, argument, |x| {
        x.do!();
    });
    stack.another!(argument, argument, argument, |x| x.destroy!(|_| {}));
    ...
    stack.very_very_very_long_macro_name_with_a_cherry!(|x| {
        x.do!();
    });
    ...
    stack.do!(|el| {
        ...
        if (el.is_some()) {
            field::borrow_mut<K, Node<K, V>>(&mut table.id, *el.borrow()).next = next;
        };
    });
    ...
    idx_sorted.find_index!(
            |idx| {
                let (el, value) = set.elems.get_entry_by_idx(*idx as u64);
                expected > *value
            },
        );


    aha.idx_sorted.find_index!(
            |idx| {
                let (el, value) = set.elems.get_entry_by_idx(*idx as u64);
                expected > *value
            },
    ...
    let insert_idx = set.idx_sorted.find_index!(|idx| {
        let (_elem, value) = set.elements.get_entry_by_idx(*idx as u64);
        total_value > *value
