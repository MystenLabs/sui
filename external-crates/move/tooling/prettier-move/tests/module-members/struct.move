...
public struct Struct2 has key { id: UID, field: String }

// struct will first try to break on fields
// and then break on type arguments
public struct Struct3<phantom T> { field: String }
...
// not on abilities
public struct Point(u64, u64) has key, store, drop;
...
// tries on fields
public struct Container<phantom T>(u64, T)

// will break on type arguments, not fields
public struct Container2<phantom T, phantom C>(u64, T)
