...
pub struct ClientState<AuthorityClient> {
    ...
    /// Persistent storage for Client state
    store: Arc<client_store::ClientStore>,
}
...
impl<A> ClientState<A> {
    pub fn new(
        ...
        // TODO: Use deterministic file path
        let path = env::temp_dir().join(format!("CLIENT_DB_{:?}", ObjectID::random()));
        ...
        let client_state = Self {
            ...
            committee,
            authority_clients,
            store: Arc::new(client_store::ClientStore::open(path, None)),
        };

        // Backfill the DB
        client_state.store.clear_pending_transfer().unwrap();
        for (id, ref_) in object_refs {
            client_state.store.insert_object_ref(&id, &ref_).unwrap();
            client_state
                .store
                .insert_sequence_number(&id, &ref_.1)
                .unwrap();
        }
        for (digest, cert_order) in certificates {
            client_state
                .store
                .insert_certified_order(&digest, &cert_order)
                .unwrap();
        }
    ...
    pub fn insert_object(
        ...
        self.store.insert_sequence_number(object_id, seq)?;
        self.store.insert_tx_digest(object_id, digest)?;
        self.store.insert_object_ref(object_id, object_ref)?;
        Ok(())
    }
    ...
    pub fn remove_object(&mut self, object_id: &ObjectID) -> Result<(), FastPayError> {
        ...
        self.store.remove_object_ref(object_id)?;
        Ok(())
    }
}
    ...
    A: AuthorityClient + Send + Sync + 'static + Clone,
{
    ...
    fn update_certificates(
        ...
        for new_cert in certificates {
            ...
            self.store
                .insert_certified_order(&new_cert.order.digest(), &new_cert.clone())?;
    ...
    fn update_objects_from_order_info(
        ...
        if let Some(v) = order_info_resp.signed_effects {
            ...
            self.store.insert_certified_order(&digest, &cert)?;

            for &(object_ref, owner) in v.effects.all_mutated() {
                let (object_id, seq, _) = object_ref;
                let old_seq = self
                    .store
                    .get_sequence_number(object_id)?
                    .unwrap_or_default();
    ...
    async fn receive_object(&mut self, certificate: &CertifiedOrder) -> Result<(), anyhow::Error> {
        ...
        match &certificate.order.kind {
            OrderKind::Transfer(transfer) => {
                ...
                {
                    self.store.insert_sequence_number(
