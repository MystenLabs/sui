...
let eventPackages = [
	'0x2d6733a32e957430324196dc5d786d7c839f3c7bbfd92b83c469448b988413b1',
	'0x2',
	'0x3',
];
...
let senders = [
	'0x18889a72b0cd8072196ee4cf269a16d8e559c69088f7488ecb76de002f088010', // from last 0x2
	'0x8eab656650ded2b5e1a2577ad102202595a361b375ba953769c192f30d59fc4c',
];
let emittingPackages = [
...
async function eventsSuite(client: SuiGraphQLClient<typeof queries>) {
	...
	for (let filter of generateFilters(
		eventPackages,
		eventModules,
		eventTypes,
		senders,
		...
		for (let sender of senders) {
			for (let emittingModule of emitModules(emittingPackages, emittingModules)) {
				await events(
					...
					{ filter: { eventType, sender, emittingModule } },
				);
				await events(
					client,
					{ paginateForwards: false, limit, numPages },
					{ filter: { eventType, sender, emittingModule } },
				);
			}
		}
