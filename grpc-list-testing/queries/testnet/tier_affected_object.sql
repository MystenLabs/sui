WITH dense AS (
  ...
  FROM (SELECT APPROX_TOP_K(OBJECT_ID, 10, 100000) topk
        FROM CHAINDATA_TESTNET.TRANSACTION_OBJECT
        WHERE CHECKPOINT < 344000000 AND OBJECT_STATUS IN ('Mutated','Created','Deleted','Wrapped','Unwrapped')),
       LATERAL FLATTEN(input => topk) f
),
recent_window AS (
  SELECT 'recent_only' tier, OBJECT_ID v, COUNT(*) cnt
  FROM CHAINDATA_TESTNET.TRANSACTION_OBJECT
  WHERE CHECKPOINT BETWEEN 334000000 AND 342000000 AND OBJECT_STATUS IN ('Mutated','Created','Deleted','Wrapped','Unwrapped')
  GROUP BY OBJECT_ID HAVING COUNT(*) > 200 ORDER BY COUNT(*) DESC LIMIT 3
),
sparse AS (
  SELECT 'sparse' tier, OBJECT_ID v, COUNT(*) cnt
  FROM CHAINDATA_TESTNET.TRANSACTION_OBJECT
  WHERE CHECKPOINT BETWEEN 343000000 AND 344000000 AND OBJECT_STATUS IN ('Mutated','Created','Deleted','Wrapped','Unwrapped')
