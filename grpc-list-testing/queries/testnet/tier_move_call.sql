WITH agg AS (
  SELECT PACKAGE || '::' || MODULE || '::' || FUNCTION_ v, COUNT(*) cnt, MIN(CHECKPOINT) lo, MAX(CHECKPOINT) hi
  FROM CHAINDATA_TESTNET.MOVE_CALL
  WHERE CHECKPOINT < 344000000 AND PACKAGE IS NOT NULL AND MODULE IS NOT NULL AND FUNCTION_ IS NOT NULL
  GROUP BY 1
),
tiered AS (
  SELECT v, cnt, lo, hi,
