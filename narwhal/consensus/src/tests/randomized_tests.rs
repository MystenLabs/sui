...
async fn bullshark_randomised_tests() {
    ...
    const GC_DEPTH: RangeInclusive<Round> = 7..=15;
...
pub fn make_certificates_with_parameters(
    ...
    for round in range {
        ...
        for name in keys.iter() {
            ...
            let mut my_parent_digest = None;

            // Move "our" previous round certificate to the front
            if let Some(my_previous_round) =
                current_parents.iter().find(|c| c.header.author == *name)
            {
                parent_digests.remove(&my_previous_round.digest());
                my_parent_digest = Some(my_previous_round.digest());
            }
...
fn generate_and_run_execution_plans(
    ...
    for i in 0..test_iterations {
        ...
        let mut bullshark = Bullshark::new(committee.clone(), store.clone(), gc_depth, metrics);
        ...
        for c in plan.certificates {
            ...
            let committed_sub_dags = bullshark.process_certificate(&mut state, c).unwrap();
