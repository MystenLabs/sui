...
use types::{
    ...
    ShutdownNotification, WorkerOthersBatchMessage, WorkerOurBatchMessage, WorkerShutdownMessage,
...
fn get_registry() -> Result<Registry> {
    ...
    let run = WorkerShutdownMessage {
        message: ShutdownNotification::Run,
    };
    let shutdown = WorkerShutdownMessage {
        message: ShutdownNotification::Shutdown,
    };
    tracer.trace_value(&mut samples, &our_batch)?;
    tracer.trace_value(&mut samples, &others_batch)?;
    tracer.trace_value(&mut samples, &sync)?;
    tracer.trace_value(&mut samples, &run)?;
