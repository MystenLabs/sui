...
impl Proposer {
    ...
    async fn create_new_header(&mut self) -> DagResult<Header> {
        ...
        let included_digests: VecDeque<_> = self.digests.drain(..num_of_digests).collect();
            ...
            tokio::select! {
                ...
                Some((commit_round, commit_headers)) = self.rx_committed_own_headers.recv() => {
                    ...
                    for round in commit_headers {
                        let (_header, included_digests) = self.proposed_headers.remove(&round).expect("Committed header not found!");
