...
impl Inner {
        ...
        if certificate.round() > self.gc_round.load(Ordering::Acquire) + 1 {
            for digest in &certificate.header.ancestor_digests() {
                if !self.certificate_store.contains(digest).unwrap() {
                    panic!("Ancestor {digest:?} not found for {certificate:?}!");
                }
            }
        }
    ...
    async fn get_missing_ancestors(
        ...
        for (round, digest) in certificate.header.ancestors() {
            if round <= self.gc_round.load(Ordering::Acquire) {
                continue;
            }
            if !self.has_processed_certificate(digest).await? {
                result.push((round, digest));
            }
        }
        if !result.is_empty() {
...
impl Synchronizer {
    ...
    /// Validates the certificate and accepts it into the DAG, if the certificate can be verified
    /// and has all ancestors in the certificate store.
    /// If the certificate has missing ancestors, wait until all ancestors are available to accept
    /// the certificate.
    ...
    /// ancestors that are missing.
    pub fn check_ancestors_for_header(
        &self,
        header: &Header,
        ...
        for ancestor in &header.ancestors {
            let Some((ancestor_round, digest)) = ancestor else {
                continue;
            };
            ...
            match cert {
                Some(certificate) => {
                    if ancestor_round + 1 == header.round {
...
impl State {
    ...
    fn run_gc(&mut self, gc_round: Round) -> Vec<SuspendedCertificate> {
        ...
        // Notify waiters on GC'ed certificates. No need to accept them.
        for gc_certificate in gc_certificates {
            gc_certificate
                .notify
                .notify()
                .expect("Suspended certificate should be notified once.");
