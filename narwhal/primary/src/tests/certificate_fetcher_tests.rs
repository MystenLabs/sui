...
use types::{
    AggregateSignatureState, BatchDigest, Certificate, CertificateAPI, CertificateDigest,
    FetchCertificatesRequest, FetchCertificatesResponse, Header, HeaderAPI, HeaderDigest, Metadata,
    ...
    for _ in 0..20 {
        ...
        for (i, _) in certificates.iter().enumerate() {
            if let Ok(Some(cert)) = certificate_store.read(certificates[i].digest()) {
                match cert.aggregate_signature_state() {
                    AggregateSignatureState::VerifiedDirectly(_) => verified_directly += 1,
                    AggregateSignatureState::VerifiedIndirectly(_) => verified_indirectly += 1,
                    _ => panic!(
...
async fn fetch_certificates_v2_basic() {
    ...
    for cert in certificates.iter().skip(num_written).take(204) {
        ...
        cert.set_aggregate_signature_state(AggregateSignatureState::Unverified(
