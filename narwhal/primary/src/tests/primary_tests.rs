...
use types::{
    now, AggregateSignatureState, Certificate, CertificateAPI, FetchCertificatesRequest, Header,
    HeaderAPI, MockPrimaryToWorker, PreSubscribedBroadcastSender, PrimaryToPrimary,
    RequestVoteRequest,
};
    ...
    for i in 0..total_authorities {
        ...
        for j in 0..=i {
            ...
            if i != 0 {
                ...
                // directly
                cert.set_aggregate_signature_state(AggregateSignatureState::VerifiedDirectly(
                    cert.aggregated_signature().clone(),
                ));
            } else {
                // Simulating some indirectly verified certificates being stored.
                cert.set_aggregate_signature_state(AggregateSignatureState::VerifiedIndirectly(
                    cert.aggregated_signature().clone(),
                ));
            }
            certificate_store
