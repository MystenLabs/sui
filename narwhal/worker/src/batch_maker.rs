...
impl BatchMaker {
    ...
    async fn run(&mut self) {
        info!(
            ...
            self.batch_size_limit, self.max_batch_delay
        );
            ...
            tokio::select! {
                ...
                Some((transaction, response_sender)) = self.rx_batch_maker.recv(), if batch_pipeline.len() < MAX_PARALLEL_BATCH => {
                    ...
                    if current_batch_size >= self.batch_size_limit {
                        ...
                        if let Some(seal) = self.seal(false, current_batch, current_batch_size, current_responses).in_monitored_scope("BatchMaker::transaction::seal").await{
