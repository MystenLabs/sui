...
use types::{
    ...
    RequestBatchResponse, ShutdownNotification, WorkerBatchMessage, WorkerDeleteBatchesMessage,
    WorkerOthersBatchMessage, WorkerShutdownMessage, WorkerSynchronizeMessage, WorkerToWorker,
...
pub struct PrimaryReceiverHandler<V> {
    ...
    pub tx_shutdown: watch::Sender<ShutdownNotification>,
...
impl<V: TransactionValidator> PrimaryToWorker for PrimaryReceiverHandler<V> {
    async fn shutdown(
        ...
        match &message {
            ...
            ShutdownNotification::Shutdown => (), // no-op
        };
