<script lang="ts">
  ...
  const connectToWallet = async () => {
    ...
    if (isweb3Enabled) {
      try {
        const walletAddressList = await window.ethereum.request({
          ...
          <div class="row align-items-center">
            <div class="col-lg-12">
              <div class="banner-content">
                ...
                <p>
                  Dropping your existing NFTs from other networks will let you
                  use them on Sui and unleash new functionality such as fully
                  on-chain dynamic and composable NFTs. Get started by
                  connecting your wallet below!
                </p>
                ...
                {#if isweb3Enabled && !isloading}
                  {#if $walletAddress}
                    <button
                      ...
                      on:click={selectedAddress}>See NFTs</button
                    >
                  {/if}
                  {#if !$walletAddress}
                    <button
                      class="sui-btn btn-fill-white btn-large"
                      on:click={switchMTWalletAddress}
                ...
                {#if !isweb3Enabled}
                  <p class="noWallet">
                    <a href="https://metamask.io/" target="_blank"
                      >(MetaMask not enabled) Please install MetaMask</a
                    >
                  </p>
                {/if}
