...
for pr_number in $(git log --grep "\[x\]" --pretty=oneline --abbrev-commit "${new_branch}"..."${prev_branch}" -- crates dashboards doc docker external-crates kiosk narwhal nre sui-execution | grep -o '#[0-9]\+' | grep -o '[0-9]\+')
do
