...
# 2. File directory is not '$TOPLEVEL/target' or "**/build", "**/node_modules", or "**/doc"
for i in $(find $TOPLEVEL  -type d \( -path '$TOPLEVEL/target' -o -name 'node_modules' -o -name 'build' -o -name 'dist' -o -name 'doc' \) -prune -o \( -iname '*.rs' -o -iname '*.move' -o -iname '*.tsx' -o -iname '*.ts' -o -iname '*.js' \) -print)
do
