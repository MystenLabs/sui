...
# Match all nextest failure statuses: FAIL, TIMEOUT, and signal-based deaths
# (SIGABRT, SIGTERM, SIGSEGV, SIGILL, SIGBUS, etc.)
FAILURE_PATTERN='TIMEOUT|FAIL|SIG[A-Z]+'

grep -EqHn "$FAILURE_PATTERN" "$LOG_DIR"/*
...
# read all filenames in $LOG_DIR that contain a failure marker into a bash array
# and print the line number and filename for each
readarray -t FAILED_LOG_FILES < <(grep -El "$FAILURE_PATTERN" "$LOG_DIR"/*)
