...
const hex = bcs.optionEnum(Coin).serialize({ some: coin }).toHex();
...
const optionEnum = bcs.optionEnum(bcs.u8()).serialize({ Some: 100 }).toBytes();
const none = bcs.optionEnum(bcs.u8()).serialize({ None: null }).toBytes();
...
```ts
import { bcs, fromHex, fromB64, fromB58 } from '@mysten/bcs';
...
// To parse a BCS from bytes, the bytes need to be a Uint8Array
