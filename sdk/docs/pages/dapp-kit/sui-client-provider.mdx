...
the `createNetworkConfigs` function returns the provided config, along with a `useNetworkConfig`
hook you can use to get the config for the currently active network.

```tsx
import { createNetworkConfigs, SuiClientProvider, useSuiClientContext } from '@mysten/dapp-kit';
...
function YourApp() {
	const { myMovePackageId } = useSuiClientContext();

	return <div>Package ID: {myMovePackageId}</div>;
}
```
