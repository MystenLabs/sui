...
- `connectionStatus`
  ...
  - 'reconnecting' - When a wallet connection attempt to the last connected wallet account is in
    progress. This is triggered when the `autoConnect` property is set on the `WalletProvider`
    component.
...
- `isReconnecting` - A derived boolean from the status variable above, provided for convenience.
