...
`useSignTransactionBlock` hook provides a `reportTransactionEffects` callback to report effects
manually, so this hook is only needed when using a non-standard flow for executing transactions.
