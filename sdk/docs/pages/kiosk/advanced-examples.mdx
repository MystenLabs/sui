...
Let's assume we have the following data & functions available.
...
Let's see how we could mint a suifren
...
async function mintFren(address: string) {
	...
	// We wrap it anyways, to support both existing kiosk & fresh kiosk flows.
	kioskTx.wrap();
...
Let's see how we could use the Kiosk SDK to mix two `bullsharks`.
