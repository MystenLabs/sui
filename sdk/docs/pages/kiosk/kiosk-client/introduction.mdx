...
> It is recommended to keep only one instance of `KioskClient` throughout your dApp or script. For
...
In order to use all the functionality of `Kiosk SDK` outside of `MAINNET` and `TESTNET`, you need to
use `Network.CUSTOM` as the network, and pass the `packageIds` for the rules and extensions you want
to use.
