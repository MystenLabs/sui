...
Removes an item from the Kiosk and returns a `TransactionArgument` to use it in a different PTB
call.
...
// do something with `item`, e.g. transfer it to someone else.
...
List an item for sale by first placing it in the kiosk (places the item & lists it for sale). It's a
short hand for `place()` and `list()`
...
Similar to `borrowTx`, borrows an item from the kiosk, but returns 2 transaction arguments: `item` &
