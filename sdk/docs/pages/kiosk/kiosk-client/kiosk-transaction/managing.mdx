...
Removes an item from the Kiosk and returns a `TransactionArgument` to use it in a different Programmable Transaction Block (PTB)
call.
...
List an item for sale by first placing it in the kiosk (places the item and lists it for sale). It's a
