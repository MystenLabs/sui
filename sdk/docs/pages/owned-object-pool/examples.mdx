...
for (let i = 0; i < 10; i++) {
  ...
  promises.push(
    eshandler.execute(
      txb, client, new DefaultSplitStrategy(300000000) // Each pool contains coins with a total balance of 300000000 MIST
    ));
}
...
To do this, you can either use the `createAdminCap` function in your smart contract, or generate multiple admin caps on package publishing. You can test this by using the [`move_examples/nft_app` example](https://github.com/MystenLabs/Sui_Owned_Object_Pools/tree/main/move_examples/nft_app). Notice how multiple `AdminCaps` are created in [`genesis.move`](https://github.com/MystenLabs/Sui_Owned_Object_Pools/blob/main/move_examples/nft_app/sources/genesis.move):
...
Now that you have the coins and the admin caps, you can define the `mintNFTTxb` function that creates and returns the transaction block of type `TransactionBlockWithLambda`.
