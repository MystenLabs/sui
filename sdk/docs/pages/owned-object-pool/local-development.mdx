...
are also a great way to get familiar with the library. 
...
1. Have an account to use as an Admin account to create and execute transactions,
...
Also, make sure that your admin account (the account `sui client active-address` identifies) owns enough SUI.
You can get SUI for a test network from the [Sui faucet](https://docs.sui.io/guides/developer/getting-started/get-coins).
...
add an `ADMIN_SECRET_KEY` and a `TEST_USER_SECRET` to the `.env`.
