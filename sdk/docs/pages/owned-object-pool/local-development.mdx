...
ADD A MORE DETAILED INSTALLATION GUIDE. HOW WOULD A USER INSTALL IF THEY WERE STARTING FROM SCRATCH?
CLONE PROJECT, INSTALL SUI, TS SDK, ETC. USE LINKS TO MAIN DOC SITE INSTEAD OF RECREATING STEPS IF
APPLICABLE.
...
Tests are a great way to get familiar with the library. For each test scenario, there is a small
description of the test's purpose and the library's commands to achieve that.
...
Usually we use the testnet network for testing. Switch to testnet with: sui client switch --env
testnet
