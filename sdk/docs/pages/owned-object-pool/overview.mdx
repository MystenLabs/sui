...
Notice the use of `TransactionBlockWithLambda()` instead of `TransactionBlock()`. The `TransactionBlockWithLambda` function is a more flexible way of defining transaction blocks. What differs is that the transaction block will be created later, just before the transaction execution is done by a worker pool.
