...
The Sui TypeScript SDK provides `Keypair` classes that handle logic for signing and verification using the cryptographic key pairs
associated with a Sui address. 
...
| ECDSA Secp256k1 | `Secp256k1Keypair` | `@mysten/sui.js/keypairs/ed25519` |
| ECDSA Secp256r1 | `Secp256r1Keypair` | `@mysten/sui.js/keypairs/ed25519` | 

For information on these schemes, see the [Sui Wallet Specifications](https://docs.sui.io/testnet/learn/cryptography/sui-wallet-specs) topic.

To use, import the key pair class your project uses from the `@mysten/sui.js/keypairs` folder. For example, to use the Ed25519 scheme, import the `Ed25519Keypair` class from `@mysten/sui.js/keypairs/ed25519`.
...
With your key pair created, you can use it to perform actions on the network. For example, you can use it to sign transactions, like the following code that creates and signs a personal message using the public key from the key pair created in the previous code:
...
Verifying transaction blocks is similar to signature verification, except you use `verifyTransactionBlock`:
