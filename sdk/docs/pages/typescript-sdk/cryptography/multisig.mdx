...
The Sui TypeScript SDK provides a `MultiSigPublicKey` class to support [Multi-Signature](https://docs.sui.io/learn/cryptography/sui-multisig) (MultiSig) transaction signing.

This class implements the same interface as the `PublicKey` classes that [Keypairs](./keypairs) uses and you call the same methods to verify signatures for `PersonalMessages` and `TransactionBlocks`.
