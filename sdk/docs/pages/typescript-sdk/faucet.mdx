...
```
Use `requestSuiFromFaucetV0` in your TypeScript code to request SUI from the network's faucet. The `getFaucetHost` function is useful for development, but you should use the actual [Mainnet RPC address](../index#network-locations) as the `host` parameter value in production apps. 
