...
`Keypair`s implement an `export` function that can be used to export a keypair in a standard format.
The return value is an object with a `scheme` property that indicates what the type of keypair was
exported, and a `privateKey` that you can use to derive the `Keypair` again.
