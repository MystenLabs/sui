...
Some APIs also accept an `order` option that be set to either `ascending` or `descending` to change
the order in which the results are returned.

The `nextCursor` can be passed to the `cursor` option of the RPC method to retrieve the next page,
