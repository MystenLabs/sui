...
Transaction Intents allow SDKs and [Transaction Plugins](../plugins) allow complex operations to
more easily be added to a Transaction. The Typescript SDK currently only includes a single Intent
...
generally been done by splitting off the gas coin:
...
Using gas for splitting off SUI also causes problems for sponsored transactions. When sponsoring
transactions, the gas coin comes from the sponsor instead of the transaction sender. Transaction
sponsors usually not sponsor transactions that use the gas coin for anything other than gas. To
