...
`zksend_address` query parameter to the URL, containing the Sui address of the user that claimed the
...
const { nfts, balances } = await link.listClaimableAssets(addressOfClaimer);
...
// links will be an array of ZkSendLinks, but won't be claimable since they don't have access to the links private key
const { nfts, balances } = await links[0].listClaimableAssets(addressOfClaimer);
```
...
const { url, transactionBlock } = await links[0].createRegenerateTransaction(addressOfLinkCreator);
