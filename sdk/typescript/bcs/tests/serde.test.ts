...
describe('serde', () => {
  it('should serialize primitives in both directions', () => {
    ...
    assert(
      u8.toString(10) ===
        bcs.de(bcs.U8, bcs.ser(bcs.U8, u8).toBytes()).toString(10)
    );
    assert(
      u64 === bcs.de(bcs.U64, bcs.ser(bcs.U64, u64).toBytes()).toString(10)
    );
    assert(
      u128 === bcs.de(bcs.U128, bcs.ser(bcs.U128, u128).toBytes()).toString(10)
    );
  ...
  it('should serde enums', () => {
    ...
    assert(
      addr ===
        bcs.de('Enum', bcs.ser('Enum', { with_value: addr }).toBytes())
          .with_value
    );
    assert(
      'no_value' in
