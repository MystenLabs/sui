...
import { SuiObjectRef, MoveCallTx, PublishTx, TransactionData } from "./types";
...
export class SuiClient {
     ...
     */
    public async myObjectsByType(address: string): Promise<{ [key: string]: SuiObjectRef[] }> {
    ...
    public async gasBalance(): Promise<number> {
        ...
        return this.rpc.fetchObj(gas.objectId).then((res) => {
            if (res.status === 'Exists') {
                ...
                throw new Error(`Gas Object not found: ${gas.objectId}! \n${JSON.stringify(res)}`);
            }
        });
    }
    ...
    public async send(
        ...
        let preparedTx = this.signTx(keypair, {
            kind: { Single: tx },
        ...
        (result.deleted) && Object.keys(result.deleted).forEach((key) => this.objects.delete(key));
    ...
    gasOrFail(): SuiObjectRef {
        if (this.gasRef === null) {
            throw new Error(`To send transactions, please provide gas with sui.setGas(id) method`);
        }
     ...
     */
    public signTx(pair: KeyPair, tx: TransactionData, size: number = 2048): SignedTx {
