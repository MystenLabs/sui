...
(function (module, exports) {
    ...
    // On NodeJS use require() or use window.BN in browser;
    // const BN = typeof require === 'function' && require('bn.js') || window;

    var BN;
    try {
        if (typeof window !== 'undefined' && typeof window.BN !== 'undefined') {
            BN = window.BN;
        } else {
            BN = require('bn.js');
        }
    ...
    if (typeof BN === 'undefined') {
        throw new Error('MoveBCS requires BN.js');
    }
     ...
     */
    MoveBCS.BcsReader = class BcsReader {
        /**
         ...
         */
        read64() {
            let value1 = this.read32(this.bytePosition);
            let value2 = this.read32(this.bytePosition);
     ...
     * @callback ReadVecCb
     * @param {BcsReader} reader A Reader instance to call reader functions.
     * @param {Number} i An index of the element read.
     ...
     *   .hex();
     */
    MoveBCS.BcsWriter = class BcsWriter {
        /**
         * @param {Number} [size=1024] Size of the buffer to reserve for serialization.
         */
        constructor(size = 1024) {
            Object.assign(this, {
                dataView: new DataView(new ArrayBuffer(size)),
                bytePosition: 0
            });
        }
        /**
    ...
    // define constants 
    MoveBCS.types = new Map();
    MoveBCS.U8 = 'u8';
    MoveBCS.U32 = 'u32';
    MoveBCS.U64 = 'u64';
    MoveBCS.U128 = 'u128';
    MoveBCS.BOOL = 'bool';
    MoveBCS.VECTOR = 'vector';
    MoveBCS.ADDRESS = 'address';
    MoveBCS.STRING = 'string';

    MoveBCS.hasType = function hasType(type) {
        return MoveBCS.types.has(type);
    };
     ...
     */
    MoveBCS.registerType = function registerType(
        name, 
        encodeCb, 
        decodeCb, 
        validateCb = () => true
        ...
        this.types.set(name, {
            encode(data, size = 1024) { return this._encodeRaw(new MoveBCS.BcsWriter(size), data); },
            decode(data) { return this._decodeRaw(new MoveBCS.BcsReader(data)); },
     ...
     */
    MoveBCS.registerVectorType = function registerVectorType(
        name,
        innerType
    ) {
        if (!this.hasType(innerType)) {
        ...
        this.types.set(name, {
            ...
            _encodeRaw(writer, data) {
                return writer.writeVec(data, (writer, el) => MoveBCS.types.get(innerType)._encodeRaw(writer, el));
            },
            _decodeRaw(reader) {
                return reader.readVec((reader) => MoveBCS.types.get(innerType)._decodeRaw(reader));
            },
        });
     ...
     */
    MoveBCS.registerStructType = function registerStructType(name, fields = {}) {
        ...
        for (let type of Object.values(struct)) {
            if (!type.constructor === String) {
                throw new Error(`Types must be specified as strings, got ${type}`);
            }
    ...
    !function registerPrimitives() {
        ...
        MoveBCS.registerType(
            ...
            (u64) => (u64.constructor === BN || u64.constructor === String)
        );
        ...
        MoveBCS.registerType(
            ...
            (u128) => (u128.constructor === BN || u128.constructor === String)
        );

        MoveBCS.registerType(
            MoveBCS.BOOL,
            (writer, data) => writer.write8(data),
            (reader) => reader.read8() == '1',
            (bool) => (bool.constructor === Boolean)
        );
        ...
        MoveBCS.registerType(
            ...
            (str) => (str.constructor === String) && /^[\x00-\x7F]*$/.test(str)
        )
    }();
     ...
     */
    MoveBCS.de = function deserialize(type, data) {
        if (!this.hasType(type)) {
            throw new Error(`No implementation found for type ${type}`);
        }

        return this.types.get(type).decode(data);
    };
     ...
     */
    MoveBCS.ser = function serialize(type, data, size = 1024) {
        if (!this.hasType(type)) {
            throw new Error(`No implementation found for type ${type}`);
        }

        return this.types.get(type).encode(data, size);
    };
     ...
     */
    MoveBCS.util.fromHex = function fromHex(hexString) {
     ...
     */
    MoveBCS.util.toHex = function toHex(buffer) {
        return [...buffer]
            .map((b) => b.toString(16).padStart(2, '0'))
            .join("");
    };

    Object.freeze(MoveBCS.util);

    if (typeof module === 'object') {
        module.exports = MoveBCS;
    } else {
        exports.MoveBCS = MoveBCS;   
    }
