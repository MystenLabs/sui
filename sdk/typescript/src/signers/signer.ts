...
export abstract class Signer {
  ...
  async signAndExecuteTransaction(
    ...
    this.checkProvider('signAndExecuteTransaction');
  ...
  async transfer(
    ...
    this.checkProviderAndSerializer('transfer');
  ...
  checkProviderAndSerializer(operation?: string): void {
    this.checkProvider(operation);
    this.checkSerializer(operation);
  }

  checkProvider(operation?: string): void {
    if (!this.provider) {
      throw new Error(`missing provider for ${operation || 'checkProvider'}`);
    }
  }

  checkSerializer(operation?: string): void {
    if (!this.serializer) {
      throw new Error(
        `missing serializer for ${operation || 'checkSerializer'}`
      );
    }
  }
}
