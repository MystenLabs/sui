...
//Test that with invalid object id/address/digest, functions will throw an error before making a request to the rpc server
  it('Test all functions with invalid Sui Address', async () => {
    //empty id
    expect(toolbox.provider.getObjectsOwnedByAddress('')).rejects.toThrowError(/Invalid Sui address/);
    
    //wrong id
    expect(toolbox.provider.getTransactionsForAddress('Wrong')).rejects.toThrowError(/Invalid Sui address/);
  })
  ...
  it('Test all functions with invalid Object Id', async () => {
    ...
    expect(toolbox.provider.getObjectBatch(objectIds)).rejects.toThrowError(/Invalid Sui Object id 0xWRONG/);;
  })
