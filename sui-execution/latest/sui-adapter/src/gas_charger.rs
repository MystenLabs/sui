    ...
    impl GasCharger {
        ...
        pub fn charge_gas<T>(
            ...
            if let Some(payer_address) = self.address_balance_payer {
                ...
                if gas_used > 0 {
                    let gas_used_u64 = gas_used.try_into().unwrap_or(0u64);
                    ...
                    let accumulator_write = AccumulatorWriteV1 {
                        ...
                        operation: AccumulatorOperation::Split, // Split = withdraw from balance
                        value: AccumulatorValue::Integer(gas_used_u64),
                    };
