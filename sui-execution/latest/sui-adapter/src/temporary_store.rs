...
impl<'backing> TemporaryStore<'backing> {
    ...
    /// Validates free tier post-execution invariants:
    ...
    pub fn check_free_tier_execution_requirements(&self) -> Result<(), String> {
        if !self.execution_results.written_objects.is_empty() {
            return Err("Free tier transactions cannot create or mutate objects".to_string());
        }
        ...
        if self.execution_results.deleted_object_ids != input_coin_ids {
            return Err(format!(
                "Free tier transaction must destroy exactly its input Coins. \
