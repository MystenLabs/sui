...
pub fn add_child_object(
    ...
    {
        let mut object_runtime: RefMut<'_, ObjectRuntime<'_>> = context
            .extensions_mut()
...
pub fn borrow_child_object(
    ...
    let mut object_runtime_mut: RefMut<'_, ObjectRuntime<'_>> = object_runtime.get_mut();
...
pub fn has_child_object(
    ...
    let parent = pop_arg!(args, AccountAddress).into();
    let has_child = {
        let mut object_runtime: RefMut<'_, ObjectRuntime<'_>> = context.extensions().get::<NativeContextMut<ObjectRuntime>>().get_mut();
