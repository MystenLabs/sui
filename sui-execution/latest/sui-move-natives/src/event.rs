...
pub fn emit_authenticated_impl(
    ...
    // This type is always sui::event::EventStreamDigest
...
struct StreamRef {
    ...
    stream_id: Value,
    // The type of the stream head. Should always be `sui::event::EventStreamDigest`
    stream_head_ty: Type,
}
...
fn emit_impl(
    ...
    native_charge_gas_early_exit!(
        context,
        event_emit_cost_params.event_emit_output_cost_per_byte * u64::from(tag_size + event_value_size).into()
    );
    ...
    {
        let prev_total = obj_runtime.state.total_events_size();
        let total_events_size = prev_total + ev_size;
    ...
    Ok(NativeResult::ok(gas_used, smallvec![]))
}
