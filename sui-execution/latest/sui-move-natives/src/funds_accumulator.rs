...
// already recorded the retry in `retry_request`; this abort makes execution fail and produce
// (failure) effects normally, and the authority then sees the recorded retry, discards those
// effects, and re-enqueues — so this abort code is never committed. It exists only to give error
// conversion a valid abort code rather than a bare `ABORTED`.
...
pub fn check_sufficient_object_funds(
    ...
    match obj_runtime.check_object_funds_sufficiency(owner, &ty_tag, limit)? {
        ...
        // Not ready yet: the temporary store has already recorded the retry in `retry_request`.
        // Abort to unwind the VM and fail the transaction; this produces (failure) effects normally,
        // but the authority then sees `inner_temp_store.retry_request` is set, discards those
        // effects, and re-enqueues, so this abort code is never committed. It exists only to give
        // error conversion a valid abort code.
