...
pub struct RuntimeResults {
    ...
    // modified child objects and their loaded version/digest.
    pub modified_child_objects: BTreeMap<ObjectID, (SequenceNumber, ObjectDigest)>,
...
impl ObjectRuntimeState {
    ...
    pub(crate) fn finish(
        ...
        let mut modified_child_objects = BTreeMap::new();
        for (child, child_object_effect) in child_object_effects {
            ...
            let loaded_child = loaded_child_objects.get(&child);
            if let Some(metadata) = loaded_child {
                modified_child_objects.insert(child, (metadata.version, metadata.digest));
            }
            ...
            match effect {
                ...
                Op::Modify(v) => {
                    ...
                    debug_assert!(loaded_child.is_some());
                    self.transfers
        ...
        let written_objects: LinkedHashMap<_, _> = transfers
            ...
            .map(|(id, (owner, type_, value))| {
                if let Some(metadata) = loaded_child_objects.get(&id) {
                    modified_child_objects.insert(id, (metadata.version, metadata.digest));
                }
