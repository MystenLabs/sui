...
pub(crate) struct Executor {
    ...
    sptb_runtime_for_timings: Arc<move_vm_runtime_replay_cut::move_vm::MoveVM>,
}
...
impl Executor {
    pub(crate) fn new(protocol_config: &ProtocolConfig, silent: bool) -> Result<Self, SuiError> {
        ...
        let sptb_runtime_for_timings = Arc::new(sui_adapter_replay_cut::adapter::new_move_vm(
            sui_move_natives_replay_cut::all_natives(silent, protocol_config),
            protocol_config,
        )?);
        Ok(Executor {
...
impl executor::Executor for Executor {
    fn execute_transaction_to_effects(
        ...
        if protocol_config.enable_ptb_execution_v2() && !COLLECT_TIMINGS {
            let new_gas_status = gas_status.clone();
            let new_vm_results = execute_transaction_to_effects::<execution_mode::Normal>(
                store,
                input_objects,
                gas,
                new_gas_status,
            ...
            compare_effects(
                (&current_main.0, &current_main.1, &current_main.2.clone()),
                (&new_vm_results.0, &new_vm_results.1, &new_vm_results.2),
            );
        } else {
            ...
            let sptb_results = replay_cut::execution_engine::execute_transaction_to_effects::<
                replay_cut::execution_mode::Normal,
            >(
                store,
                input_objects.clone(),
            ...
            if count - OLD_VAL.load(std::sync::atomic::Ordering::Relaxed) > 5_000 {
                ...
                if TELEM_COUNTER.fetch_add(1, std::sync::atomic::Ordering::Relaxed) % 50 == 0 {
            ...
            compare_effects(
                (&sptb_results.0, &sptb_results.1, &sptb_results.2),
                (&new_vm_results.0, &new_vm_results.1, &new_vm_results.2),
            );
        }
