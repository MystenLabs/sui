...
use sui_types::{
    base_types::{SuiAddress, SystemObjectVersion, TxContext},
    committee::EpochId,
    ...
    fn execute_transaction_to_effects(
        ...
        let (store_out, gas_status_out, effects, timings, result) =
            execute_transaction_to_effects::<execution_mode::Normal>(
                store,
                input_objects,
                sui_adapter_latest::temporary_store::SystemObjectVersionRequirements {
                    accumulator_version: system_object_versions
                        .accumulator_version
                        .map(SystemObjectVersion::Exact),
                },
                gas,
                gas_status,
                transaction_kind,
    ...
    fn dev_inspect_transaction(
        ...
        let (inner_temp_store, gas_status, effects, _timings, result) = if skip_all_checks {
            execute_transaction_to_effects::<execution_mode::DevInspect<true>>(
                store,
                input_objects,
                sui_adapter_latest::temporary_store::SystemObjectVersionRequirements::all_latest(),
                gas,
                gas_status,
                transaction_kind,
                rewritten_inputs,
