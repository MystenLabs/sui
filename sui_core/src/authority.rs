...
impl AuthorityState {
    ...
    async fn process_certificate(
        ...
        let unwrapped_object_ids =
            self.get_unwrapped_object_ids(&temporary_store, &tx_ctx, &gas_object_id)?;
    ...
    fn get_unwrapped_object_ids(
        ...
        let unwrapped: HashSet<_> = temporary_store
            ...
            .filter(|objid| {
                !(ids_generated.contains(objid)
                    || temporary_store.objects().contains_key(objid)
                    || gas_id == objid)
            })
            .collect();
