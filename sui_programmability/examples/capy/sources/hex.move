...
module capy::hex {
    ...
    /// Transform a vector of bytes into a HEX "string".
    public fun to_hex(bytes: vector<u8>): vector<u8> {
        let (i, r, l) = (0, vec::empty(), vec::length(&bytes));
        while (i < l) {
            vec::append(&mut r, *vec::borrow(&HEX, (*vec::borrow(&bytes, i) as u64)));
            i = i + 1;
        };
        r
    }
    ...
    #[test]
    fun test_hex_value() {
        assert!(b"ff" == to_hex(x"ff"), 0);
        assert!(b"fe" == to_hex(x"fe"), 0);
        assert!(b"00" == to_hex(x"00"), 0);
    }
}
