...
module games::drand_random_selection_tests {
    ...
    fun test_play_drand_random_selection() {
        ...
        test_scenario::next_tx(scenario, user3);
        assert!(!test_scenario::has_most_recent_for_address<WinnerTicket>(user3), 1);
        ...
        {
            drand_random_selection::collect_my_winner_ticket(&ticket, game_val, test_scenario::ctx(scenario));
            drand_random_selection::delete_ticket(ticket);
        };

        // Make sure User3 now has a winner ticket for the right game id.
        test_scenario::next_tx(scenario, user3);
        let ticket = test_scenario::take_from_address<WinnerTicket>(scenario, user3);
        assert!(drand_random_selection::get_winner_ticket_game_id(&ticket) == &ticket_game_id, 1);
