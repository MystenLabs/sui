// Copyright (c) Mysten Labs, Inc.
// SPDX-License-Identifier: Apache-2.0

#[test_only]
module games::vdf_based_lottery_tests {
    use sui::test_scenario as ts;
    use sui::clock;
    use games::vdf_based_lottery::{Self, Game, GameWinner, Ticket};

    #[test]
    #[expected_failure(abort_code = games::vdf_based_lottery::ESubmissionPhaseInProgress)]
    fun test_complete_too_early() {
        let user1 = @0x0;

        let mut ts = ts::begin(user1);
        let mut clock = clock::create_for_testing(ts.ctx());

        vdf_based_lottery::create(1000, 1000, &clock, ts.ctx());
        ts.next_tx(user1);
        let mut game = ts.take_shared<Game>();

        // User1 buys a ticket.
        ts.next_tx(user1);
        vdf_based_lottery::participate(&mut game, b"user1 randomness", &clock, ts.ctx());

        // Increment time but still in submission phase
        clock.increment_for_testing(500);

        // User1 tries to complete the lottery too early.
        ts.next_tx(user1);
        vdf_based_lottery::complete(
            &mut game,
            x"",
            x"",
            &clock
        );

        sui::clock::destroy_for_testing(clock);
        ts::return_shared(game);
        ts.end();
    }

    #[test]
    fun test_play_vdf_lottery() {
        let user1 = @0x0;
        let user2 = @0x1;
        let user3 = @0x2;
        let user4 = @0x3;

        let mut ts = ts::begin(user1);
        let mut clock = clock::create_for_testing(ts.ctx());

        vdf_based_lottery::create(1000, 1000, &clock, ts.ctx());
        ts.next_tx(user1);
        let mut game = ts.take_shared<Game>();

        // User1 buys a ticket.
        ts.next_tx(user1);
        vdf_based_lottery::participate(&mut game, b"user1 randomness", &clock, ts.ctx());
        // User2 buys a ticket.
        ts.next_tx(user2);
        vdf_based_lottery::participate(&mut game, b"user2 randomness", &clock, ts.ctx());
        // User3 buys a ticket
        ts.next_tx(user3);
        vdf_based_lottery::participate(&mut game, b"user3 randomness", &clock, ts.ctx());
        // User4 buys a ticket
        ts.next_tx(user4);
        vdf_based_lottery::participate(&mut game, b"user4 randomness", &clock, ts.ctx());

        // Increment time to after submission phase has ended
        clock::increment_for_testing(&mut clock, 1000);

        // User3 completes by submitting output and proof of the VDF
        ts.next_tx(user3);
        vdf_based_lottery::complete(
            &mut game,
            x"",
            x"",
            &clock
        );

        // User1 is the winner since the mod of the hash results in 0.
        ts.next_tx(user1);
        assert!(!ts::has_most_recent_for_address<GameWinner>(user1), 1);
        let ticket = ts.take_from_address<Ticket>(user1);
        let ticket_game_id = vdf_based_lottery::ticket_game_id(&ticket);
        vdf_based_lottery::redeem(&ticket, &game, ts.ctx());

        // Make sure User1 now has a winner ticket for the right game id.
        ts.next_tx(user1);
        let winner = ts.take_from_address<GameWinner>(user1);
        assert!(vdf_based_lottery::game_winner_game_id(&winner) == ticket_game_id, 1);
        ts::return_to_address(user1, winner);

        clock.destroy_for_testing();
        vdf_based_lottery::delete_ticket(ticket);
        ts::return_shared(game);
        ts.end();
    }

    #[test]
    #[expected_failure(abort_code = games::vdf_based_lottery::EInvalidVdfOutput)]
    fun test_invalid_vdf_output() {
        let user1 = @0x0;
        let user2 = @0x1;
        let user3 = @0x2;
        let user4 = @0x3;

        let mut ts = ts::begin(user1);
        let mut clock = clock::create_for_testing(ts.ctx());

        vdf_based_lottery::create(1000, 1000, &clock, ts.ctx());
        ts.next_tx(user1);
        let mut game = ts.take_shared<Game>();

        // User1 buys a ticket.
        ts.next_tx(user1);
        vdf_based_lottery::participate(&mut game, b"user1 randomness", &clock, ts.ctx());
        // User2 buys a ticket.
        ts.next_tx(user2);
        vdf_based_lottery::participate(&mut game, b"user2 randomness", &clock, ts.ctx());
        // User3 buys a ticket
        ts.next_tx(user3);
        vdf_based_lottery::participate(&mut game, b"user3 randomness", &clock, ts.ctx());
        // User4 buys a ticket
        ts.next_tx(user4);
        vdf_based_lottery::participate(&mut game, b"user4 randomness", &clock, ts.ctx());

        // Increment time to after submission phase has ended
        clock::increment_for_testing(&mut clock, 1000);

        // User3 completes by submitting output and proof of the VDF
        ts.next_tx(user3);
        vdf_based_lottery::complete(
            &mut game,
            x"",
            x"",
            &clock
        );

        clock.destroy_for_testing();

        ts::return_shared(game);
        ts.end();
    }
}
