module Sui::Geniteam {
    ...
    struct Player has key {
        ...
        // Collection of owned Farm
        owned_farm: Option<ChildRef<Farm>>,

        // Inventory of unassigned cosmetics 
        inventory: ChildRef<Collection::Collection>,
    }
    ...
    struct Farm has key, store {
        ...
        pet_monsters: ChildRef<Collection::Collection>,
    ...
    public fun create_monster(_player: &mut Player,
                                farm: &mut Farm,
                                pet_monsters: &mut Collection:: Collection,
                                monster_name: vector<u8>,
                                monster_img_index: u64,
                                breed: u8,
                                monster_affinity: u8,
    ...
    public fun update_farm_cosmetics(_player: &mut Player, farm: &mut Farm, _inventory: &mut Collection::Collection,
        ...
        if (cosmetic_slot_id == 0) {
            ...
            assert!(Option::is_none(&farm.applied_farm_cosmetic_0), ETOO_MANY_COSMETICS);

            // Store the cosmetic
            Option::fill(&mut farm.applied_farm_cosmetic_0, child_ref)
        } else {
            // Check that the slot has no items
            assert!(Option::is_none(&farm.applied_farm_cosmetic_1), ETOO_MANY_COSMETICS);
