...
module Sui::Validator {
    ...
    /// This can happen when a validator tries to withdraw stake but doesn't have sufficient balance
    /// to do so.
    ...
    public(script) fun destroy(
        ...
        let Validator { id, sui_address: _, name: _, ip_address: _, stake, pending_stake, pending_withdraw } = self;
        assert!(pending_withdraw == 0, EINCONSISTENT_STATE);
    ...
    /// Add stake to a pending validator. Since the validator is pending,
    /// we can add the stake directly to its stake.
    public(friend) fun add_stake_to_pending_validator(
        self: &mut Validator,
        new_stake: Coin<SUI>,
    ) {
        check_non_active_validator_invariants(self);
    ...
    /// Add stake to an active validator. Since it's active, we will need to
    /// add the stake to the pending_stake field, which will be processed at the
    /// end of epoch.
    public(friend) fun request_add_stake_to_active_validator(
        self: &mut Validator,
        new_stake: Coin<SUI>,
    ) {
    ...
    /// Withdraw stake from a pending validator. Since it's pending, we can
    /// withdraw coins immediately and send it back to the validator's address.
    public(friend) fun withdraw_stake_from_pending_validator(
        self: &mut Validator,
        withdraw_amount: u64,
        ctx: &mut TxContext,
    ) {
    ...
    public(friend) fun request_withdraw_stake_from_active_validator(
        self: &mut Validator,
        withdraw_amount: u64,
    ) {
        ...
        assert!(total_stake >= self.pending_withdraw, EINSUFFICIENT_STAKE);
    }
