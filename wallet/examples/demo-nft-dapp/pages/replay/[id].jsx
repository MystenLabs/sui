...
import { useSuiEvents } from '../../shared/events-store-context';
...
const ReplayPage = () => {
    ...
    const events = useSuiEvents();
    const boostEvents = useMemo(
        () =>
            (
                (objectId &&
                    events.filter(
                        (e) =>
                            e.event.moveEvent.type === capyBoostEvent &&
                            e.event.moveEvent.de.lottery_id ===
                                objectId.substring(2)
                    )) ||
                []
            ).map((e) => e.event.moveEvent.de),
        [events, objectId]
    );
    console.log(boostEvents);
    const existingCapys = useMemo(
        () =>
            ...
            //         }
            //     }, {}),
            boostEvents.reduce((acc, e) => {
                if (!acc[e.round.toNumber()]) {
                    acc[e.round.toNumber()] = [];
                }
                acc[e.round.toNumber()].push({
                    name: e.capy_name,
                    score: e.new_score.toNumber(),
                });
                return acc;
            }, {}),
        [(lottery, boostEvents)]
    );
    console.log(existingCapys);
    const totalCapys = lottery?.data?.fields?.capys?.length || 0;
